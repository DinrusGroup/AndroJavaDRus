 ...
 * @hide
 */
public class SeekBarVolumizer : OnSeekBarChangeListener, Handler.Callback {
    private static final String TAG = "SeekBarVolumizer";

    public interface Callback {
        void onSampleStarting(SeekBarVolumizer sbv);
        void onProgressChanged(SeekBar seekBar, int progress, bool fromTouch);
        void onMuted(bool muted, bool zenMuted);
    }
    ...
    private bool mAffectedByRingerMode;
    private bool mNotificationOrRing;
    ...
    private bool mAllowAlarms;
    private bool mAllowMedia;
    private bool mAllowRinger;
    ...
    private static bool isNotificationOrRing(int stream) {
        return stream == AudioManager.STREAM_RING || stream == AudioManager.STREAM_NOTIFICATION;
    }

    private static bool isAlarmsStream(int stream) {
        return stream == AudioManager.STREAM_ALARM;
    }

    private static bool isMediaStream(int stream) {
    ...
    protected void updateSeekBar() {
        final bool zenMuted = isZenMuted();
    ...
    private final class H : Handler {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == UPDATE_SLIDER) {
                if (mSeekBar != null) {
                    ...
                    final bool muted = ((Boolean)msg.obj).boolValue();
                    if (muted != mMuted) {
                        mMuted = muted;
                        if (mCallback != null) {
                            mCallback.onMuted(mMuted, isZenMuted());
        ...
        public void postUpdateSlider(int volume, int lastAudibleVolume, bool mute) {
    ...
    private void updateSlider() {
        if (mSeekBar != null && mAudioManager != null) {
            ...
            final bool mute = mAudioManager.isStreamMute(mStreamType);
            mUiHandler.postUpdateSlider(volume, lastAudibleVolume, mute);
        }
    }

    private final class Observer : ContentObserver {
    ...
    private final class Receiver : BroadcastReceiver {
        ...
        private void updateVolumeSlider(int streamType, int streamValue) {
            final bool streamMatch = mNotificationOrRing ? isNotificationOrRing(streamType)
                    : (streamType == mStreamType);
            if (mSeekBar != null && streamMatch && streamValue != -1) {
                final bool muted = mAudioManager.isStreamMute(mStreamType)
                        || streamValue == 0;
