 ...
 */
abstract class GenericInflater<T, P : GenericInflater.Parent> {
    ...
    private static class FactoryMerger<T> : Factory<T> {
     ...
     */
    public T inflate(@XmlRes int resource, P root, bool attachToRoot) {
     ...
     */
    protected bool onCreateCustomFromTag(XmlPullParser parser, T parent,
            final AttributeSet attrs) throws XmlPullParserException {
        return false;
    }
    
    protected P onMergeRoots(P givenRoot, bool attachToGivenRoot, P xmlRoot) {
        return xmlRoot;
    }
}
