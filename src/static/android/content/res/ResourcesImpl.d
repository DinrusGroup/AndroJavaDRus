...
public class ResourcesImpl {
    ...
    private static final bool DEBUG_LOAD = false;
    private static final bool DEBUG_CONFIG = false;

    static final String TAG_PRELOAD = TAG + ".preload";

    private static final bool TRACE_FOR_PRELOAD = false; // Do we still need it?
    private static final bool TRACE_FOR_MISS_PRELOAD = false; // Do we still need it?

    public static final bool TRACE_FOR_DETAILED_PRELOAD =
    ...
    private static bool sPreloaded;
    ...
    void getValue(@AnyRes int id, TypedValue outValue, bool resolveRefs)
    ...
    void getValueForDensity(@AnyRes int id, int density, TypedValue outValue,
            bool resolveRefs) throws NotFoundException {
    ...
    void getValue(String name, TypedValue outValue, bool resolveRefs)
            throws NotFoundException {
    ...
    Drawable loadDrawable(@NonNull Resources wrapper, @NonNull TypedValue value, int id,
        ...
        final bool useCache = density == 0 || value.density == mMetrics.densityDpi;
        ...
        try {
            ...
            Drawable dr;
            bool needsNewDrawableAfterCache = false;
            if (cs != null) {
            ...
            // cache.
            final bool canApplyTheme = dr != null && dr.canApplyTheme();
    ...
    private void cacheDrawable(TypedValue value, bool isColorDrawable, DrawableCache caches,
            Resources.Theme theme, bool usesTheme, long key, Drawable dr) {
    ...
    private bool verifyPreloadConfig(@Config int changingConfigurations,
        ...
        if (TRACE_FOR_DETAILED_PRELOAD) {
            if (((id >>> 24) == 0x1)) {
                ...
                if (name != null) {
                    ...
                    final bool isRoot = (android.os.Process.myUid() == 0);
    ...
    public class ThemeImpl {
        ...
        bool resolveAttribute(int resid, TypedValue outValue, bool resolveRefs) {
            synchronized (mKey) {
        ...
        String[] getTheme() {
            synchronized (mKey) {
                ...
                for (int i = 0, j = N - 1; i < themes.length; i += 2, --j) {
                    ...
                    final bool forced = mKey.mForce[j];
                    try {
        ...
        void rebase() {
            synchronized (mKey) {
                ...
                for (int i = 0; i < mKey.mCount; i++) {
                    ...
                    final bool force = mKey.mForce[i];
                    mAssets.applyStyleToTheme(mTheme, resId, force);
                }
            }
        }
