...
abstract class ThemedResourceCache<T> {
     ...
     */
    public void put(long key, @Nullable Theme theme, @NonNull T entry, bool usesTheme) {
        if (entry == null) {
            return;
        }
     ...
     */
    protected abstract bool shouldInvalidateEntry(@NonNull T entry, int configChanges);
     ...
     */
    @Nullable
    private LongSparseArray<WeakReference<T>> getThemedLocked(@Nullable Theme t, bool create) {
        if (t == null) {
     ...
     */
    @Nullable
    private LongSparseArray<WeakReference<T>> getUnthemedLocked(bool create) {
     ...
     */
    private bool prune(@Config int configChanges) {
        synchronized (this) {
            if (mThemedEntries != null) {
    ...
    private bool pruneEntriesLocked(@Nullable LongSparseArray<WeakReference<T>> entries,
    ...
    private bool pruneEntryLocked(@Nullable T entry, @Config int configChanges) {
