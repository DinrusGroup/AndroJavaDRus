 ...
 * </div>
 */
public class ClipDescription : Parcelable {
    /**
    ...
    /** @hide */
    public bool toShortString(StringBuilder b) {
        bool first = !toShortStringTypesOnly(b);
        if (mLabel != null) {
            if (!first) {
                b.append(' ');
            }
    ...
    /** @hide */
    public bool toShortStringTypesOnly(StringBuilder b) {
        bool first = true;
        final int size = mMimeTypes.size();
        for (int i=0; i<size; i++) {
            if (!first) {
