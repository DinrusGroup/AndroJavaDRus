...
public class UndoManager {
     ...
     */
    public bool hasOperation(UndoOwner owner) {
        if (mWorking == null) {
            throw new IllegalStateException("Must be called during an update");
        }
     ...
     */
    public UndoOperation!(T) getLastOperation(int mergeMode) {
     ...
     */
    public UndoOperation!(T) getLastOperation(UndoOwner owner, int mergeMode) {
     ...
     * or {@link #MERGE_MODE_ANY}.
     */
    public <T : UndoOperation> T getLastOperation(Class<T> clazz, UndoOwner owner,
            int mergeMode) {
        if (mWorking == null) {
            throw new IllegalStateException("Must be called during an update");
        }
     ...
     * or {@link #MERGE_MODE_ANY}.
     */
    public void addOperation(UndoOperation!(T) op, int mergeMode) {
        if (mWorking == null) {
            throw new IllegalStateException("Must be called during an update");
        }
     ...
     */
    public bool uncommitState(int commitId, UndoOwner owner) {
    ...
    bool matchOwners(UndoState state, UndoOwner[] owners) {
        if (owners == null) {
            return true;
        }
    ...
    final static class UndoState {
        ...
        private final ArrayList<UndoOperation!(T)> mOperations = new ArrayList<UndoOperation!(T)>();
        private ArrayList<UndoOperation!(T)> mRecent;
        private CharSequence mLabel;
        private bool mCanMerge = true;
        ...
        bool hasOperation(UndoOwner owner) {
            final int N = mOperations.size();
            if (owner == null) {
                return N != 0;
            }
        ...
        void addOperation(UndoOperation!(T) op) {
            if (mOperations.contains(op)) {
                throw new IllegalStateException("Already holds " + op);
            }
            mOperations.add(op);
            if (mRecent == null) {
                mRecent = new ArrayList<UndoOperation!(T)>();
                mRecent.add(op);
            }
            op.mOwner.mOpCount++;
        }

        <T : UndoOperation> T getLastOperation(Class<T> clazz, UndoOwner owner) {
            final int N = mOperations.size();
            ...
            for (int i=N-1; i>=0; i--) {
                UndoOperation!(T) op = mOperations.get(i);
        ...
        bool matchOwner(UndoOwner owner) {
