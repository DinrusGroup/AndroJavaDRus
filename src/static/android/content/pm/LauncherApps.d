...
public class LauncherApps {
     ...
     */
    public bool isPackageEnabled(String packageName, UserHandle user) {
        logErrorForInvalidProfileAccess(user);
        try {
     ...
     * {@code PackageManager#setPackagesSuspended(String[], bool, PersistableBundle,
     ...
     */
    public bool isActivityEnabled(ComponentName component, UserHandle user) {
        logErrorForInvalidProfileAccess(user);
        try {
    ...
    public void registerCallback(Callback callback, Handler handler) {
        synchronized (this) {
            if (callback != null && findCallbackLocked(callback) < 0) {
                bool addedFirstCallback = mCallbacks.size() == 0;
    ...
    private IOnAppsChangedListener.Stub mAppsChangedListener = new IOnAppsChangedListener.Stub() {
        ...
        override
        public void onPackagesAvailable(UserHandle user, String[] packageNames, bool replacing)
                throws RemoteException {
            if (DEBUG) {
        ...
        override
        public void onPackagesUnavailable(UserHandle user, String[] packageNames, bool replacing)
                throws RemoteException {
            if (DEBUG) {
    ...
    private static class CallbackMessageHandler : Handler {
     ...
     * @see #getPinItemRequest(Intent)
     */
    public static final class PinItemRequest : Parcelable {
         ...
         */
        public bool accept(@Nullable Bundle options) {
            try {
