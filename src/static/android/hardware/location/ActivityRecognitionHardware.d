...
/**
 * A class that : an {@link IActivityRecognitionHardware} backed up by the Activity
 * Recognition HAL.
 *
 * @hide
 */
public class ActivityRecognitionHardware : IActivityRecognitionHardware.Stub {
    ...
    override
    public bool isActivitySupported(String activity) {
        checkPermissions();
    ...
    override
    public bool registerSink(IActivityRecognitionHardwareSink sink) {
        checkPermissions();
        return mSinks.register(sink);
    }

    override
    public bool unregisterSink(IActivityRecognitionHardwareSink sink) {
        checkPermissions();
        return mSinks.unregister(sink);
    }

    override
    public bool enableActivityEvent(String activity, int eventType, long reportLatencyNs) {
        checkPermissions();
    ...
    override
    public bool disableActivityEvent(String activity, int eventType) {
        checkPermissions();
    ...
    private class SinkList : RemoteCallbackList<IActivityRecognitionHardwareSink> {
        override
    ...
    private static native bool nativeIsSupported();
