...
public class CaptureCollector {
    ...
    private class CaptureHolder : Comparable<CaptureHolder>{
        private final RequestHolder mRequest;
        private final LegacyRequest mLegacy;
        public final bool needsJpeg;
        public final bool needsPreview;

        private long mTimestamp = 0;
        private int mReceivedFlags = 0;
        private bool mHasStarted = false;
        private bool mFailedJpeg = false;
        private bool mFailedPreview = false;
        private bool mCompleted = false;
        private bool mPreviewCompleted = false;
     ...
     */
    public bool queueRequest(RequestHolder holder, LegacyRequest legacy, long timeout,
                                TimeUnit unit)
     ...
     * @throws InterruptedException if this thread is interrupted.
     */
    public bool waitForEmpty(long timeout, TimeUnit unit) throws InterruptedException {
        long nanos = unit.toNanos(timeout);
        final ReentrantLock lock = this.mLock;
        lock.lock();
        try {
     ...
     * @throws InterruptedException if this thread is interrupted.
     */
    public bool waitForPreviewsEmpty(long timeout, TimeUnit unit) throws InterruptedException {
        long nanos = unit.toNanos(timeout);
        final ReentrantLock lock = this.mLock;
        lock.lock();
        try {
     ...
     * @throws InterruptedException if this thread is interrupted.
     */
    public bool waitForRequestCompleted(RequestHolder holder, long timeout, TimeUnit unit,
    ...
    private bool removeRequestIfCompleted(RequestHolder holder, MutableLong timestamp) {
        int i = 0;
     ...
     */
    public bool hasPendingPreviewCaptures() {
        final ReentrantLock lock = this.mLock;
        lock.lock();
        try {
