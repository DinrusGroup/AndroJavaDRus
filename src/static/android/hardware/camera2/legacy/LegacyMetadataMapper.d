...
public class LegacyMetadataMapper {
     ...
     * being set to true.
     */
    static final bool LIE_ABOUT_AE_STATE = false;
    static final bool LIE_ABOUT_AE_MAX_REGIONS = false;
    static final bool LIE_ABOUT_AF = false;
    static final bool LIE_ABOUT_AF_MAX_REGIONS = false;
    static final bool LIE_ABOUT_AWB_STATE = false;
    static final bool LIE_ABOUT_AWB = false;
    ...
    private static void mapControlAe(CameraMetadataNative m, Camera.Parameters p) {
         ...
         */
        {
            bool aeLockAvailable = p.isAutoExposureLockSupported();
    ...
    private static void mapControlAwb(CameraMetadataNative m, Camera.Parameters p) {
...

        {
             ...
             */
            {
                bool awbLockAvailable = p.isAutoWhiteBalanceLockSupported();
...
//                    CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP                 ,
            ...
            List<Key!(T)> characteristicsKeys = new ArrayList<>(Arrays.asList(availableKeys));
            ...
            m.set(REQUEST_AVAILABLE_CHARACTERISTICS_KEYS,
                    getTagsForKeys(characteristicsKeys.toArray(new Key!(T)[0])));
        }
         ...
         */
        {
            CaptureRequest.Key!(T) defaultAvailableKeys[] = new CaptureRequest.Key!(T)[] {
            ...
            ArrayList<CaptureRequest.Key!(T)> availableKeys =
                    new ArrayList<CaptureRequest.Key!(T)>(Arrays.asList(defaultAvailableKeys));
            ...
            CaptureRequest.Key!(T) availableRequestKeys[] =
                    new CaptureRequest.Key!(T)[availableKeys.size()];
         ...
         */
        {
            CaptureResult.Key!(T) defaultAvailableKeys[] = new CaptureResult.Key!(T)[] {
...
//                    CaptureResult.STATISTICS_FACES                                 ,
            ...
            List<CaptureResult.Key!(T)> availableKeys =
                    new ArrayList<CaptureResult.Key!(T)>(Arrays.asList(defaultAvailableKeys));
            ...
            CaptureResult.Key!(T) availableResultKeys[] =
                    new CaptureResult.Key!(T)[availableKeys.size()];
        ...
        m.set(REQUEST_PIPELINE_MAX_DEPTH,
                cast(byte)(REQUEST_PIPELINE_MAX_DEPTH_HAL1 + REQUEST_PIPELINE_MAX_DEPTH_OURS));
    }
    ...
    private static int[] getTagsForKeys(Key!(T)[] keys) {
    ...
    private static int[] getTagsForKeys(CaptureRequest.Key!(T)[] keys) {
        int[] tags = new int[keys.length];
    ...
    private static int[] getTagsForKeys(CaptureResult.Key!(T)[] keys) {
        int[] tags = new int[keys.length];
