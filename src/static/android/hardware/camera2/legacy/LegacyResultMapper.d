...
public class LegacyResultMapper {
    ...
    private static CameraMetadataNative convertResultMetadata(LegacyRequest legacyRequest) {
        ...
        // jpeg.quality
        result.set(JPEG_QUALITY, cast(byte) params.getJpegQuality());

        // jpeg.thumbnailQuality
        result.set(JPEG_THUMBNAIL_QUALITY, cast(byte) params.getJpegThumbnailQuality());
    ...
    private static void mapAe(CameraMetadataNative m,
        ...
        // control.aeLock
        {
            bool lock = p.isAutoExposureLockSupported() ? p.getAutoExposureLock() : false;
    ...
    private static void mapAwb(CameraMetadataNative m, Camera.Parameters p) {
        // control.awbLock
        {
            bool lock = p.isAutoWhiteBalanceLockSupported() ?
