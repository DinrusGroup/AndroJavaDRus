 ...
 * </p>
 */
public final class DngCreator : AutoCloseable {
    ...
    private static void yuvToRgb(byte[] yuvData, int outOffset, /*out*/byte[] rgbOut) {
        ...
        // clamp to [0,255]
        rgbOut[outOffset] = cast(byte) Math.max(0, Math.min(COLOR_MAX, r));
        rgbOut[outOffset + 1] = cast(byte) Math.max(0, Math.min(COLOR_MAX, g));
        rgbOut[outOffset + 2] = cast(byte) Math.max(0, Math.min(COLOR_MAX, b));
    }
    ...
    private static void colorToRgb(int color, int outOffset, /*out*/byte[] rgbOut) {
        rgbOut[outOffset] = cast(byte) Color.red(color);
        rgbOut[outOffset + 1] = cast(byte) Color.green(color);
        rgbOut[outOffset + 2] = cast(byte) Color.blue(color);
        // Discards Alpha
    }
    ...
    private synchronized native void nativeWriteImage(OutputStream out, int width, int height,
                                                      ...
                                                      int pixStride, long offset, bool isDirect)
                                                      throws IOException;
