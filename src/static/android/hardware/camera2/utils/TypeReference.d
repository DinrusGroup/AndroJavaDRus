 ...
 *
 *      // using named classes
 *      class IntTypeReference : TypeReference&lt;Integer> {...}
...
public abstract class TypeReference<T> {
    ...
    protected TypeReference() {
         ...
         *
         *    class GenericListToken<T> : TypeReference<List<T>>
         *
    ...
    private static class SpecializedTypeReference<T> : TypeReference<T> {
    ...
    private static class SpecializedBaseTypeReference : TypeReference {
     ...
     */
    public static TypeReference!(T) createSpecializedTypeReference(Type type) {
     ...
     * returned. For example: {@code <X : Foo>} => {@code Foo}.
     * </ul>
     *
    ...
    private static final Class!(T) getRawType(Type type) {
        if (type == null) {
    ...
    private static final Class!(T) getRawType(Type[] types) {
        if (types == null) {
            return null;
        }

        for (Type type : types) {
            Class!(T) klass = getRawType(type);
            if (klass !=  null) {
                return klass;
            }
        }
    ...
    private static final Class!(T) getArrayClass(Class!(T) componentType) {
    ...
    public bool equals(Object o) {
        ...
        return o instanceof TypeReference!(T) && mType.equals(((TypeReference!(T))o).mType);
    }
     ...
     */
    public static bool containsTypeVariable(Type type) {
        if (type == null) {
            // Trivially false
            return false;
             ...
             *
             * This also covers the case of class Foo<T> : ... / : ...
             ...
             *
             *       Foo!(T) --> unbounded; trivially no type variables
             *       Foo<? super T> --> lower bound; does T have a type variable?
             *       Foo<? : T> --> upper bound; does T have a type variable?
             */
    ...
    private static void toString(Type type, StringBuilder out) {
        if (type == null) {
            ...
            // T
            out.append(((TypeVariable!(T))type).getName());
        } else if (type instanceof Class!(T)) {
            Class!(T) klass = (Class!(T))type;
            ...
            out.append(((Class!(T))p.getRawType()).getName());
     ...
     */
    private static bool containsTypeVariable(Type[] typeArray) {
        if (typeArray == null) {
            return false;
        }
