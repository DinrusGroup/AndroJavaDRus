...
public abstract class CameraMetadata<TKey> {
    ...
    /**
     * Return a list of all the Key!(T) that are declared as a field inside of the class
     * {@code type}.
     *
     * <p>
    ...
    <TKey> ArrayList<TKey> getKeys(
             Class!(T) type, Class<TKey> keyClass,
        ...
        if (vendorKeys != null) {
            for (TKey k : vendorKeys) {
                String keyName;
                long vendorId;
                if (k instanceof CaptureRequest.Key!(T)) {
                    keyName = ((CaptureRequest.Key!(T)) k).getName();
                    vendorId = ((CaptureRequest.Key!(T)) k).getVendorId();
                } else if (k instanceof CaptureResult.Key!(T)) {
                    keyName = ((CaptureResult.Key!(T)) k).getName();
                    vendorId = ((CaptureResult.Key!(T)) k).getVendorId();
                } else if (k instanceof CameraCharacteristics.Key!(T)) {
                    keyName = ((CameraCharacteristics.Key!(T)) k).getName();
                    vendorId = ((CameraCharacteristics.Key!(T)) k).getVendorId();
                } else {
                    continue;
                }
    ...
    private static <TKey> bool shouldKeyBeAdded(TKey key, Field field, int[] filterTags) {
        if (key == null) {
