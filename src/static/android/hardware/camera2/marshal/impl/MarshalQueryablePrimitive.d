 ...
 *
 * <p>Each managed type <!--(other than bool)--> must correspond 1:1 to the native type
 * (e.g. a byte will not map to a {@link CameraMetadataNative#TYPE_INT32 TYPE_INT32} or vice versa)
 * for marshaling.</p>
 */
public final class MarshalQueryablePrimitive<T> : MarshalQueryable<T> {

    private class MarshalerPrimitive : Marshaler<T> {
    ...
    public bool isTypeMappingSupported(TypeReference<T> managedType, int nativeType) {
        if (managedType.getType() instanceof Class!(T)) {
            Class!(T) klass = (Class!(T))managedType.getType();
