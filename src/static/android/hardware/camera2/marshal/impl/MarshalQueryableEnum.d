 ...
 */
public class MarshalQueryableEnum<T : Enum<T>> : MarshalQueryable<T> {
    ...
    private class MarshalerEnum : Marshaler<T> {
        ...
        public void marshal(T value, ByteBuffer buffer) {
            ...
            if (mNativeType == TYPE_INT32) {
                ...
                buffer.put(cast(byte)enumValue);
            } else {
    ...
    override
    public bool isTypeMappingSupported(TypeReference<T> managedType, int nativeType) {
        if (nativeType == TYPE_INT32 || nativeType == TYPE_BYTE) {
            if (managedType.getType() instanceof Class!(T)) {
                Class!(T) typeClass = (Class!(T))managedType.getType();
    ...
    private static final HashMap<Class<? : Enum>, int[]> sEnumValues =
            new HashMap<Class<? : Enum>, int[]>();
     ...
     */
    public static <T : Enum<T>> void registerEnumValues(Class<T> enumType, int[] values) {
     ...
     */
    private static <T : Enum<T>> int getEnumValue(T enumValue) {
        int[] values;
     ...
     */
    private static <T : Enum<T>> T getEnumFromValue(Class<T> enumType, int value) {
        int ordinal;
