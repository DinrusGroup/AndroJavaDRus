...
public final class HdmiRecordSources {
    ...
    public static abstract class RecordSource {
        ...
        /* package */ final int getDataSize(bool includeType)  {
            return includeType ? mExtraDataSize + 1 : mExtraDataSize;
        }

        /* package */ final int toByteArray(bool includeType, byte[] data, int index) {
            if (includeType) {
                ...
                data[index++] = cast(byte) mSourceType;
            }
     ...
     */
    @SystemApi
    public static final class OwnSource : RecordSource {
     ...
     * @hide
     */
    public static final class AribData : DigitalServiceIdentification {
     ...
     * @hide
     */
    public static final class AtscData : DigitalServiceIdentification {
        /** The transport_stream_ID of the transport stream carrying the required service */
        private final int mTransportStreamId;
     ...
     * @hide
     */
    public static final class DvbData : DigitalServiceIdentification {
        /** The transport_stream_ID of the transport stream carrying the required service */
        private final int mTransportStreamId;
        /** The service_ID of the required service */
        private final int mServiceId;
    ...
    private static final class ChannelIdentifier {
        ...
        private int toByteArray(byte[] data, int index) {
            ...
            data[index] = cast(byte) (((mChannelNumberFormat << 2) | (mMajorChannelNumber >>> 8) & 0x3));
            data[index + 1] = cast(byte) (mMajorChannelNumber & 0xFF);
            // Minor number uses the next 16 bits.
            shortToByteArray(cast(short) mMinorChannelNumber, data, index + 2);
            return 4;
        }
    }
     ...
     * @hide
     */
    public static final class DigitalChannelData : DigitalServiceIdentification {
     ...
     */
    @SystemApi
    public static final class DigitalServiceSource : RecordSource {
        ...
        override
        int extraParamToByteArray(byte[] data, int index) {
            data[index] = cast(byte) ((mIdentificationMethod << 7) | (mBroadcastSystem & 0x7F));
     ...
     */
    @SystemApi
    public static final class AnalogueServiceSource : RecordSource {
        ...
        /* package */ int extraParamToByteArray(byte[] data, int index) {
            ...
            data[index] = cast(byte) mBroadcastType;
            // [Analogue Frequency] - 2 bytes
            shortToByteArray(cast(short) mFrequency, data, index + 1);
            // [Broadcast System] - 1 byte
            data[index + 3] = cast(byte) mBroadcastSystem;
            return EXTRA_DATA_SIZE;
        }
    }
     ...
     */
    @SystemApi
    public static final class ExternalPlugData : RecordSource {
        ...
        override
        int extraParamToByteArray(byte[] data, int index) {
            data[index] = cast(byte) mPlugNumber;
            return EXTRA_DATA_SIZE;
        }
    }
     ...
     */
    @SystemApi
    public static final class ExternalPhysicalAddress : RecordSource {
        ...
        override
        int extraParamToByteArray(byte[] data, int index) {
            shortToByteArray(cast(short) mPhysicalAddress, data, index);
            return EXTRA_DATA_SIZE;
        }
    }
    ...
    private static int threeFieldsToSixBytes(int first, int second, int third, byte[] data,
            int index) {
        shortToByteArray(cast(short) first, data, index);
        shortToByteArray(cast(short) second, data, index + 2);
        shortToByteArray(cast(short) third, data, index + 4);
        return 6;
    }

    private static int shortToByteArray(short value, byte[] byteArray, int index) {
        byteArray[index] = cast(byte) ((value >>> 8) & 0xFF);
        byteArray[index + 1] = cast(byte) (value & 0xFF);
        return 2;
    }
     ...
     */
    @SystemApi
    public static bool checkRecordSource(byte[] recordSource) {
