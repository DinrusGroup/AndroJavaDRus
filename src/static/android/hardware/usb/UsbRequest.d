...
public class UsbRequest {
     ...
     */
    public bool initialize(UsbDeviceConnection connection, UsbEndpoint endpoint) {
        mEndpoint = endpoint;
        mConnection = Preconditions.checkNotNull(connection, "connection");

        bool wasInitialized = native_init(connection, endpoint.getAddress(),
     ...
     */
    @Deprecated
    public bool queue(ByteBuffer buffer, int length) {
        bool out = (mEndpoint.getDirection() == UsbConstants.USB_DIR_OUT);
        bool result;
     ...
     * @return true if the queueing operation succeeded
     */
    public bool queue(@Nullable ByteBuffer buffer) {
        ...
        bool isSend = (mEndpoint.getDirection() == UsbConstants.USB_DIR_OUT);
        bool wasQueued;
    ...
    /* package */ void dequeue(bool useBufferOverflowInsteadOfIllegalArg) {
        bool isSend = (mEndpoint.getDirection() == UsbConstants.USB_DIR_OUT);
    ...
    private native bool native_init(UsbDeviceConnection connection, int ep_address,
            int ep_attributes, int ep_max_packet_size, int ep_interval);
    private native void native_close();
    private native bool native_queue(ByteBuffer buffer, int offset, int length);
    private native bool native_queue_array(byte[] buffer, int length, bool out);
    private native int native_dequeue_array(byte[] buffer, int length, bool out);
    private native bool native_queue_direct(ByteBuffer buffer, int length, bool out);
    private native int native_dequeue_direct();
    private native bool native_cancel();
}
