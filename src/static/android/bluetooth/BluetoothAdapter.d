...
public final class BluetoothAdapter {
     ...
     * @hide
     */
    public bool disable(bool persist) {
    ...
    public bool setBluetoothClass(BluetoothClass bluetoothClass) {
        if (getState() != STATE_ON) {
            return false;
        }
        try {
     ...
     * @hide
     */
    public bool setScanMode(@ScanMode int mode, int duration) {
        if (getState() != STATE_ON) {
            return false;
        }
        try {
    ...
    /** @hide */
    public bool setScanMode(int mode) {
        if (getState() != STATE_ON) {
            return false;
        }
     ...
     * @hide
     */
    public bool isHardwareTrackingFiltersAvailable() {
        if (!getLeAccess()) {
            return false;
        }
        try {
     ...
     * @hide
     */
    public BluetoothServerSocket listenUsingRfcommOn(int channel, bool mitm,
            bool min16DigitPin) throws IOException {
    ...
    private BluetoothServerSocket createNewRfcommSocketAndRecord(String name, UUID uuid,
            bool auth, bool encrypt) throws IOException {
        BluetoothServerSocket socket;
     ...
     * @hide
     */
    public BluetoothServerSocket listenUsingL2capOn(int port, bool mitm, bool min16DigitPin)
            throws IOException {
        BluetoothServerSocket socket =
     ...
     * @return true on success, false on error
     */
    public bool getProfileProxy(Context context, BluetoothProfile.ServiceListener listener,
            int profile) {
    ...
    public bool enableNoAutoConnect() {
        if (isEnabled()) {
            if (DBG) {
     ...
     * @hide
     */
    public bool changeApplicationBluetoothState(bool on,
    ...
    public interface BluetoothStateChangeCallback {
         ...
         * @hide
         */
        void onBluetoothStateChange(bool on);
    }

    /**
     * @hide
     */
    public class StateChangeCallbackWrapper : IBluetoothStateChangeCallback.Stub {
        ...
        override
        public void onBluetoothStateChange(bool on) {
    ...
    @Deprecated
    @RequiresPermission(Manifest.permission.BLUETOOTH_ADMIN)
    public bool startLeScan(LeScanCallback callback) {
    ...
    @Deprecated
    @RequiresPermission(Manifest.permission.BLUETOOTH_ADMIN)
    public bool startLeScan(final UUID[] serviceUuids, final LeScanCallback callback) {
        if (DBG) {
