...
{    
     ...
     */
    public bool loadJetFile(String path) {
     ...
     * @return true if loading the .jet file was successful, false if loading failed.
     */
    public bool loadJetFile(AssetFileDescriptor afd) {
     ...
     */
    public bool queueJetSegment(int segmentNum, int libNum, int repeatCount,
     ...
     * @param transpose the amount of pitch transposition. Set to 0 for normal playback. 
     *    Range is -12 to +12.
     * @param muteArray an array of bools to specify which MIDI tracks will be muted during
     ...
     *    parameters are invalid.
     */
    public bool queueJetSegmentMuteArray(int segmentNum, int libNum, int repeatCount,
            int transpose, bool[] muteArray, byte userID) {
     ...
     */
    public bool setMuteFlags(int muteFlags, bool sync) {
     ...
     * @param muteArray an array of bools to specify which MIDI tracks are muted. The value at
     ...
     * @return true if the mute flags were successfully updated, false otherwise.
     */
    public bool setMuteArray(bool[] muteArray, bool sync) {
        if(muteArray.length != JetPlayer.getMaxTracks())
            return false;
     ...
     */
    public bool setMuteFlag(int trackId, bool muteFlag, bool sync) {
     ...
     */
    public bool triggerClip(int clipId) {
    ...
    //------------------------
    private class NativeEventHandler : Handler
    {
            ...
            case JET_EVENT:
                if (listener != null) {
                    ...
                    mJetEventListener.onJetEvent(
                            mJet,
                            cast(short)((msg.arg1 & JET_EVENT_SEG_MASK)   >> JET_EVENT_SEG_SHIFT),
                            cast(byte) ((msg.arg1 & JET_EVENT_TRACK_MASK) >> JET_EVENT_TRACK_SHIFT),
                            // JETCreator channel numbers start at 1, but the index starts at 0
                            // in the .jet files
                            cast(byte)(((msg.arg1 & JET_EVENT_CHAN_MASK)  >> JET_EVENT_CHAN_SHIFT) + 1),
                            cast(byte) ((msg.arg1 & JET_EVENT_CTRL_MASK)  >> JET_EVENT_CTRL_SHIFT),
                            cast(byte)  (msg.arg1 & JET_EVENT_VAL_MASK) );
                }
                return;
    ...
    // Native methods
    //------------------------
    private native final bool native_setup(Object Jet_this,
                int maxTracks, int trackBufferSize);
    private native final void    native_finalize();
    private native final void    native_release();
    private native final bool native_loadJetFromFile(String pathToJetFile);
    private native final bool native_loadJetFromFileD(FileDescriptor fd, long offset, long len);
    private native final bool native_closeJetFile();
    private native final bool native_playJet();
    private native final bool native_pauseJet();
    private native final bool native_queueJetSegment(int segmentNum, int libNum,
            int repeatCount, int transpose, int muteFlags, byte userID);
    private native final bool native_queueJetSegmentMuteArray(int segmentNum, int libNum, 
            int repeatCount, int transpose, bool[] muteArray, byte userID);
    private native final bool native_setMuteFlags(int muteFlags, bool sync);
    private native final bool native_setMuteArray(bool[]muteArray, bool sync);
    private native final bool native_setMuteFlag(int trackId, bool muteFlag, bool sync);
    private native final bool native_triggerClip(int clipId);
    private native final bool native_clearQueue();
