 ...
 *
 * Each instance object is connected to one corresponding updatable object which : the
...
public interface ViewGroupProvider {
    ...
    bool onTouchEvent_impl(MotionEvent ev);
    bool onTrackballEvent_impl(MotionEvent ev);
    void onFinishInflate_impl();
    void setEnabled_impl(bool enabled);
    void onVisibilityAggregated_impl(bool isVisible);
    void onLayout_impl(bool changed, int left, int top, int right, int bottom);
    ...
    bool dispatchTouchEvent_impl(MotionEvent ev);

    // ViewGroup methods
    bool checkLayoutParams_impl(LayoutParams p);
    ...
    bool shouldDelayChildPressedState_impl();
