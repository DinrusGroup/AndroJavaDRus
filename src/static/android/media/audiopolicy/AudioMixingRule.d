...
public class AudioMixingRule {
    ...
    bool isAffectingUsage(int usage) {
    ...
    private static bool areCriteriaEquivalent(ArrayList<AudioMixMatchCriterion> cr1,
    ...
    private static bool isValidSystemApiRule(int rule) {
    ...
    private static bool isValidAttributesSystemApiRule(int rule) {
        // API rules only expose the RULE_MATCH_* rules
        switch (rule) {
            case RULE_MATCH_ATTRIBUTE_USAGE:
            case RULE_MATCH_ATTRIBUTE_CAPTURE_PRESET:
    ...
    private static bool isValidRule(int rule) {
    ...
    private static bool isPlayerRule(int rule) {
        final int match_rule = rule & ~RULE_EXCLUSION_MASK;
        switch (match_rule) {
        case RULE_MATCH_ATTRIBUTE_USAGE:
        case RULE_MATCH_UID:
    ...
    private static bool isAudioAttributeRule(int match_rule) {
        switch(match_rule) {
            case RULE_MATCH_ATTRIBUTE_USAGE:
            case RULE_MATCH_ATTRIBUTE_CAPTURE_PRESET:
                return true;
