...
public final class StructIcmpHdr {
     ...
     * +-+-+-+-+-
     */
    public static StructIcmpHdr IcmpEchoHdr(bool ipv4, int seq) {
        StructIcmpHdr hdr = new StructIcmpHdr();
        hdr.packet[0] = ipv4 ? cast(byte) ICMP_ECHO : cast(byte) ICMP6_ECHO_REQUEST;
        // packet[1]: Code is always zero.
        // packet[2,3]: Checksum is computed by kernel.
        // packet[4,5]: ID (= port) inserted by kernel.
        hdr.packet[6] = cast(byte) (seq >> 8);
        hdr.packet[7] = cast(byte) seq;
        return hdr;
    }
