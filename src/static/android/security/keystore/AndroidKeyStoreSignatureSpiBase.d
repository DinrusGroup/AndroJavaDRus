 ...
 * @hide
 */
abstract class AndroidKeyStoreSignatureSpiBase : SignatureSpi
    ...
    override
    protected final bool engineVerify(byte[] signature) throws SignatureException {
        if (mCachedException != null) {
            throw new SignatureException(mCachedException);
        }
    ...
    override
    protected final bool engineVerify(byte[] sigBytes, int offset, int len)
            throws SignatureException {
     ...
     */
    protected final bool isSigning() {
        return mSigning;
    }
