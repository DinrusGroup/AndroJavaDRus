...
public class SntpClient {
    ...
    private static class InvalidServerReplyException : Exception {
     ...
     */
    public bool requestTime(String host, int timeout, Network network) {
    ...
    public bool requestTime(InetAddress address, int port, int timeout, Network network) {
        ...
        try {
            ...
            // extract the results
            final byte leap = cast(byte) ((buffer[0] >> 6) & 0x3);
            final byte mode = cast(byte) (buffer[0] & 0x7);
    ...
    @Deprecated
    public bool requestTime(String host, int timeout) {
    ...
    private void writeTimeStamp(byte[] buffer, int offset, long time) {
        // Special case: zero means zero.
        if (time == 0) {
            Arrays.fill(buffer, offset, offset + 8, cast(byte) 0x00);
            return;
        }
        ...
        buffer[offset++] = cast(byte)(seconds >> 24);
        buffer[offset++] = cast(byte)(seconds >> 16);
        buffer[offset++] = cast(byte)(seconds >> 8);
        buffer[offset++] = cast(byte)(seconds >> 0);

        long fraction = milliseconds * 0x100000000L / 1000L;
        // write fraction in big endian format
        buffer[offset++] = cast(byte)(fraction >> 24);
        buffer[offset++] = cast(byte)(fraction >> 16);
        buffer[offset++] = cast(byte)(fraction >> 8);
        // low order bits should be random data
        buffer[offset++] = cast(byte)(Math.random() * 255.0);
    }
}
