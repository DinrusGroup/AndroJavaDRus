 ...
 * {@see NsdManager}
 */
public final class NsdServiceInfo : Parcelable {
    ...
    public void setTxtRecords(@NonNull String rawRecords) {
        ...
        while (pos < txtRecordsRawBytes.length) {
            ...
            try {
                ...
                // If '=' was not found we have a bool record
                if (key == null) {
    ...
    public @NonNull byte[] getTxtRecord() {
        ...
        for (Map.Entry<String, byte[]> entry : mTxtRecord.entrySet()) {
            ...
            txtRecord[ptr++] = cast(byte) (key.length() + (value == null ? 0 : value.length) + 1);
