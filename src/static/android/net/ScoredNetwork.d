 ...
 */
@SystemApi
public class ScoredNetwork : Parcelable {
  ...
  /**
    ...
    /**
     * A bool value that indicates whether or not the network is believed to be metered.
     *
     ...
     *     indicates to the system not to request scores for this network in the future, although
     *     the scorer may choose to issue an out-of-band update at any time.
     * @param meteredHint A bool value indicating whether or not the network is believed to be
     *     metered.
     */
    public ScoredNetwork(NetworkKey networkKey, RssiCurve rssiCurve, bool meteredHint) {
    ...
    public void writeToParcel(Parcel out, int flags) {
        ...
        if (rssiCurve != null) {
            out.writeByte(cast(byte) 1);
            rssiCurve.writeToParcel(out, flags);
        } else {
            out.writeByte(cast(byte) 0);
        }
        out.writeByte(cast(byte) (meteredHint ? 1 : 0));
    ...
    private bool bundleEquals(Bundle bundle1, Bundle bundle2) {
        if (bundle1 == bundle2) {
            return true;
        }
