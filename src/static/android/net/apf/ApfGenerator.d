...
public class ApfGenerator {
     ...
     */
    public static class IllegalInstructionException : Exception {
    ...
    private class Instruction {
        ...
        private bool mHasImm;
        private byte mImmSize;
        private bool mImmSigned;
        private int mImm;
        ...
        Instruction(Opcodes opcode, Register register) {
            mOpcode = cast(byte)opcode.value;
            mRegister = cast(byte)register.value;
        }
        ...
        void setImm(int imm, bool signed) {
            mHasImm = true;
            mImm = imm;
            mImmSigned = signed;
         ...
         */
        bool shrink() throws IllegalInstructionException {
            if (mTargetLabel == null) {
                return false;
            }
            int oldSize = size();
        ...
        private byte generateInstructionByte() {
            ...
            return cast(byte)((mOpcode << 3) | (sizeField << 1) | mRegister);
        }
        ...
        private int writeValue(int value, byte[] bytecode, int writingOffset) {
            for (int i = generatedImmSize() - 1; i >= 0; i--) {
                bytecode[writingOffset++] = cast(byte)((value >> (i * 8)) & 255);
            }
        ...
        private byte calculateImmSize(int imm, bool signed) {
            if (imm == 0) {
                return 0;
            }
