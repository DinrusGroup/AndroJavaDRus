...
public class IpUtils {
    ...
    private static byte ipversion(ByteBuffer buf, int headerOffset) {
        return cast(byte) ((buf.get(headerOffset) & cast(byte) 0xf0) >> 4);
   }

    public static short ipChecksum(ByteBuffer buf, int headerOffset) {
        byte ihl = cast(byte) (buf.get(headerOffset) & 0x0f);
        return cast(short) checksum(buf, 0, headerOffset, headerOffset + ihl * 4);
    }
    ...
    private static short transportChecksum(ByteBuffer buf, int protocol,
        ...
        if (protocol == IPPROTO_UDP && sum == 0) {
            sum = cast(short) 0xffff;
        }
        return cast(short) sum;
    }
    ...
    public static bool isValidUdpOrTcpPort(int port) {
