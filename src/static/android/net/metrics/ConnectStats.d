...
public class ConnectStats {
    ...
    bool addEvent(int errno, int latencyMs, String ipAddr) {
        eventCount++;
        if (isSuccess(errno)) {
    ...
    private static bool isSuccess(int errno) {
        return (errno == 0) || isNonBlocking(errno);
    }

    static bool isNonBlocking(int errno) {
    ...
    private static bool isIPv6(String ipAddr) {
