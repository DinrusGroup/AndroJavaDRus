 ...
 */
public class WifiEnterpriseConfig : Parcelable {
     ...
     *             to this value.
     */
    private void copyFrom(WifiEnterpriseConfig source, bool ignoreMaskedPassword, String mask) {
    ...
    public interface SupplicantSaver {
         ...
         */
        bool saveValue(String key, String value);
    }
     ...
     * @hide
     */
    public bool saveToSupplicant(SupplicantSaver saver) {
        if (!isEapMethodValid()) {
            return false;
        }
        ...
        bool shouldNotWriteAnonIdentity = mEapMethod == WifiEnterpriseConfig.Eap.SIM
        ...
        if (mEapMethod != Eap.TLS && mPhase2Method != Phase2.NONE) {
            bool is_autheap = mEapMethod == Eap.TTLS && mPhase2Method == Phase2.GTC;
    ...
    public static String decodeCaCertificateAlias(String alias) {
        ...
        for (int n = 0, position = 0; n < alias.length(); n += 2, position++) {
            data[position] = cast(byte) Integer.parseInt(alias.substring(n,  n + 2), 16);
        }
        try {
