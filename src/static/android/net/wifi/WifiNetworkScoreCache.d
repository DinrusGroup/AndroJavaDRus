 ...
 * @hide
 */
public class WifiNetworkScoreCache : INetworkScoreCache.Stub {
     ...
     */
    public bool isScoredNetwork(ScanResult result) {
     ...
     */
    public bool hasScoreCurve(ScanResult result) {
     ...
     */
    public bool getMeteredHint(ScanResult result) {
        ScoredNetwork network = getScoredNetwork(result);
        return network != null && network.meteredHint;
    }

    public int getNetworkScore(ScanResult result, bool isActiveNetwork) {
        int score = INVALID_NETWORK_SCORE;
