...
public class RttManager {
    ...
    @Deprecated
    public static class ParcelableRttParams : Parcelable {
        ...
        public void writeToParcel(Parcel dest, int flags) {
            ...
            for (RttParams params : mParams) {
                ...
                dest.writeByte(params.secure ? cast(byte) 1 : 0);
    ...
    @Deprecated
    public static class ParcelableRttResults : Parcelable {
        ...
        public void writeToParcel(Parcel dest, int flags) {
            if (mResults != null) {
                ...
                for (RttResult result : mResults) {
                    ...
                    if (result.LCI.id != cast(byte) 0xFF) {
                        dest.writeByte(cast(byte)result.LCI.data.length);
                        dest.writeByteArray(result.LCI.data);
                    }
                    dest.writeByte(result.LCR.id);
                    if (result.LCR.id != cast(byte) 0xFF) {
                        dest.writeByte(cast(byte) result.LCR.data.length);
                        dest.writeByteArray(result.LCR.data);
                    }
                    dest.writeByte(result.secure ? cast(byte) 1 : 0);
                }
            } else {
                dest.writeInt(0);
            }
                ...
                new Creator<ParcelableRttResults>() {
                    ...
                    public ParcelableRttResults createFromParcel(Parcel in) {
                        ...
                        for (int i = 0; i < num; i++) {
                            ...
                            if (results[i].LCI.id != cast(byte) 0xFF) {
                            ...
                            if (results[i].LCR.id != cast(byte) 0xFF) {
                                byte length = in.readByte();
     ...
     */
    @Deprecated
    public static class ResponderConfig : Parcelable {
    ...
    public RttManager(Context context, WifiRttManager service) {
        ...
        bool rttSupported = context.getPackageManager().hasSystemFeature(
