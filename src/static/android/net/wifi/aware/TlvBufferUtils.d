...
public class TlvBufferUtils {
    ...
    public static class TlvConstructor {
        ...
        private void addHeader(int type, int length) {
            if (mTypeSize == 1) {
                mArray[mPosition] = cast(byte) type;
            } else if (mTypeSize == 2) {
                Memory.pokeShort(mArray, mPosition, cast(short) type, ByteOrder.BIG_ENDIAN);
            }
            mPosition += mTypeSize;

            if (mLengthSize == 1) {
                mArray[mPosition] = cast(byte) length;
            } else if (mLengthSize == 2) {
                Memory.pokeShort(mArray, mPosition, cast(short) length, ByteOrder.BIG_ENDIAN);
            }
     ...
     */
    public static class TlvIterable : Iterable<TlvElement> {
        private int mTypeSize;
        private int mLengthSize;
        private byte[] mArray;
        private int mArrayLength;
     ...
     * @return A bool indicating whether the array is valid (true) or invalid (false).
     */
    public static bool isValid(@Nullable byte[] array, int typeSize, int lengthSize) {
