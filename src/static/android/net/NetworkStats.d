 ...
 * @hide
 */
public class NetworkStats : Parcelable {
    ...
    private Entry getTotal(
            Entry recycle, HashSet<String> limitIface, int limitUid, bool includeTags) {
        final Entry entry = recycle != null ? recycle : new Entry();
        ...
        for (int i = 0; i < size; i++) {
            final bool matchesUid = (limitUid == UID_ALL) || (limitUid == uid[i]);
            final bool matchesIface = (limitIface == null) || (limitIface.contains(iface[i]));
     ...
     */
    public static bool setMatches(int querySet, int dataSet) {
        if (querySet == dataSet) {
            return true;
        }
     ...
     */
    public bool migrateTun(int tunUid, String tunIface, String underlyingIface) {
