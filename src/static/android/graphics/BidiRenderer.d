...
public class BidiRenderer {
     ...
     * #renderText(int, int, bool, float[], int, bool)}
     */
    public RectF renderText(int start, int limit, int bidiFlags, float[] advances,
            int advancesIndex, bool draw) {
        mBidi.setPara(Arrays.copyOfRange(mText, start, limit), cast(byte)getIcuFlags(bidiFlags), null);
        mText = mBidi.getText();
        for (int i = 0; i < mBidi.countRuns(); i++) {
            BidiRun visualRun = mBidi.getVisualRun(i);
            bool isRtl = visualRun.getDirection() == Bidi.RTL;
     ...
     */
    public RectF renderText(int start, int limit, bool isRtl, float[] advances,
            int advancesIndex, bool draw) {
     ...
     * much as possible. This also : a fallback mechanism to render characters that cannot
     * be drawn using the preferred font.
     */
    private void renderScript(int start, int limit, Font preferredFont, int flag,
            float[] advances, int advancesIndex, bool draw) {
    ...
    private void render(int start, int limit, Font font, int flag, float[] advances,
        ...
        bool frcIsAntialiased = frc.isAntiAliased();
        bool useAntialiasing = mPaint.isAntiAliased();
