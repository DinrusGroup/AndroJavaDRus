...
public class BaseCanvas_Delegate {
    ...
    private final static bool[] sBoolOut = new bool[1];
    ...
    /*package*/ static void nDrawBitmap(long nativeCanvas, int[] colors, int offset, int stride,
            final float x, final float y, int width, int height, bool hasAlpha,
    ...
    /*package*/ static void nDrawArc(long nativeCanvas,
            ...
            final bool useCenter, long paint) {
        if (right > left && bottom > top) {
            draw(nativeCanvas, paint, false /*compositeOnly*/, false /*forceSrcMode*/,
                    (graphics, paintDelegate) -> {
                        int style = paintDelegate.getStyle();
    ...
    /*package*/ static void nDrawTextRun(long nativeCanvas, String text,
            ...
            float x, float y, bool isRtl, long paint) {
        int count = end - start;
        char[] buffer = TemporaryBuffer.obtain(count);
        TextUtils.getChars(text, start, end, buffer, 0);
     ...
     */
    private static void draw(long nCanvas, long nPaint, bool compositeOnly, bool forceSrcMode,
     ...
     * The image returns, through a 1-size bool array, whether the drawing code should
    ...
    protected bool clipRect(float left, float top, float right, float bottom, int regionOp) {
