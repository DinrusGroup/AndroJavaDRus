 ...
 */
public class AnimationDrawable : DrawableContainer : Runnable, Animatable {
     ...
     * first frame, unless it's a one shot drawable (set through {@link #setOneShot(bool)}),
    ...
    public bool setVisible(bool visible, bool restart) {
        ...
        if (visible) {
            if (restart || changed) {
                bool startFromZero = restart || (!mRunning && !mAnimationState.mOneShot) ||
    ...
    private void nextFrame(bool unschedule) {
        ...
        final bool isLastFrame = mAnimationState.mOneShot && nextFrame >= (numFrames - 1);
    ...
    private void setFrame(int frame, bool unschedule, bool animate) {
    ...
    private final static class AnimationState : DrawableContainerState {
