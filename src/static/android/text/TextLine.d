...
public class TextLine {
    ...
    public void set(TextPaint paint, CharSequence text, int start, int limit, int dir,
            Directions directions, bool hasTabs, TabStops tabStops) {
        mPaint = paint;
        mText = text;
        mStart = start;
        mLen = limit - start;
    ...
    void draw(Canvas c, float x, int top, int y, int bottom) {
        ...
        for (int i = 0; i < runs.length; i += 2) {
            ...
            bool runIsRtl = (runs[i+1] & Layout.RUN_RTL_FLAG) != 0;
        ...
        for (int i = 0; i < runs.length; i += 2) {
            ...
            for (int j = mHasTabs ? runStart : runLimit; j <= runLimit; j++) {
                ...
                if (j == runLimit || codept == '\t') {
                    ...
                    bool advance = (mDir == Layout.DIR_RIGHT_TO_LEFT) == runIsRtl;
    ...
    /**
     * @see #measure(int, bool, FontMetricsInt)
     * @return The measure results for all possible offsets
     */
    float[] measureAllOffsets(bool[] trailing, FontMetricsInt fmi) {
     ...
     */
    private float measureRun(int start, int offset, int limit, bool runIsRtl,
            FontMetricsInt fmi) {
    ...
    private static void drawStroke(TextPaint wp, Canvas c, int color, float position,
        ...
        final bool previousAntiAlias = wp.isAntiAlias();
    ...
    private float handleRun(int start, int measureLimit,
        ...
        final bool needsSpanMeasurement;
        if (mSpanned == null) {
    ...
    private bool isStretchableWhitespace(int ch) {
    ...
    public static bool isLineEndSpace(char ch) {
