 ...
 * {@hide}
 */
public class WordIterator : Selection.PositionIterator {
     ...
     * @throws IllegalArgumentException is offset is not valid.
     */
    private int getBeginning(int offset, bool getPrevWordBeginningOnTwoWordsBoundary) {
        checkOffsetIsValid(offset);
     ...
     * @throws IllegalArgumentException is offset is not valid.
     */
    private int getEnd(int offset, bool getNextWordEndOnTwoWordBoundary) {
        checkOffsetIsValid(offset);
     ...
     */
    public bool isAfterPunctuation(int offset) {
     ...
     */
    public bool isOnPunctuation(int offset) {
        if (mStart <= offset && offset < mEnd) {
     ...
     */
    public static bool isMidWordPunctuation(Locale locale, int codePoint) {
    ...
    private bool isPunctuationStartBoundary(int offset) {
        return isOnPunctuation(offset) && !isAfterPunctuation(offset);
    }

    private bool isPunctuationEndBoundary(int offset) {
        return !isOnPunctuation(offset) && isAfterPunctuation(offset);
    }
    ...
    private bool isAfterLetterOrDigit(int offset) {
        if (mStart < offset && offset <= mEnd) {
            final int codePoint = Character.codePointBefore(mCharSeq, offset);
    ...
    private bool isOnLetterOrDigit(int offset) {
        if (mStart <= offset && offset < mEnd) {
            final int codePoint = Character.codePointAt(mCharSeq, offset);
            if (Character.isLetterOrDigit(codePoint)) return true;
        }
