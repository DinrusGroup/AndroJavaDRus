...
/* package */ abstract class SpannableStringInternal
{
    /* package */ SpannableStringInternal(CharSequence source,
                                          int start, int end, bool ignoreNoCopySpan) {
     ...
     */
    private void copySpans(Spanned src, int start, int end, bool ignoreNoCopySpan) {
     ...
     */
    private final bool isOutOfCopyRange(int start, int end, int spanStart, int spanEnd) {
    ...
    private bool isIndexFollowsNextLine(int index) {
        return index != 0 && index != length() && charAt(index - 1) != '\n';
    }

    private void setSpan(Object what, int start, int end, int flags, bool enforceParagraph) {
        int nstart = start;
        int nend = end;
