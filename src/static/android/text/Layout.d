...
public abstract class Layout {
    ...
    private bool isJustificationRequired(int lineNum) {
    ...
    private float getJustifyWidth(int lineNum) {
        ...
        if (mSpannedText) {
            ...
            final bool isFirstParaLine = (start == 0 || mText.charAt(start - 1) == '\n');
    ...
    public void drawText(Canvas canvas, int firstLine, int lastLine) {
        ...
        for (int lineNum = firstLine; lineNum <= lastLine; lineNum++) {
            ...
            final bool justify = isJustificationRequired(lineNum);
            ...
            bool hasTab = getLineContainsTab(lineNum);
     ...
     * @hide
     */
    public bool isLevelBoundary(int offset) {
     ...
     */
    public bool isRtlCharAt(int offset) {
        int line = getLineForOffset(offset);
        Directions dirs = getLineDirections(line);
     ...
     */
    private bool[] primaryIsTrailingPreviousAllLineOffsets(int line) {
        int lineStart = getLineStart(line);
        int lineEnd = getLineEnd(line);
        int[] runs = getLineDirections(line).mDirections;

        bool[] trailing = new bool[lineEnd - lineStart + 1];
        ...
        for (int i = 0; i < runs.length; i += 2) {
            ...
            level[limit - lineStart - 1] =
                    cast(byte) ((runs[i + 1] >>> RUN_LEVEL_SHIFT) & RUN_LEVEL_MASK);
        }

        for (int i = 0; i < runs.length; i += 2) {
            int start = lineStart + runs[i];
            byte currentLevel = cast(byte) ((runs[i + 1] >>> RUN_LEVEL_SHIFT) & RUN_LEVEL_MASK);
     ...
     * @hide
     */
    public float getPrimaryHorizontal(int offset, bool clamped) {
     ...
     * @hide
     */
    public float getSecondaryHorizontal(int offset, bool clamped) {
        bool trailing = primaryIsTrailingPrevious(offset);
        return getHorizontal(offset, !trailing, clamped);
    }

    private float getHorizontal(int offset, bool primary) {
        return primary ? getPrimaryHorizontal(offset) : getSecondaryHorizontal(offset);
    }

    private float getHorizontal(int offset, bool trailing, bool clamped) {
        int line = getLineForOffset(offset);

        return getHorizontal(offset, trailing, line, clamped);
    }

    private float getHorizontal(int offset, bool trailing, int line, bool clamped) {
        int start = getLineStart(line);
     ...
     * @return The array of results, indexed from 0, where 0 corresponds to the line start offset
     */
    private float[] getLineHorizontals(int line, bool clamped, bool primary) {
        int start = getLineStart(line);
        int end = getLineEnd(line);
        int dir = getParagraphDirection(line);
        bool hasTab = getLineContainsTab(line);
        ...
        TextLine tl = TextLine.obtain();
        tl.set(mPaint, mText, start, end, dir, directions, hasTab, tabStops);
        bool[] trailings = primaryIsTrailingPreviousAllLineOffsets(line);
        if (!primary) {
    ...
    private float getLineExtent(int line, bool full) {
        ...
        final bool hasTabs = getLineContainsTab(line);
        TabStops tabStops = null;
     ...
     */
    private float getLineExtent(int line, TabStops tabStops, bool full) {
        final int start = getLineStart(line);
        final int end = full ? getLineEnd(line) : getLineVisibleEnd(line);
        final bool hasTabs = getLineContainsTab(line);
        final Directions directions = getLineDirections(line);
     ...
     * @hide
     */
    public int getOffsetForHorizontal(int line, float horiz, bool primary) {
    ...
    private class HorizontalMeasurementProvider {
        ...
        private final bool mPrimary;

        private float[] mHorizontals;
        private int mLineStartOffset;

        HorizontalMeasurementProvider(final int line, final bool primary) {
            mLine = line;
            mPrimary = primary;
            init();
        }
     ...
     * @hide
     */
    public bool shouldClampCursor(int line) {
    ...
    protected final bool isSpanned() {
        return mSpannedText;
    }
    ...
    private void ellipsize(int start, int end, int line,
        ...
        final bool useEllipsisString = ellipsisCount >= ellipsisStringLen;
    ...
    /* package */ static class Ellipsizer : CharSequence, GetChars {
    ...
    /* package */ static class SpannedEllipsizer : Ellipsizer : Spanned {
