...
public class DynamicLayout : Layout {
    ...
    public DynamicLayout(@NonNull CharSequence base, @NonNull CharSequence display,
                         ...
                         @FloatRange(from = 0.0) float spacingmult, float spacingadd,
                         bool includepad, @BreakStrategy int breakStrategy,
    ...
    private bool contentMayProtrudeFromLineTopOrBottom(CharSequence text, int start, int end) {
        if (text instanceof Spanned) {
    ...
    public void updateBlocks(int startLine, int endLine, int newLineCount) {
        ...
        final bool createBlockBefore = startLine > (firstBlock == 0 ? 0 :
                mBlockEndLines[firstBlock - 1] + 1);
        final bool createBlock = newLineCount > 0;
        final bool createBlockAfter = endLine < mBlockEndLines[lastBlock];
    ...
    private bool getContentMayProtrudeFromTopOrBottom(int line) {
    ...
    private static class ChangeWatcher : TextWatcher, SpanWatcher {
