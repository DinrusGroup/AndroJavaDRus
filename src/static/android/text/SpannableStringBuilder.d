 ...
 */
public class SpannableStringBuilder : CharSequence, GetChars, Spannable, Editable,
    ...
    private bool removeSpansForChange(int start, int end, bool textIsRemoved, int i) {
        if ((i & 1) != 0) {
            // internal tree node
    ...
    private void change(int start, int end, CharSequence cs, int csStart, int csEnd) {
        ...
        final bool textIsRemoved = replacementLength == 0;
        ...
        if (replacedLength > 0) { // no need for span fixup on pure insertion
            ...
            final bool atEnd = (mGapStart + mGapLength == mText.length);
    ...
    private int updatedIntervalBound(int offset, int start, int nbNewChars, int flag, bool atEnd,
            bool textIsRemoved) {
    ...
    private static bool hasNonExclusiveExclusiveSpanAt(CharSequence text, int offset) {
        if (text instanceof Spanned) {
    ...
    private void setSpan(bool send, Object what, int start, int end, int flags,
            bool enforceParagraph) {
    ...
    private bool isInvalidParagraph(int index, int flag) {
    ...
    private <T> int getSpansRec(int queryStart, int queryEnd, Class<T> kind,
            int i, T[] ret, int[] priority, int[] insertionOrder, int count, bool sort) {
        if ((i & 1) != 0) {
            // internal tree node
            int left = leftChild(i);
            int spanMax = mSpanMax[left];
    ...
    /*
    private bool isprint(char c) { // XXX
        if (c >= ' ' && c <= '~')
            return true;
        else
            return false;
    ...
    */
    ...
    public void drawTextRun(BaseCanvas c, int start, int end, int contextStart, int contextEnd,
            float x, float y, bool isRtl, Paint p) {
     ...
     * {@hide}
     */
    public float getTextRunAdvances(int start, int end, int contextStart, int contextEnd, bool isRtl,
