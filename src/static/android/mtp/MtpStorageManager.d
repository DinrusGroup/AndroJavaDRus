...
public class MtpStorageManager {
    ...
    private class MtpObjectObserver : FileObserver {
        MtpObject mObject;
    ...
    public static class MtpObject {
        ...
        MtpObject(String name, int id, MtpObject parent, bool isDir) {
            mId = id;
            mName = name;
            mParent = parent;
            mObserver = null;
        ...
        private MtpObject copy(bool recursive) {
    ...
    private volatile bool mCheckConsistency;
     ...
     */
    private synchronized bool isSpecialSubDir(MtpObject obj) {
    ...
    private synchronized Stream<MtpObject> getObjects(MtpObject parent, int format, bool rec) {
     ...
     */
    private synchronized bool removeObjectFromCache(MtpObject removed, bool removeGlobal,
            bool recursive) {
        bool ret = removed.isRoot()
    ...
    private synchronized void handleAddedObject(MtpObject parent, String path, bool isDir) {
     ...
     */
    public synchronized bool checkConsistency() {
        Stream<MtpObject> objs = Stream.concat(mRoots.values().stream(),
                mObjects.values().stream());
        Iterator<MtpObject> iter = objs.iterator();
        bool ret = true;
     ...
     */
    public synchronized bool endSendObject(MtpObject obj, bool succeeded) {
        if (sDebug)
     ...
     */
    public synchronized bool beginRenameObject(MtpObject obj, String newName) {
        if (sDebug)
     ...
     */
    public synchronized bool endRenameObject(MtpObject obj, String oldName, bool success) {
        if (sDebug)
     ...
     */
    public synchronized bool beginRemoveObject(MtpObject obj) {
        if (sDebug)
     ...
     */
    public synchronized bool endRemoveObject(MtpObject obj, bool success) {
        if (sDebug)
     ...
     */
    public synchronized bool beginMoveObject(MtpObject obj, MtpObject newParent) {
        if (sDebug)
     ...
     * @return Whether cache state is correct.
     */
    public synchronized bool endMoveObject(MtpObject oldParent, MtpObject newParent, String name,
            bool success) {
        if (sDebug)
        ...
        if (oldParent.getStorageId() != newObj.getStorageId()) {
            bool ret = endRemoveObject(oldObj, success);
     ...
     */
    public synchronized bool endCopyObject(MtpObject object, bool success) {
        if (sDebug)
            Log.v(TAG, "endCopyObject " + object.getName() + " " + success);
        return generalEndCopyObject(object, success, false);
    }

    private synchronized bool generalEndAddObject(MtpObject obj, bool succeeded,
            bool removeGlobal) {
        switch (obj.getState()) {
            case FROZEN:
    ...
    private synchronized bool generalEndRemoveObject(MtpObject obj, bool success,
            bool removeGlobal) {
        switch (obj.getState()) {
            case FROZEN:
                if (success) {
    ...
    private synchronized bool generalBeginRenameObject(MtpObject fromObj, MtpObject toObj) {
    ...
    private synchronized bool generalEndRenameObject(MtpObject fromObj, MtpObject toObj,
            bool success) {
        bool ret = generalEndRemoveObject(fromObj, success, !success);
        return generalEndAddObject(toObj, success, success) && ret;
    }

    private synchronized bool generalBeginRemoveObject(MtpObject obj, MtpOperation op) {
        obj.setState(MtpObjectState.FROZEN);
        obj.setOperation(op);
        if (obj.isDir()) {
            for (MtpObject child : obj.getChildren())
    ...
    private synchronized bool generalBeginCopyObject(MtpObject obj, bool newId) {
        obj.setState(MtpObjectState.FROZEN);
    ...
    private synchronized bool generalEndCopyObject(MtpObject obj, bool success, bool addGlobal) {
