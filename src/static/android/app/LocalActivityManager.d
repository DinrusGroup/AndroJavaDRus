...
public class LocalActivityManager {
    ...
    private static class LocalActivityRecord : Binder {
     ...
     */
    public LocalActivityManager(Activity parent, bool singleMode) {
    ...
    private void performPause(LocalActivityRecord r, bool finishing) {
        final bool needState = r.instanceState == null;
    ...
    private Window performDestroy(LocalActivityRecord r, bool finish) {
        Window win;
        win = r.window;
     ...
     */
    public Window destroyActivity(String id, bool finish) {
        LocalActivityRecord r = mActivities.get(id);
        Window win = null;
        if (r != null) {
     ...
     */
    public void dispatchPause(bool finishing) {
        if (finishing) {
            mFinishing = true;
        }
        mCurState = STARTED;
     ...
     */
    public void dispatchDestroy(bool finishing) {
        final int N = mActivityArray.size();
        for (int i=0; i<N; i++) {
            LocalActivityRecord r = mActivityArray.get(i);
            if (localLOGV) Log.v(TAG, r.id + ": destroying");
