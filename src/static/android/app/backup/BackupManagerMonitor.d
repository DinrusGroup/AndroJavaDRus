...
public class BackupManagerMonitor {
  ...
  /**
   * bool: when we have an event with id LOG_EVENT_ID_KEY_VALUE_BACKUP_CANCEL we record if
  ...
  /**
   * bool: when we have an event with id LOG_EVENT_ID_RESTORE_VERSION_HIGHER we record if
  ...
  /**
   * bool: when we have an event with id LOG_EVENT_ID_APK_NOT_INSTALLED we record if
