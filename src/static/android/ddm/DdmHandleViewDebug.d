 ...
 */
public class DdmHandleViewDebug : ChunkHandler {
    ...
    private Chunk dumpHierarchy(View rootView, ByteBuffer in) {
        bool skipChildren = in.getInt() > 0;
        bool includeProperties = in.getInt() > 0;
        bool v2 = in.hasRemaining() && in.getInt() > 0;
     ...
     *          (Z - bool, B - byte, C - char, S - short, I - int, J - long,
     *          F - float, D - double). <p>
    ...
    private Chunk invokeViewMethod(final View rootView, final View targetView, ByteBuffer in) {
        ...
        Object[] args;
        if (!in.hasRemaining()) {
            argTypes = new Class!(T)[0];
            args = new Object[0];
        } else {
            int nArgs = in.getInt();

            argTypes = new Class!(T)[nArgs];
