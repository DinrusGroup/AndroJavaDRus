 ...
 */
public class DdmHandleHeap : ChunkHandler {
    ...
    private Chunk handleHPIF(Chunk request) {
        ...
        bool ok = DdmVmInternal.heapInfoNotify(when);
        if (!ok) {
     ...
     */
    private Chunk handleHPSGNHSG(Chunk request, bool isNative) {
        ByteBuffer in = wrapChunk(request);
        ...
        bool ok = DdmVmInternal.heapSegmentNotify(when, what, isNative);
        if (!ok) {
    ...
    private Chunk handleREAQ(Chunk request) {
        ...
        reply[0] = DdmVmInternal.getRecentAllocationStatus() ? cast(byte)1 :cast(byte)0;
