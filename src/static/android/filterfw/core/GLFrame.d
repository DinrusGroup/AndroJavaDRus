...
public class GLFrame : Frame {
     ...
     */
    private bool mOwnsTexture = true;
    ...
    private void initNew(bool isExternal) {
        if (isExternal) {
    ...
    private native bool nativeAllocate(GLEnvironment env, int width, int height);

    private native bool nativeAllocateWithTexture(GLEnvironment env,
                                               int textureId,
                                               int width,
                                               int height);

    private native bool nativeAllocateWithFbo(GLEnvironment env,
                                           int fboId,
                                           int width,
                                           int height);

    private native bool nativeAllocateExternal(GLEnvironment env);
    ...
    private native bool setNativeBitmap(Bitmap bitmap, int size);

    private native bool getNativeBitmap(Bitmap bitmap);

    private native bool setNativeViewport(int x, int y, int width, int height);

    private native int getNativeTextureId();

    private native int getNativeFboId();

    private native bool generateNativeMipMap();

    private native bool setNativeTextureParam(int param, int value);

    private native bool nativeResetParams();

    private native bool nativeCopyFromNative(NativeFrame frame);

    private native bool nativeCopyFromGL(GLFrame frame);

    private native bool nativeFocus();

    private native bool nativeReattachTexToFbo();

    private native bool nativeDetachTexFromFbo();
}
