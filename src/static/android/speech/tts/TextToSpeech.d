...
public class TextToSpeech {
    ...
    // too.
    private final bool mUseFallback;
     ...
     */
    public TextToSpeech(Context context, OnInitListener listener, String engine,
            String packageName, bool useFallback) {
        mContext = context;
    ...
    private <R> R runActionNoReconnect(Action<R> action, R errorResult, String method,
            bool onlyEstablishedConnection) {
    ...
    private <R> R runAction(Action<R> action, R errorResult, String method,
            bool reconnect, bool onlyEstablishedConnection) {
    ...
    private bool connectToEngine(String engine) {
        Connection connection = new Connection();
        Intent intent = new Intent(Engine.INTENT_ACTION_TTS_SERVICE);
        intent.setPackage(engine);
        bool bound = mContext.bindService(intent, connection, Context.BIND_AUTO_CREATE);
        if (!bound) {
    ...
    private static bool verifyIntegerBundleParam(Bundle bundle, String key) {
    ...
    private static bool verifyStringBundleParam(Bundle bundle, String key) {
        if (bundle.containsKey(key)) {
    ...
    private static bool verifyBooleanBundleParam(Bundle bundle, String key) {
        if (bundle.containsKey(key)) {
    ...
    private static bool verifyFloatBundleParam(Bundle bundle, String key) {
        if (bundle.containsKey(key)) {
    ...
    private class Connection : ServiceConnection {
        ...
        private class SetupConnectionAsyncTask : AsyncTask<Void, Void, Integer> {
         ...
         */
        private bool clearServiceConnection() {
            synchronized(mStartLock) {
                bool result = false;
                if (mOnSetupConnectionAsyncTask != null) {
