...
public class SystemProperties {
    ...
    private static final bool TRACK_KEY_ACCESS = false;
    ...
    private static native bool native_get_bool(String key, bool def);
    ...
    /**
     * Get the value for the given {@code key}, returned as a bool.
     ...
     * @param key the key to lookup
     * @param def a default value to return
     * @return the key parsed as a bool, or def if the key isn't found or is
     *         not able to be parsed as a bool.
     * @hide
     */
    @SystemApi
    public static bool getBoolean(@NonNull String key, bool def) {
        if (TRACK_KEY_ACCESS) onKeyAccess(key);
        return native_get_bool(key, def);
    }
