...
public abstract class UserManagerInternal {
    ...
    public abstract bool getUserRestriction(int userId, String key);
     ...
     */
    public abstract void setDeviceManaged(bool isManaged);

    /**
     * Called by {@link com.android.server.devicepolicy.DevicePolicyManagerService} to update
     * whether the user is managed by profile owner.
     */
    public abstract void setUserManaged(int userId, bool isManaged);
     ...
     */
    public abstract void setForceEphemeralUsers(bool forceEphemeralUsers);
     ...
     */
    public abstract bool removeUserEvenWhenDisallowed(int userId);
     ...
     */
    public abstract bool isUserUnlockingOrUnlocked(int userId);

    /**
     * Return whether the given user is running in an
     * {@code UserState.STATE_RUNNING_UNLOCKED} state.
     */
    public abstract bool isUserUnlocked(int userId);

    /**
     * Returns whether the given user is running
     */
    public abstract bool isUserRunning(int userId);

    /**
     * Returns whether the given user is initialized
     */
    public abstract bool isUserInitialized(int userId);

    /**
     * Returns whether the given user exists
     */
    public abstract bool exists(int userId);
     ...
     */
    public abstract bool isProfileAccessible(int callingUserId, int targetUserId,
            String debugMsg, bool throwSecurityException);
     ...
     * Context, int, String, bool)}, which should be in sync with this method.
     *
     * @return {@code true} if the change is prohibited, {@code false} if the change is allowed.
     *
     * @hide
     */
    public abstract bool isSettingRestrictedForUser(String setting, int userId, String value,
            int callingUid);
}
