 ...
 * @hide
 */
public abstract class BatteryStats : Parcelable {
    ...
    protected static final bool SCREEN_OFF_RPM_STATS_ENABLED = false;
    ...
    public static abstract class Uid {
         ...
         */
        public abstract ArrayMap<String, ? : Wakelock> getWakelockStats();
         ...
         */
        public abstract ArrayMap<String, ? : Timer> getSyncStats();
         ...
         * @return a Map from Strings to Timer objects.
         */
        public abstract ArrayMap<String, ? : Timer> getJobStats();
         ...
         */
        public abstract SparseArray<? : Sensor> getSensorStats();

        /**
         * Returns a mapping containing active process data.
         */
        public abstract SparseArray<? : Pid> getPidStats();
         ...
         */
        public abstract ArrayMap<String, ? : Proc> getProcessStats();
         ...
         */
        public abstract ArrayMap<String, ? : Pkg> getPackageStats();
        ...
        public abstract bool hasUserActivity();
        public abstract int getUserActivityCount(int type, int which);

        public abstract bool hasNetworkActivity();
        ...
        public static abstract class Pkg {
             ...
             */
            public abstract ArrayMap<String, ? : Counter> getWakeupAlarmStats();

            /**
             * Returns a mapping containing service statistics.
             */
            public abstract ArrayMap<String, ? : Serv> getServiceStats();
    ...
    public final static class HistoryItem : Parcelable {
        ...
        public void readFromParcel(Parcel src) {
            ...
            int bat = src.readInt();
            cmd = cast(byte)(bat&0xff);
            batteryLevel = cast(byte)((bat>>8)&0xff);
            batteryStatus = cast(byte)((bat>>16)&0xf);
            batteryHealth = cast(byte)((bat>>20)&0xf);
            batteryPlugType = cast(byte)((bat>>24)&0xf);
            int bat2 = src.readInt();
            batteryTemperature = cast(short)(bat2&0xffff);
        ...
        public bool sameNonEvent(HistoryItem o) {
        ...
        public bool same(HistoryItem o) {
    ...
    public final static class HistoryEventTracker {
        ...
        public bool updateState(int code, String name, int uid, int poolIdx) {
    ...
    public abstract bool startIteratingHistoryLocked();
    ...
    public abstract bool getNextHistoryLocked(HistoryItem out);

    public abstract void finishIteratingHistoryLocked();

    public abstract bool startIteratingOldHistoryLocked();

    public abstract bool getNextOldHistoryLocked(HistoryItem out);
     ...
     * actual power data.
     */
    public abstract bool hasWifiActivityReporting();
     ...
     * actual power data.
     */
    public abstract bool hasBluetoothActivityReporting();
     ...
     * actual power data.
     */
    public abstract bool hasModemActivityReporting();
     ...
     */
    public abstract bool getIsOnBattery();

    /**
     * Returns a SparseArray containing the statistics for each uid.
     */
    public abstract SparseArray<? : Uid> getUidStats();
    ...
    public abstract Map<String, ? : Timer> getWakeupReasonStats();

    public abstract Map<String, ? : Timer> getKernelWakelockStats();

    /**
     * Returns Timers tracking the total time of each Resource Power Manager state and voter.
     */
    public abstract Map<String, ? : Timer> getRpmStats();
    /**
     * Returns Timers tracking the screen-off time of each Resource Power Manager state and voter.
     */
    public abstract Map<String, ? : Timer> getScreenOffRpmStats();


    public abstract LongSparseArray<? : Timer> getKernelMemoryStats();
     ...
     */
    private static final bool printTimer(PrintWriter pw, StringBuilder sb, Timer timer,
     ...
     */
    private static bool controllerActivityHasData(ControllerActivityCounter counter, int which) {
        if (counter == null) {
            return false;
        }
    ...
    public final void dumpCheckinLocked(Context context, PrintWriter pw, int which, int reqUid,
        ...
        final SparseArray<? : Uid> uidStats = getUidStats();
        ...
        for (int iu = 0; iu < NU; iu++) {
            ...
            final ArrayMap<String, ? : BatteryStats.Uid.Wakelock> wakelocks
                    = u.getWakelockStats();
        ...
        if (reqUid < 0) {
            final Map<String, ? : Timer> kernelWakelocks = getKernelWakelockStats();
            if (kernelWakelocks.size() > 0) {
                for (Map.Entry<String, ? : Timer> ent : kernelWakelocks.entrySet()) {
                    sb.setLength(0);
            ...
            final Map<String, ? : Timer> wakeupReasons = getWakeupReasonStats();
            if (wakeupReasons.size() > 0) {
                for (Map.Entry<String, ? : Timer> ent : wakeupReasons.entrySet()) {
        ...
        final Map<String, ? : Timer> rpmStats = getRpmStats();
        final Map<String, ? : Timer> screenOffRpmStats = getScreenOffRpmStats();
        if (rpmStats.size() > 0) {
            for (Map.Entry<String, ? : Timer> ent : rpmStats.entrySet()) {
                sb.setLength(0);
        ...
        for (int iu = 0; iu < NU; iu++) {
            ...
            final ArrayMap<String, ? : Uid.Wakelock> wakelocks = u.getWakelockStats();
            for (int iw=wakelocks.size()-1; iw>=0; iw--) {
                final Uid.Wakelock wl = wakelocks.valueAt(iw);
                String linePrefix = "";
                sb.setLength(0);
            ...
            final ArrayMap<String, ? : Timer> syncs = u.getSyncStats();
            ...
            final ArrayMap<String, ? : Timer> jobs = u.getJobStats();
            ...
            final SparseArray<? : BatteryStats.Uid.Sensor> sensors = u.getSensorStats();
            ...
            final ArrayMap<String, ? : BatteryStats.Uid.Proc> processStats
                    = u.getProcessStats();
            ...
            final ArrayMap<String, ? : BatteryStats.Uid.Pkg> packageStats
                    = u.getPackageStats();
            for (int ipkg=packageStats.size()-1; ipkg>=0; ipkg--) {
                final Uid.Pkg ps = packageStats.valueAt(ipkg);
                int wakeups = 0;
                final ArrayMap<String, ? : Counter> alarms = ps.getWakeupAlarmStats();
                ...
                final ArrayMap<String, ? :  Uid.Pkg.Serv> serviceStats = ps.getServiceStats();
    ...
    public final void dumpLocked(Context context, PrintWriter pw, String prefix, final int which,
        ...
        if (reqUid < 0) {
            final Map<String, ? : BatteryStats.Timer> kernelWakelocks
                    = getKernelWakelockStats();
            if (kernelWakelocks.size() > 0) {
                final ArrayList<TimerEntry> ktimers = new ArrayList<>();
                for (Map.Entry<String, ? : BatteryStats.Timer> ent
        ...
        final LongSparseArray<? : Timer> mMemoryStats = getKernelMemoryStats();
        ...
        if (SCREEN_OFF_RPM_STATS_ENABLED) {
            ...
            if (screenOffRpmStats.size() > 0) {
                ...
                if (screenOffRpmStats.size() > 0) {
                    for (Map.Entry<String, ? : Timer> ent : screenOffRpmStats.entrySet()) {
                        final String timerName = ent.getKey();
                        final Timer timer = ent.getValue();
                        printTimer(pw, sb, timer, rawRealtime, which, prefix, timerName);
                    }
    ...
    static void printBitDescriptions(StringBuilder sb, int oldval, int newval,
            HistoryTag wakelockTag, BitDescription[] descriptions, bool longNames) {
    ...
    public static class HistoryPrinter {
        ...
        public void printNextItem(PrintWriter pw, HistoryItem rec, long baseTime, bool checkin,
                bool verbose) {
        ...
        private String printNextItem(HistoryItem rec, long baseTime, bool checkin,
                bool verbose) {
    ...
    private static bool dumpTimeEstimate(PrintWriter pw, String label1, String label2,
    ...
    private static bool dumpDurationSteps(PrintWriter pw, String prefix, String header,
            LevelStepTracker steps, bool checkin) {
        if (steps == null) {
            return false;
        }
    ...
    private void dumpHistoryLocked(PrintWriter pw, int flags, long histStart, bool checkin) {
    ...
    public void dumpLocked(Context context, PrintWriter pw, int flags, int reqUid, long histStart) {
        ...
        if (!filtering) {
            ...
            for (int i=0; i<NU; i++) {
                ...
                SparseArray<? : Uid.Pid> pids = uid.getPidStats();
                if (pids != null) {
    ...
    private void dumpProtoAppsLocked(ProtoOutputStream proto, BatteryStatsHelper helper,
        ...
        for (int iu = 0; iu < n; ++iu) {
            ...
            for (int ipkg = packageStats.size() - 1; ipkg >= 0; --ipkg) {
                ...
                final ArrayMap<String, ? :  Uid.Pkg.Serv> serviceStats =
    ...
    private void dumpProtoSystemLocked(ProtoOutputStream proto, BatteryStatsHelper helper) {
        ...
        for (int i = 0; i < NUM_DATA_CONNECTION_TYPES; ++i) {
            // Map OTHER to TelephonyManager.NETWORK_TYPE_UNKNOWN and mark NONE as a bool.
            bool isNone = (i == DATA_CONNECTION_NONE);
