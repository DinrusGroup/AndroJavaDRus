...
public class ObservableParcelTest : AndroidTestCase {
    ...
    public void testParcelByte() {
        ObservableByte obj = new ObservableByte(cast(byte) 7);
        ObservableByte read = parcelAndUnparcel(obj, ObservableByte.class);
        assertEquals(cast(byte) 7, read.get());
    }
    ...
    private <T : Parcelable> T parcelAndUnparcel(T t, Class<T> klass) {
    ...
    public static class MyParcelable : Parcelable, Serializable {
        int x;
        String y;
