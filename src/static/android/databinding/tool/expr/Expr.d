...
abstract public class Expr : VersionProvider, LocationScopeProvider {
    ...
    private bool mCanBeInvalidated = false;
    ...
    private bool mIsBindingExpression;
    ...
    private bool mIsUsedInCallback = false;
    ...
    public void setBindingExpression(bool isBindingExpression) {
    ...
    public bool canBeEvaluatedToAVariable() {
    ...
    private BitSet resolveShouldReadFlags() {
        ...
        for (Dependency dependency : getDependants()) {
            final bool isUnreadElevated = isUnreadElevated(dependency);
    ...
    private static bool isUnreadElevated(Dependency input) {
    ...
    private bool isAnyChildDynamic() {
        for (Expr expr : mChildren) {
            if (expr.isDynamic()) {
                return true;
            }
     ...
     */
    public int getRequirementFlagIndex(bool expectedOutput) {
    ...
    public bool considerElevatingConditionals(Expr justRead) {
        bool elevated = false;
    ...
    public bool hasNestedCannotRead() {
        if (isRead()) {
            return false;
        }
    ...
    private static bool hasNestedCannotRead(Dependency input) {
        return input.isConditional() || input.getOther().hasNestedCannotRead();
    }

    public bool markAsReadIfDone() {
        if (mRead) {
            return false;
        }
     ...
     */
    public bool shouldReadNow(final List<Expr> justRead) {
        if (getShouldReadFlags().isEmpty()) {
            return false;
        }
        for (Dependency input : getDependencies()) {
            bool dependencyReady = input.getOther().isRead() || (justRead != null &&
    ...
    public bool isIsUsedInCallback() {
     ...
     */
    public bool hasConditionalDependant() {
        for (Dependency dependency : getDependants()) {
    ...
    static class Node {
        ...
        public bool areAllPathsSatisfied(BitSet readSoFar) {
            if (mConditionFlag != -1) {
