...
public class ExprModel {
     ...
     */
    public <T : Expr> T register(T expr) {
    ...
    public TernaryExpr logical(Expr left, String op, Expr right) {
        if ("&&".equals(op)) {
            // left && right
            // left ? right : false
            return register(new TernaryExpr(left, right, symbol("false", bool.class)));
        } else {
            // left || right
            // left ? true : right
            return register(new TernaryExpr(left, symbol("true", bool.class), right));
        }
    }
    ...
    private bool pruneDone(List<Expr> markedSomeFlagsAsRead) {
        bool marked = true;
    ...
    private static bool hasConditionalOrNestedCannotReadDependency(Expr expr) {
