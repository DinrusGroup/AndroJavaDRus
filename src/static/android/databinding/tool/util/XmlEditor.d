...
public class XmlEditor {
    ...
    public static String strip(File f, String newTag, String encoding) throws IOException {
        ...
        List<? : ElementContext> childrenOfRoot = elements(root);
        List<? : ElementContext> dataNodes = filterNodesByName("data", childrenOfRoot);
        if (dataNodes.size() > 1) {
        ...
        List<? : ElementContext> layoutNodes =
    ...
    private static <T : XMLParser.ElementContext> List<T>
    ...
    public static List<? : AttributeContext> attributes(ElementContext elementContext) {
    ...
    public static List<? : AttributeContext> expressionAttributes(
        ...
        for (AttributeContext input : attributes(elementContext)) {
            ...
            bool isExpression = attrName.equals("android:tag");
            if (!isExpression) {
    ...
    private static bool isExpressionText(String value) {
    ...
    public static List<? : ElementContext> elements(ElementContext context) {
    ...
    private static bool replace(ArrayList<String> lines, Position start, Position end,
            String text) {
    ...
    public static bool hasExpressionAttributes(ElementContext context) {
        List<? : AttributeContext> expressions = expressionAttributes(context);
    ...
    private static int recurseReplace(ElementContext node, ArrayList<String> lines,
        ...
        bool isMerge = "merge".equals(nodeName(node));
        final bool containsInclude = filterNodesByName("include", elements(node)).size() > 0;
    ...
    private static String defaultReplacement(XMLParser.AttributeContext attr) {
        ...
        final bool isTwoWay = escapedText.startsWith("@={");
        final bool isOneWay = escapedText.startsWith("@{");
