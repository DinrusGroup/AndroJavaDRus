...
public class ListenerUtil {
    private static SparseArray<WeakHashMap<View, WeakReference!(T)>> sListeners =
            new SparseArray<WeakHashMap<View, WeakReference!(T)>>();
    ...
    public static <T> T trackListener(View view, T listener, int listenerResourceId) {
        if (VERSION.SDK_INT >= VERSION_CODES.ICE_CREAM_SANDWICH) {
            ...
            synchronized (sListeners) {
                WeakHashMap<View, WeakReference!(T)> listeners = sListeners.get(listenerResourceId);
                if (listeners == null) {
