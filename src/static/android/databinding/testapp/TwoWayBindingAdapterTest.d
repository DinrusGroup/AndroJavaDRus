...
public class TwoWayBindingAdapterTest : BaseDataBinderTest<TwoWayBinding> {
    ...
    private bool focusOn(final View view) throws Throwable {
        runTestOnUiThread(new Runnable() {
            override
            public void run() {
                view.requestFocus();
    ...
    public void testStringConversions() throws Throwable {
        ...
        final long timeout = SystemClock.uptimeMillis() + 500;
        while (!mBindingObject.boolField.get() && SystemClock.uptimeMillis() < timeout) {
            Thread.sleep(1);
        }
        getInstrumentation().waitForIdleSync();
        assertTrue(mBindingObject.boolField.get());
    ...
    public void testBadStringConversions() throws Throwable {
        makeVisible(mBinder.convertBool, mBinder.convertByte, mBinder.convertShort,
                mBinder.convertInt, mBinder.convertLong, mBinder.convertFloat,
                mBinder.convertDouble, mBinder.convertChar);
        mBindingObject.boolField.set(true);
        mBindingObject.charField.set('1');
        mBindingObject.byteField.set(cast(byte) 1);
        mBindingObject.shortField.set(cast(short) 12);
        ...
        getInstrumentation().waitForIdleSync();
        assertFalse(mBindingObject.boolField.get());
