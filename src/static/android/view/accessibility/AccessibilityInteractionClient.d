...
public final class AccessibilityInteractionClient
    ...
    private static final bool CHECK_INTEGRITY = true;
    ...
    public AccessibilityNodeInfo findAccessibilityNodeInfoByAccessibilityId(int connectionId,
            int accessibilityWindowId, long accessibilityNodeId, bool bypassCache,
    ...
    private AccessibilityNodeInfo getFindAccessibilityNodeInfoResultAndClear(int interactionId) {
        synchronized (mInstanceLock) {
            final bool success = waitForResultTimedLocked(interactionId);
        ...
        synchronized (mInstanceLock) {
            if (interactionId > mInteractionId) {
                if (infos != null) {
                    ...
                    final bool isIpcCall = (Binder.getCallingPid() != Process.myPid());
                    if (!isIpcCall) {
     ...
     * @return Whether the action was performed.
     */
    private bool getPerformAccessibilityActionResultAndClear(int interactionId) {
        synchronized (mInstanceLock) {
            final bool success = waitForResultTimedLocked(interactionId);
            final bool result = success ? mPerformAccessibilityActionResult : false;
            clearResultLocked();
            return result;
        }
    }
    ...
    private void finalizeAndCacheAccessibilityNodeInfo(AccessibilityNodeInfo info,
            int connectionId, bool bypassCache, String[] packageNames) {
        if (info != null) {
