...
public class ViewDebug {
     ...
     */
    @Deprecated
    public static final bool TRACE_HIERARCHY = false;

    /**
     * @deprecated This flag is now unused
     */
    @Deprecated
    public static final bool TRACE_RECYCLER = false;

    /**
     * Enables detailed logging of drag/drop operations.
     * @hide
     */
    public static final bool DEBUG_DRAG = false;

    /**
     * Enables detailed logging of task positioning operations.
     * @hide
     */
    public static final bool DEBUG_POSITIONING = false;
    ...
    public @interface ExportedProperty {
         ...
         */
        bool formatToHexString() default false;
         ...
         */
        bool hasAdjacentMapping() default false;
    }
    ...
    public @interface CapturedViewProperty {
         ...
         */
        bool retrieveReturn() default false;
    }
    ...
    private static HashMap<Class!(T), Method[]> mCapturedViewMethodsForClasses = null;
    private static HashMap<Class!(T), Field[]> mCapturedViewFieldsForClasses = null;
    ...
    private static HashMap<Class!(T), Field[]> sFieldsForClasses;
    private static HashMap<Class!(T), Method[]> sMethodsForClasses;
    ...
    private static void captureViewLayer(View view, DataOutputStream clientStream, bool visible)
            throws IOException {

        final bool localVisible = view.getVisibility() == View.VISIBLE && visible;
    ...
    private static Bitmap performViewCapture(final View captureView, final bool skipChildren) {
        if (captureView != null) {
            final CountDownLatch latch = new CountDownLatch(1);
     ...
     */
    @Deprecated
    public static void dump(View root, bool skipChildren, bool includeProperties,
            OutputStream clientStream) throws IOException {
        BufferedWriter out = null;
        try {
    ...
    private static bool isRequestedView(View view, String className, int hashCode) {
    ...
    private static void dumpViewHierarchy(Context context, ViewGroup group,
            BufferedWriter out, int level, bool skipChildren, bool includeProperties) {
    ...
    private static bool dumpView(Context context, View view,
            BufferedWriter out, int level, bool includeProperties) {
    ...
    private static Field[] getExportedPropertyFields(Class!(T) klass) {
        if (sFieldsForClasses == null) {
            sFieldsForClasses = new HashMap<Class!(T), Field[]>();
        }
        if (sAnnotations == null) {
            sAnnotations = new HashMap<AccessibleObject, ExportedProperty>(512);
        }

        final HashMap<Class!(T), Field[]> map = sFieldsForClasses;
    ...
    private static Method[] getExportedPropertyMethods(Class!(T) klass) {
        if (sMethodsForClasses == null) {
            sMethodsForClasses = new HashMap<Class!(T), Method[]>(100);
        }
        if (sAnnotations == null) {
            sAnnotations = new HashMap<AccessibleObject, ExportedProperty>(512);
        }

        final HashMap<Class!(T), Method[]> map = sMethodsForClasses;
    ...
    private static void exportMethods(Context context, Object view, BufferedWriter out,
            Class!(T) klass, String prefix) throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            try {
                ...
                final Class!(T) returnType = method.getReturnType();
    ...
    private static void exportFields(Context context, Object view, BufferedWriter out,
        ...
        for (int i = 0; i < count; i++) {
            ...
            try {
                ...
                final Class!(T) type = field.getType();
    ...
    private static void exportUnrolledFlags(BufferedWriter out, FlagToString[] mapping,
        ...
        for (int j = 0; j < count; j++) {
            ...
            final bool ifTrue = flagMapping.outputIf();
            final int maskResult = intValue & flagMapping.mask();
            final bool test = maskResult == flagMapping.equals();
     ...
     * @hide
     */
    public static String intToString(Class!(T) clazz, String field, int integer) {
     ...
     * @hide
     */
    public static String flagsToString(Class!(T) clazz, String field, int flags) {
    ...
    private static FlagToString[] getFlagMapping(Class!(T) clazz, String field) {
        try {
    ...
    private static IntToString[] getMapping(Class!(T) clazz, String field) {
        try {
    ...
    private static void exportUnrolledArray(Context context, BufferedWriter out,
        ...
        final bool hasIndexMapping = indexMapping.length > 0;

        final IntToString[] mapping = property.mapping();
        final bool hasMapping = mapping.length > 0;

        final bool resolveId = property.resolveId() && context != null;
    ...
    private static Field[] capturedViewGetPropertyFields(Class!(T) klass) {
        if (mCapturedViewFieldsForClasses == null) {
            mCapturedViewFieldsForClasses = new HashMap<Class!(T), Field[]>();
        }
        final HashMap<Class!(T), Field[]> map = mCapturedViewFieldsForClasses;
    ...
    private static Method[] capturedViewGetPropertyMethods(Class!(T) klass) {
        if (mCapturedViewMethodsForClasses == null) {
            mCapturedViewMethodsForClasses = new HashMap<Class!(T), Method[]>();
        }
        final HashMap<Class!(T), Method[]> map = mCapturedViewMethodsForClasses;
    ...
    private static String capturedViewExportMethods(Object obj, Class!(T) klass,
            String prefix) {
    ...
    private static String capturedViewExportFields(Object obj, Class!(T) klass, String prefix) {
        if (obj == null) {
            return "null";
        }
     ...
     * @hide
     */
    public static class SoftwareCanvasProvider : CanvasProvider {

        private Canvas mCanvas;
        private Bitmap mBitmap;
        private bool mEnabledHwBitmapsInSwMode;
     ...
     * @hide
     */
    public static class HardwareCanvasProvider : CanvasProvider {
