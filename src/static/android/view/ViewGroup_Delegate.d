...
public class ViewGroup_Delegate {
     ...
     */
    @LayoutlibDelegate
    /*package*/ static bool drawChild(ViewGroup thisVG, Canvas canvas, View child,
            long drawingTime) {
    ...
    private static int transformCanvas(ViewGroup thisVG, Canvas canvas, View child) {
        ...
        final bool childHasIdentityMatrix = child.hasIdentityMatrix();
        ...
        if ((flags & ViewGroup.FLAG_SUPPORT_STATIC_TRANSFORMATIONS) != 0) {
            ...
            final bool hasTransform = thisVG.getChildStaticTransformation(child, t);
            if (hasTransform) {
