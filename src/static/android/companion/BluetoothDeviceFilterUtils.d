...
public class BluetoothDeviceFilterUtils {
    ...
    static bool matches(ScanFilter filter, BluetoothDevice device) {
        bool result = matchesAddress(filter.getDeviceAddress(), device)
    ...
    static bool matchesAddress(String deviceAddress, BluetoothDevice device) {
        final bool result = deviceAddress == null
                || (device != null && deviceAddress.equals(device.getAddress()));
        if (DEBUG) debugLogMatchResult(result, device, deviceAddress);
        return result;
    }

    static bool matchesServiceUuids(List<ParcelUuid> serviceUuids,
    ...
    static bool matchesServiceUuid(ParcelUuid serviceUuid, ParcelUuid serviceUuidMask,
            BluetoothDevice device) {
        final bool result = serviceUuid == null ||
    ...
    static bool matchesName(@Nullable Pattern namePattern, BluetoothDevice device) {
        bool result;
        if (namePattern == null)  {
            result = true;
        } else if (device == null) {
    ...
    static bool matchesName(@Nullable Pattern namePattern, ScanResult device) {
        bool result;
        if (namePattern == null)  {
            result = true;
        } else if (device == null) {
    ...
    private static void debugLogMatchResult(
            bool result, BluetoothDevice device, Object criteria) {
        Log.i(LOG_TAG, getDeviceDisplayNameInternal(device) + (result ? " ~ " : " !~ ") + criteria);
    }

    private static void debugLogMatchResult(
            bool result, ScanResult device, Object criteria) {
        Log.i(LOG_TAG, getDeviceDisplayNameInternal(device) + (result ? " ~ " : " !~ ") + criteria);
    }
