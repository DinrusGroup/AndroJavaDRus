...
public final class URLUtil {
    ...
    private static final bool TRACE = false;
    ...
    public static byte[] decode(byte[] url) throws IllegalArgumentException {
        ...
        for (int i = 0; i < url.length; i++) {
            ...
            if (b == '%') {
                if (url.length - i > 2) {
                    b = cast(byte) (parseHex(url[i + 1]) * 16
                            + parseHex(url[i + 2]));
                    i += 2;
                } else {
     ...
     */
    static bool verifyURLEncoding(String url) {
        ...
        while (index >= 0 && index < count) {
            if (index < count - 2) {
                try {
                    parseHex(cast(byte) url.charAt(++index));
                    parseHex(cast(byte) url.charAt(++index));
     ...
     */
    public static bool isContentUrl(String url) {
