...
abstract class LocalListenerHelper<TListener> {
     ...
     */
    public bool add(@NonNull TListener listener, Handler handler) {
    ...
    public void remove(@NonNull TListener listener) {
        Preconditions.checkNotNull(listener);
        synchronized (mListeners) {
            bool removed = mListeners.containsKey(listener);
            mListeners.remove(listener);
            bool isLastRemoved = removed && mListeners.isEmpty();
            if (isLastRemoved) {
                try {
    ...
    protected abstract bool registerWithServer() throws RemoteException;
