...
public class ProgressBar : View {
    ...
    private int mMin;
    private bool mMinInitialized;
    private int mMax;
    private bool mMaxInitialized;
    ...
    private bool mRefreshIsPosted;

    /** Value used to track progress animation, in the range [0...1]. */
    private float mVisualProgress;

    bool mMirrorForRtl = false;

    private bool mAggregatedIsVisible;
     ...
     */
    private static bool needsTileify(Drawable dr) {
     ...
     */
    private Drawable tileify(Drawable drawable, bool clip) {
        // TODO: This is a terrible idea that potentially destroys any drawable
        // that : any of these classes. We *really* need to remove this.
    ...
    public synchronized bool isIndeterminate() {
    ...
    public synchronized void setIndeterminate(bool indeterminate) {
     ...
     */
    @Nullable
    private Drawable getTintTarget(int layerId, bool shouldFallback) {
        Drawable layer = null;
    ...
    private class RefreshProgressRunnable : Runnable {
        public void run() {
    ...
    private static class RefreshData {
        ...
        public static RefreshData obtain(int id, int progress, bool fromUser, bool animate) {
    ...
    private synchronized void doRefreshProgress(int id, int progress, bool fromUser,
            bool callBackToApp, bool animate) {
        int range = mMax - mMin;
        final float scale = range > 0 ? (progress - mMin) / (float) range : 0;
        final bool isPrimary = id == R.id.progress;
    ...
    void onProgressRefresh(float scale, bool fromUser, int progress) {
    ...
    private synchronized void refreshProgress(int id, int progress, bool fromUser,
            bool animate) {
     ...
     */
    public void setProgress(int progress, bool animate) {
        setProgressInternal(progress, false, animate);
    }

    @android.view.RemotableViewMethod
    synchronized bool setProgressInternal(int progress, bool fromUser, bool animate) {
        if (mIndeterminate) {
            // Not applicable.
            return false;
        }
     ...
     */
    private class AccessibilityEventSender : Runnable {
        public void run() {
     ...
     * handled by the {@link ProgressBar#setProgress(int, bool)} method. This does
