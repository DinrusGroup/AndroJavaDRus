 ...
 * @hide
 */
public class RadialTimePickerView : View {
    ...
    private bool mIs24HourMode;
    private bool mShowHours;
    ...
    interface OnValueSelectedListener {
         ...
         *                    {@link #MINUTES}.
         */
        void onValueSelected(@PickerType int pickerType, int newValue, bool autoAdvance);
    }
    ...
    public void initialize(int hour, int minute, bool is24HourMode) {
    ...
    public void setCurrentItemShowing(int item, bool animate) {
        switch (item){
            case HOURS:
                showHours(animate);
                break;
     ...
     */
    private void setCurrentHourInternal(int hour, bool callback, bool autoAdvance) {
        ...
        final bool isOnInnerCircle = getInnerCircleForHour(hour);
    ...
    private int getHourForDegrees(int degrees, bool innerCircle) {
     ...
     * @param hour the hour in 24-hour time or 12-hour time
     */
    private bool getInnerCircleForHour(int hour) {
    ...
    private void setCurrentMinuteInternal(int minute, bool callback) {
     ...
     */
    public bool setAmOrPm(int amOrPm) {
    ...
    public void showHours(bool animate) {
        showPicker(true, animate);
    }

    public void showMinutes(bool animate) {
    ...
    private void showPicker(bool hours, bool animate) {
        if (mShowHours == hours) {
            return;
        }
    ...
    private void animatePicker(bool hoursToMinutes, long duration) {
    ...
    private void drawHoursClipped(Canvas canvas, int hoursAlpha, bool showActivated) {
        // Draw outer hours.
    ...
    private void drawMinutesClipped(Canvas canvas, int minutesAlpha, bool showActivated) {
    ...
    private void drawTextElements(Canvas canvas, float textSize, Typeface typeface,
        ...
        for (int i = 0; i < 12; i++) {
            final bool activated = (activatedFloor == i || activatedCeil == i);
    ...
    private int getDegreesFromXY(float x, float y, bool constrainOutside) {
    ...
    private bool getInnerCircleFromXY(float x, float y) {
        if (mIs24HourMode && mShowHours) {
            final double dX = x - mXCenter;
            final double dY = y - mYCenter;
            final double distFromCenter = Math.sqrt(dX * dX + dY * dY);
    ...
    private bool handleTouchInput(
            float x, float y, bool forceSelection, bool autoAdvance) {
        final bool isOnInnerCircle = getInnerCircleFromXY(x, y);
    ...
    private class RadialPickerTouchHelper : ExploreByTouchHelper {
        ...
        protected void onPopulateNodeForVirtualView(int virtualViewId, AccessibilityNodeInfo node) {
            ...
            final bool selected = isVirtualViewSelected(type, value);
        ...
        override
        protected bool onPerformActionForVirtualView(int virtualViewId, int action,
                Bundle arguments) {
        ...
        private void getBoundsForVirtualView(int virtualViewId, Rect bounds) {
            ...
            final float degrees;
            if (type == TYPE_HOUR) {
                final bool innerCircle = getInnerCircleForHour(value);
                if (innerCircle) {
        ...
        private bool isVirtualViewSelected(int type, int value) {
            final bool selected;
            if (type == TYPE_HOUR) {
