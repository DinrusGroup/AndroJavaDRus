...
class FastScroller {
    ...
    private bool mShowingPrimary;

    /** Whether we're waiting for completion of scrollTo(). */
    private bool mScrollCompleted;
     ...
     * @see #setAlwaysShow(bool)
     */
    public bool isAlwaysShowEnabled() {
        return mAlwaysShow;
    }
     ...
     */
    private void onStateDependencyChanged(bool peekIfEnabled) {
        if (isEnabled()) {
    ...
    public void onItemCountChanged(int childCount, int itemCount) {
        if (mOldItemCount != itemCount || mOldChildCount != childCount) {
            ...
            final bool hasMoreItems = itemCount - childCount > 0;
    ...
    private void updateLongList(int childCount, int itemCount) {
        final bool longList = childCount > 0 && itemCount / childCount >= MIN_PAGES;
    ...
    private void refreshDrawablePressedState() {
        final bool isPressed = mState == STATE_DRAGGING;
    ...
    public void onScroll(int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        ...
        final bool hasMoreItems = totalItemCount - visibleItemCount > 0;
        if (hasMoreItems && mState != STATE_DRAGGING) {
    ...
    private void scrollTo(float position) {
        ...
        if (mCurrentSection != sectionIndex) {
            ...
            final bool hasPreview = transitionPreviewLayout(sectionIndex);
     ...
     */
    private bool transitionPreviewLayout(int sectionIndex) {
        final Object[] sections = mSections;
        String text = null;
    ...
    private float getPosFromItemCount(
        ...
        final bool hasSections = sectionIndexer != null && mSections != null
                && mSections.length > 0;
    ...
    public bool onInterceptHoverEvent(MotionEvent ev) {
        if (!isEnabled()) {
            return false;
        }
    ...
    public bool onTouchEvent(MotionEvent me) {
        if (!isEnabled()) {
            return false;
        }
    ...
    private bool isPointInsideX(float x) {
    ...
    private bool isPointInsideY(float y) {
