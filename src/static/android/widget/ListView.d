 ...
 * {@code getView(...)}:</p>
 *
 *  <pre>private class MyAdapter : BaseAdapter {
 *
...
public class ListView : AbsListView {
    ...
    public bool requestChildRectangleOnScreen(View child, Rect rect, bool immediate) {
        ...
        final bool scroll = scrollYDelta != 0;
        if (scroll) {
    ...
    private View fillFromSelection(int selectedTop, int childrenTop, int childrenBottom) {
        ...
        // Some of the newly selected item : below the bottom of the list
    ...
    private View moveSelection(View oldSel, View newSel, int delta, int childrenTop,
        ...
        if (delta > 0) {
            ...
            // Some of the newly selected item : above the top of the list
    ...
    private class FocusSelector : Runnable {
    ...
    protected bool recycleOnMeasure() {
        return true;
    }
    ...
    final int measureHeightOfChildren(int widthMeasureSpec, int startPosition, int endPosition,
        ...
        final bool recyle = recycleOnMeasure();
        final bool[] isScrap = mIsScrap;
    ...
    protected void layoutChildren() {
        final bool blockLayoutRequests = mBlockLayoutRequests;
        ...
        try {
            ...
            if (sel != null) {
                ...
                if (mItemsCanFocus && hasFocus() && !sel.hasFocus()) {
                    final bool focusWasTaken = (sel == focusLayoutRestoreDirectChild &&
                ...
                final bool inTouchMode = mTouchMode == TOUCH_MODE_TAP
    ...
    bool trackMotionScroll(int deltaY, int incrementalDeltaY) {
        final bool result = super.trackMotionScroll(deltaY, incrementalDeltaY);
        removeUnusedFixedViews(mHeaderViewInfos);
        removeUnusedFixedViews(mFooterViewInfos);
        return result;
    }
     ...
     */
    private bool isDirectChildHeaderOrFooter(View child) {
     ...
     */
    private View makeAndAddView(int position, int y, bool flow, int childrenLeft,
            bool selected) {
        if (!mDataChanged) {
     ...
     */
    private void setupChild(View child, int position, int y, bool flowDown, int childrenLeft,
            bool selected, bool isAttachedToWindow) {
        Trace.traceBegin(Trace.TRACE_TAG_VIEW, "setupListItem");

        final bool isSelected = selected && shouldShowSelector();
        final bool updateChildSelected = isSelected != child.isSelected();
        final int mode = mTouchMode;
        final bool isPressed = mode > TOUCH_MODE_DOWN && mode < TOUCH_MODE_SCROLL
                && mMotionPosition == position;
        final bool updateChildPressed = isPressed != child.isPressed();
        final bool needToMeasure = !isAttachedToWindow || updateChildSelected
    ...
    public bool dispatchKeyEvent(KeyEvent event) {
        ...
        bool handled = super.dispatchKeyEvent(event);
        if (!handled) {
    ...
    bool arrowScroll(int direction) {
        try {
            mInLayout = true;
            final bool handled = arrowScrollImpl(direction);
            if (handled) {
    ...
    private ArrowScrollFocusResult arrowScrollFocused(final int direction) {
        ...
        if (selectedView != null && selectedView.hasFocus()) {
            ...
            if (direction == View.FOCUS_DOWN) {
                final bool topFadingEdgeShowing = (mFirstPosition > 0);
                final int listTop = mListPadding.top +
                ...
                final bool bottomFadingEdgeShowing =
     ...
     */
    private bool isViewAncestorOf(View child, View parent) {
        if (child == parent) {
            return true;
        }
     ...
     */
    public void setItemsCanFocus(bool itemsCanFocus) {
    ...
    public void setCacheColorHint(int color) {
        final bool opaque = (color >>> 24) == 0xFF;
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        final bool drawOverscrollHeader = overscrollHeader != null;
        final bool drawOverscrollFooter = overscrollFooter != null;
        final bool drawDividers = dividerHeight > 0 && mDivider != null;
        ...
        if (drawDividers || drawOverscrollHeader || drawOverscrollFooter) {
            ...
            final bool headerDividers = mHeaderDividersEnabled;
            final bool footerDividers = mFooterDividersEnabled;
            final int first = mFirstPosition;
            final bool areAllItemsSelectable = mAreAllItemsSelectable;
            final ListAdapter adapter = mAdapter;
            // If the list is opaque *and* the background is not, we want to
            // fill a rect where the dividers would be for non-selectable items
            // If the list is opaque and the background is also opaque, we don't
            // need to draw anything since the background will do it for us
            final bool fillForMissingDividers = isOpaque() && !super.isOpaque();
            ...
            if (!mStackFromBottom) {
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    final bool isHeader = (itemIndex < headerCount);
                    final bool isFooter = (itemIndex >= footerLimit);
                    if ((headerDividers || !isHeader) && (footerDividers || !isFooter)) {
                        final View child = getChildAt(i);
                        bottom = child.getBottom();
                        final bool isLastItem = (i == (count - 1));
                ...
                for (int i = start; i < count; i++) {
                    ...
                    if ((headerDividers || !isHeader) && (footerDividers || !isFooter)) {
                        ...
                        if (drawDividers && (top > effectivePaddingTop)) {
                            final bool isFirstItem = (i == start);
    ...
    protected bool drawChild(Canvas canvas, View child, long drawingTime) {
        bool more = super.drawChild(canvas, child, drawingTime);
     ...
     *
     * @see #setFooterDividersEnabled(bool)
     * @see #areHeaderDividersEnabled()
     * @see #addHeaderView(android.view.View)
     */
    public void setHeaderDividersEnabled(bool headerDividersEnabled) {
     ...
     *
     * @see #setHeaderDividersEnabled(bool)
     */
     ...
     */
    public void setFooterDividersEnabled(bool footerDividersEnabled) {
     ...
     */
    override
    protected <T : View> T findViewByPredicateTraversal(
    ...
    private bool shouldAdjustHeightForDivider(int itemIndex) {
        final int dividerHeight = mDividerHeight;
        final Drawable overscrollHeader = mOverScrollHeader;
        final Drawable overscrollFooter = mOverScrollFooter;
        final bool drawOverscrollHeader = overscrollHeader != null;
        ...
        if (drawDividers) {
            ...
            if ((headerDividers || !isHeader) && (footerDividers || !isFooter)) {
                final ListAdapter adapter = mAdapter;
                if (!mStackFromBottom) {
                    final bool isLastItem = (itemIndex == (itemCount - 1));
                    ...
                    final int start = drawOverscrollHeader ? 1 : 0;
                    final bool isFirstItem = (itemIndex == start);
                    if (!isFirstItem) {
                        final int previousIndex = (itemIndex - 1);
                        // Draw dividers between enabled items, headers
                        // and/or footers when enabled and requested, and
    ...
    public void onInitializeAccessibilityNodeInfoForItem(
        ...
        final LayoutParams lp = (LayoutParams) view.getLayoutParams();
        final bool isHeading = lp != null && lp.viewType == ITEM_VIEW_TYPE_HEADER_OR_FOOTER;
        final bool isSelected = isItemChecked(position);
