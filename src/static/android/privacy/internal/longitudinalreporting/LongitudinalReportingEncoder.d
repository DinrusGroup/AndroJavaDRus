 ...
 * @hide
 */
public class LongitudinalReportingEncoder : DifferentialPrivacyEncoder {
    ...
    private LongitudinalReportingEncoder(LongitudinalReportingConfig config,
            bool secureEncoder, byte[] userSecret) {
        mConfig = config;
        mIsSecure = secureEncoder;
        final bool ignoreOriginalInput = getLongTermRandomizedResult(config.getProbabilityP(),
    ...
    override
    public byte[] encodeBoolean(bool original) {
        if (DEBUG) {
    ...
    override
    public bool isInsecureEncoderForTest() {
        return !mIsSecure;
    }
     ...
     */
    @VisibleForTesting
    public static bool getLongTermRandomizedResult(double p, bool secureEncoder,
        ...
        final bool prrInput = p < 0.5f ? false : true;
