...
//  public static final long FIELD_TYPE_GROUP = 10L << FIELD_TYPE_SHIFT; // Deprecated.
    ...
    /**
     * Write a bool value for the given fieldId.
     *
    ...
    public void write(long fieldId, bool val) {
        ...
        switch ((int)((fieldId & (FIELD_TYPE_MASK | FIELD_COUNT_MASK)) >> FIELD_TYPE_SHIFT)) {
            ...
            default: {
                throw new IllegalArgumentException("Attempt to call write(long, bool) with "
                        + getFieldIdString(fieldId));
            }
        }
    }
     ...
     */
    @Deprecated
    public void writeBool(long fieldId, bool val) {
        assertNotCompacted();
    ...
    private void writeBoolImpl(int id, bool val) {
        if (val) {
            writeTag(id, WIRE_TYPE_VARINT);
            // 0 and 1 are the same as their varint counterparts
            mBuffer.writeRawByte(cast(byte)1);
        }
    }
     ...
     */
    @Deprecated
    public void writeRepeatedBool(long fieldId, bool val) {
        assertNotCompacted();
    ...
    private void writeRepeatedBoolImpl(int id, bool val) {
        writeTag(id, WIRE_TYPE_VARINT);
        mBuffer.writeRawByte(cast(byte)(val ? 1 : 0));
    }
     ...
     */
    @Deprecated
    public void writePackedBool(long fieldId, bool[] val) {
        assertNotCompacted();
        ...
        if (N > 0) {
            ...
            for (int i=0; i<N; i++) {
                // 0 and 1 are the same as their varint counterparts
                mBuffer.writeRawByte(cast(byte)(val[i] ? 1 : 0));
            }
        }
    }
     ...
     */
    // VisibleForTesting
    public static long makeToken(int tagSize, bool repeated, int depth, int objectId,
            int sizePos) {
     ...
     */
    public static bool getRepeatedFromToken(long token) {
     ...
     */
    private long startObjectImpl(final int id, bool repeated) {
     ...
     */
    private void endObjectImpl(long token, bool repeated) {
        ...
        final bool expectedRepeated = getRepeatedFromToken(token);
