...
public final class EncodedBuffer {
    ...
    public void writeRawVarint32(int val) {
        while (true) {
            if ((val & ~0x7F) == 0) {
                writeRawByte(cast(byte)val);
                return;
            } else {
                writeRawByte(cast(byte)((val & 0x7F) | 0x80));
                val >>>= 7;
            }
        }
    }
    ...
    public void writeRawFixed32(int val) {
        writeRawByte(cast(byte)(val));
        writeRawByte(cast(byte)(val >> 8));
        writeRawByte(cast(byte)(val >> 16));
        writeRawByte(cast(byte)(val >> 24));
    }
    ...
    public void writeRawFixed64(long val) {
        ...
        writeRawByte(cast(byte)(val >> 16));
        writeRawByte(cast(byte)(val >> 24));
        writeRawByte(cast(byte)(val >> 32));
        writeRawByte(cast(byte)(val >> 40));
        writeRawByte(cast(byte)(val >> 48));
        writeRawByte(cast(byte)(val >> 56));
    }
    ...
    public void editRawFixed32(int pos, int val) {
        mBuffers.get(pos / mChunkSize)[pos % mChunkSize] = cast(byte)(val);
        mBuffers.get((pos+1) / mChunkSize)[(pos+1) % mChunkSize] = cast(byte)(val >> 8);
        mBuffers.get((pos+2) / mChunkSize)[(pos+2) % mChunkSize] = cast(byte)(val >> 16);
        mBuffers.get((pos+3) / mChunkSize)[(pos+3) % mChunkSize] = cast(byte)(val >> 24);
    }
    ...
    private static int dumpByteString(String tag, String prefix, int start, byte[] buf) {
        ...
        for (i=0; i<length; i++) {
            ...
            byte b = buf[i];
            byte c = cast(byte)((b >> 4) & 0x0f);
            if (c < 10) {
                sb.append((char)('0' + c));
            } else {
                sb.append((char)('a' - 10 + c));
            }
            byte d = cast(byte)(b & 0x0f);
            if (d < 10) {
                sb.append((char)('0' + d));
            } else {
