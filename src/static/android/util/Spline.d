...
public abstract class Spline {
    ...
    private static bool isStrictlyIncreasing(float[] x) {
    ...
    private static bool isMonotonic(float[] x) {
        if (x == null || x.length < 2) {
            throw new IllegalArgumentException("There must be at least two control points.");
        }
        float prev = x[0];
    ...
    public static class MonotoneCubicSpline : Spline {
        private float[] mX;
        private float[] mY;
        private float[] mM;
    ...
    public static class LinearSpline : Spline {
