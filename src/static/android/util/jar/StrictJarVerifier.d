...
class StrictJarVerifier {
    ...
    private final bool signatureSchemeRollbackProtectionsEnforced;
     ...
     * matches the hash.
     */
    static class VerifierEntry : OutputStream {
    ...
    StrictJarVerifier(String name, StrictJarManifest manifest,
        HashMap<String, byte[]> metaEntries, bool signatureSchemeRollbackProtectionsEnforced) {
        jarName = name;
     ...
     */
    synchronized bool readCertificates() {
   ...
   /**
    ...
    private void verifyCertificate(String certFile) {
        ...
        if (signatureSchemeRollbackProtectionsEnforced) {
            ...
            if (apkSignatureSchemeIdList != null) {
                ...
                bool v2SignatureGenerated = false;
                bool v3SignatureGenerated = false;
    ...
    /**
     * Returns a <code>bool</code> indication of whether or not the
    ...
    private bool verify(Attributes attributes, String entry, byte[] data,
            int start, int end, bool ignoreSecondEndline, bool ignorable) {
        for (int i = 0; i < DIGEST_ALGORITHMS.length; i++) {
    ...
    private static bool verifyMessageDigest(byte[] expected, byte[] encodedActual) {
        byte[] actual;
        try {
