 ...
 **/
public class ScriptIntrinsicLUT : ScriptIntrinsic {
    ...
    public static ScriptIntrinsicLUT create(RenderScript rs, Element e) {
        long id;
        bool mUseIncSupp = rs.isUseNative() &&
        ...
        for (int ct=0; ct < 256; ct++) {
            si.mCache[ct] = cast(byte)ct;
            si.mCache[ct + 256] = cast(byte)ct;
            si.mCache[ct + 512] = cast(byte)ct;
            si.mCache[ct + 768] = cast(byte)ct;
        }
    ...
    public void setGreen(int index, int value) {
        validate(index, value);
        mCache[index+256] = cast(byte)value;
        mDirty = true;
    }
    ...
    public void setBlue(int index, int value) {
        validate(index, value);
        mCache[index+512] = cast(byte)value;
        mDirty = true;
    }
    ...
    public void setAlpha(int index, int value) {
        validate(index, value);
        mCache[index+768] = cast(byte)value;
        mDirty = true;
    }
