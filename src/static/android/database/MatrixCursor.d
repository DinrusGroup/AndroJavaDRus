...
public class MatrixCursor : AbstractCursor {
     ...
     *  at cursor construction time
     */
    public void addRow(Iterable!(T) columnValues) {
        int start = rowCount * columnCount;
        int end = start + columnCount;
        ensureCapacity(end);

        if (columnValues instanceof ArrayList!(T)) {
            addRow((ArrayList!(T)) columnValues, start);
            return;
        }
    ...
    private void addRow(ArrayList!(T) columnValues, int start) {
