 ...
 * @hide
 */
public final class SQLiteConnectionPool : Closeable {
     ...
     * Setting this flag essentially : the old "db lock" concept by preventing
    ...
    public void reconfigure(SQLiteDatabaseConfiguration configuration) {
        ...
        synchronized (mLock) {
            ...
            bool onlyCompatWalChanged = (mConfiguration.openFlags ^ configuration.openFlags)
    ...
    // Can't throw.
    @GuardedBy("mLock")
    private bool recycleConnectionLocked(SQLiteConnection connection,
     ...
     * from this pool or if it has already been released.
     */
    public bool shouldYieldConnection(SQLiteConnection connection, int connectionFlags) {
        synchronized (mLock) {
    ...
    // Can't throw.
    @GuardedBy("mLock")
    private bool closeAvailableConnectionLocked(int connectionId) {
    ...
    private SQLiteConnection waitForConnection(String sql, int connectionFlags,
            CancellationSignal cancellationSignal) {
        final bool wantPrimaryConnection =
    ...
    private void wakeConnectionWaitersLocked() {
        ...
        bool primaryConnectionNotAvailable = false;
        bool nonPrimaryConnectionNotAvailable = false;
        while (waiter != null) {
            bool unpark = false;
            if (!mIsOpen) {
                unpark = true;
    ...
    private void finishAcquireConnectionLocked(SQLiteConnection connection, int connectionFlags) {
        try {
            final bool readOnly = (connectionFlags & CONNECTION_FLAG_READ_ONLY) != 0;
    ...
    private bool isSessionBlockingImportantConnectionWaitersLocked(
            bool holdingPrimaryConnection, int connectionFlags) {
        ConnectionWaiter waiter = mConnectionWaiterQueue;
        if (waiter != null) {
            final int priority = getPriority(connectionFlags);
            do {
    ...
    private ConnectionWaiter obtainConnectionWaiterLocked(Thread thread, long startTime,
            int priority, bool wantPrimaryConnection, String sql, int connectionFlags) {
    ...
    private static final class ConnectionWaiter {
        ...
        public bool mWantPrimaryConnection;
        public String mSql;
    ...
    private class IdleConnectionHandler : Handler {
