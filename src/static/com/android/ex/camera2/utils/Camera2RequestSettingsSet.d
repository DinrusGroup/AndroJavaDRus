...
public class Camera2RequestSettingsSet {
    private final Map<Key!(T), Object> mDictionary;
     ...
     */
    public <T> bool set(Key<T> key, T value) {
        if (key == null) {
     ...
     * @throws NullPointerException If {@code key} is {@code null}.
     */
    public bool contains(Key!(T) key) {
        if (key == null) {
            throw new NullPointerException("Received a null key");
        }
     ...
     */
    public <T> bool matches(Key<T> key, T value) {
     ...
     */
    public bool union(Camera2RequestSettingsSet moreSettings) {
    ...
    public CaptureRequest createRequest(CameraDevice camera, int template, Surface... targets)
        ...
        for (Key!(T) key : mDictionary.keySet()) {
