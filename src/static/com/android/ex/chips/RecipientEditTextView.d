...
public class RecipientEditTextView : MultiAutoCompleteTextView :
    ...
    private bool mNoChipMode = false;
    private bool mShouldShrink = true;
    private bool mRequiresShrinkWhenNotGone = false;
    ...
    public RecipientEditTextView(Context context, AttributeSet attrs) {
        ...
        mAlternatesListener = new OnItemClickListener() {
            override
            public void onItemClick(AdapterView!(T) adapterView,View view, int position,
                    long rowId) {
    ...
    override
    public bool onEditorAction(TextView view, int action, KeyEvent keyEvent) {
    ...
    override
    public void onFocusChanged(bool hasFocus, int direction, Rect previous) {
    ...
    override
    public <T : ListAdapter & Filterable> void setAdapter(@NonNull T adapter) {
    ...
    private void announceForAccessibilityCompat(String text) {
        ...
        final bool isAccessibilityOn = accessibilityManager.isEnabled();
    ...
    private ChipBitmapContainer createChipBitmap(RecipientEntry contact, TextPaint paint,
        ...
        bool displayIcon = contact.isValid() && contact.shouldDisplayIcon();
        int iconWidth = displayIcon ?
                height - backgroundPadding.top - backgroundPadding.bottom : 0;

        final bool shouldDisplayWarningIcon = mUntrustedAddresses.contains(
    ...
    private bool shouldPositionAvatarOnRight() {
        final bool isRtl = Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1 &&
                getLayoutDirection() == LAYOUT_DIRECTION_RTL;
        final bool assignedPosition = mAvatarPosition == AVATAR_POSITION_END;
     ...
     */
    override
    public bool onKeyPreIme(int keyCode, @NonNull KeyEvent event) {
    ...
    private bool commitChip(int start, int end, Editable editable) {
    ...
    private bool isEntryAtPositionTypePerson(int position) {
    ...
    public bool onKeyDown(int keyCode, @NonNull KeyEvent event) {
        ...
        bool isHandled = super.onKeyDown(keyCode, event);
    ...
    // Visible for testing.
    /*package*/ bool isCompletedToken(CharSequence text) {
        if (TextUtils.isEmpty(text)) {
            return false;
        }
    ...
    public bool onTouchEvent(@NonNull MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_UP) {
            bool touchedWarningIcon = touchedWarningIcon(x, y, currentChip);
    ...
    private bool touchedWarningIcon(float x, float y, DrawableRecipientChip currentChip) {
        bool touchedWarningIcon = false;
        if (currentChip != null) {
    ...
    private class RecipientTextWatcher : TextWatcher {
    ...
    void handlePasteClip(ClipData clip) {
        ...
        bool containsSupportedType = clipDesc.hasMimeType(ClipDescription.MIMETYPE_TEXT_PLAIN)
        ...
        for (int i = 0; i < clip.getItemCount(); i++) {
            ...
            final bool supportedType = ClipDescription.MIMETYPE_TEXT_PLAIN.equals(mimeType)
    ...
    private class RecipientReplacementTask : AsyncTask<Void, Void, Void> {
    ...
    private class IndividualReplacementTask
            : AsyncTask<ArrayList<DrawableRecipientChip>, Void, Void> {
        override
     ...
     */
    private class MoreImageSpan : ReplacementDrawableSpan {
     ...
     */
    override
    public bool onDragEvent(@NonNull DragEvent event) {
     ...
     */
    private final class RecipientChipShadow : DragShadowBuilder {
