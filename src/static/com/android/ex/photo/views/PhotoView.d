 ...
 */
public class PhotoView : View : OnGestureListener,
    ...
    /** Whether or not this is a still image of a video */
    private bool mVideoReady;

    /** Whether or not crop is allowed */
    private bool mAllowCrop;
    /** The crop region */
     ...
     */
    private bool mDoubleTapOccurred;
     ...
     */
    private bool mQuickScaleEnabled;
    ...
    private bool scale(MotionEvent e) {
        bool handled = false;
    ...
    override
    public bool interceptMoveLeft(float origX, float origY) {
        if (!mTransformsEnabled) {
    ...
    override
    public bool interceptMoveRight(float origX, float origY) {
        if (!mTransformsEnabled) {
            // Allow intercept if we're not in transform mode
            return false;
        } else if (mTranslateRunnable.mRunning) {
    ...
    public void bindPhoto(Bitmap photoBitmap) {
        bool currentDrawableIsBitmapDrawable = mDrawable instanceof BitmapDrawable;
        bool changed = !(currentDrawableIsBitmapDrawable);
     ...
     */
    public void setFullScreen(bool fullScreen, bool animate) {
     ...
     */
    public void enableAllowCrop(bool allowCrop) {
    ...
    override
    public bool verifyDrawable(Drawable drawable) {
    ...
    public void setFixedHeight(int fixedHeight) {
        final bool adjustBounds = (fixedHeight != mFixedHeight);
     ...
     */
    public void enableImageTransforms(bool enable) {
     ...
     */
    private void configureBounds(bool changed) {
        ...
        final bool fits = (dwidth < 0 || vwidth == dwidth) &&
    ...
    private int translate(float tx, float ty) {
        ...
        bool didTranslateX = translateX == tx;
        bool didTranslateY = translateY == ty;
     ...
     */
    private void rotate(float degrees, bool animate) {
        if (animate) {
     ...
     */
    private static class ScaleRunnable : Runnable {
         ...
         */
        public bool start(float startScale, float targetScale, float centerX, float centerY) {
            if (mRunning) {
                return false;
            }
     ...
     */
    private static class TranslateRunnable : Runnable {
         ...
         */
        public bool start(float velocityX, float velocityY) {
            if (mRunning) {
                return false;
            }
            mLastRunTime = NEVER;
     ...
     * Runnable that animates an image translation operation.
     */
    private static class SnapRunnable : Runnable {
         ...
         * Starts the animation.
         */
        public bool start(float translateX, float translateY) {
            if (mRunning) {
                return false;
            }
            mStartRunTime = NEVER;
     ...
     */
    private static class RotateRunnable : Runnable {
