...
public class BatterySaverStateMachine {
    ...
    private static final bool DEBUG = BatterySaverPolicy.DEBUG;
    ...
    @GuardedBy("mLock")
    private bool mBatteryStatusSet;
    ...
    @GuardedBy("mLock")
    private bool mIsBatteryLevelLow;

    /** Previously known value of Global.LOW_POWER_MODE. */
    @GuardedBy("mLock")
    private bool mSettingBatterySaverEnabled;

    /** Previously known value of Global.LOW_POWER_MODE_STICKY. */
    @GuardedBy("mLock")
    private bool mSettingBatterySaverEnabledSticky;
     ...
     */
    @GuardedBy("mLock")
    private bool mBatterySaverSnoozing;
    ...
    private bool isBatterySaverEnabled() {
        return mBatterySaverController.isEnabled();
    }

    private bool isAutoBatterySaverConfigured() {
    ...
    void refreshSettingsLocked() {
        final bool lowPowerModeEnabled = getGlobalSetting(
                Settings.Global.LOW_POWER_MODE, 0) != 0;
        final bool lowPowerModeEnabledSticky = getGlobalSetting(
     ...
     */
    @VisibleForTesting
    void setSettingsLocked(bool batterySaverEnabled, bool batterySaverEnabledSticky,
        ...
        final bool enabledChanged = mSettingBatterySaverEnabled != batterySaverEnabled;
        final bool stickyChanged =
                mSettingBatterySaverEnabledSticky != batterySaverEnabledSticky;
        final bool thresholdChanged
     ...
     */
    public void setBatteryStatus(bool newPowered, int newLevel, bool newBatteryLevelLow) {
        if (DEBUG) {
        ...
        synchronized (mLock) {
            ...
            final bool poweredChanged = mIsPowered != newPowered;
            final bool levelChanged = mBatteryLevel != newLevel;
            final bool lowChanged = mIsBatteryLevelLow != newBatteryLevelLow;
     ...
     */
    public void setBatterySaverEnabledManually(bool enabled) {
        if (DEBUG) {
     ...
     */
    private void enableBatterySaverLocked(bool enable, bool manual, int intReason,
            String strReason) {
        if (DEBUG) {
            Slog.d(TAG, "enableBatterySaver: enable=" + enable + " manual=" + manual
                    + " reason=" + strReason + "(" + intReason + ")");
        }
        final bool wasEnabled = mBatterySaverController.isEnabled();
    ...
    private void updateSnoozingLocked(bool snoozing, String reason) {
