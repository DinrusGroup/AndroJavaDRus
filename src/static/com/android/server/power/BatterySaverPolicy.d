 ...
 */
public class BatterySaverPolicy : ContentObserver {
    private static final String TAG = "BatterySaverPolicy";

    public static final bool DEBUG = false; // DO NOT SUBMIT WITH TRUE.
     ...
     */
    @GuardedBy("mLock")
    private bool mVibrationDisabledConfig;
     ...
     */
    @GuardedBy("mLock")
    private bool mVibrationDisabledEffective;
     ...
     */
    @GuardedBy("mLock")
    private bool mAnimationDisabled;
     ...
     */
    @GuardedBy("mLock")
    private bool mSoundTriggerDisabled;
     ...
     */
    @GuardedBy("mLock")
    private bool mFullBackupDeferred;
     ...
     */
    @GuardedBy("mLock")
    private bool mKeyValueBackupDeferred;
     ...
     */
    @GuardedBy("mLock")
    private bool mFireWallDisabled;
     ...
     */
    @GuardedBy("mLock")
    private bool mAdjustBrightnessDisabled;
     ...
     */
    @GuardedBy("mLock")
    private bool mDataSaverDisabled;

    /**
     * {@code true} if launch boost should be disabled on battery saver.
     */
    @GuardedBy("mLock")
    private bool mLaunchBoostDisabled;
     ...
     */
    @GuardedBy("mLock")
    private bool mForceAllAppsStandby;

    /**
     * Whether to put all apps in the stand-by mode.
     */
    @GuardedBy("mLock")
    private bool mForceBackgroundCheck;

    /**
     * Whether to show non-essential sensors (e.g. edge sensors) or not.
     */
    @GuardedBy("mLock")
    private bool mOptionalSensorsDisabled;

    /**
     * Whether AOD is enabled or not.
     */
    @GuardedBy("mLock")
    private bool mAodDisabled;

    /**
     * Whether BatterySavingStats should send tron events.
     */
    @GuardedBy("mLock")
    private bool mSendTronLog;
     ...
     */
    @GuardedBy("mLock")
    private bool mAccessibilityEnabled;
    ...
    public void systemReady() {
        ...
        final bool enabled = acm.isEnabled();
        synchronized (mLock) {
            mAccessibilityEnabled = enabled;
        }
        onChange(true, null);
     ...
     */
    public PowerSaveState getBatterySaverPolicy(@ServiceType int type, bool realMode) {
        synchronized (mLock) {
    ...
    public ArrayMap<String, String> getFileValues(bool interactive) {
        synchronized (mLock) {
            return interactive ? mFilesForInteractive : mFilesForNoninteractive;
        }
    }

    public bool isLaunchBoostDisabled() {
        synchronized (mLock) {
    ...
    @VisibleForTesting
    public void setAccessibilityEnabledForTest(bool enabled) {
        synchronized (mLock) {
            mAccessibilityEnabled = enabled;
        }
    }
