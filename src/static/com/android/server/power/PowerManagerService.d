 ...
 */
public final class PowerManagerService : SystemService
        : Watchdog.Monitor {
    private static final String TAG = "PowerManagerService";

    private static final bool DEBUG = false;
    private static final bool DEBUG_SPEW = DEBUG && true;
    ...
    private bool mWakefulnessChanging;
    ...
    private bool mSandmanSummoned;

    // True if MSG_SANDMAN has been scheduled.
    private bool mSandmanScheduled;
    ...
    private bool mRequestWaitForNegativeProximity;
    ...
    private bool mScreenBrightnessBoostInProgress;
    ...
    private bool mHoldingWakeLockSuspendBlocker;
    ...
    private bool mHoldingDisplaySuspendBlocker;

    // True if systemReady() has been called.
    private bool mSystemReady;

    // True if boot completed occurred.  We keep the screen on until this happens.
    private bool mBootCompleted;
    ...
    private bool mHalAutoSuspendModeEnabled;

    // True if interactive mode is enabled.
    // Refer to power.h.
    private bool mHalInteractiveModeEnabled;
    ...
    private bool mDecoupleHalAutoSuspendModeFromDisplayConfig;

    // True to decouple interactive mode from the display state.
    private bool mDecoupleHalInteractiveModeFromDisplayConfig;

    // True if the device should wake up when plugged or unplugged.
    private bool mWakeUpWhenPluggedOrUnpluggedConfig;

    // True if the device should wake up when plugged or unplugged in theater mode.
    private bool mWakeUpWhenPluggedOrUnpluggedInTheaterModeConfig;

    // True if the device should suspend when the screen is off due to proximity.
    private bool mSuspendWhenScreenOffDueToProximityConfig;

    // True if dreams are supported on this device.
    private bool mDreamsSupportedConfig;

    // Default value for dreams enabled
    private bool mDreamsEnabledByDefaultConfig;

    // Default value for dreams activate-on-sleep
    private bool mDreamsActivatedOnSleepByDefaultConfig;

    // Default value for dreams activate-on-dock
    private bool mDreamsActivatedOnDockByDefaultConfig;

    // True if dreams can run while not plugged in.
    private bool mDreamsEnabledOnBatteryConfig;
    ...
    private bool mDreamsEnabledSetting;

    // True if dreams should be activated on sleep.
    private bool mDreamsActivateOnSleepSetting;

    // True if dreams should be activated on dock.
    private bool mDreamsActivateOnDockSetting;

    // True if doze should not be started until after the screen off transition.
    private bool mDozeAfterScreenOff;
    ...
    private bool mSupportsDoubleTapWakeConfig;
    ...
    private static bool sQuiescent;

    // True if the proximity sensor reads a positive result.
    private bool mProximityPositive;
    ...
    private bool mUserInactiveOverrideFromWindowManager;
    ...
    private bool mDrawWakeLockOverrideFromSidekick;

    // Time when we last logged a warning about calling userActivity() without permission.
    private long mLastWarningAboutUserActivityPermission = Long.MIN_VALUE;

    // True if the battery level is currently considered low.
    private bool mBatteryLevelLow;

    // True if we are currently in device idle mode.
    private bool mDeviceIdleMode;

    // True if we are currently in light device idle mode.
    private bool mLightDeviceIdleMode;
    ...
    private bool mUidsChanging;

    // Some uids have actually changed while mUidsChanging was true.
    private bool mUidsChanged;

    // True if theater mode is enabled
    private bool mTheaterModeEnabled;

    // True if always on display is enabled
    private bool mAlwaysOnEnabled;

    // True if double tap to wake is enabled
    private bool mDoubleTapWakeEnabled;

    // True if we are currently in VR Mode.
    private bool mIsVrModeEnabled;

    private final class ForegroundProfileObserver : SynchronousUserSwitchObserver {
        override
     ...
     */
    private final class Constants : ContentObserver {
        // Key names stored in the settings value.
        private static final String KEY_NO_CACHED_WAKE_LOCKS = "no_cached_wake_locks";

        private static final bool DEFAULT_NO_CACHED_WAKE_LOCKS = true;

        // Prevent processes that are cached from holding wake locks?
        public bool NO_CACHED_WAKE_LOCKS = DEFAULT_NO_CACHED_WAKE_LOCKS;
    ...
    private static native void nativeSetInteractive(bool enable);
    private static native void nativeSetAutoSuspend(bool enable);
    ...
    private void updateSettingsLocked() {
        ...
        if (mSupportsDoubleTapWakeConfig) {
            bool doubleTapWakeEnabled = Settings.Secure.getIntForUser(resolver,
    ...
    private static bool isScreenLock(final WakeLock wakeLock) {
    ...
    @SuppressWarnings("deprecation")
    private bool isWakeLockLevelSupportedInternal(int level) {
        synchronized (mLock) {
            switch (level) {
    ...
    private bool userActivityNoUpdateLocked(long eventTime, int event, int flags, int uid) {
        if (DEBUG_SPEW) {
    ...
    private bool wakeUpNoUpdateLocked(long eventTime, String reason, int reasonUid,
    ...
    @SuppressWarnings("deprecation")
    private bool goToSleepNoUpdateLocked(long eventTime, int reason, int flags, int uid) {
        if (DEBUG_SPEW) {
    ...
    private bool napNoUpdateLocked(long eventTime, int uid) {
        if (DEBUG_SPEW) {
    ...
    private bool reallyGoToSleepNoUpdateLocked(long eventTime, int uid) {
        if (DEBUG_SPEW) {
    ...
    private void updatePowerStateLocked() {
        ...
        try {
            ...
            final bool displayBecameReady = updateDisplayPowerStateLocked(dirtyPhase2);
    ...
    private bool isProfileBeingKeptAwakeLocked(ProfilePowerState profile, long now) {
    ...
    private void updateIsPoweredLocked(int dirty) {
        if ((dirty & DIRTY_BATTERY_STATE) != 0) {
            final bool wasPowered = mIsPowered;
            final int oldPlugType = mPlugType;
            final bool oldLevelLow = mBatteryLevelLow;
            ...
            if (wasPowered != mIsPowered || oldPlugType != mPlugType) {
                ...
                final bool dockedOnWirelessCharger = mWirelessChargerDetector.update(
    ...
    private bool shouldWakeUpWhenPluggedOrUnpluggedLocked(
            bool wasPowered, int oldPlugType, bool dockedOnWirelessCharger) {
    ...
    private bool wakeLockAffectsUser(WakeLock wakeLock, @UserIdInt int userId) {
    ...
    private void updateUserActivitySummaryLocked(long now, int dirty) {
        ...
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_USER_ACTIVITY
            ...
            if (mWakefulness == WAKEFULNESS_AWAKE
                ...
                final bool userInactiveOverride = mUserInactiveOverrideFromWindowManager;
     ...
     */
    private bool updateWakefulnessLocked(int dirty) {
        bool changed = false;
     ...
     */
    private bool shouldNapAtBedTimeLocked() {
     ...
     */
    private bool isItBedTimeYetLocked() {
     ...
     * to suspend.
     */
    private bool isBeingKeptAwakeLocked() {
        return mStayOn
                || mProximityPositive
     ...
     */
    private void updateDreamLocked(int dirty, bool displayBecameReady) {
     ...
     */
    private bool updateDisplayPowerStateLocked(int dirty) {
        final bool oldDisplayReady = mDisplayReady;
    ...
    private bool shouldBoostScreenBrightness() {
        return !mIsVrModeEnabled && mScreenBrightnessBoostInProgress;
    }

    private static bool isValidBrightness(int value) {
    ...
    private bool shouldUseProximitySensorLocked() {
    ...
    private void updateSuspendBlockerLocked() {
        final bool needWakeLockSuspendBlocker = ((mWakeLockSummary & WAKE_LOCK_CPU) != 0);
        final bool needDisplaySuspendBlocker = needDisplaySuspendBlockerLocked();
        final bool autoSuspend = !needDisplaySuspendBlocker;
        final bool interactive = mDisplayPowerRequest.isBrightOrDim();
     ...
     */
    private bool needDisplaySuspendBlockerLocked() {
        if (!mDisplayReady) {
            return true;
        }
    ...
    private void setHalAutoSuspendModeLocked(bool enable) {
    ...
    private void setHalInteractiveModeLocked(bool enable) {
    ...
    private bool isInteractiveInternal() {
        synchronized (mLock) {
            return PowerManagerInternal.isInteractive(mWakefulness);
        }
    }

    private bool setLowPowerModeInternal(bool enabled) {
        synchronized (mLock) {
            if (DEBUG) {
    ...
    bool isDeviceIdleModeInternal() {
        synchronized (mLock) {
            return mDeviceIdleMode;
        }
    }

    bool isLightDeviceIdleModeInternal() {
        synchronized (mLock) {
    ...
    private void shutdownOrRebootInternal(final @HaltMode int haltMode, final bool confirm,
            final String reason, bool wait) {
    ...
    bool setDeviceIdleModeInternal(bool enabled) {
        synchronized (mLock) {
    ...
    bool setLightDeviceIdleModeInternal(bool enabled) {
        synchronized (mLock) {
    ...
    void updateUidProcStateInternal(int uid, int procState) {
        synchronized (mLock) {
            ...
            final bool oldShouldAllow = state.mProcState
    ...
    private bool setWakeLockDisabledStateLocked(WakeLock wakeLock) {
        if ((wakeLock.mFlags & PowerManager.WAKE_LOCK_LEVEL_MASK)
                == PowerManager.PARTIAL_WAKE_LOCK) {
            bool disabled = false;
    ...
    private bool isMaximumScreenOffTimeoutFromDeviceAdminEnforcedLocked() {
        return mMaximumScreenOffTimeoutFromDeviceAdmin >= 0
                && mMaximumScreenOffTimeoutFromDeviceAdmin < Long.MAX_VALUE;
    }

    private void setAttentionLightInternal(bool on, int color) {
        Light light;
        synchronized (mLock) {
            if (!mSystemReady) {
                return;
    ...
    private void setDozeAfterScreenOffInternal(bool on) {
        synchronized (mLock) {
    ...
    private bool isScreenBrightnessBoostedInternal() {
        synchronized (mLock) {
    ...
    private void setDrawWakeLockOverrideFromSidekickInternal(bool keepState) {
        synchronized (mLock) {
    ...
    private final class BatteryReceiver : BroadcastReceiver {
        override
    ...
    private final class DreamReceiver : BroadcastReceiver {
        override
        public void onReceive(Context context, Intent intent) {
            synchronized (mLock) {
                scheduleSandmanLocked();
    ...
    private final class UserSwitchedReceiver : BroadcastReceiver {
        override
        public void onReceive(Context context, Intent intent) {
            synchronized (mLock) {
    ...
    private final class DockReceiver : BroadcastReceiver {
        override
        public void onReceive(Context context, Intent intent) {
            synchronized (mLock) {
     ...
     */
    private final class PowerManagerHandler : Handler {
     ...
     */
    private final class WakeLock : IBinder.DeathRecipient {
        ...
        public bool mNotifiedAcquired;
        public bool mNotifiedLong;
        ...
        public bool hasSameProperties(int flags, String tag, WorkSource workSource,
        ...
        public bool hasSameWorkSource(WorkSource workSource) {
    ...
    private final class SuspendBlockerImpl : SuspendBlocker {
    ...
    private final class BinderService : IPowerManager.Stub {
        override
        ...
        override // Binder call
        public bool isWakeLockLevelSupported(int level) {
            final long ident = Binder.clearCallingIdentity();
            try {
        ...
        override // Binder call
        public bool setPowerSaveMode(bool enabled) {
            mContext.enforceCallingOrSelfPermission(
                    android.Manifest.permission.DEVICE_POWER, null);
            final long ident = Binder.clearCallingIdentity();
            try {
        ...
        override // Binder call
        public bool isLightDeviceIdleMode() {
            final long ident = Binder.clearCallingIdentity();
            try {
         ...
         */
        override // Binder call
        public void setAttentionLight(bool on, int color) {
            mContext.enforceCallingOrSelfPermission(
                    android.Manifest.permission.DEVICE_POWER, null);
        ...
        override // Binder call
        public void setDozeAfterScreenOff(bool on) {
            mContext.enforceCallingOrSelfPermission(
                    android.Manifest.permission.DEVICE_POWER, null);
        ...
        override // Binder call
        public bool isScreenBrightnessBoosted() {
            final long ident = Binder.clearCallingIdentity();
            try {
    ...
    private final class LocalService : PowerManagerInternal {
        override
        ...
        override
        public void setDrawWakeLockOverrideFromSidekick(bool keepState) {
        ...
        override
        public bool setDeviceIdleMode(bool enabled) {
            return setDeviceIdleModeInternal(enabled);
        }

        override
        public bool setLightDeviceIdleMode(bool enabled) {
