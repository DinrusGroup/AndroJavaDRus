 ...
 */
public class KeyEventDispatcher : Handler.Callback {
    // Debugging
    ...
    public bool notifyKeyEventLocked(
            KeyEvent event, int policyFlags, List<? : KeyEventFilter> keyEventFilters) {
     ...
     */
    public void setOnKeyEventResult(KeyEventFilter keyEventFilter, bool handled, int sequence) {
        synchronized (mLock) {
     ...
     */
    private bool removeReferenceToPendingEventLocked(PendingKeyEvent pendingEvent) {
    ...
    public interface KeyEventFilter {
         ...
         */
        bool onKeyEvent(KeyEvent event, int sequenceNumber);
    }
}
