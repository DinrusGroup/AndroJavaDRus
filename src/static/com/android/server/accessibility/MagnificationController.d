 ...
 * constraints.
 */
public class MagnificationController : Handler.Callback {
    ...
    private static final bool DEBUG_SET_MAGNIFICATION_SPEC = false;
    ...
    @VisibleForTesting bool mRegistered;

    private bool mUnregisterPending;
     ...
     */
    public bool isRegisteredLocked() {
        return mRegistered;
    }
     ...
     */
    public bool magnificationRegionContains(float x, float y) {
        synchronized (mLock) {
    ...
    private bool resetLocked(bool animate) {
        if (!mRegistered) {
            return false;
        }
        final MagnificationSpec spec = mCurrentMagnificationSpec;
        final bool changed = !spec.isNop();
        if (changed) {
            spec.clear();
            onMagnificationChangedLocked();
        }
     ...
     *         the spec did not change
     */
    public bool setScale(float scale, float pivotX, float pivotY, bool animate, int id) {
        synchronized (mLock) {
            if (!mRegistered) {
                return false;
            }
     ...
     *         the spec did not change
     */
    public bool setCenter(float centerX, float centerY, bool animate, int id) {
        synchronized (mLock) {
            if (!mRegistered) {
                return false;
            }
     ...
     *         the spec did not change
     */
    public bool setScaleAndCenter(
            float scale, float centerX, float centerY, bool animate, int id) {
        synchronized (mLock) {
            if (!mRegistered) {
                return false;
            }
    ...
    private bool setScaleAndCenterLocked(float scale, float centerX, float centerY,
            bool animate, int id) {
        if (DEBUG) {
            Slog.i(LOG_TAG,
        ...
        final bool changed = updateMagnificationSpecLocked(scale, centerX, centerY);
     ...
     *         otherwise
     */
    private bool updateMagnificationSpecLocked(float scale, float centerX, float centerY) {
        // Handle defaults.
    ...
    private bool updateCurrentSpecWithOffsetsLocked(float nonNormOffsetX, float nonNormOffsetY) {
        if (DEBUG) {
            Slog.i(LOG_TAG,
     ...
     */
    bool resetIfNeeded(bool animate) {
        synchronized (mLock) {
            if (isMagnifying()) {
                reset(animate);
                return true;
    ...
    void setForceShowMagnifiableBounds(bool show) {
        if (mRegistered) {
    ...
    private void sendSpecToAnimation(MagnificationSpec spec, bool animate) {
        if (DEBUG) {
     ...
     */
    private static class SpecAnimationBridge : ValueAnimator.AnimatorUpdateListener {
        private final WindowManagerInternal mWindowManager;
        ...
        public void updateSentSpecMainThread(MagnificationSpec spec, bool animate) {
            ...
            synchronized (mLock) {
                final bool changed = !mSentMagnificationSpec.equals(spec);
                if (changed) {
                    if (animate) {
    ...
    private static class ScreenStateObserver : BroadcastReceiver {
