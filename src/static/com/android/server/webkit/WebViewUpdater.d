...
class WebViewUpdater {
    ...
    private static class WebViewPackageMissingException : Exception {
    ...
    private bool mWebViewPackageDirty = false;
    private bool mAnyWebViewInstalled = false;
    ...
    void packageStateChanged(String packageName, int changedState) {
        for (WebViewProviderInfo provider : mSystemInterface.getWebViewPackages()) {
            ...
            if (webviewPackage.equals(packageName)) {
                bool updateWebView = false;
                bool removedOrChangedOldPackage = false;
     ...
     */
    static bool isInstalledAndEnabledForAllUsers(
            List<UserPackage> userPackages) {
     ...
     * again directly.
     */
    private bool webViewIsReadyLocked() {
     ...
     */
    bool isValidProvider(WebViewProviderInfo configInfo, PackageInfo packageInfo) {
     ...
     */
    private static bool versionCodeGE(long versionCode1, long versionCode2) {
    ...
    private static bool providerHasValidSignature(WebViewProviderInfo provider,
    ...
    private void dumpAllPackageInformationLocked(PrintWriter pw) {
        ...
        for (WebViewProviderInfo provider : allProviders) {
            ...
            if (validity == VALIDITY_OK) {
                bool installedForAllUsers = isInstalledAndEnabledForAllUsers(
                        mSystemInterface.getPackageInfoForProviderAllUsers(mContext, provider));
                pw.println(String.format(
