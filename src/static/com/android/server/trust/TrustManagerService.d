 ...
 */
public class TrustManagerService : SystemService {
    private static final String TAG = "TrustManagerService";
    static final bool DEBUG = Build.IS_DEBUGGABLE && Log.isLoggable(TAG, Log.VERBOSE);
    ...
    private bool mTrustAgentsCanRun = false;
    ...
    private static final class AgentInfo {
        CharSequence label;
        Drawable icon;
        ComponentName component; // service that : ITrustAgent
    ...
    private void updateTrustUsuallyManaged(int userId, bool managed) {
    ...
    void refreshAgentList(int userIdOrAll) {
        ...
        for (UserInfo userInfo : userInfos) {
            ...
            for (ResolveInfo resolveInfo : resolveInfos) {
                ...
                bool directUnlock = resolveInfo.serviceInfo.directBootAware
        ...
        bool trustMayHaveChanged = false;
    ...
    bool isDeviceLockedInner(int userId) {
    ...
    private void refreshDeviceLockedForUser(int userId) {
        ...
        for (int i = 0; i < userInfos.size(); i++) {
            ...
            int id = info.id;
            bool secure = mLockPatternUtils.isSecure(id);
            bool trusted = aggregateIsTrusted(id);
            bool showingKeyguard = true;
            ...
            bool deviceLocked = secure && showingKeyguard && !trusted &&
                    !fingerprintAuthenticated;
            setDeviceLockedForUser(id, deviceLocked);
        }
    }

    private void setDeviceLockedForUser(@UserIdInt int userId, bool locked) {
        final bool changed;
        synchronized (mDeviceLockedForUser) {
    ...
    private void dispatchDeviceLocked(int userId, bool isLocked) {
    ...
    private void maybeEnableFactoryTrustAgents(LockPatternUtils utils, int userId) {
        ...
        bool shouldUseDefaultAgent = defaultAgent != null;
    ...
    private bool aggregateIsTrusted(int userId) {
        if (!mStrongAuthTracker.isTrustAllowedForUser(userId)) {
            return false;
        }
        for (int i = 0; i < mActiveAgents.size(); i++) {
    ...
    private bool aggregateIsTrustManaged(int userId) {
        if (!mStrongAuthTracker.isTrustAllowedForUser(userId)) {
            return false;
        }
        for (int i = 0; i < mActiveAgents.size(); i++) {
    ...
    private void dispatchUnlockAttempt(bool successful, int userId) {
        if (successful) {
    ...
    private void dispatchOnTrustChanged(bool enabled, int userId, int flags) {
        if (DEBUG) {
    ...
    private void dispatchOnTrustManagedChanged(bool managed, int userId) {
        if (DEBUG) {
    ...
    private final IBinder mService = new ITrustManager.Stub() {
        override
        public void reportUnlockAttempt(bool authenticated, int userId) throws RemoteException {
        ...
        override
        public bool isDeviceLocked(int userId) throws RemoteException {
        ...
        override
        public bool isDeviceSecure(int userId) throws RemoteException {
            userId = ActivityManager.handleIncomingUser(getCallingPid(), getCallingUid(), userId,
        ...
        private void dumpUser(PrintWriter fout, UserInfo user, bool isCurrent) {
        ...
        private String dumpBool(bool b) {
            return b ? "1" : "0";
        }
        ...
        override
        public void setDeviceLockedForUser(int userId, bool locked) {
            enforceReportPermission();
    ...
    private bool isTrustUsuallyManagedInternal(int userId) {
        synchronized (mTrustUsuallyManagedForUser) {
        ...
        bool persistedValue = mLockPatternUtils.isTrustUsuallyManaged(userId);
        synchronized (mTrustUsuallyManagedForUser) {
            int i = mTrustUsuallyManagedForUser.indexOfKey(userId);
            if (i >= 0) {
            ...
            switch (msg.what) {
                ...
                case MSG_FLUSH_TRUST_USUALLY_MANAGED:
                    ...
                    for (int i = 0; i < usuallyManaged.size(); i++) {
                        ...
                        bool value = usuallyManaged.valueAt(i);
    ...
    private static class SettingsAttrs {
        ...
        public bool canUnlockProfile;

        public SettingsAttrs(
                ComponentName componentName,
                bool canUnlockProfile) {
    ...
    private class StrongAuthTracker : LockPatternUtils.StrongAuthTracker {
        ...
        bool canAgentsRunForUser(int userId) {
        ...
        void allowTrustFromUnlock(int userId) {
            ...
            bool previous = canAgentsRunForUser(userId);
