...
public class TrustAgentWrapper {
    ...
    private static final bool DEBUG = TrustManagerService.DEBUG;
    ...
    private bool mPendingSuccessfulUnlock = false;

    // Trust state
    private bool mTrusted;
    private CharSequence mMessage;
    private bool mTrustDisabledByDpm;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_ESCROW_TOKEN_STATE: {
                    ...
                    int userId = msg.getData().getInt(DATA_USER_ID);
                    bool active = mTrustManagerService.isEscrowTokenActive(handle, userId);
                    try {
                        if (mTrustAgentService != null) {
                ...
                case MSG_REMOVE_ESCROW_TOKEN: {
                    long handle = msg.getData().getLong(DATA_HANDLE);
                    int userId = msg.getData().getInt(DATA_USER_ID);
                    bool success = mTrustManagerService.removeEscrowToken(handle, userId);
                    try {
                        if (mTrustAgentService != null) {
    ...
    /**
     * @see android.service.trust.TrustAgentService#onUnlockAttempt(bool)
     */
    ...
    bool updateDevicePolicyFeatures() {
        bool trustDisabled = false;
