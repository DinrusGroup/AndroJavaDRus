 ...
 * @hide
 */
public class GestureLauncherService : SystemService {
    private static final bool DBG = false;
    private static final bool DBG_CAMERA_LIFT = false;
    ...
    private bool mCameraLaunchRegistered;
    private bool mCameraLiftRegistered;
    private int mUserId;
     ...
     */
    private bool mCameraDoubleTapPowerEnabled;
    ...
    void updateCameraDoubleTapPowerEnabled() {
        bool enabled = isCameraDoubleTapPowerSettingEnabled(mContext, mUserId);
        synchronized (this) {
    ...
    public static bool isCameraLaunchSettingEnabled(Context context, int userId) {
        return isCameraLaunchEnabled(context.getResources())
                && (Settings.Secure.getIntForUser(context.getContentResolver(),
                        Settings.Secure.CAMERA_GESTURE_DISABLED, 0, userId) == 0);
    }

    public static bool isCameraDoubleTapPowerSettingEnabled(Context context, int userId) {
    ...
    public static bool isCameraLiftTriggerSettingEnabled(Context context, int userId) {
     ...
     */
    public static bool isCameraLaunchEnabled(Resources resources) {
        bool configSet = resources.getInteger(
                com.android.internal.R.integer.config_cameraLaunchGestureSensorType) != -1;
        return configSet &&
                !SystemProperties.getBoolean("gesture.disable_camera_launch", false);
    }

    public static bool isCameraDoubleTapPowerEnabled(Resources resources) {
        return resources.getBoolean(
                com.android.internal.R.bool.config_cameraDoubleTapPowerGestureEnabled);
    }

    public static bool isCameraLiftTriggerEnabled(Resources resources) {
        bool configSet = resources.getInteger(
     ...
     */
    public static bool isGestureLauncherEnabled(Resources resources) {
        return isCameraLaunchEnabled(resources) || isCameraDoubleTapPowerEnabled(resources) ||
                isCameraLiftTriggerEnabled(resources);
    }

    public bool interceptPowerKeyDown(KeyEvent event, bool interactive,
     ...
     */
    @VisibleForTesting
    bool handleCameraGesture(bool useWakelock, int source) {
        bool userSetupComplete = Settings.Secure.getIntForUser(mContext.getContentResolver(),
    ...
    private final ContentObserver mSettingObserver = new ContentObserver(new Handler()) {
        public void onChange(bool selfChange, android.net.Uri uri, int userId) {
            if (userId == mUserId) {
                updateCameraRegistered();
                updateCameraDoubleTapPowerEnabled();
            }
    ...
    private final class GestureEventListener : SensorEventListener {
        override
    ...
    private final class CameraLiftTriggerEventListener : TriggerEventListener {
        override
        public void onTrigger(TriggerEvent event) {
            ...
            if (event.sensor == mCameraLiftTriggerSensor) {
                ...
                SensorManager sensorManager = (SensorManager) mContext.getSystemService(
                        Context.SENSOR_SERVICE);
                bool keyguardShowingAndNotOccluded =
                        mWindowManagerInternal.isKeyguardShowingAndNotOccluded();
                bool interactive = mPowerManager.isInteractive();
                if (DBG_CAMERA_LIFT) {
                    float[] values = event.values;
