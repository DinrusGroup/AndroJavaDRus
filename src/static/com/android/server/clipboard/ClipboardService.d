...
class HostClipboardMonitor : Runnable {
 ...
 */
public class ClipboardService : SystemService {

    private static final String TAG = "ClipboardService";
    private static final bool IS_EMULATOR =
    ...
    private class ClipboardImpl : IClipboard.Stub {
        override
        ...
        override
        public bool hasPrimaryClip(String callingPackage) {
            synchronized (this) {
                if (!clipboardAccessAllowed(AppOpsManager.OP_READ_CLIPBOARD, callingPackage,
                            Binder.getCallingUid()) || isDeviceLocked()) {
                    return false;
        ...
        override
        public bool hasClipboardText(String callingPackage) {
            synchronized (this) {
                if (!clipboardAccessAllowed(AppOpsManager.OP_READ_CLIPBOARD, callingPackage,
                            Binder.getCallingUid()) || isDeviceLocked()) {
                    return false;
     ...
     */
    private bool hasRestriction(String restriction, int userId) {
        try {
    ...
    void setPrimaryClipInternal(@Nullable ClipData clip, int callingUid) {
        ...
        if (related != null) {
            ...
            if (size > 1) { // Related profiles list include the current profile.
                final bool canCopy = !hasRestriction(
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (id != userId) {
                        final bool canCopyIntoProfile = !hasRestriction(
    ...
    private bool clipboardAccessAllowed(int op, String callingPackage, int callingUid) {
        // Check the AppOp.
