 ...
 */
public final class RecoverySystemService : SystemService {
    ...
    private final class BinderService : IRecoverySystem.Stub {
        override // Binder call
        public bool uncrypt(String filename, IRecoverySystemProgressListener listener) {
            if (DEBUG) Slog.d(TAG, "uncrypt: " + filename);

            synchronized (sRequestLock) {
                mContext.enforceCallingOrSelfPermission(android.Manifest.permission.RECOVERY, null);

                final bool available = checkAndWaitForUncryptService();
                if (!available) {
        ...
        override // Binder call
        public bool setupBcb(String command) {
         ...
         */
        private bool checkAndWaitForUncryptService() {
            for (int retry = 0; retry < SOCKET_CONNECTION_MAX_RETRY; retry++) {
                ...
                final bool busy = "running".equals(uncryptService) ||
        ...
        private bool setupOrClearBcb(bool isSetup, String command) {
            mContext.enforceCallingOrSelfPermission(android.Manifest.permission.RECOVERY, null);
