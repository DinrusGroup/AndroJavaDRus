...
public final class AutofillManagerServiceShellCommand : ShellCommand {
    ...
    private bool isNextArgSessions(PrintWriter pw) {
    ...
    private int waitForLatch(PrintWriter pw, CountDownLatch latch) {
        try {
            final bool received = latch.await(5, TimeUnit.SECONDS);
            if (!received) {
