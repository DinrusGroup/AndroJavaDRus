...
final class AutofillManagerServiceImpl {
    ...
    AutofillManagerServiceImpl(Context context, Object lock, LocalLog requestsHistory,
            ...
            AutofillCompatState autofillCompatState, bool disabled) {
        mContext = context;
        mLock = lock;
    ...
    private bool isSetupCompletedLocked() {
    ...
    @GuardedBy("mLock")
    void updateLocked(bool disabled) {
        final bool wasEnabled = isEnabledLocked();
        if (sVerbose) {
        ...
        final bool isEnabled = isEnabledLocked();
    ...
    @GuardedBy("mLock")
    bool addClientLocked(IAutoFillManagerClient client) {
        if (mClients == null) {
    ...
    void setHasCallback(int sessionId, int uid, bool hasIt) {
        if (!isEnabledLocked()) {
            return;
        }
        final Session session = mSessions.get(sessionId);
    ...
    int startSessionLocked(@NonNull IBinder activityToken, int uid,
            ...
            @NonNull Rect virtualBounds, @Nullable AutofillValue value, bool hasCallback,
            @NonNull ComponentName componentName, bool compatMode,
            bool bindInstantServiceAllowed, int flags) {
        if (!isEnabledLocked()) {
            return 0;
        }
    ...
    void finishSessionLocked(int sessionId, int uid) {
        ...
        final bool finished = session.showSaveLocked();
    ...
    private Session createSessionByTokenLocked(@NonNull IBinder activityToken, int uid,
            @NonNull IBinder appCallbackToken, bool hasCallback,
            @NonNull ComponentName componentName, bool compatMode,
            bool bindInstantServiceAllowed, int flags) {
     ...
     */
    private void assertCallerLocked(@NonNull ComponentName componentName, bool compatMode) {
     ...
     */
    bool restoreSession(int sessionId, int uid, @NonNull IBinder activityToken,
     ...
     */
    @GuardedBy("mLock")
    bool updateSessionLocked(int sessionId, int uid, AutofillId autofillId, Rect virtualBounds,
    ...
    @GuardedBy("mLock")
    private bool isValidEventLocked(String method, int sessionId) {
    ...
    void logContextCommittedLocked(int sessionId, @Nullable Bundle clientState,
            ...
            @NonNull ComponentName appComponentName, bool compatMode) {
    ...
    @GuardedBy("mLock")
    private bool isCalledByServiceLocked(String methodName, int callingUid) {
    ...
    private void sendStateToClients(bool resetClient) {
    ...
    @GuardedBy("mLock")
    private bool isClientSessionDestroyedLocked(IAutoFillManagerClient client) {
        final int sessionCount = mSessions.size();
     ...
     */
    @GuardedBy("mLock")
    private bool isAutofillDisabledLocked(@NonNull ComponentName componentName) {
    ...
    bool isFieldClassificationEnabled(int callingUid) {
        synchronized (mLock) {
    ...
    bool isFieldClassificationEnabledLocked() {
    ...
    private class PruneTask : AsyncTask<Void, Void, Void> {
        override
