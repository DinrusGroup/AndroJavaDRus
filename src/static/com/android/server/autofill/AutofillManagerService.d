 ...
 */
public final class AutofillManagerService : SystemService {
    ...
    @GuardedBy("mLock")
    private bool mAllowInstantService;
    ...
    public AutofillManagerService(Context context) {
        ...
        final bool debug = Build.IS_DEBUGGABLE;
        ...
        for (int i = 0; i < users.size(); i++) {
            ...
            final bool disabled = umi.getUserRestriction(userId, UserManager.DISALLOW_AUTOFILL);
            if (disabled) {
                if (disabled) {
        ...
        umi.addUserRestrictionsListener((userId, newRestrictions, prevRestrictions) -> {
            ...
            synchronized (mLock) {
                final bool disabledBefore = mDisabledUsers.get(userId);
    ...
    // Called by Shell command.
    void setAllowInstantService(bool mode) {
        mContext.enforceCallingPermission(MANAGE_AUTO_FILL, TAG);
    ...
    private void setDebugLocked(bool debug) {
        com.android.server.autofill.Helper.sDebug = debug;
        android.view.autofill.Helper.sDebug = debug;
    }

    private void setVerboseLocked(bool verbose) {
     ...
     */
    @GuardedBy("mLock")
    private void updateCachedServiceLocked(int userId, bool disabled) {
    ...
    private final class LocalService : AutofillManagerInternal {
        override
        ...
        override
        public bool isCompatibilityModeRequested(@NonNull String packageName,
    ...
    static final class AutofillCompatState {
        ...
        bool isCompatibilityModeRequested(@NonNull String packageName,
                long versionCode, @UserIdInt int userId) {
            synchronized (mLock) {
                if (mUserSpecs == null) {
                    return false;
    ...
    final class AutoFillManagerServiceStub : IAutoFillManager.Stub {
        override
        ...
        override
        public void setHasCallback(int sessionId, int userId, bool hasIt) {
            synchronized (mLock) {
                final AutofillManagerServiceImpl service = getServiceForUserLocked(userId);
        ...
        public int startSession(IBinder activityToken, IBinder appCallback, AutofillId autofillId,
                Rect bounds, AutofillValue value, int userId, bool hasCallback, int flags,
                ComponentName componentName, bool compatMode) {
        ...
        override
        public bool isFieldClassificationEnabled() throws RemoteException {
            final int userId = UserHandle.getCallingUserId();
        ...
        override
        public bool restoreSession(int sessionId, IBinder activityToken, IBinder appCallback)
        ...
        public int updateOrRestartSession(IBinder activityToken, IBinder appCallback,
                ...
                bool hasCallback, int flags, ComponentName componentName, int sessionId,
                int action, bool compatMode) {
            bool restart = false;
            synchronized (mLock) {
                final AutofillManagerServiceImpl service = peekServiceForUserLocked(userId);
                if (service != null) {
        ...
        override
        public bool isServiceSupported(int userId) {
            synchronized (mLock) {
        ...
        override
        public bool isServiceEnabled(int userId, String packageName) {
            synchronized (mLock) {
                final AutofillManagerServiceImpl service = peekServiceForUserLocked(userId);
                if (service != null) {
