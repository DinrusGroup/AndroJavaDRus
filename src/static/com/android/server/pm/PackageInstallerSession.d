...
public class PackageInstallerSession : IPackageInstallerSession.Stub {
     ...
     */
    @GuardedBy("mLock")
    private bool isInstallerDeviceOwnerOrAffiliatedProfileOwnerLocked() {
     ...
     */
    @GuardedBy("mLock")
    private bool needToAskForPermissionsLocked() {
        if (mPermissionsManuallyAccepted) {
            return false;
        }

        final bool isInstallPermissionGranted =
                (mPm.checkUidPermission(android.Manifest.permission.INSTALL_PACKAGES,
                        mInstallerUid) == PackageManager.PERMISSION_GRANTED);
        final bool isSelfUpdatePermissionGranted =
                (mPm.checkUidPermission(android.Manifest.permission.INSTALL_SELF_UPDATES,
                        mInstallerUid) == PackageManager.PERMISSION_GRANTED);
        final bool isUpdatePermissionGranted =
        ...
        final bool isPermissionGranted = isInstallPermissionGranted
                || (isUpdatePermissionGranted && targetPackageUid != -1)
                || (isSelfUpdatePermissionGranted && targetPackageUid == mInstallerUid);
        final bool isInstallerRoot = (mInstallerUid == Process.ROOT_UID);
        final bool isInstallerSystem = (mInstallerUid == Process.SYSTEM_UID);
        final bool forcePermissionPrompt =
    ...
    public PackageInstallerSession(PackageInstallerService.InternalCallback callback,
            ...
            File stageDir, String stageCid, bool prepared, bool sealed) {
        mCallback = callback;
        mContext = context;
        mPm = pm;
    ...
    public SessionInfo generateInfo(bool includeIcon) {
    ...
    public void setClientProgress(float progress) {
        synchronized (mLock) {
            ...
            final bool forcePublish = (mClientProgress == 0);
    ...
    @GuardedBy("mLock")
    private void computeProgressLocked(bool forcePublish) {
    ...
    override
    public void commit(@NonNull IntentSender statusReceiver, bool forTransfer) {
     ...
     */
    private bool mayInheritNativeLibs() {
        ...
        if (params.mode == SessionParams.MODE_FULL_INSTALL) {
            ...
            if (!ArrayUtils.isEmpty(existing.splitNames)) {
                for (int i = 0; i < existing.splitNames.length; i++) {
                    ...
                    final bool splitRemoved = removeSplitList.contains(splitName);
     ...
     */
    private bool isLinkPossible(List<File> fromFiles, File toDir) {
        try {
    ...
    private static void extractNativeLibraries(File packageDir, String abiOverride, bool inherit)
            throws PackageManagerException {
    ...
    void setPermissionsResult(bool accepted) {
        if (!mSealed) {
    ...
    private void closeInternal(bool checkCaller) {
        int activeCount;
        synchronized (mLock) {
            if (checkCaller) {
                assertCallerIsOwnerOrRootLocked();
    ...
    private void dispatchSessionFinished(int returnCode, String msg, Bundle extras) {
        ...
        final bool success = (returnCode == PackageManager.INSTALL_SUCCEEDED);

        // Send broadcast to default launcher only if it's a new install
        final bool isNewInstall = extras == null || !extras.getBoolean(Intent.EXTRA_REPLACING);
    ...
    public static PackageInstallerSession readFromXml(@NonNull XmlPullParser in,
        ...
        final bool prepared = readBooleanAttribute(in, ATTR_PREPARED, true);
        final bool sealed = readBooleanAttribute(in, ATTR_SEALED);
