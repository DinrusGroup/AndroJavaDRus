...
public final class PermissionsState {
    ...
    public void copyFrom(PermissionsState other) {
        ...
        if (other.mPermissionReviewRequired != null) {
            ...
            for (int i = 0; i < userCount; i++) {
                final bool reviewRequired = other.mPermissionReviewRequired.valueAt(i);
    ...
    public bool isPermissionReviewRequired(int userId) {
     ...
     */
    public bool hasRuntimePermission(String name, int userId) {
        enforceValidUserId(userId);
     ...
     * @return Whether this state has the permission.
     */
    public bool hasInstallPermission(String name) {
     ...
     */
    public bool hasPermission(String name, int userId) {
        enforceValidUserId(userId);
     ...
     */
    public bool hasRequestedPermission(ArraySet<String> names) {
        if (mPermissions == null) {
            return false;
        }
     ...
     */
    public bool updatePermissionFlags(BasePermission permission, int userId,
            int flagMask, int flagValues) {
        enforceValidUserId(userId);

        final bool mayChangeFlags = flagValues != 0 || flagMask != 0;
        ...
        final bool updated = permissionData.updateFlags(userId, flagMask, flagValues);
        if (updated) {
    ...
    private bool hasPermissionRequiringReview(int userId) {
        final int permissionCount = mPermissions.size();
        for (int i = 0; i < permissionCount; i++) {
    ...
    public bool updatePermissionFlagsForAllPermissions(
    ...
    private int grantPermission(BasePermission permission, int userId) {
        ...
        final bool hasGids = !ArrayUtils.isEmpty(permission.computeGids(userId));
    ...
    private static final class PermissionData {
        ...
        public bool isGranted(int userId) {
        ...
        public static bool isInstallPermissionKey(int userId) {
            return userId == UserHandle.USER_ALL;
        }

        public bool updateFlags(int userId, int flagMask, int flagValues) {
            if (isInstallPermission()) {
                userId = UserHandle.USER_ALL;
            }
        ...
        private bool isCompatibleUserId(int userId) {
            return isDefault() || !(isInstallPermission() ^ isInstallPermissionKey(userId));
        }

        private bool isInstallPermission() {
