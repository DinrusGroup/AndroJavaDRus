...
class PackageManagerShellCommand : ShellCommand {
    ...
    private int runListPackages(bool showSourceDir) throws RemoteException {
        final PrintWriter pw = getOutPrintWriter();
        int getFlags = 0;
        bool listDisabled = false, listEnabled = false;
        bool listSystem = false, listThirdParty = false;
        bool listInstaller = false;
        bool showUid = false;
        bool showVersionCode = false;
        int uid = -1;
        int userId = UserHandle.USER_SYSTEM;
        try {
            String opt;
    ...
    private int runCompile() throws RemoteException {
        final PrintWriter pw = getOutPrintWriter();
        bool checkProfiles = SystemProperties.getBoolean("dalvik.vm.usejitprofiles", false);
    ...
    private int runDexoptJob() throws RemoteException {
        ...
        bool result = mInterface.runBackgroundDexoptJob(packageNames.isEmpty() ? null :
                packageNames);
        return result ? 0 : -1;
    }
    ...
    private int runSnapshotProfile() throws RemoteException {
        ...
        // Parse the arguments
        final String packageName = getNextArg();
        final bool isBootImage = "android".equals(packageName);
    ...
    private static class SnapshotRuntimeProfileCallback
            : ISnapshotRuntimeProfileCallback.Stub {
    ...
    static class ClearDataObserver : IPackageDataObserver.Stub {
        bool finished;
        bool result;

        override
        public void onRemoveCompleted(String packageName, bool succeeded) throws RemoteException {
            synchronized (this) {
                finished = true;
                result = succeeded;
                notifyAll();
    ...
    private int runSetHiddenSetting(bool state) throws RemoteException {
        int userId = UserHandle.USER_SYSTEM;
        String option = getNextOption();
        if (option != null && option.equals("--user")) {
            userId = UserHandle.parseUserArg(getNextArgRequired());
    ...
    private int runSuspend(bool suspendedState) {
        final PrintWriter pw = getOutPrintWriter();
        int userId = UserHandle.USER_SYSTEM;
    ...
    private int runGrantRevokePermission(bool grant) throws RemoteException {
        int userId = UserHandle.USER_SYSTEM;
    ...
    private bool isVendorApp(String pkg) {
        try {
    ...
    private bool isProductApp(String pkg) {
        try {
            final PackageInfo info = mInterface.getPackageInfo(pkg, 0, UserHandle.USER_SYSTEM);
    ...
    private int runHasFeature() {
        ...
        try {
            ...
            final bool hasFeature = mInterface.hasSystemFeature(featureName, version);
    ...
    private int translateUserId(int userId, bool allowAll, String logContext) {
    ...
    private int doWriteSplit(int sessionId, String inPath, long sizeBytes, String splitName,
            bool logSuccess) throws RemoteException {
        final PrintWriter pw = getOutPrintWriter();
    ...
    private int doRemoveSplit(int sessionId, String splitName, bool logSuccess)
            throws RemoteException {
        final PrintWriter pw = getOutPrintWriter();
        PackageInstaller.Session session = null;
        try {
    ...
    private int doCommitSession(int sessionId, bool logSuccess) throws RemoteException {
        final PrintWriter pw = getOutPrintWriter();
        PackageInstaller.Session session = null;
        try {
            session = new PackageInstaller.Session(
    ...
    private int doAbandonSession(int sessionId, bool logSuccess) throws RemoteException {
        final PrintWriter pw = getOutPrintWriter();
        PackageInstaller.Session session = null;
        try {
            session = new PackageInstaller.Session(
    ...
    private void doListPermissions(ArrayList<String> groupList, bool groups, bool labels,
            bool summary, int startProtectionLevel, int endProtectionLevel)
                    throws RemoteException {
        final PrintWriter pw = getOutPrintWriter();
