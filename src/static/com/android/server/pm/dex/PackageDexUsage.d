 ...
 */
public class PackageDexUsage : AbstractStatsBase<Void> {
     ...
     */
    public bool record(String owningPackageName, String dexPath, int ownerUserId,
            String loaderIsa, bool isUsedByOtherApps, bool primaryOrSplit,
        ...
        synchronized (mPackageUseInfoMap) {
            ...
            if (packageUseInfo == null) {
                ...
                if (primaryOrSplit) {
                    ...
                    bool updateLoadingPackages = maybeAddLoadingPackage(owningPackageName,
    ...
    */
    ...
    /* package */ void read(Reader reader) throws IOException {
        ...
        while ((line = in.readLine()) != null) {
            if (line.startsWith(DEX_LINE_CHAR)) {
                ...
                bool isUsedByOtherApps = readBoolean(elems[1]);
     ...
     */
    private bool maybeAddLoadingPackage(String owningPackage, String loadingPackage,
     ...
     */
    /*package*/ bool clearUsedByOtherApps(String packageName) {
        synchronized (mPackageUseInfoMap) {
     ...
     */
    /*package*/ bool removeUserPackage(String packageName, int userId) {
        synchronized (mPackageUseInfoMap) {
            PackageUseInfo packageUseInfo = mPackageUseInfoMap.get(packageName);
            if (packageUseInfo == null) {
                return false;
     ...
     *         false if the record doesn't exist
     */
    /*package*/ bool removeDexFile(String packageName, String dexFile, int userId) {
        synchronized (mPackageUseInfoMap) {
            PackageUseInfo packageUseInfo = mPackageUseInfoMap.get(packageName);
            if (packageUseInfo == null) {
                return false;
    ...
    private bool removeDexFile(PackageUseInfo packageUseInfo, String dexFile, int userId) {
    ...
    private String writeBoolean(bool bool) {
        return bool ? "1" : "0";
    }

    private bool readBoolean(String bool) {
    ...
    public static class PackageUseInfo {
        ...
        private bool mUsedByOtherAppsBeforeUpgrade;
        ...
        private bool mergeCodePathUsedByOtherApps(String codePath, bool isUsedByOtherApps,
        ...
        public bool isAnyCodePathUsedByOtherApps() {
         ...
         */
        /*package*/ bool clearCodePathUsedByOtherApps() {
    ...
    public static class DexUseInfo {
        private bool mIsUsedByOtherApps;
        ...
        public DexUseInfo(bool isUsedByOtherApps, int ownerUserId, String classLoaderContext,
                String loaderIsa) {
        ...
        private bool merge(DexUseInfo dexUseInfo) {
            bool oldIsUsedByOtherApps = mIsUsedByOtherApps;
            mIsUsedByOtherApps = mIsUsedByOtherApps || dexUseInfo.mIsUsedByOtherApps;
            bool updateIsas = mLoaderIsas.addAll(dexUseInfo.mLoaderIsas);
            bool updateLoadingPackages = mLoadingPackages.addAll(dexUseInfo.mLoadingPackages);
