 ...
 */
public abstract class PackageSettingBase : SettingBase {
    ...
    void setInstalled(bool inst, int userId) {
        modifyUserState(userId).installed = inst;
    }

    bool getInstalled(int userId) {
    ...
    int[] queryInstalledUsers(int[] users, bool installed) {
        int num = 0;
        for (int user : users) {
    ...
    bool getStopped(int userId) {
        return readUserState(userId).stopped;
    }

    void setStopped(bool stop, int userId) {
        modifyUserState(userId).stopped = stop;
    }

    bool getNotLaunched(int userId) {
        return readUserState(userId).notLaunched;
    }

    void setNotLaunched(bool stop, int userId) {
        modifyUserState(userId).notLaunched = stop;
    }

    bool getHidden(int userId) {
        return readUserState(userId).hidden;
    }

    void setHidden(bool hidden, int userId) {
        modifyUserState(userId).hidden = hidden;
    }

    bool getSuspended(int userId) {
        return readUserState(userId).suspended;
    }

    void setSuspended(bool suspended, String suspendingPackage, String dialogMessage,
    ...
    public bool getInstantApp(int userId) {
        return readUserState(userId).instantApp;
    }

    void setInstantApp(bool instantApp, int userId) {
        modifyUserState(userId).instantApp = instantApp;
    }

    bool getVirtulalPreload(int userId) {
        return readUserState(userId).virtualPreload;
    }

    void setVirtualPreload(bool virtualPreload, int userId) {
        modifyUserState(userId).virtualPreload = virtualPreload;
    }

    void setUserState(int userId, long ceDataInode, int enabled, bool installed, bool stopped,
            bool notLaunched, bool hidden, bool suspended, String suspendingPackage,
            String dialogMessage, PersistableBundle suspendedAppExtras,
            PersistableBundle suspendedLauncherExtras, bool instantApp,
            bool virtualPreload, String lastDisableAppCaller,
    ...
    PackageUserState modifyUserStateComponents(int userId, bool disabled, bool enabled) {
        PackageUserState state = modifyUserState(userId);
