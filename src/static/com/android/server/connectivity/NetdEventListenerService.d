 ...
 */
public class NetdEventListenerService : INetdEventListener.Stub {
    ...
    public synchronized bool addNetdEventCallback(int callerType, INetdEventCallback callback) {
    ...
    public synchronized bool removeNetdEventCallback(int callerType) {
        if (!isValidCallerType(callerType)) {
            Log.e(TAG, "Invalid caller type: " + callerType);
            return false;
        }
    ...
    private static bool isValidCallerType(int callerType) {
    ...
    public synchronized void onPrivateDnsValidationEvent(int netId,
            String ipAddress, String hostname, bool validated)
            throws RemoteException {
        for (INetdEventCallback callback : mNetdEventCallbackList) {
            if (callback != null) {
