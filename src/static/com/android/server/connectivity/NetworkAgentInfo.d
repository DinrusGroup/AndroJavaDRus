...
public class NetworkAgentInfo : Comparable<NetworkAgentInfo> {
    ...
    public bool avoidUnvalidated;

    // Whether a captive portal was ever detected on this network.
    // This is a sticky bit; once set it is never cleared.
    public bool everCaptivePortalDetected;

    // Whether a captive portal was found during the last network validation attempt.
    public bool lastCaptivePortalDetected;
    ...
    public static class LingerTimer : Comparable<LingerTimer> {
    ...
    private static final bool ADD = true;
    private static final bool REMOVE = false;

    private void updateRequestCounts(bool add, NetworkRequest request) {
        int delta = add ? +1 : -1;
        switch (request.type) {
            case REQUEST:
     ...
     *         already present.
     */
    public bool addRequest(NetworkRequest networkRequest) {
     ...
     */
    public bool isSatisfyingRequest(int id) {
     ...
     */
    public bool isBackgroundNetwork() {
    ...
    public bool satisfies(NetworkRequest request) {
        return created &&
                request.networkCapabilities.satisfiedByNetworkCapabilities(networkCapabilities);
    }

    public bool satisfiesImmutableCapabilitiesOf(NetworkRequest request) {
        return created &&
    ...
    private int getCurrentScore(bool pretendValidated) {
    ...
    private bool ignoreWifiUnvalidationPenalty() {
        bool isWifi = networkCapabilities.hasTransport(NetworkCapabilities.TRANSPORT_WIFI) &&
                networkCapabilities.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET);
        bool avoidBadWifi = mConnService.avoidBadWifi() || avoidUnvalidated;
     ...
     */
    public bool unlingerRequest(NetworkRequest request) {
