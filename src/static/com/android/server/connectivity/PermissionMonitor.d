...
public class PermissionMonitor {
    ...
    public synchronized void startMonitoring() {
        ...
        for (PackageInfo app : apps) {
            ...
            bool isNetwork = hasNetworkPermission(app);
            bool hasRestrictedPermission = hasRestrictedNetworkPermission(app);
    ...
    @VisibleForTesting
    bool isPreinstalledSystemApp(PackageInfo app) {
        int flags = app.applicationInfo != null ? app.applicationInfo.flags : 0;
        return (flags & (FLAG_SYSTEM | FLAG_UPDATED_SYSTEM_APP)) != 0;
    }

    @VisibleForTesting
    bool hasPermission(PackageInfo app, String permission) {
    ...
    private bool hasNetworkPermission(PackageInfo app) {
        return hasPermission(app, CHANGE_NETWORK_STATE);
    }

    private bool hasRestrictedNetworkPermission(PackageInfo app) {
        if (isPreinstalledSystemApp(app)) return true;
        return hasPermission(app, CONNECTIVITY_INTERNAL)
                || hasPermission(app, CONNECTIVITY_USE_RESTRICTED_NETWORKS);
    }

    private bool hasUseBackgroundNetworksPermission(PackageInfo app) {
    ...
    public bool hasUseBackgroundNetworksPermission(int uid) {
    ...
    private void update(Set<Integer> users, Map<Integer, Boolean> apps, bool add) {
    ...
    private Boolean highestPermissionForUid(Boolean currentPermission, String name) {
        ...
        try {
            ...
            final bool isNetwork = hasNetworkPermission(app);
            final bool hasRestrictedPermission = hasRestrictedNetworkPermission(app);
            if (isNetwork || hasRestrictedPermission) {
