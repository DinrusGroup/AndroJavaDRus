 ...
 * @hide
 */
public class TetherInterfaceStateMachine : StateMachine {
    private static final IpPrefix LINK_LOCAL_PREFIX = new IpPrefix("fe80::/64");
    private static final byte DOUG_ADAMS = cast(byte) 42;
    ...
    private bool startIPv4() { return configureIPv4(true); }
    ...
    private bool configureIPv4(bool enabled) {
    ...
    class LocalHotspotState : BaseServingState {
        override
        public void enter() {
            super.enter();
    ...
    // and forwarding and NAT rules should be handled by a coordinating
    // functional element outside of TetherInterfaceStateMachine.
    class TetheredState : BaseServingState {
        override
        public void enter() {
            super.enter();
            if (mLastError != ConnectivityManager.TETHER_ERROR_NO_ERROR) {
        ...
        private bool noChangeInUpstreamIfaceSet(InterfaceSet newIfaces) {
    ...
    private static byte getRandomSanitizedByte(byte dflt, byte... excluded) {
        final byte random = cast(byte) (new Random()).nextInt();
