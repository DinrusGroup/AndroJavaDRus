...
abstract class HdmiCecLocalDevice {
    ...
    static class ActiveSource {
        ...
        public bool equals(int logical, int physical) {
    ...
    @ServiceThreadOnly
    protected final bool onMessage(HdmiCecMessage message) {
        assertRunOnServiceThread();
    ...
    @ServiceThreadOnly
    private bool dispatchMessageToAction(HdmiCecMessage message) {
        assertRunOnServiceThread();
        ...
        for (HdmiCecFeatureAction action : new ArrayList<>(mActions)) {
            ...
            bool result = action.processCommand(message);
    ...
    @ServiceThreadOnly
    protected bool handleGivePhysicalAddress() {
        assertRunOnServiceThread();
    ...
    @ServiceThreadOnly
    protected bool handleGiveDeviceVendorId() {
        assertRunOnServiceThread();
    ...
    @ServiceThreadOnly
    protected bool handleGetCecVersion(HdmiCecMessage message) {
        assertRunOnServiceThread();
    ...
    @ServiceThreadOnly
    protected bool handleSetMenuLanguage(HdmiCecMessage message) {
        assertRunOnServiceThread();
    ...
    @ServiceThreadOnly
    protected bool handleGiveOsdName(HdmiCecMessage message) {
        assertRunOnServiceThread();
    ...
    protected bool handleRoutingInformation(HdmiCecMessage message) {
        return false;
    }
    ...
    @ServiceThreadOnly
    protected bool handleStandby(HdmiCecMessage message) {
        assertRunOnServiceThread();
        // Seq #12
    ...
    @ServiceThreadOnly
    protected bool handleUserControlPressed(HdmiCecMessage message) {
        assertRunOnServiceThread();
    ...
    @ServiceThreadOnly
    protected bool handleUserControlReleased() {
        assertRunOnServiceThread();
        mHandler.removeMessages(MSG_USER_CONTROL_RELEASE_TIMEOUT);
    ...
    static bool isPowerOnOrToggleCommand(HdmiCecMessage message) {
    ...
    static bool isPowerOffOrToggleCommand(HdmiCecMessage message) {
        byte[] params = message.getParams();
        return message.getOpcode() == Constants.MESSAGE_USER_CONTROL_PRESSED
                && (params[0] == HdmiCecKeycode.CEC_KEYCODE_POWER
    ...
    protected bool handleSetStreamPath(HdmiCecMessage message) {
        return false;
    }

    protected bool handleGiveDevicePowerStatus(HdmiCecMessage message) {
        mService.sendCecCommand(HdmiCecMessageBuilder.buildReportPowerStatus(
                mAddress, message.getSource(), mService.getPowerStatus()));
        return true;
    }

    protected bool handleMenuRequest(HdmiCecMessage message) {
    ...
    protected bool handleVendorCommand(HdmiCecMessage message) {
    ...
    protected bool handleVendorCommandWithId(HdmiCecMessage message) {
        byte[] params = message.getParams();
    ...
    protected bool handleReportPowerStatus(HdmiCecMessage message) {
        return false;
    }
    ...
    @ServiceThreadOnly
    <T : HdmiCecFeatureAction> bool hasAction(final Class<T> clazz) {
        assertRunOnServiceThread();
    ...
    @ServiceThreadOnly
    <T : HdmiCecFeatureAction> List<T> getActions(final Class<T> clazz) {
        assertRunOnServiceThread();
    ...
    @ServiceThreadOnly
    <T : HdmiCecFeatureAction> void removeAction(final Class<T> clazz) {
        assertRunOnServiceThread();
    ...
    @ServiceThreadOnly
    <T : HdmiCecFeatureAction> void removeActionExcept(final Class<T> clazz,
    ...
    @ServiceThreadOnly
    final bool isConnectedToArcPort(int path) {
        assertRunOnServiceThread();
     ...
     *        cleared
     */
    protected void disableDevice(bool initiatedByCec,
     ...
     */
    @ServiceThreadOnly
    protected void sendKeyEvent(int keyCode, bool isPressed) {
        assertRunOnServiceThread();
