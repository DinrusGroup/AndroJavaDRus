...
public class HdmiCecMessageBuilder {
    ...
    static HdmiCecMessage buildFeatureAbortCommand(int src, int dest, int originalOpcode,
        ...
        byte[] params = new byte[] {
                cast(byte) (originalOpcode & 0xFF),
                cast(byte) (reason & 0xFF),
        };
    ...
    static HdmiCecMessage buildSetMenuLanguageCommand(int src, String language) {
        ...
        byte[] params = new byte[] {
                cast(byte) (normalized.charAt(0) & 0xFF),
                cast(byte) (normalized.charAt(1) & 0xFF),
                cast(byte) (normalized.charAt(2) & 0xFF),
        };
    ...
    static HdmiCecMessage buildReportPhysicalAddressCommand(int src, int address, int deviceType) {
        byte[] params = new byte[] {
                ...
                cast(byte) ((address >> 8) & 0xFF),
                cast(byte) (address & 0xFF),
                // One byte device type
                cast(byte) (deviceType & 0xFF)
        };
    ...
    static HdmiCecMessage buildDeviceVendorIdCommand(int src, int vendorId) {
        byte[] params = new byte[] {
                cast(byte) ((vendorId >> 16) & 0xFF),
                cast(byte) ((vendorId >> 8) & 0xFF),
                cast(byte) (vendorId & 0xFF)
        };
    ...
    static HdmiCecMessage buildRoutingChange(int src, int oldPath, int newPath) {
        byte[] param = new byte[] {
            cast(byte) ((oldPath >> 8) & 0xFF), cast(byte) (oldPath & 0xFF),
            cast(byte) ((newPath >> 8) & 0xFF), cast(byte) (newPath & 0xFF)
        };
    ...
    static HdmiCecMessage buildUserControlPressed(int src, int dest, int uiCommand) {
        return buildUserControlPressed(src, dest, new byte[] { cast(byte) (uiCommand & 0xFF) });
    }
    ...
    static HdmiCecMessage buildVendorCommandWithId(int src, int dest, int vendorId,
        ...
        params[0] = cast(byte) ((vendorId >> 16) & 0xFF);
        params[1] = cast(byte) ((vendorId >> 8) & 0xFF);
        params[2] = cast(byte) (vendorId & 0xFF);
    ...
    private static byte[] physicalAddressToParam(int physicalAddress) {
        return new byte[] {
                cast(byte) ((physicalAddress >> 8) & 0xFF),
                cast(byte) (physicalAddress & 0xFF)
        };
    }
}
