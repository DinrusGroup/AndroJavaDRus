...
final class HdmiCecKeycode {
    ...
    private static class KeycodeEntry {
        ...
        private final bool mIsRepeatable;
        private final byte[] mCecKeycodeAndParams;

        private KeycodeEntry(int androidKeycode, int cecKeycode, bool isRepeatable,
                byte[] cecParams) {
            ...
            mCecKeycodeAndParams[0] = cast(byte) (cecKeycode & 0xFF);
        }
     ...
     */
    static bool isRepeatableKey(int androidKeycode) {
        for (int i = 0; i < KEYCODE_ENTRIES.length; ++i) {
     ...
     */
    static bool isSupportedKeycode(int androidKeycode) {
     ...
     */
    public static int getMuteKey(bool muting) {
