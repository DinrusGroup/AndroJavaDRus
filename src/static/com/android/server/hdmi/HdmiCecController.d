...
final class HdmiCecController {
    ...
    private final Predicate<Integer> mRemoteDeviceAddressPredicate = new Predicate<Integer>() {
        override
        public bool test(Integer address) {
     ...
     */
    @ServiceThreadOnly
    void setOption(int flag, bool enabled) {
        assertRunOnServiceThread();
     ...
     */
    @ServiceThreadOnly
    void enableAudioReturnChannel(int port, bool enabled) {
        assertRunOnServiceThread();
    ...
    @ServiceThreadOnly
    private bool isAllocatedLocalDeviceAddress(int address) {
        assertRunOnServiceThread();
        for (int i = 0; i < mLocalDevices.size(); ++i) {
    ...
    @IoThreadOnly
    private bool sendPollMessage(int sourceAddress, int destinationAddress, int retryCount) {
        assertRunOnIoThread();
    ...
    private bool isAcceptableAddress(int address) {
     ...
     */
    @ServiceThreadOnly
    private void handleHotplug(int port, bool connected) {
        assertRunOnServiceThread();
    ...
    @ServiceThreadOnly
    private void addMessageToHistory(bool isReceived, HdmiCecMessage message) {
        assertRunOnServiceThread();
    ...
    private static native void nativeSetOption(long controllerPtr, int flag, bool enabled);
    private static native void nativeSetLanguage(long controllerPtr, String language);
    private static native void nativeEnableAudioReturnChannel(long controllerPtr, int port, bool flag);
    private static native bool nativeIsConnected(long controllerPtr, int port);

    private final class MessageHistoryRecord {
        private final long mTime;
        private final bool mIsReceived; // true if received message and false if sent message
        private final HdmiCecMessage mMessage;

        public MessageHistoryRecord(bool isReceived, HdmiCecMessage message) {
