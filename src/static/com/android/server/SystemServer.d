...
public final class SystemServer {
    ...
    private bool mOnlyCore;
    private bool mFirstBoot;
    private final bool mRuntimeRestart;
    ...
    private Future!(T) mSensorServiceStart;
    private Future!(T) mZygotePreload;
    ...
    private bool isFirstBootOrUpgrade() {
    ...
    private void performPendingShutdown() {
        ...
        if (shutdownAction != null && shutdownAction.length() > 0) {
            bool reboot = (shutdownAction.charAt(0) == '1');
    ...
    private void startBootstrapServices() {
        ...
        if (!mOnlyCore) {
            bool disableOtaDexopt = SystemProperties.getBoolean("config.disable_otadexopt",
                    false);
            if (!disableOtaDexopt) {
    ...
    private void startOtherServices() {
        ...
        bool disableSystemTextClassifier = SystemProperties.getBoolean(
                "config.disable_systemtextclassifier", false);
        bool disableCameraService = SystemProperties.getBoolean("config.disable_cameraservice",
                false);
        bool disableSlices = SystemProperties.getBoolean("config.disable_slices", false);
        bool enableLeftyService = SystemProperties.getBoolean("config.enable_lefty", false);

        bool isEmulator = SystemProperties.get("ro.kernel.qemu").equals("1");
        ...
        if (mFactoryTestMode != FactoryTest.FACTORY_TEST_LOW_LEVEL) {
            ...
            final bool hasPdb = !SystemProperties.get(PERSISTENT_DATA_BLOCK_PROP).equals("");
            if (hasPdb) {
            ...
            final bool startRulesManagerService =
        ...
        // we are in safe mode.
        final bool safeMode = wm.detectSafeMode();
        if (safeMode) {
