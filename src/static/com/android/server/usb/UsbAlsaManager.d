...
public final class UsbAlsaManager {
    ...
    private final bool mHasMidiFeature;
    ...
    /* package */ void usbDeviceAdded(String deviceAddress, UsbDevice usbDevice,
        ...
        bool hasInput = parser.hasInput();
        bool hasOutput = parser.hasOutput();
        if (DEBUG) {
            Slog.d(TAG, "hasInput: " + hasInput + " hasOutput:" + hasOutput);
        }
        if (hasInput || hasOutput) {
            bool isInputHeadset = parser.isInputHeadset();
            bool isOutputHeadset = parser.isOutputHeadset();
        ...
        bool hasMidi = parser.hasMIDIInterface();
        if (DEBUG) {
   ...
   /* package */ void setPeripheralMidiState(bool enabled, int card, int device) {
        if (!mHasMidiFeature) {
            return;
        }
