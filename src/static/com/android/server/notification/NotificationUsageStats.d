...
public class NotificationUsageStats {
    ...
    private static final bool ENABLE_AGGREGATED_IN_MEMORY_STATS = true;
    private static final bool ENABLE_SQLITE_LOG = true;
     ...
     */
    public synchronized bool isAlertRateLimited(String packageName) {
        AggregatedStats stats = getOrCreateAggregatedStatsLocked(packageName);
        if (stats != null) {
    ...
    public synchronized void registerPeopleAffinity(NotificationRecord notification, bool valid,
            bool starred, bool cached) {
        AggregatedStats[] aggregatedStatsArray = getAggregatedStatsLocked(notification);
        for (AggregatedStats stats : aggregatedStatsArray) {
            if (valid) {
    ...
    private static class AggregatedStats {
        ...
        public bool isAlertRateLimited() {
            bool limited = alertRate.shouldRateLimitAlert(SystemClock.elapsedRealtime());
            if (limited) {
                numAlertViolations++;
            }
            return limited;
    ...
    public static class SingleNotificationStats {
        ...
        public void onVisibilityChanged(bool visible) {
            ...
            final bool wasVisible = isVisible;
            isVisible = visible;
            if (visible) {
                if (currentAirtimeStartElapsedMs < 0) {
                    airtimeCount++;
        ...
        public void onExpansionChanged(bool userAction, bool expanded) {
            isExpanded = expanded;
    ...
    private static class SQLiteLog {
        ...
        private static void putNotificationDetails(NotificationRecord r, ContentValues outCv) {
            ...
            final bool noisy = r.stats.isNoisy;
