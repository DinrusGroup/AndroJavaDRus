...
abstract public class ManagedServices {
    ...
    protected final bool DEBUG = Log.isLoggable(TAG, Log.DEBUG);
    ...
    abstract protected bool checkType(IInterface service);
    ...
    private ManagedServiceInfo newServiceInfo(IInterface service,
            ComponentName component, int userId, bool isSystem, ServiceConnection connection,
    ...
    public void dump(PrintWriter pw, DumpFilter filter) {
        ...
        for (int i = 0 ; i < N; i++) {
            ...
            if (approvedByType != null) {
                ...
                for (int j = 0; j < M; j++) {
                    final bool isPrimary = approvedByType.keyAt(j);
    ...
    public void writeXml(XmlSerializer out, bool forBackup) throws IOException {
    ...
    protected void addApprovedList(String approved, int userId, bool isPrimary) {
    ...
    protected bool isComponentEnabledForPackage(String pkg) {
    ...
    protected String getApproved(int userId, bool primary) {
    ...
    protected bool isPackageOrComponentAllowed(String pkgOrComponent, int userId) {
    ...
    public void onPackagesChanged(bool removingPackage, String[] pkgList, int[] uidList) {
        ...
        if (pkgList != null && (pkgList.length > 0)) {
            bool anyServicesInvolved = false;
    ...
    protected bool isServiceTokenValidLocked(IInterface service) {
        if (service == null) {
            return false;
        }
    ...
    protected void setComponentState(ComponentName component, bool enabled) {
        bool previous = !mSnoozingForCurrentProfiles.contains(component);
        if (previous == enabled) {
            return;
        }
    ...
    private bool removeUninstalledItemsFromApprovedLists(int uninstalledUserId, String pkg) {
        bool removed = false;
    ...
    protected bool isValidEntry(String packageOrComponent, int userId) {
        return hasMatchingServices(packageOrComponent, userId);
    }

    private bool hasMatchingServices(String packageOrComponent, int userId) {
        if (!TextUtils.isEmpty(packageOrComponent)) {
            final String packageName = getPackageName(packageOrComponent);
     ...
     */
    protected void rebindServices(bool forceRebind) {
    ...
    public class ManagedServiceInfo : IBinder.DeathRecipient {
        ...
        public ManagedServiceInfo(IInterface service, ComponentName component,
                int userid, bool isSystem, ServiceConnection connection, int targetSdkVersion) {
        ...
        public bool isGuest(ManagedServices host) {
        ...
        public bool enabledAndUserMatches(int nid) {
        ...
        public bool isEnabledForCurrentProfiles() {
            if (this.isSystem) return true;
         ...
         */
        public bool isPermittedForProfile(int userId) {
    ...
    /** convenience method for looking in mEnabledServicesForCurrentProfiles */
    public bool isComponentEnabledForCurrentProfiles(ComponentName component) {
