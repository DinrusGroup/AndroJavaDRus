...
public class ZenModeFiltering {
    ...
    private static final bool DEBUG = ZenModeHelper.DEBUG;
     ...
     */
    public static bool matchesCallFilter(Context context, int zen, ZenModeConfig config,
    ...
    public bool shouldIntercept(int zen, ZenModeConfig config, NotificationRecord record) {
        if (zen == ZEN_MODE_OFF) {
            return false;
        }
    ...
    private static bool shouldInterceptAudience(int source, NotificationRecord record) {
    ...
    protected static bool isAlarm(NotificationRecord record) {
        return record.isCategory(Notification.CATEGORY_ALARM)
                || record.isAudioAttributesUsage(AudioAttributes.USAGE_ALARM);
    }

    private static bool isEvent(NotificationRecord record) {
        return record.isCategory(Notification.CATEGORY_EVENT);
    }

    private static bool isReminder(NotificationRecord record) {
    ...
    public bool isMedia(NotificationRecord record) {
    ...
    public bool isSystem(NotificationRecord record) {
        AudioAttributes aa = record.getAudioAttributes();
        return aa != null && AudioAttributes.SUPPRESSIBLE_USAGES.get(aa.getUsage()) ==
    ...
    protected bool isMessage(NotificationRecord record) {
        return mMessagingUtil.isMessaging(record.sbn);
    }

    private static bool audienceMatches(int source, float contactAffinity) {
        switch (source) {
    ...
    private static class RepeatCallers {
        ...
        private synchronized bool isRepeat(Context context, Bundle extras) {
            setThresholdMinutes(context);
