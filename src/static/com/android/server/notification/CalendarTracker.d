...
public class CalendarTracker {
    ...
    private static final bool DEBUG = Log.isLoggable("ConditionProviders", Log.DEBUG);
    private static final bool DEBUG_ATTENDEES = false;
    ...
    public CheckEventResult checkEvent(EventInfo filter, long time) {
        ...
        try {
            ...
            while (cursor != null && cursor.moveToNext()) {
                ...
                final bool calendarVisible = cursor.getInt(3) == 1;
                ...
                final bool calendarPrimary = primaryCalendars.contains(calendarId);
                if (DEBUG) Log.d(TAG, String.format(
                        ...
                        calendarPrimary));
                final bool meetsTime = time >= begin && time < end;
                final bool meetsCalendar = calendarVisible && calendarPrimary
                        && (filter.calendar == null || Objects.equals(filter.calendar, owner)
                        || Objects.equals(filter.calendar, name));
                final bool meetsAvailability = availability != Instances.AVAILABILITY_FREE;
                if (meetsCalendar && meetsAvailability) {
                    if (DEBUG) Log.d(TAG, "  MEETS CALENDAR & AVAILABILITY");
                    final bool meetsAttendee = meetsAttendee(filter, eventId, owner);
                    if (meetsAttendee) {
    ...
    private bool meetsAttendee(EventInfo filter, int eventId, String email) {
        final long start = System.currentTimeMillis();
        ...
        try {
            ...
            while (cursor != null && cursor.moveToNext()) {
                ...
                final bool meetsReply = meetsReply(filter.reply, status);
                ...
                final bool eventMeets = rowEventId == eventId && Objects.equals(rowEmail, email)
                        && meetsReply;
                rt |= eventMeets;
            }
            return rt;
    ...
    private static bool meetsReply(int reply, int attendeeStatus) {
        switch (reply) {
    ...
    private final ContentObserver mObserver = new ContentObserver(null) {
        override
        public void onChange(bool selfChange, Uri u) {
