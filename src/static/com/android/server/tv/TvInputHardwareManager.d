 ...
 * @hide
 */
class TvInputHardwareManager : TvInputHal.Callback {
    ...
    private bool checkUidChangedLocked(
    ...
    private static bool intArrayContains(int[] array, int value) {
     ...
     */
    public bool captureFrame(String inputId, Surface surface, final TvStreamConfig config,
        ...
        synchronized (mLock) {
            ...
            if (hardwareImpl != null) {
                ...
                bool result = hardwareImpl.startCapture(surface, config);
                if (result) {
    ...
    private class Connection : IBinder.DeathRecipient {
    ...
    private class TvInputHardwareImpl : ITvInputHardware.Stub {
        ...
        private void updateAudioConfigLocked() {
            bool sinkUpdated = updateAudioSinkLocked();
            bool sourceUpdated = updateAudioSourceLocked();
            ...
            bool shouldRecreateAudioPatch = sourceUpdated || sinkUpdated;
        ...
        private bool startCapture(Surface surface, TvStreamConfig config) {
            synchronized (mImplLock) {
                if (mReleased) {
                    return false;
                }
        ...
        private bool stopCapture(TvStreamConfig config) {
            synchronized (mImplLock) {
                if (mReleased) {
                    return false;
                }
        ...
        private bool updateAudioSourceLocked() {
            if (mInfo.getAudioType() == AudioManager.DEVICE_NONE) {
                return false;
            }
        ...
        private bool updateAudioSinkLocked() {
            if (mInfo.getAudioType() == AudioManager.DEVICE_NONE) {
                return false;
            }
    ...
    private class ListenerHandler : Handler {
    ...
    private final class HdmiHotplugEventListener : IHdmiHotplugEventListener.Stub {
        override
    ...
    private final class HdmiDeviceEventListener : IHdmiDeviceEventListener.Stub {
        override
    ...
    private final class HdmiSystemAudioModeChangeListener :
        ...
        override
        public void onStatusChanged(bool enabled) throws RemoteException {
            synchronized (mLock) {
                for (int i = 0; i < mConnections.size(); ++i) {
