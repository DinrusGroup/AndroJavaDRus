...
class AlarmManagerService : SystemService {
    ...
    static final bool DEBUG_BATCH = localLOGV || false;
    static final bool DEBUG_VALIDATE = localLOGV || false;
    static final bool DEBUG_ALARM_CLOCK = localLOGV || false;
    static final bool DEBUG_LISTENER_CALLBACK = localLOGV || false;
    static final bool DEBUG_WAKELOCK = localLOGV || false;
    static final bool DEBUG_BG_LIMIT = localLOGV || false;
    static final bool DEBUG_STANDBY = localLOGV || false;
    static final bool RECORD_ALARMS_IN_HISTORY = true;
    static final bool RECORD_DEVICE_IDLE_ALARMS = false;
    ...
    static final bool WAKEUP_STATS = false;
    ...
    bool mLastWakeLockUnimportantForLogging;
    ...
    private bool mNextAlarmClockMayChange;
    ...
    private bool mAppStandbyParole;
    ...
    final class Batch {
        ...
        bool canHold(long whenElapsed, long maxWhen) {
        ...
        bool remove(Predicate<Alarm> predicate) {
        ...
        bool hasPackage(final String packageName) {
    ...
    static class BatchTimeOrder : Comparator<Batch> {
    ...
    static long convertToElapsed(long when, int type) {
        final bool isRtc = (type == RTC || type == RTC_WAKEUP);
        if (isRtc) {
    ...
    static bool addBatchLocked(ArrayList<Batch> list, Batch newBatch) {
    ...
    bool haveAlarmsTimeTickAlarm(ArrayList<Alarm> alarms) {
        if (alarms.size() == 0) {
            return false;
        }
    ...
    bool haveBatchesTimeTickAlarm(ArrayList<Batch> batches) {
    ...
    void rebatchAllAlarmsLocked(bool doValidate) {
        final long start = mStatLogger.getTime();
        final int oldCount =
                getAlarmCount(mAlarmBatches) + ArrayUtils.size(mPendingWhileIdleAlarms);
        final bool oldHasTick = haveBatchesTimeTickAlarm(mAlarmBatches)
        ...
        final int newCount =
                getAlarmCount(mAlarmBatches) + ArrayUtils.size(mPendingWhileIdleAlarms);
        final bool newHasTick = haveBatchesTimeTickAlarm(mAlarmBatches)
                || haveAlarmsTimeTickAlarm(mPendingWhileIdleAlarms);
     ...
     */
    bool reorderAlarmsBasedOnStandbyBuckets(ArraySet<Pair<String, Integer>> targetPackages) {
        final long start = mStatLogger.getTime();
    ...
    void reAddAlarmLocked(Alarm a, long nowElapsed, bool doValidate) {
        a.when = a.origWhen;
    ...
    private void setImplLocked(int type, long when, long whenElapsed, long windowLength,
            ...
            String listenerTag, int flags, bool doValidate, WorkSource workSource,
     ...
     */
    private bool adjustDeliveryTimeBasedOnStandbyBucketLocked(Alarm alarm) {
    ...
    private void setImplLocked(Alarm a, bool rebatching, bool doValidate) {
     ...
     */
    private final class LocalService : AlarmManagerInternal {
        override
    ...
    void dumpImpl(PrintWriter pw) {
        synchronized (mLock) {
            ...
            for (int user : users) {
                ...
                final bool pendingSend = mPendingSendNextAlarmClockChangedForUser.get(user);
    ...
    private bool validateConsistencyLocked() {
        if (DEBUG_VALIDATE) {
    ...
    void interactiveStateChangedLocked(bool interactive) {
    ...
    bool lookForPackageLocked(String packageName) {
    ...
    private bool isBackgroundRestricted(Alarm alarm) {
        bool exemptOnBatterySaver = (alarm.flags & FLAG_ALLOW_WHILE_IDLE) != 0;
        if (alarm.alarmClock != null) {
    ...
    private long getWhileIdleMinIntervalLocked(int uid) {
        final bool dozing = mPendingIdleUntil != null;
        final bool ebs = (mAppStateTracker != null)
    ...
    bool triggerAlarmsLocked(ArrayList<Alarm> triggerList, final long nowELAPSED,
            final long nowRTC) {
        bool hasWakeup = false;
     ...
     */
    public static class IncreasingTimeOrder : Comparator<Alarm> {
    ...
    static class Alarm {
        ...
        public bool matches(PendingIntent pi, IAlarmListener rec) {
    ...
    bool checkAllowNonWakeupDelayLocked(long nowELAPSED) {
        if (mInteractive) {
            return false;
        }
    ...
    void deliverAlarmsLocked(ArrayList<Alarm> triggerList, long nowELAPSED) {
        ...
        for (int i=0; i<triggerList.size(); i++) {
            ...
            final bool allowWhileIdle = (alarm.flags&AlarmManager.FLAG_ALLOW_WHILE_IDLE) != 0;
            if (alarm.wakeup) {
            ...
            while (true)
            {
                ...
                if (result != TIME_CHANGED_MASK) {
                    ...
                    synchronized (mLock) {
                        ...
                        bool hasWakeup = triggerAlarmsLocked(triggerList, nowELAPSED, nowRTC);
                        if (!hasWakeup && checkAllowNonWakeupDelayLocked(nowELAPSED)) {
    ...
    void setWakelockWorkSource(PendingIntent pi, WorkSource ws, int type, String tag,
            int knownUid, bool first) {
        try {
            final bool unimportant = pi == mTimeTickSender;
    ...
    private class AlarmHandler : Handler {
    ...
    class InteractiveStateReceiver : BroadcastReceiver {
    ...
    final class UidObserver : IUidObserver.Stub {
     ...
     */
    final class AppStandbyTracker : UsageStatsManagerInternal.AppIdleStateChangeListener {
        override
        public void onAppIdleStateChanged(final String packageName, final @UserIdInt int userId,
                bool idle, int bucket, int reason) {
            if (DEBUG_STANDBY) {
    ...
    private final Listener mForceAppStandbyListener = new Listener() {
        ...
        override
        public void onUidForeground(int uid, bool foreground) {
            synchronized (mLock) {
                if (foreground) {
    ...
    class DeliveryTracker : IAlarmCompleteListener.Stub : PendingIntent.OnFinished {
         ...
         */
        @GuardedBy("mLock")
        public void deliverLocked(Alarm alarm, long nowELAPSED, bool allowWhileIdle) {
            if (alarm.operation != null) {
                // PendingIntent alarm
                mSendCount++;
