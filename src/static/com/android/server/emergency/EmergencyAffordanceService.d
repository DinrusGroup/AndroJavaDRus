 ...
 * should be enabled
 */
public class EmergencyAffordanceService : SystemService {
     ...
     * requires the emergency affordance. The value is a bool (1 or 0).
     * @hide
     */
    ...
    private bool mEmergencyAffordanceNeeded;
    ...
    private bool mSimNeedsEmergencyAffordance;
    private bool mNetworkNeedsEmergencyAffordance;
    ...
    private bool isEmergencyAffordanceNeeded() {
        synchronized (mLock) {
    ...
    private bool handleUpdateSimSubscriptionInfo() {
        bool neededBefore = simNeededAffordanceBefore();
        bool neededNow = neededBefore;
    ...
    private void setSimNeedsEmergencyAffordance(bool simNeedsEmergencyAffordance) {
    ...
    private bool simNeededAffordanceBefore() {
        return Settings.Global.getInt(mContext.getContentResolver(),
                EMERGENCY_SIM_INSERTED_SETTING, 0) != 0;
    }

    private bool handleUpdateCellInfo() {
        List<CellInfo> cellInfos = mTelephonyManager.getAllCellInfo();
        if (cellInfos == null) {
            return false;
        }
        bool stopScanningAfterScan = false;
    ...
    private void setNetworkNeedsEmergencyAffordance(bool needsAffordance) {
        synchronized (mLock) {
    ...
    private bool mccRequiresEmergencyAffordance(int mcc) {
