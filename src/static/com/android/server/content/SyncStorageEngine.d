...
public class SyncStorageEngine {
    ...
    private static final bool SYNC_ENABLED_DEFAULT = false;
    ...
    private volatile bool mIsClockValid;
    ...
    public static class EndPoint {
         ...
         * and match any.
         */
        public bool matchesSpec(EndPoint spec) {
    ...
    private static class AccountAuthorityValidator {
        ...
        // on the device
        bool isAccountValid(Account account, int userId) {
        ...
        bool isAuthorityValid(String authority, int userId) {
    ...
    private bool mDefaultMasterSyncAutomatically;

    private OnSyncRequestListener mSyncRequestListener;
    private OnAuthorityRemovedListener mAuthorityRemovedListener;

    private bool mGrantSyncAdaptersAccountAccess;
    ...
    public bool getSyncAutomatically(Account account, int userId, String providerName) {
        synchronized (mAuthorities) {
            if (account != null) {
    ...
    public void setSyncAutomatically(Account account, int userId, String providerName,
            bool sync, @SyncExemption int syncExemptionFlag, int callingUid) {
        if (Log.isLoggable(TAG, Log.VERBOSE)) {
     ...
     */
    private bool setBackoffLocked(Account account, int userId, String providerName,
    ...
    public void setMasterSyncAutomatically(bool flag, int userId,
    ...
    public bool getMasterSyncAutomatically(int userId) {
        synchronized (mAuthorities) {
            Boolean auto = mMasterSyncAutomatically.get(userId);
     ...
     * target.
     */
    public bool isSyncActive(EndPoint info) {
        synchronized (mAuthorities) {
    ...
    public void markPending(EndPoint info, bool pendingValue) {
        synchronized (mAuthorities) {
     ...
     */
    public List<SyncInfo> getCurrentSyncsCopy(int userId, bool canAccessAccounts) {
        synchronized (mAuthorities) {
    ...
    public bool isSyncPending(EndPoint info) {
        synchronized (mAuthorities) {
            final int N = mSyncStatus.size();
            for (int i = 0; i < N; i++) {
                SyncStatusInfo cur = mSyncStatus.valueAt(i);
     ...
     * exists.
     */
    private AuthorityInfo getOrCreateAuthorityLocked(EndPoint info, int ident, bool doWrite) {
    ...
    private AuthorityInfo createAuthorityLocked(EndPoint info, int ident, bool doWrite) {
        AuthorityInfo authority;
        if (ident < 0) {
    ...
    public bool shouldGrantSyncAdaptersAccountAccess() {
    ...
    private void parseListenForTickles(XmlPullParser parser) {
        ...
        bool listen = enabled == null || Boolean.parseBoolean(enabled);
    ...
    private void readAndDeleteLegacyAccountInfoLocked() {
        ...
        if (db != null) {
            final bool hasType = db.getVersion() >= 11;
    ...
    public bool isClockValid() {
        return mIsClockValid;
    }

    public void resetTodayStats(bool force) {
        if (force) {
