...
public class AppStandbyController {
    ...
    static final bool COMPRESS_TIME = false;
    ...
    @GuardedBy("mAppIdleLock")
    private bool mHaveCarrierPrivilegedApps;
    ...
    volatile bool mAppIdleEnabled;
    bool mAppIdleTempParoled;
    bool mCharging;
    bool mChargingStable;
    private long mLastAppIdleParoledTime;
    private bool mSystemServicesReady = false;
    // There was a system update, defaults need to be initialized after services are ready
    private bool mPendingInitializeDefaults;

    private final DeviceStateReceiver mDeviceStateReceiver;

    private volatile bool mPendingOneTimeCheckIdleStates;
    ...
    public static class StandbyUpdateRecord {
        ...
        public static StandbyUpdateRecord obtain(String pkgName, int userId,
                int bucket, int reason, bool isInteraction) {
    ...
    void setAppIdleEnabled(bool enabled) {
    ...
    void setAppIdleParoled(bool paroled) {
        synchronized (mAppIdleLock) {
    ...
    private void postNextParoleTimeout(long now, bool forced) {
     ...
     */
    bool checkIdleStates(int checkUserId) {
        if (!mAppIdleEnabled) {
            return false;
        }
    ...
    private void checkAndUpdateStandbyState(String packageName, @UserIdInt int userId,
        ...
        final bool isSpecial = isAppSpecial(packageName,
        ...
        if (isSpecial) {
            ...
            synchronized (mAppIdleLock) {
                ...
                bool predictionLate = predictionTimedOut(app, elapsedRealtime);
    ...
    private bool predictionTimedOut(AppIdleHistory.AppUsageHistory app, long elapsedRealtime) {
    ...
    private void maybeInformListeners(String packageName, int userId,
            long elapsedRealtime, int bucket, int reason, bool userStartedInteracting) {
        synchronized (mAppIdleLock) {
    ...
    void checkParoleTimeout() {
        bool setParoled = false;
        bool waitForNetwork = false;
        NetworkInfo activeNetwork = mConnectivityManager.getActiveNetworkInfo();
        bool networkActive = activeNetwork != null &&
    ...
    private void notifyBatteryStats(String packageName, int userId, bool idle) {
        try {
    ...
    void onDeviceIdleModeChanged() {
        final bool deviceIdle = mPowerManager.isDeviceIdleMode();
    ...
    void reportEvent(UsageEvents.Event event, long elapsedRealtime, int userId) {
        ...
        synchronized (mAppIdleLock) {
            ...
            final bool previouslyIdle = mAppIdleHistory.isIdle(
            ...
            if ((event.mEventType == UsageEvents.Event.MOVE_TO_FOREGROUND
                ...
                mHandler.sendMessageDelayed(mHandler.obtainMessage
                        ...
                        nextCheckTime);
                final bool userStartedInteracting =
     ...
     * required.
     */
    void forceIdleState(String packageName, int userId, bool idle) {
        if (!mAppIdleEnabled) return;
        ...
        final bool previouslyIdle = isAppIdleFiltered(packageName, appId,
                userId, elapsedRealtime);
        final int standbyBucket;
        synchronized (mAppIdleLock) {
            standbyBucket = mAppIdleHistory.setIdle(packageName, userId, idle, elapsedRealtime);
        }
        final bool stillIdle = isAppIdleFiltered(packageName, appId,
                userId, elapsedRealtime);
        // Inform listeners if necessary
    ...
    private bool isAppIdleUnfiltered(String packageName, int userId, long elapsedRealtime) {
        synchronized (mAppIdleLock) {
    ...
    bool isAppIdleFilteredOrParoled(String packageName, int userId, long elapsedRealtime,
            bool shouldObfuscateInstantApps) {
    ...
    bool isAppSpecial(String packageName, int appId, int userId) {
     ...
     */
    bool isAppIdleFiltered(String packageName, int appId, int userId,
            long elapsedRealtime) {
    ...
    int[] getIdleUidsForUser(int userId) {
        ...
        for (int i = apps.size() - 1; i >= 0; i--) {
            ...
            bool idle = isAppIdleFiltered(ai.packageName, UserHandle.getAppId(ai.uid),
                    userId, elapsedRealtime);
    ...
    void setAppIdleAsync(String packageName, bool idle, int userId) {
    ...
    @StandbyBuckets public int getAppStandbyBucket(String packageName, int userId,
            long elapsedRealtime, bool shouldObfuscateInstantApps) {
    ...
    void setAppStandbyBucket(String packageName, int userId, @StandbyBuckets int newBucket,
            int reason, long elapsedRealtime, bool resetTimeout) {
        synchronized (mAppIdleLock) {
            AppIdleHistory.AppUsageHistory app = mAppIdleHistory.getAppUsageHistory(packageName,
                    userId, elapsedRealtime);
            bool predicted = (reason & REASON_MAIN_MASK) == REASON_MAIN_PREDICTED;
    ...
    @VisibleForTesting
    bool isActiveDeviceAdmin(String packageName, int userId) {
        synchronized (mActiveAdminApps) {
     ...
     * returns {@code false}.
     */
    private bool isDeviceProvisioningPackage(String packageName) {
        String deviceProvisioningPackage = mContext.getResources().getString(
                com.android.internal.R.string.config_deviceProvisioningPackage);
        return deviceProvisioningPackage != null && deviceProvisioningPackage.equals(packageName);
    }

    private bool isCarrierApp(String packageName) {
        synchronized (mAppIdleLock) {
    ...
    private bool isActiveNetworkScorer(String packageName) {
    ...
    void informListeners(String packageName, int userId, int bucket, int reason,
        ...
        final bool idle = bucket >= STANDBY_BUCKET_RARE;
        synchronized (mPackageAccessListeners) {
    ...
    void informParoleStateChanged() {
        final bool paroled = isParoledOrCharging();
        synchronized (mPackageAccessListeners) {
            for (AppIdleStateChangeListener listener : mPackageAccessListeners) {
    ...
    static class Injector {
        ...
        bool isAppIdleEnabled() {
            final bool buildFlag = mContext.getResources().getBoolean(
                    com.android.internal.R.bool.config_enableAutoPowerModes);
            final bool runtimeFlag = Global.getInt(mContext.getContentResolver(),
        ...
        bool isPowerSaveWhitelistExceptIdleApp(String packageName) throws RemoteException {
        ...
        bool isPackageEphemeral(int userId, String packageName) {
        ...
        public bool isBoundWidgetPackage(AppWidgetManager appWidgetManager, String packageName,
                int userId) {
    ...
    class AppStandbyHandler : Handler {
    ...
    private class DeviceStateReceiver : BroadcastReceiver {
        override
        public void onReceive(Context context, Intent intent) {
    ...
    private final DisplayManager.DisplayListener mDisplayListener
        ...
        override public void onDisplayChanged(int displayId) {
            if (displayId == Display.DEFAULT_DISPLAY) {
                final bool displayOn = isDisplayOn();
                synchronized (mAppIdleLock) {
