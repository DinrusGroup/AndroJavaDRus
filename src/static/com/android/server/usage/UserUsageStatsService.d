...
class UserUsageStatsService {
    ...
    private static final bool DEBUG = UsageStatsService.DEBUG;
    ...
    private bool mStatsChanged = false;
    ...
    private static final StatCombiner<UsageStats> sUsageStatsCombiner =
            new StatCombiner<UsageStats>() {
                override
                public void combine(IntervalStats stats, bool mutable,
    ...
    void checkin(final IndentingPrintWriter pw) {
        mDatabase.checkinDailyFiles(new UsageStatsDatabase.CheckinAction() {
            override
            public bool checkin(IntervalStats stats) {
    ...
    void dump(IndentingPrintWriter pw, String pkg, bool compact) {
    ...
    private String formatDateTime(long dateTime, bool pretty) {
        if (pretty) {
    ...
    private String formatElapsedTime(long elapsedTime, bool pretty) {
        if (pretty) {
    ...
    void printEvent(IndentingPrintWriter pw, UsageEvents.Event event, bool prettyDates) {
    ...
    void printLast24HrEvents(IndentingPrintWriter pw, bool prettyDates, final String pkg) {
    ...
    void printEventAggregation(IndentingPrintWriter pw, String label,
            IntervalStats.EventTracker tracker, bool prettyDates) {
    ...
    void printIntervalStats(IndentingPrintWriter pw, IntervalStats stats,
            bool prettyDates, bool skipEvents, String pkg) {
        if (prettyDates) {
            pw.printPair("timeRange", "\"" + DateUtils.formatDateRange(mContext,
