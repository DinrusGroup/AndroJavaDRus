 ...
 */
class CommonTimeManagementService : Binder {
    /*
    ...
    private static final bool AUTO_DISABLE;
    private static final bool ALLOW_WIFI;
    ...
    static {
        ...
        if (tmp > 30)
            ...
            BASE_SERVER_PRIO = cast(byte)tmp;

        if (ALLOW_WIFI) {
            IFACE_SCORE_RULES = new InterfaceScoreRule[] {
                new InterfaceScoreRule("wlan", cast(byte)1),
                new InterfaceScoreRule("eth", cast(byte)2),
            };
        } else {
            IFACE_SCORE_RULES = new InterfaceScoreRule[] {
                new InterfaceScoreRule("eth", cast(byte)2),
    ...
    private bool mDetectedAtStartup = false;
    ...
    private void reevaluateServiceState() {
        ...
        if (doRebind && (null != mCTConfig)) {
            byte newPrio = (bestScore > 0)
                         ? cast(byte)(bestScore * BASE_SERVER_PRIO)
                         : BASE_SERVER_PRIO;
