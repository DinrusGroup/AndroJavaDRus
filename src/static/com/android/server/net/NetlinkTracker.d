 ...
 * @hide
 */
public class NetlinkTracker : BaseNetworkObserver {
    ...
    public void interfaceDnsServerInfo(String iface, long lifetime, String[] addresses) {
        if (mInterfaceName.equals(iface)) {
            ...
            bool changed = mDnsServerRepository.addServers(lifetime, addresses);
            if (changed) {
                synchronized (this) {
 ...
 */
class DnsServerEntry : Comparable<DnsServerEntry> {
...
class DnsServerRepository {
     ...
     */
    public synchronized bool addServers(long lifetime, String[] addresses) {
    ...
    private synchronized bool updateExistingEntry(InetAddress address, long expiry) {
    ...
    private synchronized bool updateCurrentServers() {
        long now = System.currentTimeMillis();
        bool changed = false;
