 ...
 * @hide
 */
public class JobSchedulerService : com.android.server.SystemService
        : StateChangedListener, JobCompletedListener {
    public static final String TAG = "JobScheduler";
    public static final bool DEBUG = Log.isLoggable(TAG, Log.DEBUG);
    public static final bool DEBUG_STANDBY = DEBUG || false;

    /** The maximum number of concurrent jobs we run at one time. */
    private static final int MAX_JOB_CONTEXTS_COUNT = 16;
    /** Enforce a per-app limit on scheduled jobs? */
    private static final bool ENFORCE_MAX_JOBS = true;
     ...
     */
    bool[] mTmpAssignAct = new bool[MAX_JOB_CONTEXTS_COUNT];
    /**
    ...
    private class ConstantsObserver : ContentObserver {
     ...
     */
    private bool isUidActive(int uid) {
     ...
     *
     */
    public bool cancelJobsForUid(int uid, String reason) {
        if (uid == Process.SYSTEM_UID) {
            Slog.wtfStack(TAG, "Can't cancel all jobs for system uid");
            return false;
        }

        bool jobsCanceled = false;
        synchronized (mLock) {
            final List<JobStatus> jobsForUid = mJobs.getJobsByUid(uid);
     ...
     */
    public bool cancelJob(int uid, int jobId, int callingUid) {
        JobStatus toCancel;
        synchronized (mLock) {
    ...
    override
    public void onDeviceIdleStateChanged(bool deviceIdle) {
        synchronized (mLock) {
            if (deviceIdle) {
    ...
    void reportActiveLocked() {
        ...
        bool active = mPendingJobs.size() > 0;
    ...
    private void startTrackingJobLocked(JobStatus jobStatus, JobStatus lastJob) {
        ...
        final bool update = mJobs.add(jobStatus);
        if (mReadyToRock) {
     ...
     * object removed.
     */
    private bool stopTrackingJobLocked(JobStatus jobStatus, JobStatus incomingJob,
            bool writeBack) {
        // Deal with any remaining work items in the old job.
        jobStatus.stopTrackingJobLocked(ActivityManager.getService(), incomingJob);

        // Remove from store as well as controllers.
        final bool removed = mJobs.remove(jobStatus, writeBack);
    ...
    private bool stopJobOnServiceContextLocked(JobStatus job, int reason, String debugReason) {
        for (int i=0; i<mActiveServices.size(); i++) {
            JobServiceContext jsc = mActiveServices.get(i);
            final JobStatus executing = jsc.getRunningJobLocked();
     ...
     * is pending.
     */
    private bool isCurrentlyActiveLocked(JobStatus job) {
        for (int i=0; i<mActiveServices.size(); i++) {
     ...
     */
    override
    public void onJobCompletedLocked(JobStatus jobStatus, bool needsReschedule) {
        if (DEBUG) {
    ...
    final private class JobHandler : Handler {
        ...
        public void handleMessage(Message message) {
            synchronized (mLock) {
                ...
                switch (message.what) {
                    ...
                    case MSG_UID_GONE: {
                        final int uid = message.arg1;
                        final bool disabled = message.arg2 != 0;
    ...
    final class ReadyJobQueueFunctor : Consumer<JobStatus> {
     ...
     */
    final class MaybeReadyJobQueueFunctor : Consumer<JobStatus> {
        int chargingCount;
     ...
     */
    class HeartbeatAlarmListener : AlarmManager.OnAlarmListener {
     ...
     */
    private bool isReadyToBeExecutedLocked(JobStatus job) {
        final bool jobReady = job.isReady();
        ...
        final bool jobExists = mJobs.containsJob(job);

        final int userId = job.getUserId();
        final bool userStarted = ArrayUtils.contains(mStartedUsers, userId);
        ...
        final bool jobPending = mPendingJobs.contains(job);
        final bool jobActive = isCurrentlyActiveLocked(job);
    ...
    private void assignJobsToContextsLocked() {
        ...
        for (int i=0; i<MAX_JOB_CONTEXTS_COUNT; i++) {
            bool preservePreferredUid = false;
            if (act[i]) {
    ...
    final class LocalService : JobSchedulerInternal {
    ...
    static class DeferredJobCounter : Consumer<JobStatus> {
     ...
     */
    final class JobSchedulerStub : IJobScheduler.Stub {
        ...
        private bool canPersistJobs(int pid, int uid) {
    ...
    int executeRunCommand(String pkgName, int userId, int jobId, bool force) {
        if (DEBUG) {
    ...
    void setMonitorBattery(bool enabled) {
        synchronized (mLock) {
    ...
    void triggerDockState(bool idleState) {
