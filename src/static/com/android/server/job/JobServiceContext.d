 ...
 */
public final class JobServiceContext : ServiceConnection {
    private static final bool DEBUG = JobSchedulerService.DEBUG;
    private static final bool DEBUG_STANDBY = JobSchedulerService.DEBUG_STANDBY;
    ...
    final class JobCallback : IJobCallback.Stub {
        public String mStoppedReason;
        public long mStoppedTime;

        override
        public void acknowledgeStartMessage(int jobId, bool ongoing) {
            doAcknowledgeStartMessage(this, jobId, ongoing);
        }

        override
        public void acknowledgeStopMessage(int jobId, bool reschedule) {
        ...
        override
        public bool completeWork(int jobId, int workId) {
            return doCompleteWork(this, jobId, workId);
        }

        override
        public void jobFinished(int jobId, bool reschedule) {
     ...
     */
    bool executeRunnableJob(JobStatus job) {
        synchronized (mLock) {
            if (!mAvailable) {
            ...
            bool binding = mContext.bindServiceAsUser(intent, this,
    ...
    @GuardedBy("mLock")
    bool timeoutIfExecutingLocked(String pkgName, int userId, bool matchJobId, int jobId,
            String reason) {
    ...
    void doJobFinished(JobCallback cb, int jobId, bool reschedule) {
        doCallback(cb, reschedule, "app called jobFinished");
    }

    void doAcknowledgeStopMessage(JobCallback cb, int jobId, bool reschedule) {
        doCallback(cb, reschedule, null);
    }

    void doAcknowledgeStartMessage(JobCallback cb, int jobId, bool ongoing) {
    ...
    bool doCompleteWork(JobCallback cb, int jobId, int workId) {
        final long ident = Binder.clearCallingIdentity();
        try {
            synchronized (mLock) {
                assertCallerLocked(cb);
     ...
     */
    private bool verifyCallerLocked(JobCallback cb) {
     ...
     */
    private class JobServiceHandler : Handler {
    ...
    void doCallback(JobCallback cb, bool reschedule, String reason) {
        final long ident = Binder.clearCallingIdentity();
        try {
            synchronized (mLock) {
                if (!verifyCallerLocked(cb)) {
    ...
    @GuardedBy("mLock")
    void doCallbackLocked(bool reschedule, String reason) {
        if (DEBUG) {
     ...
     */
    @GuardedBy("mLock")
    private void handleStartedLocked(bool workOngoing) {
        switch (mVerb) {
            case VERB_STARTING:
     ...
     */
    @GuardedBy("mLock")
    private void handleFinishedLocked(bool reschedule, String reason) {
        switch (mVerb) {
            case VERB_EXECUTING:
            case VERB_STOPPING:
     ...
     */
    @GuardedBy("mLock")
    private void closeAndCleanupJobLocked(bool reschedule, String reason) {
