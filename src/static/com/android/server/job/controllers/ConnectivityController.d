 ...
 */
public final class ConnectivityController : StateController :
    ...
    private static bool isInsane(JobStatus jobStatus, Network network,
    ...
    @SuppressWarnings("unused")
    private static bool isCongestionDelayed(JobStatus jobStatus, Network network,
            NetworkCapabilities capabilities, Constants constants) {
    ...
    @SuppressWarnings("unused")
    private static bool isStrictSatisfied(JobStatus jobStatus, Network network,
            NetworkCapabilities capabilities, Constants constants) {
    ...
    @SuppressWarnings("unused")
    private static bool isRelaxedSatisfied(JobStatus jobStatus, Network network,
            NetworkCapabilities capabilities, Constants constants) {
    ...
    @VisibleForTesting
    static bool isSatisfied(JobStatus jobStatus, Network network,
            NetworkCapabilities capabilities, Constants constants) {
    ...
    private bool updateConstraintsSatisfied(JobStatus jobStatus) {
    ...
    private bool updateConstraintsSatisfied(JobStatus jobStatus, Network network,
            NetworkCapabilities capabilities) {
        // TODO: consider matching against non-active networks

        final bool ignoreBlocked = (jobStatus.getFlags() & JobInfo.FLAG_WILL_BE_FOREGROUND) != 0;
        final NetworkInfo info = mConnManager.getNetworkInfoForUid(network,
                jobStatus.getSourceUid(), ignoreBlocked);

        final bool connected = (info != null) && info.isConnected();
        final bool satisfied = isSatisfied(jobStatus, network, capabilities, mConstants);

        final bool changed = jobStatus
    ...
    private void updateTrackedJobs(int filterUid, Network filterNetwork) {
        synchronized (mLock) {
            ...
            for (int i = mTrackedJobs.size() - 1; i >= 0; i--) {
                ...
                final bool uidMatch = (filterUid == -1 || filterUid == uid);
                if (uidMatch) {
                    ...
                    final bool networkMatch = (filterNetwork == null
                            || Objects.equals(filterNetwork, network));
                    final bool forceUpdate = !Objects.equals(js.network, network);
