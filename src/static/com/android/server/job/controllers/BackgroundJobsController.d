 ...
 *
 * - the uid-active bool state expressed by the AppStateTracker.  Jobs in 'active'
 *    uids are inherently eligible to run jobs regardless of the uid's standby bucket.
 */
public final class BackgroundJobsController : StateController {
    ...
    private void updateJobRestrictionsForUidLocked(int uid, bool isActive) {
    ...
    bool updateSingleJobRestrictionLocked(JobStatus jobStatus, int activeState) {

        final int uid = jobStatus.getSourceUid();
        final String packageName = jobStatus.getSourcePackageName();

        final bool canRun = !mAppStateTracker.areJobsRestricted(uid, packageName,
        ...
        bool didChange = jobStatus.setBackgroundNotRestrictedConstraintSatisfied(canRun);
        didChange |= jobStatus.setUidActive(isActive);
        return didChange;
    }

    private final class UpdateJobFunctor : Consumer<JobStatus> {
    ...
    private final Listener mForceAppStandbyListener = new Listener() {
        ...
        override
        public void updateJobsForUid(int uid, bool isActive) {
            synchronized (mLock) {
        ...
        override
        public void updateJobsForUidPackage(int uid, String packageName, bool isActive) {
            synchronized (mLock) {
                updateJobRestrictionsForUidLocked(uid, isActive);
            }
        }
