 ...
 */
public final class DeviceIdleJobsController : StateController {
    ...
    void updateIdleMode(bool enabled) {
        bool changed = false;
        synchronized (mLock) {
     ...
     */
    public void setUidActiveLocked(int uid, bool active) {
        final bool changed = (active != mForegroundUids.get(uid));
        if (!changed) {
            return;
        }
        if (DEBUG) {
     ...
     */
    bool isWhitelistedLocked(JobStatus job) {
     ...
     */
    bool isTempWhitelistedLocked(JobStatus job) {
        return ArrayUtils.contains(mPowerSaveTempWhitelistAppIds,
                UserHandle.getAppId(job.getSourceUid()));
    }

    private bool updateTaskStateLocked(JobStatus task) {
        final bool allowInIdle = ((task.getFlags()&JobInfo.FLAG_IMPORTANT_WHILE_FOREGROUND) != 0)
                && (mForegroundUids.get(task.getSourceUid()) || isTempWhitelistedLocked(task));
        final bool whitelisted = isWhitelistedLocked(task);
        final bool enableTask = !mDeviceIdleMode || whitelisted || allowInIdle;
    ...
    final class DeviceIdleUpdateFunctor : Consumer<JobStatus> {
        bool mChanged;
    ...
    final class DeviceIdleJobsDelayHandler : Handler {
