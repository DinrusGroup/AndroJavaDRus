...
public final class JobStatus {
    ...
    static final bool DEBUG = JobSchedulerService.DEBUG;
    ...
    static final bool DEBUG_PREPARE = true;
    ...
    public bool completeWorkLocked(IActivityManager am, int workId) {
        if (executingWork != null) {
    ...
    bool setChargingConstraintSatisfied(bool state) {
        return setConstraintSatisfied(CONSTRAINT_CHARGING, state);
    }

    bool setBatteryNotLowConstraintSatisfied(bool state) {
        return setConstraintSatisfied(CONSTRAINT_BATTERY_NOT_LOW, state);
    }

    bool setStorageNotLowConstraintSatisfied(bool state) {
        return setConstraintSatisfied(CONSTRAINT_STORAGE_NOT_LOW, state);
    }

    bool setTimingDelayConstraintSatisfied(bool state) {
        return setConstraintSatisfied(CONSTRAINT_TIMING_DELAY, state);
    }

    bool setDeadlineConstraintSatisfied(bool state) {
        return setConstraintSatisfied(CONSTRAINT_DEADLINE, state);
    }

    bool setIdleConstraintSatisfied(bool state) {
        return setConstraintSatisfied(CONSTRAINT_IDLE, state);
    }

    bool setConnectivityConstraintSatisfied(bool state) {
        return setConstraintSatisfied(CONSTRAINT_CONNECTIVITY, state);
    }

    bool setContentTriggerConstraintSatisfied(bool state) {
        return setConstraintSatisfied(CONSTRAINT_CONTENT_TRIGGER, state);
    }

    bool setDeviceNotDozingConstraintSatisfied(bool state, bool whitelisted) {
        dozeWhitelisted = whitelisted;
        return setConstraintSatisfied(CONSTRAINT_DEVICE_NOT_DOZING, state);
    }

    bool setBackgroundNotRestrictedConstraintSatisfied(bool state) {
        return setConstraintSatisfied(CONSTRAINT_BACKGROUND_NOT_RESTRICTED, state);
    }

    bool setUidActive(final bool newActiveState) {
    ...
    bool setConstraintSatisfied(int constraint, bool state) {
        bool old = (satisfiedConstraints&constraint) != 0;
        if (old == state) {
            return false;
        }
    ...
    bool isConstraintSatisfied(int constraint) {
        return (satisfiedConstraints&constraint) != 0;
    }

    bool clearTrackingController(int which) {
     ...
     * already-computed bool, which we updated whenever we see one of the states it depends
     * on here change.
     */
    public bool isReady() {
        ...
        final bool deadlineSatisfied = (!job.isPeriodic() && hasDeadlineConstraint()
                && (satisfiedConstraints & CONSTRAINT_DEADLINE) != 0);
        final bool notDozing = (satisfiedConstraints & CONSTRAINT_DEVICE_NOT_DOZING) != 0
                || (job.getFlags() & JobInfo.FLAG_WILL_BE_FOREGROUND) != 0;
        final bool notRestrictedInBg =
    ...
    public bool matches(int uid, int jobId) {
    ...
    public void dump(PrintWriter pw, String prefix, bool full, long elapsedRealtimeMillis) {
    ...
    public void dump(ProtoOutputStream proto, long fieldId, bool full, long elapsedRealtimeMillis) {
        final long token = proto.start(fieldId);
