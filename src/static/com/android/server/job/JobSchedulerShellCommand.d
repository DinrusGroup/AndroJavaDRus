...
public final class JobSchedulerShellCommand : ShellCommand {
    ...
    private bool printError(int errCode, String pkgName, int userId, int jobId) {
        PrintWriter pw;
        switch (errCode) {
    ...
    private int getBatteryCharging(PrintWriter pw) {
        bool val = mInternal.getBatteryCharging();
        pw.println(val);
        return 0;
    }

    private int getBatteryNotLow(PrintWriter pw) {
        bool val = mInternal.getBatteryNotLow();
        pw.println(val);
        return 0;
    }
    ...
    private int getStorageNotLow(PrintWriter pw) {
        bool val = mInternal.getStorageNotLow();
        pw.println(val);
        return 0;
    }
