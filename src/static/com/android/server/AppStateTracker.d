...
public class AppStateTracker {
     ...
     */
    @GuardedBy("mLock")
    bool mForceAllAppStandbyForSmallBattery;

    /**
     * True if the forced app standby feature is enabled in settings
     */
    @GuardedBy("mLock")
    bool mForcedAppStandbyEnabled;
    ...
    @VisibleForTesting
    class FeatureFlagsObserver : ContentObserver {
        ...
        public void onChange(bool selfChange, Uri uri) {
            if (Settings.Global.getUriFor(Settings.Global.FORCED_APP_STANDBY_ENABLED).equals(uri)) {
                final bool enabled = isForcedAppStandbyEnabled();
                synchronized (mLock) {
                ...
                final bool enabled = isForcedAppStandbyForSmallBatteryEnabled();
                synchronized (mLock) {
    ...
    public static abstract class Listener {
        ...
        private void onUidActiveStateChanged(AppStateTracker sender, int uid) {
            final bool isActive = sender.isUidActive(uid);
         ...
         * scheduler should re-evaluate all restrictions for all jobs.
         */
        public void updateJobsForUid(int uid, bool isNowActive) {
        }
         ...
         * scheduler should re-evaluate all restrictions for all jobs.
         */
        public void updateJobsForUidPackage(int uid, String packageName, bool isNowActive) {
        }
     ...
     */
    @GuardedBy("mLock")
    private void toggleForceAllAppsStandbyLocked(bool enable) {
     ...
     */
    @GuardedBy("mLock")
    bool isRunAnyRestrictedLocked(int uid, @NonNull String packageName) {
     ...
     */
    @GuardedBy("mLock")
    bool updateForcedAppStandbyUidPackageLocked(int uid, @NonNull String packageName,
            bool restricted) {
        final int index = findForcedAppStandbyUidPackageIndexLocked(uid, packageName);
        final bool wasRestricted = index >= 0;
    ...
    private static bool addUidToArray(SparseBooleanArray array, int uid) {
    ...
    private static bool removeUidFromArray(SparseBooleanArray array, int uid, bool remove) {
        if (UserHandle.isCore(uid)) {
            return false;
        }
        if (!array.get(uid)) {
    ...
    private final class UidObserver : IUidObserver.Stub {
        override
        ...
        override
        public void onUidCachedChanged(int uid, bool cached) {
        }
    }

    private final class AppOpsWatcher : IAppOpsCallback.Stub {
        override
    ...
    private final class MyReceiver : BroadcastReceiver {
        override
    ...
    final class StandbyTracker : AppIdleStateChangeListener {
        override
        public void onAppIdleStateChanged(String packageName, int userId, bool idle,
    ...
    private class MyHandler : Handler {
        ...
        public void handleUidGone(int uid, bool disabled) {
            removeUid(uid, true);
        }

        public void handleUidIdle(int uid, bool disabled) {
            // Just to avoid excessive memcpy, don't remove from the array in this case.
            removeUid(uid, false);
        }

        private void removeUid(int uid, bool remove) {
            synchronized (mLock) {
     ...
     */
    @VisibleForTesting
    static bool isAnyAppIdUnwhitelisted(int[] prevArray, int[] newArray) {
        int i1 = 0;
        int i2 = 0;
        bool prevFinished;
        bool newFinished;
     ...
     */
    public bool areAlarmsRestricted(int uid, @NonNull String packageName,
            bool isExemptOnBatterySaver) {
     ...
     */
    public bool areJobsRestricted(int uid, @NonNull String packageName,
            bool hasForegroundExemption) {
     ...
     */
    public bool areForegroundServicesRestricted(int uid, @NonNull String packageName) {
        synchronized (mLock) {
     ...
     */
    private bool isRestricted(int uid, @NonNull String packageName,
            bool useTempWhitelistToo, bool exemptOnBatterySaver) {
        if (isUidActive(uid)) {
            return false;
        }
        synchronized (mLock) {
     ...
     */
    public bool isUidActive(int uid) {
        if (UserHandle.isCore(uid)) {
            return true;
        }
        synchronized (mLock) {
     ...
     */
    public bool isUidActiveSynced(int uid) {
        if (isUidActive(uid)) { // Use the cached one first.
            return true;
        }
        final long start = mStatLogger.getTime();

        final bool ret = mActivityManagerInternal.isUidActive(uid);
     ...
     */
    public bool isUidInForeground(int uid) {
        if (UserHandle.isCore(uid)) {
            return true;
        }
        synchronized (mLock) {
     ...
     *
     */
    bool isForceAllAppsStandbyEnabled() {
        synchronized (mLock) {
            return mForceAllAppsStandby;
        }
    }
     ...
     */
    public bool isRunAnyInBackgroundAppOpsAllowed(int uid, @NonNull String packageName) {
        synchronized (mLock) {
            return !isRunAnyRestrictedLocked(uid, packageName);
        }
    }
     ...
     * Note clients normally shouldn't need to access it. It's only for dumpsys.
     */
    public bool isUidPowerSaveWhitelisted(int uid) {
        synchronized (mLock) {
     ...
     */
    public bool isUidPowerSaveUserWhitelisted(int uid) {
        synchronized (mLock) {
     ...
     * Note clients normally shouldn't need to access it. It's only for dumpsys.
     */
    public bool isUidTempPowerSaveWhitelisted(int uid) {
        synchronized (mLock) {
