 ...
 * @hide
 */
public class NsdService : INsdManager.Stub {
    ...
    private class NsdStateMachine : StateMachine {
        ...
        class EnabledState : State {
            ...
            private bool requestLimitReached(ClientInfo clientInfo) {
            ...
            private bool handleNativeEvent(int code, String raw, String[] cooked) {
                NsdServiceInfo servInfo;
    ...
    private void sendNsdStateChangeBroadcast(bool isEnabled) {
    ...
    private bool isNsdEnabled() {
        bool ret = mNsdSettings.isEnabled();
        if (DBG) {
    ...
    class NativeCallbackReceiver : INativeDaemonConnectorCallbacks {
    ...
    private bool registerService(int regId, NsdServiceInfo service) {
        if (DBG) {
    ...
    private bool unregisterService(int regId) {
        return mDaemon.execute("stop-register", regId);
    }

    private bool updateService(int regId, DnsSdTxtRecord t) {
        if (t == null) {
            return false;
        }
    ...
    private bool discoverServices(int discoveryId, String serviceType) {
        return mDaemon.execute("discover", discoveryId, serviceType);
    }

    private bool stopServiceDiscovery(int discoveryId) {
        return mDaemon.execute("stop-discover", discoveryId);
    }

    private bool resolveService(int resolveId, NsdServiceInfo service) {
        String name = service.getServiceName();
        String type = service.getServiceType();
        return mDaemon.execute("resolve", resolveId, name, type, "local.");
    }

    private bool stopResolveService(int resolveId) {
        return mDaemon.execute("stop-resolve", resolveId);
    }

    private bool getAddrInfo(int resolveId, String hostname) {
        return mDaemon.execute("getaddrinfo", resolveId, hostname);
    }

    private bool stopGetAddrInfo(int resolveId) {
    ...
    public interface NsdSettings {
        bool isEnabled();
        void putEnabledStatus(bool isEnabled);
        ...
        static NsdSettings makeDefault(Context context) {
            ...
            return new NsdSettings() {
                ...
                override
                public void putEnabledStatus(bool isEnabled) {
