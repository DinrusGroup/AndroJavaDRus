 ...
 */
class ActivityStack<T : StackWindowController> : ConfigurationContainer
        : StackWindowListener {
    ...
    // is being started.
    private static final bool SHOW_APP_STARTING_PREVIEW = true;
    ...
    private bool mUpdateBoundsDeferred;
    private bool mUpdateBoundsDeferredCalled;
    ...
    private bool mTopActivityOccludesKeyguard;
    ...
    private class ActivityStackHandler : Handler {
    ...
    ActivityStack(ActivityDisplay display, int stackId, ActivityStackSupervisor supervisor,
            int windowingMode, int activityType, bool onTop) {
    ...
    T createStackWindowController(int displayId, bool onTop, Rect outBounds) {
    ...
    void setWindowingMode(int preferredWindowingMode, bool animate, bool showRecents,
            bool enteringSplitScreenMode, bool deferEnsuringVisibility) {
        final bool creating = mWindowContainerController == null;
        ...
        final bool alreadyInSplitScreenMode = display.hasSplitScreenPrimaryStack();

        // Don't send non-resizeable notifications if the windowing mode changed was a side effect
        // of us entering split-screen mode.
        final bool sendNonResizeableNotification = !enteringSplitScreenMode;
        ...
        if (alreadyInSplitScreenMode && windowingMode == WINDOWING_MODE_FULLSCREEN
            ...
            final bool preferredSplitScreen =
    ...
    override
    public bool isCompatible(int windowingMode, int activityType) {
    ...
    void reparent(ActivityDisplay activityDisplay, bool onTop) {
     ...
     */
    private void postAddToDisplay(ActivityDisplay activityDisplay, Rect bounds, bool onTop) {
    ...
    /**
     * @see #getStackDockedModeBounds(Rect, Rect, Rect, bool)
     */
    void getStackDockedModeBounds(Rect currentTempTaskBounds, Rect outStackBounds,
            Rect outTempTaskBounds, bool ignoreVisibility) {
     ...
     */
    bool deferScheduleMultiWindowModeChanged() {
        return false;
    }
    ...
    void continueUpdateBounds() {
        final bool wasDeferred = mUpdateBoundsDeferred;
    ...
    bool updateBoundsAllowed(Rect bounds, Rect tempTaskBounds,
    ...
    private ActivityRecord topRunningActivityLocked(bool focusableOnly) {
        for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
    ...
    final bool updateLRUListLocked(ActivityRecord r) {
        final bool hadit = mLRUActivities.remove(r);
        mLRUActivities.add(r);
        return hadit;
    }

    final bool isHomeOrRecentsStack() {
        return isActivityTypeHome() || isActivityTypeRecents();
    }

    final bool isOnHomeDisplay() {
        return mDisplayId == DEFAULT_DISPLAY;
    }

    private bool returnsToHomeStack() {
    ...
    void findTaskLocked(ActivityRecord target, FindTaskResult result) {
        ...
        bool isDocument = intent != null & intent.isDocument();
     ...
     */
    bool goToSleepIfPossible(bool shuttingDown) {
    ...
    private bool containsActivityFromStack(List<ActivityRecord> rs) {
     ...
     */
    final bool startPausingLocked(bool userLeaving, bool uiSleeping,
            ActivityRecord resuming, bool pauseImmediately) {
        if (mPausingActivity != null) {
    ...
    final void activityPausedLocked(IBinder token, bool timeout) {
    ...
    private void completePauseLocked(bool resumeNext, ActivityRecord resuming) {
        ActivityRecord prev = mPausingActivity;
        if (DEBUG_PAUSE) Slog.v(TAG_PAUSE, "Complete pause: " + prev);

        if (prev != null) {
            prev.setWillCloseOrEnterPip(false);
            final bool wasStopping = prev.isState(STOPPING);
    ...
    void addToStopping(ActivityRecord r, bool scheduleIdle, bool idleDelayed) {
        ...
        bool forceIdle = mStackSupervisor.mStoppingActivities.size() > MAX_STOPPING_TO_FORCE
     ...
     */
    @VisibleForTesting
    bool isStackTranslucent(ActivityRecord starting) {
    ...
    bool isTopStackOnDisplay() {
        return getDisplay().isTopStack(this);
    }

    bool isTopActivityVisible() {
        final ActivityRecord topActivity = getTopActivity();
     ...
     * @param starting The currently starting activity or null if there is none.
     */
    bool shouldBeVisible(ActivityRecord starting) {
        if (!isAttached() || mForceHidden) {
            return false;
        }
        ...
        final ActivityDisplay display = getDisplay();
        bool gotSplitScreenStack = false;
        bool gotOpaqueSplitScreenPrimary = false;
        bool gotOpaqueSplitScreenSecondary = false;
        final int windowingMode = getWindowingMode();
        final bool isAssistantType = isActivityTypeAssistant();
     ...
     * @see #ensureActivitiesVisibleLocked(ActivityRecord, int, bool)
     * @see ActivityStackSupervisor#ensureActivitiesVisibleLocked(ActivityRecord, int, bool)
     */
    // TODO: Should be re-worked based on the fact that each task as a stack in most cases.
    final void ensureActivitiesVisibleLocked(ActivityRecord starting, int configChanges,
            bool preserveWindows, bool notifyClients) {
        ...
        try {
            ...
            bool aboveTop = top != null;
            final bool stackShouldBeVisible = shouldBeVisible(starting);
            bool behindFullscreenActivity = !stackShouldBeVisible;
            bool resumeNextActivity = mStackSupervisor.isFocusedStack(this)
                    && (isInStackLocked(starting) == null);
            final bool isTopNotPinnedStack =
            ...
            for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
                ...
                for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
                    ...
                    final bool visibleIgnoringKeyguard = r.shouldBeVisibleIgnoringKeyguard(
                            behindFullscreenActivity);
                    r.visibleIgnoringKeyguard = visibleIgnoringKeyguard;

                    // Now check whether it's really visible depending on Keyguard state.
                    final bool reallyVisible = checkKeyguardVisibility(r,
    ...
    void addStartingWindowsForVisibleActivities(bool taskSwitch) {
        for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
     ...
     */
    bool topActivityOccludesKeyguard() {
     ...
     */
    bool resizeStackWithLaunchBounds() {
        return inPinnedWindowingMode();
    }

    override
    public bool supportsSplitScreenWindowingMode() {
        final TaskRecord topTask = topTask();
    ...
    bool affectedBySplitScreenResize() {
     ...
     */
    bool checkKeyguardVisibility(ActivityRecord r, bool shouldBeVisible, bool isTop) {
        final int displayId = mDisplayId != INVALID_DISPLAY ? mDisplayId : DEFAULT_DISPLAY;
        final bool keyguardOrAodShowing = mStackSupervisor.getKeyguardController()
                .isKeyguardOrAodShowing(displayId);
        final bool keyguardLocked = mStackSupervisor.getKeyguardController().isKeyguardLocked();
        final bool showWhenLocked = r.canShowWhenLocked();
        final bool dismissKeyguard = r.hasDismissKeyguardWindows();
        if (shouldBeVisible) {
            ...
            final bool canShowWithKeyguard = canShowWithInsecureKeyguard()
     ...
     */
    private bool canShowWithInsecureKeyguard() {
    ...
    private bool makeVisibleAndRestartIfNeeded(ActivityRecord starting, int configChanges,
            bool isTop, bool andResume, ActivityRecord r) {
    ...
    private void makeInvisible(ActivityRecord r) {
        ...
        try {
            final bool canEnterPictureInPicture = r.checkEnterPictureInPictureState(
            ...
            final bool deferHidingClient = canEnterPictureInPicture
    ...
    private bool updateBehindFullscreen(bool stackInvisible, bool behindFullscreenActivity,
            ActivityRecord r) {
        if (r.fullscreen) {
    ...
    void cancelInitializingActivities() {
        ...
        bool behindFullscreenActivity = false;
     ...
     */
    @GuardedBy("mService")
    bool resumeTopActivityUncheckedLocked(ActivityRecord prev, ActivityOptions options) {
    ...
    @GuardedBy("mService")
    private bool resumeTopActivityInnerLocked(ActivityRecord prev, ActivityOptions options) {
        ...
        final bool hasRunningActivity = next != null;
        ...
        bool userLeaving = mStackSupervisor.mUserLeaving;
        ...
        final bool resumeWhilePausing = (next.info.flags & FLAG_RESUME_WHILE_PAUSING) != 0
                && !lastResumedCanPip;

        bool pausing = mStackSupervisor.pauseBackStacks(userLeaving, next, false);
        if (mResumedActivity != null) {
            if (DEBUG_STATES) Slog.d(TAG_STATES,
        ...
        if (next.app != null && next.app.thread != null) {
            ...
            final bool lastActivityTranslucent = lastStack != null
    ...
    private bool resumeTopActivityInNextFocusableStack(ActivityRecord prev,
    ...
    void startActivityLocked(ActivityRecord r, ActivityRecord focusedTopActivity,
            bool newTask, bool keepCurTransition, ActivityOptions options) {
     ...
     */
    private bool canEnterPipOnTaskSwitch(ActivityRecord pipCandidate,
    ...
    private bool isTaskSwitch(ActivityRecord r,
     ...
     */
    private ActivityOptions resetTargetTaskIfNeededLocked(TaskRecord task, bool forceReset) {
        ActivityOptions topOptions = null;

        int replyChainEnd = -1;
        bool canMoveOptions = true;
        ...
        for (int i = numActivities - 1; i > rootActivityNdx; --i ) {
            ...
            final bool finishOnTaskLaunch =
                    (flags & ActivityInfo.FLAG_FINISH_ON_TASK_LAUNCH) != 0;
            final bool allowTaskReparenting =
                    (flags & ActivityInfo.FLAG_ALLOW_TASK_REPARENTING) != 0;
            final bool clearWhenTaskReset =
    ...
    private int resetAffinityTaskIfNeededLocked(TaskRecord affinityTask, TaskRecord task,
            bool topTaskIsHigher, bool forceReset, int taskInsertionPoint) {
        int replyChainEnd = -1;
        ...
        for (int i = numActivities - 1; i > rootActivityNdx; --i) {
            ...
            final int flags = target.info.flags;
            bool finishOnTaskLaunch = (flags & ActivityInfo.FLAG_FINISH_ON_TASK_LAUNCH) != 0;
            bool allowTaskReparenting = (flags & ActivityInfo.FLAG_ALLOW_TASK_REPARENTING) != 0;
    ...
    private bool isATopFinishingTask(TaskRecord task) {
        for (int i = mTaskHistory.size() - 1; i >= 0; --i) {
    ...
    bool adjustFocusToNextFocusableStack(String reason) {
     ...
     */
    private bool adjustFocusToNextFocusableStack(String reason, bool allowFocusSelf) {
     ...
     */
    final bool requestFinishActivityLocked(IBinder token, int resultCode,
            Intent resultData, String reason, bool oomAdj) {
    ...
    final bool finishActivityAffinityLocked(ActivityRecord r) {
    ...
    /**
     * See {@link #finishActivityLocked(ActivityRecord, int, Intent, String, bool, bool)}
     */
    final bool finishActivityLocked(ActivityRecord r, int resultCode, Intent resultData,
            String reason, bool oomAdj) {
     ...
     */
    final bool finishActivityLocked(ActivityRecord r, int resultCode, Intent resultData,
            String reason, bool oomAdj, bool pauseImmediately) {
        if (r.finishing) {
        ...
        try {
            ...
            final bool endTask = index <= 0 && !task.isClearingToReuseTask();
            ...
            if (mResumedActivity == r) {
                ...
                final int finishMode = (r.visible || r.nowVisible) ? FINISH_AFTER_VISIBLE
                        : FINISH_AFTER_PAUSE;
                final bool removedActivity = finishCurrentActivityLocked(r, finishMode, oomAdj,
    ...
    final ActivityRecord finishCurrentActivityLocked(ActivityRecord r, int mode, bool oomAdj,
            String reason) {
        ...
        final bool finishingActivityInNonFocusedStack
        ...
        if (mode == FINISH_IMMEDIATELY
            ...
            r.makeFinishingLocked();
            bool activityRemoved = destroyActivityLocked(r, true, "finish-imm:" + reason);
    ...
    void finishAllActivitiesLocked(bool immediately) {
        bool noActivitiesInStack = true;
        for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
            final ArrayList<ActivityRecord> activities = mTaskHistory.get(taskNdx).mActivities;
            for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
                final ActivityRecord r = activities.get(activityNdx);
    ...
    bool shouldUpRecreateTaskLocked(ActivityRecord srec, String destAffinity) {
    ...
    final bool navigateUpToLocked(ActivityRecord srec, Intent destIntent, int resultCode,
            Intent resultData) {
        final TaskRecord task = srec.getTask();
        final ArrayList<ActivityRecord> activities = task.mActivities;
     ...
     */
    private void cleanUpActivityLocked(ActivityRecord r, bool cleanServices, bool setState) {
    ...
    private void removeActivityFromHistoryLocked(ActivityRecord r, String reason) {
        ...
        final TaskRecord task = r.getTask();
        final bool lastActivity = task != null ? task.removeActivity(r) : false;
        ...
        final bool onlyHasTaskOverlays = task != null
    ...
    final bool safelyDestroyActivityLocked(ActivityRecord r, String reason) {
        if (r.isDestroyable()) {
            if (DEBUG_SWITCH) Slog.v(TAG_SWITCH,
     ...
     */
    final bool destroyActivityLocked(ActivityRecord r, bool removeFromApp, String reason) {
    ...
    private bool removeHistoryRecordsForAppLocked(ProcessRecord app) {
        ...
        bool hasVisibleActivities = false;
    ...
    private void updateTaskMovement(TaskRecord task, bool toFront) {
    ...
    final void moveTaskToFrontLocked(TaskRecord tr, bool noAnimation, ActivityOptions options,
     ...
     */
    final bool moveTaskToBackLocked(int taskId) {
     ...
     */
    void ensureVisibleActivitiesConfigurationLocked(ActivityRecord start, bool preserveWindow) {
        if (start == null || !start.visible) {
            return;
        }

        final TaskRecord startTask = start.getTask();
        bool behindFullscreen = false;
    ...
    bool willActivityBeVisibleLocked(IBinder token) {
        for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
            final ArrayList<ActivityRecord> activities = mTaskHistory.get(taskNdx).mActivities;
            for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
                final ActivityRecord r = activities.get(activityNdx);
    ...
    bool finishDisabledPackageActivitiesLocked(String packageName, Set<String> filterByClasses,
            bool doit, bool evenPersistent, int userId) {
    ...
    void getRunningTasks(List<TaskRecord> tasksOut, @ActivityType int ignoreActivityType,
            @WindowingMode int ignoreWindowingMode, int callingUid, bool allowed) {
        bool focusedStack = mStackSupervisor.getFocusedStack() == this;
        bool topTask = true;
        for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
            final TaskRecord task = mTaskHistory.get(taskNdx);
            if (task.getTopActivity() == null) {
     ...
     */
    bool handleAppDiedLocked(ProcessRecord app) {
    ...
    bool dumpActivitiesLocked(FileDescriptor fd, PrintWriter pw, bool dumpAll,
            bool dumpClient, String dumpPackage, bool needSep) {
    ...
    void removeTask(TaskRecord task, String reason, int mode) {
        ...
        final bool removed = mTaskHistory.remove(task);
        ...
        if (mode == REMOVE_TASK_MODE_DESTROYING && task.mActivities.isEmpty()) {
            ...
            final bool isVoiceSession = task.voiceSession != null;
            if (isVoiceSession) {
                try {
    ...
    TaskRecord createTaskRecord(int taskId, ActivityInfo info, Intent intent,
            IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor,
            bool toTop, ActivityRecord activity, ActivityRecord source,
        ...
        final int displayId = mDisplayId != INVALID_DISPLAY ? mDisplayId : DEFAULT_DISPLAY;
        final bool isLockscreenShown = mService.mStackSupervisor.getKeyguardController()
                .isKeyguardOrAodShowing(displayId);
    ...
    void addTask(final TaskRecord task, final bool toTop, String reason) {
    ...
    // with the fall-out...
    void addTask(final TaskRecord task, int position, bool schedulePictureInPictureModeChange,
            String reason) {
        // TODO: Is this remove really needed? Need to look into the call path for the other addTask
        mTaskHistory.remove(task);
        position = getAdjustedPositionForTask(task, position, null /* starting */);
        final bool toTop = position >= mTaskHistory.size();
    ...
    void positionChildAt(TaskRecord task, int index) {
        ...
        final bool wasResumed = topRunningActivity == task.getStack().mResumedActivity;
    ...
    private ActivityStack preAddTask(TaskRecord task, String reason, bool toTop) {
    ...
    private void postAddTask(TaskRecord task, ActivityStack prevStack,
            bool schedulePictureInPictureModeChange) {
    ...
    void moveToFrontAndResumeStateIfNeeded(ActivityRecord r, bool moveToFront, bool setResume,
            bool setPause, String reason) {
        if (!moveToFront) {
            return;
        }
    ...
    bool shouldSleepOrShutDownActivities() {
