...
public class LockTaskController {
     ...
     */
    @VisibleForTesting
    bool isTaskLocked(TaskRecord task) {
     ...
     */
    private bool isRootTask(TaskRecord task) {
     ...
     */
    bool activityBlockedFromFinish(ActivityRecord activity) {
     ...
     */
    bool canMoveTaskToBack(TaskRecord task) {
        if (isRootTask(task)) {
            showLockTaskToast();
            return false;
        }
     ...
     */
    bool isTaskWhitelisted(TaskRecord task) {
     ...
     */
    bool isLockTaskModeViolation(TaskRecord task) {
     ...
     * @return whether the requested task is disallowed to be launched.
     */
    bool isLockTaskModeViolation(TaskRecord task, bool isNewClearTask) {
    ...
    private bool isLockTaskModeViolationInternal(TaskRecord task, bool isNewClearTask) {
     ...
     */
    void stopLockTaskMode(@Nullable TaskRecord task, bool isSystemCaller, int callingUid) {
     ...
     * unlike {@link #stopLockTaskMode(TaskRecord, bool, int)}, it doesn't perform the checks.
     */
     ...
     */
    void startLockTaskMode(@NonNull TaskRecord task, bool isSystemCaller, int callingUid) {
        if (!isSystemCaller) {
    ...
    void updateLockTaskPackages(int userId, String[] packages) {
        ...
        for (int taskNdx = mLockTaskModeTasks.size() - 1; taskNdx >= 0; --taskNdx) {
            ...
            final bool wasWhitelisted = lockedTask.mLockTaskAuth == LOCK_TASK_AUTH_LAUNCHABLE
                    || lockedTask.mLockTaskAuth == LOCK_TASK_AUTH_WHITELISTED;
            lockedTask.setLockTaskAuth();
            final bool isWhitelisted = lockedTask.mLockTaskAuth == LOCK_TASK_AUTH_LAUNCHABLE
                    || lockedTask.mLockTaskAuth == LOCK_TASK_AUTH_WHITELISTED;
    ...
    bool isPackageWhitelisted(int userId, String pkg) {
        if (pkg == null) {
            return false;
        }
        String[] whitelist;
    ...
    private void lockKeyguardIfNeeded() {
        try {
            bool shouldLockKeyguard = Settings.Secure.getIntForUser(
