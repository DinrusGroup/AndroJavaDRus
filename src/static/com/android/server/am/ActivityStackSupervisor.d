...
public class ActivityStackSupervisor : ConfigurationContainer : DisplayListener,
    ...
    static final bool CREATE_IF_NEEDED = true;

    // Used to indicate that windows of activities should be preserved during the resize.
    static final bool PRESERVE_WINDOWS = true;

    // Used to indicate if an object (e.g. task) should be moved/created
    // at the top of its container (e.g. stack).
    static final bool ON_TOP = true;

    // Don't execute any calls to resume.
    static final bool DEFER_RESUME = true;

    // Used to indicate that a task is removed it should also be removed from recents.
    static final bool REMOVE_FROM_RECENTS = true;
    ...
    static final bool VALIDATE_WAKE_LOCK_CALLER = false;
    ...
    bool mUserLeaving = false;

    /** Set when a power hint has started, but not ended. */
    private bool mPowerHintSent;
    ...
    private bool mTaskLayersChanged = true;
    ...
    bool canPlaceEntityOnDisplay(int displayId, bool resizeable, int callingPid,
     ...
     */
    private bool displayConfigMatchesGlobal(int displayId) {
        if (displayId == DEFAULT_DISPLAY) {
            return true;
        }
     ...
     */
    bool mAppVisibilitiesChangedSinceLastPause;
     ...
     */
    private bool mAllowDockedStackResize = true;

    /**
     * Is dock currently minimized.
     */
    bool mIsDockMinimized;
    ...
    bool isFocusable(ConfigurationContainer container, bool alwaysFocusable) {
    ...
    bool isFocusedStack(ActivityStack stack) {
    ...
    bool moveHomeStackTaskToTop(String reason) {
    ...
    bool resumeHomeStackTask(ActivityRecord prev, String reason) {
        if (!mService.mBooting && !mService.mBooted) {
            // Not ready yet!
            return false;
        }
     ...
     */
    TaskRecord anyTaskForIdLocked(int id, @AnyTaskForIdMatchTaskMode int matchMode,
            @Nullable ActivityOptions aOptions, bool onTop) {
     ...
     */
    private bool taskTopActivityIsUser(TaskRecord task, @UserIdInt int userId) {
    ...
    bool attachApplicationLocked(ProcessRecord app) throws RemoteException {
    ...
    bool allResumedActivitiesComplete() {
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            final ActivityDisplay display = mActivityDisplays.valueAt(displayNdx);
            for (int stackNdx = display.getChildCount() - 1; stackNdx >= 0; --stackNdx) {
                final ActivityStack stack = display.getChildAt(stackNdx);
    ...
    private bool allResumedActivitiesVisible() {
     ...
     */
    bool pauseBackStacks(bool userLeaving, ActivityRecord resuming, bool dontWait) {
        bool someActivityPaused = false;
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            final ActivityDisplay display = mActivityDisplays.valueAt(displayNdx);
            for (int stackNdx = display.getChildCount() - 1; stackNdx >= 0; --stackNdx) {
                final ActivityStack stack = display.getChildAt(stackNdx);
    ...
    bool allPausedActivitiesComplete() {
        bool pausing = true;
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            final ActivityDisplay display = mActivityDisplays.valueAt(displayNdx);
            for (int stackNdx = display.getChildCount() - 1; stackNdx >= 0; --stackNdx) {
    ...
    void reportActivityLaunchedLocked(bool timeout, ActivityRecord r,
     ...
     */
    ActivityRecord topRunningActivityLocked(bool considerKeyguardState) {
     ...
     */
    private bool isValidTopRunningActivity(ActivityRecord record,
            bool considerKeyguardState) {
        if (!considerKeyguardState) {
            return true;
        }

        final bool keyguardLocked = getKeyguardController().isKeyguardLocked();
    ...
    final bool realStartActivityLocked(ActivityRecord r, ProcessRecord app,
            bool andResume, bool checkConfig) throws RemoteException {
     ...
     */
    bool ensureVisibilityAndConfig(ActivityRecord starting, int displayId,
            bool markFrozenIfConfigChanged, bool deferResume) {
    ...
    void startSpecificActivityLocked(ActivityRecord r,
            bool andResume, bool checkConfig) {
    ...
    void sendPowerHintForLaunchStartIfNeeded(bool forceSend, ActivityRecord targetActivity) {
        bool sendHint = forceSend;
    ...
    bool checkStartAnyActivityPermission(Intent intent, ActivityInfo aInfo,
            String resultWho, int requestCode, int callingPid, int callingUid,
            String callingPackage, bool ignoreTargetSecurity, bool launchingInTask,
            ProcessRecord callerApp, ActivityRecord resultRecord, ActivityStack resultStack) {
        final bool isCallerRecents = mService.getRecentTasks() != null &&
    ...
    bool isCallerAllowedToLaunchOnDisplay(int callingPid, int callingUid, int launchDisplayId,
            ActivityInfo aInfo) {
        ...
        final bool uidPresentOnDisplay = activityDisplay.isUidPresent(callingUid);
    ...
    private int getComponentRestrictionForCallingPackage(ActivityInfo activityInfo,
            String callingPackage, int callingPid, int callingUid, bool ignoreTargetSecurity) {
     ...
     */
    @GuardedBy("mService")
    private bool checkFinishBootingLocked() {
        final bool booting = mService.mBooting;
    ...
    // Checked.
    @GuardedBy("mService")
    final ActivityRecord activityIdleInternalLocked(final IBinder token, bool fromTimeout,
            bool processPausingActivities, Configuration config) {
    ...
    bool resumeFocusedStackTopActivityLocked() {
    ...
    bool canUseActivityOptionsLaunchBounds(ActivityOptions options) {
    ...
    protected <T : ActivityStack> T getStack(int stackId) {
    ...
    private <T : ActivityStack> T getStack(int windowingMode, int activityType) {
        for (int i = mActivityDisplays.size() - 1; i >= 0; --i) {
    ...
    <T : ActivityStack> T getLaunchStack(@Nullable ActivityRecord r,
            @Nullable ActivityOptions options, @Nullable TaskRecord candidateTask, bool onTop) {
    ...
    private bool canLaunchOnDisplay(ActivityRecord r, int displayId) {
        if (r == null) {
            return true;
        }
    ...
    private bool isValidLaunchStack(ActivityStack stack, int displayId, ActivityRecord r) {
     ...
     */
    ActivityStack getNextFocusableStackLocked(ActivityStack currentFocus, bool ignoreCurrent) {
        mWindowManager.getDisplaysInFocusOrder(mTmpOrderedDisplayIds);
    ...
    void resizeStackLocked(ActivityStack stack, Rect bounds, Rect tempTaskBounds,
            Rect tempTaskInsetBounds, bool preserveWindows, bool allowResizeInDockedMode,
            bool deferResume) {
        ...
        final bool splitScreenActive = getDefaultDisplay().hasSplitScreenPrimaryStack();
    ...
    private void moveTasksToFullscreenStackInSurfaceTransaction(ActivityStack fromStack,
            int toDisplayId, bool onTop) {

        mWindowManager.deferSurfaceLayout();
        try {
            final int windowingMode = fromStack.getWindowingMode();
            final bool inPinnedWindowingMode = windowingMode == WINDOWING_MODE_PINNED;
            ...
            final bool schedulePictureInPictureModeChange = inPinnedWindowingMode;
    ...
    void moveTasksToFullscreenStackLocked(ActivityStack fromStack, bool onTop) {
        moveTasksToFullscreenStackLocked(fromStack, DEFAULT_DISPLAY, onTop);
    }

    void moveTasksToFullscreenStackLocked(ActivityStack fromStack, int toDisplayId, bool onTop) {
        mWindowManager.inSurfaceTransaction(() ->
                moveTasksToFullscreenStackInSurfaceTransaction(fromStack, toDisplayId, onTop));
    }

    void setSplitScreenResizing(bool resizing) {
    ...
    private void resizeDockedStackLocked(Rect dockedBounds, Rect tempDockedTaskBounds,
            Rect tempDockedTaskInsetBounds, Rect tempOtherTaskBounds, Rect tempOtherTaskInsetBounds,
            bool preserveWindows, bool deferResume) {
    ...
    /**
     * See {@link #removeTaskByIdLocked(int, bool, bool, bool)}
     */
    bool removeTaskByIdLocked(int taskId, bool killProcess, bool removeFromRecents,
            String reason) {
     ...
     */
    bool removeTaskByIdLocked(int taskId, bool killProcess, bool removeFromRecents,
            bool pauseImmediately, String reason) {
    ...
    void cleanUpRemovedTaskLocked(TaskRecord tr, bool killProcess, bool removeFromRecents) {
     ...
     */
    bool restoreRecentTaskLocked(TaskRecord task, ActivityOptions aOptions, bool onTop) {
    ...
    override
    public void onRecentTaskRemoved(TaskRecord task, bool wasTrimmed) {
        if (wasTrimmed) {
     ...
     */
    void moveStackToDisplayLocked(int stackId, int displayId, bool onTop) {
        final ActivityDisplay activityDisplay = getActivityDisplayOrCreateLocked(displayId);
        if (activityDisplay == null) {
    ...
    ActivityStack getReparentTargetStack(TaskRecord task, ActivityStack stack, bool toTop) {
        final ActivityStack prevStack = task.getStack();
        final int stackId = stack.mStackId;
        final bool inMultiWindowMode = stack.inMultiWindowMode();
    ...
    bool moveTopStackActivityToPinnedStackLocked(int stackId, Rect destBounds) {
        final ActivityStack stack = getStack(stackId);
        if (stack == null) {
            throw new IllegalArgumentException(
    ...
    bool moveFocusableActivityStackToFrontLocked(ActivityRecord r, String reason) {
    ...
    bool shutdownLocked(int timeout) {
        goingToSleepLocked();
    ...
    void applySleepTokensLocked(bool applyToStacks) {
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            // Set the sleeping state of the display.
            final ActivityDisplay display = mActivityDisplays.valueAt(displayNdx);
            final bool displayShouldSleep = display.shouldSleep();
    ...
    void checkReadyForSleepLocked(bool allowDelay) {
    ...
    private bool putStacksToSleepLocked(bool allowDelay, bool shuttingDown) {
        bool allSleep = true;
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            final ActivityDisplay display = mActivityDisplays.valueAt(displayNdx);
            for (int stackNdx = display.getChildCount() - 1; stackNdx >= 0; --stackNdx) {
    ...
    bool reportResumedActivityLocked(ActivityRecord r) {
    ...
    bool switchUserLocked(int userId, UserState uss) {
        ...
        final bool homeInFront = stack.isActivityTypeHome();
        if (stack.isOnHomeDisplay()) {
     ...
     */
    bool isStoppingNoHistoryActivity() {
    ...
    final ArrayList<ActivityRecord> processStoppingActivitiesLocked(ActivityRecord idleActivity,
            bool remove, bool processPausingActivities) {
        ArrayList<ActivityRecord> stops = null;

        final bool nowVisible = allResumedActivitiesVisible();
        for (int activityNdx = mStoppingActivities.size() - 1; activityNdx >= 0; --activityNdx) {
            ActivityRecord s = mStoppingActivities.get(activityNdx);
            bool waitingVisible = mActivitiesWaitingForVisibleActivity.contains(s);
            ...
            if (remove) {
                ...
                final bool shouldSleepOrShutDown = stack != null
     ...
     */
    ArrayList<ActivityRecord> getDumpActivitiesLocked(String name, bool dumpVisibleStacksOnly,
            bool dumpFocusedStackOnly) {
    ...
    static bool printThisActivity(PrintWriter pw, ActivityRecord activity, String dumpPackage,
            bool needSep, String prefix) {
        if (activity != null) {
    ...
    bool dumpActivitiesLocked(FileDescriptor fd, PrintWriter pw, bool dumpAll,
            bool dumpClient, String dumpPackage) {
        bool printed = false;
        bool needSep = false;
        for (int displayNdx = 0; displayNdx < mActivityDisplays.size(); ++displayNdx) {
            ActivityDisplay activityDisplay = mActivityDisplays.valueAt(displayNdx);
            ...
            for (int stackNdx = display.getChildCount() - 1; stackNdx >= 0; --stackNdx) {
                ...
                needSep = printed;
                bool pr = printThisActivity(pw, stack.mPausingActivity, dumpPackage, needSep,
                        "    mPausingActivity: ");
                if (pr) {
                    printed = true;
                    needSep = false;
    ...
    static bool dumpHistoryList(FileDescriptor fd, PrintWriter pw, List<ActivityRecord> list,
            String prefix, String label, bool complete, bool brief, bool client,
            String dumpPackage, bool needNL, String header, TaskRecord lastTask) {
        ...
        for (int i=list.size()-1; i>=0; i--) {
            ...
            printed = true;
            final bool full = !brief && (complete || !r.isInHistory());
            if (needNL) {
                pw.println("");
                needNL = false;
            }
    ...
    bool isDisplayAdded(int displayId) {
    ...
    void handleNonResizableTaskIfNeeded(TaskRecord task, int preferredWindowingMode,
            int preferredDisplayId, ActivityStack actualStack, bool forceNonResizable) {
        final bool isSecondaryDisplayPreferred =
                (preferredDisplayId != DEFAULT_DISPLAY && preferredDisplayId != INVALID_DISPLAY);
        final bool inSplitScreenMode = actualStack != null
    ...
    void updatePictureInPictureMode(TaskRecord task, Rect targetStackBounds, bool forceUpdate) {
    ...
    void setDockedStackMinimized(bool minimized) {
     ...
     */
    private bool readyToResume() {
        return mDeferResumeCount == 0;
    }

    private final class ActivityStackSupervisorHandler : Handler {

        public ActivityStackSupervisorHandler(Looper looper) {
            super(looper);
        }

        void activityIdleInternal(ActivityRecord r, bool processPausingActivities) {
            synchronized (mService) {
    ...
    static class WaitInfo {
        ...
        public bool matches(ComponentName targetComponent) {
    ...
    private final class SleepTokenImpl : SleepToken {
