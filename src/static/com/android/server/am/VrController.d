...
final class VrController {
     ...
     */
    public bool onVrModeChanged(ActivityRecord record) {
     ...
     * <p>Note: This must be called with the global ActivityManagerService lock held.
     */
    public bool shouldDisableNonVrUiLocked() {
     ...
     */
    private bool changeVrModeLocked(bool vrMode, ProcessRecord proc) {
        ...
        bool changed = (oldVrState != mVrState);
     ...
     */
    private int updateVrRenderThreadLocked(int newTid, bool suppressLogs) {
     ...
     *          mode after this call completes (this may be the previous thread).
     */
    private int setPersistentVrRenderThreadLocked(int newTid, bool suppressLogs) {
       if (!hasPersistentVrFlagSet()) {
            if (!suppressLogs) {
     ...
     *          mode after this call completes (this may be the previous thread).
     */
    private int setVrRenderThreadLocked(int newTid, int schedGroup, bool suppressLogs) {
        bool inVr = inVrMode();
        bool inPersistentVr = hasPersistentVrFlagSet();
     ...
     * @param suppressLogs {@code true} if any error logging should be disabled.
     */
    private void clearVrRenderThreadLocked(bool suppressLogs) {
     ...
     */
    private bool hasPersistentVrFlagSet() {
