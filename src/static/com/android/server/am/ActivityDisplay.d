 ...
 */
class ActivityDisplay : ConfigurationContainer<ActivityStack>
    ...
    <T : ActivityStack> T getStack(int stackId) {
     ...
     */
    <T : ActivityStack> T getStack(int windowingMode, int activityType) {
    ...
    private bool alwaysCreateStack(int windowingMode, int activityType) {
     ...
     * @see #createStack(int, int, bool)
     */
    <T : ActivityStack> T getOrCreateStack(int windowingMode, int activityType,
            bool onTop) {
     ...
     * if a compatible stack doesn't exist.
     * @see #getOrCreateStack(int, int, bool)
     */
    <T : ActivityStack> T getOrCreateStack(@Nullable ActivityRecord r,
     ...
     */
    <T : ActivityStack> T createStack(int windowingMode, int activityType, bool onTop) {
    ...
    @VisibleForTesting
    <T : ActivityStack> T createStackUnchecked(int windowingMode, int activityType,
            int stackId, bool onTop) {
        if (windowingMode == WINDOWING_MODE_PINNED) {
     ...
     */
    private bool isWindowingModeSupported(int windowingMode, bool supportsMultiWindow,
            bool supportsSplitScreen, bool supportsFreeform, bool supportsPip,
            int activityType) {
    ...
    int resolveWindowingMode(@Nullable ActivityRecord r, @Nullable ActivityOptions options,
        ...
        final ActivityManagerService service = mSupervisor.mService;
        bool supportsMultiWindow = service.mSupportsMultiWindow;
        bool supportsSplitScreen = service.mSupportsSplitScreenMultiWindow;
        bool supportsFreeform = service.mSupportsFreeformWindowManagement;
        bool supportsPip = service.mSupportsPictureInPicture;
        if (supportsMultiWindow) {
            if (task != null) {
        ...
        final bool inSplitScreenMode = hasSplitScreenPrimaryStack();
    ...
    bool isTopStack(ActivityStack stack) {
        return stack == getTopStack();
    }

    bool isTopNotPinnedStack(ActivityStack stack) {
        for (int i = mStacks.size() - 1; i >= 0; --i) {
    ...
    void remove() {
        final bool destroyContentOnRemoval = shouldDestroyContentOnRemove();
    ...
    private bool shouldDestroyContentOnRemove() {
    ...
    void moveStackBehindBottomMostVisibleStack(ActivityStack stack) {
        ...
        for (int stackNdx = 0; stackNdx < numStacks; stackNdx++) {
            ...
            final bool isValidWindowingMode = winMode == WINDOWING_MODE_FULLSCREEN ||
    ...
    void setIsSleeping(bool asleep) {
        mSleeping = asleep;
    }
