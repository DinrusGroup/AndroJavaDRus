 ...
 */
class TaskLaunchParamsModifier : LaunchParamsModifier {
    ...
    private static final bool ALLOW_RESTART = true;
    ...
    private void position(ArrayList<TaskRecord> tasks, Rect availableRect,
            Rect proposal, bool allowRestart, int shiftPolicy, Rect result) {
    ...
    private bool shiftedTooFar(Rect start, Rect availableRect, int shiftPolicy) {
        switch (shiftPolicy) {
    ...
    private static bool boundsConflict(Rect proposal, ArrayList<TaskRecord> tasks) {
    ...
    private static final bool closeLeftTopCorner(Rect first, Rect second) {
    ...
    private static final bool closeRightTopCorner(Rect first, Rect second) {
        return Math.abs(first.right - second.right) < BOUNDS_CONFLICT_MIN_DISTANCE
                && Math.abs(first.top - second.top) < BOUNDS_CONFLICT_MIN_DISTANCE;
    }

    private static final bool closeLeftBottomCorner(Rect first, Rect second) {
        return Math.abs(first.left - second.left) < BOUNDS_CONFLICT_MIN_DISTANCE
                && Math.abs(first.bottom - second.bottom) < BOUNDS_CONFLICT_MIN_DISTANCE;
    }

    private static final bool closeRightBottomCorner(Rect first, Rect second) {
        return Math.abs(first.right - second.right) < BOUNDS_CONFLICT_MIN_DISTANCE
                && Math.abs(first.bottom - second.bottom) < BOUNDS_CONFLICT_MIN_DISTANCE;
    }
}
