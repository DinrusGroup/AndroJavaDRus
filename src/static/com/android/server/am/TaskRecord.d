...
class TaskRecord : ConfigurationContainer : TaskWindowContainerListener {
    ...
    bool realActivitySuspended; // True if the actual activity component that started the
                                   // task is suspended.
    bool inRecents;      // Actually in the recents list?
    long lastActiveTime;    // Last time this task was active in the current device session,
                            ...
                            // restored from disk.
    bool isAvailable;    // Is the activity available to be launched?
    bool rootWasReset;   // True if the intent at the root of the task had
                            // the FLAG_ACTIVITY_RESET_TASK_IF_NEEDED flag.
    bool autoRemoveRecents;  // If true, we should automatically remove the task from
                                // recents when activity finishes
    bool askedCompatMode;// Have asked the user about compat mode for this task.
    bool hasBeenVisible; // Set if any activities in the task have been visible to the user.

    String stringName;      // caching of toString() result.
    int userId;             // user for which this task was created
    bool mUserSetupComplete; // The user set-up is complete as of the last time the task activity
                                // was changed.
    ...
    private bool mSupportsPictureInPicture;  // Whether or not this task and its activities
    ...
    private bool mNeverRelinquishIdentity = true;

    // Used in the unique case where we are clearing the task in order to reuse it. In that case we
    // do not want to delete the stack when the task goes empty.
    private bool mReuseTask = false;
    ...
    TaskRecord(ActivityManagerService service, int _taskId, Intent _intent,
            ...
            ComponentName _realActivity, ComponentName _origActivity, bool _rootWasReset,
            bool _autoRemoveRecents, bool _askedCompatMode, int _userId,
            int _effectiveUid, String _lastDescription, ArrayList<ActivityRecord> activities,
            long lastTimeMoved, bool neverRelinquishIdentity,
            TaskDescription _lastTaskDescription, int taskAffiliation, int prevTaskId,
            int nextTaskId, int taskAffiliationColor, int callingUid, String callingPackage,
            int resizeMode, bool supportsPictureInPicture, bool _realActivitySuspended,
            bool userSetupComplete, int minWidth, int minHeight) {
        mService = service;
        taskId = _taskId;
        intent = _intent;
    ...
    void createWindowContainer(bool onTop, bool showForAllUsers) {
    ...
    /**
     * Should only be invoked from {@link #createWindowContainer(bool, bool)}.
     */
    @VisibleForTesting
    ...
    void setTaskDockedResizing(bool resizing) {
    ...
    bool resize(Rect bounds, int resizeMode, bool preserveWindow, bool deferResume) {
        ...
        try {
            ...
            final bool forced = (resizeMode & RESIZE_MODE_FORCED) != 0;
            ...
            final bool updatedConfig = updateOverrideConfiguration(bounds);
     ...
     */
    bool reparent(ActivityStack preferredStack, bool toTop,
            @ReparentMoveStackMode int moveStackMode, bool animate, bool deferResume,
            String reason) {
    ...
    bool reparent(ActivityStack preferredStack, bool toTop,
            @ReparentMoveStackMode int moveStackMode, bool animate, bool deferResume,
            bool schedulePictureInPictureModeChange, String reason) {
    ...
    bool reparent(ActivityStack preferredStack, int position,
            @ReparentMoveStackMode int moveStackMode, bool animate, bool deferResume,
            String reason) {
    ...
    bool reparent(ActivityStack preferredStack, int position,
        ...
        final bool mightReplaceWindow = topActivity != null
        ...
        try {
            final ActivityRecord r = topRunningActivityLocked();
            final bool wasFocused = r != null && supervisor.isFocusedStack(sourceStack)
                    && (topRunningActivityLocked() == r);
            final bool wasResumed = r != null && sourceStack.getResumedActivity() == r;
            final bool wasPaused = r != null && sourceStack.mPausingActivity == r;

            // In some cases the focused stack isn't the front stack. E.g. pinned stack.
            // Whenever we are moving the top activity from the front stack we want to make sure to
            // move the stack to the front.
            final bool wasFront = r != null && sourceStack.isTopStackOnDisplay()
            ...
            final bool moveStackToFront = moveStackMode == REPARENT_MOVE_STACK_TO_FRONT
            ...
            final bool toStackSplitScreenPrimary =
     ...
     * is ready.
     */
    private static bool replaceWindowsOnTaskMove(
     ...
     */
    TaskSnapshot getSnapshot(bool reducedResolution) {
     ...
     */
    bool isSameIntentFilter(ActivityRecord r) {
    ...
    ActivityRecord getTopActivity(bool includeOverlays) {
     ...
     */
    bool removeActivity(ActivityRecord r) {
        return removeActivity(r, false /* reparenting */);
    }

    bool removeActivity(ActivityRecord r, bool reparenting) {
        if (r.getTask() != this) {
     ...
     */
    bool onlyHasTaskOverlayActivities(bool excludeFinishing) {
        int count = 0;
        for (int i = mActivities.size() - 1; i >= 0; i--) {
            final ActivityRecord r = mActivities.get(i);
     ...
     */
    final void performClearTaskAtIndexLocked(int activityNdx, bool pauseImmediately,
            String reason) {
    ...
    void removeTaskActivitiesLocked(bool pauseImmediately, String reason) {
    ...
    private bool isResizeable(bool checkSupportsPip) {
    ...
    void updateTaskDescription() {
        ...
        int activityNdx;
        final int numActivities = mActivities.size();
        final bool relinquish = numActivities != 0 &&
    ...
    private void adjustForMinimalTaskDimensions(Rect bounds) {
        ...
        final bool adjustWidth = minWidth > bounds.width();
        final bool adjustHeight = minHeight > bounds.height();
     ...
     */
    bool updateOverrideConfiguration(Rect bounds) {
     ...
     * @return True if the override configuration was updated.
     */
    bool updateOverrideConfiguration(Rect bounds, @Nullable Rect insetBounds) {
        ...
        final bool matchParentBounds = bounds == null || bounds.isEmpty();
        final bool persistBounds = getWindowConfiguration().persistTaskBounds();
    ...
    public void onConfigurationChanged(Configuration newParentConfig) {
        final bool wasInMultiWindowMode = inMultiWindowMode();
    ...
    void computeOverrideConfiguration(Configuration config, Rect bounds, Rect insetBounds,
            bool overrideWidth, bool overrideHeight) {
    ...
    static class TaskRecordFactory {
        ...
        TaskRecord create(ActivityManagerService service, int taskId, Intent intent,
                ...
                ComponentName realActivity, ComponentName origActivity, bool rootWasReset,
                bool autoRemoveRecents, bool askedCompatMode, int userId,
                ...
                int nextTaskId, int taskAffiliationColor, int callingUid, String callingPackage,
                int resizeMode, bool supportsPictureInPicture, bool realActivitySuspended,
                bool userSetupComplete, int minWidth, int minHeight) {
        ...
        TaskRecord restoreFromXml(XmlPullParser in, ActivityStackSupervisor stackSupervisor)
            ...
            bool realActivitySuspended = false;
            ComponentName origActivity = null;
            String affinity = null;
            String rootAffinity = null;
            bool hasRootAffinity = false;
            bool rootHasReset = false;
            bool autoRemoveRecents = false;
            bool askedCompatMode = false;
            int taskType = 0;
            int userId = 0;
            bool userSetupComplete = true;
            int effectiveUid = -1;
            String lastDescription = null;
            long lastTimeOnTop = 0;
            bool neverRelinquishIdentity = true;
