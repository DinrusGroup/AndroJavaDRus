 ...
 */
final class ActivityRecord : ConfigurationContainer : AppWindowContainerListener {
    ...
    private static final bool SHOW_ACTIVITY_START_TIME = true;
    ...
    final bool stateNotNeeded; // As per ActivityInfo.flags
    bool fullscreen; // The activity is opaque and fills the entire space of this task.
    // TODO: See if it possible to combine this with the fullscreen field.
    final bool hasWallpaper; // Has a wallpaper window as a background.
    final bool noDisplay;  // activity is not displayed?
    private final bool componentSpecified;  // did caller specify an explicit component?
    final bool rootVoiceInteraction;  // was this the root activity of a voice interaction?
    ...
    private bool mLastReportedMultiWindowMode;
    private bool mLastReportedPictureInPictureMode;
    ...
    bool frontOfTask;    // is this the root activity of its task?
    bool launchFailed;   // set if a launched failed, to abort on 2nd try
    bool haveState;      // have we gotten the last activity state?
    bool stopped;        // is activity pause finished?
    bool delayedResume;  // not yet resumed because of stopped app switches?
    bool finishing;      // activity in pending finish list?
    bool deferRelaunchUntilPaused;   // relaunch of activity is being deferred until pause is
                                        // completed
    bool preserveWindowOnDeferredRelaunch; // activity windows are preserved on deferred relaunch
    int configChangeFlags;  // which config values have changed
    private bool keysPaused;     // has key dispatching been paused for it?
    int launchMode;         // the launch mode activity attribute.
    int lockTaskLaunchMode; // the lockTaskMode manifest attribute, subject to override
    bool visible;        // does this activity's window need to be shown?
    bool visibleIgnoringKeyguard; // is this activity visible, ignoring the fact that Keyguard
                                     // might hide this activity?
    private bool mDeferHidingClient; // If true we told WM to defer reporting to the client
                                        // process that it is hidden.
    bool sleeping;       // have we told the activity to sleep?
    bool nowVisible;     // is this activity's window visible?
    bool mClientVisibilityDeferred;// was the visibility change message to client deferred?
    bool idle;           // has the activity gone idle?
    bool hasBeenLaunched;// has this activity ever been launched?
    bool frozenBeforeDestroy;// has been frozen but not yet destroyed.
    bool immersive;      // immersive mode (don't interrupt if possible)
    bool forceNewConfig; // force re-create with new config next time
    bool supportsEnterPipOnTaskSwitch;  // This flag is set by the system to indicate that the
    ...
    private bool inHistory;  // are we in the history stack?
    ...
    bool mTaskOverlay = false; // Task is always on-top of other activities in the task.

    TaskDescription taskDescription; // the recents information for this activity
    bool mLaunchTaskBehind; // this activity is actively being launched with
    ...
    bool pendingVoiceInteractionStart;   // Waiting for activity-invoked voice session
    ...
    private bool mShowWhenLocked;
    private bool mTurnScreenOn;

    /**
     * Temp configs used in {@link #ensureActivityConfiguration(int, bool)}
     */
    ...
    private bool crossesHorizontalSizeThreshold(int firstDp, int secondDp) {
        return crossesSizeThreshold(mHorizontalSizeConfigurations, firstDp, secondDp);
    }

    private bool crossesVerticalSizeThreshold(int firstDp, int secondDp) {
        return crossesSizeThreshold(mVerticalSizeConfigurations, firstDp, secondDp);
    }

    private bool crossesSmallestSizeThreshold(int firstDp, int secondDp) {
     ...
     * of the threshold.
     */
    private static bool crossesSizeThreshold(int[] thresholds, int firstDp,
            int secondDp) {
        if (thresholds == null) {
            return false;
        }
    ...
    void updateMultiWindowMode() {
        ...
        final bool inMultiWindowMode = inMultiWindowMode();
    ...
    void updatePictureInPictureMode(Rect targetStackBounds, bool forceUpdate) {
        if (task == null || task.getStack() == null || app == null || app.thread == null) {
            return;
        }

        final bool inPictureInPictureMode = inPinnedWindowingMode() && targetStackBounds != null;
     ...
     */
    void setTask(TaskRecord task, bool reparenting) {
    ...
    /**
     * See {@link AppWindowContainerController#setWillCloseOrEnterPip(bool)}
     */
    void setWillCloseOrEnterPip(bool willCloseOrEnterPip) {
        getWindowContainerController().setWillCloseOrEnterPip(willCloseOrEnterPip);
    }

    static class Token : IApplicationToken.Stub {
    ...
    ActivityRecord(ActivityManagerService _service, ProcessRecord _caller, int _launchedFromPid,
            ...
            bool _componentSpecified, bool _rootVoiceInteraction,
        ...
        if (options != null) {
            ...
            final bool useLockTask = pendingOptions.getLockTaskMode();
    ...
    private bool isHomeIntent(Intent intent) {
    ...
    static bool isMainIntent(Intent intent) {
        return ACTION_MAIN.equals(intent.getAction())
    ...
    private bool canLaunchHomeActivity(int uid, ActivityRecord sourceRecord) {
     ...
     */
    private bool canLaunchAssistActivity(String packageName) {
    ...
    private void setActivityType(bool componentSpecified, int launchedFromUid, Intent intent,
    ...
    bool changeWindowTranslucency(bool toOpaque) {
     ...
     */
    bool isNonResizableOrForcedResizable() {
     ...
     */
    private bool supportsResizeableMultiWindow() {
     ...
     */
    bool canBeLaunchedOnDisplay(int displayId) {
     ...
     */
    bool checkEnterPictureInPictureState(String caller, bool beforeStopping) {
        ...
        bool isKeyguardLocked = service.isKeyguardLocked();
        bool isCurrentAppLocked = service.getLockTaskModeState() != LOCK_TASK_MODE_NONE;
        final ActivityDisplay display = getDisplay();
        bool hasPinnedStack = display != null && display.hasPinnedStack();
        // Don't return early if !isNotLocked, since we want to throw an exception if the activity
        // is in an incorrect state
        bool isNotLockedOrOnKeyguard = !isKeyguardLocked && !isCurrentAppLocked;
     ...
     */
    private bool checkEnterPictureInPictureAppOpsState() {
        try {
     ...
     */
    bool hasDismissKeyguardWindows() {
    ...
    final void deliverNewIntentLocked(int callingUid, Intent intent, String referrer) {
        ...
        bool unsent = true;
        final bool isTopActivityWhileSleeping = isTopRunningActivity() && isSleeping();
        ...
        if (pendingOptions != null
            ...
            switch (animationType) {
                ...
                case ANIM_THUMBNAIL_SCALE_DOWN:
                    final bool scaleUp = (animationType == ANIM_THUMBNAIL_SCALE_UP);
    ...
    void clearOptionsLocked(bool withAbort) {
    ...
    void setDeferHidingClient(bool deferHidingClient) {
     ...
     */
    bool isState(ActivityState state) {
        return state == mState;
    }
     ...
     */
    bool isState(ActivityState state1, ActivityState state2) {
     ...
     * Returns {@code true} if the Activity is in one of the specified states.
     */
    bool isState(ActivityState state1, ActivityState state2, ActivityState state3) {
    ...
    void notifyAppResumed(bool wasStopped) {
     ...
     */
    bool shouldBeVisibleIgnoringKeyguard(bool behindFullscreenActivity) {
        if (!okToShowLocked()) {
            return false;
        }
    ...
    void makeVisibleIfNeeded(ActivityRecord starting, bool reportToClient) {
    ...
    private bool shouldPauseWhenBecomingVisible() {
    ...
    bool continueLaunchTickingLocked() {
        if (launchTickTime == 0) {
            return false;
        }
    ...
    public bool mayFreezeScreenLocked(ProcessRecord app) {
    ...
    public void stopFreezingScreenLocked(bool force) {
    ...
    public void reportFullyDrawnLocked(bool restoredFromBundle) {
    ...
    override
    public bool keyDispatchingTimedOut(String reason, int windowPid) {
        ActivityRecord anrActivity;
        ProcessRecord anrApp;
        bool windowFromSameProcessAsActivity;
        synchronized (service) {
     ...
     */
    public bool isInterestingToUserLocked() {
    ...
    void setSleeping(bool _sleeping, bool force) {
    ...
    static int getTaskForActivityLocked(IBinder token, bool onlyRoot) {
        final ActivityRecord r = ActivityRecord.forTokenLocked(token);
        if (r == null) {
    ...
    void showStartingWindow(ActivityRecord prev, bool newTask, bool taskSwitch) {
    ...
    void showStartingWindow(ActivityRecord prev, bool newTask, bool taskSwitch,
        ...
        final bool shown = mWindowContainerController.addStartingWindow(packageName, theme,
    ...
    void removeOrphanedStartingWindow(bool behindFullscreenActivity) {
    ...
    void setDisablePreviewScreenshots(bool disable) {
    ...
    bool isConfigurationCompatible(Configuration config) {
    ...
    bool ensureActivityConfiguration(int globalChanges, bool preserveWindow) {
    ...
    bool ensureActivityConfiguration(int globalChanges, bool preserveWindow,
        ...
        final bool displayChanged = mLastReportedDisplayId != newDisplayId;
        if (displayChanged) {
     ...
     */
    private bool shouldRelaunchLocked(int changes, Configuration changesConfig) {
        int configChanged = info.getRealConfigChanged();
        bool onlyVrUiModeChanged = onlyVrUiModeChanged(changes, changesConfig);
     ...
     */
    private bool onlyVrUiModeChanged(int changes, Configuration lastReportedConfig) {
    ...
    private int getConfigurationChanges(Configuration lastReportedConfig) {
        ...
        if ((changes & CONFIG_SCREEN_SIZE) != 0) {
            final bool crosses = crossesHorizontalSizeThreshold(lastReportedConfig.screenWidthDp,
    ...
    private static bool isResizeOnlyChange(int change) {
        return (change & ~(CONFIG_SCREEN_SIZE | CONFIG_SMALLEST_SCREEN_SIZE | CONFIG_ORIENTATION
                | CONFIG_SCREEN_LAYOUT)) == 0;
    }

    void relaunchActivityLocked(bool andResume, bool preserveWindow) {
     ...
     */
    private bool allowTaskSnapshot() {
        if (newIntents == null) {
            return true;
        }
    ...
    private static bool isInVrUiMode(Configuration config) {
