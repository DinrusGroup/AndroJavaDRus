...
final class ActivityManagerShellCommand : ShellCommand {
    ...
    private bool mWaitOption = false;
    private bool mStopOption = false;
    ...
    private bool mStreaming;   // Streaming the profiling output to a file.
    private String mAgent;  // Agent to attach on startup.
    private bool mAttachAgentDuringBind;  // Whether agent should be attached late.
    private int mDisplayId;
    private int mWindowingMode;
    private int mActivityType;
    private int mTaskId;
    private bool mIsTaskOverlay;
    private bool mIsLockTask;

    final bool mDumping;

    ActivityManagerShellCommand(ActivityManagerService service, bool dumping) {
        mInterface = service;
        mInternal = service;
    ...
    private Intent makeIntent(int defUser) throws URISyntaxException {
        ...
        return Intent.parseCommandArgs(this, new Intent.CommandOptionHandler() {
            override
            public bool handleOption(String opt, ShellCommand cmd) {
                if (opt.equals("-D")) {
    ...
    int runStartService(PrintWriter pw, bool asForeground) throws RemoteException {
    ...
    final static class IntentReceiver : IIntentReceiver.Stub {
    ...
    static final class MyActivityController : IActivityController.Stub {
        final IActivityManager mInterface;
        ...
        override
        public bool activityResuming(String pkg) {
            synchronized (this) {
        ...
        override
        public bool activityStarting(Intent intent, String pkg) {
            synchronized (this) {
        ...
        override
        public bool appCrashed(String processName, int pid, String shortMsg, String longMsg,
    ...
    static final class MyUidObserver : IUidObserver.Stub
            : ActivityManagerService.OomAdjObserver {
        final IActivityManager mInterface;
        final ActivityManagerService mInternal;
        final PrintWriter mPw;
        final InputStream mInput;
        ...
        override
        public void onUidGone(int uid, bool disabled) throws RemoteException {
            synchronized (this) {
                final StrictMode.ThreadPolicy oldPolicy = StrictMode.allowThreadDiskWrites();
                try {
                    mPw.print(uid);
        ...
        override
        public void onUidIdle(int uid, bool disabled) throws RemoteException {
            synchronized (this) {
                final StrictMode.ThreadPolicy oldPolicy = StrictMode.allowThreadDiskWrites();
                try {
                    mPw.print(uid);
        ...
        override
        public void onUidCachedChanged(int uid, bool cached) throws RemoteException {
            synchronized (this) {
                final StrictMode.ThreadPolicy oldPolicy = StrictMode.allowThreadDiskWrites();
                try {
                    mPw.print(uid);
    ...
    int runStartUser(PrintWriter pw) throws RemoteException {
        String user = getNextArgRequired();
        bool success = mInterface.startUserInBackground(Integer.parseInt(user));
        if (success) {
    ...
    int runUnlockUser(PrintWriter pw) throws RemoteException {
        ...
        bool success = mInterface.unlockUser(userId, token, secret, null);
        if (success) {
    ...
    static final class StopUserCallback : IStopUserCallback.Stub {
        private bool mFinished = false;
    ...
    int runIsUserStopped(PrintWriter pw) {
        ...
        bool stopped = mInternal.isUserStopped(userId);
        pw.println(stopped);
        return 0;
    }
    ...
    int runSuppressResizeConfigChanges(PrintWriter pw) throws RemoteException {
        bool suppress = Boolean.valueOf(getNextArgRequired());
    ...
    int runSetStandbyBucket(PrintWriter pw) throws RemoteException {
        ...
        bool multiple = peekNextArg() != null;
    ...
    int runGetInactive(PrintWriter pw) throws RemoteException {
        ...
        IUsageStatsManager usm = IUsageStatsManager.Stub.asInterface(ServiceManager.getService(
                Context.USAGE_STATS_SERVICE));
        bool isIdle = usm.isAppInactive(packageName, userId);
...
//                    options != null ? options.toBundle() : null, mUserId);
    ...
    int resizeStackUnchecked(int stackId, Rect bounds, int delayMs, bool animate)
    ...
    void taskResize(int taskId, Rect bounds, int delay_ms, bool pretendUserResize)
            throws RemoteException {
    ...
    int moveTask(int taskId, Rect taskRect, Rect stackRect, int stepSize,
            int maxToTravel, bool movingForward, bool horizontal, int delay_ms)
            throws RemoteException {
        int maxMove;
        if (movingForward) {
            while (maxToTravel > 0
    ...
    int getStepSize(int current, int target, int inStepSize, bool greaterThanTarget) {
        int stepSize = 0;
    ...
    static void dumpHelp(PrintWriter pw, bool dumping) {
        if (dumping) {
