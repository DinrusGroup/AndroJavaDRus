...
class RecentTasks {
    ...
    private static final bool TRIMMED = true;

    private static final int DEFAULT_INITIAL_CAPACITY = 5;

    // Whether or not to move all affiliated tasks to the front when one of the tasks is launched
    private static final bool MOVE_AFFILIATED_TASKS_TO_FRONT = false;
    ...
    interface Callbacks {
         ...
         */
        void onRecentTaskRemoved(TaskRecord task, bool wasTrimmed);
    }
    ...
    private bool mHasVisibleRecentTasks;
     ...
     */
    bool isCallerRecents(int callingUid) {
     ...
     *         recents component.
     */
    bool isRecentsComponent(ComponentName cn, int uid) {
     ...
     */
    bool isRecentsComponentHomeActivity(int userId) {
    ...
    private void notifyTaskRemoved(TaskRecord task, bool wasTrimmed) {
        for (int i = 0; i < mCallbacks.size(); i++) {
     ...
     */
    bool containsTaskId(int taskId, int userId) {
        loadPersistedTaskIdsForUserLocked(userId);
     ...
     */
    void notifyTaskPersisterLocked(TaskRecord task, bool flush) {
    ...
    private static bool shouldPersistTaskLocked(TaskRecord task) {
    ...
    void onPackagesSuspendedChanged(String[] packages, bool suspended, int userId) {
    ...
    void cleanupDisabledPackageTasksLocked(String packageName, Set<String> filterByClasses,
        ...
        for (int i = mTasks.size() - 1; i >= 0; --i) {
            ...
            final bool sameComponent = cn != null && cn.getPackageName().equals(packageName)
     ...
     */
    private bool canAddTaskWithoutTrim(TaskRecord task) {
    ...
    ParceledListSlice<ActivityManager.RecentTaskInfo> getRecentTasks(int maxNum, int flags,
            bool getTasksAllowed, bool getDetailedTasks, int userId, int callingUid) {
        final bool withExcluded = (flags & RECENT_WITH_EXCLUDED) != 0;
    ...
    void add(TaskRecord task) {
        ...
        final bool isAffiliated = task.mAffiliatedTaskId != task.taskId
     ...
     */
    bool addToBottom(TaskRecord task) {
     ...
     */
    private bool isActiveRecentTask(TaskRecord task, SparseBooleanArray quietProfileUserIds) {
     ...
     */
    private bool isVisibleRecentTask(TaskRecord task) {
     ...
     */
    private bool isInVisibleRange(TaskRecord task, int numVisibleTasks) {
        // Keep the last most task even if it is excluded from recents
        final bool isExcludeFromRecents =
     ...
     */
    protected bool isTrimmable(TaskRecord task) {
        final ActivityStack stack = task.getStack();
    ...
    private int findRemoveIndexForAddTask(TaskRecord task) {
        ...
        final bool document = intent != null && intent.isDocument();
        ...
        for (int i = 0; i < recentsCount; i++) {
            ...
            if (task != tr) {
                ...
                final bool sameAffinity =
                        task.affinity != null && task.affinity.equals(tr.affinity);
                final bool sameIntent = intent != null && intent.filterEquals(trIntent);
                bool multiTasksAllowed = false;
                final int flags = intent.getFlags();
                if ((flags & (FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_NEW_DOCUMENT)) != 0
                        && (flags & FLAG_ACTIVITY_MULTIPLE_TASK) != 0) {
                    multiTasksAllowed = true;
                }
                final bool trIsDocument = trIntent != null && trIntent.isDocument();
                final bool bothDocuments = document && trIsDocument;
                ...
                if (bothDocuments) {
                    ...
                    final bool sameActivity = task.realActivity != null
    ...
    private bool moveAffiliatedTasksToFront(TaskRecord task, int taskIndex) {
        int recentsCount = mTasks.size();
        TaskRecord top = task;
        ...
        bool sane = top.mAffiliatedTaskId == task.mAffiliatedTaskId;
    ...
    void dump(PrintWriter pw, bool dumpAll, String dumpPackage) {
     ...
     */
    private bool hasCompatibleActivityTypeAndWindowingMode(TaskRecord t1, TaskRecord t2) {
        final int activityType = t1.getActivityType();
        final int windowingMode = t1.getWindowingMode();
        final bool isUndefinedType = activityType == ACTIVITY_TYPE_UNDEFINED;
        final bool isUndefinedMode = windowingMode == WINDOWING_MODE_UNDEFINED;
        final int otherActivityType = t2.getActivityType();
        final int otherWindowingMode = t2.getWindowingMode();
        final bool isOtherUndefinedType = otherActivityType == ACTIVITY_TYPE_UNDEFINED;
        final bool isOtherUndefinedMode = otherWindowingMode == WINDOWING_MODE_UNDEFINED;
        ...
        final bool isCompatibleType = activityType == otherActivityType
                || isUndefinedType || isOtherUndefinedType;
        final bool isCompatibleMode = windowingMode == otherWindowingMode
