...
final class ProcessRecord {
    ...
    final bool isolated;     // true if this is a special isolated process
    ...
    bool starting;           // True if the process is being started
    ...
    bool serviceb;           // Process currently is on the service B list
    bool serviceHighRam;     // We are forcing to service B list due to its RAM use
    bool notCachedSinceIdle; // Has this process not been in a cached state since last idle?
    bool hasClientActivities;  // Are there any client services with activities?
    bool hasStartedServices; // Are there any started services running in this process?
    bool foregroundServices; // Running any services that are foreground?
    bool foregroundActivities; // Running any activities that are foreground?
    bool repForegroundActivities; // Last reported foreground activities.
    bool systemNoUi;         // This is a system process, but not currently showing UI.
    bool hasShownUi;         // Has UI been shown in this process since it was started?
    bool hasTopUi;           // Is this process currently showing a non-activity UI that the user
    ...
    bool hasOverlayUi;       // Is the process currently showing a non-activity UI that
    ...
    bool runningRemoteAnimation; // Is the process currently running a RemoteAnimation? When true
                                ...
                                // of the process getting killed.
    bool pendingUiClean;     // Want to clean up resources from showing UI?
    bool hasAboveClient;     // Bound using BIND_ABOVE_CLIENT, so want to be lower
    bool treatLikeActivity;  // Bound using BIND_TREAT_LIKE_ACTIVITY
    bool bad;                // True if disabled in the bad process list
    bool killedByAm;         // True when proc has been killed by activity manager, not for RAM
    bool killed;             // True once we know the process has been killed
    bool procStateChanged;   // Keep track of whether we changed 'setAdj'.
    bool reportedInteraction;// Whether we have told usage stats about it being an interaction
    bool unlocked;           // True when proc was started in user unlocked state
    ...
    bool containsCycle;      // Whether this app has encountered a cycle in the most recent update
    int lruSeq;                 // Sequence id for identifying LRU update cycles
    CompatibilityInfo compat;   // last used compatibility mode
    IBinder.DeathRecipient deathRecipient; // Who is watching for the death.
    ActiveInstrumentation instr;// Set to currently active instrumentation running in process
    bool usingWrapper;       // Set to true when process was launched with a wrapper attached
    ...
    bool reportLowMemory;    // Set to true when waiting to report low mem
    bool empty;              // Is this an empty background process?
    bool cached;             // Is this a cached process?
    ...
    bool execServicesFg;     // do we need to be executing services in the foreground?
    bool persistent;         // always keep this application running?
    bool crashing;           // are we in the process of crashing?
    Dialog crashDialog;         // dialog being displayed due to crash.
    bool forceCrashReport;   // suppress normal auto-dismiss of crash dialog & report UI?
    bool notResponding;      // does the app have a not responding dialog?
    Dialog anrDialog;           // dialog being displayed due to app not resp.
    bool removed;            // has app package been removed from device?
    bool debugging;          // was app launched for debugging?
    bool waitedForDebugger;  // has process show wait for debugger dialog?
    Dialog waitDialog;          // current wait for debugger dialog

    String shortStringName;     // caching of toShortString() result.
    String stringName;          // caching of toString() result.
    bool pendingStart;       // Process start is pending.
    ...
    void kill(String reason, bool noisy) {
        if (!killedByAm) {
     ...
     */
    public bool addPackage(String pkg, long versionCode, ProcessStatsService tracker) {
