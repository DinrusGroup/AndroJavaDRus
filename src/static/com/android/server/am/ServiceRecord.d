 ...
 */
final class ServiceRecord : Binder : ComponentName.WithComponentName {
    ...
    final bool exported; // from ServiceInfo.exported
    ...
    bool whitelistManager; // any bindings to this service have BIND_ALLOW_WHITELIST_MANAGEMENT?
    bool delayed;        // are we waiting to start this service in the background?
    bool fgRequired;     // is the service required to go foreground after starting?
    bool fgWaiting;      // is a timeout for going foreground already scheduled?
    bool isForeground;   // is service currently in foreground mode?
    int foregroundId;       // Notification ID of last foreground req.
    Notification foregroundNoti; // Notification record of foreground state.
    long lastActivity;      // last time there was some activity on the service.
    long startingBgTimeout;  // time at which we scheduled this for a delayed start.
    bool startRequested; // someone explicitly called start?
    bool delayedStop;    // service has been stopped but is in a delayed start?
    bool stopIfKilled;   // last onStart() said to stop if service killed?
    bool callStart;      // last onStart() has asked to always be called on restart.
    int executeNesting;     // number of outstanding operations keeping foreground.
    bool executeFg;      // should we be executing in the foreground?
    long executingStart;    // start time of last execute request.
    bool createdFromFg;  // was this service last created due to a foreground process call?
    ...
    bool destroying;     // set when we have started destroying the service
    ...
    static class StartItem {
        ...
        StartItem(ServiceRecord _sr, bool _taskRemoved, int _id, Intent _intent,
    ...
    ServiceRecord(ActivityManagerService ams,
            ...
            Intent.FilterComparison intent, ServiceInfo sInfo, bool callerIsFg,
            Runnable restarter) {
        this.ams = ams;
    ...
    public bool hasAutoCreateConnections() {
    ...
    public StartItem findDeliveredStart(int id, bool taskRemoved, bool remove) {
        final int N = deliveredStarts.size();
        for (int i=0; i<N; i++) {
