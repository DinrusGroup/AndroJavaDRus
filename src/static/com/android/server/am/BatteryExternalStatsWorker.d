 ...
 */
class BatteryExternalStatsWorker : BatteryStatsImpl.ExternalStatsSync {
    ...
    @GuardedBy("this")
    private Future!(T) mCurrentFuture = null;

    @GuardedBy("this")
    private String mCurrentReason = null;

    @GuardedBy("this")
    private bool mOnBattery;

    @GuardedBy("this")
    private bool mOnBatteryScreenOff;

    @GuardedBy("this")
    private bool mUseLatestStates = true;

    @GuardedBy("this")
    private final IntArray mUidsToRemove = new IntArray();

    @GuardedBy("this")
    private Future!(T) mWakelockChangesUpdate;

    @GuardedBy("this")
    private Future!(T) mBatteryLevelSync;
    ...
    override
    public synchronized Future!(T) scheduleSync(String reason, int flags) {
        return scheduleSyncLocked(reason, flags);
    }

    override
    public synchronized Future!(T) scheduleCpuSyncDueToRemovedUid(int uid) {
        mUidsToRemove.add(uid);
        return scheduleSyncLocked("remove-uid", UPDATE_CPU);
    }

    override
    public synchronized Future!(T) scheduleCpuSyncDueToSettingChange() {
        return scheduleSyncLocked("setting-change", UPDATE_CPU);
    }

    override
    public Future!(T) scheduleReadProcStateCpuTimes(
            bool onBattery, bool onBatteryScreenOff, long delayMillis) {
        synchronized (mStats) {
    ...
    override
    public Future!(T) scheduleCopyFromAllUidsCpuTimes(
    ...
    override
    public Future!(T) scheduleCpuSyncDueToScreenStateChange(
            bool onBattery, bool onBatteryScreenOff) {
        synchronized (BatteryExternalStatsWorker.this) {
    ...
    override
    public Future!(T) scheduleCpuSyncDueToWakelockChange(long delayMillis) {
        synchronized (BatteryExternalStatsWorker.this) {
    ...
    override
    public Future!(T) scheduleSyncDueToBatteryLevelChange(long delayMillis) {
        synchronized (BatteryExternalStatsWorker.this) {
     ...
     */
    @GuardedBy("this")
    private Future!(T) scheduleDelayedSyncLocked(Future!(T) lastScheduledSync, Runnable syncRunnable,
            long delayMillis) {
        if (mExecutorService.isShutdown()) {
    ...
    public synchronized Future!(T) scheduleWrite() {
        if (mExecutorService.isShutdown()) {
            return CompletableFuture.failedFuture(new IllegalStateException("worker shutdown"));
        }
    ...
    @GuardedBy("this")
    private Future!(T) scheduleSyncLocked(String reason, int flags) {
        if (mExecutorService.isShutdown()) {
            return CompletableFuture.failedFuture(new IllegalStateException("worker shutdown"));
        }
    ...
    private final Runnable mSyncTask = new Runnable() {
        ...
        public void run() {
            ...
            final bool onBatteryScreenOff;
            final bool useLatestStates;
            synchronized (BatteryExternalStatsWorker.this) {
    ...
    private void updateExternalStatsLocked(final String reason, int updateFlags,
            bool onBattery, bool onBatteryScreenOff, bool useLatestStates) {
     ...
     */
    private static <T : Parcelable> T awaitControllerInfo(
