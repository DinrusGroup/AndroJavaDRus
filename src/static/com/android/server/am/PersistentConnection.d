...
public abstract class PersistentConnection<T> {
    ...
    // TODO too many bools... Should clean up.
     ...
     */
    @GuardedBy("mLock")
    private bool mShouldBeBound;

    @GuardedBy("mLock")
    private bool mRebindScheduled;
     ...
     */
    public final bool isRebindScheduled() {
        synchronized (mLock) {
    ...
    @GuardedBy("mLock")
    public final void bindInnerLocked(bool resetBackoff) {
        ...
        final bool success = mContext.bindServiceAsUser(service, mServiceConnection,
