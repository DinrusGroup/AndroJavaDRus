 ...
 * battery life.
 */
public final class BatteryStatsService : IBatteryStats.Stub
    ...
    private final class LocalService : BatteryStatsInternal {
        override
    ...
    private static void awaitUninterruptibly(Future!(T) future) {
        while (true) {
            try {
                future.get();
                return;
    ...
    public void noteInteractive(bool interactive) {
        enforceCallingPermission();
        synchronized (mStats) {
    ...
    public void notePhoneDataConnectionState(int dataType, bool hasData) {
        enforceCallingPermission();
        synchronized (mStats) {
    ...
    public void noteWifiSupplicantStateChanged(int supplState, bool failedAuth) {
        enforceCallingPermission();
        synchronized (mStats) {
    ...
    override
    public void noteBleScanStarted(WorkSource ws, bool isUnoptimized) {
        enforceCallingPermission();
        synchronized (mStats) {
    ...
    override
    public void noteBleScanStopped(WorkSource ws, bool isUnoptimized) {
        enforceCallingPermission();
        synchronized (mStats) {
    ...
    public void setBatteryState(final int status, final int health, final int plugType,
        ...
        mWorker.scheduleRunnable(() -> {
            synchronized (mStats) {
                final bool onBattery = BatteryStatsImpl.isOnBattery(plugType, status);
    ...
    final class WakeupReasonThread : Thread {
    ...
    private int doEnableOrDisable(PrintWriter pw, int i, String[] args, bool enable) {
        i++;
        if (i >= args.length) {
    ...
    protected void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        int flags = 0;
        bool useCheckinFormat = false;
        bool toProto = false;
        bool isRealCheckin = false;
        bool noOutput = false;
    ...
    private bool shouldCollectExternalStats() {
     ...
     */
    private static bool onlyCaller(int[] requestUids) {
