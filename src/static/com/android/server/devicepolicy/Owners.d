...
class Owners {
    ...
    private static final bool DEBUG = false; // DO NOT SUBMIT WITH TRUE
     ...
     */
    bool setSystemUpdateFreezePeriodRecord(LocalDate start, LocalDate end) {
        bool changed = false;
        synchronized (mLock) {
    ...
    bool isDeviceOwnerUserId(int userId) {
        synchronized (mLock) {
            return mDeviceOwner != null && mDeviceOwnerUserId == userId;
        }
    }

    bool hasProfileOwner(int userId) {
        synchronized (mLock) {
     ...
     */
    bool getDeviceOwnerUserRestrictionsNeedsMigration() {
        synchronized (mLock) {
     ...
     */
    bool getProfileOwnerUserRestrictionsNeedsMigration(int userId) {
        synchronized (mLock) {
            OwnerInfo profileOwner = mProfileOwners.get(userId);
    ...
    private bool readLegacyOwnerFileLocked(File file) {
        if (!file.exists()) {
     ...
     */
    bool saveSystemUpdateInfo(@Nullable SystemUpdateInfo newInfo) {
        synchronized (mLock) {
    ...
    private abstract static class FileReadWriter {
        ...
        abstract bool readInner(XmlPullParser parser, int depth, String tag);
    }

    private class DeviceOwnerReadWriter : FileReadWriter {
        ...
        override
        bool readInner(XmlPullParser parser, int depth, String tag) {
            if (depth > 2) {
                return true; // Ignore
            }
            switch (tag) {
    ...
    private class ProfileOwnerReadWriter : FileReadWriter {
    ...
    static class OwnerInfo {
        ...
        public bool userRestrictionsMigrated;
        public String remoteBugreportUri;
        public String remoteBugreportHash;

        public OwnerInfo(String name, String packageName, bool userRestrictionsMigrated,
        ...
        public OwnerInfo(String name, ComponentName admin, bool userRestrictionsMigrated,
                String remoteBugreportUri, String remoteBugreportHash) {
            this.name = name;
            this.admin = admin;
        ...
        public static OwnerInfo readFromXml(XmlPullParser parser) {
            ...
            final bool userRestrictionsMigrated =
