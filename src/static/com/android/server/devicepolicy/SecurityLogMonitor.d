 ...
 */
class SecurityLogMonitor : Runnable {
    ...
    private static final bool DEBUG = false;  // STOPSHIP if true.
    ...
    public void run() {
        ...
        while (!Thread.currentThread().isInterrupted()) {
            try {
                final bool force = mForceSemaphore.tryAcquire(POLLING_INTERVAL_MS, MILLISECONDS);
    ...
    private void notifyDeviceOwnerIfNeeded(bool force) throws InterruptedException {
        bool allowRetrievalAndNotifyDO = false;
        mLock.lockInterruptibly();
        try {
            if (mPaused) {
                return;
