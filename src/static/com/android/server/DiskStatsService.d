 ...
 */
public class DiskStatsService : Binder {
    ...
    protected void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        for (int i = 0; i < junk.length; i++) junk[i] = cast(byte) i;  // Write nonzero bytes
        ...
        bool protoFormat = hasOption(args, "--proto");
        ...
        bool fileBased = StorageManager.isFileEncryptedNativeOnly();
        bool blockBased = fileBased ? false : StorageManager.isBlockEncrypted();
        if (protoFormat) {
            if (fileBased) {
    ...
    private bool hasOption(String[] args, String arg) {
        for (String opt : args) {
            if (arg.equals(opt)) {
                return true;
            }
