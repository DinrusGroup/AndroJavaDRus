 ...
 * @hide
 */
public class LockSettingsService : ILockSettings.Stub {
        ...
        public LockSettingsStorage getStorage() {
            ...
            storage.setDatabaseOnCreateCallback(new LockSettingsStorage.Callback() {
                ...
                public void initialize(SQLiteDatabase db) {
                    ...
                    bool lockScreenDisable = SystemProperties.getBoolean(
    ...
    private void maybeShowEncryptionNotificationForUser(@UserIdInt int userId) {
        ...
        final bool isSecure = isUserSecure(userId);
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < profiles.size(); i++) {
                    ...
                    final bool isSecure = isUserSecure(profile.id);
    ...
    private void migrateOldData() {
        ...
        bool isWatch = mContext.getPackageManager().hasSystemFeature(
    ...
    override
    public bool getSeparateProfileChallengeEnabled(int userId) {
    ...
    override
    public void setSeparateProfileChallengeEnabled(int userId, bool enabled,
    ...
    private void setSeparateProfileChallengeEnabledLocked(@UserIdInt int userId, bool enabled,
            String managedUserPassword) {
    ...
    override
    public void setBoolean(String key, bool value, int userId) {
        checkWritePermission(userId);
    ...
    override
    public bool getBoolean(String key, bool defaultValue, int userId) {
    ...
    override
    public bool havePassword(int userId) throws RemoteException {
    ...
    override
    public bool havePattern(int userId) throws RemoteException {
        checkPasswordHavePermission(userId);
        synchronized (mSpManager) {
            if (isSyntheticPasswordBasedCredentialLocked(userId)) {
                long handle = getSyntheticPasswordHandleLocked(userId);
    ...
    private bool isUserSecure(int userId) {
        synchronized (mSpManager) {
            if (isSyntheticPasswordBasedCredentialLocked(userId)) {
                long handle = getSyntheticPasswordHandleLocked(userId);
                return mSpManager.getCredentialType(handle, userId) !=
    ...
    private void unlockChildProfile(int profileHandle, bool ignoreUserNotAuthenticated)
    ...
    private bool tiedManagedProfileReadyToUnlock(UserInfo userInfo) {
    ...
    private bool isManagedProfileWithUnifiedLock(int userId) {
        return mUserManager.getUserInfo(userId).isManagedProfile()
                && !mLockPatternUtils.isSeparateProfileChallengeEnabled(userId);
    }

    private bool isManagedProfileWithSeparatedLock(int userId) {
        return mUserManager.getUserInfo(userId).isManagedProfile()
                && mLockPatternUtils.isSeparateProfileChallengeEnabled(userId);
    }
    ...
    private VerifyCredentialResponse doVerifyCredential(String credential, int credentialType,
            bool hasChallenge, long challenge, int userId,
            ICheckCredentialProgressCallback progressCallback) throws RemoteException {
        if (TextUtils.isEmpty(credential)) {
        ...
        bool shouldReEnrollBaseZero = storedHash.type == LockPatternUtils.CREDENTIAL_TYPE_PATTERN
    ...
    private VerifyCredentialResponse verifyCredential(int userId, CredentialHash storedHash,
            String credential, bool hasChallenge, long challenge,
            ICheckCredentialProgressCallback progressCallback) throws RemoteException {
        ...
        bool shouldReEnroll = gateKeeperResponse.getShouldReEnroll();
    ...
    override
    public bool checkVoldPassword(int userId) throws RemoteException {
        if (!mFirstCallToVold) {
            return false;
        }
    ...
    private void removeUser(int userId, bool unknownUser) {
    ...
    private class GateKeeperDiedRecipient : IBinder.DeathRecipient {
        override
    ...
    private bool shouldCacheSpForUser(@UserIdInt int userId) {
    ...
    private bool isSyntheticPasswordBasedCredentialLocked(int userId) {
        if (userId == USER_FRP) {
    ...
    @VisibleForTesting
    protected bool shouldMigrateToSyntheticPasswordLocked(int userId) {
        long handle = getSyntheticPasswordHandleLocked(userId);
        // This is a global setting
        long enabled = getLong(SYNTHETIC_PASSWORD_ENABLED_KEY,
                SYNTHETIC_PASSWORD_ENABLED_BY_DEFAULT, UserHandle.USER_SYSTEM);
    ...
    private VerifyCredentialResponse spBasedDoVerifyCredential(String userCredential, int
            credentialType, bool hasChallenge, long challenge, int userId,
            ICheckCredentialProgressCallback progressCallback) throws RemoteException {
    ...
    private bool isEscrowTokenActive(long handle, int userId) {
        synchronized (mSpManager) {
            return mSpManager.existsHandle(handle, userId);
        }
    }

    private bool removeEscrowToken(long handle, int userId) {
        synchronized (mSpManager) {
    ...
    private bool setLockCredentialWithToken(String credential, int type, long tokenHandle,
    ...
    private bool setLockCredentialWithTokenInternal(String credential, int type,
    ...
    private bool unlockUserWithToken(long tokenHandle, byte[] token, int userId)
            throws RemoteException {
    ...
    private class DeviceProvisionedObserver : ContentObserver {
        ...
        override
        public void onChange(bool selfChange, Uri uri, @UserIdInt int userId) {
    ...
    private final class LocalService : LockSettingsInternal {
        ...
        override
        public bool setLockCredentialWithToken(String credential, int type, long tokenHandle,
