...
class LockSettingsShellCommand : ShellCommand {
    ...
    private void runSetDisabled() throws RemoteException {
        final bool disabled = Boolean.parseBoolean(mNew);
    ...
    private void runGetDisabled() {
        bool isLockScreenDisabled = mLockPatternUtils.isLockScreenDisabled(mCurrentUserId);
        getOutPrintWriter().println(isLockScreenDisabled);
    }

    private bool checkCredential() throws RemoteException {
        final bool havePassword = mLockPatternUtils.isLockPasswordEnabled(mCurrentUserId);
        final bool havePattern = mLockPatternUtils.isLockPatternEnabled(mCurrentUserId);
