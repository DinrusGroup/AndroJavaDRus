...
public class WifiConnectivityManager {
    ...
    private static final bool SCAN_IMMEDIATELY = true;
    private static final bool SCAN_ON_SCHEDULE = false;
    ...
    private bool mDbg = false;
    private bool mWifiEnabled = false;
    private bool mWifiConnectivityManagerEnabled = true;
    private bool mScreenOn = false;
    private int mWifiState = WIFI_STATE_UNKNOWN;
    private bool mUntrustedConnectionAllowed = false;
    ...
    private bool mPnoScanStarted = false;
    private bool mPeriodicScanTimerSet = false;
    // Device configs
    private bool mEnableAutoJoinWhenAssociated;
    private bool mWaitForFullBandScanResults = false;
    private bool mUseSingleRadioChainScanResults = false;
    ...
    // if the start scan command failed. An timer is used here to make it a deferred retry.
    private class RestartSingleScanListener : AlarmManager.OnAlarmListener {
        private final bool mIsFullBandScan;

        RestartSingleScanListener(bool isFullBandScan) {
     ...
     */
    private bool handleScanResults(List<ScanDetail> scanDetails, String listenerName) {
    ...
    //       other modules.
    private class AllSingleScanListener : WifiScanner.ScanListener {
        ...
        public void onResults(WifiScanner.ScanData[] results) {
            ...
            bool wasConnectAttempted = handleScanResults(mScanDetails, "AllSingleScanListener");
            clearScanDetails();
    ...
    private class SingleScanListener : WifiScanner.ScanListener {
        private final bool mIsFullBandScan;

        SingleScanListener(bool isFullBandScan) {
            mIsFullBandScan = isFullBandScan;
        }
    ...
    private class PnoScanListener : WifiScanner.PnoScanListener {
        private List<ScanDetail> mScanDetails = new ArrayList<ScanDetail>();
    ...
    private class OnSavedNetworkUpdateListener :
    ...
    WifiConnectivityManager(Context context, ScoringParams scoringParams,
            ...
            Clock clock, LocalLog localLog, bool enable, FrameworkFacade frameworkFacade,
        ...
        bool hs2Enabled = context.getPackageManager().hasSystemFeature(
     ...
     */
    private bool shouldSkipConnectionAttempt(Long timeMillis) {
    ...
    private int getScanBand(bool isFullBandScan) {
        if (isFullBandScan) {
    ...
    private bool setScanChannels(ScanSettings settings) {
    ...
    private void startPeriodicSingleScan() {
        ...
        bool isFullBandScan = true;
        bool isTrafficOverThreshold = mWifiInfo.txSuccessRate > mFullScanMaxTxRate
    ...
    // Start a single scan
    private void startSingleScan(bool isFullBandScan, WorkSource workSource) {
        if (!mWifiEnabled || !mWifiConnectivityManagerEnabled) {
            return;
        }
    ...
    private void startPeriodicScan(bool scanImmediately) {
        mPnoScanListener.resetLowRssiNetworkRetryDelay();
    ...
    private void scheduleDelayedSingleScan(bool isFullBandScan) {
    ...
    private void startConnectivityScan(bool scanImmediately) {
     ...
     */
    public void handleScreenStateChanged(bool screenOn) {
     ...
     */
    public void setUntrustedConnectionAllowed(bool allowed) {
     ...
     */
    private bool updateBssidBlacklist(String bssid, bool enable, int reasonCode) {
     ...
     * @return true if blacklist is updated; false otherwise
     */
    public bool trackBssid(String bssid, bool enable, int reasonCode) {
     ...
     */
    @VisibleForTesting
    public bool isBssidDisabled(String bssid) {
        BssidBlacklistStatus status = mBssidBlacklist.get(bssid);
