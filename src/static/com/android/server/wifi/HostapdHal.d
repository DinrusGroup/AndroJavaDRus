...
public class HostapdHal {
    ...
    private final bool mEnableAcs;
    private final bool mEnableIeee80211AC;
     ...
     * @return true on success, false otherwise.
     */
    private bool linkToHostapdDeath() {
        synchronized (mLock) {
     ...
     * @return true on success, false otherwise.
     */
    private bool initHostapdService() {
        synchronized (mLock) {
            try {
     ...
     * @return true on success, false otherwise.
     */
    public bool addAccessPoint(@NonNull String ifaceName, @NonNull WifiConfiguration config) {
        synchronized (mLock) {
     ...
     * @return true on success, false otherwise.
     */
    public bool removeAccessPoint(@NonNull String ifaceName) {
        synchronized (mLock) {
     ...
     */
    public bool registerDeathHandler(@NonNull HostapdDeathEventHandler handler) {
     ...
     */
    private bool checkHostapdAndLogFailure(String methodStr) {
        synchronized (mLock) {
            if (mIHostapd == null) {
     ...
     * otherwise
     */
    private bool checkStatusAndLogFailure(HostapdStatus status,
            String methodStr) {
        synchronized (mLock) {
