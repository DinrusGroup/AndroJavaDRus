...
public class WifiMetrics {
    ...
    private bool mIsWifiNetworksAvailableNotificationOn = false;
    ...
    private bool mIsMacRandomizationOn = false;
    ...
    public void endConnectionEvent(int level2FailureCode, int connectivityFailureCode) {
        synchronized (mLock) {
            if (mCurrentConnectionEvent != null) {
                bool result = (level2FailureCode == 1)
    ...
    void setIsLocationEnabled(bool enabled) {
        synchronized (mLock) {
            mWifiLogProto.isLocationEnabled = enabled;
        }
    }

    void setIsScanningAlwaysEnabled(bool enabled) {
        synchronized (mLock) {
   ...
   /**
     ...
     */
    public void incrementWifiSystemScanStateCount(int state, bool screenOn) {
        synchronized (mLock) {
            if (DBG) {
     ...
     */
    public int getSystemStateCount(int state, bool screenOn) {
        synchronized (mLock) {
            int index = state * 2 + (screenOn ? SCREEN_ON : SCREEN_OFF);
    ...
    private bool mWifiWins = false; // Based on scores, use wifi instead of mobile data?
     ...
     * codes.
     */
    public void incrementSoftApStartResult(bool result, int failureCode) {
        synchronized (mLock) {
            if (result) {
     ...
     */
    public void addSoftApUpChangedEvent(bool isUp, int mode) {
    ...
    public void incrementAvailableNetworksHistograms(List<ScanDetail> scanDetails,
        ...
        synchronized (mLock) {
            ...
            for (ScanDetail scanDetail : scanDetails) {
                ...
                ssids.add(matchInfo);
                bssids++;
                bool isOpen = matchInfo.networkType == ScanResultMatchInfo.NETWORK_TYPE_OPEN;
                WifiConfiguration config =
                        mWifiConfigManager.getConfiguredNetworkForScanDetail(scanDetail);
                bool isSaved = (config != null) && !config.isEphemeral()
                        && !config.isPasspoint();
                bool isSavedPasspoint = passpointProvider != null;
                if (isOpen) {
    ...
    public void setIsWifiNetworksAvailableNotificationEnabled(String notifierTag, bool enabled) {
        synchronized (mLock) {
    ...
    public void setIsMacRandomizationOn(bool enabled) {
        synchronized (mLock) {
     ...
     */
    private void consolidateProto(bool incremental) {
     ...
     */
    public void setScreenState(bool screenOn) {
        synchronized (mLock) {
            mScreenOn = screenOn;
        }
    }
