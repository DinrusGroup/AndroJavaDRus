...
public abstract class ChannelHelper {
     ...
     */
    public abstract bool settingsContainChannel(WifiScanner.ScanSettings settings, int channel);
    ...
    public abstract class ChannelCollection {
         ...
         */
        public abstract bool isAllChannels();
        /**
         ...
         */
        public bool containsSettings(WifiScanner.ScanSettings scanSettings) {
            if (scanSettings.band == WifiScanner.WIFI_BAND_UNSPECIFIED) {
                for (int j = 0; j < scanSettings.channels.length; ++j) {
         ...
         */
        public bool partiallyContainsSettings(WifiScanner.ScanSettings scanSettings) {
            if (scanSettings.band == WifiScanner.WIFI_BAND_UNSPECIFIED) {
                for (int j = 0; j < scanSettings.channels.length; ++j) {
                    if (containsChannel(scanSettings.channels[j].frequency)) {
                        return true;
