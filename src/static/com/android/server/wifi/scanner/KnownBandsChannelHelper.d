 ...
 */
public class KnownBandsChannelHelper : ChannelHelper {
    ...
    private bool isDfsChannel(int frequency) {
    ...
    override
    public bool settingsContainChannel(WifiScanner.ScanSettings settings, int channel) {
     ...
     */
    public class KnownBandsChannelCollection : ChannelCollection {
        /**
        ...
        override
        public bool containsBand(int band) {
            WifiScanner.ChannelSpec[] bandChannels = getAvailableScanChannels(band);
            for (int i = 0; i < bandChannels.length; ++i) {
        ...
        override
        public bool partiallyContainsBand(int band) {
            WifiScanner.ChannelSpec[] bandChannels = getAvailableScanChannels(band);
            for (int i = 0; i < bandChannels.length; ++i) {
                if (mChannels.contains(bandChannels[i].frequency)) {
                    return true;
