...
public abstract class Utils {
    ...
    public static byte[] hexToBytes(String text) {
        ...
        for (int n = 0; n < text.length(); n += 2) {
            data[position] =
                    cast(byte) (((fromHex(text.charAt(n), false) & NIBBLE_MASK) << 4) |
    ...
    public static int fromHex(char ch, bool lenient) throws NumberFormatException {
        if (ch <= '9' && ch >= '0') {
            return ch - '0';
        } else if (ch >= 'a' && ch <= 'f') {
            return ch + 10 - 'a';
    ...
    public static <T : Comparable> int compare(Comparable<T> c1, T c2) {
        if (c1 == null) {
            return c2 == null ? 0 : -1;
        }
        else if (c2 == null) {
