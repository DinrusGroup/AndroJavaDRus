...
public class OsuServerConnection {
     ...
     * @return bool true if capability available
     */
    public bool canValidateServer() {
     ...
     * @return bool value, true if connection was successful
     *
     * Relies on the caller to ensure that the capability to validate the OSU
     * Server is available.
     */
    public bool connect(URL url, Network network) {
        mNetwork = network;
        mUrl = url;
     ...
     */
    public bool validateProvider(String friendlyName) {
    ...
    private class WFATrustManager : X509TrustManager {
        ...
        public X509Certificate getProviderCert() {
            ...
            try {
                for (X509Certificate certificate : mServerCerts) {
                    Collection<List!(T)> col = certificate.getSubjectAlternativeNames();
                    if (col == null) {
                        continue;
                    }
                    for (List!(T) name : col) {
