...
public class WifiAwareDiscoverySessionState {
    ...
    private bool mIsPublishSession;
    private bool mIsRangingEnabled;
    ...
    public WifiAwareDiscoverySessionState(WifiAwareNativeApi wifiAwareNativeApi, int sessionId,
            byte pubSubId, IWifiAwareDiscoverySessionCallback callback, bool isPublishSession,
            bool isRangingEnabled, long creationTime) {
    ...
    public bool isPublishSession() {
        return mIsPublishSession;
    }

    public bool isRangingEnabled() {
    ...
    public void terminate() {
        ...
        if (mIsPublishSession) {
            mWifiAwareNativeApi.stopPublish(cast(short) 0, mPubSubId);
        } else {
            mWifiAwareNativeApi.stopSubscribe(cast(short) 0, mPubSubId);
        }
    }
     ...
     */
    public bool isPubSubIdSession(int pubSubId) {
     ...
     */
    public bool updatePublish(short transactionId, PublishConfig config) {
        if (!mIsPublishSession) {
        ...
        bool success = mWifiAwareNativeApi.publish(transactionId, mPubSubId, config);
        if (!success) {
            try {
                mCallback.onSessionConfigFail(NanStatusType.INTERNAL_FAILURE);
            } catch (RemoteException e) {
     ...
     */
    public bool updateSubscribe(short transactionId, SubscribeConfig config) {
        if (mIsPublishSession) {
        ...
        bool success = mWifiAwareNativeApi.subscribe(transactionId, mPubSubId, config);
        if (!success) {
            try {
                mCallback.onSessionConfigFail(NanStatusType.INTERNAL_FAILURE);
            } catch (RemoteException e) {
     ...
     */
    public bool sendMessage(short transactionId, int peerId, byte[] message, int messageId) {
        ...
        bool success = mWifiAwareNativeApi.sendMessage(transactionId, mPubSubId,
