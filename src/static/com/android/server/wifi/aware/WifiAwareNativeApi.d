 ...
 */
public class WifiAwareNativeApi : WifiAwareShellCommand.DelegatedShellCommand {
    ...
    private static final int PARAM_ENABLE_DW_EARLY_TERM_DEFAULT = 0; // bool: 0 = false
    private static final int PARAM_ENABLE_DW_EARLY_TERM_INACTIVE = 0; // bool: 0 = false
    private static final int PARAM_ENABLE_DW_EARLY_TERM_IDLE = 0; // bool: 0 = false
     ...
     */
    public bool getCapabilities(short transactionId) {
     ...
     */
    public bool enableAndConfigure(short transactionId, ConfigRequest configRequest,
            bool notifyIdentityChange, bool initialConfiguration, bool isInteractive,
            bool isIdle) {
        if (mDbg) {
        ...
        try {
            ...
            if (initialConfiguration) {
                ...
                req.hopCountMax = 2;
                req.configParams.masterPref = cast(byte) configRequest.mMasterPreference;
                ...
                config24.dwellTimeMs = cast(byte) 200;
                ...
                if (configRequest.mDiscoveryWindowInterval[ConfigRequest.NAN_BAND_24GHZ]
                    ...
                    config24.discoveryWindowIntervalVal =
                            cast(byte) configRequest.mDiscoveryWindowInterval[ConfigRequest
                                    .NAN_BAND_24GHZ];
                }
                ...
                config5.dwellTimeMs = cast(byte) 200;
                ...
                req.debugConfigs.clusterIdTopRangeVal = cast(short) configRequest.mClusterHigh;
                req.debugConfigs.clusterIdBottomRangeVal = cast(short) configRequest.mClusterLow;
                ...
                req.masterPref = cast(byte) configRequest.mMasterPreference;
     ...
     */
    public bool disable(short transactionId) {
     ...
     */
    public bool publish(short transactionId, byte publishId, PublishConfig publishConfig) {
        if (mDbg) {
        ...
        req.baseConfigs.ttlSec = cast(short) publishConfig.mTtlSec;
     ...
     */
    public bool subscribe(short transactionId, byte subscribeId,
        ...
        req.baseConfigs.ttlSec = cast(short) subscribeConfig.mTtlSec;
        req.baseConfigs.discoveryWindowPeriod = 1;
        req.baseConfigs.discoveryCount = 0;
        ...
        if (subscribeConfig.mMinDistanceMmSet) {
            req.baseConfigs.distanceEgressCm = cast(short) Math.min(
        ...
        if (subscribeConfig.mMaxDistanceMmSet) {
            req.baseConfigs.distanceIngressCm = cast(short) Math.min(
     ...
     */
    public bool sendMessage(short transactionId, byte pubSubId, int requestorInstanceId,
     ...
     *            creating a session.
     */
    public bool stopPublish(short transactionId, byte pubSubId) {
        if (mDbg) {
     ...
     *            creating a session.
     */
    public bool stopSubscribe(short transactionId, byte pubSubId) {
        if (mDbg) {
     ...
     */
    public bool createAwareNetworkInterface(short transactionId, String interfaceName) {
        if (mDbg) {
     ...
     * @param interfaceName The name of the interface, e.g. "aware0".
     */
    public bool deleteAwareNetworkInterface(short transactionId, String interfaceName) {
        if (mDbg) {
     ...
     */
    public bool initiateDataPath(short transactionId, int peerId, int channelRequestType,
            int channel, byte[] peer, String interfaceName, byte[] pmk, String passphrase,
            bool isOutOfBand, Capabilities capabilities) {
        if (mDbg) {
     ...
     * @param capabilities The capabilities of the firmware.
     */
    public bool respondToDataPathRequest(short transactionId, bool accept, int ndpId,
            String interfaceName, byte[] pmk, String passphrase, bool isOutOfBand,
     ...
     */
    public bool endDataPath(short transactionId, int ndpId) {
        if (mDbg) {
    ...
    private void updateConfigForPowerSettings(NanConfigRequest req,
            NanConfigRequestSupplemental configSupplemental12, bool isInteractive,
            bool isIdle) {
    ...
    private void updateSingleConfigForPowerSettings(NanBandSpecificConfig cfg, int override) {
        if (override != -1) {
            ...
            cfg.discoveryWindowIntervalVal = cast(byte) override;
        }
    }
