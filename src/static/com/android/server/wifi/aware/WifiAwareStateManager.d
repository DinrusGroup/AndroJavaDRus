 ...
 */
public class WifiAwareStateManager : WifiAwareShellCommand.DelegatedShellCommand {
    private static final String TAG = "WifiAwareStateManager";
    private static final bool VDBG = false; // STOPSHIP if true
    private static final bool VVDBG = false; // STOPSHIP if true - for detailed state machine
    /* package */ bool mDbg = false;
     ...
     */
    private volatile bool mUsageEnabled = false;
    ...
    private bool mCurrentIdentityNotification = false;
    ...
    public void start(Context context, Looper looper, WifiAwareMetrics awareMetrics,
        ...
        mContext.registerReceiver(new BroadcastReceiver() {
            override
            public void onReceive(Context context, Intent intent) {
                bool isEnabled = intent.getIntExtra(WifiManager.EXTRA_WIFI_STATE,
     ...
     * now) or disabled.
     *
     * @return A bool indicating whether Aware usage is enabled (true) or
     *         disabled (false).
     */
    ...
    public void initiateDataPathSetup(WifiAwareNetworkSpecifier networkSpecifier, int peerId,
            ...
            String passphrase, bool isOutOfBand) {
        Message msg = mSm.obtainMessage(MESSAGE_TYPE_COMMAND);
     ...
     */
    public void respondToDataPathRequest(bool accept, int ndpId, String interfaceName,
            byte[] pmk, String passphrase, bool isOutOfBand) {
        Message msg = mSm.obtainMessage(MESSAGE_TYPE_COMMAND);
     ...
     */
    public void onSessionConfigSuccessResponse(short transactionId, bool isPublish,
            byte pubSubId) {
        Message msg = mSm.obtainMessage(MESSAGE_TYPE_RESPONSE);
     ...
     */
    public void onSessionConfigFailResponse(short transactionId, bool isPublish, int reason) {
        Message msg = mSm.obtainMessage(MESSAGE_TYPE_RESPONSE);
     ...
     * completed.
     */
    public void onCreateDataPathInterfaceResponse(short transactionId, bool success,
            int reasonOnFailure) {
        Message msg = mSm.obtainMessage(MESSAGE_TYPE_RESPONSE);
     ...
     * completed.
     */
    public void onDeleteDataPathInterfaceResponse(short transactionId, bool success,
            int reasonOnFailure) {
        Message msg = mSm.obtainMessage(MESSAGE_TYPE_RESPONSE);
     ...
     * {@link #respondToDataPathRequest(bool, int, String, byte[], String, bool)}
     */
    public void onRespondToDataPathSetupRequestResponse(short transactionId, bool success,
            int reasonOnFailure) {
        Message msg = mSm.obtainMessage(MESSAGE_TYPE_RESPONSE);
     ...
     */
    public void onEndDataPathResponse(short transactionId, bool success, int reasonOnFailure) {
        Message msg = mSm.obtainMessage(MESSAGE_TYPE_RESPONSE);
     ...
     */
    public void onSessionTerminatedNotification(int pubSubId, int reason, bool isPublish) {
        Message msg = mSm.obtainMessage(MESSAGE_TYPE_NOTIFICATION);
     ...
     * data-path is now up.
     */
    public void onDataPathConfirmNotification(int ndpId, byte[] mac, bool accept, int reason,
     ...
     */
    @VisibleForTesting
    class WifiAwareStateMachine : StateMachine {
        ...
        private bool mSendQueueBlocked = false;
        ...
        private class WaitForResponseState : State {
        ...
        private void processNotification(Message msg) {
            ...
            switch (msg.arg1) {
                ...
                case NOTIFICATION_TYPE_SESSION_TERMINATED: {
                    ...
                    bool isPublish = msg.getData().getBoolean(MESSAGE_BUNDLE_KEY_SESSION_TYPE);
                ...
                case NOTIFICATION_TYPE_ON_MESSAGE_SEND_SUCCESS: {
                    short transactionId = cast(short) msg.arg2;
         ...
         */
        private bool processCommand(Message msg) {
            if (VDBG) {
            ...
            switch (msg.arg1) {
                case COMMAND_TYPE_CONNECT: {
                    ...
                    bool notifyIdentityChange = msg.getData().getBoolean(
                ...
                case COMMAND_TYPE_INITIATE_DATA_PATH_SETUP: {
                    ...
                    bool isOutOfBand = data.getBoolean(MESSAGE_BUNDLE_KEY_OOB);
    ...
    private void sendAwareStateChangedBroadcast(bool enabled) {
        if (VDBG) {
    ...
    private bool connectLocal(short transactionId, int clientId, int uid, int pid,
            String callingPackage, IWifiAwareEventCallback callback, ConfigRequest configRequest,
            bool notifyIdentityChange) {
        if (VDBG) {
        ...
        bool success = mWifiAwareNativeApi.enableAndConfigure(transactionId, merged,
    ...
    private bool disconnectLocal(short transactionId, int clientId) {
        if (VDBG) {
            Log.v(TAG,
        ...
        bool notificationReqs = doesAnyClientNeedIdentityChangeNotifications();
    ...
    private bool reconfigureLocal(short transactionId) {
    ...
    private bool publishLocal(short transactionId, int clientId, PublishConfig publishConfig,
            IWifiAwareDiscoverySessionCallback callback) {
        if (VDBG) {
        ...
        bool success = mWifiAwareNativeApi.publish(transactionId, cast(byte) 0, publishConfig);
        if (!success) {
            try {
    ...
    private bool updatePublishLocal(short transactionId, int clientId, int sessionId,
        ...
        bool status = session.updatePublish(transactionId, publishConfig);
        if (!status) {
            mAwareMetrics.recordDiscoveryStatus(client.getUid(), NanStatusType.INTERNAL_FAILURE,
                    true);
        }
    ...
    private bool subscribeLocal(short transactionId, int clientId,
        ...
        bool success = mWifiAwareNativeApi.subscribe(transactionId, cast(byte) 0, subscribeConfig);
        if (!success) {
            try {
                callback.onSessionConfigFail(NanStatusType.INTERNAL_FAILURE);
            } catch (RemoteException e) {
    ...
    private bool updateSubscribeLocal(short transactionId, int clientId, int sessionId,
        ...
        bool status = session.updateSubscribe(transactionId, subscribeConfig);
        if (!status) {
            mAwareMetrics.recordDiscoveryStatus(client.getUid(), NanStatusType.INTERNAL_FAILURE,
                    false);
        }
    ...
    private bool sendFollowonMessageLocal(short transactionId, int clientId, int sessionId,
    ...
    private bool disableUsageLocal(short transactionId) {
        if (VDBG) {
        ...
        mUsageEnabled = false;
        bool callDispatched = mWifiAwareNativeApi.disable(transactionId);
    ...
    private bool initiateDataPathSetupLocal(short transactionId,
        ...
        bool success = mWifiAwareNativeApi.initiateDataPath(transactionId, peerId,
    ...
    private bool respondToDataPathRequestLocal(short transactionId, bool accept,
            int ndpId, String interfaceName, byte[] pmk, String passphrase, bool isOutOfBand) {
        if (VDBG) {
            Log.v(TAG,
        ...
        bool success = mWifiAwareNativeApi.respondToDataPathRequest(transactionId, accept, ndpId,
    ...
    private bool endDataPathLocal(short transactionId, int ndpId) {
        if (VDBG) {
            Log.v(TAG,
    ...
    private void onConfigCompletedLocal(Message completedCommand) {
        ...
        if (completedCommand.arg1 == COMMAND_TYPE_CONNECT) {
            ...
            bool notifyIdentityChange = data.getBoolean(
                    MESSAGE_BUNDLE_KEY_NOTIFY_IDENTITY_CHANGE);
    ...
    private void onSessionConfigFailLocal(Message failedCommand, bool isPublish, int reason) {
        if (VDBG) {
    ...
    private void onCreateDataPathInterfaceResponseLocal(Message command, bool success,
            int reasonOnFailure) {
        if (VDBG) {
    ...
    private void onDeleteDataPathInterfaceResponseLocal(Message command, bool success,
            int reasonOnFailure) {
        if (VDBG) {
    ...
    private void onRespondToDataPathSetupRequestResponseLocal(Message command, bool success,
            int reasonOnFailure) {
        if (VDBG) {
    ...
    private void onEndPathEndResponseLocal(Message command, bool success, int reasonOnFailure) {
        if (VDBG) {
    ...
    private void onSessionTerminatedLocal(int pubSubId, bool isPublish, int reason) {
        if (VDBG) {
    ...
    private ConfigRequest mergeConfigRequests(ConfigRequest configRequest) {
        ...
        bool support5gBand = false;
        int masterPreference = 0;
        bool clusterIdValid = false;
        int clusterLow = 0;
    ...
    private bool doesAnyClientNeedIdentityChangeNotifications() {
        for (int i = 0; i < mClients.size(); ++i) {
