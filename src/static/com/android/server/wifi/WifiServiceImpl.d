 ...
 * @hide
 */
public class WifiServiceImpl : IWifiManager.Stub {
    ...
    private final bool mPermissionReviewRequired;
    ...
    public final class LocalOnlyRequestorCallback
            : LocalOnlyHotspotRequestInfo.RequestingApplicationDeathCallback {
        /**
     ...
     */
    private class ClientHandler : WifiHandler {
         ...
         */
        private bool checkChangePermissionAndReplyIfNotAuthorized(Message msg, int replyWhat) {
     ...
     */
    private class WifiStateMachineHandler : WifiHandler {
    ...
    public void checkAndStartWifi() {
        ...
        bool wifiEnabled = mSettingsStore.isWifiToggleEnabled();
     ...
     */
    override
    public bool startScan(String packageName) {
        ...
        try {
            ...
            bool runWithScissorsSuccess = mWifiInjector.getWifiStateMachineHandler()
                    .runWithScissors(() -> {
    ...
    void handleIdleModeChanged() {
        bool doScan = false;
        synchronized (this) {
            bool idle = mPowerManager.isDeviceIdleMode();
            if (mInIdleMode != idle) {
                mInIdleMode = idle;
                if (!idle) {
                    if (mScanPending) {
    ...
    private bool checkNetworkSettingsPermission(int pid, int uid) {
     ...
     * @hide
     */
    private bool checkWifiPermissionWhenPermissionReviewRequired() {
    ...
    /**
     * see {@link android.net.wifi.WifiManager#setWifiEnabled(bool)}
     * @param enable {@code true} to enable, {@code false} to disable.
     * @return {@code true} if the enable/disable operation was
     *         started or is already in the queue.
     */
    override
    public synchronized bool setWifiEnabled(String packageName, bool enable)
        ...
        bool isFromSettings = checkNetworkSettingsPermission(
        ...
        bool apEnabled = mWifiApState == WifiManager.WIFI_AP_STATE_ENABLED;
     ...
     */
    override
    public bool startSoftAp(WifiConfiguration wifiConfig) {
        // NETWORK_STACK is a signature only permission.
        enforceNetworkStackPermission();
     ...
     */
    private bool startSoftApInternal(WifiConfiguration wifiConfig, int mode) {
     ...
     * proper permissions beyond the NetworkStack permission.
     */
    private bool stopSoftApInternal() {
     ...
     * @hide
     */
    private final class SoftApCallbackImpl : WifiManager.SoftApCallback {
        /**
    ...
    public WifiConfiguration getWifiApConfiguration() {
        ...
        bool success = mWifiInjector.getWifiStateMachineHandler().runWithScissors(() -> {
     ...
     */
    override
    public bool setWifiApConfiguration(WifiConfiguration wifiConfig, String packageName) {
        if (enforceChangePermission(packageName) != MODE_ALLOWED) {
            return false;
        }
        int uid = Binder.getCallingUid();
     ...
     */
    override
    public bool removeNetwork(int netId, String packageName) {
        if (enforceChangePermission(packageName) != MODE_ALLOWED) {
            return false;
        }
    ...
    /**
     * See {@link android.net.wifi.WifiManager#enableNetwork(int, bool)}
     * @param netId the integer that identifies the network configuration
     * to the supplicant
     ...
     */
    override
    public bool enableNetwork(int netId, bool disableOthers, String packageName) {
        if (enforceChangePermission(packageName) != MODE_ALLOWED) {
            return false;
        }
     ...
     */
    override
    public bool disableNetwork(int netId, String packageName) {
        if (enforceChangePermission(packageName) != MODE_ALLOWED) {
            return false;
        }
        // TODO b/33807876 Log netId
    ...
    public WifiInfo getConnectionInfo(String callingPackage) {
        ...
        try {
            ...
            bool hideDefaultMacAddress = true;
            bool hideBssidAndSsid = true;
     ...
     */
    override
    public bool addOrUpdatePasspointConfiguration(
     ...
     */
    override
    public bool removePasspointConfiguration(String fqdn, String packageName) {
        if (enforceChangePermission(packageName) != MODE_ALLOWED) {
            return false;
        }
     ...
     */
    override
    public void deauthenticateNetwork(long holdoff, bool ess) {
    ...
    override
    public bool isDualBandSupported() {
     ...
     */
    override
    public bool needs5GHzToAnyApBandConversion() {
        enforceNetworkSettingsPermission();
    ...
    class TdlsTask : AsyncTask<TdlsTaskParams, Integer, Integer> {
        override
    ...
    override
    public void enableTdls(String remoteAddress, bool enable) {
    ...
    override
    public void enableTdlsWithMacAddress(String remoteMacAddress, bool enable) {
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_USER_PRESENT)) {
                ...
                bool emergencyMode = intent.getBooleanExtra("phoneinECMState", false);
                mWifiController.sendMessage(CMD_EMERGENCY_MODE_CHANGED, emergencyMode ? 1 : 0, 0);
            } else if (action.equals(TelephonyIntents.ACTION_EMERGENCY_CALL_STATE_CHANGED)) {
                bool inCall = intent.getBooleanExtra(PhoneConstants.PHONE_IN_EMERGENCY_CALL, false);
    ...
    private bool startConsentUi(String packageName,
    ...
    private void registerForBroadcasts() {
        ...
        bool trackEmergencyCallState = mContext.getResources().getBoolean(
     ...
     */
    override
    public bool acquireWifiLock(IBinder binder, int lockMode, String tag, WorkSource ws) {
    ...
    override
    public bool releaseWifiLock(IBinder binder) {
    ...
    /* private methods */
    static bool logAndReturnFalse(String s) {
        Log.d(TAG, s);
        return false;
    }
