 ...
 * @hide
 */
public class WifiP2pServiceImpl : IWifiP2pManager.Stub {
    ...
    // is invoked
    private bool mAutonomousGroup;

    // Invitation to join an existing p2p group
    private bool mJoinExistingGroup;

    // Track whether we are in p2p discovery. This is used to avoid sending duplicate
    // broadcasts
    private bool mDiscoveryStarted;

    // Track whether servcice/peer discovery is blocked in favor of other wifi actions
    // (notably dhcp)
    private bool mDiscoveryBlocked;

    // remember if we were in a scan when it had to be stopped
    private bool mDiscoveryPostponed = false;

    private NetworkInfo mNetworkInfo;

    private bool mTemporarilyDisconnectedWifi = false;
    ...
    private bool getWfdPermission(int uid) {
     ...
     */
    private class P2pStateMachine : StateMachine {
        ...
        private bool mIsInterfaceAvailable = false;
        // Is wifi on or off.
        private bool mIsWifiEnabled = false;
        ...
        P2pStateMachine(String name, Looper looper, bool p2pSupported) {
            super(name, looper);
            ...
            if (p2pSupported) {
                ...
                mWifiNative.registerInterfaceAvailableListener((bool isAvailable) -> {
        ...
        class P2pNotSupportedState : State {
            override
            public bool processMessage(Message message) {
                switch (message.what) {
                    case WifiP2pManager.DISCOVER_PEERS:
        ...
        class P2pEnabledState : State {
            ...
            public bool processMessage(Message message) {
                ...
                switch (message.what) {
                    ...
                    case BLOCK_DISCOVERY:
                        bool blocked = (message.arg1 == ENABLED ? true : false);
        ...
        class GroupCreatingState : State {
            override
            public void enter() {
                if (DBG) logd(getName());
        ...
        class UserAuthorizingNegotiationRequestState : State {
            override
            public void enter() {
                if (DBG) logd(getName());
        ...
        class UserAuthorizingInviteRequestState : State {
            override
            public void enter() {
                if (DBG) logd(getName());
                notifyInvitationReceived();
        ...
        class ProvisionDiscoveryState : State {
            override
            public void enter() {
                if (DBG) logd(getName());
        ...
        class GroupNegotiationState : State {
            override
            public void enter() {
                if (DBG) logd(getName());
            }
        ...
        class FrequencyConflictState : State {
        ...
        class UserAuthorizingJoinState : State {
            override
            public void enter() {
                if (DBG) logd(getName());
                notifyInvitationReceived();
        ...
        class OngoingGroupRemovalState : State {
            override
            public void enter() {
                if (DBG) logd(getName());
            }
        ...
        private void sendP2pStateChangedBroadcast(bool enabled) {
        ...
        private void sendP2pDiscoveryChangedBroadcast(bool started) {
        ...
        private void notifyInvitationReceived() {
            ...
            if ((r.getConfiguration().uiMode & Configuration.UI_MODE_TYPE_APPLIANCE)
                ...
                dialog.setOnKeyListener(new DialogInterface.OnKeyListener() {
                    ...
                    override
                    public bool onKey(DialogInterface dialog, int keyCode, KeyEvent event) {
         ...
         * @param bool if true, reload the group list from scratch
         *                and send broadcast message with fresh list
         */
        private void updatePersistentNetworks(bool reload) {
         ...
         */
        private bool isConfigInvalid(WifiP2pConfig config) {
         ...
         */
        private bool reinvokePersistentGroup(WifiP2pConfig config) {
            if (config == null) {
                Log.e(TAG, "Illegal argument(s)");
                return false;
            }
         ...
         */
        private bool removeClientFromList(int netId, String addr, bool isRemovable) {
            StringBuilder modifiedClientList =  new StringBuilder();
            String[] currentClientList = getClientList(netId);
            bool isClientRemoved = false;
        ...
        private bool setAndPersistDeviceName(String devName) {
        ...
        private bool setWfdInfo(WifiP2pWfdInfo wfdInfo) {
            bool success;
        ...
        private void handleGroupCreationFailure() {
            ...
            bool peersChanged = mPeers.remove(mPeersLostDuringConnection);
         ...
         */
        private bool updateSupplicantServiceRequest() {
            clearSupplicantServiceRequest();
        ...
        private bool addServiceRequest(Messenger m, WifiP2pServiceRequest req) {
            if (m == null || req == null) {
                Log.e(TAG, "Illegal argument(s)");
                return false;
            }
        ...
        private bool addLocalService(Messenger m, WifiP2pServiceInfo servInfo) {
         ...
         * @return the specified ClientInfo.
         */
        private ClientInfo getClientInfo(Messenger m, bool createIfNotExist) {
