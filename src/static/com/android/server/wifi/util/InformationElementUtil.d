...
public class InformationElementUtil {
    ...
    public static class ExtendedCapabilities {
         ...
         */
        public bool is80211McRTTResponder() {
    ...
    public static class Capabilities {
        ...
        private static bool isWpsElement(InformationElement ie) {
            ByteBuffer buf = ByteBuffer.wrap(ie.bytes).order(ByteOrder.LITTLE_ENDIAN);
            try {
                // WPS OUI and type
        ...
        private static bool isWpaOneElement(InformationElement ie) {
            ByteBuffer buf = ByteBuffer.wrap(ie.bytes).order(ByteOrder.LITTLE_ENDIAN);
        ...
        public String generateCapabilitiesString() {
            ...
            bool isWEP = (protocol.isEmpty()) && isPrivacy;
    ...
    public static class WifiMode {
         ...
         */
        public static int determineMode(int frequency, int maxRate, bool foundVht,
                bool foundHt, bool foundErp) {
            if (foundVht) {
                return MODE_11AC;
            } else if (foundHt) {
                return MODE_11N;
