...
public class ScanRequestProxy {
    ...
    private bool mScanningForHiddenNetworksEnabled = false;
    // Flag to indicate that we're waiting for scan results from an existing request.
    private bool mIsScanProcessingComplete = true;
    ...
    private class ScanRequestProxyScanListener : WifiScanner.ScanListener {
        override
     ...
     */
    public void enableScanningForHiddenNetworks(bool enable) {
        if (mVerboseLoggingEnabled) {
     ...
     */
    private bool retrieveWifiScannerIfNecessary() {
     ...
     */
    private void sendScanResultBroadcastIfScanProcessingNotComplete(bool scanSucceeded) {
     ...
     */
    private void sendScanResultBroadcast(bool scanSucceeded) {
     ...
     */
    private bool shouldScanRequestBeThrottledForForegroundApp(
            int callingUid, String packageName) {
        LinkedList<Long> scanRequestTimestamps =
     ...
     */
    private bool shouldScanRequestBeThrottledForBackgroundApp() {
     ...
     */
    private bool isRequestFromBackground(int callingUid, String packageName) {
     ...
     * to be throttled.
     */
    private bool shouldScanRequestBeThrottledForApp(int callingUid, String packageName) {
        bool isThrottled;
     ...
     */
    public bool startScan(int callingUid, String packageName) {
        if (!retrieveWifiScannerIfNecessary()) {
            Log.e(TAG, "Failed to retrieve wifiscanner");
            sendScanResultFailureBroadcastToPackage(packageName);
            return false;
        }
        bool fromSettingsOrSetupWizard =
