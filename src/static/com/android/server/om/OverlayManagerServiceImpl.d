...
final class OverlayManagerServiceImpl {
     ...
     */
    private static bool mustReinitializeOverlay(@NonNull final PackageInfo theTruth,
     ...
     */
    private bool updateAllOverlaysForTarget(@NonNull final String targetPackageName,
    ...
    bool setEnabled(@NonNull final String packageName, final bool enable,
            final int userId) {
        if (DEBUG) {
        ...
        try {
            final OverlayInfo oi = mSettings.getOverlayInfo(packageName, userId);
            bool modified = mSettings.setEnabled(packageName, userId, enable);
    ...
    bool setEnabledExclusive(@NonNull final String packageName, bool withinCategory,
            final int userId) {
        if (DEBUG) {
    ...
    private bool isPackageUpdatableOverlay(@NonNull final String packageName, final int userId) {
        final PackageInfo overlayPackage = mPackageManager.getPackageInfo(packageName, userId);
     ...
     */
    private bool updateState(@NonNull final String targetPackageName,
    ...
    private @OverlayInfo.State int calculateNewState(@Nullable final PackageInfo targetPackage,
        ...
        final bool enabled = mSettings.getEnabled(overlayPackage.packageName, userId);
    ...
    interface PackageManagerHelper {
        ...
        bool signaturesMatching(@NonNull String packageName1, @NonNull String packageName2,
                                   int userId);
