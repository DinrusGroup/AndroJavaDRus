 ...
 */
abstract class RemoteListenerHelper<TListener : IInterface> {
    ...
    private volatile bool mIsRegistered;  // must access only on handler thread, or read-only

    private bool mHasIsSupported;
    ...
    public bool addListener(@NonNull TListener listener) {
    ...
    protected abstract bool isAvailableInPlatform();
    protected abstract bool isGpsEnabled();
    ...
    protected interface ListenerOperation<TListener : IInterface> {
    ...
    protected void setSupported(bool value) {
        synchronized (mListenerMap) {
    ...
    private class LinkedListener : IBinder.DeathRecipient {
    ...
    private class HandlerRunnable : Runnable {
        private final TListener mListener;
