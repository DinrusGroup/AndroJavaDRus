...
/* package */ class ContextHubClientManager {
     ...
     */
    private static final bool DEBUG_LOG_ENABLED = true;
    ...
    private synchronized ContextHubClientBroker createNewClientBroker(
        ...
        for (int i = 0; i <= MAX_CLIENT_ID; i++) {
            if (!mHostEndPointIdToClientMap.containsKey(cast(short)id)) {
                broker = new ContextHubClientBroker(
                        mContext, mContextHubProxy, this, contextHubId, cast(short)id, clientCallback);
                mHostEndPointIdToClientMap.put(cast(short)id, broker);
