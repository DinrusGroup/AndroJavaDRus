...
public class TarBackupReader {
    ...
    public FileMetadata readTarHeaders() throws IOException {
        ...
        bool gotHeader = readTarHeader(block);
        if (gotHeader) {
            try {
    ...
    public RestorePolicy chooseRestorePolicy(PackageManager packageManager,
            bool allowApks, FileMetadata info, Signature[] signatures,
    ...
    private bool readTarHeader(byte[] block) throws IOException {
    ...
    private bool readPaxExtendedHeader(FileMetadata info)
            throws IOException {
