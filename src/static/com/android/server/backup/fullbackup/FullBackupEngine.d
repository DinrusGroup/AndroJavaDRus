...
public class FullBackupEngine {
    ...
    class FullBackupRunner : Runnable {
        ...
        FullBackupRunner(PackageInfo pack, IBackupAgent agent, ParcelFileDescriptor pipe,
                int token, bool sendApk, bool writeManifest, byte[] widgetData)
                throws IOException {
            mPackage = pack;
        ...
        public void run() {
            try {
                ...
                if (mWriteManifest) {
                    final bool writeWidgetData = mWidgetData != null;
                    if (MORE_DEBUG) {
    ...
    public FullBackupEngine(BackupManagerService backupManagerService,
            ...
            bool alsoApks, BackupRestoreTask timeoutMonitor, long quota, int opToken,
            int transportFlags) {
    ...
    public int backupOnePackage() throws RemoteException {
        ...
        if (initializeAgent()) {
            ...
            try {
                ...
                final bool sendApk = mIncludeApks
                        && !isSharedStorage
