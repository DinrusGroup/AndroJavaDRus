 ...
 */
public class PerformAdbBackupTask : FullBackupTask : BackupRestoreTask {
    ...
    public PerformAdbBackupTask(BackupManagerService backupManagerService,
            ...
            bool includeApks, bool includeObbs, bool includeShared, bool doWidgets,
            String curPassword, String encryptPassword, bool doAllApps, bool doSystem,
            bool doCompress, bool doKeyValue, String[] packages, AtomicBoolean latch) {
        super(observer);
    ...
    public void run() {
        ...
        try {
            bool encrypting = (mEncryptPassword != null && mEncryptPassword.length() > 0);
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (mIncludeObbs && !isSharedStorage) {
                    bool obbOkay = obbConnection.backupObbs(pkg, out);
                    if (!obbOkay) {
