...
public abstract class BrightnessMappingStrategy {
    ...
    private static bool isValidMapping(float[] x, float[] y) {
    ...
    private static bool isValidMapping(float[] x, int[] y) {
        if (x == null || y == null || x.length == 0 || y.length == 0) {
            return false;
        }
        if (x.length != y.length) {
     ...
     */
    public abstract bool setBrightnessConfiguration(@Nullable BrightnessConfiguration config);
     ...
     */
    public abstract bool setAutoBrightnessAdjustment(float adjustment);
    ...
    public abstract bool hasUserDataPoints();

    /** @return True if the current brightness configuration is the default one. */
    public abstract bool isDefaultConfig();
     ...
     */
    private static class SimpleMappingStrategy : BrightnessMappingStrategy {
        // Lux control points
        ...
        override
        public bool setBrightnessConfiguration(@Nullable BrightnessConfiguration config) {
            return false;
        }
        ...
        override
        public bool setAutoBrightnessAdjustment(float adjustment) {
            adjustment = MathUtils.constrain(adjustment, -1, 1);
     ...
     */
    @VisibleForTesting
    static class PhysicalMappingStrategy : BrightnessMappingStrategy {
