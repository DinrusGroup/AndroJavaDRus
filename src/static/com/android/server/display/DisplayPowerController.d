 ...
 */
final class DisplayPowerController : AutomaticBrightnessController.Callbacks {
    private static final String TAG = "DisplayPowerController";
    private static final String SCREEN_ON_BLOCKED_TRACE_NAME = "Screen on blocked";
    private static final String SCREEN_OFF_BLOCKED_TRACE_NAME = "Screen off blocked";

    private static final bool DEBUG = false;
    private static final bool DEBUG_PRETEND_PROXIMITY_SENSOR_ABSENT = false;

    // If true, uses the color fade on animation.
    // We might want to turn this off if we cannot get a guarantee that the screen
    // actually turns on and starts showing new content after the call to set the
    // screen state returns.  Playing the animation can also be somewhat slow.
    private static final bool USE_COLOR_FADE_ON_ANIMATION = false;
    ...
    private bool mUseSoftwareAutoBrightnessConfig;

    // True if should use light sensor to automatically determine doze screen brightness.
    private final bool mAllowAutoBrightnessWhileDozingConfig;

    // Whether or not the color fade on screen on / off is enabled.
    private final bool mColorFadeEnabled;

    // True if we should fade the screen while turning it off, false if we should play
    // a stylish color fade animation instead.
    private bool mColorFadeFadesConfig;
    ...
    private bool mDisplayBlanksAfterDozeConfig;
    ...
    private bool mBrightnessBucketsInDozeConfig;
    ...
    // Guarded by mLock.
    private bool mPendingWaitForNegativeProximityLocked;

    // True if the pending power request or wait for negative proximity flag
    // has been changed since the last update occurred.
    // Guarded by mLock.
    private bool mPendingRequestChangedLocked;

    // Set to true when the important parts of the pending power request have been applied.
    // The important parts are mainly the screen state.  Brightness changes may occur
    // concurrently.
    // Guarded by mLock.
    private bool mDisplayReadyLocked;

    // Set to true if a power state update is required.
    // Guarded by mLock.
    private bool mPendingUpdatePowerStateLocked;
    ...
    private bool mWaitingForNegativeProximity;
    ...
    private bool mProximitySensorEnabled;
    ...
    private bool mScreenOffBecauseOfProximity;
    ...
    private bool mPendingScreenOff;

    // True if we have unfinished business and are holding a suspend blocker.
    private bool mUnfinishedBusiness;
    ...
    private bool mAppliedAutoBrightness;
    private bool mAppliedDimming;
    private bool mAppliedLowPower;
    private bool mAppliedScreenBrightnessOverride;
    private bool mAppliedTemporaryBrightness;
    private bool mAppliedTemporaryAutoBrightnessAdjustment;
    private bool mAppliedBrightnessBoost;
    ...
    private final bool mSkipScreenOnBrightnessRamp;
    ...
    public DisplayPowerController(Context context,
        ...
        if (mUseSoftwareAutoBrightnessConfig) {
            ...
            bool autoBrightnessResetAmbientLuxAfterWarmUp = resources.getBoolean(
     ...
     */
    public bool isProximitySensorAvailable() {
    ...
    public ParceledListSlice<BrightnessChangeEvent> getBrightnessEvents(
            @UserIdInt int userId, bool includePackage) {
     ...
     */
    public bool requestPowerState(DisplayPowerRequest request,
            bool waitForNegativeProximity) {
        if (DEBUG) {
    ...
    private void updatePowerState() {
        ...
        bool performScreenOffTransition = false;
        ...
        final bool autoBrightnessEnabledInDoze =
                mAllowAutoBrightnessWhileDozingConfig && Display.isDozeState(state);
        final bool autoBrightnessEnabled = mPowerRequest.useAutoBrightness
                    && (state == Display.STATE_ON || autoBrightnessEnabledInDoze)
                    && brightness < 0
                    && mAutomaticBrightnessController != null;

        final bool userSetBrightnessChanged = updateUserSetScreenBrightness();
        ...
        final bool autoBrightnessAdjustmentChanged = updateAutoBrightnessAdjustment();
        ...
        final bool userInitiatedChange = brightness < 0
                && (autoBrightnessAdjustmentChanged || userSetBrightnessChanged);

        bool hadUserBrightnessPoint = false;
        ...
        if (!mPendingScreenOff) {
            ...
            final bool wasOrWillBeInVr =
                    (state == Display.STATE_VR || oldState == Display.STATE_VR);
            final bool initialRampSkip =
            ...
            final bool hasBrightnessBuckets =
            ...
            final bool isDisplayContentVisible =
                    mColorFadeEnabled && mPowerState.getColorFadeLevel() == 1.0f;
            final bool brightnessIsTemporary =
        ...
        final bool ready = mPendingScreenOnUnblocker == null &&
                (!mColorFadeEnabled ||
                        (!mColorFadeOnAnimator.isStarted() && !mColorFadeOffAnimator.isStarted()))
                && mPowerState.waitUntilClean(mCleanListener);
        final bool finished = ready
    ...
    private bool setScreenState(int state) {
        return setScreenState(state, false /*reportOnly*/);
    }

    private bool setScreenState(int state, bool reportOnly) {
        final bool isOff = (state == Display.STATE_OFF);
    ...
    private void animateScreenStateChange(int target, bool performScreenOffTransition) {
    ...
    private void handleProximitySensorEvent(long time, bool positive) {
        if (mProximitySensorEnabled) {
    ...
    private void handleSettingsChange(bool userSwitch) {
    ...
    private bool updateAutoBrightnessAdjustment() {
    ...
    private bool updateUserSetScreenBrightness() {
    ...
    private void notifyBrightnessChanged(int brightness, bool userInitiated,
            bool hadUserDataPoint) {
    ...
    private final class DisplayControllerHandler : Handler {
    ...
    private final SensorEventListener mProximitySensorListener = new SensorEventListener() {
        ...
        public void onSensorChanged(SensorEvent event) {
            if (mProximitySensorEnabled) {
                ...
                bool positive = distance >= 0.0f && distance < mProximityThreshold;
    ...
    private final class ScreenOnUnblocker : WindowManagerPolicy.ScreenOnListener {
        override
    ...
    private final class ScreenOffUnblocker : WindowManagerPolicy.ScreenOffListener {
        override
