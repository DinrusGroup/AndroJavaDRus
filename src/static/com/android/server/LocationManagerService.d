 ...
 * updates and alerts.
 */
public class LocationManagerService : ILocationManager.Stub {
    private static final String TAG = "LocationManagerService";
    public static final bool D = Log.isLoggable(TAG, Log.DEBUG);
    ...
    private bool mGnssBatchingInProgress = false;
    ...
    private void onUidImportanceChanged(int uid, int importance) {
        bool foreground = isImportanceForeground(importance);
    ...
    private static bool isImportanceForeground(int importance) {
     ...
     */
    private bool isCurrentProfile(int userId) {
        synchronized (mLock) {
    ...
    private void loadProvidersLocked() {
        ...
        */
        ...
        bool activityRecognitionHardwareIsSupported = ActivityRecognitionHardware.isSupported();
     ...
     * location updates.
     */
    private final class Receiver : IBinder.DeathRecipient, PendingIntent.OnFinished {
        ...
        final bool mHideFromAppOps; // True if AppOps should not monitor this receiver.
        final Object mKey;
        ...
        bool mOpHighPowerMonitoring;
        int mPendingBroadcasts;
        PowerManager.WakeLock mWakeLock;

        Receiver(ILocationListener listener, PendingIntent intent, int pid, int uid,
                String packageName, WorkSource workSource, bool hideFromAppOps) {
            mListener = listener;
         ...
         */
        public void updateMonitoring(bool allow) {
            if (mHideFromAppOps) {
                return;
            }

            bool requestingLocation = false;
            bool requestingHighPowerLocation = false;
            if (allow) {
            ...
            bool wasHighPowerMonitoring = mOpHighPowerMonitoring;
         ...
         */
        private bool updateMonitoring(bool allowMonitoring, bool currentlyMonitoring,
                int op) {
        ...
        public bool callStatusChangedLocked(String provider, int status, Bundle extras) {
            if (mListener != null) {
                try {
                    synchronized (this) {
        ...
        public bool callLocationChangedLocked(Location location) {
            if (mListener != null) {
                try {
                    synchronized (this) {
                        // synchronize to ensure incrementPendingBroadcastsLocked()
        ...
        public bool callProviderEnabledLocked(String provider, bool enabled) {
     ...
     */
    private bool hasGnssPermissions(String packageName) {
     ...
     */
    override
    public bool addGnssBatchingCallback(IBatchedLocationCallback callback, String packageName) {
        mContext.enforceCallingPermission(android.Manifest.permission.LOCATION_HARDWARE,
                "Location Hardware permission not granted to access hardware batching");
    ...
    private class LinkedCallback : IBinder.DeathRecipient {
     ...
     */
    override
    public bool startGnssBatch(long periodNanos, bool wakeOnFifoFull, String packageName) {
        mContext.enforceCallingPermission(android.Manifest.permission.LOCATION_HARDWARE,
                "Location Hardware permission not granted to access hardware batching");
     ...
     */
    private bool isAllowedByCurrentUserSettingsLocked(String provider) {
     ...
     */
    private bool isAllowedByUserSettingsLockedForUser(String provider, int userId) {
     ...
     * @param userId   the user id to query
     */
    private bool isAllowedByUserSettingsLocked(String provider, int uid, int userId) {
    ...
    bool reportLocationAccessNoThrow(
    ...
    bool checkLocationAccess(int pid, int uid, String packageName, int allowedResolutionLevel) {
        int op = resolutionLevelToOp(allowedResolutionLevel);
        if (op >= 0) {
    ...
    override
    public bool providerMeetsCriteria(String provider, Criteria criteria) {
        LocationProviderInterface p = mProvidersByName.get(provider);
        if (p == null) {
            throw new IllegalArgumentException("provider=" + provider);
        }

        bool result = LocationProvider.propertiesMeetCriteria(
    ...
    private void updateProvidersLocked() {
        ...
        for (int i = mProviders.size() - 1; i >= 0; i--) {
            ...
            bool isEnabled = p.isEnabled();
            String name = p.getName();
            bool shouldBeEnabled = isAllowedByCurrentUserSettingsLocked(name);
    ...
    private void updateProviderListenersLocked(String provider, bool enabled) {
        int listeners = 0;
     ...
     */
    private static bool isValidWorkSource(WorkSource workSource) {
    ...
    private bool isThrottlingExemptLocked(Identity identity) {
    ...
    private class UpdateRecord {
         ...
         */
        void disposeLocked(bool removeReceiver) {
    ...
    private Receiver getReceiverLocked(PendingIntent intent, int pid, int uid, String packageName,
            WorkSource workSource, bool hideFromAppOps) {
    ...
    private LocationRequest createSanitizedRequest(LocationRequest request, int resolutionLevel,
            bool callerHasLocationHardwarePermission) {
    ...
    private Receiver checkListenerOrIntentLocked(ILocationListener listener, PendingIntent intent,
            int pid, int uid, String packageName, WorkSource workSource, bool hideFromAppOps) {
    ...
    public void requestLocationUpdates(LocationRequest request, ILocationListener listener,
        ...
        bool hideFromAppOps = request.getHideFromAppOps();
        if (hideFromAppOps) {
    ...
    private void requestLocationUpdatesLocked(LocationRequest request, Receiver receiver,
        ...
        bool isProviderEnabled = isAllowedByUserSettingsLocked(name, uid, mCurrentUserId);
     ...
     */
    override
    public bool injectLocation(Location location) {
        mContext.enforceCallingPermission(android.Manifest.permission.LOCATION_HARDWARE,
    ...
    override
    public bool registerGnssStatusCallback(IGnssStatusListener callback, String packageName) {
    ...
    override
    public bool addGnssMeasurementsListener(
    ...
    override
    public bool addGnssNavigationMessageListener(
     ...
     */
    override
    public bool isLocationEnabledForUser(int userId) {
     ...
     */
    override
    public void setLocationEnabledForUser(bool enabled, int userId) {
     ...
     */
    override
    public bool isProviderEnabledForUser(String provider, int userId) {
        // Check INTERACT_ACROSS_USERS permission if userId is not current user id.
        checkInteractAcrossUsersPermission(userId);
     ...
     */
    override
    public bool setProviderEnabledForUser(String provider, bool enabled, int userId) {
        mContext.enforceCallingPermission(
                android.Manifest.permission.WRITE_SECURE_SETTINGS,
                "Requires WRITE_SECURE_SETTINGS permission");
     ...
     */
    private bool isLocationProviderEnabledForUser(String provider, int userId) {
        long identity = Binder.clearCallingIdentity();
        try {
            // Use system settings
     ...
     */
    private bool isUidALocationProvider(int uid) {
     ...
     */
    private bool doesUidHavePackage(int uid, String packageName) {
        if (packageName == null) {
            return false;
        }
    ...
    override
    public void reportLocation(Location location, bool passive) {
        checkCallerIsProvider();
    ...
    private static bool shouldBroadcastSafe(
    ...
    private void handleLocationChangedLocked(Location location, bool passive) {
    ...
    private class LocationWorkerHandler : Handler {
    ...
    private bool isMockProvider(String provider) {
        synchronized (mLock) {
            return mMockProviders.containsKey(provider);
        }
    }

    private void handleLocationChanged(Location location, bool passive) {
    ...
    private bool canCallerAccessMockLocation(String opPackageName) {
    ...
    override
    public void setTestProviderEnabled(String provider, bool enabled, String opPackageName) {
        if (!canCallerAccessMockLocation(opPackageName)) {
            return;
        }
        setTestProviderEnabled(provider, enabled);
    ...
    private void setTestProviderEnabled(String provider, bool enabled) {
        synchronized (mLock) {
            MockProvider mockProvider = mMockProviders.get(provider);
            if (mockProvider == null) {
                throw new IllegalArgumentException("Provider \"" + provider + "\" unknown");
