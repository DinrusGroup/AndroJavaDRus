...
class WindowSurfaceController {
    ...
    private bool mSurfaceShown = false;
    ...
    private bool mHiddenForCrop = false;

    // Initially a surface is hidden after just being created.
    private bool mHiddenForOtherReasons = true;
    ...
    void setCropInTransaction(Rect clipRect, bool recoveringMemory) {
    ...
    void clearCropInTransaction(bool recoveringMemory) {
        if (SHOW_TRANSACTIONS) logSurface(
                "CLEAR CROP", null);
        try {
    ...
    void setPositionInTransaction(float left, float top, bool recoveringMemory) {
        setPosition(null, left, top, recoveringMemory);
    }

    void setPosition(SurfaceControl.Transaction t, float left, float top,
            bool recoveringMemory) {
        final bool surfaceMoved = mSurfaceX != left || mSurfaceY != top;
        if (surfaceMoved) {
            mSurfaceX = left;
            mSurfaceY = top;
    ...
    void setGeometryAppliesWithResizeInTransaction(bool recoveringMemory) {
    ...
    void setMatrix(SurfaceControl.Transaction t, float dsdx, float dtdx,
            float dtdy, float dsdy, bool recoveringMemory) {
        final bool matrixChanged = mLastDsdx != dsdx || mLastDtdx != dtdx ||
    ...
    bool setSizeInTransaction(int width, int height, bool recoveringMemory) {
        final bool surfaceResized = mSurfaceW != width || mSurfaceH != height;
        if (surfaceResized) {
            mSurfaceW = width;
            mSurfaceH = height;
    ...
    bool prepareToShowInTransaction(float alpha,
            float dsdx, float dtdx, float dsdy,
            float dtdy, bool recoveringMemory) {
        if (mSurfaceControl != null) {
            try {
                mSurfaceAlpha = alpha;
    ...
    bool showRobustlyInTransaction() {
        if (SHOW_TRANSACTIONS) logSurface(
    ...
    void forceScaleableInTransaction(bool force) {
    ...
    bool clearWindowContentFrameStats() {
        if (mSurfaceControl == null) {
            return false;
        }
    ...
    bool getWindowContentFrameStats(WindowContentFrameStats outStats) {
        if (mSurfaceControl == null) {
            return false;
        }
    ...
    void setShown(bool surfaceShown) {
