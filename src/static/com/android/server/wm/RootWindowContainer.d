...
class RootWindowContainer : WindowContainer<DisplayContent> {
    private static final String TAG = TAG_WITH_CLASS_NAME ? "RootWindowContainer" : TAG_WM;

    private static final int SET_SCREEN_BRIGHTNESS_OVERRIDE = 1;
    private static final int SET_USER_ACTIVITY_TIMEOUT = 2;

    private bool mWallpaperForceHidingChanged = false;
    private Object mLastWindowFreezeSource = null;
    private Session mHoldScreen = null;
    private float mScreenBrightness = -1;
    private long mUserActivityTimeout = -1;
    private bool mUpdateRotation = false;
    ...
    private bool mObscureApplicationContentOnSecondaryDisplays = false;

    private bool mSustainedPerformanceModeEnabled = false;
    private bool mSustainedPerformanceModeCurrent = false;

    bool mWallpaperMayChange = false;
    ...
    bool mOrientationChangeComplete = true;
    bool mWallpaperActionPending = false;
    ...
    WindowState computeFocusedWindow() {
        ...
        final bool forceDefaultDisplay = mService.isKeyguardShowingAndNotOccluded();
    ...
    int[] setDisplayOverrideConfigurationIfNeeded(Configuration newConfiguration, int displayId) {
        ...
        final bool configChanged = currentConfig.diff(newConfiguration) != 0;
        if (!configChanged) {
            return null;
        }
    ...
    private void setGlobalConfigurationIfNeeded(Configuration newConfiguration,
        ...
        final bool configChanged = getConfiguration().diff(newConfiguration) != 0;
        if (!configChanged) {
            return;
        }
    ...
    void setSecureSurfaceState(int userId, bool disabled) {
        forAllWindows((w) -> {
    ...
    void updateHiddenWhileSuspendedState(final ArraySet<String> packages, final bool suspended) {
        forAllWindows((w) -> {
    ...
    bool canShowStrictModeViolation(int pid) {
    ...
    bool hasPendingLayoutChanges(WindowAnimator animator) {
    ...
    bool reclaimSomeSurfaceMemory(WindowStateAnimator winAnimator, String operation,
            bool secure) {
        final WindowSurfaceController surfaceController = winAnimator.mSurfaceController;
        bool leakedSurface = false;
        bool killedApps = false;
    ...
    void performSurfacePlacement(bool recoveringMemory) {
        if (DEBUG_WINDOW_TRACE) Slog.v(TAG, "performSurfacePlacementInner: entry. Called by "
                + Debug.getCallers(3));

        int i;
        bool updateInputWindowsNeeded = false;
    ...
    private void applySurfaceChangesTransaction(bool recoveringMemory, int defaultDw,
            int defaultDh) {
     ...
     */
    bool handleNotObscuredLocked(WindowState w, bool obscured, bool syswin) {
        final WindowManager.LayoutParams attrs = w.mAttrs;
        final int attrFlags = attrs.flags;
        final bool onScreen = w.isOnScreen();
        final bool canBeSeen = w.isDisplayedLw();
    ...
    void dumpWindowsNoHeader(PrintWriter pw, bool dumpAll, ArrayList<WindowState> windows) {
