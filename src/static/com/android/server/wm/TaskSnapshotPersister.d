...
class TaskSnapshotPersister {
    ...
    static final bool DISABLE_FULL_SIZED_BITMAPS = ActivityManager.isLowRamDeviceStatic();
    ...
    private bool createDirectory(int userId) {
    ...
    private Thread mPersister = new Thread("TaskSnapshotPersister") {
        public void run() {
            ...
            while (true) {
                ...
                synchronized (mLock) {
                    final bool writeQueueEmpty = mWriteQueue.isEmpty();
    ...
    private class StoreWriteQueueItem : WriteQueueItem {
    ...
    private class DeleteWriteQueueItem : WriteQueueItem {
        private final int mTaskId;
        private final int mUserId;
    ...
    @VisibleForTesting
    class RemoveObsoleteFilesQueueItem : WriteQueueItem {
