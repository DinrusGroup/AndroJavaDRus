...
class PinnedStackController {
     ...
     */
    private class PinnedStackControllerCallback : IPinnedStackController.Stub {

        override
        public void setIsMinimized(final bool isMinimized) {
            mHandler.post(() -> {
     ...
     */
    private class PinnedStackListenerDeathHandler : IBinder.DeathRecipient {
     ...
     */
    public bool isValidPictureInPictureAspectRatio(float aspectRatio) {
     ...
     */
    bool onTaskStackBoundsChanged(Rect targetBounds, Rect outBounds) {
        synchronized (mService.mWindowMap) {
     ...
     */
    void setAdjustedForIme(bool adjustedForIme, int imeHeight) {
        ...
        final bool imeShowing = adjustedForIme && imeHeight > 0;
     ...
     */
    void setAdjustedForShelf(bool adjustedForShelf, int shelfHeight) {
        final bool shelfShowing = adjustedForShelf && shelfHeight > 0;
     ...
     */
    private void notifyImeVisibilityChanged(bool imeVisible, int imeHeight) {
        if (mPinnedStackListener != null) {
            try {
    ...
    private void notifyShelfVisibilityChanged(bool shelfVisible, int shelfHeight) {
        if (mPinnedStackListener != null) {
            try {
     ...
     */
    private void notifyMinimizeChanged(bool isMinimized) {
        if (mPinnedStackListener != null) {
            try {
     ...
     */
    private void notifyMovementBoundsChanged(bool fromImeAdjustment,
            bool fromShelfAdjustment) {
        synchronized (mService.mWindowMap) {
            if (mPinnedStackListener == null) {
                return;
            }
     ...
     *         controller.
     */
    private Rect getMovementBounds(Rect stackBounds, bool adjustForIme, bool adjustForShelf) {
        synchronized (mService.mWindowMap) {
