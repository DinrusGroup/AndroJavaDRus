...
public class TaskStack : WindowContainer<Task> :
    ...
    private bool mAnimationBackgroundSurfaceIsShown = false;
    ...
    private bool mAdjustedForIme;
    private bool mImeGoingAway;
    ...
    private bool mBoundsAnimating = false;
    ...
    private bool mBoundsAnimatingRequested = false;
    private bool mBoundsAnimatingToFullscreen = false;
    private bool mCancelCurrentBoundsAnimation = false;
    ...
    private void setAdjustedBounds(Rect bounds) {
        ...
        final bool adjusted = !mAdjustedBounds.isEmpty();
    ...
    private void alignTasksToAdjustedBounds(Rect adjustedBounds, Rect tempInsetBounds) {
        ...
        final bool alignBottom = mAdjustedForIme && getDockSide() == DOCKED_TOP;
     ...
     */
    void setAnimationFinalBounds(Rect sourceHintBounds, Rect destBounds, bool toFullscreen) {
    ...
    bool updateBoundsAfterConfigChange() {
        if (mDisplayContent == null) {
        ...
        if (inPinnedWindowingMode()) {
            ...
            bool updated = mDisplayContent.mPinnedStackControllerLocked.onTaskStackBoundsChanged(
                    mTmpRect2, mTmpRect3);
            if (updated) {
     ...
     */
    void addTask(Task task, int position, bool showForAllUsers, bool moveParents) {
    ...
    override
    void positionChildAt(int position, Task child, bool includingParents) {
        positionChildAt(position, child, includingParents, child.showForAllUsers());
    }

    /**
     * Overridden version of {@link TaskStack#positionChildAt(int, Task, bool)}. Used in
     * {@link TaskStack#addTask(Task, int, bool showForAllUsers, bool)}, as it can receive
     * showForAllUsers param from {@link AppWindowToken} instead of {@link Task#showForAllUsers()}.
     */
    private void positionChildAt(int position, Task child, bool includingParents,
            bool showForAllUsers) {
    ...
    private int findPositionForTask(Task task, int targetPosition, bool showForAllUsers,
            bool addingNew) {
        final bool canShowTask =
    ...
    private Rect calculateBoundsForWindowModeChange() {
        final bool inSplitScreenPrimary = inSplitScreenPrimaryWindowingMode();
        ...
        if (inSplitScreenPrimary || (splitScreenStack != null
            ...
            final bool dockedOnTopOrLeft = mService.mDockedStackCreateMode
                    == SPLIT_SCREEN_CREATE_MODE_TOP_OR_LEFT;
    ...
    void getStackDockedModeBoundsLocked(Rect currentTempTaskBounds, Rect outStackBounds,
        ...
        final bool dockedOnTopOrLeft = dockedSide == DOCKED_TOP || dockedSide == DOCKED_LEFT;
    ...
    private void getStackDockedModeBounds(
            ...
            bool dockOnTopOrLeft) {
        final bool dockedStack = inSplitScreenPrimaryWindowingMode();
        final bool splitHorizontally = displayRect.width() > displayRect.height();
     ...
     */
    void setAdjustedForIme(WindowState imeWin, bool forceUpdate) {
        mImeWin = imeWin;
        mImeGoingAway = false;
    ...
    bool isAdjustedForIme() {
        return mAdjustedForIme;
    }

    bool isAnimatingForIme() {
     ...
     */
    bool updateAdjustForIme(float adjustAmount, float adjustDividerAmount, bool force) {
     ...
     *                        with IME window.
     */
    void resetAdjustedForIme(bool adjustBoundsNow) {
        if (adjustBoundsNow) {
            mImeWin = null;
            mImeGoingAway = false;
            mAdjustImeAmount = 0f;
     ...
     */
    bool setAdjustedForMinimizedDock(float minimizeAmount) {
    ...
    private bool adjustForIME(final WindowState imeWin) {
        final int dockedSide = getDockSide();
        final bool dockedTopOrBottom = dockedSide == DOCKED_TOP || dockedSide == DOCKED_BOTTOM;
    ...
    private bool adjustForMinimizedDockedStack(float minimizeAmount) {
    ...
    private bool isMinimizedDockAndHomeStackResizable() {
    ...
    private void updateAdjustedBounds() {
        ...
        final bool isImeTarget = (mService.getImeFocusStackLocked() == this);
    ...
    bool isAdjustedForMinimizedDockedStack() {
    ...
    bool hasTaskForUser(int userId) {
    ...
    void setTouchExcludeRegion(Task focusedTask, int delta, Region touchExcludeRegion,
        ...
        for (int i = mChildren.size() - 1; i >= 0; --i) {
            ...
            final bool isFreeformed = task.inFreeformWindowingMode();
    ...
    public bool setPinnedStackSize(Rect stackBounds, Rect tempTaskBounds) {
    ...
    public void onAnimationStart(bool schedulePipModeChangedCallback, bool forceUpdate) {
        // Hold the lock since this is called from the BoundsAnimator running on the UiThread
        synchronized (mService.mWindowMap) {
    ...
    override  // AnimatesBounds
    public void onAnimationEnd(bool schedulePipModeChangedCallback, Rect finalStackSize,
            bool moveToFullscreen) {
        if (inPinnedWindowingMode()) {
    ...
    override
    public bool shouldDeferStartOnMoveToFullscreen() {
     ...
     */
    public bool deferScheduleMultiWindowModeChanged() {
        if (inPinnedWindowingMode()) {
    ...
    public bool isForceScaled() {
        return mBoundsAnimating;
    }

    public bool isAnimatingBounds() {
        return mBoundsAnimating;
    }

    public bool lastAnimatingBoundsWasToFullscreen() {
        return mBoundsAnimatingToFullscreen;
    }

    public bool isAnimatingBoundsToFullscreen() {
        return isAnimatingBounds() && lastAnimatingBoundsWasToFullscreen();
    }

    public bool pinnedStackResizeDisallowed() {
    ...
    private bool canSpecifyOrientation() {
        final int windowingMode = getWindowingMode();
