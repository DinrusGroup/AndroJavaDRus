 ...
 */
public class RecentsAnimationController : DeathRecipient {
    ...
    private bool mPendingStart = true;
    ...
    private bool mInputConsumerEnabled;

    // Whether or not the recents animation should cause the primary split-screen stack to be
    // minimized
    private bool mSplitScreenMinimized;

    private final Rect mTmpRect = new Rect();

    private bool mLinkedToDeathOfRunner;

    public interface RecentsAnimationCallbacks {
        void onAnimationFinished(@ReorderMode int reorderMode, bool runSychronously);
    }
    ...
    private final IRecentsAnimationController mController =
        ...
        override
        public void finish(bool moveHomeToTop) {
        ...
        override
        public void setAnimationTargetsBehindSystemBars(bool behindSystemBars)
        ...
        override
        public void setInputConsumerEnabled(bool enabled) {
        ...
        override
        public void setSplitScreenMinimized(bool minimized) {
            final long token = Binder.clearCallingIdentity();
            try {
                synchronized (mService.getWindowManagerLock()) {
                    if (mCanceled) {
    ...
    @VisibleForTesting
    AnimationAdapter addAnimation(Task task, bool isRecentTaskInvisible) {
    ...
    private void cancelAnimation(@ReorderMode int reorderMode, bool runSynchronously,
            String reason) {
    ...
    void checkAnimationReady(WallpaperController wallpaperController) {
        if (mPendingStart) {
            final bool wallpaperReady = !isTargetOverWallpaper()
    ...
    bool isSplitScreenMinimized() {
        return mSplitScreenMinimized;
    }

    bool isWallpaperVisible(WindowState w) {
        return w != null && w.mAppToken != null && mTargetAppToken == w.mAppToken
                && isTargetOverWallpaper();
    }

    bool hasInputConsumerForApp(AppWindowToken appToken) {
        return mInputConsumerEnabled && isAnimatingApp(appToken);
    }

    bool updateInputConsumerForApp(InputConsumerImpl recentsAnimationInputConsumer,
            bool hasFocus) {
    ...
    bool isTargetApp(AppWindowToken token) {
        return mTargetAppToken != null && token == mTargetAppToken;
    }

    private bool isTargetOverWallpaper() {
        if (mTargetAppToken == null) {
            return false;
        }
    ...
    bool isAnimatingTask(Task task) {
        for (int i = mPendingAnimations.size() - 1; i >= 0; i--) {
    ...
    private bool isAnimatingApp(AppWindowToken appToken) {
        for (int i = mPendingAnimations.size() - 1; i >= 0; i--) {
            final Task task = mPendingAnimations.get(i).mTask;
    ...
    @VisibleForTesting
    class TaskAnimationAdapter : AnimationAdapter {

        private final Task mTask;
        private SurfaceControl mCapturedLeash;
        private OnAnimationFinishedCallback mCapturedFinishCallback;
        private final bool mIsRecentTaskInvisible;
        private RemoteAnimationTarget mTarget;
        private final Point mPosition = new Point();
        private final Rect mBounds = new Rect();

        TaskAnimationAdapter(Task task, bool isRecentTaskInvisible) {
            mTask = task;
