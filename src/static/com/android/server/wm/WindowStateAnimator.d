...
class WindowStateAnimator {
     ...
     */
    bool mSurfaceDestroyDeferred;

    private bool mDestroyPreservedSurfaceUponRedraw;
    float mShownAlpha = 0;
    float mAlpha = 0;
    float mLastAlpha = 0;
     ...
     */
    private bool mOffsetPositionForStackResize;
    ...
    // once per animation.
    bool mPipAnimationStarted = false;
    ...
    bool finishDrawingLocked() {
        final bool startingWindow =
    ...
    bool commitFinishDrawingLocked() {
    ...
    WindowSurfaceController createSurfaceLocked(int windowType, int ownerUid) {
        ...
        try {
            ...
            final bool isHwAccelerated = (attrs.flags & FLAG_HARDWARE_ACCELERATED) != 0;
     ...
     */
    private bool calculateCrop(Rect clipRect) {
        final WindowState w = mWin;
        ...
        final bool fullscreen = w.fillsDisplay() || (task != null && task.isFullscreen());
        final bool isFreeformResizing =
    ...
    private void applyCrop(Rect clipRect, bool recoveringMemory) {
    ...
    void setSurfaceBoundariesLocked(final bool recoveringMemory) {
        if (mSurfaceController == null) {
            return;
        }
        ...
        bool wasForceScaled = mForceScaleUntilResize;
        bool wasSeamlesslyRotated = w.mSeamlesslyRotated;
        ...
        final bool relayout = !w.mRelayoutCalled || w.mInRelayout;
        if (relayout) {
    ...
    void prepareSurfaceLocked(final bool recoveringMemory) {
        final WindowState w = mWin;
        if (!hasSurface()) {
    ...
    bool setWallpaperOffset(int dx, int dy) {
     ...
     */
    bool tryChangeFormatInPlaceLocked() {
        if (mSurfaceController == null) {
            return false;
        }
        final LayoutParams attrs = mWin.getAttrs();
    ...
    void setOpaqueLocked(bool isOpaque) {
        if (mSurfaceController == null) {
            return;
        }
    ...
    void setSecureLocked(bool isSecure) {
        if (mSurfaceController == null) {
            return;
        }
     ...
     */
    private bool showSurfaceRobustlyLocked() {
        if (mWin.getWindowConfiguration().windowsAreScaleable()) {
            mSurfaceController.forceScaleableInTransaction(true);
        }

        bool shown = mSurfaceController.showRobustlyInTransaction();
        if (!shown)
            return false;
     ...
     */
    bool applyAnimationLocked(int transit, bool isEntrance) {
    ...
    void reclaimSomeSurfaceMemory(String operation, bool secure) {
    ...
    void seamlesslyRotateWindow(SurfaceControl.Transaction t,
        ...
        // and now : off the bottom of the screen (and only halfway across). Essentially we
    ...
    void setOffsetPositionForStackResize(bool offsetPositionForStackResize) {
