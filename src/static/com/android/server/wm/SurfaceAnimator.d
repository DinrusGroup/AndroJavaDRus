...
class SurfaceAnimator {
    ...
    void endDelayingAnimationStart() {
        final bool delayed = mAnimationStartDelayed;
     ...
     *                      to another animator.
     */
    private void cancelAnimation(Transaction t, bool restarting, bool forwardCancel) {
    ...
    private void reset(Transaction t, bool destroyLeash) {
        final SurfaceControl surface = mAnimatable.getSurfaceControl();
        final SurfaceControl parent = mAnimatable.getParentSurfaceControl();

        bool scheduleAnim = false;

        // If the surface was destroyed, we don't care to reparent it back.
        final bool destroy = mLeash != null && surface != null && parent != null;
        if (destroy) {
    ...
    interface Animatable {
         ...
         */
        default bool shouldDeferAnimationFinish(Runnable endDeferFinishCallback) {
            return false;
        }
    }
}
