...
class DragDropController {
    ...
    IBinder performDrag(SurfaceSession session, int callerPid, int callerUid, IWindow window,
        ...
        final bool callbackResult = mCallback.get().prePerformDrag(window, dragToken,
    ...
    void reportDropResult(IWindow window, bool consumed) {
     ...
     */
    void handleMotionEvent(bool keepHandling, float newX, float newY) {
        synchronized (mService.mWindowMap) {
            if (!dragDropActiveLocked()) {
    ...
    private class DragHandler : Handler {
        /**
