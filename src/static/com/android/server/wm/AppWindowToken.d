...
class AppTokenList : ArrayList<AppWindowToken> {
}
 ...
 */
class AppWindowToken : WindowToken : WindowManagerService.AppFreezeListener {
    ...
    bool inPendingTransaction;
    bool allDrawn;
    private bool mLastAllDrawn;

    // Set to true when this app creates a surface while in the middle of an animation. In that
    // case do not clear allDrawn until the animation completes.
    bool deferClearAllDrawn;
    ...
    bool startingDisplayed;
    bool startingMoved;

    // True if the hidden state of this token was forced to false due to a transferred starting
    // window.
    private bool mHiddenSetFromTransferredStartingWindow;
    bool firstWindowDrawn;
    ...
    bool mLaunchTaskBehind;
    bool mEnteringAnimation;

    private bool mAlwaysFocusable;

    bool mAppStopped;
    int mRotationAnimationHint;
    private int mPendingRelaunchCount;

    private bool mLastContainsShowWhenLockedWindow;
    private bool mLastContainsDismissKeyguardWindow;

    ArrayDeque<Rect> mFrozenBounds = new ArrayDeque<>();
    ArrayDeque<Configuration> mFrozenMergedConfig = new ArrayDeque<>();

    private bool mDisablePreviewScreenshots;
     ...
     */
    private bool mCanTurnScreenOn = true;
     ...
     * the WM side.
     */
    private bool mWillCloseOrEnterPip;

    AppWindowToken(WindowManagerService service, IApplicationToken token, bool voiceInteraction,
            DisplayContent dc, long inputDispatchingTimeoutNanos, bool fullscreen,
            bool showForAllUsers, int targetSdk, int orientation, int rotationAnimationHint,
            int configChanges, bool launchTaskBehind, bool alwaysFocusable,
    ...
    AppWindowToken(WindowManagerService service, IApplicationToken token, bool voiceInteraction,
            DisplayContent dc, bool fillsParent) {
    ...
    void updateReportedVisibilityLocked() {
        ...
        bool nowGone = mReportedVisibilityResults.nowGone;

        bool nowDrawn = numInteresting > 0 && numDrawn >= numInteresting;
        bool nowVisible = numInteresting > 0 && numVisible >= numInteresting && !isHidden();
        if (!nowGone) {
    ...
    void setClientHidden(bool hideClient) {
    ...
    bool setVisibility(WindowManager.LayoutParams lp,
            bool visible, int transit, bool performLayout, bool isVoiceInteraction) {
        ...
        if (isHidden() == visible || (isHidden() && mIsExiting) || (visible && waitingForReplacement())) {
            ...
            bool runningAppAnimation = false;
     ...
     */
    WindowState findMainWindow(bool includeStartingApp) {
    ...
    void onRemovedFromDisplay() {
        ...
        bool delayed = setVisibility(null, false, TRANSIT_UNSET, true, mVoiceInteraction);
    ...
    void clearAnimatingFlags() {
        bool wallpaperMightChange = false;
        for (int i = mChildren.size() - 1; i >= 0; i--) {
            final WindowState win = mChildren.get(i);
     ...
     */
    private void destroySurfaces(bool cleanupOnResume) {
    ...
    void setWillReplaceWindows(bool animate) {
     ...
     */
    override
    protected bool isFirstChildWindowGreaterThanSecond(WindowState newWindow,
    ...
    void addWindow(WindowState w) {
        ...
        bool gotReplacementWindow = false;
        for (int i = mChildren.size() - 1; i >= 0; i--) {
    ...
    void stopFreezingScreen(bool unfreezeSurfaceNow, bool force) {
        if (!mFreezingScreen) {
            return;
        }
    ...
    bool transferStartingWindow(IBinder transferFrom) {
    ...
    bool isLastWindow(WindowState win) {
     ...
     */
    private bool allDrawnStatesConsidered() {
        for (int i = mChildren.size() - 1; i >= 0; --i) {
     ...
     */
    bool updateDrawnWindowStates(WindowState w) {
        ...
        bool isInterestingAndDrawn = false;
    ...
    void layoutLetterbox(WindowState winHint) {
        ...
        final bool surfaceReady = w.isDrawnLw()  // Regular case
                || w.mWinAnimator.mSurfaceDestroyDeferred  // The preserved surface is still ready.
                || w.isDragResizeChanged();  // Waiting for relayoutWindow to call preserveSurface.
        final bool needsLetterbox = w.isLetterboxedAppWindow() && fillsParent() && surfaceReady;
        if (needsLetterbox) {
            if (mLetterbox == null) {
    ...
    bool forAllWindowsUnchecked(ToBooleanFunction<WindowState> callback,
    ...
    void setFillsParent(bool fillsParent) {
        mFillsParent = fillsParent;
    }

    bool containsDismissKeyguardWindow() {
    ...
    bool containsShowWhenLockedWindow() {
    ...
    void checkKeyguardFlagsChanged() {
        final bool containsDismissKeyguard = containsDismissKeyguardWindow();
        final bool containsShowWhenLocked = containsShowWhenLockedWindow();
     ...
     */
    void setCanTurnScreenOn(bool canTurnScreenOn) {
     ...
     *         screenshot.
     */
    bool shouldUseAppThemeSnapshot() {
    ...
    private bool shouldAnimate(int transit) {
        final bool isSplitScreenPrimary =
                getWindowingMode() == WINDOWING_MODE_SPLIT_SCREEN_PRIMARY;
        final bool allowSplitScreenPrimaryAnimation = transit != TRANSIT_WALLPAPER_OPEN;
    ...
    bool applyAnimationLocked(WindowManager.LayoutParams lp, int transit, bool enter,
    ...
    private Animation loadAnimation(WindowManager.LayoutParams lp, int transit, bool enter,
            bool isVoiceInteraction) {
        ...
        final bool freeform = win != null && win.inFreeformWindowingMode();
        if (win != null) {
    ...
    override
    public bool shouldDeferAnimationFinish(Runnable endDeferFinishCallback) {
    ...
    bool isWaitingForTransitionStart() {
    ...
    void prepareSurfaces() {
        ...
        // to check super here.
        final bool reallyAnimating = super.isSelfAnimating();
        final bool show = !isHidden() || reallyAnimating;
     ...
     */
    bool isSurfaceShowing() {
        return mLastSurfaceShowing;
    }

    bool isFreezingScreen() {
     ...
     * the given {@code rect}.
     */
    bool isLetterboxOverlappingWith(Rect rect) {
