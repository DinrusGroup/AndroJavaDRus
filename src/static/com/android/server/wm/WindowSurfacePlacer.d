...
class WindowSurfacePlacer {
    ...
    final void performSurfacePlacement(bool force) {
    ...
    int handleAppTransitionReadyLocked() {
        ...
        final bool hasWallpaperTarget = mWallpaperControllerLocked.getWallpaperTarget() != null;
        final bool openingAppHasWallpaper = canBeWallpaperTarget(mService.mOpeningApps)
                && hasWallpaperTarget;
        final bool closingAppHasWallpaper = canBeWallpaperTarget(mService.mClosingApps)
                && hasWallpaperTarget;
        ...
        final bool voiceInteraction = containsVoiceInteraction(mService.mOpeningApps)
    ...
    private bool containsVoiceInteraction(ArraySet<AppWindowToken> apps) {
    ...
    private void handleClosingApps(int transit, LayoutParams animLp, bool voiceInteraction,
    ...
    private bool transitionGoodToGo(int appsCount, SparseIntArray outReasons) {
        ...
        if (!mService.mAppTransition.isTimeout()) {
            ...
            for (int i = 0; i < appsCount; i++) {
                ...
                final bool allDrawn = wtoken.allDrawn && !wtoken.isRelaunching();
            ...
            bool wallpaperReady = !mWallpaperControllerLocked.isWallpaperVisible() ||
    ...
    private int maybeUpdateTransitToWallpaper(int transit, bool openingAppHasWallpaper,
            bool closingAppHasWallpaper) {
        ...
        bool openingCanBeWallpaperTarget = canBeWallpaperTarget(openingApps);
        if (DEBUG_APP_TRANSITIONS) Slog.v(TAG,
    ...
    int maybeUpdateTransitToTranslucentAnim(int transit) {
        final bool taskOrActivity = AppTransition.isTaskTransit(transit)
                || AppTransition.isActivityTransit(transit);
        bool allOpeningVisible = true;
        bool allTranslucentOpeningApps = !mService.mOpeningApps.isEmpty();
        ...
        bool allTranslucentClosingApps = !mService.mClosingApps.isEmpty();
    ...
    private bool canBeWallpaperTarget(ArraySet<AppWindowToken> apps) {
        for (int i = apps.size() - 1; i >= 0; i--) {
     ...
     */
    private AppWindowToken getTopApp(ArraySet<AppWindowToken> apps, bool ignoreHidden) {
