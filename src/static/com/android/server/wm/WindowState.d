...
class WindowState : WindowContainer<WindowState> : WindowManagerPolicy.WindowState {
    ...
    final bool mOwnerCanAddInternalSystemWindow;
    ...
    private bool mIsChildWindow;
    final int mBaseLayer;
    final int mSubLayer;
    final bool mLayoutAttached;
    final bool mIsImWindow;
    final bool mIsWallpaper;
    private final bool mIsFloatingLayer;
    int mSeq;
    bool mEnforceSizeCompat;
    int mViewVisibility;
     ...
     * animation is done.
     */
    bool mPolicyVisibilityAfterAnim = true;
    // overlay window is hidden because the owning app is suspended
    private bool mHiddenWhileSuspended;
    private bool mAppOpVisibility = true;
    bool mPermanentlyHidden; // the window should never be shown again
    // This is a non-system overlay window that is currently force hidden.
    private bool mForceHideNonSystemOverlayWindow;
    bool mAppFreezing;
    bool mHidden = true;    // Used to determine if to show child windows.
    bool mWallpaperVisible;  // for wallpaper, what was last vis report?
    private bool mDragResizing;
    private bool mDragResizingChangeReported = true;
    ...
    private bool mVisibleInsetsChanged;
    ...
    private bool mContentInsetsChanged;
    ...
    private bool mOverscanInsetsChanged;
    ...
    private bool mStableInsetsChanged;
    ...
    private bool mOutsetsChanged = false;
    ...
    private bool mDisplayCutoutChanged;

    /**
     * Set to true if we are waiting for this window to receive its
     * given internal insets before laying out other windows based on it.
     */
    bool mGivenInsetsPending;
    ...
    private bool mFrameSizeChanged = false;
    ...
    private bool mParentFrameWasClippedByDisplayCutout;
     ...
     */
    private bool mOrientationChanging;
     ...
     */
    private bool mOrientationChangeTimedOut;
     ...
     */
    bool mReportOrientationChanged;
     ...
     * TODO: We should either have different bools for the removal reason or use a bit-field.
     */
    bool mWindowRemovalAllowed;
    ...
    private bool mWasVisibleBeforeClientHidden;

    // This window will be replaced due to relaunch. This allows window manager
    // to differentiate between simple removal of a window and replacement. In the latter case it
    // will preserve the old window until the new one is drawn.
    bool mWillReplaceWindow = false;
    // If true, the replaced window was already requested to be removed.
    private bool mReplacingRemoveRequested = false;
    // Whether the replacement of the window should trigger app transition animation.
    private bool mAnimateReplacingWindow = false;
    ...
    // the new window.
    bool mSkipEnterAnimationForSeamlessReplacement = false;
    // Whether this window is being moved via the resize API
    private bool mMovedByResize;
     ...
     */
    private bool mDrawnStateEvaluated;
    ...
    WindowState(WindowManagerService service, Session s, IWindow c, WindowToken token,
            ...
            int viewVisibility, int ownerId, bool ownerCanAddInternalSystemWindow) {
     ...
     */
    void setDrawnStateEvaluated(bool evaluated) {
    ...
    override
    public bool canAddInternalSystemWindow() {
        return mOwnerCanAddInternalSystemWindow;
    }

    override
    public bool canAcquireSleepToken() {
    ...
    public void computeFrameLw(Rect parentFrame, Rect displayFrame, Rect overscanFrame,
            ...
            bool parentFrameWasClippedByDisplayCutout) {
        ...
        final bool inFullscreenContainer = inFullscreenContainer();
        final bool windowsAreFloating = task != null && task.isFloating();
        ...
        final bool hasOutsets = outsetFrame != null;
        if (hasOutsets) {
        ...
        if (mAttrs.type == TYPE_DOCK_DIVIDER) {
            ...
            // non-fullscreen mode.
            bool overrideRightInset = !windowsAreFloating && !inFullscreenContainer
                    && mFrame.right > mTmpRect.right;
            bool overrideBottomInset = !windowsAreFloating && !inFullscreenContainer
    ...
    override
    public bool getGivenInsetsPendingLw() {
    ...
    override
    public bool getNeedsMenuLw(WindowManagerPolicy.WindowState bottom) {
    ...
    void updateResizingWindowIfNeeded() {
        ...
        bool configChanged = isConfigChanged();
        if (DEBUG_CONFIGURATION && configChanged) {
            Slog.v(TAG_WM, "Win " + this + " config changed: " + getConfiguration());
        }

        final bool dragResizingChanged = isDragResizeChanged()
    ...
    void getVisibleBounds(Rect bounds) {
        final Task task = getTask();
        bool intersectWithStackBounds = task != null && task.cropWindowsToStackBounds();
        bounds.setEmpty();
        mTmpRect.setEmpty();
    ...
    override
    public bool hasAppShownWindows() {
        return mAppToken != null && (mAppToken.firstWindowDrawn || mAppToken.startingDisplayed);
    }

    bool isIdentityMatrix(float dsdx, float dtdx, float dsdy, float dtdy) {
     ...
     *         otherwise.
     */
    bool wouldBeVisibleIfPolicyIgnored() {
        return mHasSurface && !isParentWindowHidden()
                && !mAnimatingExit && !mDestroying && (!mIsWallpaper || mWallpaperVisible);
    }

    override
    public bool isVisibleLw() {
        return isVisible();
    }
     ...
     */
    bool mightAffectAllDrawn() {
        final bool isAppType = mWinAnimator.mAttrType == TYPE_BASE_APPLICATION
    ...
    override
    public bool canAffectSystemUiFlags() {
        final bool translucent = mAttrs.alpha == 0.0f;
        if (translucent) {
            return false;
        }
        if (mAppToken == null) {
            final bool shown = mWinAnimator.getShown();
            final bool exiting = mAnimatingExit || mDestroying;
            return shown && !exiting;
        } else {
            final Task task = getTask();
            final bool canFromTask = task != null && task.canAffectSystemUiFlags();
     ...
     */
    override
    public bool isAnimatingLw() {
        return isAnimating();
    }

    override
    public bool isGoneForLayoutLw() {
        final AppWindowToken atoken = mAppToken;
    ...
    bool onAppVisibilityChanged(bool visible, bool runningAppAnimation) {
        bool changed = false;
        ...
        final bool isVisibleNow = isVisibleNow();
    ...
    void handleWindowMovedIfNeeded() {
        ...
        final Task task = getTask();
        final bool adjustedForMinimizedDockOrIme = task != null
    ...
    override
    void forceWindowsScaleableInTransaction(bool force) {
    ...
    private void removeIfPossible(bool keepVisibleDeadWindow) {
        mWindowRemovalAllowed = true;
        if (DEBUG_ADD_REMOVE) Slog.v(TAG,
                "removeIfPossible: " + this + " callers=" + Debug.getCallers(5));

        final bool startingWindow = mAttrs.type == TYPE_APPLICATION_STARTING;
        ...
        try {
            ...
            if (mHasSurface && mToken.okToAnimate()) {
                ...
                final bool isAnimating = mWinAnimator.isAnimationSet()
                        && (mAppToken == null || !mAppToken.isWaitingForTransitionStart());
                final bool lastWindowIsStartingWindow = startingWindow && mAppToken != null
    ...
    private void setupWindowForRemoveOnExit() {
        ...
        final bool focusChanged = mService.updateFocusedWindowLocked(
    ...
    bool canBeImeTarget() {
        ...
        final bool windowsAreFocusable = mAppToken == null || mAppToken.windowsAreFocusable();
    ...
    private final class DeadWindowEventReceiver : InputEventReceiver {
    ...
    bool removeReplacedWindowIfNeeded(WindowState replacement) {
    ...
    bool setReplacementWindowIfNeeded(WindowState replacementCandidate) {
        bool replacementSet = false;
    ...
    int getTouchableRegion(Region region, int flags) {
        final bool modal = (flags & (FLAG_NOT_TOUCH_MODAL | FLAG_NOT_FOCUSABLE)) == 0;
    ...
    void prepareWindowToDisplayDuringRelayout(bool wasVisible) {
        // We need to turn on screen regardless of visibility.
        bool hasTurnScreenOnFlag = (mAttrs.flags & FLAG_TURN_SCREEN_ON) != 0;
        bool allowTheaterMode =
                mService.mAllowTheaterModeWakeFromLayout || Settings.Global.getInt(
                        ...
                        == 0;
        bool canTurnScreenOn = mAppToken == null || mAppToken.canTurnScreenOn();
    ...
    void setWindowScale(int requestedWidth, int requestedHeight) {
        final bool scaledWindow = (mAttrs.flags & FLAG_SCALED) != 0;
    ...
    private class DeathRecipient : IBinder.DeathRecipient {
        override
        public void binderDied() {
            try {
                bool resetSplitScreenResizing = false;
     ...
     * interacts with it.
     */
    private bool shouldKeepVisibleDeadAppWindow() {
    ...
    bool canReceiveTouchInput() {
        return mAppToken != null && mAppToken.getTask() != null
                && mAppToken.getTask().mStack.shouldIgnoreInput();
    }

    override
    public bool hasDrawnLw() {
        return mWinAnimator.mDrawState == WindowStateAnimator.HAS_DRAWN;
    }

    override
    public bool showLw(bool doAnimation) {
        return showLw(doAnimation, true);
    }

    bool showLw(bool doAnimation, bool requestAnim) {
        if (isHiddenFromUserLocked()) {
            return false;
        }
        if (!mAppOpVisibility) {
    ...
    override
    public bool hideLw(bool doAnimation) {
        return hideLw(doAnimation, true);
    }

    bool hideLw(bool doAnimation, bool requestAnim) {
        if (doAnimation) {
            if (!mToken.okToAnimate()) {
                doAnimation = false;
            }
        }
        bool current = doAnimation ? mPolicyVisibilityAfterAnim : mPolicyVisibility;
        if (!current) {
            // Already hiding.
            return false;
        }
    ...
    void setForceHideNonSystemOverlayWindowIfNeeded(bool forceHide) {
    ...
    void setHiddenWhileSuspended(bool hide) {
        if (mOwnerCanAddInternalSystemWindow
                || (!isSystemAlertWindowType(mAttrs.type) && mAttrs.type != TYPE_TOAST)) {
            return;
        }
    ...
    private void setAppOpVisibilityLw(bool state) {
    ...
    void sendAppVisibilityToClients() {
        ...
        final bool clientHidden = mAppToken.isClientHidden();
    ...
    bool onStopFreezingScreen() {
        bool unfrozeWindows = false;
        for (int i = mChildren.size() - 1; i >= 0; --i) {
            final WindowState c = mChildren.get(i);
    ...
    bool destroySurface(bool cleanupOnResume, bool appStopped) {
    ...
    void setShowToOwnerOnlyLocked(bool showToOwnerOnly) {
        mShowToOwnerOnly = showToOwnerOnly;
    }

    private bool isHiddenFromUserLocked() {
     ...
     */
    void reportFocusChangedSerialized(bool focused, bool inTouchMode) {
        try {
    ...
    void reportResized() {
        ...
        try {
            ...
            final bool reportDraw = mWinAnimator.mDrawState == DRAW_PENDING;
            final bool reportOrientation = mReportOrientationChanged;
            final int displayId = getDisplayId();
    ...
    Rect getBackdropFrame(Rect frame) {
        ...
        bool resizing = isDragResizing() || isDragResizeChanged();
    ...
    private void dispatchResized(Rect frame, Rect overscanInsets, Rect contentInsets,
            ...
            MergedConfiguration mergedConfiguration, bool reportOrientation, int displayId,
            DisplayCutout displayCutout)
            throws RemoteException {
        final bool forceRelayout = isDragResizeChanged() || reportOrientation;
    ...
    private bool inFullscreenContainer() {
     ...
     */
    private bool frameCoversEntireAppTokenBounds() {
    ...
    override
    public bool isLetterboxedOverlappingWith(Rect rect) {
        return mAppToken != null && mAppToken.isLetterboxOverlappingWith(rect);
    }

    bool isDragResizeChanged() {
     ...
     */
    private bool isDragResizingChangeReported() {
    ...
    private bool computeDragResizing() {
        final Task task = getTask();
        if (task == null) {
            return false;
        }
    ...
    void setDragResizing() {
        final bool resizing = computeDragResizing();
    ...
    @CallSuper
    override
    public void writeToProto(ProtoOutputStream proto, long fieldId, bool trim) {
    ...
    override
    void dump(PrintWriter pw, String prefix, bool dumpAll) {
        final TaskStack stack = getStack();
    ...
    void applyGravityAndUpdateFrame(Rect containingFrame, Rect displayFrame) {
        ...
        final Task task = getTask();
        final bool inNonFullscreenContainer = !inFullscreenContainer();
        final bool noLimits = (mAttrs.flags & FLAG_LAYOUT_NO_LIMITS) != 0;
        ...
        // the display.
        final bool fitToDisplay = (task == null || !inNonFullscreenContainer)
     ...
     */
    bool hideNonSystemOverlayWindowsWhenVisible() {
    ...
    bool isParentWindowHidden() {
        final WindowState parent = getParentWindow();
        return parent != null && parent.mHidden;
    }

    void setWillReplaceWindow(bool animate) {
        for (int i = mChildren.size() - 1; i >= 0; i--) {
            final WindowState c = mChildren.get(i);
    ...
    bool shouldBeReplacedWithChildren() {
    ...
    override
    public bool isInputMethodWindow() {
        return mIsImWindow;
    }
    ...
    WindowInfo getWindowInfo() {
        ...
        final bool isPanelWindow = (mAttrs.type >= WindowManager.LayoutParams.FIRST_SUB_WINDOW)
        ...
        final bool isAccessibilityOverlay =
    ...
    override
    bool forAllWindows(ToBooleanFunction<WindowState> callback, bool traverseTopToBottom) {
    ...
    private bool forAllWindowBottomToTop(ToBooleanFunction<WindowState> callback) {
    ...
    private bool forAllWindowTopToBottom(ToBooleanFunction<WindowState> callback) {
    ...
    private bool applyImeWindowsIfNeeded(ToBooleanFunction<WindowState> callback,
            bool traverseTopToBottom) {
    ...
    private bool applyInOrderWithImeWindows(ToBooleanFunction<WindowState> callback,
            bool traverseTopToBottom) {
        if (traverseTopToBottom) {
     ...
     */
    @VisibleForTesting
    bool isSelfOrAncestorWindowAnimatingExit() {
    ...
    void onExitAnimationDone() {
        ...
        final bool hasSurface = mWinAnimator.hasSurface();
    ...
    void hideWallpaperWindow(bool wasDeferred, String reason) {
     ...
     */
    void dispatchWallpaperVisibility(final bool visible) {
        final bool hideAllowed =
    ...
    bool hasVisibleNotDrawnWallpaper() {
    ...
    int relayoutVisibleWindow(int result, int attrChanges, int oldVisibility) {
        final bool wasVisible = isVisibleLw();
        ...
        final bool freeformResizing = isDragResizing()
                && getResizeMode() == DRAG_RESIZE_MODE_FREEFORM;
        final bool dockedResizing = isDragResizing()
    ...
    private static final class WindowId : IWindowId.Stub {
    ...
    bool needsRelativeLayeringToIme() {
        ...
        if (isChildWindow()) {
            ...
            bool inTokenWithAndAboveImeTarget = imeTarget != null && imeTarget != this
    ...
    void updateTapExcludeRegion(int regionId, int left, int top, int width, int height) {
        ...
        final bool isAppFocusedOnDisplay = mService.mFocusedApp != null
    ...
    private final class MoveAnimationSpec : AnimationSpec {
