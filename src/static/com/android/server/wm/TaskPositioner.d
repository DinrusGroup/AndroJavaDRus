...
class TaskPositioner {
    private static final bool DEBUG_ORIENTATION_VIOLATIONS = false;
    ...
    private bool mPreserveOrientation;
    private bool mStartOrientationWasLandscape;
    ...
    private final class WindowPositionerEventReceiver : BatchedInputEventReceiver {
        ...
        public void onInputEvent(InputEvent event, int displayId) {
            ...
            try {
                ...
                if (mDragEnded) {
                    final bool wasResizing = mResizing;
                    synchronized (mService.mWindowMap) {
                        endDragLocked();
                        mTask.getDimBounds(mTmpRect);
                    }
    ...
    void startDrag(WindowState win, bool resize, bool preserveOrientation, float startX,
                   float startY) {
        if (DEBUG_TASK_POSITIONING) {
    ...
    @VisibleForTesting
    void startDrag(bool resize, bool preserveOrientation,
    ...
    private bool notifyMoveLocked(float x, float y) {
        if (DEBUG_TASK_POSITIONING) {
    ...
    void resizeDrag(float x, float y) {
        ...
        if (mPreserveOrientation && ((mStartOrientationWasLandscape && aspect < MIN_ASPECT)
            ...
            // do the opposite.
            final bool grows = width > (right - left) || height > (bottom - top);
