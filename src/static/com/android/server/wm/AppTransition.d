...
public class AppTransition : Dump {
    ...
    private bool mNextAppTransitionScaleUp;
    ...
    private bool mNextAppTransitionAnimationsSpecsPending;
    ...
    private bool mLastHadClipReveal;

    private final bool mGridLayoutRecentsEnabled;
    private final bool mLowRamRecentsEnabled;
    ...
    bool isTransitionSet() {
        return mNextAppTransition != TRANSIT_UNSET;
    }

    bool isTransitionEqual(@TransitionType int transit) {
    ...
    bool isNextThumbnailTransitionAspectScaled() {
     ...
     */
    bool isFetchingAppTransitionsSpecs() {
    ...
    private bool needsBoosting() {
        final bool recentsAnimRunning = mService.getRecentsAnimationController() != null;
        return mNextAppTransition != TRANSIT_UNSET
    ...
    private Animation createScaleUpAnimationLocked(int transit, bool enter,
     ...
     */
    private long calculateClipRevealTransitionDuration(bool cutOff, float translationX,
    ...
    private Animation createClipRevealAnimationLocked(int transit, bool enter, Rect appFrame,
            Rect displayFrame) {
     ...
     */
    int getThumbnailTransitionState(bool enter) {
        if (enter) {
    ...
    Animation createAspectScaledThumbnailEnterExitAnimationLocked(int thumbTransitState,
            ...
            @Nullable Rect surfaceInsets, @Nullable Rect stableInsets, bool freeform,
            int taskId) {
        Animation a;
        final int appWidth = containingFrame.width();
        final int appHeight = containingFrame.height();
        ...
        switch (thumbTransitState) {
            ...
            case THUMBNAIL_TRANSITION_EXIT_SCALE_DOWN: {
                final bool scaleUp = thumbTransitState == THUMBNAIL_TRANSITION_ENTER_SCALE_UP;
                if (freeform && scaleUp) {
    ...
    private AnimationSet createAspectScaledThumbnailFreeformAnimationLocked(Rect sourceFrame,
            Rect destFrame, @Nullable Rect surfaceInsets, bool enter) {
     ...
     *                      bigger.
     */
    Animation loadAnimation(LayoutParams lp, int transit, bool enter, int uiMode,
            int orientation, Rect frame, Rect displayFrame, Rect insets,
            @Nullable Rect surfaceInsets, @Nullable Rect stableInsets, bool isVoiceInteraction,
            bool freeform, int taskId) {
        Animation a;
    ...
    void overridePendingAppTransitionThumb(GraphicBuffer srcThumb, int startX, int startY,
                                           IRemoteCallback startedCallback, bool scaleUp) {
        if (canOverridePendingAppTransition()) {
            clear();
    ...
    void overridePendingAppTransitionAspectScaledThumb(GraphicBuffer srcThumb, int startX, int startY,
            int targetWidth, int targetHeight, IRemoteCallback startedCallback, bool scaleUp) {
        if (canOverridePendingAppTransition()) {
            clear();
    ...
    private bool canOverridePendingAppTransition() {
     ...
     *         already running
     */
    bool prepareAppTransitionLocked(@TransitionType int transit, bool alwaysKeepCurrent,
        ...
        if (DEBUG_APP_TRANSITIONS) Slog.v(TAG, "Prepare app transition:"
                ...
                + " Callers=" + Debug.getCallers(3));
        final bool allowSetCrashing = !isKeyguardTransit(mNextAppTransition)
     ...
     */
    public static bool isKeyguardGoingAwayTransit(int transit) {
        return transit == TRANSIT_KEYGUARD_GOING_AWAY
                || transit == TRANSIT_KEYGUARD_GOING_AWAY_ON_WALLPAPER;
    }

    private static bool isKeyguardTransit(int transit) {
        return isKeyguardGoingAwayTransit(transit) || transit == TRANSIT_KEYGUARD_OCCLUDE
                || transit == TRANSIT_KEYGUARD_UNOCCLUDE;
    }

    static bool isTaskTransit(int transit) {
    ...
    private static  bool isTaskOpenTransit(int transit) {
        return transit == TRANSIT_TASK_OPEN
                || transit == TRANSIT_TASK_OPEN_BEHIND
                || transit == TRANSIT_TASK_TO_FRONT;
    }

    static bool isActivityTransit(int transit) {
     ...
     */
    private bool shouldScaleDownThumbnailTransition(int uiMode, int orientation) {
