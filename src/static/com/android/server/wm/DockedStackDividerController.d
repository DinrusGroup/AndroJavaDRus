...
public class DockedStackDividerController {
    ...
    void reevaluateVisibility(bool force) {
        if (mWindow == null) {
            return;
        }
    ...
    void setAdjustedForIme(
            bool adjustedForIme, bool adjustedForDivider,
            bool animate, WindowState imeWin, int imeHeight) {
    ...
    private void notifyDockedDividerVisibilityChanged(bool visible) {
     ...
     */
    bool isImeHideRequested() {
        return mImeHideRequested;
    }

    private void notifyDockedStackMinimizedChanged(bool minimizedDock, bool animate,
            bool isHomeStackResizable) {
        long animDuration = 0;
        if (animate) {
    ...
    private void notifyAdjustedForImeChanged(bool adjustedForIme, long animDuration) {
        final int size = mDockedStackListeners.beginBroadcast();
        for (int i = 0; i < size; ++i) {
            final IDockedStackListener listener = mDockedStackListeners.getBroadcastItem(i);
            try {
     ...
     */
    void setResizeDimLayer(bool visible, int targetWindowingMode, float alpha) {
        ...
        bool visibleAndValid = visible && stack != null && dockedStack != null;
    ...
    void notifyAppTransitionStarting(ArraySet<AppWindowToken> openingApps, int appTransition) {
        final bool wasMinimized = mMinimizedDock;
     ...
     */
    private bool containsAppInDockedStack(ArraySet<AppWindowToken> apps) {
    ...
    bool isMinimizedDock() {
        return mMinimizedDock;
    }

    void checkMinimizeChanged(bool animate) {
        ...
        final bool minimizedForRecentsAnimation = recentsAnim != null &&
                recentsAnim.isSplitScreenMinimized();
        bool homeVisible = homeTask.getTopVisibleAppToken() != null;
    ...
    private bool isWithinDisplay(Task task) {
     ...
     */
    private void setMinimizedDockedStack(bool minimizedDock, bool animate) {
        final bool wasMinimized = mMinimizedDock;
        ...
        final bool imeChanged = clearImeAdjustAnimation();
        bool minimizedChange = false;
    ...
    private bool clearImeAdjustAnimation() {
        final bool changed = mDisplayContent.clearImeAdjustAnimation();
    ...
    private void startImeAdjustAnimation(
            bool adjustedForIme, bool adjustedForDivider, WindowState imeWin) {
    ...
    private bool animateForIme(long now) {
    ...
    private bool animateForMinimizedDockedStack(long now) {
        final TaskStack stack = mDisplayContent.getSplitScreenPrimaryStackIgnoringVisibility();
