 ...
 * @hide
 */
public class AudioService : IAudioService.Stub
    ...
    /** Debug audio mode */
    protected static final bool DEBUG_MODE = Log.isLoggable(TAG + ".MOD", Log.DEBUG);

    /** Debug audio policy feature */
    protected static final bool DEBUG_AP = Log.isLoggable(TAG + ".AP", Log.DEBUG);

    /** Debug volumes */
    protected static final bool DEBUG_VOL = Log.isLoggable(TAG + ".VOL", Log.DEBUG);

    /** debug calls to devices APIs */
    protected static final bool DEBUG_DEVICES = Log.isLoggable(TAG + ".DEVICES", Log.DEBUG);
    ...
    private final bool mIsSingleVolume;

    private bool isPlatformVoice() {
        return mPlatformType == AudioSystem.PLATFORM_VOICE;
    }

    private bool isPlatformTelevision() {
        return mPlatformType == AudioSystem.PLATFORM_TELEVISION;
    }

    private bool isPlatformAutomotive() {
   ...
   /** Maximum volume index values for audio streams */
    ...
    private final bool mUseFixedVolume;
   ...
   protected static final int DEFAULT_VOL_STREAM_NO_PLAYBACK = AudioSystem.STREAM_MUSIC;
    ...
    // Is there a vibrator
    private final bool mHasVibrator;
    ...
    private bool mUserSwitchedReceived;
    ...
    private bool mUserSelectedVolumeControlStream = false;
    ...
    private bool mBluetoothA2dpEnabled;
    ...
    private final bool mMonitorRotation;

    private bool mDockAudioMediaEnabled = true;
    ...
    private bool mAvrcpAbsVolSupported = false;
    ...
    private bool mSurroundModeChanged;
    ...
    final private IUidObserver mUidObserver = new IUidObserver.Stub() {
        ...
        override public void onUidGone(int uid, bool disabled) {
        ...
        override public void onUidIdle(int uid, bool disabled) {
        }

        override public void onUidCachedChanged(int uid, bool cached) {
            disableAudioForUid(cached, uid);
        }

        private void disableAudioForUid(bool disable, int uid) {
    ...
    public AudioService(Context context) {
        ...
        bool cameraSoundForced = readCameraSoundForced();
    ...
    private void onDispatchAudioServerStateChange(bool state) {
    ...
    private void updateStreamVolumeAlias(bool updateVolumes, String caller) {
        int dtmfStreamAlias;
    ...
    {
        final bool masterMono = System.getIntForUser(
    ...
    private void sendEnabledSurroundFormats(ContentResolver cr, bool forceUpdate) {
    ...
    private void onEnableSurroundFormats(ArrayList<Integer> enabledSurroundFormats) {
        ...
        for (int surroundFormat : AudioFormat.SURROUND_SOUND_ENCODING) {
            bool enabled = enabledSurroundFormats.contains(surroundFormat);
    ...
    private void readUserRestrictions() {
        ...
        bool microphoneMute = mUserManagerInternal.getUserRestriction(
    ...
    private void adjustSuggestedStreamVolume(int direction, int suggestedStreamType, int flags,
        ...
        final bool isMute = isMuteAdjust(direction);
    ...
    protected void adjustStreamVolume(int streamType, int direction, int flags,
        ...
        bool isMuteAdjust = isMuteAdjust(direction);
    ...
    private bool isAndroidNPlus(String caller) {
        try {
    ...
    private bool wouldToggleZenMode(int newMode) {
    ...
    private bool canChangeAccessibilityVolume() {
    ...
    private bool volumeAdjustmentAllowedByDnd(int streamTypeAlias, int flags) {
    ...
    private class ForceControlStreamClient : IBinder.DeathRecipient {
    ...
    // UI update and Broadcast Intent
    private void sendMasterMuteUpdate(bool muted, int flags) {
        mVolumeController.postMasterMuteChanged(updateFlagsForSystemAudio(flags));
        broadcastMasterMuteStatus(muted);
    }

    private void broadcastMasterMuteStatus(bool muted) {
    ...
    private void setSystemAudioMute(bool state) {
    ...
    public bool isStreamMute(int streamType) {
    ...
    private class RmtSbmxFullVolDeathHandler : IBinder.DeathRecipient {
    ...
    private bool discardRmtSbmxFullVolDeathHandlerFor(IBinder cb) {
    ...
    private bool hasRmtSbmxFullVolDeathHandlerFor(IBinder cb) {
        Iterator<RmtSbmxFullVolDeathHandler> it = mRmtSbmxFullVolDeathHandlers.iterator();
        while (it.hasNext()) {
    ...
    public void forceRemoteSubmixFullVolume(bool startForcing, IBinder cb) {
        if (cb == null) {
            return;
        }
        if ((PackageManager.PERMISSION_GRANTED != mContext.checkCallingOrSelfPermission(
    ...
    private void setMasterMuteInternal(bool mute, int flags, String callingPackage, int uid,
            int userId) {
    ...
    private void setMasterMuteInternalNoCallerCheck(bool mute, int flags, int userId) {
        if (DEBUG_VOL) {
    ...
    public void setMasterMute(bool mute, int flags, String callingPackage, int userId) {
    ...
    /** @see AudioManager#setMicrophoneMute(bool) */
    override
    public void setMicrophoneMute(bool on, String callingPackage, int userId) {
        // If we are being called by the system check for user we are going to change
        // so we handle user restrictions correctly.
    ...
    private void setMicrophoneMuteNoCallerCheck(bool on, int userId) {
        if (DEBUG_VOL) {
            Log.d(TAG, String.format("Mic mute %s, user=%d", on, userId));
        }
        // only mute for the current user
        if (getCurrentUserId() == userId) {
            final bool currentMute = AudioSystem.isMicrophoneMuted();
    ...
    public bool isValidRingerMode(int ringerMode) {
    ...
    private bool maybeVibrate(VibrationEffect effect) {
        if (!mHasVibrator) {
            return false;
        }
    ...
    private void setRingerMode(int ringerMode, String caller, bool external) {
        if (mUseFixedVolume || mIsSingleVolume) {
            return;
        }
    ...
    private void muteRingerModeStreams() {
        ...
        final bool ringerModeMute = ringerMode == AudioManager.RINGER_MODE_VIBRATE
                || ringerMode == AudioManager.RINGER_MODE_SILENT;
        final bool shouldRingSco = ringerMode == AudioManager.RINGER_MODE_VIBRATE
                && isBluetoothScoOn();
        ...
        for (int streamType = numStreamTypes - 1; streamType >= 0; streamType--) {
            final bool isMuted = isStreamMutedByRingerOrZenMode(streamType);
            final bool muteAllowedBySco =
                    !(shouldRingSco && streamType == AudioSystem.STREAM_RING);
            final bool shouldZenMute = shouldZenMuteStream(streamType);
            final bool shouldMute = shouldZenMute || (ringerModeMute
    ...
    private bool isAlarm(int streamType) {
        return streamType == AudioSystem.STREAM_ALARM;
    }

    private bool isNotificationOrRinger(int streamType) {
        return streamType == AudioSystem.STREAM_NOTIFICATION
                || streamType == AudioSystem.STREAM_RING;
    }

    private bool isMedia(int streamType) {
        return streamType == AudioSystem.STREAM_MUSIC;
    }


    private bool isSystem(int streamType) {
        return streamType == AudioSystem.STREAM_SYSTEM;
    }

    private void setRingerModeInt(int ringerMode, bool persist) {
        final bool change;
        synchronized(mSettingsLock) {
    ...
    public bool shouldVibrate(int vibrateType) {
    ...
    private class SetModeDeathHandler : IBinder.DeathRecipient {
        private IBinder mCb; // To be notified of client's death
        private int mPid;
    ...
    class SoundPoolListenerThread : Thread {
    ...
    private void readAudioSettings(bool userSwitch) {
    ...
    /** @see AudioManager#setSpeakerphoneOn(bool) */
    ...
    /** @see AudioManager#setBluetoothScoOn(bool) */
    ...
    public void setBluetoothScoOnInt(bool on, String eventSource) {
    ...
    /** @see AudioManager#setBluetoothA2dpOn(bool) */
    public void setBluetoothA2dpOn(bool on) {
        // for logging only
    ...
    private class ScoClient : IBinder.DeathRecipient {
        private IBinder mCb; // To be notified of client's death
        ...
        public void clearCount(bool stopSco) {
            synchronized(mScoClients) {
                if (mStartcount != 0) {
                    try {
                        mCb.unlinkToDeath(this, 0);
    ...
    private ScoClient getScoClient(IBinder cb, bool create) {
        synchronized(mScoClients) {
    ...
    public void clearAllScoClients(int exceptPid, bool stopSco) {
        synchronized(mScoClients) {
    ...
    private bool getBluetoothHeadset() {
        bool result = false;
        BluetoothAdapter adapter = BluetoothAdapter.getDefaultAdapter();
        if (adapter != null) {
    ...
    private static bool disconnectBluetoothScoAudioHelper(BluetoothHeadset bluetoothHeadset,
    ...
    private static bool connectBluetoothScoAudioHelper(BluetoothHeadset bluetoothHeadset,
            BluetoothDevice device, int scoAudioMode) {
        switch (scoAudioMode) {
            case SCO_MODE_RAW:
    ...
    private bool handleBtScoActiveDeviceChange(BluetoothDevice btDevice, bool isActive) {
        if (btDevice == null) {
            return true;
        }
    ...
    private void onConfigureSafeVolume(bool force, String caller) {
        synchronized (mSafeMediaVolumeState) {
            ...
            if ((mMcc != mcc) || ((mMcc == 0) && force)) {
                ...
                bool safeMediaVolumeEnabled =
                        SystemProperties.getBoolean("audio.safemedia.force", false)
                        || mContext.getResources().getBoolean(
                                com.android.internal.R.bool.config_safe_media_volume_enabled);

                bool safeMediaVolumeBypass =
                        SystemProperties.getBoolean("audio.safemedia.bypass", false);
     ...
     */
    private int checkForRingerModeChange(int oldIndex, int direction, int step, bool isMuted,
    ...
    override
    public bool isStreamAffectedByRingerMode(int streamType) {
        return (mRingerModeAffectedStreams & (1 << streamType)) != 0;
    }

    private bool shouldZenMuteStream(int streamType) {
        ...
        final bool muteAlarms = (zenPolicy.priorityCategories
                & NotificationManager.Policy.PRIORITY_CATEGORY_ALARMS) == 0;
        final bool muteMedia = (zenPolicy.priorityCategories
                & NotificationManager.Policy.PRIORITY_CATEGORY_MEDIA) == 0;
        final bool muteSystem = (zenPolicy.priorityCategories
                & NotificationManager.Policy.PRIORITY_CATEGORY_SYSTEM) == 0;
        final bool muteNotificationAndRing = ZenModeConfig
    ...
    private bool isStreamMutedByRingerOrZenMode(int streamType) {
     ...
     */
    private bool updateZenModeAffectedStreams() {
    ...
    @GuardedBy("mSettingsLock")
    private bool updateRingerAndZenModeAffectedStreams() {
        bool updatedZenModeAffectedStreams = updateZenModeAffectedStreams();
    ...
    override
    public bool isStreamAffectedByMute(int streamType) {
     ...
     */
    private bool wasStreamActiveRecently(int stream, int delay_ms) {
    ...
    bool checkAudioSettingsPermission(String method) {
    ...
    private int getDevicesForStream(int stream, bool checkOthers) {
    ...
    public int setBluetoothHearingAidDeviceConnectionState(
            BluetoothDevice device, int state, bool suppressNoisyIntent,
            int musicDevice)
    {
        int delay;
        synchronized (mConnectedDevices) {
    ...
    public int setBluetoothA2dpDeviceConnectionStateSuppressNoisyIntent(BluetoothDevice device,
                int state, int profile, bool suppressNoisyIntent, int a2dpVolume)
    {
    ...
    public int setBluetoothA2dpDeviceConnectionStateInt(
            BluetoothDevice device, int state, int profile, bool suppressNoisyIntent,
    ...
    public class VolumeStreamState {
        ...
        public int observeDevicesForStream_syncVSS(bool checkOthers) {
        ...
        private bool hasValidSettingsName() {
        ...
        public bool adjustIndex(int deltaIndex, int device, String caller) {
            return setIndex(getIndex(device) + deltaIndex, device, caller);
        }

        public bool setIndex(int index, int device, String caller) {
            bool changed;
            int oldIndex;
            synchronized (mSettingsLock) {
                synchronized (VolumeStreamState.class) {
                    ...
                    final bool isCurrentDevice = (device == getDeviceForStream(mStreamType));
        ...
        public bool hasIndexForDevice(int device) {
            synchronized (VolumeStreamState.class) {
    ...
    private class AudioSystemThread : Thread {
        AudioSystemThread() {
            super("AudioService");
        }
    ...
    private class AudioHandler : Handler {
        ...
        private bool onLoadSoundEffects() {
            int status;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_LOAD_SOUND_EFFECTS:
                    ...
                    bool loaded = onLoadSoundEffects();
                    if (msg.obj != null) {
    ...
    private class SettingsObserver : ContentObserver {
    ...
    // must be called synchronized on mConnectedDevices
    private bool hasScheduledA2dpDockTimeout() {
    ...
    private void onSetA2dpSinkConnectionState(BluetoothDevice btDevice, int state, int a2dpVolume)
    {
        ...
        synchronized (mConnectedDevices) {
            ...
            bool isConnected = deviceSpec != null;
    ...
    public void avrcpSupportsAbsoluteVolume(String address, bool support) {
    ...
    private bool handleDeviceConnection(bool connect, int device, String address,
            String deviceName) {
        if (DEBUG_DEVICES) {
     ...
     */
    private bool hasMediaDynamicPolicy() {
        synchronized (mAudioPolicies) {
     ...
     */
    private class AudioServiceBroadcastReceiver : BroadcastReceiver {
        override
    ...
    private class AudioServiceUserRestrictionsListener : UserRestrictionsListener {
     ...
     */
    private bool forceFocusDuckingForAccessibility(@Nullable AudioAttributes aa,
            int request, int uid) {
    ...
    //==========================================================================================
    private bool readCameraSoundForced() {
    ...
    private void handleConfigurationChanged(Context context) {
        try {
            ...
            bool cameraSoundForced = readCameraSoundForced();
            synchronized (mSettingsLock) {
                final bool cameraSoundForcedChanged = (cameraSoundForced != mCameraSoundForced);
    ...
    // Must be called synchronized on mConnectedDevices
    public void setBluetoothA2dpOnInt(bool on, String eventSource) {
        synchronized (mBluetoothA2dpEnabledLock) {
            mBluetoothA2dpEnabled = on;
    ...
    private void setSafeMediaVolumeEnabled(bool on, String caller) {
        synchronized (mSafeMediaVolumeState) {
    ...
    private bool checkSafeMediaVolume(int streamType, int index, int device) {
        synchronized (mSafeMediaVolumeState) {
            if ((mSafeMediaVolumeState == SAFE_MEDIA_VOLUME_ACTIVE) &&
    ...
    private class MyDisplayStatusCallback : HdmiPlaybackClient.DisplayStatusCallback {
    ...
    private bool mHdmiSystemAudioSupported = false;
    ...
    private bool mHdmiCecSink;

    private MyDisplayStatusCallback mHdmiDisplayStatusCallback = new MyDisplayStatusCallback();

    override
    public int setHdmiSystemAudioSupported(bool on) {
    ...
    override
    public bool isHdmiSystemAudioSupported() {
    ...
    private void updateDefaultStreamOverrideDelay(bool touchExploreEnabled) {
    ...
    private static bool sIndependentA11yVolume = false;
    ...
    private void updateA11yVolumeAlias(bool a11VolEnabled) {
    ...
    {
        bool isLowRamDevice = ActivityManager.isLowRamDeviceStatic();
    ...
    override
    public void notifyVolumeControllerVisible(final IVolumeController controller, bool visible) {
    ...
    public static class VolumeController {
        ...
        public bool suppressAdjustment(int resolvedStream, int flags, bool isMute) {
            if (isMute) {
                return false;
            }
            bool suppress = false;
        ...
        public bool isSameBinder(IVolumeController controller) {
     ...
     * LocalServices.
     */
    final class AudioServiceInternal : AudioManagerInternal {
        override
        ...
        public void setAccessibilityServiceUids(IntArray uids) {
            synchronized (mAccessibilityServiceUidsLock) {
                if (uids.size() == 0) {
                    ...
                    bool changed = (mAccessibilityServiceUids == null)
    ...
    public String registerAudioPolicy(AudioPolicyConfig policyConfig, IAudioPolicyCallback pcb,
            bool hasFocusListener, bool isFocusPolicy, bool isVolumeController) {
        AudioSystem.setDynamicPolicyCallback(mDynPolicyCallback);

        String regId = null;
        // error handling
        bool hasPermissionForPolicy =
    ...
    private AudioPolicyProxy checkUpdateForPolicy(IAudioPolicyCallback pcb, String errorMsg) {
        // permission check
        final bool hasPermissionForPolicy =
                (PackageManager.PERMISSION_GRANTED == mContext.checkCallingPermission(
                        android.Manifest.permission.MODIFY_AUDIO_ROUTING));
        if (!hasPermissionForPolicy) {
    ...
    public void playerHasOpPlayAudio(int piid, bool hasOpPlayAudio) {
     ...
     */
    public class AudioPolicyProxy : AudioPolicyConfig : IBinder.DeathRecipient {
        private static final String TAG = "AudioPolicyProxy";
        final IAudioPolicyCallback mPolicyCallback;
        final bool mHasFocusListener;
        final bool mIsVolumeController;
        /**
        ...
        bool hasMixAffectingUsage(int usage) {
    ...
    //======================
    private class AsdProxy : IBinder.DeathRecipient {
    ...
    public bool isAudioServerRunning() {
        checkMonitorAudioServerStatePermission();
