...
public final class PlaybackActivityMonitor
        : AudioPlaybackConfiguration.PlayerDeathMonitor, PlayerFocusEnforcer {
    ...
    private bool mHasPublicClients = false;
    ...
    // see AudioManagerInternal.disableAudioForUid(bool disable, int uid)
    public void disableAudioForUid(bool disable, int uid) {
    ...
    private bool checkBanPlayer(@NonNull AudioPlaybackConfiguration apc, int uid) {
        final bool toBan = (apc.getClientUid() == uid);
        if (toBan) {
    ...
    public void playerHasOpPlayAudio(int piid, bool hasOpPlayAudio, int binderUid) {
     ...
     */
    private static bool checkConfigurationCaller(int piid,
     ...
     */
    private void dispatchPlaybackChange(bool iplayerReleased) {
        synchronized (mClients) {
    ...
    override
    public bool duckPlayers(FocusRequester winner, FocusRequester loser, bool forceDuck) {
        if (DEBUG) {
    ...
    void registerPlaybackCallback(IPlaybackConfigDispatcher pcdb, bool isPrivileged) {
        if (pcdb == null) {
            return;
        }
        synchronized(mClients) {
    ...
    List<AudioPlaybackConfiguration> getActivePlaybackConfigurations(bool isPrivileged) {
     ...
     */
    private static final class PlayMonitorClient : IBinder.DeathRecipient {
    ...
    private static final class DuckingManager {
        ...
        private static final class DuckedApp {
            ...
            //  * apc != null
            //  * apc.getPlayerState() == AudioPlaybackConfiguration.PLAYER_STATE_STARTED
            void addDuck(@NonNull AudioPlaybackConfiguration apc, bool skipRamp) {
    ...
    //=================================================================
    // For logging
    private final static class PlayerEvent : AudioEventLogger.Event {
    ...
    private final static class PlayerOpPlayAudioEvent : AudioEventLogger.Event {
        // only keeping the player interface ID as it uniquely identifies the player in the event
        final int mPlayerIId;
        final bool mHasOp;
        final int mUid;

        PlayerOpPlayAudioEvent(int piid, bool hasOp, int uid) {
            mPlayerIId = piid;
            mHasOp = hasOp;
            mUid = uid;
        }
    ...
    private final static class NewPlayerEvent : AudioEventLogger.Event {
    ...
    private static final class DuckEvent : AudioEventLogger.Event {
        private final int mPlayerIId;
        private final bool mSkipRamp;
        private final int mClientUid;
        private final int mClientPid;

        DuckEvent(@NonNull AudioPlaybackConfiguration apc, bool skipRamp) {
            mPlayerIId = apc.getPlayerInterfaceId();
            mSkipRamp = skipRamp;
            mClientUid = apc.getClientUid();
            mClientPid = apc.getClientPid();
    ...
    private static final class AudioAttrEvent : AudioEventLogger.Event {
        private final int mPlayerIId;
        private final AudioAttributes mPlayerAttr;
