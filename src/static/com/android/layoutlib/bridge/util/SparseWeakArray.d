...
public class SparseWeakArray<E> {
    ...
    private static final WeakReference!(T) DELETED = new WeakReference(DELETED_REF);
    ...
    private void gc() {
        ...
        int o = 0;
        long[] keys = mKeys;
        WeakReference!(T)[] values = mValues;

        for (int i = 0; i < n; i++) {
            WeakReference!(T) val = values[i];
    ...
    private long[] mKeys;
    private WeakReference!(T)[] mValues;
    private int mSize;
}
