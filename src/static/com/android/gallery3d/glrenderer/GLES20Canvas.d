...
public class GLES20Canvas : GLCanvas {
    // ************** Constants **********************
    ...
    private static class UniformShaderParameter : ShaderParameter {
    ...
    private static class AttributeShaderParameter : ShaderParameter {
    ...
    public void save(int saveFlags) {
        bool saveAlpha = (saveFlags & SAVE_FLAG_ALPHA) == SAVE_FLAG_ALPHA;
        if (saveAlpha) {
        ...
        bool saveMatrix = (saveFlags & SAVE_FLAG_MATRIX) == SAVE_FLAG_MATRIX;
        if (saveMatrix) {
    ...
    public void restore() {
        ...
        bool restoreAlpha = (restoreFlags & SAVE_FLAG_ALPHA) == SAVE_FLAG_ALPHA;
        if (restoreAlpha) {
            mCurrentAlphaIndex--;
        }
        bool restoreMatrix = (restoreFlags & SAVE_FLAG_MATRIX) == SAVE_FLAG_MATRIX;
        if (restoreMatrix) {
            mCurrentMatrixIndex -= MATRIX_SIZE;
        }
    }
    ...
    private void prepareDraw(int offset, int color, float lineWidth) {
        ...
        bool blendingEnabled = (colorArray[3] < 1f);
    ...
    private void enableBlending(bool enableBlending) {
        if (enableBlending) {
    ...
    override
    public bool unloadTexture(BasicTexture texture) {
        bool unload = texture.isLoaded();
        if (unload) {
