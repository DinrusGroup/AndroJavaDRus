...
public class InputMethodSettingValuesWrapper {
    ...
    public bool isAlwaysCheckedIme(InputMethodInfo imi, Context context) {
        final bool isEnabled = isEnabledImi(imi);
        synchronized (mMethodMap) {
    ...
    public bool isEnabledImi(InputMethodInfo imi) {
        final List<InputMethodInfo> enabledImis;
        synchronized (mMethodMap) {
            enabledImis = mSettings.getEnabledInputMethodListLocked();
        }
    ...
    public bool isValidSystemNonAuxAsciiCapableIme(InputMethodInfo imi, Context context) {
