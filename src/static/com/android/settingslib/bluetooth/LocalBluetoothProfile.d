...
public interface LocalBluetoothProfile {
     ...
     */
    bool isAutoConnectable();

    bool connect(BluetoothDevice device);

    bool disconnect(BluetoothDevice device);

    int getConnectionStatus(BluetoothDevice device);

    bool isPreferred(BluetoothDevice device);

    int getPreferred(BluetoothDevice device);

    void setPreferred(BluetoothDevice device, bool preferred);
