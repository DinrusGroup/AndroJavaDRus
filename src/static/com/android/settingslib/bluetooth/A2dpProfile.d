...
public class A2dpProfile : LocalBluetoothProfile {
    ...
    public bool setActiveDevice(BluetoothDevice device) {
        if (mService == null) return false;
    ...
    public bool supportsHighQualityAudio(BluetoothDevice device) {
        int support = mServiceWrapper.supportsOptionalCodecs(device);
        return support == BluetoothA2dp.OPTIONAL_CODECS_SUPPORTED;
    }

    public bool isHighQualityAudioEnabled(BluetoothDevice device) {
    ...
    public void setHighQualityAudioEnabled(BluetoothDevice device, bool enabled) {
