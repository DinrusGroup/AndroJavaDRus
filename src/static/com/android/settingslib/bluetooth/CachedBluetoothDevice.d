 ...
 * etc.).
 */
public class CachedBluetoothDevice : Comparable<CachedBluetoothDevice> {
    private static final String TAG = "CachedBluetoothDevice";
    private static final bool DEBUG = Utils.V;
    ...
    private bool mLocalNapRoleConnected;

    private bool mJustDiscovered;
     ...
     */
    private bool mIsConnectingErrorPossible;
    ...
    // Active device state
    private bool mIsActiveDeviceA2dp = false;
    private bool mIsActiveDeviceHeadset = false;
    private bool mIsActiveDeviceHearingAid = false;
    /**
    ...
    public void connect(bool connectAllProfiles) {
        if (!ensurePaired()) {
            return;
        }
    ...
    private void connectWithoutResettingTimer(bool connectAllProfiles) {
    ...
    public void unpair() {
        ...
        if (state != BluetoothDevice.BOND_NONE) {
            ...
            if (dev != null) {
                final bool successful = dev.removeBond();
                if (successful) {
                    if (Utils.D) {
    ...
    public void setJustDiscovered(bool justDiscovered) {
     ...
     */
    public void onActiveDeviceChanged(bool isActive, int bluetoothProfile) {
        bool changed = false;
    ...
    public bool isActiveDevice(int bluetoothProfile) {
        switch (bluetoothProfile) {
            case BluetoothProfile.A2DP:
    ...
    public bool isConnectedProfile(LocalBluetoothProfile profile) {
        int status = getProfileConnectionState(profile);
     ...
     */
    public bool checkAndIncreaseMessageRejectionCount() {
    ...
    public String getConnectionSummary() {
        bool profileConnected = false;    // Updated as long as BluetoothProfile is connected
        bool a2dpConnected = true;        // A2DP is connected
        bool hfpConnected = true;         // HFP is connected
        bool hearingAidConnected = true;  // Hearing Aid is connected
    ...
    public String getCarConnectionSummary() {
        bool profileConnected = false;       // at least one profile is connected
        bool a2dpNotConnected = false;       // A2DP is preferred but not connected
        bool hfpNotConnected = false;        // HFP is preferred but not connected
        bool hearingAidNotConnected = false; // Hearing Aid is preferred but not connected
