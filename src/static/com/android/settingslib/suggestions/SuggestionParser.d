...
public class SuggestionParser {
    ...
    public SuggestionList getSuggestions(bool isSmartSuggestionEnabled) {
     ...
     * be disabled.
     */
    public bool dismissSuggestion(Tile suggestion) {
    ...
    public void filterSuggestions(
            List<Tile> suggestions, int countBefore, bool isSmartSuggestionEnabled) {
    ...
    void readSuggestions(
            SuggestionCategory category, List<Tile> suggestions, bool isSmartSuggestionEnabled) {
    ...
    private bool isAvailable(Tile suggestion) {
    ...
    private bool satisifesRequiredUserType(Tile suggestion) {
        ...
        if (requiredUser != null) {
            ...
            for (String userType : requiredUser.split("\\|")) {
                final bool primaryUserCondtionMet = userInfo.isPrimary()
                        && META_DATA_PRIMARY_USER_TYPE_VALUE.equals(userType);
                final bool adminUserConditionMet = userInfo.isAdmin()
                        && META_DATA_ADMIN_USER_TYPE_VALUE.equals(userType);
                final bool guestUserCondtionMet = userInfo.isGuest()
                        && META_DATA_GUEST_USER_TYPE_VALUE.equals(userType);
                final bool restrictedUserCondtionMet = userInfo.isRestricted()
    ...
    public bool satisfiesRequiredAccount(Tile suggestion) {
        ...
        bool satisfiesRequiredAccount = accounts.length > 0;
    ...
    public bool isSupported(Tile suggestion) {
    ...
    private bool satisfiesConnectivity(Tile suggestion) {
        final bool isConnectionRequired =
        ...
        bool satisfiesConnectivity = netInfo != null && netInfo.isConnectedOrConnecting();
    ...
    public bool isCategoryDone(String category) {
     ...
     */
    private bool isExclusiveCategoryExpired(SuggestionCategory category) {
    ...
    @VisibleForTesting
    bool isDismissed(Tile suggestion, bool isSmartSuggestionEnabled) {
        ...
        final bool isDismissed = mSharedPrefs.getBoolean(keyBase + IS_DISMISSED, false);
        if (isDismissed) {
            return true;
        }
    ...
    private String getDismissControl(Tile suggestion, bool isSmartSuggestionEnabled) {
    ...
    private static class SuggestionOrderInflater {
        ...
        protected void onAddChildItem(Object parent, Object child) {
            if (parent instanceof List!(T) && child instanceof SuggestionCategory) {
