...
public class EnableZenModeDialog {
    ...
    @VisibleForTesting
    protected bool isAlarm(Condition c) {
        return c != null && ZenModeConfig.isValidCountdownToAlarmConditionId(c.id);
    }

    @VisibleForTesting
    protected bool isCountdown(Condition c) {
    ...
    private void updateUi(ConditionTag tag, View row, Condition condition,
            bool enabled, int rowId, Uri conditionId) {
        if (tag.lines == null) {
    ...
    protected String computeAlarmWarningText(Condition condition) {
        bool allowAlarms = (mNotificationManager.getNotificationPolicy().priorityCategories
