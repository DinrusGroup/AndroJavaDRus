 ...
 */
public abstract class ReflectionInflater<T> : SimpleInflater<T> {

    /* static section */

    private static final Class!(T)[] CONSTRUCTOR_SIGNATURE =
            new Class!(T)[] {Context.class, AttributeSet.class};

    private static final HashMap<String, Constructor!(T)> sConstructorMap = new HashMap<>();
    ...
    public final T createItem(String tagName, String prefix, AttributeSet attrs) {
        ...
        Constructor<? : T> constructor =
                (Constructor<? : T>) sConstructorMap.get(qualifiedName);
        ...
        try {
            if (constructor == null) {
                ...
                @SuppressWarnings("unchecked") // qualifiedName should correspond to a subclass of T
                Class<? : T> clazz =
                        (Class<? : T>) mContext.getClassLoader().loadClass(qualifiedName);
