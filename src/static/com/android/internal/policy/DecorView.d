...
/** @hide */
public class DecorView : FrameLayout : RootViewSurfaceTaker, WindowCallbacks {
    private static final String TAG = "DecorView";

    private static final bool DEBUG_MEASURE = false;

    private static final bool SWEEP_OPEN_MENU = false;
    ...
    private bool mAllowUpdateElevation = false;

    private bool mElevationAdjustedForStack = false;

    // Keeps track of the picture-in-picture mode for the view shadow
    private bool mIsInPictureInPictureMode;
    ...
    private bool mHasCaption = false;

    private bool mChanging;

    private Drawable mMenuBackground;
    private bool mWatchingForMenu;
    private int mDownY;
    ...
    final bool mForceWindowDrawsStatusBarBackground;
    ...
    private bool mLastHasTopStableInset = false;
    private bool mLastHasBottomStableInset = false;
    private bool mLastHasRightStableInset = false;
    private bool mLastHasLeftStableInset = false;
    private int mLastWindowFlags = 0;
    private bool mLastShouldAlwaysConsumeNavBar = false;
    ...
    private bool mWindowResizeCallbacksAdded = false;
    ...
    private bool mApplyFloatingVerticalInsets = false;
    private bool mApplyFloatingHorizontalInsets = false;
    ...
    public bool gatherTransparentRegion(Region region) {
        bool statusOpaque = gatherTransparentRegion(mStatusColorViewState, region);
        bool navOpaque = gatherTransparentRegion(mNavigationColorViewState, region);
        bool decorOpaque = super.gatherTransparentRegion(region);

        // combine bools after computation, so each method above always executes
        return statusOpaque || navOpaque || decorOpaque;
    }

    bool gatherTransparentRegion(ColorViewState colorViewState, Region region) {
    ...
    public bool dispatchKeyEvent(KeyEvent event) {
        ...
        final bool isDown = action == KeyEvent.ACTION_DOWN;

        if (isDown && (event.getRepeatCount() == 0)) {
            // First handle chording of panel key: if a panel key is held
            // but not released, try to execute a shortcut in it.
            if ((mWindow.mPanelChordingKey > 0) && (mWindow.mPanelChordingKey != keyCode)) {
                bool handled = dispatchKeyShortcutEvent(event);
                if (handled) {
                    return true;
                }
            }
        ...
        if (!mWindow.isDestroyed()) {
            ...
            final bool handled = cb != null && mFeatureId < 0 ? cb.dispatchKeyEvent(event)
    ...
    override
    public bool dispatchKeyShortcutEvent(KeyEvent ev) {
    ...
    private bool isOutOfInnerBounds(int x, int y) {
    ...
    protected bool setFrame(int l, int t, int r, int b) {
        bool changed = super.setFrame(l, t, r, b);
        if (changed) {
    ...
    private bool showContextMenuForChildInternal(View originalView,
            float x, float y) {
        ...
        final bool isPopup = !Float.isNaN(x) && !Float.isNaN(y);
        if (isPopup) {
    ...
    public static bool isNavBarToRightEdge(int bottomInset, int rightInset) {
        return bottomInset == 0 && rightInset > 0;
    }

    public static bool isNavBarToLeftEdge(int bottomInset, int leftInset) {
    ...
    WindowInsets updateColorViews(WindowInsets insets, bool animate) {
        ...
        if (!mWindow.mIsFloating || isImeWindow) {
            ...
            if (insets != null) {
                ...
                bool hasTopStableInset = insets.getStableInsetTop() != 0;
                disallowAnimate |= (hasTopStableInset != mLastHasTopStableInset);
                mLastHasTopStableInset = hasTopStableInset;

                bool hasBottomStableInset = insets.getStableInsetBottom() != 0;
                disallowAnimate |= (hasBottomStableInset != mLastHasBottomStableInset);
                mLastHasBottomStableInset = hasBottomStableInset;

                bool hasRightStableInset = insets.getStableInsetRight() != 0;
                disallowAnimate |= (hasRightStableInset != mLastHasRightStableInset);
                mLastHasRightStableInset = hasRightStableInset;

                bool hasLeftStableInset = insets.getStableInsetLeft() != 0;
            ...
            bool navBarToRightEdge = isNavBarToRightEdge(mLastBottomInset, mLastRightInset);
            bool navBarToLeftEdge = isNavBarToLeftEdge(mLastBottomInset, mLastLeftInset);
            ...
            bool statusBarNeedsRightInset = navBarToRightEdge
                    && mNavigationColorViewState.present;
            bool statusBarNeedsLeftInset = navBarToLeftEdge
                    && mNavigationColorViewState.present;
        ...
        bool consumingStatusBar = (sysUiVisibility & SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN) == 0
    ...
    private void updateColorViewInt(final ColorViewState state, int sysUiVis, int color,
            int dividerColor, int size, bool verticalBar, bool seascape, int sideMargin,
            bool animate, bool force) {
        state.present = state.attributes.isPresent(sysUiVis, mWindow.getAttributes().flags, force);
        bool show = state.attributes.isVisible(state.present, color,
                mWindow.getAttributes().flags, force);
        bool showView = show && !isResizing() && size > 0;
    ...
    private static void setColor(View v, int color, int dividerColor, bool verticalBar,
            bool seascape) {
        if (dividerColor != 0) {
        ...
        if (mPrimaryActionModeView != null) {
            if (mPrimaryActionModeView.getLayoutParams() instanceof MarginLayoutParams) {
                ...
                if (mPrimaryActionModeView.isShown()) {
                    ...
                    final bool nonOverlay = (mWindow.getLocalFeaturesPrivate()
     ...
     */
    public void updatePictureInPictureOutlineProvider(bool isInPictureInPictureMode) {
    ...
    public void setSurfaceKeepScreenOn(bool keepOn) {
    ...
    bool shouldAnimatePrimaryActionModeView() {
     ...
     **/
    void enableCaption(bool attachedAndVisible) {
    ...
    private DecorCaptionView createDecorCaptionView(LayoutInflater inflater) {
        ...
        final WindowManager.LayoutParams attrs = mWindow.getAttributes();
        final bool isApplication = attrs.type == TYPE_BASE_APPLICATION ||
    ...
    public static Drawable getResizingBackgroundDrawable(Context context, int backgroundRes,
            int backgroundFallbackRes, bool windowTranslucent) {
        if (backgroundRes != 0) {
    ...
    private void updateElevation() {
        float elevation = 0;
        final bool wasAdjustedForStack = mElevationAdjustedForStack;
    ...
    public static class ColorViewAttributes {
        ...
        public bool isPresent(int sysUiVis, int windowFlags, bool force) {
        ...
        public bool isVisible(bool present, int color, int windowFlags, bool force) {
            return present
                    && (color & Color.BLACK) != 0
                    && ((windowFlags & translucentFlag) == 0  || force);
        }

        public bool isVisible(int sysUiVis, int color, int windowFlags, bool force) {
            final bool present = isPresent(sysUiVis, windowFlags, force);
     ...
     */
    private class ActionModeCallback2Wrapper : ActionMode.Callback2 {
        ...
        public void onDestroyActionMode(ActionMode mode) {
            ...
            final bool isMncApp = mContext.getApplicationInfo().targetSdkVersion
                    >= M;
