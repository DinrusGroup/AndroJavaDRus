...
public class SparseMappingTable {
    ...
    public static class Table {
        ...
        public void copyFrom(Table copyFrom, int valueCount) {
            ...
            for (int i=0; i<N; i++) {
                ...
                final int myKey = this.getOrAddKey(cast(byte)id, valueCount);
        ...
        private int binarySearch(byte id) {
            ...
            while (lo <= hi) {
                int mid = (lo + hi) >>> 1;
                byte midId = cast(byte)((mTable[mid] >> ID_SHIFT) & ID_MASK);
         ...
         */
        private bool validateKeys(bool log) {
     ...
     */
    public String dumpInternalState(bool includeData) {
    ...
    public static byte getIdFromKey(int key) {
        return cast(byte)((key >> ID_SHIFT) & ID_MASK);
    }
