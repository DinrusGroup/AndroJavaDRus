...
public final class ProcessState {
    ...
    private static final bool DEBUG_PARCEL = false;
    ...
    public void setMultiPackage(bool val) {
        mMultiPackage = val;
    }
    ...
    public bool readFromParcel(Parcel in, bool fully) {
        bool multiPackage = in.readInt() != 0;
        if (fully) {
    ...
    public void addPss(long pss, long uss, long rss, bool always, int type, long duration,
            ArrayMap<String, ProcessStateHolder> pkgList) {
        ensureNotDead();
        switch (type) {
    ...
    public long getPssSampleCount(int state) {
        return mPssTable.getValueForId(cast(byte)state, PSS_SAMPLE_COUNT);
    }

    public long getPssMinimum(int state) {
        return mPssTable.getValueForId(cast(byte)state, PSS_MINIMUM);
    }

    public long getPssAverage(int state) {
        return mPssTable.getValueForId(cast(byte)state, PSS_AVERAGE);
    }

    public long getPssMaximum(int state) {
        return mPssTable.getValueForId(cast(byte)state, PSS_MAXIMUM);
    }

    public long getPssUssMinimum(int state) {
        return mPssTable.getValueForId(cast(byte)state, PSS_USS_MINIMUM);
    }

    public long getPssUssAverage(int state) {
        return mPssTable.getValueForId(cast(byte)state, PSS_USS_AVERAGE);
    }

    public long getPssUssMaximum(int state) {
        return mPssTable.getValueForId(cast(byte)state, PSS_USS_MAXIMUM);
    }

    public long getPssRssMinimum(int state) {
        return mPssTable.getValueForId(cast(byte)state, PSS_RSS_MINIMUM);
    }

    public long getPssRssAverage(int state) {
        return mPssTable.getValueForId(cast(byte)state, PSS_RSS_AVERAGE);
    }

    public long getPssRssMaximum(int state) {
        return mPssTable.getValueForId(cast(byte)state, PSS_RSS_MAXIMUM);
    }
    ...
    public void aggregatePss(TotalMemoryUseCollection data, long now) {
        ...
        bool fgHasBg = false;
        bool fgHasCached = false;
        bool bgHasCached = false;
        ...
        for (int is=0; is<screenStates.length; is++) {
            ...
            for (int im=0; im<memStates.length; im++) {
                for (int ip=0; ip<procStates.length; ip++) {
                    ...
                    long time = mDurations.getValueForId(cast(byte)bucket);
                    String running = "";
                    if (mCurState == bucket) {
                        running = " (running)";
                    }
    ...
    private void dumpProcessSummaryDetails(PrintWriter pw, String prefix,
            ...
            long now, long totalTime, bool full) {
    ...
    public void dumpInternalLocked(PrintWriter pw, String prefix, bool dumpAll) {
        if (dumpAll) {
    ...
    public void dumpCsv(PrintWriter pw,
            bool sepScreenStates, int[] screenStates, bool sepMemStates,
            int[] memStates, bool sepProcStates, int[] procStates, long now) {
