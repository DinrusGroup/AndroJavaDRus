 ...
 */
public class SysMemUsageTable : SparseMappingTable.Table {
    /**
    ...
    public void mergeStats(int state, long[] addData, int addOff) {
        final int key = getOrAddKey(cast(byte)state, SYS_MEM_USAGE_COUNT);
    ...
    public void dump(PrintWriter pw, String prefix, int[] screenStates, int[] memStates) {
        ...
        for (int is=0; is<screenStates.length; is++) {
            ...
            for (int im=0; im<memStates.length; im++) {
                ...
                long count = getValueForId(cast(byte)bucket, SYS_MEM_USAGE_SAMPLE_COUNT);
                if (count > 0) {
                    pw.print(prefix);
    ...
    private void dumpCategory(PrintWriter pw, String prefix, String label, int bucket, int index) {
        ...
        pw.print(": ");
        DebugUtils.printSizeValue(pw, getValueForId(cast(byte)bucket, index) * 1024);
        pw.print(" min, ");
        DebugUtils.printSizeValue(pw, getValueForId(cast(byte)bucket, index + 1) * 1024);
        pw.print(" avg, ");
        DebugUtils.printSizeValue(pw, getValueForId(cast(byte)bucket, index+2) * 1024);
        pw.println(" max");
    }
