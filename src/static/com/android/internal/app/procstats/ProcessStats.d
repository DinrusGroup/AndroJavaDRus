...
public final class ProcessStats : Parcelable {
    ...
    public ProcessStats(bool running) {
        mRunning = running;
        reset();
        if (running) {
    ...
    public void computeTotalMemoryUse(TotalMemoryUseCollection data, long now) {
        ...
        for (int is=0; is<data.screenStates.length; is++) {
            for (int im=0; im<data.memStates.length; im++) {
                ...
                final int sysKey = mSysMemUsage.getKey(cast(byte)stateBucket);
    ...
    public bool evaluateSystemProperties(bool update) {
        bool changed = false;
    ...
    private bool readCheckedInt(Parcel in, int val, String what) {
        int got;
    ...
    public void readFromParcel(Parcel in) {
        final bool hadData = mPackages.getMap().size() > 0
    ...
    public void dumpLocked(PrintWriter pw, String reqPackage, long now, bool dumpSummary,
        ...
        for (int ip=0; ip<pkgMap.size(); ip++) {
            ...
            for (int iu=0; iu<uids.size(); iu++) {
                ...
                for (int iv=0; iv<vpkgs.size(); iv++) {
                    ...
                    final int NPROCS = pkgState.mProcesses.size();
                    final int NSRVS = pkgState.mServices.size();
                    final bool pkgMatch = reqPackage == null || reqPackage.equals(pkgName);
                    if (!pkgMatch) {
    ...
    public void dumpSummaryLocked(PrintWriter pw, String reqPackage, long now, bool activeOnly) {
        long totalTime = DumpUtils.dumpSingleTime(null, null, mMemFactorDurations, mMemFactor,
                mStartTime, now);
    ...
    void dumpFilteredSummaryLocked(PrintWriter pw, String header, String prefix,
            ...
            int[] sortProcStates, long now, long totalTime, String reqPackage, bool activeOnly) {
    ...
    public static final class ProcessDataCollection {
        ...
        void print(PrintWriter pw, long overallTime, bool full) {
