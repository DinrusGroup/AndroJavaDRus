...
public class GpsNetInitiatedHandler {
    ...
    private static final bool VERBOSE = false;
    ...
    private bool mPlaySounds = false;
    private bool mPopupImmediately = true;

    // read the SUPL_ES form gps.conf
    private volatile bool mIsSuplEsEnabled;

    // Set to true if the phone is having emergency call.
    private volatile bool mIsInEmergency;

    // If Location function is enabled.
    private volatile bool mIsLocationEnabled = false;

    private final INetInitiatedListener mNetInitiatedListener;

    // Set to true if string from HAL is encoded as Hex, e.g., "3F0039"
    static private bool mIsHexInput = true;
    ...
    public void setSuplEsEnabled(bool isEnabled) {
        mIsSuplEsEnabled = isEnabled;
    }

    public bool getSuplEsEnabled() {
     ...
     */
    public bool getLocationEnabled() {
    ...
    public void setInEmergency(bool isInEmergency) {
        mIsInEmergency = isInEmergency;
    }

    public bool getInEmergency() {
        bool isInEmergencyCallback = mTelephonyManager.getEmergencyCallbackMode();
    ...
    private void handleNiInEs(GpsNiNotification notif) {
        ...
        /*
           ...
           4. When SUPL ES bit is on but UE is not in emergency mode:
                  Ignore the emergency SUPL INIT.
        */
        bool isNiTypeES = (notif.niType == GPS_NI_TYPE_EMERGENCY_SUPL);
    ...
    static byte[] stringToByteArray(String original, bool isHex)
    {
        ...
        if (isHex)
        {
            for (i = 0; i < length; i++)
            {
                output[i] = cast(byte) Integer.parseInt(original.substring(i*2, i*2+2), 16);
            }
        }
        else {
            for (i = 0; i < length; i++)
            {
                output[i] = cast(byte) original.charAt(i);
            }
        }
     ...
     */
    static private String decodeString(String original, bool isHex, int coding)
    {
