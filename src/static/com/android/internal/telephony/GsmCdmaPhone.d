 ...
 * {@hide}
 */
public class GsmCdmaPhone : Phone {
    ...
    private bool mResetModemOnRadioTechnologyChange = false;

    private int mRilVersion;
    private bool mBroadcastEmergencyCallStateChanges = false;
    ...
    public GsmCdmaPhone(Context context, CommandsInterface ci, PhoneNotifier notifier,
                        bool unitTestMode, int phoneId, int precisePhoneType,
    ...
    override
    public void sendEmergencyCallStateChange(bool callActive) {
    ...
    override
    public void setBroadcastEmergencyCallStateChanges(bool broadcast) {
    ...
    private bool handleCallDeflectionIncallSupplementaryService(
            String dialString) {
    ...
    //GSM
    private bool handleCallWaitingIncallSupplementaryService(String dialString) {
    ...
    private bool handleCallHoldIncallSupplementaryService(String dialString) {
        int len = dialString.length();
    ...
    private bool handleMultipartyIncallSupplementaryService(String dialString) {
        if (dialString.length() > 1) {
            return false;
        }
    ...
    private bool handleEctIncallSupplementaryService(String dialString) {
    ...
    private bool handleCcbsIncallSupplementaryService(String dialString) {
        if (dialString.length() > 1) {
            return false;
        }
    ...
    override
    public bool handleInCallMmiCommands(String dialString) throws CallStateException {
        if (!isPhoneTypeGsm()) {
    ...
    public Connection dial(String dialString, @NonNull DialArgs dialArgs)
        ...
        bool isEmergency = PhoneNumberUtils.isEmergencyNumber(getSubId(), dialString);
        Phone imsPhone = mImsPhone;

        CarrierConfigManager configManager =
                (CarrierConfigManager) mContext.getSystemService(Context.CARRIER_CONFIG_SERVICE);
        bool alwaysTryImsForEmergencyCarrierConfig = configManager.getConfigForSubId(getSubId())
                .getBoolean(CarrierConfigManager.KEY_CARRIER_USE_IMS_FIRST_FOR_EMERGENCY_BOOL);

        bool useImsForCall = isImsUseEnabled()
                 && imsPhone != null
                 && (imsPhone.isVolteEnabled() || imsPhone.isWifiCallingEnabled() ||
                 (imsPhone.isVideoEnabled() && VideoProfile.isVideo(dialArgs.videoState)))
                 && (imsPhone.getServiceState().getState() == ServiceState.STATE_IN_SERVICE);

        bool useImsForEmergency = imsPhone != null
                && isEmergency
        ...
        bool isUt = (dialPart.startsWith("*") || dialPart.startsWith("#"))
                && dialPart.endsWith("#");

        bool useImsForUt = imsPhone != null && imsPhone.isUtEnabled();
     ...
     */
    public bool isNotificationOfWfcCallRequired(String dialString) {
        CarrierConfigManager configManager =
                (CarrierConfigManager) mContext.getSystemService(Context.CARRIER_CONFIG_SERVICE);
        ...
        // default.
        bool shouldNotifyInternationalCallOnWfc = config != null
                && config.getBoolean(
   ...
   override
    ...
    override
    public bool handleUssdRequest(String ussdRequest, ResultReceiver wrappedCallback) {
    ...
    public String getDeviceId() {
        if (isPhoneTypeGsm()) {
            ...
            bool force_imei = configManager.getConfigForSubId(getSubId())
    ...
    private bool isValidCommandInterfaceCFReason (int commandInterfaceCFReason) {
    ...
    private bool isValidCommandInterfaceCFAction (int commandInterfaceCFAction) {
    ...
    private bool isCfEnable(int action) {
    ...
    public bool supports3gppCallForwardingWhileRoaming() {
        CarrierConfigManager configManager = (CarrierConfigManager)
                getContext().getSystemService(Context.CARRIER_CONFIG_SERVICE);
        PersistableBundle b = configManager.getConfig();
        if (b != null) {
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_CARRIER_CONFIG_CHANGED:
                ...
                if (b != null) {
                    bool broadcastEmergencyCallStateChanges = b.getBoolean(
     ...
     * @return true for success; false otherwise.
     */
    private bool updateCurrentCarrierInProvider(String operatorNumeric) {
        if (isPhoneTypeCdma()
     ...
     */
    public bool shouldForceAutoNetworkSelect() {
    ...
    private bool isManualSelProhibitedInGlobalMode() {
    ...
    //CDMA
    public void notifyEmergencyCallRegistrants(bool started) {
    ...
    //CDMA
    private static bool isIs683OtaSpDialStr(String dialStr) {
        int sysSelCodeInt;
        bool isOtaspDialString = false;
     ...
     */
    private static bool checkOtaSpNumBasedOnSysSelCode(int sysSelCodeInt, String sch[]) {
        bool isOtaSpNum = false;
        try {
     ...
     */
    private bool isCarrierOtaSpNum(String dialStr) {
        bool isOtaSpNum = false;
    ...
    private void phoneObjectUpdater(int newVoiceRadioTech) {
        ...
        if(mRilVersion == 6 && getLteOnCdmaMode() == PhoneConstants.LTE_ON_CDMA_TRUE) {
            ...
            bool matchCdma = ServiceState.isCdma(newVoiceRadioTech);
            bool matchGsm = ServiceState.isGsm(newVoiceRadioTech);
        ...
        bool oldPowerState = false; // old power state to off
    ...
    public bool setOperatorBrandOverride(String brand) {
        ...
        bool status = card.setOperatorBrandOverride(brand);
