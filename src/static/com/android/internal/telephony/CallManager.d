 ...
 * and control calls. It : Phone interface.
 *
...
public class CallManager {
    ...
    private bool mSpeedUpAudioForMtCall = false;
    // FIXME Taken from klp-sprout-dev but setAudioMode was removed in L.
    //private bool mIsEccDialing = false;
    ...
    */
    ...
    public void acceptCall(Call ringingCall) throws CallStateException {
        ...
        if ( hasActiveFgCall() ) {
            ...
            bool hasBgCall = ! (activePhone.getBackgroundCall().isIdle());
            bool sameChannel = (activePhone == ringingPhone);
     ...
     */
    public bool canConference(Call heldCall) {
        Phone activePhone = null;
        Phone heldPhone = null;
     ...
     * @return true if the phone can conference; false otherwise.
     */
    public bool canConference(Call heldCall, int subId) {
        Phone activePhone = null;
        Phone heldPhone = null;
     ...
     */
    private bool canDial(Phone phone) {
        int serviceState = phone.getServiceState().getState();
        int subId = phone.getSubId();
        bool hasRingingCall = hasActiveRingingCall();
        Call.State fgCallState = getActiveFgCallState(subId);

        bool result = (serviceState != ServiceState.STATE_POWER_OFF
                && !hasRingingCall
     ...
     */
    public bool canTransfer(Call heldCall) {
        Phone activePhone = null;
        Phone heldPhone = null;
     ...
     * @return true if the phone can do explicit call transfer; false otherwise.
     */
    public bool canTransfer(Call heldCall, int subId) {
        Phone activePhone = null;
        Phone heldPhone = null;
     ...
     */
    public List<? : MmiCode> getPendingMmiCodes(Phone phone) {
     ...
     */
    public bool sendUssdResponse(Phone phone, String ussdMessge) {
     ...
     *
     */
    public bool sendBurstDtmf(String dtmfString, int on, int off, Message onComplete) {
        if (hasActiveFgCall()) {
     ...
     */
    public bool hasActiveFgCall(int subId) {
     ...
     * on a particular subId or an active sip call
     */
    public bool hasActiveBgCall(int subId) {
        // TODO since hasActiveBgCall may get called often
        // better to cache it to improve performance
     ...
     *
     */
    public bool hasActiveRingingCall() {
     ...
     * Return true if there is at least one active ringing call
     */
    public bool hasActiveRingingCall(int subId) {
     ...
     */
    public bool hasDisconnectedFgCall() {
     ...
     * @return true if there is at least one Foreground call in disconnected state
     */
    public bool hasDisconnectedFgCall(int subId) {
     ...
     */
    public bool hasDisconnectedBgCall() {
     ...
     * @return true if there is at least one background call in disconnected state
     */
    public bool hasDisconnectedBgCall(int subId) {
    ...
    private bool hasMoreThanOneRingingCall() {
        int count = 0;
     ...
     *
     */
    private bool hasMoreThanOneRingingCall(int subId) {
        int count = 0;
        for (Call call : mRingingCalls) {
            if ((call.getState().isRinging()) &&
                ((call.getPhone().getSubId() == subId) ||
     ...
     *
     */
    private bool hasMoreThanOneHoldingCall(int subId) {
        int count = 0;
    ...
    /* FIXME Taken from klp-sprout-dev but setAudioMode was removed in L.
    private bool isServiceStateInService() {
    ...
    */
    ...
    private class CallManagerHandler : Handler {
        override
