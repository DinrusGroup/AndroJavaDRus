...
public abstract class IntRangeManager {
     ...
     */
    public synchronized bool enableRange(int startId, int endId, String client) {
        ...
        for (int startIndex = 0; startIndex < len; startIndex++) {
            ...
            if ((startId) >= range.mStartId && (endId) <= range.mEndId) {
                ...
                if ((endId + 1) < range.mStartId) {
                    ...
                    // endId : past all existing IntRanges: combine them all together
                    if (tryAddRanges(startId, endId, true)) {
                        range.mStartId = startId;
                        range.mEndId = endId;
                        // insert new ClientRange before existing ranges
     ...
     * @return true if successful, false otherwise
     */
    public synchronized bool disableRange(int startId, int endId, String client) {
        int len = mRanges.size();
     ...
     * @return true if successful, false otherwise
     */
    protected bool tryAddRanges(int startId, int endId, bool selected) {
     ...
     * @param selected true to enable range, false to disable range
     */
    protected abstract void addRange(int startId, int endId, bool selected);
     ...
     * @return true if successful, false otherwise
     */
    protected abstract bool finishUpdate();
}
