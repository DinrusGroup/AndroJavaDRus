 ...
 * {@hide}
 */
public class ServiceStateTracker : Handler {
    static final String LOG_TAG = "SST";
    static final bool DBG = true;
    ...
    private bool mDesiredPowerState;
     ...
     */
    private bool mDontPollSignalStrength = false;
    ...
    private bool mPendingRadioPowerOffAfterDataOff = false;
    ...
    private bool mImsRegistrationOnOff = false;
    private bool mAlarmSwitch = false;
    /** Radio is disabled by carrier. Radio power will not be override if this field is set */
    private bool mRadioDisabledByCarrier = false;
    ...
    private bool mPowerOffDelayNeed = true;
    private bool mDeviceShuttingDown = false;
    /** Keep track of SPN display rules, so we only broadcast intent if something changes. */
    private bool mSpnUpdatePending = false;
    private String mCurSpn = null;
    private String mCurDataSpn = null;
    private String mCurPlmn = null;
    private bool mCurShowPlmn = false;
    private bool mCurShowSpn = false;
    @VisibleForTesting
    public int mSubId = SubscriptionManager.INVALID_SUBSCRIPTION_ID;
    private int mPrevSubId = SubscriptionManager.INVALID_SUBSCRIPTION_ID;

    private bool mImsRegistered = false;
    ...
    private class SstSubscriptionsChangedListener : OnSubscriptionsChangedListener {
        ...
        public void onSubscriptionsChanged() {
            ...
            if (mPreviousSubId.getAndSet(subId) != subId) {
                if (SubscriptionManager.isValidSubscriptionId(subId)) {
                    ...
                    bool restoreSelection = !context.getResources().getBoolean(
     ...
     */
    private bool mGsmRoaming = false;
    /**
     * Data roaming status solely based on TS 27.007 10.1.19 CGREG. Only used by
     * handlePollStateResult to store CGREG roaming result.
     */
    private bool mDataRoaming = false;
    /**
     * Mark when service state is in emergency call only mode
     */
    private bool mEmergencyOnly = false;
    /** Started the recheck process after finding gprs should registered but not. */
    private bool mStartedGprsRegCheck;
    /** Already sent the event-log for no gprs register. */
    private bool mReportedGprsNoReg;
    ...
    private bool mIsSimReady = false;
    ...
    private bool mIsMinInfoReady = false;
    private bool mIsEriTextLoaded = false;
    private bool mIsSubscriptionFromRuim = false;
    ...
    public bool getDesiredPowerState() {
        return mDesiredPowerState;
    }
    public bool getPowerStateFromCarrier() { return !mRadioDisabledByCarrier; }

    private SignalStrength mLastSignalStrength = null;
    protected bool notifySignalStrength() {
        bool notified = false;
    ...
    protected void updatePhoneObject() {
        if (mPhone.getContext().getResources().
            ...
            bool isRegistered = mSS.getVoiceRegState() == ServiceState.STATE_IN_SERVICE ||
     ...
     */
    public void registerForDataRoamingOff(Handler h, int what, Object obj, bool notifyNow) {
        Registrant r = new Registrant(h, what, obj);
     ...
     */
    public void setRadioPowerFromCarrier(bool enable) {
     ...
     */
    private bool mWantContinuousLocationUpdates;
    private bool mWantSingleLocationUpdate;
     ...
     */
    private bool isHomeSid(int sid) {
        if (mHomeSystemId != null) {
            for (int i=0; i < mHomeSystemId.length; i++) {
        ...
        if (mPollingContext[0] == 0) {
            if (mPhone.isPhoneTypeGsm()) {
                ...
                if (mIsSubscriptionFromRuim) {
                    bool isRoamingBetweenOperators = isRoamingBetweenOperators(
                ...
                if (ServiceState.isCdma(dataRat)) {
                    final bool isVoiceInService =
                            (mNewSS.getVoiceRegState() == ServiceState.STATE_IN_SERVICE);
                    if (isVoiceInService) {
                        bool isVoiceRoaming = mNewSS.getVoiceRoaming();
                         ...
                         */
                        bool isRoamIndForHomeSystem = isRoamIndForHomeSystem(
     ...
     */
    private bool isRoamingBetweenOperators(bool cdmaRoaming, ServiceState s) {
        ...
        switch (what) {
            case EVENT_POLL_STATE_REGISTRATION: {
                ...
                if (mPhone.isPhoneTypeGsm()) {
                    ...
                    bool isVoiceCapable = mPhone.getContext().getResources()
            ...
            case EVENT_POLL_STATE_GPRS: {
                ...
                if (mPhone.isPhoneTypeGsm()) {
                    ...
                    bool isDataRoaming = regCodeIsRoaming(registrationState);
    ...
    private static bool isValidLteBandwidthKhz(int bandwidth) {
     ...
     */
    private bool isRoamIndForHomeSystem(String roamInd) {
    ...
    protected void updateRoamingState() {
        if (mPhone.isPhoneTypeGsm()) {
             ...
             */
            bool roaming = (mGsmRoaming || mDataRoaming);
    ...
    protected void updateSpnDisplay() {
        ...
        if (mPhone.isPhoneTypeGsm()) {
            ...
            if (combinedRegState == ServiceState.STATE_OUT_OF_SERVICE
                ...
                final bool forceDisplayNoService = mPhone.getContext().getResources().getBoolean(
            ...
            String dataSpn = spn;
            bool showSpn = !noService && !TextUtils.isEmpty(spn)
     ...
     */
    public bool isConcurrentVoiceAndDataAllowed() {
    ...
    public void pollState(bool modemTriggered) {
    ...
    private void pollStateDone() {
        ...
        bool hasDataAttached =
                mSS.getDataRegState() != ServiceState.STATE_IN_SERVICE
                        && mNewSS.getDataRegState() == ServiceState.STATE_IN_SERVICE;

        bool hasDataDetached =
                mSS.getDataRegState() == ServiceState.STATE_IN_SERVICE
                        && mNewSS.getDataRegState() != ServiceState.STATE_IN_SERVICE;

        bool hasDataRegStateChanged =
                mSS.getDataRegState() != mNewSS.getDataRegState();

        bool hasVoiceRegStateChanged =
                mSS.getVoiceRegState() != mNewSS.getVoiceRegState();

        bool hasLocationChanged = !mNewCellLoc.equals(mCellLoc);

        // ratchet the new tech up through its rat family but don't drop back down
        // until cell change or device is OOS
        bool isDataInService = mNewSS.getDataRegState() == ServiceState.STATE_IN_SERVICE;

        if (isDataInService) {
            mRatRatcheter.ratchet(mSS, mNewSS, hasLocationChanged);
        }

        bool hasRilVoiceRadioTechnologyChanged =
                mSS.getRilVoiceRadioTechnology() != mNewSS.getRilVoiceRadioTechnology();

        bool hasRilDataRadioTechnologyChanged =
                mSS.getRilDataRadioTechnology() != mNewSS.getRilDataRadioTechnology();

        bool hasChanged = !mNewSS.equals(mSS);

        bool hasVoiceRoamingOn = !mSS.getVoiceRoaming() && mNewSS.getVoiceRoaming();

        bool hasVoiceRoamingOff = mSS.getVoiceRoaming() && !mNewSS.getVoiceRoaming();

        bool hasDataRoamingOn = !mSS.getDataRoaming() && mNewSS.getDataRoaming();

        bool hasDataRoamingOff = mSS.getDataRoaming() && !mNewSS.getDataRoaming();

        bool hasRejectCauseChanged = mRejectCode != mNewRejectCode;

        bool hasCssIndicatorChanged = (mSS.getCssIndicator() != mNewSS.getCssIndicator());

        bool has4gHandoff = false;
        bool hasMultiApnSupport = false;
        bool hasLostMultiApnSupport = false;
        ...
        if (hasChanged) {
            ...
            if (isInvalidOperatorNumeric(operatorNumeric)) {
                ...
                // Update Time Zone.
                bool iccCardExists = iccCardExists();
                bool networkIsoChanged =
                ...
                bool countryChanged = iccCardExists && networkIsoChanged;
                if (DBG) {
    ...
    private void updateOperatorNameFromEri() {
        if (mPhone.isPhoneTypeCdma()) {
            ...
            bool hasBrandOverride = mUiccController.getUiccCard(getPhoneId()) != null &&
     ...
     */
    private bool isInHomeSidNid(int sid, int nid) {
    ...
    private bool isInvalidOperatorNumeric(String operatorNumeric) {
    ...
    private String fixUnknownMcc(String operatorNumeric, int sid) {
        ...
        bool isDst = nitzData != null && nitzData.isDst();
     ...
     */
    private bool isGprsConsistent(int dataRegState, int voiceRegState) {
     ...
     */
    private bool regCodeIsRoaming (int code) {
        return NetworkRegistrationState.REG_STATE_ROAMING == code;
    }

    private bool isSameOperatorNameFromSimAndSS(ServiceState s) {
        ...
        bool equalsOnsl = !TextUtils.isEmpty(spn) && spn.equalsIgnoreCase(onsl);
        bool equalsOnss = !TextUtils.isEmpty(spn) && spn.equalsIgnoreCase(onss);
     ...
     */
    private bool isSameNamedOperators(ServiceState s) {
     ...
     */
    private bool currentMccEqualsSimMcc(ServiceState s) {
     ...
     */
    private bool isOperatorConsideredNonRoaming(ServiceState s) {
        String operatorNumeric = s.getOperatorNumeric();
    ...
    private bool isOperatorConsideredRoaming(ServiceState s) {
        String operatorNumeric = s.getOperatorNumeric();
        final CarrierConfigManager configManager = (CarrierConfigManager) mPhone.getContext()
                .getSystemService(Context.CARRIER_CONFIG_SERVICE);
        String[] numericArray = null;
    ...
    public void setNotification(int notifyType) {
        ...
        bool isSetNotification = mPhone.getContext().getResources().getBoolean(
        ...
        if (configManager != null) {
            ...
            if (bundle != null) {
                bool disableVoiceBarringNotification = bundle.getBoolean(
        ...
        final bool multipleSubscriptions = (((TelephonyManager) mPhone.getContext()
     ...
     */
    private int selectResourceForRejectCode(int rejCode, bool multipleSubscriptions) {
        int rejResourceId = 0;
        switch (rejCode) {
     ...
     */
    public bool processPendingRadioPowerOffAfterDataOff() {
        synchronized(this) {
            if (mPendingRadioPowerOffAfterDataOff) {
     ...
     */
    private bool containsEarfcnInEarfcnRange(ArrayList<Pair<Integer, Integer>> earfcnPairList,
            int earfcn) {
     ...
     */
    protected bool onSignalStrengthResult(AsyncResult ar) {
        bool isGsm = false;
        ...
        bool ssChanged = notifySignalStrength();
     ...
     */
    private bool networkCountryIsoChanged(String newCountryIsoCode, String prevCountryIsoCode) {
    ...
    private bool iccCardExists() {
        bool iccCardExist = false;
        if (mUiccApplcation != null) {
    ...
    protected bool isCallerOnDifferentThread() {
        bool value = Thread.currentThread() != getLooper().getThread();
     ...
     */
    protected bool inSameCountry(String operatorNumeric) {
     ...
     */
    protected final bool alwaysOnHomeNetwork(BaseBundle b) {
     ...
     * @see CarrierConfigManager
     */
    private bool isInNetwork(BaseBundle b, String network, String key) {
    ...
    protected final bool isRoamingInGsmNetwork(BaseBundle b, String network) {
        return isInNetwork(b, network, CarrierConfigManager.KEY_GSM_ROAMING_NETWORKS_STRING_ARRAY);
    }

    protected final bool isNonRoamingInGsmNetwork(BaseBundle b, String network) {
        return isInNetwork(b, network, CarrierConfigManager.KEY_GSM_NONROAMING_NETWORKS_STRING_ARRAY);
    }

    protected final bool isRoamingInCdmaNetwork(BaseBundle b, String network) {
        return isInNetwork(b, network, CarrierConfigManager.KEY_CDMA_ROAMING_NETWORKS_STRING_ARRAY);
    }

    protected final bool isNonRoamingInCdmaNetwork(BaseBundle b, String network) {
    ...
    public bool isDeviceShuttingDown() {
