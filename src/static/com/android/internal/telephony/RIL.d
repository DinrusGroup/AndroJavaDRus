 ...
 * {@hide}
 */
public class RIL : BaseCommands : CommandsInterface {
    static final String RILJ_LOG_TAG = "RILJ";
    static final String RILJ_WAKELOCK_TAG = "*telephony-radio*";
    // Have a separate wakelock instance for Ack
    static final String RILJ_ACK_WAKELOCK_NAME = "RILJ_ACK_WL";
    static final bool RILJ_LOGD = true;
    static final bool RILJ_LOGV = false; // STOPSHIP if true
    ...
    bool mIsMobileNetworkSupported;
    ...
    @VisibleForTesting
    public class RilHandler : Handler {
    ...
    final class RadioProxyDeathRecipient : HwBinder.DeathRecipient {
        override
    ...
    override
    public void setCallWaiting(bool enable, int serviceClass, Message result) {
        IRadio radioProxy = getRadioProxy(result);
        if (radioProxy != null) {
    ...
    override
    public void acknowledgeLastIncomingGsmSms(bool success, int cause, Message result) {
        IRadio radioProxy = getRadioProxy(result);
        if (radioProxy != null) {
    ...
    override
    public void setMute(bool enableMute, Message result) {
        IRadio radioProxy = getRadioProxy(result);
        if (radioProxy != null) {
    ...
    override
    public void setLocationUpdates(bool enable, Message result) {
        IRadio radioProxy = getRadioProxy(result);
        if (radioProxy != null) {
    ...
    private void constructCdmaSendSmsRilRequest(CdmaSmsMessage msg, byte[] pdu) {
        ...
        try {
            ...
            msg.isServicePresent = cast(byte) dis.readInt() == 1 ? true : false; // servicePresent
            ...
            addrNbrOfDigits = cast(byte) dis.read();
            ...
            msg.subAddress.odd = cast(byte) dis.read() == 1 ? true : false; //subaddr odd
            subaddrNbrOfDigits = cast(byte) dis.read();
    ...
    override
    public void acknowledgeLastIncomingCdmaSms(bool success, int cause, Message result) {
        IRadio radioProxy = getRadioProxy(result);
        if (radioProxy != null) {
    ...
    override
    public void acknowledgeIncomingGsmSmsWithPdu(bool success, String ackPdu, Message result) {
        IRadio radioProxy = getRadioProxy(result);
        if (radioProxy != null) {
    ...
    public void sendImsGsmSms(String smscPdu, String pdu, int retry, int messageRef,
        ...
        if (radioProxy != null) {
            ...
            msg.retry = cast(byte) retry >= 1 ? true : false;
    ...
    override
    public void sendDeviceState(int stateType, bool state,
                                Message result) {
        IRadio radioProxy = getRadioProxy(result);
        if (radioProxy != null) {
     ...
     */
    override
    public void handleCallSetupRequestFromSim(bool accept, Message result) {
        IRadio radioProxy = getRadioProxy(result);
        if (radioProxy != null) {
    ...
    private bool clearWakeLock(int wakeLockType) {
     ...
     */
    private void clearRequestList(int error, bool loggable) {
        RILRequest rr;
        synchronized (mRequestList) {
            int count = mRequestList.size();
