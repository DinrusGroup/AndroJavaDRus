...
public class CarrierKeyDownloadManager {
    ...
    private void handleAlarmOrConfigChange() {
        if (carrierUsesKeys()) {
            if (areCarrierKeysAbsentOrExpiring()) {
                bool downloadStartedSuccessfully = downloadKey();
     ...
     **/
    @VisibleForTesting
    public bool isValidDownload(String mccMnc) {
        String mccCurrent = "";
        String mncCurrent = "";
        String mccSource = "";
        String mncSource = "";
     ...
     * determination.
     * @return bool returns true if carrier requires keys, else false.
     **/
    private bool carrierUsesKeys() {
     ...
     */
    @VisibleForTesting
    public bool isKeyEnabled(int keyType) {
     ...
     * @return bool returns true when keys are absent or close to expiration, else false.
     */
    @VisibleForTesting
    public bool areCarrierKeysAbsentOrExpiring() {
        for (int key_type : CARRIER_KEY_TYPES) {
            if (!isKeyEnabled(key_type)) {
                continue;
            }
