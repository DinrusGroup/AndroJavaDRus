...
class CallInfo {
    ...
    CallInfo (bool isMT, State state, bool isMpty, String number) {
        mIsMT = isMT;
        mState = state;
        mIsMpty = isMpty;
        mNumber = number;
...
class InvalidStateEx : Exception {
    InvalidStateEx() {
...
class SimulatedGsmCallState : Handler {
    //***** Instance Variables

    CallInfo mCalls[] = new CallInfo[MAX_CALLS];

    private bool mAutoProgressConnecting = true;
    private bool mNextDialFailImmediately;
