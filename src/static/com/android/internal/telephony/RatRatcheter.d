...
public class RatRatcheter {
    ...
    private bool mVoiceRatchetEnabled = true;
    private bool mDataRatchetEnabled = true;
     ...
     */
    public static bool updateBandwidths(int[] bandwidths, ServiceState serviceState) {
        if (bandwidths == null) {
            return false;
        }
    ...
    public void ratchet(ServiceState oldSS, ServiceState newSS, bool locationChange) {
        ...
        bool newUsingCA = oldSS.isUsingCarrierAggregation()
                || newSS.isUsingCarrierAggregation()
                || newSS.getCellBandwidths().length > 1;
        newSS.setIsUsingCarrierAggregation(newUsingCA);
    }

    private bool isSameRatFamily(ServiceState ss1, ServiceState ss2) {
        synchronized (mRatFamilyMap) {
