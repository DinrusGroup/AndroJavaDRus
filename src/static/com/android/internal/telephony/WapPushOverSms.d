 ...
 * @hide
 */
public class WapPushOverSms : ServiceConnection {
    ...
    private class BindServiceThread : Thread {
     ...
     */
    public bool isWapPushForMms(byte[] pdu, InboundSmsHandler handler) {
        DecodedResult result = decodeWapPdu(pdu, handler);
        return result.statusCode == Activity.RESULT_OK
            && WspTypeDecoder.CONTENT_TYPE_B_MMS.equals(result.mimeType);
    }

    private static bool shouldParseContentDisposition(int subId) {
        return SmsManager
    ...
    private static bool isDuplicateNotification(Context context, NotificationInd nInd) {
