...
public class NitzStateMachine {
    ...
    private static final bool DBG = ServiceStateTracker.DBG;
     ...
     * know the country.
     */
    private bool mNeedCountryCodeForNitz = false;

    private TimeStampedValue<NitzData> mLatestNitzSignal;
    private bool mGotCountryCode = false;
     ...
     * code.
     */
    private bool mNitzTimeZoneDetectionSuccessful = false;
    ...
    public NitzStateMachine(GsmCdmaPhone phone, TimeServiceHelper timeServiceHelper,
        ...
        mTimeServiceHelper.setListener(new TimeServiceHelper.Listener() {
            override
            public void onTimeDetectionChange(bool enabled) {
                if (enabled) {
            ...
            override
            public void onTimeZoneDetectionChange(bool enabled) {
                if (enabled) {
     ...
     *     probably hasn't
     */
    public void handleNetworkCountryCodeSet(bool countryChanged) {
        ...
        if (countryChanged || mNeedCountryCodeForNitz) {
            ...
            final bool isTimeZoneSettingInitialized =
     ...
     */
    private static bool nitzOffsetMightBeBogus(NitzData nitzData) {
    ...
    private static bool offsetInfoDiffers(NitzData one, NitzData two) {
    ...
    private void handleTimeFromNitz(TimeStampedValue<NitzData> nitzSignal) {
        try {
            bool ignoreNitz = mDeviceState.getIgnoreNitz();
            if (ignoreNitz) {
                Rlog.d(LOG_TAG,
     ...
     */
    public bool getNitzTimeZoneDetectionSuccessful() {
