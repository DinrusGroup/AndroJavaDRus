...
class SelectItemResponseData : ResponseData {
    // members
    private int mId;
...
class GetInkeyInputResponseData : ResponseData {
    // members
    private bool mIsUcs2;
    private bool mIsPacked;
    private bool mIsYesNo;
    private bool mYesNoResponse;
    ...
    public GetInkeyInputResponseData(String inData, bool ucs2, bool packed) {
        super();
        mIsUcs2 = ucs2;
        mIsPacked = packed;
        mInData = inData;
    ...
    public GetInkeyInputResponseData(bool yesNoResponse) {
        super();
        mIsUcs2 = false;
        mIsPacked = false;
        mInData = "";
...
class LanguageResponseData : ResponseData {
...
// See TS 31.111 section 6.4.15/ETSI TS 102 223
// TS 31.124 section 27.22.4.15 for test spec
class DTTZResponseData : ResponseData {
    ...
    public void format(ByteArrayOutputStream buf) {
        ...
        if (TextUtils.isEmpty(tz)) {
            data[7] = cast(byte) 0xFF;    // set FF in terminal response
        } else {
    ...
    private byte byteToBCD(int value) {
        ...
        return cast(byte) ((value / 10) | ((value % 10) << 4));
    }

    private byte getTZOffSetByte(long offSetVal) {
        bool isNegative = (offSetVal < 0);
