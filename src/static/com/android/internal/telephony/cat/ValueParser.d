...
abstract class ValueParser {
        ...
        if (length != 0) {
            ...
            try {
                for (int i = 0; i < itemCount; i++, valueIndex += 4) {
                    ...
                    bool bold = (format & 0x10) != 0;
                    bool italic = (format & 0x20) != 0;
                    bool underlined = (format & 0x40) != 0;
                    bool strikeThrough = (format & 0x80) != 0;
    ...
    static String retrieveTextString(ComprehensionTlv ctlv) throws ResultException {
        ...
        try {
            codingScheme = cast(byte) (rawValue[valueIndex] & 0x0c);
