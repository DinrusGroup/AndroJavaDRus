...
public abstract class SMSDispatcher : Handler {
    ...
    protected bool mSmsCapable = true;
    protected bool mSmsSendDisabled;
     ...
     */
    protected abstract class SmsSender : CarrierMessagingServiceManager {
     ...
     */
    protected final class TextSmsSender : SmsSender {
     ...
     */
    protected final class DataSmsSender : SmsSender {
     ...
     */
    protected final class SmsSenderCallback : ICarrierMessagingCallback.Stub {
     ...
     */
    private final class MultipartSmsSender : CarrierMessagingServiceManager {
     ...
     * Once the result is ready, the carrier messaging service connection is disposed.
     */
    private final class MultipartSmsSenderCallback : ICarrierMessagingCallback.Stub {
     ...
     */
    protected abstract bool shouldBlockSmsForEcbm();
     ...
     * @param expectMore is a bool to indicate the sending messages through same link or not.
    ...
    public void sendText(String destAddr, String scAddr, String text,
                         ...
                         String callingPkg, bool persistMessage, int priority,
    ...
    private bool sendSmsByCarrierApp(bool isDataSms, SmsTracker tracker ) {
    ...
    protected abstract SmsMessageBase.SubmitPduBase getSubmitPdu(String scAddr, String destAddr,
            String message, bool statusReportRequested, SmsHeader smsHeader,
            int priority, int validityPeriod);

    protected abstract SmsMessageBase.SubmitPduBase getSubmitPdu(String scAddr, String destAddr,
            int destPort, byte[] message, bool statusReportRequested);
    ...
    protected void sendMultipartText(String destAddr, String scAddr,
            ...
            bool persistMessage, int priority, bool expectMore, int validityPeriod) {
    ...
    private SmsTracker getNewSubmitPduTracker(String destinationAddress, String scAddress,
            ...
            PendingIntent sentIntent, PendingIntent deliveryIntent, bool lastPart,
            AtomicInteger unsentPartCount, AtomicBoolean anyPartFailed, Uri messageUri,
            String fullMessageText, int priority, bool expectMore, int validityPeriod) {
        if (isCdmaMo()) {
     ...
     */
    bool checkDestination(SmsTracker tracker) {
     ...
     */
    private bool denyIfQueueLimitReached(SmsTracker tracker) {
     ...
     */
    protected void handleConfirmShortCode(bool isPremium, SmsTracker tracker) {
        if (denyIfQueueLimitReached(tracker)) {
            return;     // queue limit reached; error was returned to caller
        }
    ...
    public static class SmsTracker {
        ...
        public bool mUsesImsServiceForIms;
        public int mMessageRef;
        public bool mExpectMore;
        ...
        private SmsTracker(HashMap<String, Object> data, PendingIntent sentIntent,
                ...
                AtomicInteger unsentPartCount, AtomicBoolean anyPartFailed, Uri messageUri,
                SmsHeader smsHeader, bool expectMore, String fullMessageText, int subId,
                bool isText, bool persistMessage, int userId, int priority,
                int validityPeriod) {
            mData = data;
        ...
        public void onFailed(Context context, int error, int errorCode) {
            ...
            bool isSinglePartOrLastPart = true;
    ...
    protected SmsTracker getSmsTracker(HashMap<String, Object> data, PendingIntent sentIntent,
            ...
            bool expectMore, String fullMessageText, bool isText, bool persistMessage,
            int priority, int validityPeriod) {
        // Get calling app package name via UID from Binder call
        PackageManager pm = mContext.getPackageManager();
        String[] packageNames = pm.getPackagesForUid(Binder.getCallingUid());
    ...
    protected SmsTracker getSmsTracker(HashMap<String, Object> data, PendingIntent sentIntent,
            PendingIntent deliveryIntent, String format, Uri messageUri, bool expectMore,
            String fullMessageText, bool isText, bool persistMessage) {
    ...
    protected SmsTracker getSmsTracker(HashMap<String, Object> data, PendingIntent sentIntent,
            PendingIntent deliveryIntent, String format, Uri messageUri, bool expectMore,
            String fullMessageText, bool isText, bool persistMessage, int priority,
            int validityPeriod) {
        return getSmsTracker(data, sentIntent, deliveryIntent, format, null/*unsentPartCount*/,
    ...
    private final class ConfirmDialogListener
        ...
        private bool mRememberChoice;    // default is unchecked
    ...
    protected String getCarrierAppPackageName() {
        ...
        // If there is no carrier package which : CarrierMessagingService, then lookup if
        // for a carrierImsPackage that : CarrierMessagingService.
