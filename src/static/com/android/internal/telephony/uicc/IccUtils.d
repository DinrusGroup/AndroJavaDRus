...
public class IccUtils {
    ...
    public static void bcdToBytes(String bcd, byte[] bytes) {
        ...
        for (int i = 0, j = 0; i + 1 < size; i += 2, j++) {
            bytes[j] = cast(byte) (charToByte(bcd.charAt(i + 1)) << 4 | charToByte(bcd.charAt(i)));
        }
    }
    ...
    public static String bcdPlmnToString(byte[] data, int offset) {
        ...
        trans[0] = cast(byte) ((data[0 + offset] << 4) | ((data[0 + offset] >> 4) & 0xF));
        trans[1] = cast(byte) ((data[1 + offset] << 4) | (data[2 + offset] & 0xF));
        trans[2] = cast(byte) ((data[2 + offset] & 0xF0) | ((data[1 + offset] >> 4) & 0xF));
    ...
    adnStringFieldToString(byte[] data, int offset, int length) {
        ...
        if (length >= 3 && data[offset] == cast(byte) 0x81) {
            ...
            offset += 3;
            isucs2 = true;
        } else if (length >= 4 && data[offset] == cast(byte) 0x82) {
            len = data[offset + 1] & 0xFF;
            if (len > length - 4)
                len = length - 4;
    ...
    hexStringToBytes(String s) {
        ...
        for (int i=0 ; i <sz ; i+=2) {
            ret[i/2] = cast(byte) ((hexCharToInt(s.charAt(i)) << 4)
    ...
    private static int intToBytes(int value, byte[] dest, int offset, bool signed) {
        ...
        for (int i = l - 1, v = value; i >= 0; i--, v >>>= 8) {
            byte b = cast(byte) (v & 0xFF);
            dest[offset + i] = b;
        }
        return l;
    }

    private static int byteNumForInt(int value, bool signed) {
        if (value < 0) {
            throw new IllegalArgumentException("value must be 0 or positive: " + value);
        }
        if (signed) {
    ...
    private static byte charToByte(char c) {
        if (c >= 0x30 && c <= 0x39) {
            return cast(byte) (c - 0x30);
        } else if (c >= 0x41 && c <= 0x46) {
            return cast(byte) (c - 0x37);
        } else if (c >= 0x61 && c <= 0x66) {
            return cast(byte) (c - 0x57);
        }
        return 0;
    }
}
