...
class Tags {
    ...
    static final byte[] EUICC_PROFILE_TAGS = new byte[] {
            TAG_ICCID,
            cast(byte) TAG_NICKNAME,
            cast(byte) TAG_SERVICE_PROVIDER_NAME,
            cast(byte) TAG_PROFILE_NAME,
            cast(byte) TAG_OPERATOR_ID,
            cast(byte) (TAG_PROFILE_STATE / 256),
            cast(byte) (TAG_PROFILE_STATE % 256),
            cast(byte) TAG_PROFILE_CLASS,
            cast(byte) TAG_PROFILE_POLICY_RULE,
            cast(byte) (TAG_CARRIER_PRIVILEGE_RULES / 256),
            cast(byte) (TAG_CARRIER_PRIVILEGE_RULES % 256),
    };
