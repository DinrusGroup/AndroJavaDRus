 ...
 * {@hide}
 */
public class SIMRecords : IccRecords {
    protected static final String LOG_TAG = "SIMRecords";

    private static final bool CRASH_RIL = false;
    ...
    setVoiceMessageWaiting(int line, int countWaiting) {
        ...
        try {
            if (mEfMWIS != null) {
                ...
                mEfMWIS[0] = cast(byte)((mEfMWIS[0] & 0xfe)
                                    | (countWaiting == 0 ? 0 : 1));
                ...
                if (countWaiting < 0) {
                    ...
                    mEfMWIS[1] = cast(byte) countWaiting;
                }
            ...
            if (mEfCPHS_MWI != null) {
                ...
                mEfCPHS_MWI[0] = cast(byte)((mEfCPHS_MWI[0] & 0xf0)
    ...
    private bool validEfCfis(byte[] data) {
        if (data != null) {
     ...
     */
    override
    public void setVoiceCallForwardingFlag(int line, bool enable, String dialNumber) {
        ...
        try {
            if (validEfCfis(mEfCfis)) {
                ...
                if (enable && !TextUtils.isEmpty(dialNumber)) {
                    ...
                    mEfCfis[CFIS_BCD_NUMBER_LENGTH_OFFSET] = cast(byte) (bcdNumber.length);
                    mEfCfis[CFIS_ADN_CAPABILITY_ID_OFFSET] = cast(byte) 0xFF;
                    mEfCfis[CFIS_ADN_EXTENSION_ID_OFFSET] = cast(byte) 0xFF;
                }
            ...
            if (mEfCff != null) {
                if (enable) {
                    mEfCff[0] = cast(byte) ((mEfCff[0] & CFF_LINE1_RESET)
     ...
     */
    override
    public void onRefresh(bool fileChanged, int[] fileList) {
        if (fileChanged) {
    ...
    public void handleMessage(Message msg) {
        ...
        bool isRecordLoadResponse = false;
    ...
    private class EfPlLoaded : IccRecordLoaded {
    ...
    private class EfUsimLiLoaded : IccRecordLoaded {
        public String getEfName() {
            return "EF_LI";
        }
    ...
    private bool useRoamingFromServiceState() {
     ...
     */
    private bool isOnMatchingPlmn(String plmn) {
     ...
     */
    private void getSpnFsm(bool start, AsyncResult ar) {
        byte[] data;
     ...
     */
    private bool isCphsMailboxEnabled() {
    ...
    private void handleEfCspData(byte[] data) {
        ...
        byte valueAddedServicesGroup = cast(byte)0xC0;
