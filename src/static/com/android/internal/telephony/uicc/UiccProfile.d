 ...
 * {@hide}
 */
public class UiccProfile : IccCard {
    ...
    private void setCurrentAppType(bool isGsm) {
        if (VDBG) log("setCurrentAppType");
        synchronized (mLock) {
            bool isLteOnCdmaMode = TelephonyManager.getLteOnCdmaModeStatic()
    ...
    private void handleCarrierNameOverride() {
        ...
        bool preferCcName = config.getBoolean(
    ...
    private void updateIccAvailability(bool allAppsChanged) {
        synchronized (mLock) {
    ...
    private void setExternalState(IccCardConstants.State newState, bool override) {
        synchronized (mLock) {
    ...
    override
    public bool getIccPin2Blocked() {
        /* defaults to disabled */
        return mUiccApplication != null && mUiccApplication.getIccPin2Blocked();
    }

    override
    public bool getIccPuk2Blocked() {
        /* defaults to disabled */
        return mUiccApplication != null && mUiccApplication.getIccPuk2Blocked();
    }

    override
    public void setIccLockEnabled(bool enabled, String password, Message onComplete) {
        synchronized (mLock) {
            if (mUiccApplication != null) {
    ...
    override
    public void setIccFdnEnabled(bool enabled, String password, Message onComplete) {
        synchronized (mLock) {
            if (mUiccApplication != null) {
     ...
     */
    private bool isSupportedApplication(UiccCardApplication app) {
    ...
    private void checkAndUpdateIfAnyAppToBeIgnored() {
        bool[] appReadyStateTracker = new bool[AppType.APPTYPE_ISIM.ordinal() + 1];
        for (UiccCardApplication app : mUiccApplications) {
    ...
    private bool areAllApplicationsReady() {
        for (UiccCardApplication app : mUiccApplications) {
            if (app != null && isSupportedApplication(app) && !app.isReady()
                    && !app.isAppIgnored()) {
    ...
    private bool areAllRecordsLoaded() {
        for (UiccCardApplication app : mUiccApplications) {
    ...
    static bool isPackageInstalled(Context context, String pkgName) {
    ...
    private void onCarrierPrivilegesLoadedMessage() {
        ...
        synchronized (mLock) {
            ...
            bool isProvisioned = Settings.Global.getInt(
     ...
     */
    public bool isApplicationOnIcc(IccCardApplicationStatus.AppType type) {
        synchronized (mLock) {
            for (int i = 0; i < mUiccApplications.length; i++) {
     ...
     */
    public bool resetAppWithAid(String aid) {
        synchronized (mLock) {
            bool changed = false;
            for (int i = 0; i < mUiccApplications.length; i++) {
                if (mUiccApplications[i] != null
     ...
     */
    public bool areCarrierPriviligeRulesLoaded() {
     ...
     */
    public bool hasCarrierPrivilegeRules() {
        UiccCarrierPrivilegeRules carrierPrivilegeRules = getCarrierPrivilegeRules();
