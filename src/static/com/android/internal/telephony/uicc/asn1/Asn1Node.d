...
public final class Asn1Node {
    ...
    public static final class Builder {
        ...
        public Builder addChildAsBits(int tag, int value) {
            ...
            for (int i = 1; i < dataBytes.length; i++) {
                dataBytes[i] = cast(byte) (value >> ((INT_BYTES - i) * Byte.SIZE));
                if (dataBytes[i] != 0) {
                    dataLength = i;
                }
            }
        ...
        /**
         * Adds a child of non-constructed tag with a bool as the data.
         *
         * @return This builder.
         * @throws IllegalStateException If the {@code tag} is not constructed..
         */
        public Builder addChildAsBoolean(int tag, bool value) {
            if (isConstructedTag(tag)) {
                throw new IllegalStateException("Cannot set value of a constructed tag: " + tag);
            }
    ...
    private final int mTag;
    private final bool mConstructed;
    ...
    private static bool isConstructedTag(int tag) {
     ...
     */
    public bool hasChild(int tag, int... tags) throws InvalidAsn1DataException {
        try {
            getChild(tag, tags);
     ...
     * @throws InvalidAsn1DataException If the data bytes cannot be parsed.
     */
    public bool asBoolean() throws InvalidAsn1DataException {
        if (mConstructed) {
            throw new IllegalStateException("Cannot get value of a constructed node.");
        }
        if (mDataBytes == null) {
        ...
        if (mDataLength != 1) {
            throw new InvalidAsn1DataException(
                    mTag, "Cannot parse data bytes as bool: length=" + mDataLength);
        }
        ...
        throw new InvalidAsn1DataException(
                mTag, "Cannot parse data bytes as bool: " + mDataBytes[mDataOffset]);
    }
    ...
    public String getHeadAsHex() {
        ...
        if (mDataLength <= 127) {
            headHex += IccUtils.byteToHex(cast(byte) mDataLength);
        } else {
            byte[] lenBytes = IccUtils.unsignedIntToBytes(mDataLength);
            headHex += IccUtils.byteToHex(cast(byte) (lenBytes.length | 0x80));
