...
public class AnswerToReset {
    ...
    public static final byte EUICC_SUPPORTED = cast(byte) 0x82;
    public static final byte DIRECT_CONVENTION = cast(byte) 0x3B;
    public static final byte INVERSE_CONVENTION = cast(byte) 0x3F;
    ...
    private bool mIsDirectConvention;
    private bool mOnlyTEqualsZero = true;
    private bool mIsEuiccSupported;
    ...
    private int parseInterfaceBytes(byte[] atrBytes, int index) {
        ...
        while (true) {
            ...
            if (VDBG) log("lastTD & TA_MASK: " + IccUtils.byteToHex(cast(byte) (lastTD & TA_MASK)));
            if ((lastTD & TA_MASK) != 0) {
                if (index >= atrBytes.length) {
            ...
            if (VDBG) log("lastTD & TB_MASK: " + IccUtils.byteToHex(cast(byte) (lastTD & TB_MASK)));
            if ((lastTD & TB_MASK) != 0) {
                if (index >= atrBytes.length) {
            ...
            if (VDBG) log("lastTD & TC_MASK: " + IccUtils.byteToHex(cast(byte) (lastTD & TC_MASK)));
            if ((lastTD & TC_MASK) != 0) {
                if (index >= atrBytes.length) {
            ...
            if (VDBG) log("lastTD & TD_MASK: " + IccUtils.byteToHex(cast(byte) (lastTD & TD_MASK)));
            if ((lastTD & TD_MASK) != 0) {
                if (index >= atrBytes.length) {
    ...
    private bool parseAtrString(String atr) {
        if (atr == null) {
    ...
    public bool isExtendedApduSupported() {
