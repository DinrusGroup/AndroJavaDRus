 ...
 *
 */
public class AdnRecord : Parcelable {
    ...
    public bool hasExtendedRecord() {
        return mExtRecord != 0 && mExtRecord != 0xff;
    }

    /** Helper function for {@link #isEqual}. */
    private static bool stringCompareNullEqualsEmpty(String s1, String s2) {
        if (s1 == s2) {
            return true;
        }
        if (s1 == null) {
    ...
    public bool isEqual(AdnRecord adn) {
    ...
    public byte[] buildAdnString(int recordSize) {
        ...
        if (byteTag.length > footerOffset) {
            ...
            adnString[footerOffset + ADN_BCD_NUMBER_LENGTH]
                    = cast(byte) (bcdNumber.length);
            adnString[footerOffset + ADN_CAPABILITY_ID]
                    = cast(byte) 0xFF; // Capability Id
            adnString[footerOffset + ADN_EXTENSION_ID]
                    = cast(byte) 0xFF; // Extension Record Id
