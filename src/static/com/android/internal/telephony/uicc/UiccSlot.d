...
public class UiccSlot : Handler {
    ...
    public UiccSlot(Context c, bool isActive) {
        if (DBG) log("Creating");
        mContext = c;
        mActive = isActive;
        mCardState = null;
    ...
    private bool absentStateUpdateNeeded(CardState oldState) {
    ...
    private void nullifyUiccCard(bool stateUnknown) {
    ...
    private void onIccSwap(bool isAdded) {

        bool isHotSwapSupported = mContext.getResources().getBoolean(
    ...
    private void promptForRestart(bool isAdded) {
        synchronized (mLock) {
