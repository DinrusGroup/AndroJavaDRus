 ...
 */
public class UsimDataDownloadHandler : Handler {
    ...
    private void handleDataDownload(SmsMessage smsMessage) {
        ...
        envelope[index++] = cast(byte) BER_SMS_PP_DOWNLOAD_TAG;
        if (bodyLength > 127) {
            envelope[index++] = cast(byte) 0x81;    // length 128-255 encoded as 0x81 + length
        }
        envelope[index++] = cast(byte) bodyLength;

        // Device identities TLV
        envelope[index++] = cast(byte) (0x80 | ComprehensionTlvTag.DEVICE_IDENTITIES.value());
        envelope[index++] = cast(byte) 2;
        envelope[index++] = cast(byte) DEV_ID_NETWORK;
        envelope[index++] = cast(byte) DEV_ID_UICC;

        // Address TLV (if present). Encoded length is assumed to be < 127 bytes.
        if (scAddressLength != 0) {
            envelope[index++] = cast(byte) ComprehensionTlvTag.ADDRESS.value();
            envelope[index++] = cast(byte) scAddressLength;
            System.arraycopy(pdu, 1, envelope, index, scAddressLength);
            index += scAddressLength;
        }

        // SMS TPDU TLV. Length is assumed to be < 256 bytes.
        envelope[index++] = cast(byte) (0x80 | ComprehensionTlvTag.SMS_TPDU.value());
        if (tpduLength > 127) {
            envelope[index++] = cast(byte) 0x81;    // length 128-255 encoded as 0x81 + length
        }
        envelope[index++] = cast(byte) tpduLength;
    ...
    private void sendSmsAckForEnvelopeResponse(IccIoResult response, int dcs, int pid) {
        ...
        if (success) {
            ...
            smsAckPdu[index++] = 0x00;      // TP-MTI, TP-UDHI
            smsAckPdu[index++] = cast(byte)
                    CommandsInterface.GSM_SMS_FAIL_CAUSE_USIM_DATA_DOWNLOAD_ERROR;  // TP-FCS
            smsAckPdu[index++] = 0x07;      // TP-PI: TP-PID, TP-DCS, TP-UDL present
        }

        smsAckPdu[index++] = cast(byte) pid;
        smsAckPdu[index++] = cast(byte) dcs;

        if (is7bitDcs(dcs)) {
            int septetCount = responseBytes.length * 8 / 7;
            smsAckPdu[index++] = cast(byte) septetCount;
        } else {
            smsAckPdu[index++] = cast(byte) responseBytes.length;
        }
     ...
     */
    private static bool is7bitDcs(int dcs) {
