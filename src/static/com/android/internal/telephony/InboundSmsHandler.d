 ...
 */
public abstract class InboundSmsHandler : StateMachine {
    protected static final bool DBG = true;
    private static final bool VDBG = false; // STOPSHIP if true, logs user data
    ...
    private final bool mSmsReceiveDisabled;
    ...
    protected InboundSmsHandler(String name, Context context, SmsStorageMonitor storageMonitor,
        ...
        bool smsCapable = mContext.getResources().getBoolean(
     ...
     */
    private class DeliveringState : State {
        override
        public void enter() {
    ...
    private void handleNewSms(AsyncResult ar) {
        ...
        if (result != Activity.RESULT_OK) {
            bool handled = (result == Intents.RESULT_SMS_HANDLED);
     ...
     */
    protected abstract void acknowledgeLastIncomingSms(bool success,
     ...
     * @param response callback message sent when operation completes.
     */
    private void notifyAndAcknowledgeLastIncomingSms(bool success,
            int result, Message response) {
        if (!success) {
     ...
     */
    protected abstract bool is3gpp2();
     ...
     */
    protected int addTrackerToRawTableAndSendMessage(InboundSmsTracker tracker, bool deDup) {
     ...
     */
    private bool processMessagePart(InboundSmsTracker tracker) {
     ...
     */
    private bool processMessagePartWithUserLocked(InboundSmsTracker tracker,
     ...
     */
    private bool filterSms(byte[][] pdus, int destPort,
        InboundSmsTracker tracker, SmsBroadcastReceiver resultReceiver, bool userUnlocked) {
     ...
     */
    private bool duplicateExists(InboundSmsTracker tracker) throws SQLException {
        String address = tracker.getAddress();
     ...
     */
    private int addTrackerToRawTable(InboundSmsTracker tracker, bool deDup) {
        if (deDup) {
            try {
     ...
     */
    static bool isCurrentFormat3gpp2() {
     ...
     */
    private final class SmsBroadcastReceiver : BroadcastReceiver {
    ...
    private final class CarrierServicesSmsFilterCallback :
        ...
        private final bool mUserUnlocked;

        CarrierServicesSmsFilterCallback(byte[][] pdus, int destPort, String smsFormat,
                         SmsBroadcastReceiver smsBroadcastReceiver,  bool userUnlocked) {
            mPdus = pdus;
            mDestPort = destPort;
     ...
     */
    private bool isSkipNotifyFlagSet(int callbackResult) {
    ...
    */
     ...
     * default SMS app.
     */
    private static class NewMessageNotificationActionReceiver : BroadcastReceiver {
        override
        public void onReceive(Context context, Intent intent) {
