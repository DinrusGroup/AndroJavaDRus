...
/**
 * This class : the character set mapping between
...
public class GsmAlphabet {
     ...
     */
    public static int
    charToGsm(char c, bool throwException) throws EncodeException {
        int ret;
    ...
    public static byte[] stringToGsm7BitPacked(String data, int startingSeptetOffset,
        ...
        ret[0] = cast(byte) (septetCount);  // Validated by check above.
        return ret;
    }
    ...
    packSmsChar(byte[] packedChars, int bitOffset, int value) {
        ...
        if (shift > 1) {
            packedChars[++byteOffset] = cast(byte)(value >> (8 - shift));
        }
    }
    ...
    stringToGsm8BitUnpackedField(String s, byte dest[], int offset, int length) {
        ...
        for (int i = 0, sz = s.length()
            ...
            dest[outByteIndex++] = cast(byte)v;
        }

        // pad with 0xff's
        while((outByteIndex - offset) < length) {
            dest[outByteIndex++] = cast(byte)0xff;
        }
    }
     ...
     */
    public static int
    countGsmSeptets(char c, bool throwsException) throws EncodeException {
    ...
    public static bool isGsmSeptets(char c) {
        if (sCharsToGsmTables[0].get(c, -1) != -1) {
            return true;
        }
     ...
     */
    public static int countGsmSeptetsUsingTables(CharSequence s, bool use7bitOnly,
            int languageTable, int languageShiftTable) {
        int count = 0;
        int sz = s.length();
        SparseIntArray charToLanguageTable = sCharsToGsmTables[languageTable];
    ...
    countGsmSeptets(CharSequence s, bool use7bitOnly) {
    ...
    private static bool sDisableCountryEncodingCheck = false;
