...
public class ImsServiceController {
    ...
    class ImsDeathRecipient : IBinder.DeathRecipient {
    ...
    class ImsServiceConnection : ServiceConnection {
    ...
    private class ImsFeatureContainer {
        ...
        public <T : IInterface> T resolve(Class<T> className) {
     ...
     * has failed.
     */
    public bool bind(HashSet<ImsFeatureConfiguration.FeatureSlotPair> imsFeatureSet) {
        synchronized (mLock) {
            if (!mIsBound && !mIsBinding) {
                ...
                try {
                    bool bindSucceeded = startBindToService(imsServiceIntent,
     ...
     */
    protected bool startBindToService(Intent intent, ImsServiceConnection connection,
            int flags) {
     ...
     */
    protected bool isServiceControllerAvailable() {
