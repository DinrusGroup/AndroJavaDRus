...
public class ImsResolver : ImsServiceController.ImsServiceControllerCallbacks {
    ...
    public static class ImsServiceInfo {
        ...
        // query.
        public bool featureFromMetadata = true;
    ...
    private final int mNumSlots;
    private final bool mIsDynamicBinding;
    ...
    private Handler mHandler = new Handler(Looper.getMainLooper(), (msg) -> {
        switch (msg.what) {
            ...
            case HANDLER_OVERRIDE_IMS_SERVICE_CONFIG: {
                ...
                bool isCarrierImsService = (msg.arg2 == 1);
                String packageName = (String) msg.obj;
    ...
    public bool overrideImsServiceConfiguration(int slotId, bool isCarrierService,
            String packageName) {
        if (slotId < 0 || slotId >= mNumSlots) {
        ...
        // encode bool to int for Message.
    ...
    // used for testing only.
    public String getImsServiceConfiguration(int slotId, bool isCarrierService) {
        if (slotId < 0 || slotId >= mNumSlots) {
    ...
    // killed.
    // Called from the handler ONLY
    private bool maybeRemovedImsService(String packageName) {
    ...
    private bool isActiveCarrierService(ImsServiceInfo info) {
        for (int i = 0; i < mNumSlots; i++) {
    ...
    private bool isDeviceService(ImsServiceInfo info) {
     ...
     */
    private bool shouldFeaturesCauseBind(
            HashSet<ImsFeatureConfiguration.FeatureSlotPair> features) {
    ...
    private void startDynamicQuery(ImsServiceInfo service) {
        bool queryStarted = mFeatureQueryManager.startQuery(service.name,
     ...
     */
    public bool isResolvingBinding() {
