...
public class KernelWakelockReader {
     ...
     */
    @VisibleForTesting
    public KernelWakelockStats parseProcWakelocks(byte[] wlBuffer, int len, bool wakeup_sources,
        ...
        synchronized(this) {
            ...
            while (endIndex < len) {
                ...
                for (int j = startIndex; j < endIndex; j++) {
                    if ((wlBuffer[j] & 0x80) != 0) wlBuffer[j] = cast(byte) '?';
                }
                bool parsed = Process.parseProcLine(wlBuffer, startIndex, endIndex,
