...
public abstract class LockSettingsInternal {
     ...
     */
    public abstract bool removeEscrowToken(long handle, int userId);
     ...
     */
    public abstract bool isEscrowTokenActive(long handle, int userId);

    public abstract bool setLockCredentialWithToken(String credential, int type,
            long tokenHandle, byte[] token, int requestedQuality, int userId);

    public abstract bool unlockUserWithToken(long tokenHandle, byte[] token, int userId);
}
