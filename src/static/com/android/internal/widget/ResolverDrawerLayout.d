...
public class ResolverDrawerLayout : ViewGroup {
    ...
    private bool mShowAtTop;

    private bool mIsDragging;
    private bool mOpenOnClick;
    private bool mOpenOnLayout;
    private bool mDismissOnScrollerFinished;
    ...
    public void setSmallCollapsed(bool smallCollapsed) {
        mSmallCollapsed = smallCollapsed;
        requestLayout();
    }

    public bool isSmallCollapsed() {
    ...
    public void setShowAtTop(bool showOnTop) {
        mShowAtTop = showOnTop;
        invalidate();
        requestLayout();
    }

    public bool getShowAtTop() {
        return mShowAtTop;
    }
    ...
    public void setDismissLocked(bool locked) {
    ...
    private bool updateCollapseOffset(int oldCollapsibleHeight, bool remainClosed) {
        ...
        if (isLaidOut()) {
            final bool isCollapsedOld = mCollapseOffset != 0;
            ...
            final bool isCollapsedNew = mCollapseOffset != 0;
    ...
    public bool onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                mInitialTouchY = mLastTouchY = y;
                mActivePointerId = ev.getPointerId(0);
                final bool hitView = findChildUnder(mInitialTouchX, mInitialTouchY) != null;
            ...
            case MotionEvent.ACTION_UP: {
                final bool wasDragging = mIsDragging;
                mIsDragging = false;
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            final bool keepGoing = !mScroller.isFinished();
    ...
    private float performDrag(float dy) {
        ...
        if (newPos != mCollapseOffset) {
            ...
            mTopOffset += dy;
            final bool isCollapsedNew = newPos != 0;
            if (isCollapsedOld != isCollapsedNew) {
                onCollapsedChanged(isCollapsedNew);
            }
            postInvalidateOnAnimation();
     ...
     */
    private bool isListChildUnderClipped(float x, float y) {
        final View listChild = findListChildUnder(x, y);
        return listChild != null && isDescendantClipped(listChild);
    }

    private bool isDescendantClipped(View child) {
    ...
    private static bool isChildUnder(View child, float x, float y) {
    ...
    public static class LayoutParams : MarginLayoutParams {
        ...
        public bool hasNestedScrollIndicator;
    ...
    private class RunOnDismissedListener : Runnable {
        override
        public void run() {
            dispatchOnDismissed();
        }
