 ...
 */
public class BluetoothPacketEncoder : PacketEncoder {
    ...
    private final MidiReceiver mFramedDataReceiver = new MidiReceiver() {
        ...
        public void onSend(byte[] msg, int offset, int count, long timestamp)
            ...
            synchronized (mLock) {
                ...
                bool isSysExStart = (status == MidiConstants.STATUS_SYSTEM_EXCLUSIVE);
                // Because of the MidiFramer, if it is not a status byte then it
                // must be a continuation.
                bool isSysExContinuation = ((status & 0x80) == 0);
                ...
                bool needsTimestamp = (status != mRunningStatus)
                ...
                if (needsTimestamp) {
                    ...
                    mAccumulationBuffer[mAccumulatedBytes++] =
                            cast(byte)(0x80 | (milliTimestamp & 0x7F));
    ...
    private void flushLocked(bool canBlock) {
