...
public class RecurrenceProcessor
{
     ...
     */
    private static bool listContains(int[] a, int N, int v)
    {
        for (int i=0; i<N; i++) {
            if (a[i] == v) {
                return true;
     ...
     */
    private static bool listContains(int[] a, int N, int v, int max)
    {
        for (int i=0; i<N; i++) {
            int w = a[i];
            if (w > 0) {
...
bysetpos:
     ...
     */
    private static bool filterMonthlySetPos(EventRecurrence r, Time instance) {
        /*
    ...
    private static bool useBYX(int freq, int freqConstant, int count)
    {
    ...
    public static class DaySet
    {
        ...
        bool get(Time iterator, int day)
        {
    ...
    public void expand(Time dtstart,
        ...
        try {
            ...
            bool usebymonth = useBYX(freq, EventRecurrence.MONTHLY, bymonthCount);
            bool useDays = freq >= EventRecurrence.WEEKLY &&
                                 (r.bydayCount > 0 || r.bymonthdayCount > 0);
            int byhourCount = r.byhourCount;
            bool usebyhour = useBYX(freq, EventRecurrence.HOURLY, byhourCount);
            int byminuteCount = r.byminuteCount;
            bool usebyminute = useBYX(freq, EventRecurrence.MINUTELY, byminuteCount);
            int bysecondCount = r.bysecondCount;
            bool usebysecond = useBYX(freq, EventRecurrence.SECONDLY, bysecondCount);
