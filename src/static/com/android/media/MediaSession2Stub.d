...
public class MediaSession2Stub : IMediaSession2.Stub {
    ...
    private static final bool DEBUG = true; // TODO(jaewan): Rename.
    ...
    private void onCommand(@NonNull IMediaController2 caller, int commandCode,
        ...
        session.getCallbackExecutor().execute(() -> {
            ...
            if (command != null) {
                bool accepted = session.getCallback().onCommandRequest(session.getInstance(),
                        controller, command);
                if (!accepted) {
    ...
    private bool isSubscribed(ControllerInfo controller, String parentId) {
        synchronized (mLock) {
