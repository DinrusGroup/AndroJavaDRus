...
public class ExternalStorageProvider : FileSystemProvider {
    ...
    private static class RootInfo {
        ...
        public File path;
        public bool reportAvailableBytes = true;
    }
    ...
    private String getDocIdForFileMaybeCreate(File file, bool createNewDir)
            throws FileNotFoundException {
        String path = file.getAbsolutePath();

        // Find the most-specific root path
        bool visiblePath = false;
    ...
    private RootInfo getMostSpecificRootForPath(String path, bool visible) {
        // Find the most-specific root path
    ...
    override
    protected File getFileForDocId(String docId, bool visible) throws FileNotFoundException {
        return getFileForDocId(docId, visible, true);
    }

    private File getFileForDocId(String docId, bool visible, bool mustExist)
            throws FileNotFoundException {
        RootInfo root = getRootFromDocId(docId);
        return buildFile(root, docId, visible, mustExist);
    }

    private Pair<RootInfo, File> resolveDocId(String docId, bool visible)
            throws FileNotFoundException {
        RootInfo root = getRootFromDocId(docId);
    ...
    private File buildFile(RootInfo root, String docId, bool visible, bool mustExist)
            throws FileNotFoundException {
        final int splitIndex = docId.indexOf(':', 1);
    ...
    private Uri getDocumentUri(String path, List<UriPermission> accessUriPermissions)
        ...
        for (UriPermission uriPermission : accessUriPermissions) {
            ...
            if (AUTHORITY.equals(uri.getAuthority())) {
                bool matchesRequestedDoc = false;
    ...
    private static bool allowsBothReadAndWrite(UriPermission permission) {
