...
public class KeyButtonView : ImageView : ButtonInterface {
    private static final String TAG = KeyButtonView.class.getSimpleName();

    private final bool mPlaySounds;
    ...
    private bool mSupportsLongpress = true;
    private AudioManager mAudioManager;
    private bool mGestureAborted;
    private bool mLongClicked;
    ...
    public bool onTouchEvent(MotionEvent ev) {
        final bool showSwipeUI = mOverviewProxyService.shouldShowSwipeUpUI();
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                bool exceededTouchSlopX = Math.abs(x - mTouchDownX) > (mIsVertical
                        ? NavigationBarCompat.getQuickScrubTouchSlopPx()
                        : NavigationBarCompat.getQuickStepTouchSlopPx());
                bool exceededTouchSlopY = Math.abs(y - mTouchDownY) > (mIsVertical
                        ? NavigationBarCompat.getQuickStepTouchSlopPx()
                        : NavigationBarCompat.getQuickScrubTouchSlopPx());
            ...
            case MotionEvent.ACTION_UP:
                final bool doIt = isPressed() && !mLongClicked;
                setPressed(false);
                final bool doHapticFeedback = (SystemClock.uptimeMillis() - mDownTime) > 150;
                if (showSwipeUI) {
                    if (doIt) {
    ...
    override
    public void setDelayTouchFeedback(bool shouldDelay) {
