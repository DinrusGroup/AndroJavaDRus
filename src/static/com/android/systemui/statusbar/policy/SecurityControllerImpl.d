...
public class SecurityControllerImpl : CurrentUserTracker : SecurityController {
    ...
    override
    public bool isNetworkLoggingEnabled() {
    ...
    override
    public bool hasCACertInCurrentUser() {
        Boolean hasCACerts = mHasCACerts.get(mCurrentUserId);
        return hasCACerts != null && hasCACerts.boolValue();
    }

    override
    public bool hasCACertInWorkProfile() {
    ...
    private bool isVpnPackageBranded(String packageName) {
        bool isBranded;
        try {
    ...
    protected class CACertLoader : AsyncTask<Integer, Void, Pair<Integer, Boolean> > {
        ...
        protected Pair<Integer, Boolean> doInBackground(Integer... userId) {
            try (KeyChainConnection conn = KeyChain.bindAsUser(mContext,
                ...
                bool hasCACerts = !(conn.getService().getUserCaAliases().getList().isEmpty());
