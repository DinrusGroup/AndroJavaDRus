...
public class NotificationMenuRow : NotificationMenuRowPlugin, View.OnClickListener,
    ...
    private bool mMenuFadedIn;

    private bool mOnLeft;
    private bool mIconsPlaced;

    private bool mDismissing;
    private bool mSnapping;
    ...
    private bool mMenuSnappedTo;
    private bool mMenuSnappedOnLeft;
    private bool mShouldShowMenu;

    private NotificationSwipeActionHelper mSwipeHelper;
    private bool mIsUserTouching;
    ...
    private void createMenuViews(bool resetState) {
        ...
        if (mParent != null && mParent.getStatusBarNotification() != null) {
            ...
            bool isForeground = (flags & Notification.FLAG_FOREGROUND_SERVICE) != 0;
            if (!isForeground) {
    ...
    override
    public bool onTouchEvent(View view, MotionEvent ev, float velocity) {
    ...
    private bool handleUpEvent(MotionEvent ev, View animView, float velocity) {
        ...
        final bool gestureTowardsMenu = isTowardsMenu(velocity);
        final bool gestureFastEnough =
                mSwipeHelper.getMinDismissVelocity() <= Math.abs(velocity);
        final bool gestureFarEnough =
                mSwipeHelper.swipedFarEnough(mTranslation, mParent.getWidth());
        final double timeForGesture = ev.getEventTime() - ev.getDownTime();
        final bool showMenuForSlowOnGoing = !mParent.canViewBeDismissed()
        ...
        if (mMenuSnappedTo && isMenuVisible() && mMenuSnappedOnLeft == mOnLeft) {
            ...
            bool withinSnapMenuThreshold = mOnLeft
                    ? mTranslation > targetLeft && mTranslation < targetRight
                    : mTranslation < -targetLeft && mTranslation > -targetRight;
            bool shouldSnapTo = mOnLeft ? mTranslation < targetLeft : mTranslation > -targetLeft;
            if (DEBUG) {
     ...
     *         to be dismissed.
     */
    private bool swipedEnoughToShowMenu() {
     ...
     */
    private bool isTowardsMenu(float movement) {
    ...
    private bool isMenuLocationChange() {
        bool onLeft = mTranslation > mIconPadding;
        bool onRight = mTranslation < -mIconPadding;
    ...
    private void setMenuLocation() {
        bool showOnLeft = mTranslation > 0;
    ...
    private final class CheckForDrag : Runnable {
        override
        public void run() {
    ...
    private void fadeInMenu(final float notiThreshold) {
        ...
        final bool fromLeft = mTranslation > 0;
        setMenuLocation();
        ...
        mFadeAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                bool pastMenu = (fromLeft && transX <= notiThreshold)
    ...
    public static class NotificationMenuItem : MenuItem {
        View mMenuView;
