 ...
 */
public class NotificationEntryManager : Dumpable, NotificationInflater.InflationCallback,
    ...
    protected static final bool ENABLE_HEADS_UP = true;
    ...
    protected bool mUseHeadsUp = false;
    protected bool mDisableNotificationAlerts;
    ...
    private final class NotificationClicker : View.OnClickListener {
    ...
    public void setDisableNotificationAlerts(bool disableNotificationAlerts) {
    ...
    override
    public void logNotificationExpansion(String key, bool userAction, bool expanded) {
    ...
    private bool shouldSuppressFullScreenIntent(NotificationData.Entry entry) {
    ...
    private void addEntry(NotificationData.Entry shadeEntry) {
        bool isHeadsUped = shouldPeek(shadeEntry);
        if (isHeadsUped) {
    ...
    public void onAsyncInflationFinished(NotificationData.Entry entry) {
        ...
        bool isNew = mNotificationData.get(entry.key) == null;
    ...
    public void removeNotification(String key, NotificationListenerService.RankingMap ranking) {
        ...
        if (mHeadsUpManager.isHeadsUp(key)) {
            ...
            bool ignoreEarliestRemovalTime = mRemoteInputManager.getController().isSpinning(key)
                    && !FORCE_REMOTE_INPUT_HISTORY
    ...
    public StatusBarNotification rebuildNotificationWithRemoteInput(NotificationData.Entry entry,
            CharSequence remoteInputText, bool showSpinner) {
    ...
    private bool shouldKeepForSmartReply(NotificationData.Entry entry) {
        return entry != null && mSmartReplyController.isSendingSmartReply(entry.key);
    }

    private bool shouldKeepForRemoteInput(NotificationData.Entry entry) {
        if (entry == null) {
            return false;
        }
    ...
    public void updateNotificationsOnDensityOrFontScaleChanged() {
        ...
        for (int i = 0; i < userNotifications.size(); i++) {
            ...
            bool exposedGuts = mGutsManager.getExposedGuts() != null
    ...
    protected void updateNotification(NotificationData.Entry entry, PackageManager pmUser,
        ...
        bool isLowPriority = mNotificationData.isAmbient(sbn.getKey());
        bool isUpdate = mNotificationData.get(entry.key) != null;
        bool wasLowPriority = row.isLowPriority();
        ...
        bool useIncreasedCollapsedHeight = mMessagingUtil.isImportantMessaging(sbn,
                mNotificationData.getImportance(sbn.getKey()));
        bool useIncreasedHeadsUp = useIncreasedCollapsedHeight
    ...
    public void updateNotificationsForAppOp(int appOp, int uid, String pkg, bool showIcon) {
    ...
    private bool alertAgain(NotificationData.Entry oldEntry, Notification newNotification) {
    ...
    private void updateNotificationInternal(StatusBarNotification notification,
        ...
        bool shouldPeek = shouldPeek(entry, notification);
        bool alertAgain = alertAgain(entry, n);
        ...
        if (DEBUG) {
            // Is this for you?
            bool isForCurrentUser = mPresenter.isNotificationForCurrentProfiles(notification);
    ...
    protected bool shouldPeek(NotificationData.Entry entry) {
        return shouldPeek(entry, entry.notification);
    }

    public bool shouldPeek(NotificationData.Entry entry, StatusBarNotification sbn) {
        ...
        bool inUse = mPowerManager.isScreenOn() && !mSystemServicesProxy.isDreaming();
    ...
    protected bool isSnoozedPackage(StatusBarNotification sbn) {
        return mHeadsUpManager.isSnoozed(sbn.getPackageName());
    }

    protected void updateHeadsUp(String key, NotificationData.Entry entry, bool shouldPeek,
            bool alertAgain) {
        final bool wasHeadsUp = isHeadsUp(key);
        if (wasHeadsUp) {
            if (!shouldPeek) {
    ...
    protected bool isHeadsUp(String key) {
        return mHeadsUpManager.isHeadsUp(key);
    }

    public bool isNotificationKeptForRemoteInput(String key) {
    ...
    public interface Callback {
         ...
         */
        bool shouldPeek(NotificationData.Entry entry, StatusBarNotification sbn);
    }
}
