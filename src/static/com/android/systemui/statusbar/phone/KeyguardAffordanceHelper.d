...
public class KeyguardAffordanceHelper {
    ...
    private bool mMotionCancelled;
    private int mTouchTargetSize;
    private View mTargetedView;
    private bool mTouchSlopExeeded;
    ...
    public bool onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                bool hintOnTheRight = mTargetedView == mRightIcon;
                trackMovement(event);
                endMotion(!isUp, x, y);
    ...
    public bool isOnAffordanceIcon(float x, float y) {
        return isOnIcon(mLeftIcon, x, y) || isOnIcon(mRightIcon, x, y);
    }

    private bool isOnIcon(View icon, float x, float y) {
    ...
    private void endMotion(bool forceSnapBack, float lastX, float lastY) {
        if (mSwipingInProgress) {
    ...
    private bool rightSwipePossible() {
        return mRightIcon.getVisibility() == View.VISIBLE;
    }

    private bool leftSwipePossible() {
    ...
    public void startHintAnimation(bool right,
            Runnable onFinishedListener) {
        cancelAnimation();
        startHintAnimationPhase1(right, onFinishedListener);
    }

    private void startHintAnimationPhase1(final bool right, final Runnable onFinishedListener) {
     ...
     * Phase 2: Move back.
     */
    private void startUnlockHintAnimationPhase2(bool right, final Runnable onFinishedListener) {
    ...
    private ValueAnimator getAnimatorToRadius(final bool right, int radius) {
        final KeyguardAffordanceView targetView = right ? mRightIcon : mLeftIcon;
    ...
    private void flingWithCurrentVelocity(bool forceSnapBack, float lastX, float lastY) {
        ...
        bool velIsInWrongDirection = vel * mTranslation < 0;
        snapBack |= Math.abs(vel) > mMinFlingVelocity && velIsInWrongDirection;
        vel = snapBack ^ velIsInWrongDirection ? 0 : vel;
        fling(vel, snapBack || forceSnapBack, mTranslation < 0);
    }

    private bool isBelowFalsingThreshold() {
    ...
    private void fling(float vel, final bool snapBack, bool right) {
    ...
    private void setTranslation(float translation, bool isReset, bool animateReset) {
        ...
        if (translation != mTranslation || isReset) {
            ...
            bool animateIcons = isReset && animateReset;
            bool forceNoCircleAnimation = isReset && !animateReset;
            float radius = getRadiusFromTranslation(absTranslation);
            bool slowAnimation = isReset && isBelowFalsingThreshold();
            if (!isReset) {
    ...
    private void updateIcon(KeyguardAffordanceView view, float circleRadius, float alpha,
                            bool animate, bool slowRadiusAnimation, bool force,
                            bool forceNoCircleAnimation) {
    ...
    private void updateIconAlpha(KeyguardAffordanceView view, float alpha, bool animate) {
    ...
    public bool isSwipingInProgress() {
        return mSwipingInProgress;
    }

    public void launchAffordance(bool animate, bool left) {
        if (mSwipingInProgress) {
    ...
    public interface Callback {
         ...
         */
        void onAnimationToSideStarted(bool rightPage, float translation, float vel);
        ...
        void onSwipingStarted(bool rightIcon);

        void onSwipingAborted();

        void onIconClicked(bool rightIcon);
