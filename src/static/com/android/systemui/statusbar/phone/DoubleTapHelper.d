...
public class DoubleTapHelper {
    ...
    public bool onTouchEvent(MotionEvent event, int maxTouchableHeight) {
            ...
            case MotionEvent.ACTION_UP:
                if (isWithinTouchSlop(event)) {
                    ...
                    if (!mActivated) {
                        ...
                        bool withinDoubleTapSlop = isWithinDoubleTapSlop(event);
    ...
    private bool isWithinTouchSlop(MotionEvent event) {
        return Math.abs(event.getX() - mDownX) < mTouchSlop
                && Math.abs(event.getY() - mDownY) < mTouchSlop;
    }

    public bool isWithinDoubleTapSlop(MotionEvent event) {
        if (!mActivated) {
    ...
    public interface DoubleTapLogListener {
        void onDoubleTapLog(bool accepted, float dx, float dy);
    }
}
