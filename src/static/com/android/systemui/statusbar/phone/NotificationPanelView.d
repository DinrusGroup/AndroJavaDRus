...
public class NotificationPanelView : PanelView :
    ...
    private bool mAnimateNextPositionUpdate;

    private int mTrackingPointer;
    private VelocityTracker mQsVelocityTracker;
    private bool mQsTracking;

    /**
     * If set, the ongoing touch gesture might both trigger the expansion in {@link PanelView} and
     * the expansion for quick settings.
     */
    private bool mConflictingQsExpansionGesture;

    /**
     * Whether we are currently handling a motion gesture in #onInterceptTouchEvent, but haven't
     * intercepted yet.
     */
    private bool mIntercepting;
    private bool mPanelExpanded;
    private bool mQsExpanded;
    private bool mQsExpandedWhenExpandingStarted;
    private bool mQsFullyExpanded;
    private bool mKeyguardShowing;
    private bool mDozing;
    private bool mDozingOnDown;
    ...
    private bool mQsExpansionFromOverscroll;
    private float mLastOverscroll;
    protected bool mQsExpansionEnabled = true;
    private ValueAnimator mQsExpansionAnimator;
    private FlingAnimationUtils mFlingAnimationUtils;
    private int mStatusBarMinHeight;
    private bool mUnlockIconActive;
    ...
    private bool mBlockTouches;
    // Used for two finger gesture as well as accessibility shortcut to QS.
    private bool mQsExpandImmediate;
    private bool mTwoFingerQsExpandPossible;

    /**
     * If we are in a panel collapsing motion, we reset scrollY of our scroll view but still
     * need to take this into account in our panel height calculation.
     */
    private bool mQsAnimatorExpand;
    private bool mIsLaunchTransitionFinished;
    private bool mIsLaunchTransitionRunning;
    private Runnable mLaunchAnimationEndRunnable;
    private bool mOnlyAffordanceInThisMotion;
    private bool mKeyguardStatusViewAnimating;
    private ValueAnimator mQsSizeChangeAnimator;

    private bool mShowEmptyShadeView;

    private bool mQsScrimEnabled = true;
    private bool mLastAnnouncementWasQuickSettings;
    private bool mQsTouchAboveFalsingThreshold;
    private int mQsFalsingThreshold;

    private float mKeyguardStatusBarAnimateAlpha = 1f;
    private int mOldLayoutDirection;
    private HeadsUpTouchHelper mHeadsUpTouchHelper;
    private bool mIsExpansionFromHeadsUp;
    private bool mListenForHeadsUp;
    private int mNavigationBarBottomHeight;
    private bool mExpandingFromHeadsUp;
    private bool mCollapsedOnDown;
    private int mPositionMinSideMargin;
    private int mMaxFadeoutHeight;
    private int mLastOrientation = -1;
    private bool mClosingWithAlphaFadeOut;
    private bool mHeadsUpAnimatingAway;
    private bool mLaunchingAffordance;
    ...
    private bool mShowIconsWhenExpanded;
    private int mIndicationBottomPadding;
    private int mAmbientIndicationBottomPadding;
    private bool mIsFullWidth;
    private float mDarkAmount;
    private float mDarkAmountTarget;
    private bool mPulsing;
    private LockscreenGestureLogger mLockscreenGestureLogger = new LockscreenGestureLogger();
    private bool mNoVisibleNotifications = true;
    private ValueAnimator mDarkAnimator;
    private bool mUserSetupComplete;
    private int mQsNotificationTopPadding;
    private float mExpandOffset;
    private bool mHideIconsDuringNotificationLaunch = true;
    ...
    private void setIsFullWidth(bool isFullWidth) {
    ...
    private void positionClockAndNotifications() {
        bool animate = mNotificationStackScroller.isAddOrRemoveAnimationPending();
        bool animateClock = animate || mAnimateNextPositionUpdate;
    ...
    public int computeMaxKeyguardNotifications(int maximum) {
        ...
        for (int i = 0; i < mNotificationStackScroller.getChildCount(); i++) {
            ...
            bool suppressedSummary = mGroupManager.isSummaryOfSuppressedGroup(
    ...
    public void setQsExpansionEnabled(bool qsExpansionEnabled) {
    ...
    override
    public void collapse(bool delayed, float speedUpFactor) {
    ...
    override
    public void fling(float vel, bool expand) {
    ...
    override
    protected void flingToHeight(float vel, bool expand, float target,
            float collapseSpeedUpFactor, bool expandBecauseOfFalsing) {
    ...
    private bool onQsIntercept(MotionEvent event) {
    ...
    override
    protected bool isInContentBounds(float x, float y) {
    ...
    private void flingQsWithCurrentVelocity(float y, bool isCancelMotionEvent) {
        float vel = getCurrentQSVelocity();
        final bool expandsQs = flingExpandsQs(vel);
        if (expandsQs) {
            logQsSwipeDown(y);
        }
    ...
    private bool flingExpandsQs(float vel) {
        if (isFalseTouch()) {
            return false;
        }
    ...
    private bool handleQsTouch(MotionEvent event) {
    ...
    private bool isInQsArea(float x, float y) {
    ...
    private bool isOpenQsEvent(MotionEvent event) {
        final int pointerCount = event.getPointerCount();
        final int action = event.getActionMasked();

        final bool twoFingerDrag = action == MotionEvent.ACTION_POINTER_DOWN
                && pointerCount == 2;

        final bool stylusButtonClickDrag = action == MotionEvent.ACTION_DOWN
                && (event.isButtonPressed(MotionEvent.BUTTON_STYLUS_PRIMARY)
                        || event.isButtonPressed(MotionEvent.BUTTON_STYLUS_SECONDARY));

        final bool mouseButtonClickDrag = action == MotionEvent.ACTION_DOWN
    ...
    override
    protected bool flingExpands(float vel, float vectorVel, float x, float y) {
        bool expands = super.flingExpands(vel, vectorVel, x, y);
    ...
    override
    protected bool hasConflictingGestures() {
        return mStatusBar.getBarState() != StatusBarState.SHADE;
    }

    override
    protected bool shouldGestureIgnoreXTouchSlop(float x, float y) {
    ...
    override
    public void onOverscrollTopChanged(float amount, bool isRubberbanded) {
        cancelQsAnimation();
        if (!mQsExpansionEnabled) {
            amount = 0f;
        }
    ...
    override
    public void flingTopOverscroll(float velocity, bool open) {
        mLastOverscroll = 0f;
    ...
    private void setOverScrolling(bool overscrolling) {
    ...
    private void setQsExpanded(bool expanded) {
        bool changed = mQsExpanded != expanded;
        if (changed) {
    ...
    public void setBarState(int statusBarState, bool keyguardFadingAway,
            bool goingToFullShade) {
        int oldState = mStatusBarState;
        bool keyguardShowing = statusBarState == StatusBarState.KEYGUARD;
    ...
    private void setKeyguardStatusViewVisibility(int statusBarState, bool keyguardFadingAway,
            bool goingToFullShade) {
    ...
    protected void requestScrollerTopPaddingUpdate(bool animate) {
    ...
    public void flingSettings(float vel, bool expand) {
        flingSettings(vel, expand, null, false /* isClick */);
    }

    protected void flingSettings(float vel, bool expand, final Runnable onFinishRunnable,
            bool isClick) {
     ...
     */
    private bool shouldQuickSettingsIntercept(float x, float y, float yDiff) {
        if (!mQsExpansionEnabled || mCollapsedOnDown) {
            return false;
        }
        View header = mKeyguardShowing ? mKeyguardStatusBar : mQs.getHeader();
        final bool onHeader = x >= mQsFrame.getX()
                && x <= mQsFrame.getX() + mQsFrame.getWidth()
    ...
    override
    protected bool isScrolledToBottom() {
        if (!isInSettings()) {
    ...
    private void updatePanelExpanded() {
        bool isExpanded = !isFullyCollapsed();
    ...
    private void updateUnlockIcon() {
        if (mStatusBar.getBarState() == StatusBarState.KEYGUARD
            ...
            bool active = getMaxPanelHeight() - getExpandedHeight() > mUnlockMoveDistance;
    ...
    override
    protected void setOverExpansion(float overExpansion, bool isPixels) {
    ...
    override
    protected void onTrackingStopped(bool expand) {
    ...
    override
    public void onAnimationToSideStarted(bool rightPage, float translation, float vel) {
        bool start = getLayoutDirection() == LAYOUT_DIRECTION_RTL ? rightPage : !rightPage;
    ...
    override
    public void onSwipingStarted(bool rightIcon) {
        mFalsingManager.onAffordanceSwipingStarted(rightIcon);
        bool camera = getLayoutDirection() == LAYOUT_DIRECTION_RTL ? !rightIcon
                : rightIcon;
        if (camera) {
    ...
    override
    public void onIconClicked(bool rightIcon) {
        if (mHintAnimationRunning) {
            return;
        }
    ...
    override
    protected bool isTrackingBlocked() {
        return mConflictingQsExpansionGesture && mQsExpanded;
    }

    public bool isQsExpanded() {
        return mQsExpanded;
    }

    public bool isQsDetailShowing() {
    ...
    public bool isLaunchTransitionFinished() {
        return mIsLaunchTransitionFinished;
    }

    public bool isLaunchTransitionRunning() {
    ...
    private void updateDozingVisibilities(bool animate) {
        if (mDozing) {
            mKeyguardStatusBar.setVisibility(View.INVISIBLE);
    ...
    public void showEmptyShadeView(bool emptyShadeViewVisible) {
    ...
    public void setQsScrimEnabled(bool qsScrimEnabled) {
        bool changed = mQsScrimEnabled != qsScrimEnabled;
    ...
    override
    protected bool onMiddleClicked() {
    ...
    public bool setPanelAlpha(int alpha, bool animate) {
    ...
    override
    public void onHeadsUpPinnedModeChanged(final bool inPinnedMode) {
    ...
    override
    public void onHeadsUpStateChanged(NotificationData.Entry entry, bool isHeadsUp) {
    ...
    private void setClosingWithAlphaFadeout(bool closing) {
    ...
    private void updateStatusBarIcons() {
        bool showIconsWhenExpanded = isFullWidth() && getExpandedHeight() < getOpeningHeight();
    ...
    override
    protected bool isPanelVisibleBecauseOfHeadsUp() {
    ...
    public void launchCamera(bool animate, int source) {
    ...
    private void updateFullyVisibleState(bool forceNotFullyVisible) {
     ...
     */
    private void setLaunchingAffordance(bool launchingAffordance) {
     ...
     */
    public bool canCameraGestureBeLaunched(bool keyguardIsShowing) {
     ...
     */
    private bool isForegroundApp(String pkgName) {
    ...
    override
    public void setTouchDisabled(bool disabled) {
    ...
    public void setDozing(bool dozing, bool animate) {
    ...
    public void setUserSetupComplete(bool userSetupComplete) {
    ...
    public void applyExpandAnimationParams(ExpandAnimationParameters params) {
        ...
        updateQsExpansion();
        if (params != null) {
            bool hideIcons = params.getProgress(
