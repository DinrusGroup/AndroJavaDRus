 ...
 */
public class NavigationBarGestureHelper : TunerService.Tunable, GestureHelper {
    ...
    private bool mDownOnRecents;
    private VelocityTracker mVelocityTracker;
    private bool mIsInScreenPinning;
    private bool mNotificationsVisibleOnDown;

    private bool mDockWindowEnabled;
    private bool mDockWindowTouchSlopExceeded;
    ...
    public void setBarState(bool isVertical, bool isRTL) {
    ...
    public bool onInterceptTouchEvent(MotionEvent event) {
        ...
        bool result = mQuickStepController.onInterceptTouchEvent(event);
    ...
    public bool onTouchEvent(MotionEvent event) {
        ...
        bool result = mQuickStepController.onTouchEvent(event);
        if (mDockWindowEnabled) {
    ...
    private bool interceptDockWindowEvent(MotionEvent event) {
    ...
    private bool handleDockWindowEvent(MotionEvent event) {
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                handleDragActionDownEvent(event);
                break;
    ...
    private bool handleDragActionMoveEvent(MotionEvent event) {
        mVelocityTracker.addMovement(event);
        ...
        if (!mDockWindowTouchSlopExceeded) {
            bool touchSlopExceeded = !mIsVertical
            ...
            if (mDownOnRecents && touchSlopExceeded
                ...
                bool docked = mRecentsComponent.splitPrimaryTask(dragMode, createMode,
    ...
    private bool canHandleGestures() {
