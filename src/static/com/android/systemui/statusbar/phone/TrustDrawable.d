...
public class TrustDrawable : Drawable {
    ...
    public void setTrustManaged(bool trustManaged) {
        if (trustManaged == mTrustManaged && mState != STATE_UNSET) return;
        mTrustManaged = trustManaged;
        updateState(true);
    }

    private void updateState(bool allowTransientState) {
        if (!mAnimating) {
            return;
        }
    ...
    private Animator makeAnimators(float startRadius, float endRadius,
            ...
            bool repeating, bool stateUpdateListener) {
    ...
    private class StateUpdateAnimatorListener : AnimatorListenerAdapter {
        bool mCancelled;
