...
public class NavigationBarView : FrameLayout : PluginListener<NavGesture> {
    final static bool DEBUG = false;
    final static String TAG = "StatusBar/NavBarView";

    // slippery nav bar when everything is disabled, e.g. during setup
    final static bool SLIPPERY_WHEN_DISABLED = true;

    final static bool ALTERNATE_CAR_MODE_UI = false;
    ...
    bool mShowMenu;
    bool mShowAccessibilityButton;
    bool mLongClickableAccessibilityButton;
    bool mShowRotateButton;
    ...
    private bool mDeadZoneConsuming = false;
    private final NavigationBarTransitions mBarTransitions;
    private final OverviewProxyService mOverviewProxyService;

    // workaround for LayoutTransitions leaving the nav buttons in a weird state (bug 5549288)
    final static bool WORKAROUND_INVALID_LAYOUT = true;
    ...
    private bool mLayoutTransitionsEnabled = true;
    private bool mWakeAndUnlocking;
    private bool mUseCarModeUi = false;
    private bool mInCarMode = false;
    private bool mDockedStackExists;
    ...
    private class NavTransitionListener : TransitionListener {
        private bool mBackTransitioning;
        private bool mHomeAppearing;
    ...
    private bool shouldDeadZoneConsumeTouchEvents(MotionEvent event) {
    ...
    public bool isRecentsButtonVisible() {
    ...
    public bool isQuickStepSwipeUpEnabled() {
        return mOverviewProxyService.shouldShowSwipeUpUI() && isOverviewEnabled();
    }

    public bool isQuickScrubEnabled() {
    ...
    public KeyButtonDrawable getHomeDrawable(Context lightContext, Context darkContext) {
        final bool quickStepEnabled = mOverviewProxyService.shouldShowSwipeUpUI();
    ...
    private KeyButtonDrawable getDrawable(Context lightContext, Context darkContext,
            @DrawableRes int icon, bool hasShadow) {
    ...
    private KeyButtonDrawable getBackIconWithAlt(bool carMode, bool landscape) {
        return landscape
                ? carMode ? mBackAltLandCarModeIcon : mBackIcon
                : carMode ? mBackAltCarModeIcon : mBackIcon;
    }

    private KeyButtonDrawable getBackIcon(bool carMode, bool landscape) {
        return landscape
    ...
    public void setNavigationIconHints(int hints) {
        ...
        final bool backAlt = (hints & StatusBarManager.NAVIGATION_HINT_BACK_ALT) != 0;
    ...
    public void setDisabledFlags(int disabledFlags) {
        ...
        final bool overviewEnabledBefore = isOverviewEnabled();
    ...
    public void updateNavButtonIcons() {
        ...
        bool disableHome = ((mDisabledFlags & View.STATUS_BAR_DISABLE_HOME) != 0);

        // Always disable recents when alternate car mode UI is active.
        bool disableRecent = mUseCarModeUi || !isOverviewEnabled();

        bool disableBack = ((mDisabledFlags & View.STATUS_BAR_DISABLE_BACK) != 0) && !useAltBack;

        // When screen pinning, don't hide back and home when connected service or back and
        // recents buttons when disconnected from launcher service in screen pinning mode,
        // as they are used for exiting.
        final bool pinningActive = ActivityManagerWrapper.getInstance().isScreenPinningActive();
    ...
    public bool inScreenPinning() {
        return ActivityManagerWrapper.getInstance().isScreenPinningActive();
    }

    public void setLayoutTransitionsEnabled(bool enabled) {
        mLayoutTransitionsEnabled = enabled;
        updateLayoutTransitionsEnabled();
    }

    public void setWakeAndUnlocking(bool wakeAndUnlocking) {
    ...
    private void updateLayoutTransitionsEnabled() {
        bool enabled = !mWakeAndUnlocking && mLayoutTransitionsEnabled;
        ViewGroup navButtons = (ViewGroup) getCurrentView().findViewById(R.id.nav_buttons);
        LayoutTransition lt = navButtons.getLayoutTransition();
        if (lt != null) {
            if (enabled) {
    ...
    private void setUseFadingAnimations(bool useFadingAnimations) {
        WindowManager.LayoutParams lp = (WindowManager.LayoutParams) ((ViewGroup) getParent())
                .getLayoutParams();
        if (lp != null) {
            bool old = lp.windowAnimations != 0;
    ...
    public void onPanelExpandedChange(bool expanded) {
        updateSlippery();
    }

    public void updateStates() {
        final bool showSwipeUpUI = mOverviewProxyService.shouldShowSwipeUpUI();
    ...
    public void setMenuVisibility(final bool show) {
        setMenuVisibility(show, false);
    }

    public void setMenuVisibility(final bool show, final bool force) {
        if (!force && mShowMenu == show) return;

        mShowMenu = show;

        // Only show Menu if IME switcher, rotate and Accessibility buttons are not shown.
        final bool shouldShow = mShowMenu &&
                !mShowAccessibilityButton &&
                !mShowRotateButton &&
                ((mNavigationIconHints & StatusBarManager.NAVIGATION_HINT_IME_SHOWN) == 0);
    ...
    public void setAccessibilityButtonState(final bool visible, final bool longClickable) {
    ...
    public void updateRotateSuggestionButtonStyle(@StyleRes int style, bool setIcon) {
    ...
    public int setRotateButtonVisibility(final bool visible) {
        // Never show if a11y is visible
        final bool adjVisible = visible && !mShowAccessibilityButton;
    ...
    public bool isRotateButtonVisible() { return mShowRotateButton; }
    ...
    public bool needsReorient(int rotation) {
    ...
    private void updateTaskSwitchHelper() {
        ...
        bool isRtl = (getLayoutDirection() == View.LAYOUT_DIRECTION_RTL);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        final bool newVertical = w > 0 && h > w;
    ...
    private void notifyVerticalChangedListener(bool newVertical) {
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        bool uiCarModeChanged = updateCarMode(newConfig);
        updateTaskSwitchHelper();
     ...
     */
    private bool updateCarMode(Configuration newConfig) {
        bool uiCarModeChanged = false;
        if (newConfig != null) {
            int uiMode = newConfig.uiMode & Configuration.UI_MODE_TYPE_MASK;
            final bool isCarMode = (uiMode == Configuration.UI_MODE_TYPE_CAR);
    ...
    */
    ...
    private void setUpSwipeUpOnboarding(bool connectedToOverviewProxy) {
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        final bool offscreen = r.right > size.x || r.bottom > size.y;
        pw.println("      window: "
                + r.toShortString()
    ...
    public interface OnVerticalChangedListener {
        void onVerticalChanged(bool isVertical);
    }
