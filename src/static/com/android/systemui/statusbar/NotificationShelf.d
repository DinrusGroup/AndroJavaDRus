 ...
 */
public class NotificationShelf : ActivatableNotificationView :
        View.OnLayoutChangeListener {

    public static final bool SHOW_AMBIENT_ICONS = true;
    private static final bool USE_ANIMATIONS_WHEN_OPENING =
            SystemProperties.getBoolean("debug.icon_opening_animations", true);
    private static final bool ICON_ANMATIONS_WHILE_SCROLLING
    ...
    private bool mHasItemsInStableShelf;
    ...
    private bool mNoAnimationsInThisFrame;
    private bool mAnimationsEnabled = true;
    private bool mShowNotificationShelf;
    ...
    public void updateAppearance() {
        ...
        bool backgroundForceHidden = false;
        ...
        bool scrollingFast = currentScrollVelocity > mScrollFastThreshold
                || (mAmbientState.isExpansionChanging()
                        && Math.abs(mAmbientState.getExpandingVelocity()) > mScrollFastThreshold);
        bool scrolling = currentScrollVelocity > 0;
        bool expandingAnimated = mAmbientState.isExpansionChanging()
        ...
        for (int i = 0; i < mHostLayout.getChildCount(); i++) {
            ...
            bool aboveShelf = ViewState.getFinalTranslationZ(row) > baseZHeight
                    || row.isPinned();
            bool isLastChild = child == lastChild;
        ...
        bool hideBackground = numViewsInShelf < 1.0f;
    ...
    private void updateContinuousClipping(final ExpandableNotificationRow row) {
        StatusBarIconView icon = row.getEntry().expandedIcon;
        bool needsContinuousClipping = ViewState.isAnimatingY(icon) && !mAmbientState.isDark();
        bool isContinuousClipping = icon.getTag(TAG_CONTINUOUS_CLIPPING) != null;
            ...
            ViewTreeObserver.OnPreDrawListener predrawListener =
                    new ViewTreeObserver.OnPreDrawListener() {
                        ...
                        public bool onPreDraw() {
                            bool animatingY = ViewState.isAnimatingY(icon);
                            if (!animatingY) {
    ...
    private void updateNotificationClipHeight(ExpandableNotificationRow row,
        ...
        bool isPinned = (row.isPinned() || row.isHeadsUpAnimatingAway())
    ...
    private float updateIconAppearance(ExpandableNotificationRow row, float expandAmount,
            bool scrolling, bool scrollingFast, bool expandingAnimated,
            bool isLastChild) {
        StatusBarIconView icon = row.getEntry().expandedIcon;
        NotificationIconContainer.IconState iconState = getIconState(icon);
        if (iconState == null) {
        ...
        if (expandingAnimated && mAmbientState.getScrollY() == 0
            ...
            if (position < maxShelfStart && position + row.getIntrinsicHeight() >= maxShelfStart
                ...
                bool forceInShelf = mMaxLayoutHeight - getIntrinsicHeight() - position
                        < getIntrinsicHeight();
                if (!forceInShelf) {
    ...
    private void updateIconPositioning(ExpandableNotificationRow row, float iconTransitionAmount,
            float fullTransitionAmount, float iconTransformDistance, bool scrolling,
            bool scrollingFast, bool expandingAnimated, bool isLastChild) {
        StatusBarIconView icon = row.getEntry().expandedIcon;
        NotificationIconContainer.IconState iconState = getIconState(icon);
        if (iconState == null) {
            return;
        }
        bool forceInShelf = iconState.isLastExpandIcon && !iconState.hasCustomTransformHeight();
    ...
    private void setIconTransformationAmount(ExpandableNotificationRow row,
            float transitionAmount, float iconTransformDistance, bool usingLinearInterpolation,
            bool isLastChild) {
        StatusBarIconView icon = row.getEntry().expandedIcon;
        NotificationIconContainer.IconState iconState = getIconState(icon);
        ...
        bool stayingInShelf = row.isInShelf() && !row.isTransformingIntoShelf();
        ...
        if (iconState != null) {
            ...
            bool isAppearing = row.isDrawingAppearAnimation() && !row.isInShelf();
            if (isAppearing) {
    ...
    private void setHideBackground(bool hideBackground) {
    ...
    public bool hidesBackground() {
        return mHideBackground;
    }

    override
    protected bool needsOutline() {
    ...
    private void setOpenedAmount(float openedAmount) {
        ...
        bool hasOverflow = mCollapsedIcons.hasOverflow();
    ...
    private void setHasItemsInStableShelf(bool hasItemsInStableShelf) {
     ...
     */
    public bool hasItemsInStableShelf() {
    ...
    private class ShelfState : ExpandableViewState {
        private float openedAmount;
        private bool hasItemsInStableShelf;
