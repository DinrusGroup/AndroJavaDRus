...
public class TransformState {
    ...
    private static ViewClippingUtil.ClippingParameters CLIPPING_PARAMETERS
        ...
        override
        public bool shouldFinish(View view) {
        ...
        override
        public void onClippingStateChanged(View view, bool isClipping) {
            if (view instanceof ExpandableNotificationRow) {
                ExpandableNotificationRow row = (ExpandableNotificationRow) view;
                if (isClipping) {
    ...
    protected void transformViewFrom(TransformState otherState, int transformationFlags,
        ...
        bool transformX = (transformationFlags & TRANSFORM_X) != 0;
        bool transformY = (transformationFlags & TRANSFORM_Y) != 0;
        int viewHeight = getViewHeight();
        int otherHeight = otherState.getViewHeight();
        bool differentHeight = otherHeight != viewHeight && otherHeight != 0 && viewHeight != 0;
        int viewWidth = getViewWidth();
        int otherWidth = otherState.getViewWidth();
        bool differentWidth = otherWidth != viewWidth && otherWidth != 0 && viewWidth != 0;
        bool transformScale = transformScale(otherState) && (differentHeight || differentWidth);
    ...
    protected bool transformScale(TransformState otherState) {
        return false;
    }
     ...
     */
    public bool transformViewTo(TransformState otherState, float transformationAmount) {
        mTransformedView.animate().cancel();
        if (sameAs(otherState)) {
    ...
    private void transformViewTo(TransformState otherState, int transformationFlags,
        ...
        bool transformX = (transformationFlags & TRANSFORM_X) != 0;
        bool transformY = (transformationFlags & TRANSFORM_Y) != 0;
        bool transformScale = transformScale(otherState);
        // lets animate the positions correctly
        if (transformationAmount == 0.0f) {
            if (transformX) {
    ...
    protected void setClippingDeactivated(final View transformedView, bool deactivated) {
    ...
    public void setIsSameAsAnyView(bool sameAsAny) {
    ...
    public void setVisible(bool visible, bool force) {
