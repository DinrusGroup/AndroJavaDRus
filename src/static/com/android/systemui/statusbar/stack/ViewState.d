...
public class ViewState {
    ...
    public void applyToView(View view) {
        ...
        // apply xTranslation
        bool animatingX = isAnimating(view, TAG_ANIMATOR_TRANSLATION_X);
        if (animatingX) {
        ...
        // apply yTranslation
        bool animatingY = isAnimating(view, TAG_ANIMATOR_TRANSLATION_Y);
        if (animatingY) {
        ...
        // apply zTranslation
        bool animatingZ = isAnimating(view, TAG_ANIMATOR_TRANSLATION_Z);
        if (animatingZ) {
        ...
        // apply scaleX
        bool animatingScaleX = isAnimating(view, SCALE_X_PROPERTY);
        if (animatingScaleX) {
        ...
        // apply scaleY
        bool animatingScaleY = isAnimating(view, SCALE_Y_PROPERTY);
        if (animatingScaleY) {
        ...
        bool becomesInvisible = this.alpha == 0.0f
                || (this.hidden && (!isAnimating(view) || oldVisibility != View.VISIBLE));
        bool animatingAlpha = isAnimating(view, TAG_ANIMATOR_ALPHA);
        if (animatingAlpha) {
            updateAlphaAnimation(view);
        } else if (view.getAlpha() != this.alpha) {
            // apply layer type
            bool becomesFullyVisible = this.alpha == 1.0f;
            bool newLayerTypeIsHardware = !becomesInvisible && !becomesFullyVisible
    ...
    public bool isAnimating(View view) {
    ...
    private static bool isAnimating(View view, int tag) {
        return getChildTag(view, tag) != null;
    }

    public static bool isAnimating(View view, AnimatableProperty property) {
    ...
    public void animateTo(View child, AnimationProperties animationProperties) {
        bool wasVisible = child.getVisibility() == View.VISIBLE;
        ...
        bool alphaChanging = this.alpha != childAlpha;
    ...
    public static bool isAnimatingY(View child) {
