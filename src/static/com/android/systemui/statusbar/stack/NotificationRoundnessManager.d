 ...
 */
class NotificationRoundnessManager : OnHeadsUpChangedListener {
    ...
    public void onHeadsupAnimatingAwayChanged(ExpandableNotificationRow row,
            bool isAnimatingAway) {
        updateRounding(row, false /* animate */);
    }

    private void updateRounding(ActivatableNotificationView view, bool animate) {
        float topRoundness = getRoundness(view, true /* top */);
        float bottomRoundness = getRoundness(view, false /* top */);
        bool firstChanged = view.setTopRoundness(topRoundness, animate);
        bool secondChanged = view.setBottomRoundness(bottomRoundness, animate);
        if ((view == mFirst || view == mLast) && (firstChanged || secondChanged)) {
            mRoundingChangedCallback.run();
        }
    }

    private float getRoundness(ActivatableNotificationView view, bool top) {
    ...
    public void setFirstAndLastBackgroundChild(ActivatableNotificationView first,
        ...
        bool firstChanged = mFirst != first;
        bool lastChanged = mLast != last;
