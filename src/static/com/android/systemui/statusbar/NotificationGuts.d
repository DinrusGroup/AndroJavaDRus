 ...
 */
public class NotificationGuts : FrameLayout {
    ...
    private bool mExposed;

    private Handler mHandler;
    private Runnable mFalsingCheck;
    private bool mNeedsFalsingProtection;
    ...
    public interface GutsContent {
         ...
         */
        public bool handleCloseControls(bool save, bool force);
         ...
         */
        public default bool isLeavebehind() {
            return false;
        }
    ...
    public void openControls(
            bool shouldDoCircularReveal,
            int x,
            int y,
            bool needsFalsingProtection,
     ...
     */
    public void closeControls(bool leavebehinds, bool controls, int x, int y, bool force) {
     ...
     * @param force whether the guts should be force-closed regardless of state.
     */
    public void closeControls(int x, int y, bool save, bool force) {
        // First try to dismiss any blocking helper.
        bool wasBlockingHelperDismissed =
    ...
    private void animateOpen(
            bool shouldDoCircularReveal, int x, int y, @Nullable Runnable onAnimationEnd) {
    ...
    @VisibleForTesting
    void animateClose(int x, int y, bool shouldDoCircularReveal) {
        if (isAttachedToWindow()) {
            if (shouldDoCircularReveal) {
                // Circular reveal originating at (x, y)
                if (x == -1 || y == -1) {
    ...
    @VisibleForTesting
    void setExposed(bool exposed, bool needsFalsingProtection) {
        final bool wasExposed = mExposed;
        mExposed = exposed;
    ...
    public bool isLeavebehind() {
        return mGutsContent != null && mGutsContent.isLeavebehind();
    }

    /** Listener for animations executed in {@link #animateOpen(bool, int, int, Runnable)}. */
    private static class AnimateOpenListener : AnimatorListenerAdapter {
    ...
    /** Listener for animations executed in {@link #animateClose(int, int, bool)}. */
    private static class AnimateCloseListener : AnimatorListenerAdapter {
        final View mView;
