...
public class SwipeHelper : Gefingerpoken {
    static final String TAG = "com.android.systemui.SwipeHelper";
    private static final bool DEBUG = false;
    private static final bool DEBUG_INVALIDATE = false;
    private static final bool SLOW_ANIMATIONS = false; // DEBUG;
    private static final bool CONSTRAIN_SWIPE = true;
    private static final bool FADE_OUT_DURING_SWIPE = true;
    private static final bool DISMISS_IF_SWIPED_FAR_ENOUGH = true;
    ...
    private bool mSnappingChild;
    private View mCurrView;
    private bool mCanCurrViewBeDimissed;
    private float mDensityScale;
    private float mTranslation = 0;

    private bool mMenuRowIntercepting;
    ...
    private bool mDisableHwLayers;
    private final bool mFadeDependingOnAmountSwiped;
    ...
    public void setDisableHardwareLayers(bool disableHwLayers) {
    ...
    private void updateSwipeProgressFromOffset(View animView, bool dismissable) {
    ...
    override
    public bool onInterceptTouchEvent(final MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                final bool captured = (mDragging || mLongPressSent || mMenuRowIntercepting);
                mDragging = false;
                mCurrView = null;
                mLongPressSent = false;
     ...
     */
    public void dismissChild(final View view, float velocity, bool useAccelerateInterpolator) {
    ...
    public void dismissChild(final View animView, float velocity, final Runnable endAction,
            long delay, bool useAccelerateInterpolator, long fixedDuration,
            bool isDismissAll) {
        final bool canBeDismissed = mCallback.canChildBeDismissed(animView);
        float newPos;
        bool isLayoutRtl = animView.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL;

        // if we use the Menu to dismiss an item in landscape, animate up
        bool animateUpForMenu = velocity == 0 && (getTranslation(animView) == 0 || isDismissAll)
                && mSwipeDirection == Y;
        // if the language is rtl we prefer swiping to the left
        bool animateLeftForRtl = velocity == 0 && (getTranslation(animView) == 0 || isDismissAll)
                && isLayoutRtl;
        bool animateLeft = (Math.abs(velocity) > getEscapeVelocity() && velocity < 0) ||
     ...
     */
    public void onTranslationUpdate(View animView, float value, bool canBeDismissed) {
        updateSwipeProgressFromOffset(animView, canBeDismissed, value);
    }

    private void snapChildInstantly(final View view) {
        final bool canAnimViewBeDismissed = mCallback.canChildBeDismissed(view);
     ...
     */
    public void snapChildIfNeeded(final View view, bool animate, float targetLeft) {
    ...
    public bool isFalseGesture(MotionEvent ev) {
        bool falsingDetected = mCallback.isAntiFalsingNeeded();
    ...
    protected bool swipedFastEnough() {
        float velocity = getVelocity(mVelocityTracker);
        float translation = getTranslation(mCurrView);
        bool ret = (Math.abs(velocity) > getEscapeVelocity())
                && (velocity > 0) == (translation > 0);
        return ret;
    }

    protected bool handleUpEvent(MotionEvent ev, View animView, float velocity,
            float translation) {
        return false;
    }
    ...
    public interface Callback {
        ...
        bool canChildBeDismissed(View v);

        bool isAntiFalsingNeeded();
         ...
         */
        bool updateSwipeProgress(View animView, bool dismissable, float swipeProgress);
         ...
         */
        default bool canChildBeDragged(@NonNull View animView) { return true; }
    }
}
