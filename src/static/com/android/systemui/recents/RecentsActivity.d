 ...
 */
public class RecentsActivity : Activity : ViewTreeObserver.OnPreDrawListener,
    ...
    private bool mFinishedOnStartup;
    private bool mIgnoreAltTabRelease;
    private bool mIsVisible;
    private bool mRecentsStartRequested;
     ...
     */
    class LaunchHomeRunnable : Runnable {
     ...
     */
    bool dismissRecentsToFocusedTask(int logCategory) {
     ...
     */
    bool dismissRecentsToLaunchTargetTaskOrHome() {
        SystemServicesProxy ssp = Recents.getSystemServices();
        if (ssp.isRecentsActivityVisible()) {
            // If we have a focused Task, launch that Task now
     ...
     * Dismisses recents if we are already visible and the intent is to toggle the recents view.
     */
    bool dismissRecentsToFocusedTaskOrHome() {
        SystemServicesProxy ssp = Recents.getSystemServices();
        if (ssp.isRecentsActivityVisible()) {
            // If we have a focused Task, launch that Task now
     ...
     */
    void dismissRecentsToHome(bool animateTaskViews) {
     ...
     */
    void dismissRecentsToHome(bool animateTaskViews, ActivityOptions overrideAnimation) {
    ...
    bool dismissRecentsToHomeIfVisible(bool animated) {
        SystemServicesProxy ssp = Recents.getSystemServices();
        if (ssp.isRecentsActivityVisible()) {
            // Return to Home
    ...
    protected void onStart() {
        ...
        if (isRelaunching != null && isRelaunching instanceof Boolean && (bool) isRelaunching) {
    ...
    public void onColorsChanged(ColorExtractor colorExtractor, int which) {
        if ((which & WallpaperManager.FLAG_SYSTEM) != 0) {
            ...
            bool darkText = colors.supportsDarkText();
    ...
    private void reloadStackView() {
        ...
        bool animateNavBarScrim = !launchState.launchedViaDockGesture;
        ...
        bool wasLaunchedByAm = !launchState.launchedFromHome &&
    ...
    public bool onKeyDown(int keyCode, KeyEvent event) {
        switch (keyCode) {
            case KeyEvent.KEYCODE_TAB: {
                ...
                bool hasRepKeyTimeElapsed = (SystemClock.elapsedRealtime() -
                        mLastTabKeyEventTime) > altTabKeyDelay;
                if (event.getRepeatCount() <= 0 || hasRepKeyTimeElapsed) {
                    // Focus the next task in the stack
                    final bool backward = event.isShiftPressed();
                    if (backward) {
    ...
    private void reloadTaskStack(bool isInMultiWindowMode, bool sendConfigChangedEvent) {
        ...
        bool showDeferredAnimation = numStackTasks > 0;
