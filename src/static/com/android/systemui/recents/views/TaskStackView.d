...
public class TaskStackView : FrameLayout : TaskStack.TaskStackCallbacks,
    ...
    private bool mTaskViewsClipDirty = true;
    @ViewDebug.ExportedProperty(category="recents")
    private bool mEnterAnimationComplete = false;
    @ViewDebug.ExportedProperty(category="recents")
    private bool mStackReloaded = false;
    @ViewDebug.ExportedProperty(category="recents")
    private bool mFinishedLayoutAfterStackReload = false;
    @ViewDebug.ExportedProperty(category="recents")
    private bool mLaunchNextAfterFirstMeasure = false;
    @ViewDebug.ExportedProperty(category="recents")
    @InitialStateAction
    private int mInitialState = INITIAL_STATE_UPDATE_ALL;
    @ViewDebug.ExportedProperty(category="recents")
    private bool mInMeasureLayout = false;
    @ViewDebug.ExportedProperty(category="recents")
    bool mTouchExplorationEnabled;
    @ViewDebug.ExportedProperty(category="recents")
    bool mScreenPinningEnabled;
    ...
    private bool mResetToInitialStateWhenResized;
    private int mLastWidth;
    private int mLastHeight;
    private bool mStackActionButtonVisible;
    ...
    private DropTarget mStackDropTarget = new DropTarget() {
        override
        public bool acceptsDrop(int x, int y, int width, int height, Rect insets,
     ...
     */
    void onReload(bool isResumingFromVisible) {
     ...
     */
    public void setTasks(TaskStack stack, bool allowNotifyStackChanges) {
        bool isInitialized = mLayoutAlgorithm.isInitialized();
    ...
    int[] computeVisibleTaskTransforms(ArrayList<TaskViewTransform> taskTransforms,
            ...
            ArraySet<Task.TaskKey> ignoreTasksSet, bool ignoreTaskOverrides) {
        int taskCount = tasks.size();
        int[] visibleTaskRange = mTmpIntPair;
        visibleTaskRange[0] = -1;
        visibleTaskRange[1] = -1;
        bool useTargetStackScroll = Float.compare(curStackScroll, targetStackScroll) != 0;
     ...
     */
    void bindVisibleTaskViews(float targetStackScroll, bool ignoreTaskOverrides) {
    ...
    /**
     * @see #relayoutTaskViews(AnimationProps, ArrayMap<Task, AnimationProps>, bool)
     */
    ...
    private void relayoutTaskViews(AnimationProps animation,
            ArrayMap<Task, AnimationProps> animationOverrides, bool ignoreTaskOverrides) {
    ...
    public void getLayoutTaskTransforms(float stackScroll, int focusState, ArrayList<Task> tasks,
            bool ignoreTaskOverrides, ArrayList<TaskViewTransform> transformsOut) {
        matchTaskListSize(tasks, transformsOut);
        for (int i = tasks.size() - 1; i >= 0; i--) {
            Task task = tasks.get(i);
            TaskViewTransform transform = transformsOut.get(i);
    ...
    public void updateLayoutAlgorithm(bool boundScrollToNewMinMax) {
   ...
   public void updateLayoutAlgorithm(bool boundScrollToNewMinMax,
     ...
     */
    public bool setFocusedTask(int taskIndex, bool scrollToTask,
            final bool requestViewFocus) {
     ...
     * @return whether or not the stack will scroll as a part of this focus change
     */
    public bool setFocusedTask(int focusTaskIndex, bool scrollToTask,
            bool requestViewFocus, int timerIndicatorDuration) {
     ...
     *                            focus.
     */
    public void setRelativeFocusedTask(bool forward, bool stackTasksOnly, bool animated) {
     ...
     */
    public void setRelativeFocusedTask(bool forward, bool stackTasksOnly, bool animated,
                                       bool cancelWindowAnimations, int timerIndicatorDuration) {
        ...
        if (newIndex != -1) {
            bool willScroll = setFocusedTask(newIndex, true /* scrollToTask */,
    ...
    override
    public bool onGenericMotionEvent(MotionEvent ev) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        bool resetToInitialState = (width != mLastWidth || height != mLastHeight)
     ...
     * Lays out a TaskView.
     */
    private void layoutTaskView(bool changed, TaskView tv) {
        if (changed) {
            Rect padding = new Rect();
            if (tv.getBackground() != null) {
                tv.getBackground().getPadding(padding);
    ...
    void onFirstLayout() {
        ...
        bool setFocusOnFirstLayout = !useGridLayout() || launchState.launchedWithAltTab;
    ...
    public bool isTouchPointInView(float x, float y, TaskView tv) {
    ...
    public void onStackTaskRemoved(TaskStack stack, Task removedTask, Task newFrontMostTask,
            AnimationProps animation, bool fromDockGesture, bool dismissRecentsIfAllRemoved) {
    ...
    override
    public void onPickUpViewFromPool(TaskView tv, Task task, bool isNewView) {
    ...
    override
    public bool hasPreferredData(TaskView tv, Task preferredData) {
    ...
    private void updatePrefetchingTask(ArrayList<Task> tasks, int frontIndex, int backIndex) {
        Task t = null;
        bool somethingVisible = frontIndex != -1 && backIndex != -1;
    ...
    public final void onBusEvent(DragDropTargetChangedEvent event) {
        ...
        bool ignoreTaskOverrides = false;
