...
class Range {
     ...
     */
    public bool isInRange(float absX) {
...
public class TaskStackLayoutAlgorithm {
     ...
     */
    public bool setSystemInsets(Rect systemInsets) {
        bool changed = !mSystemInsets.equals(systemInsets);
    ...
    public void update(TaskStack stack, ArraySet<Task.TaskKey> ignoreTasksSet,
        ...
        if (getInitialFocusState() == STATE_FOCUSED) {
            ...
            bool scrollToFront = launchState.launchedFromHome || launchState.launchedFromPipApp
     ...
     */
    public void setTaskOverridesForInitialState(TaskStack stack, bool ignoreScrollToFront) {
        RecentsActivityLaunchState launchState = Recents.getConfiguration().getLaunchState();

        mTaskIndexOverrideMap.clear();

        bool scrollToFront = launchState.launchedFromHome ||
    ...
    private bool isInvalidOverrideX(float x, float overrideX, float newOverrideX) {
        bool outOfBounds = mUnfocusedRange.getNormalizedX(newOverrideX) < 0f ||
    ...
    public VisibilityReport computeStackVisibilityReport(ArrayList<Task> tasks) {
        ...
        for (int i = tasks.size() - 1; i >= 0; i--) {
            ...
            bool hasVisibleThumbnail = (prevScreenY - screenY) > taskBarHeight;
    ...
    public TaskViewTransform getStackTransform(Task task, float stackScroll,
            TaskViewTransform transformOut, TaskViewTransform frontTransform,
            bool ignoreTaskOverrides) {
        return getStackTransform(task, stackScroll, mFocusState, transformOut, frontTransform,
                false /* forceUpdate */, ignoreTaskOverrides);
    }

    public TaskViewTransform getStackTransform(Task task, float stackScroll, int focusState,
            TaskViewTransform transformOut, TaskViewTransform frontTransform, bool forceUpdate,
            bool ignoreTaskOverrides) {
        if (useGridLayout()) {
    ...
    public void getStackTransform(float taskProgress, float nonOverrideTaskProgress,
            ...
            TaskViewTransform frontTransform, bool ignoreSingleTaskCase, bool forceUpdate) {
        SystemServicesProxy ssp = Recents.getSystemServices();

        // Ensure that the task is in range
        mUnfocusedRange.offset(stackScroll);
        mFocusedRange.offset(stackScroll);
        bool unfocusedVisible = mUnfocusedRange.isInRange(taskProgress);
        bool focusedVisible = mFocusedRange.isInRange(taskProgress);
    ...
    public static int getDimensionForDevice(Context ctx, int phonePortResId, int phoneLandResId,
        ...
        bool isLandscape = Utilities.getAppConfiguration(ctx).orientation ==
                Configuration.ORIENTATION_LANDSCAPE;
