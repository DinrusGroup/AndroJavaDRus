 ...
 */
public class RecentsImpl : ActivityOptions.OnAnimationFinishedListener {
    ...
    private static bool mWaitingForTransitionStart = false;
    ...
    private static bool mToggleFollowingTransitionStart = true;
    ...
    bool mDraggingInRecents;
    bool mLaunchedWhileDocking;
    ...
    protected bool mTriggeredFromAltTab;
     ...
     */
    public void onVisibilityChanged(Context context, bool visible) {
    ...
    public void showRecents(bool triggeredFromAltTab, bool draggingInRecents,
            bool animate, int growTarget) {
        final SystemServicesProxy ssp = Recents.getSystemServices();
        final MutableBoolean isHomeStackVisible = new MutableBoolean(true);
        final bool isRecentsVisible = Recents.getSystemServices().isRecentsActivityVisible(
                isHomeStackVisible);
        final bool fromHome = isHomeStackVisible.value;
        final bool launchedWhileDockingTask =
        ...
        try {
            ...
            final bool forceVisible = launchedWhileDockingTask || draggingInRecents;
    ...
    public void hideRecents(bool triggeredFromAltTab, bool triggeredFromHomeKey) {
            ...
            if (ssp.isRecentsActivityVisible(isHomeStackVisible)) {
                ...
                if (!launchState.launchedWithAltTab) {
                    if (Recents.getConfiguration().isGridEnabled) {
                        ...
                        bool isQuickTap = elapsedTime < ViewConfiguration.getDoubleTapTimeout();
                        if (isQuickTap) {
     ...
     */
    public void showRelativeAffiliatedTask(bool showNextTask) {
        SystemServicesProxy ssp = Recents.getSystemServices();
        RecentsTaskLoader loader = Recents.getTaskLoader();
        RecentsTaskLoadPlan plan = new RecentsTaskLoadPlan(mContext);
        loader.preloadTasks(plan, -1);
    ...
    public void setWaitingForTransitionStart(bool waitingForTransitionStart) {
    ...
    private void updateHeaderBarLayout(TaskStack stack, Rect windowRectOverride) {
        ...
        int taskViewWidth = 0;
        bool useGridLayout = mDummyStackView.useGridLayout();
    ...
    private Pair<ActivityOptions, AppTransitionAnimationSpecsFuture>
        ...
        final bool isLowRamDevice = Recents.getConfiguration().isLowRamDevice;
    ...
    private Bitmap drawThumbnailTransitionBitmap(Task toTask,
        ...
        if (toTransform != null && toTask.key != null && width > 0 && height > 0) {
            synchronized (mHeaderBarLock) {
                bool disabledInSafeMode = !toTask.isSystemApp && ssp.isInSafeMode();
    ...
    protected void startRecentsActivityAndDismissKeyguardIfNeeded(
            final ActivityManager.RunningTaskInfo runningTask, final bool isHomeStackVisible,
            final bool animate, final int growTarget) {
    ...
    private void startRecentsActivity(ActivityManager.RunningTaskInfo runningTask,
            bool isHomeStackVisible, bool animate, int growTarget) {
        RecentsTaskLoader loader = Recents.getTaskLoader();
        ...
        TaskStack stack = sInstanceLoadPlan.getTaskStack();
        bool hasRecentTasks = stack.getTaskCount() > 0;
        bool useThumbnailTransition = (runningTask != null) && !isHomeStackVisible &&
                hasRecentTasks;
