...
/**
 * Represents a subscriber, which : various event bus handler methods.
 */
class Subscriber {
...
class EventHandlerMethod {
    ...
    Class<? : EventBus.Event> eventType;

    EventHandlerMethod(Method method, Class<? : EventBus.Event> eventType) {
        mMethod = method;
 ...
 * }
 * -keepclassmembers class ** : **.EventBus$InterprocessEvent {
 ...
 * </p>
 */
public class EventBus : BroadcastReceiver {

    private static final String TAG = "EventBus";
    private static final bool DEBUG_TRACE_ALL = false;
     ...
     */
    public static class Event : Cloneable {
     ...
     */
    public static class AnimatedEvent : Event {
     ...
     */
    public static class ReusableEvent : Event {
     ...
     * invocations.
     */
    public static class InterprocessEvent : Event {
    ...
    private bool mHasRegisteredReceiver;

    /**
     * Map from event class -> event handler list.  Keeps track of the actual mapping from event
     * to subscriber method.
     */
    private HashMap<Class<? : Event>, ArrayList<EventHandler>> mEventTypeMap = new HashMap<>();
     ...
     */
    private HashMap<Class<? : Object>, ArrayList<EventHandlerMethod>> mSubscriberTypeMap = new HashMap<>();
     ...
     */
    private HashMap<String, Class<? : InterprocessEvent>> mInterprocessEventNameMap = new HashMap<>();
    ...
    public void unregister(Object subscriber) {
        ...
        Class!(T) subscriberType = subscriber.getClass();
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        Class<? : InterprocessEvent> eventType = mInterprocessEventNameMap.get(intent.getAction());
        try {
            Constructor<? : InterprocessEvent> ctor = eventType.getConstructor(Bundle.class);
    ...
    public String dumpInternal(String prefix) {
        ...
        output.append("\n");
        ArrayList<Class!(T)> subsciberTypes = new ArrayList<>(mSubscriberTypeMap.keySet());
        Collections.sort(subsciberTypes, new Comparator<Class!(T)>() {
            override
        ...
        for (int i = 0; i < subsciberTypes.size(); i++) {
            Class!(T) clz = subsciberTypes.get(i);
        ...
        output.append("\n");
        ArrayList<Class!(T)> classes = new ArrayList<>(mEventTypeMap.keySet());
        Collections.sort(classes, new Comparator<Class!(T)>() {
            override
            public int compare(Class!(T) o1, Class!(T) o2) {
                return o1.getSimpleName().compareTo(o2.getSimpleName());
            }
    ...
    private void registerSubscriber(Object subscriber, int priority,
        ...
        for (Method m : methods) {
            Class!(T)[] parameterTypes = m.getParameterTypes();
                ...
                if (isInterprocessEvent.value) {
                    try {
                        ...
                        mInterprocessEventNameMap.put(eventType.getName(),
                                (Class<? : InterprocessEvent>) eventType);
     ...
     */
    private bool findRegisteredSubscriber(Object subscriber, bool removeFoundSubscriber) {
     ...
     */
    private bool isValidEventBusHandlerMethod(Method method, Class!(T)[] parameterTypes,
