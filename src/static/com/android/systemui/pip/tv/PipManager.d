 ...
 */
public class PipManager : BasePipManager {
     ...
     */
    private class PinnedStackListener : IPinnedStackListener.Stub {
        ...
        override
        public void onMinimizedStateChanged(bool isMinimized) {}

        override
        public void onMovementBoundsChanged(Rect insetBounds, Rect normalBounds,
                Rect animatingBounds, bool fromImeAdjustment, bool fromShelfAdjustment,
                int displayRotation) {
            mHandler.post(() -> {
    ...
    private void closePipInternal(bool removePipStack) {
        mState = STATE_NO_PIP;
    ...
    void resizePinnedStack(int state) {
        ...
        bool wasStateNoPip = (mState == STATE_NO_PIP);
        for (int i = mListeners.size() - 1; i >= 0; --i) {
    ...
    private void handleMediaResourceGranted(String[] packageNames) {
        if (getState() == STATE_NO_PIP) {
            ...
            bool requestedFromLastPackages = false;
    ...
    int getPlaybackState() {
        ...
        bool isPlaying = (state == PlaybackState.STATE_BUFFERING
    ...
    private SysUiTaskStackChangeListener mTaskStackListener = new SysUiTaskStackChangeListener() {
        ...
        override
        public void onPinnedActivityRestartAttempt(bool clearedTask) {
    ...
    private void updatePipVisibility(final bool visible) {
