...
public class PipTouchHandler {
    ...
    private static final bool ENABLE_MINIMIZE = false;
    // Allow the PIP to be flung from anywhere on the screen to the bottom to be dismissed.
    private static final bool ENABLE_FLING_DISMISS = false;

    private static final int SHOW_DISMISS_AFFORDANCE_DELAY = 225;

    // Allow dragging the PIP to a location to close it
    private static final bool ENABLE_DISMISS_DRAG_TO_EDGE = true;
    ...
    private bool mShowPipMenuOnAnimationEnd = false;
    ...
    private bool mIsMinimized;
    private bool mIsImeShowing;
    private int mImeHeight;
    private int mImeOffset;
    private bool mIsShelfShowing;
    private int mShelfHeight;
    private float mSavedSnapFraction = -1f;
    private bool mSendingHoverAccessibilityEvents;
    private bool mMovementWithinMinimize;
    private bool mMovementWithinDismiss;
     ...
     */
    private class PipMenuListener : PipMenuActivityController.Listener {
        override
        public void onPipMenuStateChanged(int menuState, bool resize) {
    ...
    public void onImeVisibilityChanged(bool imeVisible, int imeHeight) {
        mIsImeShowing = imeVisible;
        mImeHeight = imeHeight;
    }

    public void onShelfVisibilityChanged(bool shelfVisible, int shelfHeight) {
    ...
    public void onMovementBoundsChanged(Rect insetBounds, Rect normalBounds, Rect animatingBounds,
            bool fromImeAdjustment, bool fromShelfAdjustment, int displayRotation) {
     ...
     */
    private void setMinimizedStateInternal(bool isMinimized) {
        if (!ENABLE_MINIMIZE) {
            return;
        }
     ...
     * Sets the minimized state.
     */
    void setMinimizedState(bool isMinimized, bool fromController) {
        if (!ENABLE_MINIMIZE) {
            return;
        }
     ...
     */
    private void setMenuState(int menuState, bool resize) {
    ...
    private PipTouchGesture mDefaultMovementGesture = new PipTouchGesture() {
        ...
        override
        bool onMove(PipTouchState touchState) {
            if (!touchState.isUserInteracting()) {
                return false;
            }
        ...
        override
        public bool onUp(PipTouchState touchState) {
            if (ENABLE_DISMISS_DRAG_TO_EDGE) {
            ...
            final bool isHorizontal = Math.abs(vel.x) > Math.abs(vel.y);
            final float velocity = PointF.length(vel.x, vel.y);
            final bool isFling = velocity > mFlingAnimationUtils.getMinVelocityPxPerSecond();
            final bool isUpWithinDimiss = ENABLE_FLING_DISMISS
                    ...
                    && mMotionHelper.isGestureToDismissArea(mMotionHelper.getBounds(), vel.x,
                            vel.y, isFling);
            final bool isFlingToBot = isFling && vel.y > 0 && !isHorizontal
            ...
            if (touchState.isDragging()) {
                final bool isFlingToEdge = isFling && isHorizontal && mMovementWithinMinimize
    ...
    private void updateMovementBounds(int menuState) {
        bool isMenuExpanded = menuState == MENU_STATE_FULL;
