...
public class PowerUI : SystemUI {
    ...
    private bool mLowWarningShownThisChargeCycle;
    private bool mSevereWarningShownThisChargeCycle;
    ...
    @VisibleForTesting
    final class Receiver : BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (PowerManager.ACTION_POWER_SAVE_MODE_CHANGED.equals(action)) {
                ...
                final bool plugged = mPlugType != 0;
                final bool oldPlugged = oldPlugType != 0;
    ...
    protected void maybeShowBatteryWarning(bool plugged, bool oldPlugged, int oldBucket,
        int bucket) {
        bool isPowerSaver = mPowerManager.isPowerSaveMode();
        // only play SFX when the dialog comes up or the bucket changes
        final bool playSound = bucket != oldBucket || oldPlugged;
        final bool hybridEnabled = mEnhancedEstimates.isHybridNotificationEnabled();
        if (hybridEnabled) {
    ...
    @VisibleForTesting
    bool shouldShowLowBatteryWarning(bool plugged, bool oldPlugged, int oldBucket,
            int bucket, long timeRemaining, bool isPowerSaver, int batteryStatus) {
    ...
    @VisibleForTesting
    bool shouldDismissLowBatteryWarning(bool plugged, int oldBucket, int bucket,
            long timeRemaining, bool isPowerSaver) {
        final bool hybridWouldDismiss = mEnhancedEstimates.isHybridNotificationEnabled()
                && timeRemaining > mEnhancedEstimates.getLowWarningThreshold();
        final bool standardWouldDismiss = (bucket > oldBucket && bucket > 0);
        return isPowerSaver
                || plugged
    ...
    private bool isEnhancedTrigger(bool plugged, long timeRemaining, bool isPowerSaver,
            int batteryStatus) {
        ...
        final bool canShowWarning = !mLowWarningShownThisChargeCycle
                && (timeRemaining < mEnhancedEstimates.getLowWarningThreshold()
                        || mBatteryLevel <= warnLevel);

        // Only show the severe warning once per charge cycle
        final bool canShowSevereWarning = !mSevereWarningShownThisChargeCycle
    ...
    public interface WarningsUI {
        ...
        void showLowBatteryWarning(bool playSound);
        void dismissInvalidChargerWarning();
        void showInvalidChargerWarning();
        void updateLowBatteryWarning();
        bool isInvalidChargerWarningShowing();
    ...
    private final class ThermalEventListener : IThermalEventListener.Stub {
        override public void notifyThrottling(bool isThrottling, Temperature temp) {
