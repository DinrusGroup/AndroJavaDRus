...
public class PageIndicator : ViewGroup {
    ...
    private void animate(int from, int to) {
        ...
        bool fromTransition = (from & 1) != 0;
        bool isAState = fromTransition ? from > to : from < to;
    ...
    private float getAlpha(bool isMajor) {
    ...
    private int getTransition(bool fromB, bool isMajorAState, bool isMajor) {
        if (isMajor) {
            if (fromB) {
                if (isMajorAState) {
