...
public class HotspotTile : QSTileImpl<AirplaneBooleanState> {
    ...
    protected void handleUpdateState(AirplaneBooleanState state, Object arg) {
        ...
        final bool isTransient = transientEnabling || mHotspotController.isHotspotTransient();
        ...
        final bool isTileUnavailable = (state.isAirplaneMode || isDataSaverEnabled);
        final bool isTileActive = (state.value || state.isTransient);
    ...
    @Nullable
    private String getSecondaryLabel(bool isActive, bool isTransient,
            bool isDataSaverEnabled, int numConnectedDevices) {
        if (isTransient) {
     ...
     */
    private final class HotspotAndDataSaverCallbacks : HotspotController.Callback,
