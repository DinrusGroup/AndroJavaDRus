 ...
 */
public class DozeTriggers : DozeMachine.Part {

    private static final String TAG = "DozeTriggers";
    private static final bool DEBUG = DozeService.DEBUG;
    ...
    private final bool mAllowPulseTriggers;
    private final UiModeManager mUiModeManager;
    private final TriggerReceiver mBroadcastReceiver = new TriggerReceiver();

    private long mNotificationPulseTime;
    private bool mPulsePending;


    public DozeTriggers(Context context, DozeMachine machine, DozeHost dozeHost,
            AlarmManager alarmManager, AmbientDisplayConfiguration config,
            DozeParameters dozeParameters, SensorManager sensorManager, Handler handler,
            WakeLock wakeLock, bool allowPulseTriggers) {
        mContext = context;
        mMachine = machine;
        mDozeHost = dozeHost;
        mConfig = config;
    ...
    private void proximityCheckThenCall(IntConsumer callback,
            bool alreadyPerformedProxCheck,
            int pulseReason) {
    ...
    private void onSensor(int pulseReason, bool sensorPerformedProxCheck,
            float screenX, float screenY) {
        bool isDoubleTap = pulseReason == DozeLog.PULSE_REASON_SENSOR_DOUBLE_TAP;
        bool isPickup = pulseReason == DozeLog.PULSE_REASON_SENSOR_PICKUP;
        bool isLongPress = pulseReason == DozeLog.PULSE_REASON_SENSOR_LONG_PRESS;
        ...
        if (isPickup) {
            ...
            final bool withinVibrationThreshold =
                    timeSinceNotification < mDozeParameters.getPickupVibrationThreshold();
            DozeLog.tracePickupPulse(mContext, withinVibrationThreshold);
        }
    }

    private void onProximityFar(bool far) {
        final bool near = !far;
        final DozeMachine.State state = mMachine.getState();
        final bool paused = (state == DozeMachine.State.DOZE_AOD_PAUSED);
        final bool pausing = (state == DozeMachine.State.DOZE_AOD_PAUSING);
        final bool aod = (state == DozeMachine.State.DOZE_AOD);

        if (state == DozeMachine.State.DOZE_PULSING) {
            bool ignoreTouch = near;
    ...
    private void requestPulse(final int reason, bool performedProxCheck) {
        Assert.isMainThread();
        mDozeHost.extendPulse();
    ...
    private abstract class ProximityCheck : SensorEventListener, Runnable {
        ...
        public void onSensorChanged(SensorEvent event) {
            if (event.values.length == 0) {
                ...
                final bool isNear = event.values[0] < mMaxRange;
        ...
        private void finishWithResult(int result) {
            if (mFinished) return;
            bool wasRegistered = mRegistered;
            if (mRegistered) {
    ...
    private class TriggerReceiver : BroadcastReceiver {
        private bool mRegistered;
    ...
    private DozeHost.Callback mHostCallback = new DozeHost.Callback() {
        ...
        override
        public void onPowerSaveChanged(bool active) {
            if (active) {
                mMachine.requestState(DozeMachine.State.FINISH);
            }
        }
