...
public class DozeLog {
    ...
    private static bool sRegisterKeyguardCallback = true;

    private static long[] sTimes;
    private static String[] sMessages;
    private static int sPosition;
    private static int sCount;
    private static bool sPulsing;
    ...
    public static void tracePickupPulse(Context context, bool withinVibrationThreshold) {
        if (!ENABLED) return;
        init(context);
    ...
    public static void traceDozing(Context context, bool dozing) {
        if (!ENABLED) return;
        sPulsing = false;
        init(context);
        log("dozing " + dozing);
    ...
    public static void traceFling(bool expand, bool aboveThreshold, bool thresholdNeeded,
            bool screenOnFromTouch) {
        if (!ENABLED) return;
    ...
    public static void traceKeyguardBouncerChanged(bool showing) {
        if (!ENABLED) return;
        log("bouncer " + showing);
    }
    ...
    public static void traceKeyguard(bool showing) {
        if (!ENABLED) return;
    ...
    public static void traceProximityResult(Context context, bool near, long millis,
            int pulseReason) {
        if (!ENABLED) return;
        init(context);
    ...
    public static void tracePulseDropped(Context context, bool pulsePending,
            DozeMachine.State state, bool blocked) {
        if (!ENABLED) return;
        init(context);
    ...
    public static void tracePulseTouchDisabledByProx(Context context, bool disabled) {
        if (!ENABLED) return;
        init(context);
        log("pulseTouchDisabledByProx " + disabled);
    }

    public static void setRegisterKeyguardCallback(bool registerKeyguardCallback) {
        if (!ENABLED) return;
        synchronized (DozeLog.class) {
    ...
    private static final KeyguardUpdateMonitorCallback sKeyguardCallback =
        ...
        override
        public void onKeyguardBouncerChanged(bool bouncer) {
