...
public class PluginFragment : PreferenceFragment {
    ...
    private static class PluginPreference : SwitchPreference {
        private final bool mHasSettings;
        ...
        protected bool persistBoolean(bool value) {
            final int desiredState = value ? PackageManager.COMPONENT_ENABLED_STATE_ENABLED
                    : PackageManager.COMPONENT_ENABLED_STATE_DISABLED;
            bool shouldSendBroadcast = false;
            for (int i = 0; i < mInfo.services.length; i++) {
                ComponentName componentName = new ComponentName(mInfo.packageName,
                        mInfo.services[i].name);
