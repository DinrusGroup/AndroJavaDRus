 ...
 * @see Plugin
 */
public class PluginManagerImpl : BroadcastReceiver : PluginManager {

    static final String DISABLE_PLUGIN = "com.android.systemui.action.DISABLE_PLUGIN";

    private static PluginManager sInstance;

    private final ArrayMap<PluginListener!(T), PluginInstanceManager> mPluginMap
            = new ArrayMap<>();
    private final Map<String, ClassLoader> mClassLoaders = new ArrayMap<>();
    private final ArraySet<String> mOneShotPackages = new ArraySet<>();
    private final Context mContext;
    private final PluginInstanceManagerFactory mFactory;
    private final bool isDebuggable;
    private final PluginPrefs mPluginPrefs;
    private ClassLoaderFilter mParentClassLoader;
    private bool mListening;
    private bool mHasOneShot;
    private Looper mLooper;
    private bool mWtfsSet;
    ...
    @VisibleForTesting
    PluginManagerImpl(Context context, PluginInstanceManagerFactory factory, bool debuggable,
    ...
    public <T : Plugin> T getOneShotPlugin(Class<T> cls) {
    ...
    public <T : Plugin> T getOneShotPlugin(String action, Class!(T) cls) {
        if (!isDebuggable) {
    ...
    public <T : Plugin> void addPluginListener(PluginListener<T> listener, Class!(T) cls) {
    ...
    public <T : Plugin> void addPluginListener(String action, PluginListener<T> listener,
            Class!(T) cls) {
        addPluginListener(action, listener, cls, false);
    }

    public <T : Plugin> void addPluginListener(String action, PluginListener<T> listener,
            Class cls, bool allowMultiple) {
        if (!isDebuggable) {
            // Never ever ever allow these on production builds, they are only for prototyping.
            return;
        }
    ...
    public void removePluginListener(PluginListener!(T) listener) {
        if (!isDebuggable) {
            // Never ever ever allow these on production builds, they are only for prototyping.
            return;
        }
    ...
    private bool clearClassLoader(String pkg) {
    ...
    public <T> bool dependsOn(Plugin p, Class<T> cls) {
    ...
    public static class PluginInstanceManagerFactory {
        public <T : Plugin> PluginInstanceManager createPluginInstanceManager(Context context,
                String action, PluginListener<T> listener, bool allowMultiple, Looper looper,
                Class!(T) cls, PluginManagerImpl manager) {
    ...
    private static class ClassLoaderFilter : ClassLoader {
        ...
        override
        protected Class!(T) loadClass(String name, bool resolve) throws ClassNotFoundException {
            if (!name.startsWith(mPackage)) super.loadClass(name, resolve);
            return mBase.loadClass(name);
        }
    }

    private class PluginExceptionHandler : UncaughtExceptionHandler {
        ...
        public void uncaughtException(Thread thread, Throwable throwable) {
            ...
            bool disabledAny = checkStack(throwable);
            if (!disabledAny) {
        ...
        private bool checkStack(Throwable throwable) {
    ...
    private class CrashWhilePluginActiveException : RuntimeException {
