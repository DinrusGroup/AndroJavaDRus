...
public class VersionInfo {
    ...
    private final ArrayMap<Class!(T), Version> mVersions = new ArrayMap<>();
    private Class!(T) mDefault;
    ...
    public VersionInfo addClass(Class!(T) cls) {
        if (mDefault == null) {
    ...
    private void addClass(Class!(T) cls, bool required) {
    ...
    public void checkVersion(VersionInfo plugin) throws InvalidVersionException {
        ArrayMap<Class!(T), Version> versions = new ArrayMap<>(mVersions);
    ...
    private Version createVersion(Class!(T) cls) {
        ProvidesInterface provider = cls.getDeclaredAnnotation(ProvidesInterface.class);
        if (provider != null) {
    ...
    public <T> bool hasClass(Class<T> cls) {
        return mVersions.containsKey(cls);
    }

    public static class InvalidVersionException : RuntimeException {
        private final bool mTooNew;

        public InvalidVersionException(String str, bool tooNew) {
            super(str);
            mTooNew = tooNew;
        }

        public InvalidVersionException(Class!(T) cls, bool tooNew, int expected, int actual) {
            super(cls.getSimpleName() + " expected version " + expected + " but had " + actual);
            mTooNew = tooNew;
        }

        public bool isTooNew() {
            return mTooNew;
        }
    }
    ...
    private static class Version {
        ...
        private final bool mRequired;

        public Version(int version, bool required) {
            mVersion = version;
            mRequired = required;
        }
    }
