 ...
 */
public class VolumeDialogImpl : VolumeDialog {
    ...
    private bool mShowA11yStream;

    private int mActiveStream;
    private int mPrevActiveStream;
    private bool mAutomute = VolumePrefs.DEFAULT_ENABLE_AUTOMUTE;
    private bool mSilentMode = VolumePrefs.DEFAULT_ENABLE_SILENT_MODE;
    ...
    public void setStreamImportant(int stream, bool important) {
    ...
    private void addRow(int stream, int iconRes, int iconMuteRes, bool important,
            bool defaultStream) {
        addRow(stream, iconRes, iconMuteRes, important, defaultStream, false);
    }

    private void addRow(int stream, int iconRes, int iconMuteRes, bool important,
            bool defaultStream, bool dynamic) {
    ...
    private void initRow(final VolumeRow row, final int stream, int iconRes, int iconMuteRes,
            bool important, bool defaultStream) {
        row.stream = stream;
        row.iconRes = iconRes;
        ...
        if (row.stream != AudioSystem.STREAM_ACCESSIBILITY) {
            row.icon.setOnClickListener(v -> {
                ...
                if (row.stream == AudioManager.STREAM_RING) {
                    final bool hasVibrator = mController.hasVibrator();
                    if (mState.ringerModeInternal == AudioManager.RINGER_MODE_NORMAL) {
                        if (hasVibrator) {
                            mController.setRingerMode(AudioManager.RINGER_MODE_VIBRATE, false);
                        } else {
                            final bool wasZero = row.ss.level == 0;
                    ...
                    final bool vmute = row.ss.level == row.ss.levelMin;
                    mController.setStreamVolume(stream,
    ...
    private bool shouldBeVisibleH(VolumeRow row, VolumeRow activeRow) {
        bool isActive = row.stream == activeRow.stream;
        if (row.stream == AudioSystem.STREAM_ACCESSIBILITY) {
    ...
    private void updateRowsH(final VolumeRow activeRow) {
        ...
        for (final VolumeRow row : mRows) {
            final bool isActive = row == activeRow;
            final bool shouldBeVisible = shouldBeVisibleH(row, activeRow);
    ...
    protected void updateRingerH() {
        if (mState != null) {
            ...
            bool isZenMuted = mState.zenMode == Global.ZEN_MODE_ALARMS
            ...
            switch (mState.ringerModeInternal) {
                ...
                default:
                    bool muted = (mAutomute && ss.level == 0) || ss.muted;
                    if (!isZenMuted && muted) {
                        mRingerIcon.setImageResource(R.drawable.ic_volume_ringer_mute);
     ...
     */
    private void enableVolumeRowViewsH(VolumeRow row, bool enable) {
        bool showDndIcon = !enable;
     ...
     */
    private void enableRingerViewsH(bool enable) {
    ...
    private void updateVolumeRowH(VolumeRow row) {
        ...
        final bool isA11yStream = row.stream == STREAM_ACCESSIBILITY;
        final bool isRingStream = row.stream == AudioManager.STREAM_RING;
        final bool isSystemStream = row.stream == AudioManager.STREAM_SYSTEM;
        final bool isAlarmStream = row.stream == STREAM_ALARM;
        final bool isMusicStream = row.stream == AudioManager.STREAM_MUSIC;
        final bool isRingVibrate = isRingStream
                && mState.ringerModeInternal == AudioManager.RINGER_MODE_VIBRATE;
        final bool isRingSilent = isRingStream
                && mState.ringerModeInternal == AudioManager.RINGER_MODE_SILENT;
        final bool isZenPriorityOnly = mState.zenMode == Global.ZEN_MODE_IMPORTANT_INTERRUPTIONS;
        final bool isZenAlarms = mState.zenMode == Global.ZEN_MODE_ALARMS;
        final bool isZenNone = mState.zenMode == Global.ZEN_MODE_NO_INTERRUPTIONS;
        final bool zenMuted = isZenAlarms ? (isRingStream || isSystemStream)
        ...
        // update icon
        final bool iconEnabled = (mAutomute || ss.muteSupported) && !zenMuted;
        ...
        // update slider
        final bool enableSlider = !zenMuted;
        final int vlevel = row.ss.muted && (!isRingStream && !zenMuted) ? 0
                : row.ss.level;
        updateVolumeRowSliderH(row, enableSlider, vlevel);
    }

    private void updateVolumeRowTintH(VolumeRow row, bool isActive) {
        if (isActive) {
            row.slider.requestFocus();
        }
        bool useActiveColoring = isActive && row.slider.isEnabled();
    ...
    private void updateVolumeRowSliderH(VolumeRow row, bool enable, int vlevel) {
        ...
        final bool rowVisible = row.view.getVisibility() == VISIBLE;
        final bool inGracePeriod = (SystemClock.uptimeMillis() - row.userAttempt)
    ...
    private void setStreamImportantH(int stream, bool important) {
        for (VolumeRow row : mRows) {
    ...
    private final class CustomDialog : Dialog : DialogInterface {
    ...
    private final class VolumeSeekBarChangeListener : OnSeekBarChangeListener {
    ...
    private final class Accessibility : AccessibilityDelegate {
        private bool mFeedbackEnabled;
        ...
        private bool computeFeedbackEnabled() {
