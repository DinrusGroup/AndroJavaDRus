 ...
 */
public class VolumeDialogControllerImpl : VolumeDialogController, Dumpable {
    ...
    private bool mShowVolumeDialog;
    private bool mShowSafetyWarning;
    private long mLastToggledRingerOn;
    private final NotificationManager mNotificationManager;

    private bool mDestroyed;
    private VolumePolicy mVolumePolicy;
    private bool mShowDndTile = true;
    @GuardedBy("this")
    ...
    public VolumeDialogControllerImpl(Context context) {
        ...
        bool accessibilityVolumeStreamActive = context.getSystemService(
    ...
    public void setRingerMode(int value, bool external) {
        if (mDestroyed) return;
    ...
    public void setStreamMute(int stream, bool mute) {
        if (mDestroyed) return;
    ...
    public void setEnableDialogs(bool volumeUi, bool safetyWarning) {
    ...
    private void onNotifyVisibleW(bool visible) {
        if (mDestroyed) return; 
    ...
    private bool checkRoutedToBluetoothW(int stream) {
        bool changed = false;
    ...
    private bool shouldShowUI(int flags) {
        updateStatusBar();
    ...
    bool onVolumeChangedW(int stream, int flags) {
        final bool showUI = shouldShowUI(flags);
        final bool fromKey = (flags & AudioManager.FLAG_FROM_KEY) != 0;
        final bool showVibrateHint = (flags & AudioManager.FLAG_SHOW_VIBRATE_HINT) != 0;
        final bool showSilentHint = (flags & AudioManager.FLAG_SHOW_SILENT_HINT) != 0;
        bool changed = false;
        if (showUI) {
    ...
    private bool updateActiveStreamW(int activeStream) {
    ...
    private bool updateStreamRoutedToBluetoothW(int stream, bool routedToBluetooth) {
        final StreamState ss = streamStateW(stream);
    ...
    private bool updateStreamLevelW(int stream, int level) {
        final StreamState ss = streamStateW(stream);
    ...
    private static bool isLogWorthy(int stream) {
        switch (stream) {
    ...
    private bool updateStreamMuteW(int stream, bool muted) {
        final StreamState ss = streamStateW(stream);
    ...
    private static bool isRinger(int stream) {
        return stream == AudioManager.STREAM_RING || stream == AudioManager.STREAM_NOTIFICATION;
    }

    private bool updateEffectsSuppressorW(ComponentName effectsSuppressor) {
    ...
    private bool updateZenConfig() {
        final NotificationManager.Policy policy = mNotificationManager.getNotificationPolicy();
        bool disallowAlarms = (policy.priorityCategories & NotificationManager.Policy
                .PRIORITY_CATEGORY_ALARMS) == 0;
        bool disallowMedia = (policy.priorityCategories & NotificationManager.Policy
                .PRIORITY_CATEGORY_MEDIA) == 0;
        bool disallowSystem = (policy.priorityCategories & NotificationManager.Policy
                .PRIORITY_CATEGORY_SYSTEM) == 0;
        bool disallowRinger = ZenModeConfig.areAllPriorityOnlyNotificationZenSoundsMuted(policy);
    ...
    private bool updateRingerModeExternalW(int rm) {
    ...
    private bool updateRingerModeInternalW(int rm) {
    ...
    private void onSetRingerModeW(int mode, bool external) {
        if (external) {
    ...
    private void onSetStreamMuteW(int stream, bool mute) {
    ...
    private void onSetActiveStreamW(int stream) {
        bool changed = updateActiveStreamW(stream);
        if (changed) {
            mCallbacks.onStateChanged(mState);
        }
    }
    ...
    public void showDndTile(bool visible) {
        if (D.BUG) Log.d(TAG, "showDndTile");
        DndTile.setVisible(mContext, visible);
    }

    private final class VC : IVolumeController.Stub {
    ...
    class C : Callbacks {
        ...
        public void onAccessibilityModeChanged(Boolean showA11yStream) {
            bool show = showA11yStream == null ? false : showA11yStream;
            for (final Map.Entry<Callbacks, Handler> entry : mCallbackMap.entrySet()) {
                entry.getValue().post(new Runnable() {
                    override
                    public void run() {
    ...
    private final class SettingObserver : ContentObserver {
    ...
    private final class Receiver : BroadcastReceiver {
    ...
    protected final class MediaSessionsCallbacks : MediaSessions.Callbacks {
        ...
        public void onRemoteUpdate(Token token, String name, PlaybackInfo pi) {
            ...
            bool changed = mState.states.indexOfKey(stream) < 0;
            final StreamState ss = streamStateW(stream);
            ss.dynamic = true;
            ss.levelMin = 0;
