...
public class ZenModePanel : FrameLayout {
    ...
    private void updateWidgets() {
        ...
        final bool zenImportant = zen == Global.ZEN_MODE_IMPORTANT_INTERRUPTIONS;
        final bool zenNone = zen == Global.ZEN_MODE_NO_INTERRUPTIONS;
        final bool zenAlarm = zen == Global.ZEN_MODE_ALARMS;
        final bool introduction = (zenImportant && !mPrefs.mConfirmedPriorityIntroduction
    ...
    private String computeAlarmWarningText(bool zenNone) {
        if (!zenNone) {
            return null;
        }
        ...
        final bool soon = (nextAlarm - now) < 24 * 60 * 60 * 1000;
        final bool is24 = DateFormat.is24HourFormat(mContext, ActivityManager.getCurrentUser());
    ...
    private void bindNextAlarm(Condition c) {
        ...
        bool showAlarm = tag != null && tag.condition != null;
    ...
    private static bool isAlarm(Condition c) {
        return c != null && ZenModeConfig.isValidCountdownToAlarmConditionId(c.id);
    }

    private static bool isCountdown(Condition c) {
        return c != null && ZenModeConfig.isValidCountdownConditionId(c.id);
    }

    private bool isForever(Condition c) {
    ...
    private void bind(final Condition condition, final View row, final int rowId) {
        ...
        final bool enabled = condition.state == Condition.STATE_TRUE;
        final ConditionTag tag =
                row.getTag() != null ? (ConditionTag) row.getTag() : new ConditionTag();
        row.setTag(tag);
        final bool first = tag.rb == null;
        if (tag.rb == null) {
    ...
    private void onClickTimeButton(View row, ConditionTag tag, bool up, int rowId) {
    ...
    private final class ZenPrefs : OnSharedPreferenceChangeListener {
        private final int mNoneDangerousThreshold;

        private int mMinuteIndex;
        private int mNoneSelected;
        private bool mConfirmedPriorityIntroduction;
        private bool mConfirmedSilenceIntroduction;
        private bool mConfirmedAlarmIntroduction;
        ...
        private void updateConfirmedPriorityIntroduction() {
            final bool confirmed =  Prefs.getBoolean(mContext,
    ...
    protected final SegmentedButtons.Callback mZenButtonsCallback = new SegmentedButtons.Callback() {
        override
        public void onSelected(final Object value, bool fromClick) {
    ...
    private final class TransitionHelper : TransitionListener, Runnable {
        private final ArraySet<View> mTransitioningViews = new ArraySet<View>();

        private bool mTransitioning;
        private bool mPendingUpdateWidgets;
        ...
        private void updateTransitioning() {
            final bool transitioning = isTransitioning();
