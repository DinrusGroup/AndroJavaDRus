...
public class TrackedObjects {
    ...
    private final WeakHashMap<Class!(T), TrackedClass!(T)> mTrackedClasses = new WeakHashMap<>();
    ...
    public static bool isTrackedObject(Collection!(T) collection) {
        return collection instanceof TrackedClass;
    }

    private static class TrackedClass<T> : AbstractCollection<T> {
