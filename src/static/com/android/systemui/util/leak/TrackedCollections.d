...
public class TrackedCollections {
    ...
    private final WeakIdentityHashMap<Collection!(T), CollectionState> mCollections
            = new WeakIdentityHashMap<>();

    /**
     * @see LeakDetector#trackCollection(Collection, String)
     */
    public synchronized void track(Collection!(T) collection, String tag) {
    ...
    public synchronized void dump(PrintWriter pw, Predicate<Collection!(T)> filter) {
        for (Map.Entry<WeakReference<Collection!(T)>, CollectionState> entry
                : mCollections.entrySet()) {
            Collection!(T) key = entry.getKey().get();
