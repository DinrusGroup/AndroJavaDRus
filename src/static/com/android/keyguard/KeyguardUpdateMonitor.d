 ...
 */
public class KeyguardUpdateMonitor : TrustManager.TrustListener {

    private static final String TAG = "KeyguardUpdateMonitor";
    private static final bool DEBUG = KeyguardConstants.DEBUG;
    private static final bool DEBUG_SIM_STATES = KeyguardConstants.DEBUG_SIM_STATES;
     ...
     */
    public static final bool CORE_APPS_ONLY;
    static {
        try {
    ...
    private bool mKeyguardIsVisible;
    private bool mKeyguardGoingAway;
    private bool mGoingToSleep;
    private bool mBouncer;
    private bool mBootCompleted;
    private bool mNeedsSlowUnlockTransition;
    private bool mHasLockscreenWallpaper;
    private bool mAssistantVisible;
    private bool mKeyguardOccluded;

    // Device provisioning state
    private bool mDeviceProvisioned;
    ...
    private bool mIsDreaming;
    private final DevicePolicyManager mDevicePolicyManager;
    private bool mLogoutEnabled;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_ASSISTANT_STACK_CHANGED:
                    mAssistantVisible = (bool)msg.obj;
    ...
    private static bool sDisableHandlerCheckForTesting;
    ...
    override
    public void onTrustChanged(bool enabled, int userId, int flags) {
    ...
    private void handleSimSubscriptionInfoChanged() {
        ...
        for (int i = 0; i < subscriptionInfos.size(); i++) {
            ...
            bool changed = refreshSimState(info.getSubscriptionId(), info.getSimSlotIndex());
            if (changed) {
    ...
    public List<SubscriptionInfo> getSubscriptionInfo(bool forceReload) {
    ...
    override
    public void onTrustManagedChanged(bool managed, int userId) {
        checkIsHandlerThread();
     ...
     * @param goingAway
     */
    public void setKeyguardGoingAway(bool goingAway) {
    ...
    private void setFingerprintRunningState(int fingerprintRunningState) {
        bool wasRunning = mFingerprintRunningState == FINGERPRINT_STATE_RUNNING;
        bool isRunning = fingerprintRunningState == FINGERPRINT_STATE_RUNNING;
    ...
    private void handleFaceUnlockStateChanged(bool running, int userId) {
        checkIsHandlerThread();
    ...
    public bool isFaceUnlockRunning(int userId) {
        return mUserFaceUnlockRunning.get(userId);
    }

    public bool isFingerprintDetectionRunning() {
        return mFingerprintRunningState == FINGERPRINT_STATE_RUNNING;
    }

    private bool isTrustDisabled(int userId) {
        // Don't allow trust agent if device is secured with a SIM PIN. This is here
        // mainly because there's no other way to prompt the user to enter their SIM PIN
        // once they get past the keyguard screen.
        final bool disabledBySimPin = isSimPinSecure();
        return disabledBySimPin;
    }

    private bool isFingerprintDisabled(int userId) {
    ...
    public bool getUserCanSkipBouncer(int userId) {
        return getUserHasTrust(userId) || (mUserFingerprintAuthenticated.get(userId)
                && isUnlockingWithFingerprintAllowed());
    }

    public bool getUserHasTrust(int userId) {
        return !isTrustDisabled(userId) && mUserHasTrust.get(userId);
    }

    public bool getUserTrustIsManaged(int userId) {
        return mUserTrustIsManaged.get(userId) && !isTrustDisabled(userId);
    }

    public bool isUnlockingWithFingerprintAllowed() {
    ...
    public bool needsSlowUnlockTransition() {
    ...
    public class StrongAuthTracker : LockPatternUtils.StrongAuthTracker {
        ...
        public bool hasUserAuthenticatedSinceBoot() {
            int userId = getCurrentUser();
    ...
    private void updateFingerprintListeningState() {
        ...
        mHandler.removeCallbacks(mRetryFingerprintAuthentication);
        bool shouldListenForFingerprint = shouldListenForFingerprint();
    ...
    private bool shouldListenForFingerprintAssistant() {
        return mAssistantVisible && mKeyguardOccluded
                && !mUserFingerprintAuthenticated.get(getCurrentUser(), false)
                && !mUserHasTrust.get(getCurrentUser(), false);
    }

    private bool shouldListenForFingerprint() {
    ...
    public bool isUnlockWithFingerprintPossible(int userId) {
    ...
    private bool isDeviceProvisionedInSettingsDb() {
    ...
    private void watchForDeviceProvisioning() {
        ...
        bool provisioned = isDeviceProvisionedInSettingsDb();
     ...
     */
    public void setHasLockscreenWallpaper(bool hasLockscreenWallpaper) {
        checkIsHandlerThread();
    ...
    private void handleBatteryUpdate(BatteryStatus status) {
        ...
        final bool batteryUpdateInteresting = isBatteryUpdateInteresting(mBatteryStatus, status);
    ...
    private bool resolveNeedsSlowUnlockTransition() {
    ...
    private bool isBatteryUpdateInteresting(BatteryStatus old, BatteryStatus current) {
        final bool nowPluggedIn = current.isPluggedIn();
        final bool wasPluggedIn = old.isPluggedIn();
        final bool stateChangedWhilePluggedIn = wasPluggedIn && nowPluggedIn
     ...
     */
    public void sendKeyguardBouncerChanged(bool showingBouncer) {
     ...
     */
    public void reportEmergencyCallAction(bool bypassHandler) {
        if (!bypassHandler) {
    ...
    public bool isSimPinVoiceSecure() {
        // TODO: only count SIMs that handle voice
        return isSimPinSecure();
    }

    public bool isSimPinSecure() {
     ...
     */
    private bool refreshSimState(int subId, int slotId) {
    ...
    public static bool isSimPinSecure(IccCardConstants.State state) {
    ...
    private void updateLogoutEnabled() {
        checkIsHandlerThread();
        bool logoutEnabled = mDevicePolicyManager.isLogoutEnabled();
