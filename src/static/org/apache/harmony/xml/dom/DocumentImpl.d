 ...
 * structure.
 */
public final class DocumentImpl : InnerNodeImpl : Document {
    ...
    private bool xmlStandalone = false;
    private bool strictErrorChecking = true;
    ...
    private static bool isXMLIdentifierStart(char c) {
        return (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || (c == '_');
    }

    private static bool isXMLIdentifierPart(char c) {
        return isXMLIdentifierStart(c) || (c >= '0' && c <= '9') || (c == '-') || (c == '.');
    }

    static bool isXMLIdentifier(String s) {
        if (s.length() == 0) {
            return false;
        }
     ...
     */
    Node cloneOrImportNode(short operation, Node node, bool deep) {
