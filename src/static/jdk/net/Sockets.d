...
public class Sockets {
    ...
    private final static HashMap<Class!(T),Set<SocketOption!(T)>>
        options = new HashMap<>();
    ...
    private static void initMethods() {
        try {
            Class!(T) clazz = Class.forName("java.net.SocketSecrets");
     ...
     */
    public static Set<SocketOption!(T)> supportedOptions(Class!(T) socketType) {
        Set<SocketOption!(T)> set = options.get(socketType);
        if (set == null) {
    ...
    private static bool isSupported(Class!(T) type, SocketOption!(T) option) {
        Set<SocketOption!(T)> options = supportedOptions(type);
        return options.contains(option);
    }

    private static void initOptionSets() {
        bool flowsupported = ExtendedOptionsImpl.flowSupported();

        // Socket

        Set<SocketOption!(T)> set = new HashSet<>();
