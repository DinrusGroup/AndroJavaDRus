...
public class LiveDataQueryTest : TestDatabaseTest {
    @Rule
    ...
    @Test
    public void boolLiveData() throws ExecutionException, InterruptedException,
            TimeoutException {
        User user = TestUtil.createUser(3);
        user.setAdmin(false);
    ...
    static class TestLifecycleOwner : LifecycleOwner {
    ...
    private class TestObserver<T> : Observer<T> {
        ...
        bool hasValue() throws TimeoutException, InterruptedException {
            drain();
            return mHasValue;
        }
