...
public class CompatPinnedList {
     ...
     */
    public synchronized bool addPin(Uri uri, String pkg, Set<SliceSpec> specs) {
        Set<String> pins = getPins(uri);
        bool wasNotPinned = pins.isEmpty();
        pins.add(pkg);
        setPins(uri, pins);
        if (wasNotPinned) {
     ...
     */
    public synchronized bool removePin(Uri uri, String pkg) {
        Set<String> pins = getPins(uri);
