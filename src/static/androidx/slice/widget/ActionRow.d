...
public class ActionRow : FrameLayout {
    ...
    private final bool mFullActions;
    private int mColor = Color.BLACK;

    public ActionRow(Context context, bool fullActions) {
        super(context);
    ...
    private ImageView addAction(IconCompat icon, bool allowTint) {
        ...
        for (final SliceItem action : actions) {
            ...
            if (input != null && image != null) {
                ...
                if (iconItem != null && ac.getActionItem() != null) {
                    bool tint = ac.getImageMode() == ICON_IMAGE;
    ...
    private void handleSetRemoteInputActions(final SliceItem input, SliceItem image,
        ...
        if (input.getRemoteInput().getAllowFreeFormInput()) {
            bool tint = !image.hasHint(HINT_NO_TINT);
    ...
    @RequiresApi(21)
    private bool handleRemoteInputClick(View view, SliceItem action,
            RemoteInput input) {
        if (input == null) {
            return false;
        }
