...
public class GridContent {
     ...
     */
    private bool populate(SliceItem gridItem) {
    ...
    private List<SliceItem> filterAndProcessItems(List<SliceItem> items) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            bool containsSeeMore = SliceQuery.find(item, null, HINT_SEE_MORE, null) != null;
            bool isNonCellContent = containsSeeMore
    ...
    private int getHeight(bool isSmall) {
        if (!isValid()) {
            return 0;
        }
        if (mAllImages) {
            ...
            bool twoLines = getMaxCellLineCount() > 1;
    ...
    public static class CellContent {
         ...
         */
        public bool populate(SliceItem cellItem) {
         ...
         */
        private bool isValidCellContent(SliceItem cellItem) {
            final String format = cellItem.getFormat();
            bool isNonCellContent = SUBTYPE_CONTENT_DESCRIPTION.equals(cellItem.getSubType())
