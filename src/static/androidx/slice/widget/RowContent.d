...
public class RowContent {
    ...
    private bool mEndItemsContainAction;
    ...
    public RowContent(Context context, SliceItem rowSlice, bool isHeader) {
     ...
     */
    private bool populate(SliceItem rowSlice, bool isHeader) {
        mIsHeader = isHeader;
        mRowSlice = rowSlice;
        ...
        if (rowItems.size() > 0) {
            ...
            bool hasTimestamp = mStartItem != null
                    && FORMAT_LONG.equals(mStartItem.getFormat());
            for (int i = 0; i < endItems.size(); i++) {
                final SliceItem item = endItems.get(i);
                bool isAction = SliceQuery.find(item, FORMAT_ACTION) != null;
    ...
    private void processContent(@NonNull SliceItem item, bool isAction) {
        if (isAction) {
     ...
     */
    public bool endItemsContainAction() {
    ...
    private static bool hasText(SliceItem textSlice) {
     ...
     */
    private static bool isValidRow(SliceItem rowSlice) {
        if (rowSlice == null) {
            return false;
        }
     ...
     */
    private static bool isValidRowContent(SliceItem slice, SliceItem item) {
