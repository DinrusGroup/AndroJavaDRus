 ...
 * not enabled (see {@link #setScrollable(bool)}) the view will show as many items as it can
 * comfortably fit.</li>
 * </ul>
 * <p>
 ...
 * @see SliceLiveData
 */
public class SliceView : ViewGroup : Observer<Slice>, View.OnClickListener {
    ...
    private bool mShowActions = false;
    private bool mIsScrollable = true;
    private bool mShowLastUpdated = true;
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public bool isSliceViewClickable() {
    ...
    public bool onInterceptTouchEvent(MotionEvent ev) {
        bool ret = super.onInterceptTouchEvent(ev);
    ...
    public bool onTouchEvent(MotionEvent ev) {
        bool ret = super.onTouchEvent(ev);
        if (mLongClickListener != null) {
            return handleTouchForLongpress(ev);
        }
        return ret;
    ...
    private bool handleTouchForLongpress(MotionEvent ev) {
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public void setMode(@SliceMode int mode, bool animate) {
        if (animate) {
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public void setShowActionRow(bool show) {
        mShowActions = show;
        updateActions();
    }
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public bool isShowingActionRow() {
        return mShowActions;
    }
    ...
    private void reinflate() {
        ...
        int mode = getMode();
        bool isCurrentViewShortcut = mCurrentView instanceof ShortcutView;
        ...
        bool expired = expiry != 0 && expiry != SliceHints.INFINITY && now > expiry;
