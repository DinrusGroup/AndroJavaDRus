...
public class GridRowView : SliceChildView : View.OnClickListener {
     ...
     */
    override
    public void setSliceItem(SliceItem slice, bool isHeader, int rowIndex,
    ...
    private void populateViews(GridContent gc) {
        ...
        bool hasSeeMore = gc.getSeeMoreItem() != null;
    ...
    private void addCell(GridContent.CellContent cell, int index, int total) {
        ...
        int imageCount = 0;
        bool added = false;
        bool singleItem = cellItems.size() == 1;
     ...
     */
    private bool addItem(SliceItem item, int color, ViewGroup container, int padding) {
        final String format = item.getFormat();
        View addedView = null;
        if (FORMAT_TEXT.equals(format) || FORMAT_LONG.equals(format)) {
            bool title = SliceQuery.hasAnyHints(item, HINT_LARGE, HINT_TITLE);
            ...
            if (item.hasHint(HINT_LARGE)) {
                ...
                bool isIcon = !item.hasHint(HINT_NO_TINT);
    ...
    private void makeClickable(View layout, bool isClickable) {
