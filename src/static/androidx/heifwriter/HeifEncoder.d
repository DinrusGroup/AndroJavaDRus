 ...
 * @hide
 */
public final class HeifEncoder : AutoCloseable,
     ...
     */
    public HeifEncoder(int width, int height, bool useGrid,
    ...
    public void onFrameAvailable(SurfaceTexture surfaceTexture) {
        synchronized (this) {
            ...
            bool takeFrame = mEOSTracker.updateLastInputAndEncoderTime(timestampNs,
    ...
    public void addBitmap(@NonNull Bitmap bitmap) {
        ...
        bool takeFrame = mEOSTracker.updateLastInputAndEncoderTime(
    ...
    private void maybeCopyOneTileYUV() {
        ...
        while ((currentBuffer = getCurrentBuffer()) != null && !mCodecInputBuffers.isEmpty()) {
            ...
            bool inputEOS = (mInputIndex % mNumTiles == 0) && (currentBuffer.remaining() == 0);
     ...
     * Only called on callback handler thread.
     */
    private void returnEmptyBufferAndNotify(bool inputEOS) {
        synchronized (mEmptyBuffers) {
            mInputEOS |= inputEOS;
    ...
    private class SurfaceEOSTracker {
        private static final bool DEBUG_EOS = false;

        final bool mCopyTiles;
        ...
        SurfaceEOSTracker(bool copyTiles) {
        ...
        synchronized bool updateLastInputAndEncoderTime(long inputTimeNs, long encoderTimeUs) {
            if (DEBUG_EOS) Log.d(TAG,
                    "updateLastInputAndEncoderTime: " + inputTimeNs + ", " + encoderTimeUs);

            bool shouldTakeFrame = mInputEOSTimeNs < 0 || inputTimeNs <= mInputEOSTimeNs;
            if (shouldTakeFrame) {
