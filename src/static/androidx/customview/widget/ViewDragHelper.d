...
public class ViewDragHelper {
    ...
    public abstract static class Callback {
         ...
         */
        public abstract bool tryCaptureView(@NonNull View child, int pointerId);
     ...
     */
    public bool smoothSlideViewTo(@NonNull View child, int finalLeft, int finalTop) {
        mCapturedView = child;
        mActivePointerId = INVALID_POINTER;
     ...
     * @return true if animation should continue through {@link #continueSettling(bool)} calls
     */
    public bool settleCapturedViewAt(int finalLeft, int finalTop) {
     ...
     * @return true if animation should continue through {@link #continueSettling(bool)} calls
     */
    private bool forceSettleCapturedViewAt(int finalLeft, int finalTop, int xvel, int yvel) {
     ...
     */
    protected bool canScroll(@NonNull View v, bool checkV, int dx, int dy, int x, int y) {
     ...
     */
    public bool shouldInterceptTouchEvent(@NonNull MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                for (int i = 0; i < pointerCount; i++) {
                    ...
                    final View toCapture = findTopChildUnder((int) x, (int) y);
                    final bool pastSlop = toCapture != null && checkTouchSlop(toCapture, dx, dy);
                    if (pastSlop) {
    ...
    private bool checkTouchSlop(View child, float dx, float dy) {
        ...
        final bool checkHorizontal = mCallback.getViewHorizontalDragRange(child) > 0;
        final bool checkVertical = mCallback.getViewVerticalDragRange(child) > 0;
    ...
    public bool checkTouchSlop(int directions, int pointerId) {
        ...
        final bool checkHorizontal = (directions & DIRECTION_HORIZONTAL) == DIRECTION_HORIZONTAL;
        final bool checkVertical = (directions & DIRECTION_VERTICAL) == DIRECTION_VERTICAL;
     ...
     */
    public bool isViewUnder(@Nullable View view, int x, int y) {
        if (view == null) {
            return false;
        }
    ...
    private bool isValidPointerForActionMove(int pointerId) {
        if (!isPointerDown(pointerId)) {
