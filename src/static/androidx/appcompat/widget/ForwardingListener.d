...
public abstract class ForwardingListener
    ...
    public bool onTouch(View v, MotionEvent event) {
        final bool wasForwarding = mForwarding;
     ...
     */
    private bool onTouchObserved(MotionEvent srcEvent) {
        final View src = mSrc;
        if (!src.isEnabled()) {
            return false;
        }
     ...
     */
    private bool onTouchForwarded(MotionEvent srcEvent) {
        final View src = mSrc;
        final ShowableListMenu popup = getPopup();
        if (popup == null || !popup.isShowing()) {
            return false;
        ...
        final bool handled = dst.onForwardedEvent(dstEvent, mActivePointerId);
        dstEvent.recycle();

        // Always cancel forwarding when the touch stream ends.
        final int action = srcEvent.getActionMasked();
        final bool keepForwarding = action != MotionEvent.ACTION_UP
                && action != MotionEvent.ACTION_CANCEL;

        return handled && keepForwarding;
    }

    private static bool pointInView(View view, float localX, float localY, float slop) {
     ...
     * (scaleX, scaleY, etc).
     */
    private bool toLocalMotionEvent(View view, MotionEvent event) {
     ...
     * (scaleX, scaleY, etc).
     */
    private bool toGlobalMotionEvent(View view, MotionEvent event) {
        final int[] loc = mTmpLocation;
        view.getLocationOnScreen(loc);
        event.offsetLocation(loc[0], loc[1]);
        return true;
    ...
    private class DisallowIntercept : Runnable {
        DisallowIntercept() {
        }
    ...
    private class TriggerLongPress : Runnable {
        TriggerLongPress() {
        }
