...
/**
 * This class : the route controller dialog for {@link MediaRouter}.
 * <p>
 ...
 */
public class MediaRouteControllerDialog : AlertDialog {
    ...
    private bool mVolumeControlEnabled = true;
    ...
    Uri mArtIconUri;
    bool mArtIconIsLoaded;
    Bitmap mArtIconLoadedBitmap;
    int mArtIconBackgroundColor;

    bool mHasPendingUpdate;
    bool mPendingUpdateAnimationNeeded;

    bool mIsGroupExpanded;
    bool mIsGroupListAnimating;
    bool mIsGroupListAnimationPending;
     ...
     * </p>
     */
    public void setVolumeControlEnabled(bool enable) {
    ...
    private bool isBitmapRecycled(Bitmap bitmap) {
        return bitmap != null && bitmap.isRecycled();
    }

    private bool canShowPlaybackControlLayout() {
     ...
     * volume control.
     */
    private int getMainControllerHeight(bool showPlaybackControl) {
        int height = 0;
    ...
    private void updateMediaControlVisibility(bool canShowPlaybackControlLayout) {
    ...
    void updateLayoutHeight(final bool animate) {
     ...
     */
    void updateLayoutHeightInternal(bool animate) {
    ...
    private void rebuildVolumeGroupList(bool animate) {
    ...
    void finishAnimation(bool animate) {
        mGroupMemberRoutesAdded = null;
        mGroupMemberRoutesRemoved = null;
    ...
    void clearGroupListAnimation(bool exceptAddedRoutes) {
        int first = mVolumeGroupList.getFirstVisiblePosition();
        for (int i = 0; i < mVolumeGroupList.getChildCount(); ++i) {
            View view = mVolumeGroupList.getChildAt(i);
            int position = first + i;
    ...
    private void updatePlaybackControlLayout() {
        if (canShowPlaybackControlLayout()) {
            ...
            bool hasTitle = !TextUtils.isEmpty(title);

            CharSequence subtitle = mDescription == null ? null : mDescription.getSubtitle();
            bool hasSubtitle = !TextUtils.isEmpty(subtitle);
            ...
            if (mState != null) {
                bool isPlaying = mState.getState() == PlaybackStateCompat.STATE_BUFFERING
    ...
    private bool isPlayActionSupported() {
        return (mState.getActions() & (ACTION_PLAY | ACTION_PLAY_PAUSE)) != 0;
    }

    private bool isPauseActionSupported() {
        return (mState.getActions() & (ACTION_PAUSE | ACTION_PLAY_PAUSE)) != 0;
    }

    private bool isStopActionSupported() {
        return (mState.getActions() & ACTION_STOP) != 0;
    }

    bool isVolumeControlAvailable(MediaRouter.RouteInfo route) {
    ...
    private final class MediaRouterCallback : MediaRouter.Callback {
        MediaRouterCallback() {
        }
    ...
    private final class MediaControllerCallback : MediaControllerCompat.Callback {
    ...
    private final class ClickListener : View.OnClickListener {
        ClickListener() {
        }
        ...
        public void onClick(View v) {
            ...
            if (id == BUTTON_STOP_RES_ID || id == BUTTON_DISCONNECT_RES_ID) {
                ...
                if (mMediaController != null && mState != null) {
                    bool isPlaying = mState.getState() == PlaybackStateCompat.STATE_PLAYING;
    ...
    private class VolumeChangeListener : SeekBar.OnSeekBarChangeListener {
    ...
    private class VolumeGroupAdapter : ArrayAdapter<MediaRouter.RouteInfo> {
    ...
    private class FetchArtTask : AsyncTask<Void, Void, Bitmap> {
