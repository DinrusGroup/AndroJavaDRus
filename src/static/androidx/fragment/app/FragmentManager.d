...
public abstract class FragmentManager {
     ...
     * @return Returns true if there was something popped, else false.
     */
    public abstract bool popBackStackImmediate(@Nullable String name, int flags);
     ...
     */
    public abstract bool isStateSaved();
...
final class FragmentManagerState : Parcelable {
 ...
 */
final class FragmentManagerImpl : FragmentManager : LayoutInflater.Factory2 {
    ...
    private static final class FragmentLifecycleCallbacksHolder {
        ...
        final bool mRecursive;

        FragmentLifecycleCallbacksHolder(FragmentLifecycleCallbacks callback, bool recursive) {
            mCallback = callback;
    ...
    bool mNeedMenuInvalidate;
    bool mStateSaved;
    bool mStopped;
    bool mDestroyed;
    String mNoTransactionsBecause;
    bool mHavePendingDeferredStart;
    ...
    static bool modifiesAlpha(AnimationOrAnimator anim) {
    ...
    static bool modifiesAlpha(Animator anim) {
        if (anim == null) {
            return false;
        }
    ...
    static bool shouldRunOnHWLayer(View v, AnimationOrAnimator anim) {
        if (v == null || anim == null) {
            return false;
        }
    ...
    public bool executePendingTransactions() {
        bool updates = execPendingActions();
    ...
    override
    public bool popBackStackImmediate(@Nullable String name, int flags) {
        checkStateLoss();
    ...
    override
    public bool popBackStackImmediate(int id, int flags) {
        checkStateLoss();
        execPendingActions();
        if (id < 0) {
            throw new IllegalArgumentException("Bad id: " + id);
     ...
     */
    private bool popBackStackImmediate(String name, int id, int flags) {
        execPendingActions();
        ...
        bool executePop = popBackStackState(mTmpRecords, mTmpIsPop, name, id, flags);
        if (executePop) {
    ...
    AnimationOrAnimator loadAnimation(Fragment fragment, int transit, bool enter,
            int transitionStyle) {
     ...
     */
    void moveToState(int newState, bool always) {
    ...
    public void addFragment(Fragment fragment, bool moveToStateNow) {
    ...
    public void removeFragment(Fragment fragment) {
        ...
        final bool inactive = !fragment.isInBackStack();
     ...
     */
    public void enqueueAction(OpGenerator action, bool allowStateLoss) {
        if (!allowStateLoss) {
            checkStateLoss();
        }
        synchronized (this) {
     ...
     * the first time {@link #enqueueAction(OpGenerator, bool)} is called or when
    ...
    private void scheduleCommit() {
        synchronized (this) {
            ...
            bool pendingReady = mPendingActions != null && mPendingActions.size() == 1;
     ...
     *                       checked.
     */
    private void ensureExecReady(bool allowStateLoss) {
        if (mExecutingActions) {
    ...
    public void execSingleAction(OpGenerator action, bool allowStateLoss) {
     ...
     * {@link FragmentTransaction#setReorderingAllowed(bool)}.
     * <p>
    ...
    private void removeRedundantOperationsAndExecute(ArrayList<BackStackRecord> records,
        ...
        for (int recordNum = 0; recordNum < numRecords; recordNum++) {
            final bool canReorder = records.get(recordNum).mReorderingAllowed;
            if (!canReorder) {
    ...
    private void executeOpsTogether(ArrayList<BackStackRecord> records,
        ...
        final bool allowReordering = records.get(startIndex).mReorderingAllowed;
        ...
        for (int recordNum = startIndex; recordNum < endIndex; recordNum++) {
            ...
            final bool isPop = isRecordPop.get(recordNum);
            if (!isPop) {
    ...
    private int postponePostponableTransactions(ArrayList<BackStackRecord> records,
        ...
        for (int i = endIndex - 1; i >= startIndex; i--) {
            ...
            final bool isPop = isRecordPop.get(i);
            bool isPostponed = record.isPostponed()
     ...
     */
    private void completeExecute(BackStackRecord record, bool isPop, bool runTransitions,
            bool moveToState) {
        if (isPop) {
     ...
     * @param isPop All pending actions will generate bools to add to this. This contains
     *              an entry for each entry in records to indicate whether or not it is a
     *              pop action.
     */
    private bool generateOpsForPendingActions(ArrayList<BackStackRecord> records,
    ...
    bool popBackStackState(ArrayList<BackStackRecord> records, ArrayList<Boolean> isRecordPop,
    ...
    public void dispatchMultiWindowModeChanged(bool isInMultiWindowMode) {
        for (int i = mAdded.size() - 1; i >= 0; --i) {
    ...
    public void dispatchPictureInPictureModeChanged(bool isInPictureInPictureMode) {
        for (int i = mAdded.size() - 1; i >= 0; --i) {
            final Fragment f = mAdded.get(i);
            if (f != null) {
    ...
    public bool dispatchCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        if (mCurState < Fragment.CREATED) {
            return false;
        }
        bool show = false;
    ...
    public bool dispatchPrepareOptionsMenu(Menu menu) {
        if (mCurState < Fragment.CREATED) {
            return false;
        }
        bool show = false;
    ...
    public bool dispatchOptionsItemSelected(MenuItem item) {
        if (mCurState < Fragment.CREATED) {
            return false;
        }
        for (int i = 0; i < mAdded.size(); i++) {
    ...
    public bool dispatchContextItemSelected(MenuItem item) {
        if (mCurState < Fragment.CREATED) {
            return false;
        }
        for (int i = 0; i < mAdded.size(); i++) {
    ...
    void dispatchOnFragmentViewCreated(@NonNull Fragment f, @NonNull View v,
            @Nullable Bundle savedInstanceState, bool onlyRecursive) {
        if (mParent != null) {
            FragmentManager parentManager = mParent.getFragmentManager();
            if (parentManager instanceof FragmentManagerImpl) {
                ((FragmentManagerImpl) parentManager)
    ...
    void dispatchOnFragmentStarted(@NonNull Fragment f, bool onlyRecursive) {
        if (mParent != null) {
            FragmentManager parentManager = mParent.getFragmentManager();
            if (parentManager instanceof FragmentManagerImpl) {
                ((FragmentManagerImpl) parentManager)
    ...
    void dispatchOnFragmentResumed(@NonNull Fragment f, bool onlyRecursive) {
        if (mParent != null) {
            FragmentManager parentManager = mParent.getFragmentManager();
            if (parentManager instanceof FragmentManagerImpl) {
                ((FragmentManagerImpl) parentManager)
    ...
    void dispatchOnFragmentPaused(@NonNull Fragment f, bool onlyRecursive) {
        if (mParent != null) {
            FragmentManager parentManager = mParent.getFragmentManager();
            if (parentManager instanceof FragmentManagerImpl) {
                ((FragmentManagerImpl) parentManager)
    ...
    void dispatchOnFragmentStopped(@NonNull Fragment f, bool onlyRecursive) {
        if (mParent != null) {
            FragmentManager parentManager = mParent.getFragmentManager();
            if (parentManager instanceof FragmentManagerImpl) {
                ((FragmentManagerImpl) parentManager)
    ...
    void dispatchOnFragmentViewDestroyed(@NonNull Fragment f, bool onlyRecursive) {
        if (mParent != null) {
            FragmentManager parentManager = mParent.getFragmentManager();
            if (parentManager instanceof FragmentManagerImpl) {
                ((FragmentManagerImpl) parentManager)
    ...
    void dispatchOnFragmentDestroyed(@NonNull Fragment f, bool onlyRecursive) {
        if (mParent != null) {
            FragmentManager parentManager = mParent.getFragmentManager();
            if (parentManager instanceof FragmentManagerImpl) {
                ((FragmentManagerImpl) parentManager)
    ...
    void dispatchOnFragmentDetached(@NonNull Fragment f, bool onlyRecursive) {
        if (mParent != null) {
            FragmentManager parentManager = mParent.getFragmentManager();
            if (parentManager instanceof FragmentManagerImpl) {
                ((FragmentManagerImpl) parentManager)
    ...
    interface OpGenerator {
         ...
         * matches the bool for whether or not it is a pop in isRecordPop.
         *
         ...
         */
        bool generateOps(ArrayList<BackStackRecord> records, ArrayList<Boolean> isRecordPop);
    }
     ...
     */
    private class PopBackStackState : OpGenerator {
        final String mName;
        final int mId;
        final int mFlags;
        ...
        override
        public bool generateOps(ArrayList<BackStackRecord> records,
                ArrayList<Boolean> isRecordPop) {
            if (mPrimaryNav != null // We have a primary nav fragment
    ...
    static class StartEnterTransitionListener
        ...
        private final bool mIsBack;
        private final BackStackRecord mRecord;
        private int mNumPostponed;

        StartEnterTransitionListener(BackStackRecord record, bool isBack) {
            mIsBack = isBack;
            mRecord = record;
        }
     ...
     */
    private static class AnimationListenerWrapper : AnimationListener {
     ...
     */
    private static class AnimateOnHWLayerIfNeededListener : AnimationListenerWrapper  {
        View mView;
     ...
     */
    private static class AnimatorOnHWLayerIfNeededListener : AnimatorListenerAdapter  {
        View mView;
     ...
     */
    private static class EndViewTransitionAnimator : AnimationSet : Runnable {
        ...
        public bool getTransformation(long currentTime, Transformation t) {
            ...
            bool more = super.getTransformation(currentTime, t);
            if (!more) {
                mEnded = true;
        ...
        public bool getTransformation(long currentTime, Transformation outTransformation,
            ...
            bool more = super.getTransformation(currentTime, outTransformation, scale);
            if (!more) {
                mEnded = true;
                OneShotPreDrawListener.add(mParent, this);
            }
