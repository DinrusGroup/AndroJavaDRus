 ...
 *
 */
public class Fragment : ComponentCallbacks, OnCreateContextMenuListener, LifecycleOwner,
        ViewModelStoreOwner {
    private static final SimpleArrayMap<String, Class!(T)> sClassMap =
            new SimpleArrayMap<String, Class!(T)>();
    ...
    public static class InstantiationException : RuntimeException {
     ...
     */
    static bool isSupportFragmentClass(Context context, String fname) {
        try {
            Class!(T) clazz = sClassMap.get(fname);
            if (clazz == null) {
                // Class not found in the cache, see if it's real, and try to add it
    ...
    final public bool hasOptionsMenu() {
        return mHasMenu;
    }

    /** @hide */
    @RestrictTo(LIBRARY_GROUP)
    final public bool isMenuVisible() {
        return mMenuVisible;
    }
     ...
     * {@link #onCreateAnimation(int, bool, int)} returns null. Note that if
     * {@link FragmentTransaction#setCustomAnimations(int, int)} was called with
     ...
     * {@link FragmentManagerImpl#moveToState(Fragment, int, int, int, bool)}
     ...
     * {@link FragmentTransaction#setReorderingAllowed(bool) FragmentTransaction reordering} is
     * enabled.
     *
