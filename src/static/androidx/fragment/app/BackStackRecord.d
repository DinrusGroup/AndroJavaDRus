...
final class BackStackState : Parcelable {
    final int[] mOps;
 ...
 */
final class BackStackRecord : FragmentTransaction :
    ...
    bool mAllowAddToBackStack = true;
    ...
    public void dump(String prefix, PrintWriter writer, bool full) {
        if (full) {
    ...
    override
    public FragmentTransaction setReorderingAllowed(bool reorderingAllowed) {
        mReorderingAllowed = reorderingAllowed;
        return this;
    }

    override
    public FragmentTransaction setAllowOptimization(bool allowOptimization) {
        return setReorderingAllowed(allowOptimization);
    }

    int commitInternal(bool allowStateLoss) {
     ...
     */
    override
    public bool generateOps(ArrayList<BackStackRecord> records, ArrayList<Boolean> isRecordPop) {
        if (FragmentManagerImpl.DEBUG) {
            Log.v(TAG, "Run: " + this);
        }
    ...
    bool interactsWith(int containerId) {
        final int numOps = mOps.size();
        for (int opNum = 0; opNum < numOps; opNum++) {
            final Op op = mOps.get(opNum);
    ...
    bool interactsWith(ArrayList<BackStackRecord> records, int startIndex, int endIndex) {
     ...
     */
    void executePopOps(bool moveToState) {
    ...
    private static bool isFragmentPostponed(Op op) {
