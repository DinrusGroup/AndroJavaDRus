...
class TestUtils {
    ...
    static <T : Activity> T recreateActivity(final T activity, ActivityTestRule rule)
            throws Throwable {
    ...
    static void waitTillCreated(final LifecycleOwner owner, ActivityTestRule!(T) activityRule)
            throws Throwable {
        waitTillState(owner, activityRule, CREATED);
    }

    static void waitTillStarted(final LifecycleOwner owner, ActivityTestRule!(T) activityRule)
            throws Throwable {
        waitTillState(owner, activityRule, STARTED);
    }

    static void waitTillResumed(final LifecycleOwner owner, ActivityTestRule!(T) activityRule)
            throws Throwable {
        waitTillState(owner, activityRule, RESUMED);
    }

    static void waitTillDestroyed(final LifecycleOwner owner, ActivityTestRule!(T) activityRule)
            throws Throwable {
        waitTillState(owner, activityRule, DESTROYED);
    }

    static void waitTillState(final LifecycleOwner owner, ActivityTestRule!(T) activityRule,
        ...
        bool latchResult = latch.await(1, TimeUnit.MINUTES);
