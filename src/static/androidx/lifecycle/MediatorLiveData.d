...
public class MediatorLiveData<T> : MutableLiveData<T> {
    private SafeIterableMap<LiveData!(T), Source!(T)> mSources = new SafeIterableMap<>();
    ...
    public <S> void addSource(@NonNull LiveData<S> source, @NonNull Observer<? super S> onChanged) {
        ...
        Source!(T) existing = mSources.putIfAbsent(source, e);
    ...
    public <S> void removeSource(@NonNull LiveData<S> toRemote) {
        Source!(T) source = mSources.remove(toRemote);
        if (source != null) {
            source.unplug();
        }
    }
    ...
    protected void onActive() {
        for (Map.Entry<LiveData!(T), Source!(T)> source : mSources) {
    ...
    private static class Source<V> : Observer<V> {
