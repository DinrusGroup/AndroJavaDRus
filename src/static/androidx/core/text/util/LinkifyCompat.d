...
public final class LinkifyCompat {
     ...
     */
    public static bool addLinks(@NonNull Spannable text, @LinkifyMask int mask) {
     ...
     *  @return True if at least one link is found and applied.
     */
    public static bool addLinks(@NonNull TextView text, @LinkifyMask int mask) {
        if (shouldAddLinksFallbackToFramework()) {
            return Linkify.addLinks(text, mask);
        }
        if (mask == 0) {
     ...
     *                      prepended to the links that do not start with this scheme.
     */
    public static bool addLinks(@NonNull Spannable text, @NonNull Pattern pattern,
            @Nullable String scheme) {
        if (shouldAddLinksFallbackToFramework()) {
     ...
     * @return True if at least one link is found and applied.
     */
    public static bool addLinks(@NonNull Spannable spannable, @NonNull Pattern pattern,
            @Nullable String scheme, @Nullable MatchFilter matchFilter,
            @Nullable TransformFilter transformFilter) {
        if (shouldAddLinksFallbackToFramework()) {
