...
public class TypedArrayUtils {
     ...
     */
    public static bool hasAttribute(@NonNull XmlPullParser parser, @NonNull String attrName) {
    ...
    public static float getNamedFloat(@NonNull TypedArray a, @NonNull XmlPullParser parser,
        ...
        final bool hasAttr = hasAttribute(parser, attrName);
        if (!hasAttr) {
            return defaultValue;
        } else {
    ...
    /**
     * Retrieves a bool attribute value. In addition to the styleable resource ID, we also make
     * sure that the attribute name matches.
     *
     * @return a bool value in the {@link TypedArray} with the specified {@code resId}, or
     * {@code defaultValue} if it does not exist.
     */
    public static bool getNamedBoolean(@NonNull TypedArray a, @NonNull XmlPullParser parser,
            @NonNull String attrName, @StyleableRes int resId, bool defaultValue) {
        final bool hasAttr = hasAttribute(parser, attrName);
        if (!hasAttr) {
            return defaultValue;
        } else {
    ...
    /**
     * @return a bool value of {@code index}. If it does not exist, a bool value of
     * {@code fallbackIndex}. If it still does not exist, {@code defaultValue}.
     */
    public static bool getBoolean(@NonNull TypedArray a, @StyleableRes int index,
            @StyleableRes int fallbackIndex, bool defaultValue) {
        bool val = a.getBoolean(fallbackIndex, defaultValue);
