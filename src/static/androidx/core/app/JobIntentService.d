 ...
 *      complete}
 */
public abstract class JobIntentService : Service {
     ...
     */
    static final class CompatWorkEnqueuer : WorkEnqueuer {
     ...
     */
    @RequiresApi(26)
    static final class JobServiceEngineImpl : JobServiceEngine
            : JobIntentService.CompatJobEngine {
        ...
        final class WrapperWorkItem : JobIntentService.GenericWorkItem {
        ...
        override
        public bool onStartJob(JobParameters params) {
        ...
        override
        public bool onStopJob(JobParameters params) {
            if (DEBUG) Log.d(TAG, "onStartJob: " + params);
            bool result = mService.doStopCurrentWork();
            synchronized (mLock) {
    ...
    @RequiresApi(26)
    static final class JobWorkEnqueuer : JobIntentService.WorkEnqueuer {
     ...
     */
    final class CompatWorkItem : GenericWorkItem {
     ...
     */
    final class CommandProcessor : AsyncTask<Void, Void, Void> {
        override
    ...
    static WorkEnqueuer getWorkEnqueuer(Context context, ComponentName cn, bool hasJobId,
            int jobId) {
     ...
     * {@link AsyncTask#cancel(bool) AsyncTask.cancel(true)} to interrupt the running
     * task.
     *
     * @param interruptIfStopped Set to true to allow the system to interrupt actively
     * running work.
     */
    public void setInterruptIfStopped(bool interruptIfStopped) {
    ...
    void ensureProcessorRunningLocked(bool reportStarted) {
        if (mCurProcessor == null) {
