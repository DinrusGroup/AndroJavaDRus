...
final class PagedStorage<T> : AbstractList<T> {
    /**
    ...
    public void insertPage(int position, @NonNull List<T> page, @Nullable Callback callback) {
        ...
        if (newPageSize != mPageSize) {
            ...
            int size = size();
            bool addingLastPage = position == (size - size % mPageSize)
                    && newPageSize < mPageSize;
            bool onlyEndPagePresent = mTrailingNullCount == 0 && mPages.size() == 1
                    && newPageSize > mPageSize;
    ...
    public bool hasPage(int pageSize, int index) {
