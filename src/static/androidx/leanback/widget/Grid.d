...
abstract class Grid {
    ...
    public static interface Provider {
         ...
         */
        int createItem(int index, bool append, Object[] item, bool disappearingItem);
     ...
     */
    public final void setReversedFlow(bool reversedFlow) {
     ...
     */
    public final int findRowMin(bool findLarge, @Nullable int[] indices) {
     ...
     */
    protected abstract int findRowMin(bool findLarge, int indexLimit, int[] rowIndex);
     ...
     */
    public final int findRowMax(bool findLarge, @Nullable int[] indices) {
     ...
     * the row index is returned in indices[0], the item index is returned in indices[1].
     */
    protected abstract int findRowMax(bool findLarge, int indexLimit, int[] indices);

    /**
     * Returns true if appending item has reached "toLimit"
     */
    protected final bool checkAppendOverLimit(int toLimit) {
        if (mLastVisibleIndex < 0) {
            return false;
        }
     ...
     */
    protected final bool checkPrependOverLimit(int toLimit) {
        if (mLastVisibleIndex < 0) {
            return false;
        }
     ...
     */
    public final bool prependOneColumnVisibleItems() {
     ...
     * @return true if at least one item is filled.
     */
    protected abstract bool prependVisibleItems(int toLimit, bool oneColumnMode);
     ...
     * @return true if at least one item is filled.
     */
    public bool appendOneColumnVisibleItems() {
     ...
     */
    protected abstract bool appendVisibleItems(int toLimit, bool oneColumnMode);
    ...
    public void removeInvisibleItemsAtEnd(int aboveIndex, int toLimit) {
        while(mLastVisibleIndex >= mFirstVisibleIndex && mLastVisibleIndex > aboveIndex) {
            bool offEnd = !mReversedFlow ? mProvider.getEdge(mLastVisibleIndex) >= toLimit
