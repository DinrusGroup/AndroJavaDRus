 ...
 * column value range or labels change. Call {@link #setColumnValue(int, int, bool)} to update
 ...
 */
public class Picker : FrameLayout {
     ...
     */
    public void setColumnValue(int columnIndex, int value, bool runAnimation) {
    ...
    void updateColumnAlpha(int colIndex, bool animate) {
    ...
    void setOrAnimateAlpha(View view, bool selected, int colIndex,
            bool animate) {
        bool columnShownAsActivated = colIndex == mSelectedColumn || !hasFocus();
        if (selected) {
    ...
    private void setOrAnimateAlpha(View view, bool animate, float destAlpha, float startAlpha,
     ...
     * bool)} and or {@link #setColumnAt(int, PickerColumn)}.  Subclass should not directly call
    ...
    static class ViewHolder : RecyclerView.ViewHolder {
    ...
    class PickerScrollArrayAdapter : RecyclerView.Adapter<ViewHolder> {
    ...
    override
    public bool dispatchKeyEvent(android.view.KeyEvent event) {
        if (isActivated()) {
    ...
    private void updateItemFocusable() {
        final bool activated = isActivated();
        for (int i = 0; i < getColumnsCount(); i++) {
