...
final class GridLayoutManager : RecyclerView.LayoutManager {
     ...
     */
    final static class LayoutParams : RecyclerView.LayoutParams {
     ...
     */
    abstract class GridLinearSmoothScroller : LinearSmoothScroller {
        bool mSkipOnStopInternal;
     ...
     * during scroll.
     */
    final class PendingMoveSmoothScroller : GridLinearSmoothScroller {
        ...
        private final bool mStaggeredGrid;
        // Number of pending movements on primary direction, negative if PREV_ITEM.
        private int mPendingMoves;

        PendingMoveSmoothScroller(int initialPendingMoves, bool staggeredGrid) {
    ...
    public void setItemAlignmentOffsetWithPadding(bool withPadding) {
        mItemAlignment.mainAxis().setItemAlignmentOffsetWithPadding(withPadding);
        updateChildAlignments();
    }

    public bool isItemAlignmentOffsetWithPadding() {
    ...
    public void setFocusOutAllowed(bool throughFront, bool throughEnd) {
        mFlag = (mFlag & ~PF_FOCUS_OUT_MASKS)
                | (throughFront ? PF_FOCUS_OUT_FRONT : 0)
                | (throughEnd ? PF_FOCUS_OUT_END : 0);
    }

    public void setFocusOutSideAllowed(bool throughStart, bool throughEnd) {
    ...
    protected bool hasDoneFirstLayout() {
        return mGrid != null;
    }
    ...
    bool hasOnChildViewHolderSelectedListener() {
    ...
    private bool processRowSizeSecondary(bool measure) {
     ...
     */
    <E> E getFacet(RecyclerView.ViewHolder vh, Class<? : E> facetClass) {
        E facet = null;
    ...
    private Grid.Provider mGridProvider = new Grid.Provider() {
        ...
        public void addItem(Object item, int index, int length, int rowIndex, int edge) {
            ...
            bool edgeIsMin = !mGrid.isReversedFlow();
            if (edgeIsMin) {
                start = edge;
                end = edge + length;
            } else {
    ...
    private bool appendOneColumnVisibleItems() {
    ...
    private bool prependOneColumnVisibleItems() {
    ...
    private void focusToViewInLayout(bool hadFocus, bool alignToView, int extraDelta,
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        final bool scrollToFocus = !isSmoothScrolling()
        ...
        bool hadFocus = mBaseGridView.hasFocus();
    ...
    void updateScrollLimits() {
        ...
        final bool highAvailable = highVisiblePos == highMaxPos;
        final bool lowAvailable = lowVisiblePos == lowMinPos;
    ...
    public void setSelection(int position, int subposition, bool smooth,
            int primaryScrollExtra) {
    ...
    void scrollToSelection(int position, int subposition,
            bool smooth, int primaryScrollExtra) {
        ...
        // a layout.
        final bool notSmoothScrolling = !isSmoothScrolling();
    ...
    int startPositionSmoothScroller(int position) {
        LinearSmoothScroller linearSmoothScroller = new GridLinearSmoothScroller() {
            ...
            public PointF computeScrollVectorForPosition(int targetPosition) {
                ...
                final bool isStart = (mFlag & PF_REVERSE_FLOW_PRIMARY) != 0
    ...
    private void processPendingMovement(bool forward) {
    ...
    override
    public bool requestChildRectangleOnScreen(RecyclerView parent, View view, Rect rect,
            bool immediate) {
     ...
     */
    void scrollToView(View view, bool smooth) {
        scrollToView(view, view == null ? null : view.findFocus(), smooth);
    }

    void scrollToView(View view, bool smooth, int extraDelta, int extraDeltaSecondary) {
        scrollToView(view, view == null ? null : view.findFocus(), smooth, extraDelta,
                extraDeltaSecondary);
    }

    private void scrollToView(View view, View childView, bool smooth) {
        scrollToView(view, childView, smooth, 0, 0);
    }
    /**
     * Scroll to a given child view and change mFocusPosition. Ignored when in slideOut() state.
     */
    private void scrollToView(View view, View childView, bool smooth, int extraDelta,
            int extraDeltaSecondary) {
        if ((mFlag & PF_SLIDING) != 0) {
            return;
        }
    ...
    bool getScrollPosition(View view, View childView, int[] deltas) {
    ...
    private bool getNoneAlignedPosition(View view, int[] deltas) {
    ...
    private bool getAlignedPosition(View view, View childView, int[] deltas) {
    ...
    private void scrollGrid(int scrollPrimary, int scrollSecondary, bool smooth) {
        if ((mFlag & PF_STAGE_MASK) == PF_STAGE_LAYOUT) {
    ...
    public void setPruneChild(bool pruneChild) {
    ...
    public bool getPruneChild() {
        return (mFlag & PF_PRUNE_CHILD) != 0;
    }

    public void setScrollEnabled(bool scrollEnabled) {
    ...
    void onFocusChanged(bool gainFocus, int direction, Rect previouslyFocusedRect) {
        if (gainFocus) {
    ...
    public View onInterceptFocusSearch(View focused, int direction) {
        ...
        if (direction == View.FOCUS_FORWARD || direction == View.FOCUS_BACKWARD) {
            ...
            if (canScrollHorizontally()) {
                bool rtl = getLayoutDirection() == ViewCompat.LAYOUT_DIRECTION_RTL;
        ...
        final bool isScroll = mBaseGridView.getScrollState() != RecyclerView.SCROLL_STATE_IDLE;
    ...
    bool hasPreviousViewInSameRow(int pos) {
    ...
    override
    public bool onAddFocusables(RecyclerView recyclerView,
    ...
    bool canScrollTo(View view) {
        return view.getVisibility() == View.VISIBLE && (!hasFocus() || view.hasFocusable());
    }

    bool gridOnRequestFocusInDescendants(RecyclerView recyclerView, int direction,
    ...
    private bool gridOnRequestFocusInDescendantsAligned(RecyclerView recyclerView,
            int direction, Rect previouslyFocusedRect) {
        View view = findViewByPosition(mFocusPosition);
        if (view != null) {
            bool result = view.requestFocus(direction, previouslyFocusedRect);
            if (!result && DEBUG) {
    ...
    private bool gridOnRequestFocusInDescendantsUnaligned(RecyclerView recyclerView,
            int direction, Rect previouslyFocusedRect) {
    ...
    final static class SavedState : Parcelable {
    ...
    public bool performAccessibilityAction(Recycler recycler, State state, int action,
        ...
        bool reverseFlowPrimary = (mFlag & PF_REVERSE_FLOW_PRIMARY) != 0;
     ...
     */
    int processSelectionMoves(bool preventScroll, int moves) {
        if (mGrid == null) {
            return moves;
        }
