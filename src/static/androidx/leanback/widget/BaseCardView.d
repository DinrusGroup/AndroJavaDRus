 ...
 * are set by calling {@link #setActivated(bool) setActivated} and
 * {@link #setSelected(bool) setSelected}.
 * <p>
 * See {@link BaseCardView.LayoutParams} for layout attributes.
 * </p>
 */
public class BaseCardView : FrameLayout {
    ...
    private bool mDelaySelectedAnim;
     ...
     * type : one) should run immediately after the card is selected,
     ...
     */
    public void setSelectedAnimationDelayed(bool delay) {
        mDelaySelectedAnim = delay;
    }

    /**
     * Returns a bool indicating if the selected animation will run
     ...
     */
    public bool isSelectedAnimationDelayed() {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        bool infoAnimating = hasInfoRegion() && mInfoVisibility == CARD_REGION_VISIBLE_SELECTED;
    ...
    private bool hasInfoRegion() {
        return mCardType != CARD_TYPE_MAIN_ONLY;
    }

    private bool hasExtraRegion() {
     ...
     */
    private bool isRegionVisible(int regionVisibility) {
     ...
     */
    private bool isCurrentRegionVisible(int regionVisibility) {
        switch (regionVisibility) {
            case CARD_REGION_VISIBLE_ALWAYS:
                return true;
            case CARD_REGION_VISIBLE_ACTIVATED:
    ...
    private void findChildrenViews() {
        ...
        bool infoVisible = hasInfoRegion() && isCurrentRegionVisible(mInfoVisibility);
        bool extraVisible = hasExtraRegion() && mInfoOffset > 0f;
    ...
    private void setInfoViewVisibility(bool visible) {
    ...
    private void applySelectedState(bool focused) {
        removeCallbacks(mAnimationTrigger);
    ...
    // card is selected.
    void animateInfoOffset(bool shown) {
        cancelAnimations();
    ...
    private void animateInfoHeight(bool shown) {
        cancelAnimations();
    ...
    // the card.
    private void animateInfoAlpha(bool shown) {
        cancelAnimations();
    ...
    final class InfoOffsetAnimation : AnimationBase {
    ...
    // for the info fields.
    final class InfoHeightAnimation : AnimationBase {
        private float mStartValue;
        private float mDelta;
    ...
    final class InfoAlphaAnimation : AnimationBase {
        private float mStartValue;
        private float mDelta;
