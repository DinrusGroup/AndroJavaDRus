 ...
 */
abstract class MotionInputHandler<K> : SimpleOnGestureListener {
    ...
    final bool selectItem(@NonNull ItemDetails<K> details) {
    ...
    protected final bool focusItem(@NonNull ItemDetails<K> details) {
        checkArgument(details != null);
        checkArgument(hasSelectionKey(details));
    ...
    final bool isRangeExtension(@NonNull MotionEvent e) {
    ...
    bool shouldClearSelection(@NonNull MotionEvent e, @NonNull ItemDetails<K> item) {
        return !MotionEvents.isCtrlKeyPressed(e)
                && !item.inSelectionHotspot(e)
                && !mSelectionTracker.isSelected(item.getSelectionKey());
    }

    static bool hasSelectionKey(@Nullable ItemDetails!(T) item) {
        return item != null && item.getSelectionKey() != null;
    }

    static bool hasPosition(@Nullable ItemDetails!(T) item) {
