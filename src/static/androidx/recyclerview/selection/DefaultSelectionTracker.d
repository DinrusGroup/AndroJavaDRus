...
public class DefaultSelectionTracker<K> : SelectionTracker<K> {
    ...
    private final bool mSingleSelect;
    ...
    override
    public bool isSelected(@Nullable K key) {
    ...
    override
    public bool setItemsSelected(@NonNull Iterable<K> keys, bool selected) {
        bool changed = setItemsSelectedQuietly(keys, selected);
        notifySelectionChanged();
        return changed;
    }

    private bool setItemsSelectedQuietly(@NonNull Iterable<K> keys, bool selected) {
        bool changed = false;
        for (K key: keys) {
            bool itemChanged = selected
    ...
    override
    public bool select(@NonNull K key) {
    ...
    override
    public bool deselect(@NonNull K key) {
        checkArgument(key != null);
    ...
    private bool canSetState(@NonNull K key, bool nextState) {
     ...
     */
    private void notifyItemStateChanged(@NonNull K key, bool selected) {
        checkArgument(key != null);
    ...
    private void updateForRange(int begin, int end, bool selected, @RangeType int type) {
        switch (type) {
    ...
    private void updateForRegularRange(int begin, int end, bool selected) {
    ...
    private void updateForProvisionalRange(int begin, int end, bool selected) {
        checkArgument(end >= begin);
        ...
        for (int i = begin; i <= end; i++) {
            ...
            if (selected) {
                bool canSelect = canSetState(key, true);
    ...
    private final class RangeCallbacks : Range.Callbacks {
        override
        void updateForRange(int begin, int end, bool selected, int type) {
            switch (type) {
                case Range.TYPE_PRIMARY:
                    updateForRegularRange(begin, end, selected);
                    break;
    ...
    private static final class AdapterObserver : AdapterDataObserver {

        private final DefaultSelectionTracker!(T) mSelectionTracker;

        AdapterObserver(@NonNull DefaultSelectionTracker!(T) selectionTracker) {
