...
final class GridModel<K> {
     ...
     */
    private bool isPossiblePositionNearestOrigin(int columnIndex, int columnStartIndex,
     ...
     */
    private static class Limits : Comparable<Limits> {
    ...
    private static class RelativePoint {
        ...
        override
        public bool equals(@Nullable Object other) {
    ...
    private bool areItemsCoveredByBand(
    ...
    private bool doesCoordinateLocationCoverItems(
     ...
     * @param <K> Selection key type. @see {@link StorageStrategy} for supported types.
     */
    abstract static class GridHost<K> : BandSelectionHelper.BandHost<K> {
         ...
         */
        abstract bool hasView(int adapterPosition);
    }
}
