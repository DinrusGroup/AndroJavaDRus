...
public class SortedList<T> {
     ...
     */
    public void addAll(@NonNull T[] items, bool mayModifyInput) {
        throwIfInMutationOperation();
        if (items.length == 0) {
            return;
        }
     ...
     *
     * @see SortedList#addAll(T[] items, bool mayModifyInput)
     *
     * @param items Array of items to be added into the list.
     */
     ...
     * @see #replaceAll(T[])
     */
    public void replaceAll(@NonNull T[] items, bool mayModifyInput) {
        throwIfInMutationOperation();
    ...
    private void replaceAllInternal(@NonNull T[] newData) {
        final bool forceBatchedUpdates = !(mCallback instanceof BatchedCallback);
     ...
     * </pre>
     * <p>
     * Instead of using this method to batch calls, you can use a Callback that :
    ...
    private int add(T item, bool notify) {
    ...
    private void removeItemAtIndex(int index, bool notify) {
     ...
     * data.
     */
    public static abstract class Callback<T2> : Comparator<T2>, ListUpdateCallback {
         ...
         */
        abstract public bool areContentsTheSame(T2 oldItem, T2 newItem);
         ...
         */
        abstract public bool areItemsTheSame(T2 item1, T2 item2);
     ...
     */
    public static class BatchedCallback<T2> : Callback<T2> {
        ...
        override
        public bool areContentsTheSame(T2 oldItem, T2 newItem) {
            return mWrappedCallback.areContentsTheSame(oldItem, newItem);
        }

        override
        public bool areItemsTheSame(T2 item1, T2 item2) {
