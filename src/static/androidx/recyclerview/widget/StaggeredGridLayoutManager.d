 ...
 */
public class StaggeredGridLayoutManager : RecyclerView.LayoutManager :
     ...
     */
    private bool mLaidOutInvalidFullSpan = false;

    /**
     * Works the same way as {@link android.widget.AbsListView#setSmoothScrollbarEnabled(bool)}.
     * see {@link android.widget.AbsListView#setSmoothScrollbarEnabled(bool)}
     */
    ...
    private bool checkSpanForGap(Span span) {
        if (mShouldReverseLayout) {
     ...
     */
    public void setReverseLayout(bool reverseLayout) {
        assertNotInLayoutOrScroll(null);
    ...
    private void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        if (shouldCheckForGaps && !state.isPreLayout()) {
            final bool needToCheckForGaps = mGapStrategy != GAP_HANDLING_NONE
                    && getChildCount() > 0
    ...
    private bool updateAnchorFromChildren(RecyclerView.State state, AnchorInfo anchorInfo) {
     ...
     * ({@link #setReverseLayout(bool)}). Views are sorted by their positions in the adapter,
     * not in the layout.
     * <p>
    ...
    private void measureChildWithDecorationsAndMargin(View child, int widthSpec,
        ...
        final bool measure = alreadyMeasured
    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutState,
        ...
        while (layoutState.hasMore(state)
            ...
            Span currentSpan;
            final bool assignSpan = spanIndex == LayoutParams.INVALID_SPAN_ID;
            if (assignSpan) {
     ...
     */
    private bool preferLastSpan(int layoutDir) {
        if (mOrientation == HORIZONTAL) {
    ...
    private Span getNextSpan(LayoutState layoutState) {
        final bool preferLastSpan = preferLastSpan(layoutState.mLayoutDirection);
    ...
    public void collectAdjacentPrefetchPositions(int dx, int dy, RecyclerView.State state,
         ...
         * will be assigned, one-to-one, to spans, where ordering is based on which span  :
        ...
        for (int i = 0; i < mSpanCount; i++) {
            // compute number of pixels past the edge of the viewport that the current span :
            ...
            if (distance >= 0) {
                // span : to the edge, so prefetch next item
    ...
    override
    public bool checkLayoutParams(RecyclerView.LayoutParams lp) {
     ...
     */
    public static class LayoutParams : RecyclerView.LayoutParams {
         ...
         * @see #isFullSpan()
         */
        public void setFullSpan(bool fullSpan) {
            mFullSpan = fullSpan;
        }
    ...
    static class LazySpanLookup {
         ...
         */
        static class FullSpanItem : Parcelable {
