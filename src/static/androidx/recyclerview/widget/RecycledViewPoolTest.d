...
public class RecycledViewPoolTest {
    ...
    private void onAdapterChanged(
            int attachCount,
            bool oldAdapterNotNull,
            bool notCompatibleWithPrevious,
            bool clears) {
        RecyclerView.RecycledViewPool pool = new RecyclerView.RecycledViewPool();
        pool.putRecycledView(makeHolder(1));
    ...
    private static class MockViewHolder : RecyclerView.ViewHolder {
        MockViewHolder(Context context) {
            super(new View(context));
        }
    }

    private class TestAdapter : RecyclerView.Adapter {
