...
public class GridLayoutManagerCustomSizeInScrollDirectionTest : BaseGridLayoutManagerTest {
    @Parameterized.Parameters(name = "addDecorOffsets:{1},addMargins:{2},config:{0}")
    public static List<Object[]> getParams() {
        List<Object[]> params = new ArrayList<>();
        Boolean[] options = new Boolean[]{true, false};
        for (bool addMargins : options) {
            for (bool addDecorOffsets : options) {
    ...
    private final bool mAddDecorOffsets;
    private final bool mAddMargins;
    private final Config mConfig;

    public GridLayoutManagerCustomSizeInScrollDirectionTest(Config config, bool addDecorOffsets,
            bool addMargins) {
        mConfig = config;
