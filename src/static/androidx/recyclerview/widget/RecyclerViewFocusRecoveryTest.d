...
public class RecyclerViewFocusRecoveryTest : BaseRecyclerViewInstrumentationTest {
    ...
    public RecyclerViewFocusRecoveryTest(bool focusOnChild, bool disableRecovery,
    ...
    void setupBasic(bool hasStableIds) throws Throwable {
    ...
    private void assertFocusTransition(RecyclerView.ViewHolder oldVh,
            RecyclerView.ViewHolder newVh, bool typeChanged) {
        if (mDisableRecovery) {
            if (mDisableAnimation) {
                return;
            }
    ...
    private void testFocusRecoveryInTypeChange(bool withAnimation) throws Throwable {
        setupBasic();
        if (!mDisableAnimation) {
    ...
    private void recoverAdapterChangeViaStableId(final bool swap, final bool changeType)
            throws Throwable {
        setupBasic(true);
    ...
    private void assertFocus(RecyclerView.ViewHolder viewHolder, bool hasFocus) {
        assertFocus("", viewHolder, hasFocus);
    }

    private void assertFocus(String msg, RecyclerView.ViewHolder vh, bool hasFocus) {
        FocusViewHolder fvh = cast(vh);
        assertThat(msg, fvh.getViewToFocus().hasFocus(), is(hasFocus));
    }

    private <T : FocusViewHolder> T cast(RecyclerView.ViewHolder vh) {
        assertThat(vh, instanceOf(FocusViewHolder.class));
        //noinspection unchecked
        return (T) vh;
    }

    private class FocusTestAdapter : TestAdapter {
    ...
    private class FocusLayoutManager : TestLayoutManager {
        override
        public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
            detachAndScrapAttachedViews(recycler);
            layoutRange(recycler, 0, state.getItemCount());
    ...
    private class FocusViewHolderWithChildren : FocusViewHolder {
    ...
    private class SimpleFocusViewHolder : FocusViewHolder {
    ...
    private abstract class FocusViewHolder : TestViewHolder {
        ...
        abstract void setFocusable(bool focusable);
