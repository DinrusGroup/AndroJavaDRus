...
public class EnqueueRunnable : Runnable {
    ...
    public void run() {
        ...
        bool needsScheduling = addToDatabase();
        if (needsScheduling) {
    ...
    public bool addToDatabase() {
        ...
        try {
            bool needsScheduling = processContinuation(mWorkContinuation);
    ...
    private static bool processContinuation(@NonNull WorkContinuationImpl workContinuation) {
    ...
    private static bool enqueueContinuation(@NonNull WorkContinuationImpl workContinuation) {
        Set<String> prerequisiteIds = WorkContinuationImpl.prerequisitesFor(workContinuation);

        bool needsScheduling = enqueueWorkWithPrerequisites(
     ...
     */
    private static bool enqueueWorkWithPrerequisites(
            WorkManagerImpl workManagerImpl,
            @NonNull List<? : WorkRequest> workList,
        ...
        bool hasPrerequisite = (prerequisiteIds != null && prerequisiteIds.length > 0);
        bool hasCompletedAllPrerequisites = true;
        bool hasFailedPrerequisites = false;
        bool hasCancelledPrerequisites = false;
        ...
        bool isNamed = !TextUtils.isEmpty(name);

        // We only apply existing work policies for unique tag sequences that are the beginning of
        // chains.
        bool shouldApplyExistingWorkPolicy = isNamed && !hasPrerequisite;
