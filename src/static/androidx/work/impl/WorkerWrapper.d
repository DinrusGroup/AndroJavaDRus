...
public class WorkerWrapper : Runnable {
    ...
    private volatile bool mInterrupted;
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
    public void interrupt(bool cancelled) {
        mInterrupted = true;
    ...
    private bool tryCheckForInterruptionAndNotify() {
        if (mInterrupted) {
    ...
    private void notifyListener(final bool isSuccessful, final bool needsReschedule) {
        if (mListener == null) {
            return;
        }
    ...
    private void resetPeriodicAndNotify(bool isSuccessful) {
        mWorkDatabase.beginTransaction();
        try {
    ...
    public static Worker workerFromClassName(
        ...
        try {
            Class!(T) clazz = Class.forName(workerClassName);
