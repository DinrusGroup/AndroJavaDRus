...
public class NetworkStateTracker : ConstraintTracker<NetworkState> {
    ...
    private static bool isNetworkCallbackSupported() {
    ...
    private NetworkState getActiveNetworkState() {
        ...
        bool isConnected = info != null && info.isConnected();
        bool isValidated = isActiveNetworkValidated();
        bool isMetered = ConnectivityManagerCompat.isActiveNetworkMetered(mConnectivityManager);
        bool isNotRoaming = info != null && !info.isRoaming();
        return new NetworkState(isConnected, isValidated, isMetered, isNotRoaming);
    }

    private bool isActiveNetworkValidated() {
    ...
    @RequiresApi(24)
    private class NetworkStateCallback : NetworkCallback {
        override
