...
final class EmojiProcessor {
    ...
    CharSequence process(@NonNull final CharSequence charSequence, @IntRange(from = 0) int start,
        ...
        final bool isSpannableBuilder = charSequence instanceof SpannableBuilder;
     ...
     */
    static bool handleOnKeyDown(@NonNull final Editable editable, final int keyCode,
    ...
    private static bool delete(final Editable content, final KeyEvent event,
            final bool forwardDelete) {
     ...
     * @return {@code true} if an {@link EmojiSpan} is deleted
     */
    static bool handleDeleteSurroundingText(@NonNull final InputConnection inputConnection,
            @NonNull final Editable editable, @IntRange(from = 0) final int beforeLength,
            @IntRange(from = 0) final int afterLength, final bool inCodePoints) {
    ...
    private static bool hasInvalidSelection(final int start, final int end) {
        return start == -1 || end == -1 || start != end;
    }

    private static bool hasModifiers(KeyEvent event) {
     ...
     */
    private bool hasGlyph(final CharSequence charSequence, int start, final int end,
        ...
        if (metadata.getHasGlyph() == EmojiMetadata.HAS_GLYPH_UNKNOWN) {
            final bool hasGlyph = mGlyphChecker.hasGlyph(charSequence, start, end);
    ...
    static final class ProcessorSm {
        ...
        ProcessorSm(MetadataRepo.Node rootNode, bool useEmojiAsDefaultStyle,
         ...
         */
        private static bool isEmojiStyle(int codePoint) {
         ...
         */
        private static bool isTextStyle(int codePoint) {
    ...
    private static final class CodepointIndexFinder {
        ...
        private static int findIndexBackward(final CharSequence cs, final int from,
            ...
            bool waitingHighSurrogate = false;
        ...
        private static int findIndexForward(final CharSequence cs, final int from,
                final int numCodePoints) {
            int currentIndex = from;
            bool waitingLowSurrogate = false;
            final int length = cs.length();
            if (currentIndex < 0 || length < currentIndex) {
                return INVALID_INDEX;  // The starting point is out of range.
            }
    ...
    public static class GlyphChecker {
         ...
         * @return {@code true} if the OS can render emoji, {@code false} otherwise
         */
        public bool hasGlyph(final CharSequence charSequence, int start, final int end) {
