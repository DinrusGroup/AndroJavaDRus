...
public class BidiBenchmark {
    ...
    public void time_createBidiFromCharArray(int reps) {
        for (int i = 0; i < reps; i++) {
            Bidi bd = new Bidi(new char[]{'s', 's', 's'}, 0, new byte[]{cast(byte) 1,
                    cast(byte) 2, cast(byte) 3}, 0, 3, Bidi.DIRECTION_RIGHT_TO_LEFT);
        }
    }
    ...
    public void time_hebrewBidi(int reps) {
        for (int i = 0; i < reps; i++) {
            Bidi bd = new Bidi(new char[]{'\u05D0', '\u05D0', '\u05D0'}, 0,
                    new byte[]{cast(byte) -1, cast(byte) -2, cast(byte) -3}, 0, 3,
