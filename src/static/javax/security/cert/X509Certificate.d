    ...
    throws CertificateException {
        ...
        try {
            ...
            if (value instanceof InputStream) {
                params = new Class!(T)[] { InputStream.class };
            } else if (value instanceof byte[]) {
                params = new Class!(T)[] { value.getClass() };
            } else
                throw new CertificateException("Unsupported argument type");
            Class!(T) certClass = Class.forName(className);

            // get the appropriate constructor and instantiate it
            Constructor!(T) cons = certClass.getConstructor(params);
