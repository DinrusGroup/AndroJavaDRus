...
final class MethodTypeForm {
    ...
    protected MethodTypeForm(MethodType erasedType) {
        ...
        Class!(T)[] ptypes = erasedType.ptypes();
        ...
        Class!(T) rt = erasedType.returnType();
        Class!(T) bt = rt;
        if (rt != Object.class) {
    ...
    public static MethodType canonicalize(MethodType mt, int howRet, int howArgs) {
        Class!(T)[] ptypes = mt.ptypes();
        Class!(T)[] ptc = MethodTypeForm.canonicalizeAll(ptypes, howArgs);
        Class!(T) rtype = mt.returnType();
        Class!(T) rtc = MethodTypeForm.canonicalize(rtype, howRet);
     ...
     */
    static Class!(T) canonicalize(Class!(T) t, int how) {
        Class!(T) ct;
        if (t == Object.class) {
            // no change, ever
     ...
     */
    static Class!(T)[] canonicalizeAll(Class!(T)[] ts, int how) {
        Class!(T)[] cs = null;
        for (int imax = ts.length, i = 0; i < imax; i++) {
            Class!(T) c = canonicalize(ts[i], how);
            if (c == void.class)
