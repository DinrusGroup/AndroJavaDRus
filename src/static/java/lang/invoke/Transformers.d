...
public class Transformers {
    ...
    /** @hide */
    public static abstract class Transformer : MethodHandle : Cloneable {
     ...
     * @hide
     */
    public static class AlwaysThrow : Transformer {
        private final Class<? : Throwable> exceptionType;

        public AlwaysThrow(Class!(T) nominalReturnType, Class<? :  Throwable> exType) {
     ...
     */
    public static class DropArguments : Transformer {
     ...
     */
    public static class CatchException : Transformer {
        ...
        public CatchException(MethodHandle target, MethodHandle handler, Class!(T) exType) {
            super(target.type());
     ...
     */
    public static class GuardWithTest : Transformer {
        ...
        public void transform(EmulatedStackFrame emulatedStackFrame) throws Throwable {
            ...
            // We know that the return value for test is going to be bool.class, so we don't have
            // to do the copyReturnValue dance.
            final bool value = (bool) test.invoke(testFrame);
            if (value) {
                target.invoke(emulatedStackFrame);
            } else {
     ...
     */
    public static class ReferenceArrayElementGetter : Transformer {
        private final Class!(T) arrayClass;

        public ReferenceArrayElementGetter(Class!(T) arrayClass) {
            super(MethodType.methodType(arrayClass.getComponentType(),
                    new Class!(T)[]{arrayClass, int.class}));
     ...
     */
    public static class ReferenceArrayElementSetter : Transformer {
        private final Class!(T) arrayClass;

        public ReferenceArrayElementSetter(Class!(T) arrayClass) {
            super(MethodType.methodType(void.class,
                    new Class!(T)[] { arrayClass, int.class, arrayClass.getComponentType() }));
            this.arrayClass = arrayClass;
        }
     ...
     */
    public static class ReferenceIdentity : Transformer {
        private final Class!(T) type;

        public ReferenceIdentity(Class!(T) type) {
     ...
     */
    public static class Constant : Transformer {
        private final Class!(T) type;
        ...
        public Constant(Class!(T) type, Object value) {
            ...
            if (!type.isPrimitive()) {
                ...
                asInt = (int) cast(short) value;
                typeChar = 'S';
            } else if (type == byte.class) {
                asInt = (int) cast(byte) value;
                typeChar = 'B';
            } else if (type == bool.class) {
                asInt = ((bool) value) ? 1 : 0;
                typeChar = 'Z';
        ...
        public void transform(EmulatedStackFrame emulatedStackFrame) throws Throwable {
            ...
            switch (typeChar) {
                ...
                case 'S' : { writer.putNextShort(cast(short) asInt); break; }
                case 'B' : { writer.putNextByte(cast(byte) asInt); break; }
    ...
    /*package*/ static class Construct : Transformer {
        ...
        private static bool isAbstract(Class!(T) klass) {
            return (klass.getModifiers() & Modifier.ABSTRACT) == Modifier.ABSTRACT;
        }

        private static void checkInstantiable(Class!(T) klass) throws InstantiationException {
            if (isAbstract(klass)) {
        ...
        override
        public void transform(EmulatedStackFrame emulatedStackFrame) throws Throwable {
            final Class!(T) receiverType = type().rtype();
     ...
     * @hide
     */
    public static class BindTo : Transformer {
        private final MethodHandle delegate;
     ...
     */
    public static class FilterReturnValue : Transformer {
        private final MethodHandle target;
     ...
     * @hide
     */
    public static class PermuteArguments : Transformer {
        private final MethodHandle target;
        ...
        public void transform(EmulatedStackFrame emulatedStackFrame) throws Throwable {
            ...
            final Class!(T)[] ptypes = type().ptypes();
            for (int i = 0; i < ptypes.length; ++i) {
                final Class!(T) ptype = ptypes[i];
                if (!ptype.isPrimitive()) {
                    arguments[i] = reader.nextReference(ptype);
                } else if (ptype == bool.class) {
            ...
            for (int i = 0; i < ptypes.length; ++i) {
                int idx = reorder[i];
                final Class!(T) ptype = ptypes[idx];
                final Object argument = arguments[idx];

                if (!ptype.isPrimitive()) {
                    writer.putNextReference(argument, ptype);
                } else if (ptype == bool.class) {
                    writer.putNextBoolean((bool) argument);
                } else if (ptype == byte.class) {
                    writer.putNextByte(cast(byte) argument);
                } else if (ptype == char.class) {
                    writer.putNextChar((char) argument);
                } else if (ptype == short.class) {
                    writer.putNextShort(cast(short) argument);
                } else if (ptype == int.class) {
     ...
     * @hide
     */
    /*package*/ static class VarargsCollector : Transformer {
        ...
        private static bool lastParameterTypeIsAnArray(Class!(T)[] parameterTypes) {
            if (parameterTypes.length == 0) return false;
            return parameterTypes[parameterTypes.length - 1].isArray();
        }

        override
        public bool isVarargsCollector() { return true; }
        ...
        public void transform(EmulatedStackFrame callerFrame) throws Throwable {
            ...
            Class!(T)[] callerPTypes = callerFrameType.ptypes();
            Class!(T)[] targetPTypes = type().ptypes();
            ...
            Class!(T) elementType = targetPTypes[lastTargetIndex].getComponentType();
        ...
        private static bool arityArgumentsConvertible(Class!(T)[] ptypes, int arityStart,
                                                         Class!(T) elementType) {
        ...
        private static Object referenceArray(StackFrameReader reader, Class!(T)[] ptypes,
                                             Class!(T) elementType, int offset, int length) {
            Object arityArray = Array.newInstance(elementType, length);
            for (int i = 0; i < length; ++i) {
                Class!(T) argumentType = ptypes[i + offset];
                Object o = null;
        ...
        private static Object intArray(StackFrameReader reader, Class!(T) ptypes[],
        ...
        private static Object longArray(StackFrameReader reader, Class!(T) ptypes[],
                                        int offset, int length) {
        ...
        private static Object byteArray(StackFrameReader reader, Class!(T) ptypes[],
                                        int offset, int length) {
        ...
        private static Object shortArray(StackFrameReader reader, Class!(T) ptypes[],
                                        int offset, int length) {
        ...
        private static Object charArray(StackFrameReader reader, Class!(T) ptypes[],
                                        int offset, int length) {
        ...
        private static Object boolArray(StackFrameReader reader, Class!(T) ptypes[],
                                        int offset, int length) {
            bool[] arityArray = new bool[length];
            for (int i = 0; i < length; ++i) {
                Class!(T) argumentType = ptypes[i + offset];
                switch (Wrapper.basicTypeChar(argumentType)) {
        ...
        private static Object floatArray(StackFrameReader reader, Class!(T) ptypes[],
                                        int offset, int length) {
        ...
        private static Object doubleArray(StackFrameReader reader, Class!(T) ptypes[],
                                        int offset, int length) {
        ...
        private static Object makeArityArray(MethodType callerFrameType,
                                             ...
                                             Class!(T) arityArrayType) {
            int arityArrayLength = callerFrameType.ptypes().length - indexOfArityArray;
            Class!(T) elementType = arityArrayType.getComponentType();
            Class!(T)[] callerPTypes = callerFrameType.ptypes();
            ...
            switch (elementBasicType) {
                ...
                case 'C': return charArray(callerFrameReader, callerPTypes,
                                           indexOfArityArray, arityArrayLength);
                case 'Z': return boolArray(callerFrameReader, callerPTypes,
                                              indexOfArityArray, arityArrayLength);
        ...
        public static Object collectArguments(char basicComponentType, Class!(T) componentType,
                                              StackFrameReader reader, Class!(T)[] types,
            ...
            switch (basicComponentType) {
                ...
                case 'Z': return boolArray(reader, types, startIdx, length);
        ...
        private static void prepareFrame(EmulatedStackFrame callerFrame,
            ...
            for (int i = 0; i < indexOfArityArray; ++i) {
                Class!(T) ptype = targetMethodType.ptypes()[i];
                copyParameter(callerReader, targetWriter, ptype);
            }

            // Add arity array as last parameter in |targetFrame|.
            Class!(T) arityArrayType = targetMethodType.ptypes()[indexOfArityArray];
        ...
        private static MethodType makeTargetFrameType(MethodType callerType,
            ...
            final Class!(T)[] ptypes = new Class!(T)[ptypesLength];
     ...
     */
    static class Invoker : Transformer {
        private final MethodType targetType;
        private final bool isExactInvoker;
        private final EmulatedStackFrame.Range copyRange;

        Invoker(MethodType targetType, bool isExactInvoker) {
     ...
     */
    static class Spreader : Transformer {
        ...
        Spreader(MethodHandle target, MethodType spreaderType, int numArrayArgs) {
            ...
            final Class!(T) componentType = spreaderType.ptypes()[arrayOffset].getComponentType();
        ...
        public void transform(EmulatedStackFrame callerFrame) throws Throwable {
            ...
            switch (arrayTypeChar) {
                ...
                case 'C':
                    ...
                    break;
                case 'Z':
                    spreadArray((bool[]) arrayObj, writer, type, numArrayArgs, arrayOffset);
                    break;
                case 'F':
        ...
        public static void spreadArray(Object[] array, StackFrameWriter writer, MethodType type,
            ...
            for (int i = 0; i < numArgs; ++i) {
                ...
                switch (Wrapper.basicTypeChar(argumentType)) {
                    ...
                    case 'B': { writer.putNextByte(cast(byte) o); break; }
                    case 'S': { writer.putNextShort(cast(short) o); break; }
                    case 'C': { writer.putNextChar((char) o); break; }
                    case 'Z': { writer.putNextBoolean((bool) o); break; }
        ...
        public static void spreadArray(bool[] array,
                                       StackFrameWriter writer, MethodType type,
                                       int numArgs, int offset) {
            final Class!(T)[] ptypes = type.ptypes();
            for (int i = 0; i < numArgs; ++i) {
     ...
     */
    static class Collector : Transformer {
        private final MethodHandle target;
        ...
        Collector(MethodHandle delegate, Class!(T) arrayType, int length) {
        ...
        public void transform(EmulatedStackFrame callerFrame) throws Throwable {
            ...
            switch (arrayTypeChar) {
                case 'L': {
                    ...
                    final Class!(T) targetType = target.type().ptypes()[arrayOffset];
                    final Class!(T) targetComponentType = targetType.getComponentType();
                    final Class!(T) adapterComponentType = type().lastParameterType();
                ...
                case 'Z': {
                    bool[] array = new bool[numArrayArgs];
                    for (int i = 0; i < numArrayArgs; ++i) {
                        array[i] = reader.nextBoolean();
                    }
                    writer.putNextReference(array, bool[].class);
                    break;
                }
                case 'F': {
     ...
     */
    static class FilterArguments : Transformer {
        /** The target handle. */
        private final MethodHandle target;
        ...
        private static MethodType deriveType(MethodHandle target, int pos, MethodHandle[] filters) {
            final Class!(T)[] filterArgs = new Class!(T)[filters.length];
        ...
        public void transform(EmulatedStackFrame stackFrame) throws Throwable {
            ...
            final Class!(T)[] ptypes = target.type().ptypes();
            for (int i = 0; i < ptypes.length; ++i) {
     ...
     */
    static class CollectArguments : Transformer {
        private final MethodHandle target;
        ...
        CollectArguments(MethodHandle target, MethodHandle collector, int pos,
            ...
            final Class!(T) collectorRType = collector.type().rtype();
     ...
     */
    static class FoldArguments : Transformer {
        private final MethodHandle target;
        ...
        FoldArguments(MethodHandle target, MethodHandle combiner) {
            ...
            final Class!(T) combinerRType = combiner.type().rtype();
     ...
     */
    static class InsertArguments : Transformer {
        private final MethodHandle target;
        private final int pos;
        ...
        public void transform(EmulatedStackFrame stackFrame) throws Throwable {
            ...
            for (int i = 0; i < values.length; ++i) {
                final Class!(T) ptype = ptypes[i + pos];
                if (ptype.isPrimitive()) {
                    if (ptype == bool.class) {
                        writer.putNextBoolean((bool) values[i]);
                    } else if (ptype == byte.class) {
                        writer.putNextByte(cast(byte) values[i]);
                    } else if (ptype == char.class) {
                        writer.putNextChar((char) values[i]);
                    } else if (ptype == short.class) {
                        writer.putNextShort(cast(short) values[i]);
                    } else if (ptype == int.class) {
     ...
     */
    public static class ExplicitCastArguments : Transformer {
        private final MethodHandle target;
        ...
        private void explicitCastArguments(final EmulatedStackFrame callerFrame,
            ...
            final Class!(T)[] fromTypes = type().ptypes();
            final Class!(T)[] toTypes = target.type().ptypes();
        ...
        private void explicitCastReturnValue(final EmulatedStackFrame callerFrame,
                                             final EmulatedStackFrame targetFrame) {
            Class!(T) from = target.type().rtype();
            Class!(T) to = type().rtype();
            if (to != void.class) {
                final StackFrameWriter writer = new StackFrameWriter();
        ...
        private static void throwUnexpectedType(final Class!(T) unexpectedType) {
            throw new InternalError("Unexpected type: " + unexpectedType);
        }

        private static void explicitCastFromBoolean(bool fromValue,
                                                    final StackFrameWriter writer,
                                                    final Class!(T) to) {
            int value = fromValue ? 1 : 0;
            if (to == byte.class) {
                writer.putNextByte(cast(byte) value);
            } else if (to == char.class) {
                writer.putNextChar((char) value);
            } else if (to == short.class) {
                writer.putNextShort(cast(short) value);
        ...
        /**
         * Converts byte value to bool according to
         * {@link java.lang.invoke.MethodHandles#explicitCast()}
         */
        private static bool toBoolean(byte value) {
            return (value & 1) == 1;
        }
        ...
        private static byte readPrimitiveAsByte(final StackFrameReader reader,
                                                final Class!(T) from) {
            if (from == byte.class) {
                return cast(byte) reader.nextByte();
            } else if (from == char.class) {
                return cast(byte) reader.nextChar();
            } else if (from == short.class) {
                return cast(byte) reader.nextShort();
            } else if (from == int.class) {
                return cast(byte) reader.nextInt();
            } else if (from == long.class) {
                return cast(byte) reader.nextLong();
            } else if (from == float.class) {
                return cast(byte) reader.nextFloat();
            } else if (from == double.class) {
                return cast(byte) reader.nextDouble();
            } else {
        ...
        private static short readPrimitiveAsShort(final StackFrameReader reader,
                                                  final Class!(T) from) {
            if (from == byte.class) {
                return cast(short) reader.nextByte();
            } else if (from == char.class) {
                return cast(short) reader.nextChar();
            } else if (from == short.class) {
                return cast(short) reader.nextShort();
            } else if (from == int.class) {
                return cast(short) reader.nextInt();
            } else if (from == long.class) {
                return cast(short) reader.nextLong();
            } else if (from == float.class) {
                return cast(short) reader.nextFloat();
            } else if (from == double.class) {
                return cast(short) reader.nextDouble();
            } else {
                throwUnexpectedType(from);
                return 0;
            }
        ...
        private static void unboxNull(final StackFrameWriter writer, final Class!(T) to) {
            if (to == bool.class) {
                writer.putNextBoolean(false);
            } else if (to == byte.class) {
                writer.putNextByte(cast(byte) 0);
            } else if (to == char.class) {
                writer.putNextChar((char) 0);
            } else if (to == short.class) {
                writer.putNextShort(cast(short) 0);
            } else if (to == int.class) {
        ...
        private static void unboxNonNull(final Object ref, final Class!(T) from,
                                         final StackFrameWriter writer, final Class!(T) to) {
            if (to == bool.class) {
                if (from == Boolean.class) {
                    writer.putNextBoolean((bool) ref);
                } else if (from == Float.class || from == Double.class) {
                    byte b = cast(byte) ((double) ref);
                    writer.putNextBoolean(toBoolean(b));
                } else {
                    byte b = cast(byte) ((long) ref);
                    writer.putNextBoolean(toBoolean(b));
                }
            } else if (to == byte.class) {
                writer.putNextByte(cast(byte) ref);
            } else if (to == char.class) {
                writer.putNextChar((char) ref);
            } else if (to == short.class) {
                writer.putNextShort(cast(short) ref);
            } else if (to == int.class) {
        ...
        private static void unbox(final Object ref, final Class!(T) from,
                                  final StackFrameWriter writer, final Class!(T) to) {
            if (ref == null) {
                unboxNull(writer, to);
            } else {
        ...
        private static void box(final StackFrameReader reader, final Class!(T) from,
                                final StackFrameWriter writer, final Class!(T) to) {
            Object boxed = null;
            if (from == bool.class) {
        ...
        private static void explicitCast(final StackFrameReader reader, final Class!(T) from,
                                         final StackFrameWriter writer, final Class!(T) to) {
            if (from.equals(to)) {
