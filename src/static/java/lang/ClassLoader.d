 ...
 *
 * <blockquote><pre>
 *     class NetworkClassLoader : ClassLoader {
 *         String host;
 *         int port;
 *
...
public abstract class ClassLoader {
     ...
     * interfaces to the proxy class that : all of them. It is declared
     * here so that these generated classes can be unloaded with their class
     * loader.
     *
     * @hide
     */
    public final Map<List<Class!(T)>, Class!(T)> proxyCache =
            new HashMap<List<Class!(T)>, Class!(T)>();
     ...
     * #loadClass(String, bool)} method.  It is invoked by the Java virtual
     * machine to resolve class references.  Invoking this method is equivalent
     * to invoking {@link #loadClass(String, bool) <tt>loadClass(name,
     * false)</tt>}.
     *
     * @param  name
    ...
    protected Class!(T) loadClass(String name, bool resolve)
        throws ClassNotFoundException
    {
            // First, check if the class has already been loaded
            Class!(T) c = findLoadedClass(name);
            if (c == null) {
                try {
                    if (parent != null) {
     ...
     */
    @Deprecated
    protected final Class!(T) defineClass(byte[] b, int off, int len)
        throws ClassFormatError
    {
     ...
     * @since  1.1
     */
    protected final Class!(T) defineClass(String name, byte[] b, int off, int len)
        throws ClassFormatError
    {
        throw new UnsupportedOperationException("can't load this type of class file");
    }
     ...
     * @since  1.5
     */
    protected final Class!(T) defineClass(String name, java.nio.ByteBuffer b,
                                         ProtectionDomain protectionDomain)
        throws ClassFormatError
    {
        throw new UnsupportedOperationException("can't load this type of class file");
     ...
     */
    protected final void resolveClass(Class!(T) c) {
    }
     ...
     * @see  #getParent()
     */
    protected final Class!(T) findSystemClass(String name)
        throws ClassNotFoundException
    {
     ...
     */
    private Class!(T) findBootstrapClassOrNull(String name)
    {
        return null;
    }
     ...
     * @since  1.1
     */
    protected final Class!(T) findLoadedClass(String name) {
        ClassLoader loader;
     ...
     * @since  1.1
     */
    protected final void setSigners(Class!(T) c, Object[] signers) {
    }
    ...
    public Enumeration<URL> getResources(String name) throws IOException {
        ...
        Enumeration<URL>[] tmp = (Enumeration<URL>[]) new Enumeration!(T)[2];
        if (parent != null) {
     ...
     * invoking {@link #setPackageAssertionStatus(String, bool)} or {@link
     ...
     * {@link #setClassAssertionStatus(String, bool)}.  </p>
     *
     * @param  packageName
...
class BootClassLoader : ClassLoader {
    ...
    override
    protected Class!(T) loadClass(String className, bool resolve)
           throws ClassNotFoundException {
        Class!(T) clazz = findLoadedClass(className);
