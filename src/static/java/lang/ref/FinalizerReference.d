 ...
 * @hide
 */
public final class FinalizerReference<T> : Reference<T> {
    ...
    private static FinalizerReference!(T) head = null;

    // The links used to construct the list.
    private FinalizerReference!(T) prev;
    private FinalizerReference!(T) next;
    ...
    public static void add(Object referent) {
        FinalizerReference!(T) reference = new FinalizerReference<Object>(referent, queue);
    ...
    public static void remove(FinalizerReference!(T) reference) {
        synchronized (LIST_LOCK) {
            FinalizerReference!(T) next = reference.next;
            FinalizerReference!(T) prev = reference.prev;
            reference.next = null;
            reference.prev = null;
            if (prev != null) {
                prev.next = next;
    ...
    private static bool enqueueSentinelReference(Sentinel sentinel) {
        synchronized (LIST_LOCK) {
            ...
            for (FinalizerReference!(T) r = head; r != null; r = r.next) {
    ...
    @FastNative
    private native bool makeCircularListIfUnenqueued();
