...
public class ReferenceQueue<T> {
     ...
     * @GuardedBy("lock")
     */
    private bool enqueueLocked(Reference<? : T> r) {
     ...
     */
    bool isEnqueued(Reference<? : T> reference) {
        synchronized (lock) {
     ...
     */
    bool enqueue(Reference<? : T> reference) {
        synchronized (lock) {
    ...
    // @GuardedBy("lock")
    private Reference<? : T> reallyPollLocked() {
        if (head != null) {
            Reference<? : T> r = head;
            if (head == tail) {
                tail = null;
        ...
        throws IllegalArgumentException, InterruptedException
    {
        ...
        synchronized (lock) {
            Reference<? : T> r = reallyPollLocked();
            if (r != null) return r;
     ...
     * @hide
     */
    public static void enqueuePending(Reference!(T) list) {
        Reference!(T) start = list;
        do {
            ReferenceQueue queue = list.queue;
            if (queue == null) {
                Reference!(T) next = list.pendingNext;
     ...
     * @hide
     */
    public static Reference!(T) unenqueued = null;

    static void add(Reference!(T) list) {
        synchronized (ReferenceQueue.class) {
            if (unenqueued == null) {
                unenqueued = list;
            } else {
                // Find the last element in unenqueued.
                Reference!(T) last = unenqueued;
