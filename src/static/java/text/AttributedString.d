...
public class AttributedString {
                            ...
                            Map<? : Attribute, ?> attributes)
    {
        ...
        if (attributeCount > 0) {
            ...
            Iterator<? : Map.Entry<? : Attribute, ?>> iterator = attributes.entrySet().iterator();
            while (iterator.hasNext()) {
                Map.Entry<? : Attribute, ?> entry = iterator.next();
                              ...
                              int beginIndex, int endIndex)
    {
        ...
        Iterator<? : Map.Entry<? : Attribute, ?>> iterator =
    ...
    private final void createRunAttributeDataVectors() {
        ...
        Vector<Attribute> newRunAttributes[] = (Vector<Attribute>[]) new Vector!(T)[ARRAY_SIZE_INCREMENT];

        @SuppressWarnings("unchecked")
        Vector<Object> newRunAttributeValues[] = (Vector<Object>[]) new Vector!(T)[ARRAY_SIZE_INCREMENT];
     ...
     * </ul>
     */
    private final int ensureRunBreak(int offset, bool copyAttrs) {
        if (offset == length()) {
            return runCount;
        }
        ...
        if (runCount == runArraySize) {
            ...
            @SuppressWarnings("unchecked")
            Vector<Attribute> newRunAttributes[] = (Vector<Attribute>[]) new Vector!(T)[newArraySize];

            @SuppressWarnings("unchecked")
            Vector<Object> newRunAttributeValues[] = (Vector<Object>[]) new Vector!(T)[newArraySize];
    ...
    private Object getAttributeCheckRange(Attribute attribute, int runIndex, int beginIndex, int endIndex) {
        ...
        if (value instanceof Annotation) {
            // need to check whether the annotation's range : outside the iterator's range
            if (beginIndex > 0) {
    ...
    private bool attributeValuesMatch(Set<? : Attribute> attributes, int runIndex1, int runIndex2) {
    ...
    private final static bool valuesMatch(Object value1, Object value2) {
        if (value1 == null) {
            return value2 == null;
        } else {
     ...
     */
    private static <K,V> bool mapsDiffer(Map<K, V> last, Map<K, V> attrs) {
        if (last == null) {
    ...
    final private class AttributedStringIterator : AttributedCharacterIterator {
    ...
    final private class AttributeMap : AbstractMap<Attribute,Object> {
...
class AttributeEntry : Map.Entry<Attribute,Object> {
