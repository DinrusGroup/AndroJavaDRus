    ...
    : OutputStream : ObjectOutput, ObjectStreamConstants
{
    ...
    /*
    private static final bool extendedDebugInfo =
        java.security.AccessController.doPrivileged(
            new sun.security.action.GetBooleanAction(
                "sun.io.serialization.extendedDebugInfo")).boolValue();
    */
    private static final bool extendedDebugInfo = false;
     ...
     *          OutputStream.
     */
    protected void annotateClass(Class!(T) cl) throws IOException {
    }
     ...
     * @since   1.3
     */
    protected void annotateProxyClass(Class!(T) cl) throws IOException {
    }
     ...
     * stream.
     *
     * @param   enable bool parameter to enable replacement of objects
     ...
     * @see java.io.SerializablePermission
     */
    protected bool enableReplaceObject(bool enable)
     ...
     * Writes a bool.
     *
     * @param   val the bool to be written
     * @throws  IOException if I/O errors occur while writing to the underlying
     *          stream
     */
    ...
    public static abstract class PutField {
        ...
        /**
         * Put the value of the named bool field into the persistent field.
         *
                         ...
                         cl = cl.getSuperclass())
                    {
                        try {
                            cl.getDeclaredMethod(
                                "writeUnshared", new Class!(T)[] { Object.class });
                            return Boolean.FALSE;
                        } catch (NoSuchMethodException ex) {
                        }
                        try {
                            cl.getDeclaredMethod("putFields", (Class!(T)[]) null);
                            return Boolean.FALSE;
                        } catch (NoSuchMethodException ex) {
                        }
                    }
...
// END Android-changed:  Make Class and ObjectStreamClass replaceable.
            ...
            // END Android-changed:  Make Class and ObjectStreamClass replaceable.
                ...
                writeEnum((Enum!(T)) obj, desc, unshared);
     ...
     */
    private void writeClass(Class!(T) cl, bool unshared) throws IOException {
        ...
        throws IOException
    {
        ...
        Class!(T)[] ifaces = cl.getInterfaces();
     ...
     */
    private void writeString(String str, bool unshared) throws IOException {
        ...
        throws IOException
    {
        ...
        Class!(T) ccl = desc.forClass().getComponentType();
        if (ccl.isPrimitive()) {
            if (ccl == Integer.TYPE) {
                int[] ia = (int[]) array;
     ...
     */
    private void writeEnum(Enum!(T) en,
                           ObjectStreamClass desc,
                           bool unshared)
        throws IOException
    {
    ...
    private class PutFieldImpl : PutField {
        ...
        public void put(String name, bool val) {
        ...
        : OutputStream : DataOutput
    {
         ...
         * http://b/28159133
         */
        private bool warnOnceWhenWriting;
         ...
         * mode.
         */
        bool setBlockDataMode(bool mode) throws IOException {
            if (blkmode == mode) {
                return blkmode;
            }
            drain();
         ...
         * original byte array).
         */
        void write(byte[] b, int off, int len, bool copy)
            throws IOException
        {
        ...
        public void writeShort(int v) throws IOException {
            if (pos + 2 <= MAX_BLOCK_SIZE) {
                Bits.putShort(buf, pos, cast(short) v);
                pos += 2;
            } else {
                dout.writeShort(v);
            }
        ...
        void writeBooleans(bool[] v, int off, int len) throws IOException {
            int endoff = off + len;
            while (off < endoff) {
                if (pos >= MAX_BLOCK_SIZE) {
                    drain();
                ...
                for (int cpos = 0; cpos < csize; cpos++) {
                    ...
                    if (pos <= limit) {
                        if (c <= 0x007F && c != 0) {
                            ...
                            buf[pos + 2] = cast(byte) (0x80 | ((c >> 0) & 0x3F));
                            buf[pos + 1] = cast(byte) (0x80 | ((c >> 6) & 0x3F));
                            buf[pos + 0] = cast(byte) (0xE0 | ((c >> 12) & 0x0F));
                            pos += 3;
                        } else {
                            buf[pos + 1] = cast(byte) (0x80 | ((c >> 0) & 0x3F));
                            buf[pos + 0] = cast(byte) (0xC0 | ((c >> 6) & 0x1F));
                            pos += 2;
                        }
