 ...
 * @since   JDK1.1
 */
public class ObjectStreamClass : Serializable {
    ...
    */
    ...
    private static class Caches {
        ...
        static final ConcurrentMap<WeakClassKey,Reference!(T)> localDescs =
            new ConcurrentHashMap<>();

        /** cache mapping field group/local desc pairs -> field reflectors */
        static final ConcurrentMap<FieldReflectorKey,Reference!(T)> reflectors =
            new ConcurrentHashMap<>();

        /** queue for WeakReferences to local classes */
        private static final ReferenceQueue<Class!(T)> localDescsQueue =
            new ReferenceQueue<>();
        /** queue for WeakReferences to field reflectors keys */
        private static final ReferenceQueue<Class!(T)> reflectorsQueue =
            new ReferenceQueue<>();
    }
    ...
    private bool isEnum;
    /** true if represented class : Serializable */
    private bool serializable;
    /** true if represented class : Externalizable */
    private bool externalizable;
    /** true if desc has data written by class-defined writeObject method */
    private bool hasWriteObjectData;
    /**
     ...
     */
    private bool hasBlockExternalData = true;
    ...
    */
     ...
     */
    public static ObjectStreamClass lookup(Class!(T) cl) {
     ...
     * @since 1.6
     */
    public static ObjectStreamClass lookupAny(Class!(T) cl) {
     ...
     */
    static ObjectStreamClass lookup(Class!(T) cl, bool all) {
        if (!(all || Serializable.class.isAssignableFrom(cl))) {
            return null;
        }
        processQueue(Caches.localDescsQueue, Caches.localDescs);
        WeakClassKey key = new WeakClassKey(cl, Caches.localDescsQueue);
        Reference!(T) ref = Caches.localDescs.get(key);
        Object entry = null;
        if (ref != null) {
            entry = ref.get();
        }
        ...
        if (entry == null) {
            ...
            Reference!(T) newRef = new SoftReference<>(newEntry);
            do {
                if (ref != null) {
    ...
    private static class EntryFuture {
         ...
         */
        synchronized bool set(Object entry) {
            if (this.entry != unset) {
                return false;
            }
            this.entry = entry;
     ...
     */
    private ObjectStreamClass(final Class!(T) cl) {
        this.cl = cl;
        name = cl.getName();
        ...
        Class!(T) superCl = cl.getSuperclass();
                ...
                public Void run() {
                    ...
                    if (externalizable) {
                        ...
                        writeObjectMethod = getPrivateMethod(cl, "writeObject",
                            new Class!(T)[] { ObjectOutputStream.class },
                            Void.TYPE);
                        readObjectMethod = getPrivateMethod(cl, "readObject",
                            new Class!(T)[] { ObjectInputStream.class },
                            Void.TYPE);
     ...
     * returned.
     */
    ObjectStreamField[] getFields(bool copy) {
     ...
     */
    ObjectStreamField getField(String name, Class!(T) type) {
        for (int i = 0; i < fields.length; i++) {
            ObjectStreamField f = fields[i];
    ...
    /**
     * Returns true if represented class : Externalizable, false
     * otherwise.
     */
    ...
    /**
     * Returns true if represented class : Serializable, false
     * otherwise.
     */
    bool isSerializable() {
        requireInitialized();
    ...
    static class ClassDataSlot {
        ...
        ClassDataSlot(ObjectStreamClass desc, bool hasData) {
            this.desc = desc;
        ...
        throws InvalidClassException
    {
        ...
        for (ObjectStreamClass d = this; d != null; d = d.superDesc) {
            ...
            Class!(T) match = null;
            for (Class!(T) c = start; c != end; c = c.getSuperclass()) {
            ...
            if (match != null) {
                for (Class!(T) c = start; c != match; c = c.getSuperclass()) {
     ...
     */
    private ObjectStreamClass getVariantFor(Class!(T) cl)
        throws InvalidClassException
    {
        if (this.cl == cl) {
            return this;
     ...
     */
    private static Constructor!(T) getExternalizableConstructor(Class!(T) cl) {
        try {
            Constructor!(T) cons = cl.getDeclaredConstructor((Class!(T)[]) null);
     ...
     */
    private static Constructor!(T) getSerializableConstructor(Class!(T) cl) {
        Class!(T) initCl = cl;
        ...
        try {
            Constructor!(T) cons = initCl.getDeclaredConstructor((Class!(T)[]) null);
     ...
     * method (if any).
     */
    private static Method getInheritableMethod(Class!(T) cl, String name,
                                               Class!(T)[] argTypes,
                                               Class!(T) returnType)
    {
        Method meth = null;
     ...
     */
    private static Method getPrivateMethod(Class!(T) cl, String name,
                                           Class!(T)[] argTypes,
                                           Class!(T) returnType)
    {
        try {
     ...
     * otherwise.
     */
    private static bool packageEquals(Class!(T) cl1, Class!(T) cl2) {
     ...
     */
    private static String getPackageName(Class!(T) cl) {
        String s = cl.getName();
        int i = s.lastIndexOf('[');
        if (i >= 0) {
            s = s.substring(i + 2);
     ...
     */
    private static String getClassSignature(Class!(T) cl) {
     ...
     */
    private static String getMethodSignature(Class!(T)[] paramTypes,
                                             Class!(T) retType)
    {
        StringBuilder sbuf = new StringBuilder();
        sbuf.append('(');
     ...
     */
    private static ObjectStreamField[] getSerialFields(Class!(T) cl)
        throws InvalidClassException
    {
     ...
     */
    private static ObjectStreamField[] getDeclaredSerialFields(Class!(T) cl)
        throws InvalidClassException
    {
     ...
     */
    private static ObjectStreamField[] getDefaultSerialFields(Class!(T) cl) {
     ...
     * null if none.
     */
    private static Long getDeclaredSUID(Class!(T) cl) {
        try {
     ...
     */
    private static long computeDefaultSUID(Class!(T) cl) {
        ...
        try {
            ...
            if (!cl.isArray()) {
                 ...
                 */
                Class!(T)[] interfaces = cl.getInterfaces();
            ...
            bool checkSuperclass = !(VMRuntime.getRuntime().getTargetSdkVersion()
            ...
            Constructor!(T)[] cons = cl.getDeclaredConstructors();
     ...
     */
    private native static bool hasStaticInitializer(Class!(T) cl, bool checkSuperclass);
    ...
    private static class MemberSignature {
        ...
        public MemberSignature(Constructor!(T) cons) {
            member = cons;
            name = cons.getName();
    ...
    private static class FieldReflector {
        ...
        FieldReflector(ObjectStreamField[] fields) {
            ...
            ArrayList<Class!(T)> typeList = new ArrayList<>();
            ...
            types = typeList.toArray(new Class!(T)[typeList.size()]);
        ...
        throws InvalidClassException
    {
        ...
        Class!(T) cl = (localDesc != null && fields.length > 0) ?
            localDesc.cl : null;
        processQueue(Caches.reflectorsQueue, Caches.reflectors);
        FieldReflectorKey key = new FieldReflectorKey(cl, fields,
                                                      Caches.reflectorsQueue);
        Reference!(T) ref = Caches.reflectors.get(key);
        Object entry = null;
        if (ref != null) {
            entry = ref.get();
        }
     ...
     */
    private static class FieldReflectorKey : WeakReference<Class!(T)> {

        private final String sigs;
        private final int hash;
        private final bool nullClass;

        FieldReflectorKey(Class!(T) cl, ObjectStreamField[] fields,
                          ReferenceQueue<Class!(T)> queue)
        {
            super(cl, queue);
            nullClass = (cl == null);
            StringBuilder sbuf = new StringBuilder();
    ...
    private static long getConstructorId(Class!(T) clazz) {
        ...
        throw new UnsupportedOperationException("ObjectStreamClass.getConstructorId(Class!(T)) is " +
                                                "not supported on SDK " + targetSdkVersion);
    }
    private static Object newInstance(Class!(T) clazz, long constructorId) {
        final int targetSdkVersion = VMRuntime.getRuntime().getTargetSdkVersion();
        if (targetSdkVersion > 0 && targetSdkVersion <= 24) {
            System.logE("WARNING: ObjectStreamClass.newInstance(Class!(T), long) is private API and" +
                        "will be removed in a future Android release.");
            return sun.misc.Unsafe.getUnsafe().allocateInstance(clazz);
        }

        throw new UnsupportedOperationException("ObjectStreamClass.newInstance(Class!(T), long) " +
     ...
     */
    static void processQueue(ReferenceQueue<Class!(T)> queue,
                             ConcurrentMap<? :
                             WeakReference<Class!(T)>, ?> map)
    {
        Reference<? : Class!(T)> ref;
     ...
     *
     **/
    static class WeakClassKey : WeakReference<Class!(T)> {
        /**
         ...
         * with a queue.
         */
        WeakClassKey(Class!(T) cl, ReferenceQueue<Class!(T)> refQueue) {
            super(cl, refQueue);
