 ...
 * party, and the receiver's version : classes that are not extended by
    ...
    : InputStream : ObjectInput, ObjectStreamConstants
{
    ...
    private static final HashMap<String, Class!(T)> primClasses
        = new HashMap<>(8, 1.0F);
    static {
        primClasses.put("bool", bool.class);
    ...
    private static class Caches {
        ...
        static final ReferenceQueue<Class!(T)> subclassAuditsQueue =
            new ReferenceQueue<>();
    }
    ...
    private final bool enableOverride;
     ...
     *          be found.
     */
    protected Class!(T) resolveClass(ObjectStreamClass desc)
        throws IOException, ClassNotFoundException
    {
        ...
        try {
            ...
            Class!(T) cl = primClasses.get(name);
            if (cl != null) {
                return cl;
            } else {
                throw ex;
    ...
    /**
     * Returns a proxy class that : the interfaces named in a proxy
     ...
     * @since 1.3
     */
    protected Class!(T) resolveProxyClass(String[] interfaces)
        throws IOException, ClassNotFoundException
    {
        ...
        Class!(T)[] classObjs = new Class!(T)[interfaces.length];
        for (int i = 0; i < interfaces.length; i++) {
            Class!(T) cl = Class.forName(interfaces[i], false, latestLoader);
     ...
     * @see java.io.SerializablePermission
     */
    protected bool enableResolveObject(bool enable)
    ...
    public static abstract class GetField {
         ...
         */
        public abstract bool defaulted(String name) throws IOException;

        /**
         * Get the value of the named bool field from the persistent field.
         *
         * @param  name the name of the field
         * @param  val the default value to use if <code>name</code> does not
         *         have a value
         * @return the value of the named <code>bool</code> field
         * @throws IOException if there are I/O errors while reading from the
         *         underlying <code>InputStream</code>
     ...
     */
    private static bool auditSubclass(final Class!(T) subcl) {
        Boolean result = AccessController.doPrivileged(
            new PrivilegedAction<Boolean>() {
                public Boolean run() {
                    for (Class!(T) cl = subcl;
     ...
     */
    private Object readObject0(bool unshared) throws IOException {
     ...
     */
    private Object readHandle(bool unshared) throws IOException {
     ...
     * handle table).
     */
    private Class!(T) readClass(bool unshared) throws IOException {
        if (bin.readByte() != TC_CLASS) {
            throw new InternalError();
        }
        ObjectStreamClass desc = readClassDesc(false);
        Class!(T) cl = desc.forClass();
        ...
        throws IOException
    {
        ...
        ClassNotFoundException resolveEx = null;
        bin.setBlockDataMode(true);
        final bool checksRequired = isCustomSubclass();
        try {
     ...
     * assigned handle.
     */
    private String readString(bool unshared) throws IOException {
        String str;
     ...
     */
    private Object readArray(bool unshared) throws IOException {
     ...
     */
    private Enum!(T) readEnum(bool unshared) throws IOException {
        ...
        if (cl != null) {
            try {
                ...
                Enum!(T) en = Enum.valueOf((Class)cl, name);
                result = en;
    ...
    private class GetFieldImpl : GetField {
        ...
        public bool defaulted(String name) throws IOException {
            return (getFieldOffset(name, null) < 0);
        }

        public bool get(String name, bool val) throws IOException {
         ...
         */
        private int getFieldOffset(String name, Class!(T) type) {
     ...
     */
    private static class PeekInputStream : InputStream {
        ...
        : InputStream : DataInput
    {
         ...
         */
        bool setBlockDataMode(bool newmode) throws IOException {
            if (blkmode == newmode) {
                return blkmode;
            }
            if (newmode) {
         ...
         */
        private int readBlockHeader(bool canBlock) throws IOException {
            if (defaultDataEnd) {
                /*
                 * Fix for 4360508: stream is currently at the end of a field
                 * value block written via default serialization; since there
         ...
         * b).
         */
        int read(byte[] b, int off, int len, bool copy) throws IOException {
            if (len == 0) {
                return 0;
            } else if (blkmode) {
                if (pos == end) {
        ...
        public void readFully(byte[] b, int off, int len, bool copy)
            throws IOException
        {
            while (len > 0) {
        ...
        void readBooleans(bool[] v, int off, int len) throws IOException {
     ...
     * This class : an exception-propagation algorithm for
    ...
    private static class HandleTable {
         ...
         */
        void clear() {
            Arrays.fill(status, 0, size, cast(byte) 0);
