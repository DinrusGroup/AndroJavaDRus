 ...
 *     in-memory implementation. Or user can : one and tell CookieManager
 *     to use it.
 *   </li>
 *   <li>
...
public class CookieManager : CookieHandler
{
        ...
        throws IOException
    {
        ...
        bool secureLink = "https".equalsIgnoreCase(uri.getScheme());
    ...
    static private bool isInPortList(String lst, int port) {
        int i = lst.indexOf(",");
        int val = -1;
        while (i > 0) {
            try {
     ...
     */
    private static bool pathMatches(URI uri, HttpCookie cookie) {
    ...
    static class CookiePathComparator : Comparator<HttpCookie> {
