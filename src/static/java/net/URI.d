    ...
    : Comparable<URI>, Serializable
{
                                 ...
                                 int port)
    {
        if (host != null) {
            ...
            bool needBrackets = ((host.indexOf(':') >= 0)
                                    && !host.startsWith("[")
                                    && !host.endsWith("]"));
    ...
    private static void removeDots(char[] path, int[] segs, bool removeLeading) {
        int ns = segs.length;
    ...
    private static String normalize(String ps) {
        ...
        // Controlled by the "bool removeLeading" argument added to normalize().
        return normalize(ps, false);
    }

    private static String normalize(String ps, bool removeLeading) {
    ...
    private static byte decode(char c1, char c2) {
        return cast(byte)(  ((decode(c1) & 0xf) << 4)
                      | ((decode(c2) & 0xf) << 0));
    }
    ...
    private class Parser {
        ...
        private bool requireServerAuthority = false;
        ...
        //
        private bool at(int start, int end, char c) {
        ...
        //
        private bool at(int start, int end, String s) {
            int p = start;
            int sn = s.length();
            if (sn > end - p)
                return false;
        ...
        //
        void parse(bool rsa) throws URISyntaxException {
        ...
        //
        private int scanIPv4Address(int start, int n, bool strict)
            throws URISyntaxException
        {
            int p = start;
            int q;
