...
class Inet6AddressImpl : InetAddressImpl {
    ...
    private native bool isReachable0(byte[] addr, int scope, int timeout, byte[] inf, int ttl, int if_scope) throws IOException;
    */
    override
    ...
    override
    public bool isReachable(InetAddress addr, int timeout, NetworkInterface netif, int ttl) throws IOException {
    ...
    private bool tcpEcho(InetAddress addr, int timeout, InetAddress sourceAddr, int ttl)
            throws IOException {
    ...
    protected bool icmpEcho(InetAddress addr, int timeout, InetAddress sourceAddr, int ttl)
            throws IOException {

        FileDescriptor fd = null;
        try {
            bool isIPv4 = addr instanceof Inet4Address;
            ...
            for (int to = timeout, seq = 0; to > 0; ++seq) {
                ...
                if (size == packet.length) {
                    byte expectedType = isIPv4 ? cast(byte) ICMP_ECHOREPLY
                            : cast(byte) ICMP6_ECHO_REPLY;
                    if (receivedPacket.getAddress().equals(addr)
                            && received[0] == expectedType
                            && received[4] == cast(byte) (icmpId >> 8)
                            && received[5] == cast(byte) icmpId
                            && received[6] == cast(byte) (seq >> 8)
                            && received[7] == cast(byte) seq) {
