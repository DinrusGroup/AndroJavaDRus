...
public class ModifiedUtf8 {
     ...
     */
    public static long countBytes(String s, bool shortLength) throws UTFDataFormatException {
        long counter = 0;
        int strLen = s.length();
    ...
    public static void encode(byte[] dst, int offset, String s) {
        ...
        for (int i = 0; i < strLen; i++) {
            ...
            if (c < '\u0080') {
                if (c == 0) {
                    dst[offset++] = cast(byte) 0xc0;
                    dst[offset++] = cast(byte) 0x80;
                } else {
                    dst[offset++] = cast(byte) c;
                }
            } else if (c < '\u0800') {
                dst[offset++] = cast(byte) ((c >>> 6) | 0xc0);
                dst[offset++] = cast(byte) ((c & 0x3f) | 0x80);
            } else {
                dst[offset++] = cast(byte) ((c >>> 12) | 0xe0);
                dst[offset++] = cast(byte) (((c >>> 6) & 0x3f) | 0x80);
                dst[offset++] = cast(byte) ((c & 0x3f) | 0x80);
            }
        }
    }
    ...
    public static byte[] encode(String s) throws UTFDataFormatException {
        ...
        encode(output, 2, s);
        output[0] = cast(byte) (size >>> 8);
        output[1] = cast(byte) size;
        return output;
    }
