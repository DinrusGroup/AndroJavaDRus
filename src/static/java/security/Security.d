...
// END Android-changed: doPrivileged is stubbed on Android.
     ...
     * provider : the specified algorithm or type for the specified
     ...
     * <p> A provider
     * satisfies this selection criterion iff the provider : the
     * specified algorithm or type for the specified cryptographic service.
    ...
    private static final Map<String, Class!(T)> spiMap =
     ...
     * only.
     */
    private static Class!(T) getSpiClass(String type) {
        Class!(T) clazz = spiMap.get(type);
        if (clazz != null) {
            return clazz;
        }
        try {
    ...
    private static void invalidateSMCache(String key) {
        ...
        final bool pa = key.equals("package.access");
        final bool pd = key.equals("package.definition");
