 ...
 * {@link #on(bool) on}). When it is on, a call to one of the
 * {@code read} methods
...
public class DigestInputStream : FilterInputStream {
     ...
     * digest function is on (see {@link #on(bool) on}), this method
     ...
     *
     * @see MessageDigest#updatecast(byte)
     */
    public int read() throws IOException {
        int ch = in.read();
        if (on && ch != -1) {
            digest.update(cast(byte)ch);
        }
        return ch;
    }
     ...
     * {@link #on(bool) on}), this method will then call {@code update}
