...
public class X509CertSelector : CertSelector {
    ...
    private Set<List!(T)> subjectAlternativeNames;
     ...
     * <p>
     * Note that the bool array supplied here is cloned to protect against
     * subsequent modifications.
     *
     * @param keyUsage a bool array in the same format as the bool
     *                 array returned by
     * {@link X509Certificate#getKeyUsage() X509Certificate.getKeyUsage()}.
     *                 Or {@code null}.
     * @see #getKeyUsage
     */
    public void setKeyUsage(bool[] keyUsage) {
        if (keyUsage == null) {
            this.keyUsage = null;
        } else {
     ...
     */
    public void setMatchAllSubjectAltNames(bool matchAllNames) {
     ...
     */
    public void setSubjectAlternativeNames(Collection<List!(T)> names)
            throws IOException {
        if (names == null) {
            ...
            Set<List!(T)> tempNames = cloneAndCheckNames(names);
    ...
    private void addSubjectAlternativeNameInternal(int type, Object name)
        ...
        if (subjectAlternativeNames == null) {
            subjectAlternativeNames = new HashSet<List!(T)>();
        }
     ...
     * @throws IOException if a parsing error occurs
     */
    private static Set<GeneralNameInterface> parseNames(Collection<List!(T)> names) throws IOException {
        Set<GeneralNameInterface> genNames = new HashSet<GeneralNameInterface>();
        for (List!(T) nameList : names) {
            if (nameList.size() != 2) {
     ...
     */
    static bool equalNames(Collection!(T) object1, Collection!(T) object2) {
     ...
     * @see #getPathToNames
     */
    public void setPathToNames(Collection<List!(T)> names) throws IOException {
    ...
    void setPathToNamesInternal(Set<GeneralNameInterface> names) {
        ...
        pathToNames = Collections.<List!(T)>emptySet();
    ...
    private void addPathToNameInternal(int type, Object name)
        ...
        if (pathToGeneralNames == null) {
            pathToNames = new HashSet<List!(T)>();
     ...
     * check will be done.
     * <p>
     * Note that the bool array returned is cloned to protect against
     * subsequent modifications.
     *
     * @return a bool array in the same format as the bool
     *                 array returned by
     * {@link X509Certificate#getKeyUsage() X509Certificate.getKeyUsage()}.
     *                 Or {@code null}.
     * @see #setKeyUsage
     ...
     */
    public Collection<List!(T)> getSubjectAlternativeNames() {
        if (subjectAlternativeNames == null) {
            return null;
        }
     ...
     */
    private static Set<List!(T)> cloneNames(Collection<List!(T)> names) {
        try {
     ...
     * @throws IOException if a parsing error occurs
     */
    private static Set<List!(T)> cloneAndCheckNames(Collection<List!(T)> names) throws IOException {
        // Copy the Lists and Collection
        Set<List!(T)> namesCopy = new HashSet<List!(T)>();
        for (List!(T) o : names)
        {
     ...
     * @see #setPathToNames
     */
    public Collection<List!(T)> getPathToNames() {
        if (pathToNames == null) {
            return null;
        }
    ...
    public String toString() {
        ...
        if (subjectAlternativeNames != null) {
            ...
            Iterator<List!(T)> i = subjectAlternativeNames.iterator();
            while (i.hasNext()) {
                List!(T) list = i.next();
     ...
     */
    public bool match(Certificate cert) {
        ...
        bool result = matchBasicConstraints(xcert)
                      && matchKeyUsage(xcert)
    ...
    private bool matchKeyUsage(X509Certificate xcert) {
        ...
        bool[] certKeyUsage = xcert.getKeyUsage();
