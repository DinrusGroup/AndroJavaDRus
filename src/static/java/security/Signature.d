 ...
 * {@link #updatecast(byte) update} methods.
 *
 ...
 * </ol>
 *
 * <p>Note that this class is abstract and : from
...
public abstract class Signature : SignatureSpi {
    ...
    */
    ...
    /**
     * Returns a Signature object that : the specified signature
     * algorithm.
     *
    ...
    private static bool isSpi(Service s) {
        ...
        if (result == null) {
            try {
                ...
                bool r = (instance instanceof SignatureSpi)
    ...
    /**
     * Returns a Signature object that : the specified
     * signature algorithm.
     *
     * <p> A new Signature object encapsulating the
     * SignatureSpi implementation from the specified Provider
    ...
    public final void initVerify(Certificate certificate)
        ...
        if (certificate instanceof java.security.cert.X509Certificate) {
            ...
            if (critSet != null && !critSet.isEmpty()
                ...
                bool[] keyUsageInfo = cert.getKeyUsage();
     ...
     */
    public final bool verify(byte[] signature) throws SignatureException {
        if (state == VERIFY) {
     ...
     * @since 1.4
     */
    public final bool verify(byte[] signature, int offset, int length)
        throws SignatureException {
        if (state == VERIFY) {
            if (signature == null) {
    ...
    private static class Delegate : Signature {
...
// END Android-changed: Provider selection; loop over a new list each time.
        ...
        protected bool engineVerify(byte[] sigBytes)
                throws SignatureException {
            chooseFirstProvider();
            return sigSpi.engineVerify(sigBytes);
        }

        protected bool engineVerify(byte[] sigBytes, int offset, int length)
                throws SignatureException {
            chooseFirstProvider();
    ...
    @SuppressWarnings("deprecation")
    private static class CipherAdapter : SignatureSpi {
