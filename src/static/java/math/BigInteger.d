...
public class BigInteger : Number
    ...
    private transient bool nativeIsValid = false;

    private transient bool javaIsValid = false;
    ...
    private static bool isSmallPrime(int x) {
        if (x == 2) {
            return true;
        }
        if ((x % 2) == 0) {
    ...
    private byte[] twosComplement() {
        ...
        if (sign < 0) {
            ...
            for (int i = 0; i < bytesInInteger; i++, digit >>= 8) {
                bytes[--bytesLen] = cast(byte) digit;
            }
