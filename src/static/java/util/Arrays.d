...
public class Arrays {
    ...
    /**
     * A comparator that : the natural ordering of a group of
     ...
     */
    static final class NaturalOrder : Comparator<Object> {
     ...
     * to be sorted : from the index {@code fromIndex}, inclusive, to
     ...
     * The range to be sorted : from the index {@code fromIndex},
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T : Comparable<? super T>> void parallelSort(T[] a) {
        int n = a.length, p, g;
        if (n <= MIN_ARRAY_SORT_GRAN ||
            (p = ForkJoinPool.getCommonPoolParallelism()) == 1)
     ...
     * elements.  The range to be sorted : from index
     ...
     * sorted : from index {@code fromIndex}, inclusive, to index
    ...
    /**
     * Returns <tt>true</tt> if the two specified arrays of bools are
     * <i>equal</i> to one another.  Two arrays are considered equal if both
     * arrays contain the same number of elements, and all corresponding pairs
     * of elements in the two arrays are equal.  In other words, two arrays
     * are equal if they contain the same elements in the same order.  Also,
    ...
    /**
     * Assigns the specified bool value to each element of the specified
     * array of bools.
     *
     * @param a the array to be filled
     * @param val the value to be stored in all elements of the array
    ...
    /**
     * Assigns the specified bool value to each element of the specified
     * range of the specified array of bools.  The range to be filled
     * : from index <tt>fromIndex</tt>, inclusive, to index
     * <tt>toIndex</tt>, exclusive.  (If <tt>fromIndex==toIndex</tt>, the
     * range to be filled is empty.)
     *
     ...
     * valid in both the original array and the copy, the two arrays will
     * contain identical values.  For any indices that are valid in the
     * copy but not the original, the copy will contain <tt>cast(byte)0</tt>.
     * Such indices will exist if and only if the specified length
     * is greater than that of the original array.
     *
     * @param original the array to be copied
     ...
     * valid in both the original array and the copy, the two arrays will
     * contain identical values.  For any indices that are valid in the
     * copy but not the original, the copy will contain <tt>cast(short)0</tt>.
     * Such indices will exist if and only if the specified length
     * is greater than that of the original array.
     *
     * @param original the array to be copied
     ...
     * @since 1.6
     */
    public static <T,U> T[] copyOfRange(U[] original, int from, int to, Class<? : T[]> newType) {
     ...
     * (<tt>to</tt>), which must be greater than or equal to <tt>from</tt>,
     * may be greater than <tt>original.length</tt>, in which case
     * <tt>cast(byte)0</tt> is placed in all elements of the copy whose index is
     * greater than or equal to <tt>original.length - from</tt>.  The length
     * of the returned array will be <tt>to - from</tt>.
     *
     * @param original the array from which a range is to be copied
     ...
     * (<tt>to</tt>), which must be greater than or equal to <tt>from</tt>,
     * may be greater than <tt>original.length</tt>, in which case
     * <tt>cast(short)0</tt> is placed in all elements of the copy whose index is
     * greater than or equal to <tt>original.length - from</tt>.  The length
     * of the returned array will be <tt>to - from</tt>.
     *
     * @param original the array from which a range is to be copied
     ...
     * serializable and : {@link RandomAccess}.
     *
     ...
     * @serial include
     */
    private static class ArrayList<E> : AbstractList<E>
    ...
    public static int deepHashCode(Object a[]) {
        ...
        for (Object element : a) {
            ...
            if (element != null) {
                Class!(T) cl = element.getClass().getComponentType();
                if (cl == null)
    ...
    static bool deepEquals0(Object e1, Object e2) {
        // BEGIN Android-changed: getComponentType() is faster than instanceof()
        Class!(T) cl1 = e1.getClass().getComponentType();
        Class!(T) cl2 = e2.getClass().getComponentType();
     ...
     * separated by the characters <tt>", "</tt> (a comma followed by a
     * space).  Elements are converted to strings as by
     * <tt>String.valueOfcast(short)</tt>.  Returns <tt>"null"</tt> if <tt>a</tt>
     * is <tt>null</tt>.
     *
     * @param a the array whose string representation to return
     * @return a string representation of <tt>a</tt>
     ...
     * <tt>String.valueOfcast(byte)</tt>.  Returns <tt>"null"</tt> if
    ...
    private static void deepToString(Object[] a, StringBuilder buf,
        ...
        for (int i = 0; ; i++) {
            ...
            if (element == null) {
                ...
                Class!(T) eClass = element.getClass();
     ...
     * @since 1.8
     */
    public static <T> void setAll(T[] array, IntFunction<? : T> generator) {
     ...
     * @since 1.8
     */
    public static <T> void parallelSetAll(T[] array, IntFunction<? : T> generator) {
        Objects.requireNonNull(generator);
