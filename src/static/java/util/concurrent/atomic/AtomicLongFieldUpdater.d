...
public abstract class AtomicLongFieldUpdater<T> {
    ...
    public static <U> AtomicLongFieldUpdater<U> newUpdater(Class<U> tclass,
                                                           String fieldName) {
      Class!(T) caller = VMStack.getStackClass1(); // Android-changed
    ...
    private static final class CASUpdater<T> : AtomicLongFieldUpdater<T> {
        ...
        public final bool compareAndSet(T obj, long expect, long update) {
            accessCheck(obj);
            return U.compareAndSwapLong(obj, offset, expect, update);
        }

        public final bool weakCompareAndSet(T obj, long expect, long update) {
            accessCheck(obj);
            return U.compareAndSwapLong(obj, offset, expect, update);
        }
    ...
    private static final class LockedUpdater<T> : AtomicLongFieldUpdater<T> {
        private static final sun.misc.Unsafe U = sun.misc.Unsafe.getUnsafe();
        private final long offset;
        /**
         * if field is protected, the subclass constructing updater, else
    ...
    //  */
    // static bool isAncestor(ClassLoader first, ClassLoader second) {
