 ...
 *
 * class ForEach<E> : CountedCompleter<Void> {
 *
 ...
 *   ForEach(CountedCompleter!(T) p, E[] array, MyOperation<E> op, int lo, int hi) {
 *     super(p);
 ...
 * in this tree : an {@link #onCompletion(CountedCompleter)} method,
 ...
 *
 * <pre> {@code
 * class Searcher<E> : CountedCompleter<E> {
 *   final E[] array; final AtomicReference<E> result; final int lo, hi;
 *   Searcher(CountedCompleter!(T) p, E[] array, AtomicReference<E> result, int lo, int hi) {
 *     super(p);
 ...
 * class MapReducer<E> : CountedCompleter<E> {
 *   final E[] array; final MyMapper<E> mapper;
 *   final MyReducer<E> reducer; final int lo, hi;
 *   MapReducer<E> sibling;
 *   E result;
 *   MapReducer(CountedCompleter!(T) p, E[] array, MyMapper<E> mapper,
 ...
 *
 * <pre> {@code
 * class MapReducer<E> : CountedCompleter<E> { // version 2
 *   final E[] array; final MyMapper<E> mapper;
 *   final MyReducer<E> reducer; final int lo, hi;
 ...
 *     for (CountedCompleter!(T) c = firstComplete(); c != null; c = c.nextComplete()) {
 ...
 *
 * <pre> {@code
 * class HeaderBuilder : CountedCompleter<...> { ... }
 * class BodyBuilder : CountedCompleter<...> { ... }
 * class PacketSender : CountedCompleter<...> {
 *   PacketSender(...) { super(null, 1); ... } // trigger on second completion
 *   public void compute() { } // never called
 *   public void onCompletion(CountedCompleter!(T) caller) { sendPacket(); }
 * }
 * // sample use:
...
public abstract class CountedCompleter<T> : ForkJoinTask<T> {
    ...
    final CountedCompleter!(T) completer;
     ...
     */
    protected CountedCompleter(CountedCompleter!(T) completer,
     ...
     */
    public bool onExceptionalCompletion(Throwable ex, CountedCompleter!(T) caller) {
        return true;
    }
     ...
     */
    public final CountedCompleter!(T) getCompleter() {
        return completer;
    }
     ...
     */
    public final bool compareAndSetPendingCount(int expected, int count) {
     ...
     */
    public final CountedCompleter!(T) getRoot() {
        CountedCompleter!(T) a = this, p;
     ...
     */
    public final CountedCompleter!(T) firstComplete() {
        for (int c;;) {
            if ((c = pending) == 0)
                return this;
     ...
     *
     * <pre> {@code
     * for (CountedCompleter!(T) c = firstComplete();
     *      c != null;
     *      c = c.nextComplete()) {
     *   // ... process c ...
     * }}</pre>
     ...
     */
    public final CountedCompleter!(T) nextComplete() {
        CountedCompleter!(T) p;
        if ((p = completer) != null)
    ...
    public final void quietlyCompleteRoot() {
        for (CountedCompleter!(T) a = this, p;;) {
