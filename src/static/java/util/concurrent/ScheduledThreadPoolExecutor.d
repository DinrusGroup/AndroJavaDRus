 ...
 *
 * <pre> {@code
 * public class CustomScheduledExecutor : ScheduledThreadPoolExecutor {
 *
...
public class ScheduledThreadPoolExecutor
    ...
    private class ScheduledFutureTask<V>
            : FutureTask<V> : RunnableScheduledFuture<V> {
        ...
        public int compareTo(Delayed other) {
            ...
            if (other instanceof ScheduledFutureTask) {
                ScheduledFutureTask!(T) x = (ScheduledFutureTask!(T))other;
     ...
     * @param task the task
     */
    private void delayedExecute(RunnableScheduledFuture!(T) task) {
        if (isShutdown())
            reject(task);
        else {
     ...
     * @param task the task
     */
    void reExecutePeriodic(RunnableScheduledFuture!(T) task) {
        ...
        if (!keepDelayed && !keepPeriodic) {
            for (Object e : q.toArray())
                if (e instanceof RunnableScheduledFuture!(T))
                    ((RunnableScheduledFuture!(T)) e).cancel(false);
            q.clear();
        }
        else {
     ...
     */
    public ScheduledFuture!(T) schedule(Runnable command,
                                       long delay,
                                       TimeUnit unit) {
     ...
     */
    public ScheduledFuture!(T) scheduleAtFixedRate(Runnable command,
                                                  long initialDelay,
                                                  long period,
                                                  TimeUnit unit) {
        if (command == null || unit == null)
     ...
     * @throws IllegalArgumentException   {@inheritDoc}
     */
    public ScheduledFuture!(T) scheduleWithFixedDelay(Runnable command,
                                                     long initialDelay,
                                                     long delay,
                                                     TimeUnit unit) {
        if (command == null || unit == null)
     ...
     * @since 1.7
     */
    public void setRemoveOnCancelPolicy(bool value) {
     ...
     */
    static class DelayedWorkQueue : AbstractQueue<Runnable>
        ...
        private RunnableScheduledFuture!(T)[] queue =
            new RunnableScheduledFuture!(T)[INITIAL_CAPACITY];
         ...
         */
        private void setIndex(RunnableScheduledFuture!(T) f, int idx) {
         ...
         */
        private void siftUp(int k, RunnableScheduledFuture!(T) key) {
            while (k > 0) {
                int parent = (k - 1) >>> 1;
                RunnableScheduledFuture!(T) e = queue[parent];
         ...
         * Call only when holding lock.
         */
        private void siftDown(int k, RunnableScheduledFuture!(T) key) {
            int half = size >>> 1;
            while (k < half) {
                int child = (k << 1) + 1;
                RunnableScheduledFuture!(T) c = queue[child];
                int right = child + 1;
        ...
        public bool remove(Object x) {
            ...
            try {
                ...
                setIndex(queue[i], -1);
                int s = --size;
                RunnableScheduledFuture!(T) replacement = queue[s];
                queue[s] = null;
                if (s != i) {
        ...
        public RunnableScheduledFuture!(T) peek() {
            final ReentrantLock lock = this.lock;
            lock.lock();
            try {
                return queue[0];
        ...
        public bool offer(Runnable x) {
            if (x == null)
                throw new NullPointerException();
            RunnableScheduledFuture!(T) e = (RunnableScheduledFuture!(T))x;
            final ReentrantLock lock = this.lock;
            lock.lock();
            try {
                int i = size;
        ...
        public bool add(Runnable e) {
            return offer(e);
        }

        public bool offer(Runnable e, long timeout, TimeUnit unit) {
            return offer(e);
        }
         ...
         */
        private RunnableScheduledFuture!(T) finishPoll(RunnableScheduledFuture!(T) f) {
            int s = --size;
            RunnableScheduledFuture!(T) x = queue[s];
            queue[s] = null;
            if (s != 0)
                siftDown(0, x);
            setIndex(f, -1);
        ...
        public RunnableScheduledFuture!(T) poll() {
            final ReentrantLock lock = this.lock;
            lock.lock();
            try {
                RunnableScheduledFuture!(T) first = queue[0];
        ...
        public RunnableScheduledFuture!(T) take() throws InterruptedException {
            final ReentrantLock lock = this.lock;
        ...
        public RunnableScheduledFuture!(T) poll(long timeout, TimeUnit unit)
        ...
        public void clear() {
            ...
            try {
                for (int i = 0; i < size; i++) {
                    RunnableScheduledFuture!(T) t = queue[i];
                    if (t != null) {
                        queue[i] = null;
                        setIndex(t, -1);
                    }
         ...
         */
        private RunnableScheduledFuture!(T) peekExpired() {
        ...
        public int drainTo(Collection<? super Runnable> c) {
            ...
            lock.lock();
            try {
                RunnableScheduledFuture!(T) first;
                int n = 0;
        ...
        private class Itr : Iterator<Runnable> {
            final RunnableScheduledFuture!(T)[] array;
            int cursor;        // index of next element to return; initially 0
            int lastRet = -1;  // index of last element returned; -1 if no such

            Itr(RunnableScheduledFuture!(T)[] array) {
                this.array = array;
            }
