 ...
 * first used.
 *
 * <li>A property "&lt;logger&gt;.useParentHandlers". This defines a bool
...
public class LogManager {
    ...
    private volatile bool readPrimordialConfiguration;
    ...
    final void ensureLogManagerInitialized() {
        ...
        synchronized(this) {
            ...
            final bool isRecursiveInitialization = (initializedCalled == true);
    ...
    Logger demandLogger(String name, String resourceBundleName, Class!(T) caller) {
        ...
        if (result == null) {
            ...
            do {
                ...
                // addLogger() returns a bool instead of the Logger
    ...
    class LoggerContext {
        ...
        final bool requiresDefaultLoggers() {
            final bool requiresDefaultLoggers = (getOwner() == manager);
        ...
        bool addLocalLogger(Logger logger) {
            // no need to add default loggers if it's not required
            return addLocalLogger(logger, requiresDefaultLoggers());
        }

        bool addLocalLogger(Logger logger, LogManager manager) {
            // no need to add default loggers if it's not required
            return addLocalLogger(logger, requiresDefaultLoggers(), manager);
        }

        bool addLocalLogger(Logger logger, bool addDefaultLoggersIfNeeded) {
            return addLocalLogger(logger, addDefaultLoggersIfNeeded, manager);
        }

        // Add a logger to this context.  This method will only set its level
        // and process parent loggers.  It doesn't set its handlers.
        synchronized bool addLocalLogger(Logger logger, bool addDefaultLoggersIfNeeded,
                                            LogManager manager) {
    ...
    final class SystemLoggerContext : LoggerContext {
        ...
        AccessController.doPrivileged(new PrivilegedAction<Object>() {
            ...
            public Object run() {
                ...
                for (int i = 0; i < names.length; i++) {
                    ...
                    try {
                        // Android-changed:
                        Class!(T) clz = getClassInstance(word);
    ...
    //
    final class LoggerWeakRef : WeakReference<Logger> {
        private String                name;       // for namedLoggers cleanup
        private LogNode               node;       // for loggerRef cleanup
        private WeakReference<Logger> parentRef;  // for kids cleanup
        private bool disposed = false;         // avoid calling dispose twice
    ...
    // Package private method to get a bool property.
    // If the property is not defined or cannot be parsed
    // we return the given default value.
    ...
    // are first needed.
    private final class RootLogger : Logger {
        private RootLogger() {
    ...
    synchronized private void setLevelsOnExistingLoggers() {
        Enumeration!(T) enum_ = props.propertyNames();
        while (enum_.hasMoreElements()) {
    ...
    private static class Beans {
        private static final Class!(T) propertyChangeListenerClass =
            getClass("java.beans.PropertyChangeListener");

        private static final Class!(T) propertyChangeEventClass =
        ...
        private static final Constructor!(T) propertyEventCtor =
        ...
        private static Class!(T) getClass(String name) {
            try {
        ...
        private static Constructor!(T) getConstructor(Class!(T) c, Class!(T)... types) {
            try {
        ...
        private static Method getMethod(Class!(T) c, String name, Class!(T)... types) {
            try {
