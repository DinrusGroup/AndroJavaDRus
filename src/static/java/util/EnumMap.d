 ...
 * @since 1.5
 */
public class EnumMap<K : Enum<K>, V> : AbstractMap<K, V>
    : java.io.Serializable.Serializable, Cloneable
{
    /**
    ...
    private static final Enum!(T)[] ZERO_LENGTH_ENUM_ARRAY = new Enum!(T)[0];
    ...
    public bool containsKey(Object key) {
        return isValidKey(key) && vals[((Enum!(T))key).ordinal()] != null;
    }

    private bool containsMapping(Object key, Object value) {
        return isValidKey(key) &&
            maskNull(value).equals(vals[((Enum!(T))key).ordinal()]);
    }
    ...
    public V get(Object key) {
        return (isValidKey(key) ?
                unmaskNull(vals[((Enum!(T))key).ordinal()]) : null);
    }
    ...
    private class EntryIterator : EnumMapIterator<Map.Entry<K,V>> {
        ...
        private class Entry : Map.Entry<K,V> {
            private int index;
    ...
    private bool equals(EnumMap<?,?> em) {
        if (em.keyType != keyType)
     ...
     */
    private static <K : Enum<K>> K[] getKeyUniverse(Class<K> keyType) {
