...
public class SimpleTimeZone : TimeZone {
   ...
   /**
     ...
     */
    public bool inDaylightTime(Date date)
    {
    ...
    /**
     * A bool value which is true if and only if this zone uses daylight
    ...
    private byte[] packRules()
    {
        ...
        rules[0] = cast(byte)startDay;
        rules[1] = cast(byte)startDayOfWeek;
        rules[2] = cast(byte)endDay;
        rules[3] = cast(byte)endDayOfWeek;

        // As of serial version 2, include time modes
        rules[4] = cast(byte)startTimeMode;
        rules[5] = cast(byte)endTimeMode;
