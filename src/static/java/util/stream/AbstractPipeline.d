 ...
 * {@code AbstractPipeline}, a shape-specific pipeline class which : it
 * (e.g., {@code IntPipeline}) which is also abstract, and an operation-specific
 * concrete class which : that.  {@code AbstractPipeline} contains most of
 * the mechanics of evaluating the pipeline, and : methods that will be
 ...
 */
public abstract class AbstractPipeline<E_IN, E_OUT, S : BaseStream<E_OUT, S>>
        : PipelineHelper<E_OUT> : BaseStream<E_OUT, S> {
     ...
     */
    private Supplier<? : Spliterator!(T)> sourceSupplier;
     ...
     */
    AbstractPipeline(Supplier<? : Spliterator!(T)> source,
     ...
     */
    AbstractPipeline(Spliterator!(T) source,
                     int sourceFlags, bool parallel) {
        this.previousStage = null;
     ...
     */
    @SuppressWarnings("unchecked")
    private Spliterator!(T) sourceSpliterator(int terminalFlags) {
        ...
        if (sourceStage.sourceSpliterator != null) {
            ...
            spliterator = (Spliterator!(T)) sourceStage.sourceSupplier.get();
            sourceStage.sourceSupplier = null;
        }
        else {
            throw new IllegalStateException(MSG_CONSUMED);
    ...
    override
    final <P_IN, S : Sink<E_OUT>> S wrapAndCopyInto(S sink, Spliterator<P_IN> spliterator) {
     ...
     * @param supplier the supplier of a spliterator
     */
    public abstract Spliterator<E_OUT> lazySpliterator(Supplier<? : Spliterator<E_OUT>> supplier);
     ...
     */
    public abstract bool opIsStateful();
