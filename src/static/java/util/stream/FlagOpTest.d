...
@Test
public class FlagOpTest : OpTestCase {
    ...
    static class TestFlagPassThroughOp<T> : FlagDeclaringOp<T> {
    ...
    static class ParSerTestFlagExpectedOp<T> : FlagDeclaringOp<T> {
        ...
        override
        @SuppressWarnings({"unchecked", "rawtypes"})
        public Sink<T> opWrapSink(int flags, bool parallel, Sink upstream) {
            assertFlags(flags, parallel);
            return upstream;
        }

        protected void assertFlags(int flags, bool parallel) {
            if (parallel) {
                for (StreamOpFlag f : parKnown) {
