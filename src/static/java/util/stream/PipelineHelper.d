...
public abstract class PipelineHelper<P_OUT> {
     ...
     */
    abstract<P_IN, S : Sink<P_OUT>> S wrapAndCopyInto(S sink, Spliterator<P_IN> spliterator);
     ...
     * elements of the input type and : all the intermediate operations
     * described by this {@code PipelineHelper}, delivering the result into the
     * provided {@code Sink}.
     *
     * @param sink the {@code Sink} to receive the results
     * @return a {@code Sink} that : the pipeline stages and sends
