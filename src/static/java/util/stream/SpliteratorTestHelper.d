...
public class SpliteratorTestHelper {
    ...
    public interface ContentAsserter<T> {
        void assertContents(Collection<T> actual, Collection<T> expected, bool isOrdered);
    }
    ...
    public static void testIntSpliterator(Supplier<Spliterator.OfInt> supplier,
                                          ContentAsserter<Integer> asserter) {
        class BoxingAdapter : Consumer<Integer>, IntConsumer {
    ...
    public static void testLongSpliterator(Supplier<Spliterator.OfLong> supplier,
        ...
        class BoxingAdapter : Consumer<Long>, LongConsumer {
    ...
    public static void testDoubleSpliterator(Supplier<Spliterator.OfDouble> supplier,
        ...
        class BoxingAdapter : Consumer<Double>, DoubleConsumer {
    ...
    static <T, S : Spliterator<T>> void testSpliterator(Supplier<S> supplier,
    ...
    private static <T, S : Spliterator<T>> void testNullPointerException(Supplier<S> s) {
        S sp = s.get();
    ...
    private static <T, S : Spliterator<T>> void testForEach(
            Collection<T> exp,
            Supplier<S> supplier,
            UnaryOperator<Consumer<T>> boxingAdapter,
            ContentAsserter<T> asserter) {
        ...
        bool isOrdered = spliterator.hasCharacteristics(Spliterator.ORDERED);
    ...
    private static <T, S : Spliterator<T>> void testTryAdvance(
            Collection<T> exp,
            Supplier<S> supplier,
            UnaryOperator<Consumer<T>> boxingAdapter,
            ContentAsserter<T> asserter) {
    ...
    private static <T, S : Spliterator<T>> void testMixedTryAdvanceForEach(
            Collection<T> exp,
            Supplier<S> supplier,
            UnaryOperator<Consumer<T>> boxingAdapter,
            ContentAsserter<T> asserter) {
    ...
    private static <T, S : Spliterator<T>> void testMixedTraverseAndSplit(
            Collection<T> exp,
            Supplier<S> supplier,
            UnaryOperator<Consumer<T>> boxingAdapter,
            ContentAsserter<T> asserter) {
    ...
    private static <T, S : Spliterator<T>> void testSplitAfterFullTraversal(
            Supplier<S> supplier,
            UnaryOperator<Consumer<T>> boxingAdapter) {
    ...
    private static <T, S : Spliterator<T>> void testSplitOnce(
            Collection<T> exp,
            Supplier<S> supplier,
            UnaryOperator<Consumer<T>> boxingAdapter,
            ContentAsserter<T> asserter) {
    ...
    private static <T, S : Spliterator<T>> void testSplitSixDeep(
            Collection<T> exp,
            Supplier<S> supplier,
            UnaryOperator<Consumer<T>> boxingAdapter,
            ContentAsserter<T> asserter) {
    ...
    static void splitSixDeepVisitorUnsafe(int depth, int curLevel, List dest,
        ...
        int rootCharacteristics, bool useTryAdvance) {
    ...
    // Android-changed: Workaround for jack type inference bug
     private static <T>
    // private static <T, S : Spliterator<T>>
    // Android-changed: Workaround for jack type inference bug
    ...
    private static <T, S : Spliterator<T>> void testSplitUntilNull(
            Collection<T> exp,
            Supplier<S> supplier,
            UnaryOperator<Consumer<T>> boxingAdapter,
            ContentAsserter<T> asserter) {
        Spliterator<T> s = supplier.get();
        bool isOrdered = s.hasCharacteristics(Spliterator.ORDERED);
        assertSpliterator(s);
    ...
    private static void assertSpliterator(Spliterator!(T) s, int rootCharacteristics) {
        if ((rootCharacteristics & Spliterator.SUBSIZED) != 0) {
    ...
    private static void assertSpliterator(Spliterator!(T) s) {
    ...
    private static<T> void assertContents(Collection<T> actual, Collection<T> expected, bool isOrdered) {
        if (isOrdered) {
    ...
    private static void executeAndCatch(Class<? : Exception> expected, Runnable r) {
