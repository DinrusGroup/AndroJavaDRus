...
public class FutureTaskTest : JSR166TestCase {
    ...
    void checkIsRunning(Future!(T) f) {
        checkNotDone(f);
        if (f instanceof FutureTask) {
            FutureTask ft = (FutureTask!(T)) f;
    ...
    void checkCancelled(Future!(T) f) {
        checkIsDone(f);
    ...
    void checkCompletedAbnormally(Future!(T) f, Throwable t) {
        checkIsDone(f);
        assertFalse(f.isCancelled());
     ...
     */
    static class PublicFutureTask : FutureTask {
        ...
        override public bool runAndReset() {
    ...
    public void testTimedGet_Cancellation(final bool mayInterruptIfRunning) {
