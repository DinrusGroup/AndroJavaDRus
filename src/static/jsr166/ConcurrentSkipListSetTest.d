...
public class ConcurrentSkipListSetTest : JSR166TestCase {
    ...
    static class MyReverseComparator : Comparator {
    ...
    void bashSubSet(NavigableSet<Integer> set,
        ...
        bool lowIncl = rnd.nextBoolean();
        bool highIncl = rnd.nextBoolean();
        if (ascending) {
    ...
    void check(NavigableSet<Integer> set,
               final int min, final int max, final bool ascending,
               final BitSet bs) {
        class ReferenceSet {
    ...
    static bool eq(Integer i, int j) {
