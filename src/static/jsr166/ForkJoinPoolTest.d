...
public class ForkJoinPoolTest : JSR166TestCase {
    ...
    static class MyHandler : Thread.UncaughtExceptionHandler {
    ...
    // to test handlers
    static class FailingFJWSubclass : ForkJoinWorkerThread {
    ...
    static class SubFJP : ForkJoinPool { // to expose protected
        SubFJP() { super(1); }
        public int drainTasksTo(Collection<? super ForkJoinTask!(T)> c) {
            return super.drainTasksTo(c);
        }
        public ForkJoinTask!(T) pollSubmission() {
            return super.pollSubmission();
        }
    }

    static class ManagedLocker : ForkJoinPool.ManagedBlocker {
    ...
    static final class FibTask : RecursiveTask<Integer> {
        final int number;
    ...
    static final class FailingTask : ForkJoinTask<Void> {
        public final Void getRawResult() { return null; }
        protected final void setRawResult(Void mustBeNull) { }
        protected final bool exec() { throw new Error(); }
        FailingTask() {}
    }

    // Fib needlessly using locking to test ManagedBlockers
    static final class LockingFibTask : RecursiveTask<Integer> {
        final int number;
    ...
    public void testExecuteRunnable() throws Throwable {
        ...
        try (PoolCleaner cleaner = cleaner(e)) {
            ...
            Future!(T) future = e.submit(new CheckedRunnable() {
                public void realRun() {
                    done.set(true);
                }});
    ...
    public void testSubmitRunnable() throws Throwable {
        ExecutorService e = new ForkJoinPool(1);
        try (PoolCleaner cleaner = cleaner(e)) {
            Future!(T) future = e.submit(new NoOpRunnable());
            assertNull(future.get());
            assertTrue(future.isDone());
            assertFalse(future.isCancelled());
        }
    ...
    public void testExecuteNullRunnable() {
        ...
        try (PoolCleaner cleaner = cleaner(e)) {
            try {
                Future!(T) future = e.submit((Runnable) null);
                shouldThrow();
            } catch (NullPointerException success) {}
        }
    }
