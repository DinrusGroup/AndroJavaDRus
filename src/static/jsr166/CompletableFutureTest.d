...
public class CompletableFutureTest : JSR166TestCase {
    ...
    static class CFException : RuntimeException {}

    void checkIncomplete(CompletableFuture!(T) f) {
    ...
    void checkCompletedExceptionally(CompletableFuture!(T) f,
                                     bool wrapped,
    ...
    void checkCompletedWithWrappedCFException(CompletableFuture!(T) f) {
        checkCompletedExceptionally(f, true,
            (t) -> assertTrue(t instanceof CFException));
    }

    void checkCompletedWithWrappedCancellationException(CompletableFuture!(T) f) {
        checkCompletedExceptionally(f, true,
            (t) -> assertTrue(t instanceof CancellationException));
    }

    void checkCompletedWithTimeoutException(CompletableFuture!(T) f) {
        checkCompletedExceptionally(f, false,
            (t) -> assertTrue(t instanceof TimeoutException));
    }

    void checkCompletedWithWrappedException(CompletableFuture!(T) f,
                                            Throwable ex) {
        checkCompletedExceptionally(f, true, (t) -> assertSame(t, ex));
    }

    void checkCompletedExceptionally(CompletableFuture!(T) f, Throwable ex) {
        checkCompletedExceptionally(f, false, (t) -> assertSame(t, ex));
    }

    void checkCancelled(CompletableFuture!(T) f) {
        long startTime = System.nanoTime();
        try {
            f.get(LONG_DELAY_MS, MILLISECONDS);
            shouldThrow();
    ...
    public void testCancel() {
        for (bool mayInterruptIfRunning : new bool[] { true, false })
    {
        CompletableFuture<Integer> f = new CompletableFuture<>();
        checkIncomplete(f);
    ...
    abstract class CheckedIntegerAction : CheckedAction {
        Integer value;
    ...
    class IntegerSupplier : CheckedAction
        : Supplier<Integer>
    {
        final Integer value;
    ...
    class NoopConsumer : CheckedIntegerAction
        : Consumer<Integer>
    {
    ...
    class IncFunction : CheckedIntegerAction
    ...
    class SubtractAction : CheckedIntegerAction
    ...
    class SubtractFunction : CheckedIntegerAction
    ...
    class Noop : CheckedAction : Runnable {
    ...
    class FailingSupplier : CheckedAction
        : Supplier<Integer>
    {
    ...
    class FailingConsumer : CheckedIntegerAction
        : Consumer<Integer>
    {
    ...
    class FailingBiConsumer : CheckedIntegerAction
        : BiConsumer<Integer, Integer>
    {
    ...
    class FailingFunction : CheckedIntegerAction
        : Function<Integer, Integer>
    {
    ...
    class FailingBiFunction : CheckedIntegerAction
        : BiFunction<Integer, Integer, Integer>
    {
    ...
    class FailingRunnable : CheckedAction : Runnable {
    ...
    class CompletableFutureInc : CheckedIntegerAction
        : Function<Integer, CompletableFuture<Integer>>
    {
    ...
    class FailingCompletableFutureFunction : CheckedIntegerAction
        : Function<Integer, CompletableFuture<Integer>>
    {
    ...
    static final class ThreadExecutor : Executor {
        final AtomicInteger count = new AtomicInteger(0);
        static final ThreadGroup tg = new ThreadGroup("ThreadExecutor");
        static bool startedCurrentThread() {
    ...
    static final bool defaultExecutorIsCommonPool
        ...
        SYNC {
            ...
            public <T,U> CompletableFuture<U> thenCompose
                (CompletableFuture<T> f,
                 Function<? super T,? : CompletionStage<U>> a) {
                return f.thenCompose(a);
            }
            public <T,U> CompletableFuture<U> handle
                (CompletableFuture<T> f,
                 BiFunction<? super T,Throwable,? : U> a) {
                return f.handle(a);
            }
            ...
            public <T,U,V> CompletableFuture<V> thenCombine
                (CompletableFuture<T> f,
                 CompletionStage<? : U> g,
                 BiFunction<? super T,? super U,? : V> a) {
    ...
    public void testExceptionally_normalCompletion() {
        for (bool createIncomplete : new bool[] { true, false })
        for (Integer v1 : new Integer[] { 1, null })
    {
    ...
    public void testThenCombine_normalCompletion() {
        for (ExecutionMode m : ExecutionMode.values())
        for (bool fFirst : new bool[] { true, false })
        for (Integer v1 : new Integer[] { 1, null })
    ...
    public void testThenCombine_exceptionalCompletion() throws Throwable {
        for (ExecutionMode m : ExecutionMode.values())
        for (bool fFirst : new bool[] { true, false })
        for (bool failFirst : new bool[] { true, false })
        for (Integer v1 : new Integer[] { 1, null })
    {
        final CompletableFuture<Integer> f = new CompletableFuture<>();
        final CompletableFuture<Integer> g = new CompletableFuture<>();
    ...
    public void testNPE() {
        ...
        Runnable[] throwingActions = {
            ...
            () -> CompletableFuture.allOf((CompletableFuture!(T))null),
            () -> CompletableFuture.allOf((CompletableFuture!(T)[])null),
            () -> CompletableFuture.allOf(f, null),
            () -> CompletableFuture.allOf(null, f),

            () -> CompletableFuture.anyOf((CompletableFuture!(T))null),
            () -> CompletableFuture.anyOf((CompletableFuture!(T)[])null),
    ...
    Object resultOf(CompletableFuture!(T) f) {
        try {
        ...
        for (ExecutionMode m : ExecutionMode.values())
    {
        ...
        List<Function<CompletableFuture<Integer>, CompletableFuture!(T)>> funs
            = new ArrayList<>();
        ...
        funs.add((y) -> CompletableFuture.allOf(new CompletableFuture!(T)[] {y, v42}));
        funs.add((y) -> CompletableFuture.anyOf(new CompletableFuture!(T)[] {y, incomplete}));

        for (Function<CompletableFuture<Integer>, CompletableFuture!(T)>
                 fun : funs) {
            CompletableFuture<Integer> f = new CompletableFuture<>();
            f.completeExceptionally(ex);
            CompletableFuture<Integer> src = m.thenApply(f, new IncFunction(m));
            checkCompletedWithWrappedException(src, ex);
            CompletableFuture!(T) dep = fun.apply(src);
    ...
    public void testMinimalCompletionStage_minimality() {
        ...
        for (Method method : allMethods) {
            Class!(T)[] parameterTypes = method.getParameterTypes();
            Object[] args = new Object[parameterTypes.length];
            // Manufacture boxed primitives for primitive params
            for (int i = 0; i < args.length; i++) {
                Class!(T) type = parameterTypes[i];
                if (parameterTypes[i] == bool.class)
                    args[i] = false;
    ...
    static class Monad {
        static class ZeroException : RuntimeException {
        ...
        static void assertZero(CompletableFuture!(T) f) {
            try {
                f.getNow(null);
        ...
        static class PlusFuture<T> : CompletableFuture<T> {
            AtomicReference<Throwable> firstFailure = new AtomicReference<>(null);
        }

        /** Implements "monadic plus". */
        static <T> CompletableFuture<T> plus(CompletableFuture<? : T> f,
                                             CompletableFuture<? : T> g) {
...
//     static <U> bool isDone(CompletionStage<U> stage) {
//         CompletableFuture<U> f = new CompletableFuture<>();
//         stage.whenComplete((v, ex) -> {
//             if (ex != null) f.completeExceptionally(ex); else f.complete(v);
//         });
...
//     static <U> bool isDone2(CompletionStage<U> stage) {
