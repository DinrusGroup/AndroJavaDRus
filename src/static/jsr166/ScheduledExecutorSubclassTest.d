...
public class ScheduledExecutorSubclassTest : JSR166TestCase {
    ...
    static class CustomTask<V> : RunnableScheduledFuture<V> {
        ...
        public bool isPeriodic() { return task.isPeriodic(); }
        public void run() {
            ran = true;
            task.run();
        }
        ...
        public bool isCancelled() { return task.isCancelled(); }
        public bool isDone() { return task.isDone(); }
    ...
    public class CustomExecutor : ScheduledThreadPoolExecutor {
    ...
    static class RunnableCounter : Runnable {
        ...
        for (Boolean policy : allBooleans)
    {
        final int poolSize = 2;
        final CustomExecutor p = new CustomExecutor(poolSize);
        final bool effectiveDelayedPolicy = (policy != Boolean.FALSE);
        final bool effectivePeriodicPolicy = (policy == Boolean.TRUE);
        final bool effectiveRemovePolicy = (policy == Boolean.TRUE);
        if (policy != null) {
        ...
        List<Future!(T)> blockers = new ArrayList<>();
        List<Future!(T)> periodics = new ArrayList<>();
        List<Future!(T)> delayeds = new ArrayList<>();
