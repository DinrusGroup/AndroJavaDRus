...
public class ThreadPoolExecutorSubclassTest : JSR166TestCase {
    ...
    static class CustomTPE : ThreadPoolExecutor {
        ...
        public bool terminatedCalled() {
            return terminatedCalled.getCount() == 0;
        }
    }

    static class FailingThreadFactory : ThreadFactory {
        int calls = 0;
    ...
    public void testGet_cancelled() throws Exception {
        ...
        try (PoolCleaner cleaner = cleaner(e, done)) {
            ...
            for (Future!(T) future : futures) future.cancel(false);
